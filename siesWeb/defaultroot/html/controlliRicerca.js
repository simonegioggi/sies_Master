	...
	for (var ele=0;ele<document.forms[aForm].elements.length && OK;ele++)
	{
		...
		if (OK && (nome_campo.substring(0,6).toUpperCase()=="GIORNO"))
		{
			if (campo.value.length>0)
			{
				...
				nome_campo=campo.name;
				data_to_confirm=data_to_confirm+campo.value;
				OK=ControllaData(data_to_confirm);
				if (!OK) alert ('La  data ' + campo.title + ' non è una data valida');
			}
			Preso=true;
		}
		...
		if (!(Preso))
		{
			if (campo.value.length<2 && campo.value.length>0 && !((campo.type).toUpperCase()=='HIDDEN'))
			{
				alert ('Il campo ' + campo.title + ' DEVE contenere almeno 2 caratteri');
				OK=false;
			}
