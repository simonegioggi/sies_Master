 ...
 */
	ByteArrayOutputStream report = 
	  (ByteArrayOutputStream)request.getAttribute("report");
  ...
  TikaParser lTikaParser = 
      new TikaParser( report.toByteArray(), getServletContext(), null );
  
  try {
    lTikaParser.autoDetect(); // Esegue Parsing del file. 

    // [FT] - 05/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger()
    siesLogger.debug(">> [ DownloadDocument.jsp ] - lTikaParser.getMetadata()  : " + 
        lTikaParser.getMetadata() + " <<" );    
  ...
  //}catch( IOException ioex ){
    ...
    // Controllo errore di eccezione, per documenti RTF, precedenti alla 
    // modifica della sovrascrittura del BLOB in TableOracleDAO.update();
    // del 2009-09-01.    
    
  
  	//  TaggedIOException lIOEx = 
  	//    new TaggedIOException((IOException)ioex,lTikaParser);
