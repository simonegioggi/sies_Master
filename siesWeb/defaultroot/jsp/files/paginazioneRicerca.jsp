...
int NumeroTotalePagine=new BigDecimal(CountRisultati.doubleValue()/IWebConstants.RESULT_PER_PAGE).intValue();
...
<%
  ...
  double lNumLayer = Math.ceil((CountRisultati.doubleValue()/(PagesPerLayer*IWebConstants.RESULT_PER_PAGE)));
//  double MaxPage= Math.floor(CountRisultati.doubleValue()/IWebConstants.RESULT_PER_PAGE)+1;
  int MaxPage = CountRisultati.intValue()/IWebConstants.RESULT_PER_PAGE; // troncamento
  int resto = CountRisultati.intValue()%IWebConstants.RESULT_PER_PAGE;
...
<%
// Sostituisce allo spazio il caratte "%20" per evitare errori nella costruzione del link
