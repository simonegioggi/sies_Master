...
%>
<!-- LoadInserisciOrdineLiberazione -->
<html>
  <head>
    <title>[S.I.E.S.] -Gestione Misure sicurezza- Emissione Ordine di Liberazione -(applicazione M.S.)</title>
	...
	<script language="JavaScript">
    	...
    	// Funzione di impostazione della data Odierna.
    	...
    	{
   	      <% if (avvocati.size()==1) {%>
				document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
    	      <% } else { %>
    	      for (var i=0; i< <%=avvocati.size()%>; i++){ 
  				document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
    	      }
   	     <% } %>
    	}
    	...
    	function Difensore()
        {
            ...
            if( document.LoadInsOEInterna.Difesa.checked )
         	{ 
           		nodequan.style.display='block';
           		
       	        <% if (avvocati.size()==1) {%>
            		document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.disabled=false;
           			document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
               		document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.disabled=false;
               		document.LoadInsOEInterna.<%=ICostantiNotifica.CAMPO_NOTE%>.disabled=false;
          	    <% } else { %>
        	      	for (var i=0; i< <%=avvocati.size()%>; i++){ 
	            		document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].disabled=false;
	    				document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
	               		document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].disabled=false;
	               		document.LoadInsOEInterna.<%=ICostantiNotifica.CAMPO_NOTE%>[i].disabled=false;
					}
   	   	     <% } %>
       	  	}
            else
            {
            	nodequan.style.display='none';
       	        ...
       	        <% if (avvocati.size()==1) {%>
	        		document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value="";
	        		document.LoadInsOEInterna.<%=ICostantiNotifica.CAMPO_NOTE%>.value="";
        			document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
            		//document.LoadInsOEInterna.<%=ICostantiNotifica.CAMPO_NOTE%>.disabled=true;
          	    <% } else { %>
   	      			for (var i=0; i< <%=avvocati.size()%>; i++){ 
		        		document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value="";
		        		document.LoadInsOEInterna.<%=ICostantiNotifica.CAMPO_NOTE%>[i].value="";
	    				document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
	            		//document.LoadInsOEInterna.<%=ICostantiNotifica.CAMPO_NOTE%>[i].disabled=true;
					}
   	   	     <% } %>
            }
        }
    	...
    	function AutEsterna()
        {
            ...
            if( document.LoadInsOEInterna.SiNoTe.checked )
         	{ 
  		...
  		function Verify()
  		{
		  	...
		  	if (document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value.length==1)
			  	document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value='0'+document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value;
		  	if (document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value.length==1)
			  	document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value='0'+document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value;

		  	var data_to_verify = document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value+'/'+document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value+'/'+document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_ANNO_DATA_TRASMISSIONE_ATTI%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('Data Trasmissione NON Valida');
        		document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.focus();
			   	return false;
		  	}
		    ...
		    // CONTROLLO DATA EMISSIONE
		  	if (document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
			  	document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
		  	if (document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
			  	document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value;

		  	var data_to_verify = document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('Data di emissione non valida');
        		document.LoadInsOEInterna.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.focus();
			   	return false;
		  	}
				...
				if ( ((penaresidua.getFlagErgastolo() == null) || (penaresidua.getFlagErgastolo() != null && !penaresidua.getFlagErgastolo().equals("S") && !penaresidua.getFlagErgastolo().equals("D"))))
	 			{
		  			...
		  			{
		%>
					      if (document.LoadInsOEInterna.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value.length==1)
							  document.LoadInsOEInterna.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value='0'+document.LoadInsOEInterna.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value;
						  if (document.LoadInsOEInterna.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value.length==1)
							  document.LoadInsOEInterna.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value='0'+document.LoadInsOEInterna.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value;
				
						  var data_to_verifica = document.LoadInsOEInterna.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value+'/'+document.LoadInsOEInterna.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value+'/'+document.LoadInsOEInterna.<%=ICostantiPenaResidua.CAMPO_ANNO_DATA_FINE%>.value;
		
				      	  if (!ControllaData(data_to_verifica) )
						  {
			          			alert('Data fine pena non valida');
			          			document.LoadInsOEInterna.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.focus();
					 			return false;
				  		  }
		<%
					}
...
%>
				// CONTROLLO notifica all' AUTORITA 	
				if(document.LoadInsOEInterna.<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>.value == "" 
					&& document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.value == "-" )
				{
						alert('ATTENZIONE : Inserire almeno un Destinatario per Esecuzione');
	          			document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.focus();
			 			return false;
				}

				if(document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.value != "-" 
					&& document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.value == "" )
				{
						alert('ATTENZIONE : Inserire la descrizione SEDE Destinatario per Esecuzione ');
			       		document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.focus();
				 		return false;		
				}
				...
				{
	       	        <% if (avvocati.size()==1) {%>
						if(document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value == "-" 
						&& document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value == "" )
					{
						alert('Attenzione : Inserire Dati Destinatario notifica Difensore ');
	          			document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.focus();
			 			return false;		
					}

					if(document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1
						&& document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value == "" )
					{
						alert('Attenzione : Inserire la Sede Destinatario notifica Difensore ');
	          			document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.focus();
			 			return false;		
					}

					if(document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value != ""
						&& document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value == "-" )
					{
	       	   ...
	       	   <% } else { %>
		    	    for (var i=0; i< <%=avvocati.size()%>; i++) { 
						if(document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value == "-" 
							&& document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value == "" )
						{
							alert('Attenzione : Inserire Dati Destinatario notifica Difensore '+i);
		          			document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].focus();
				 			return false;		
						}
	
						if(document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1
						&& document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value == "" )
						{
							alert('Attenzione : Inserire la Sede Destinatario notifica Difensore ');
		          			document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].focus();
				 			return false;		
						}
	
						if(document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value != ""
							&& document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value == "-" )
						{
							alert('Attenzione : Inserire il tipo di  Destinatario notifica Difensore  ');
				          	document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].focus();
						 	return false;		
			...
			if( document.LoadInsOEInterna.SiNoTe.checked ) {
	   	        <% if (avvocati.size()==1) {%>
						document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value = "-";
		        		document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=0;
		        		document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value='C0';
	   	   <% } else { %>
		    	    for (var i=0; i< <%=avvocati.size()%>; i++) { 
						document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value = "-";
		        		document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=0;
		        		document.LoadInsOEInterna.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value='C0';
					}
			<%	} %>
			} else {
				if( document.LoadInsOEInterna.Difesa.checked ) {
			...
			if(document.LoadInsOEInterna.tipoUDS.value != "-")
			{
				if(document.LoadInsOEInterna.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.value == "")
				{
					alert(' Inserire Sede Destinatario Sorveglianza');
					document.LoadInsOEInterna.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.focus();
					return false;
				}	
			}
...
<body class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Emissione Ordine di Liberazione </font>
      </td>
    </tr>
  </table>
  <br>
  ...
  <br>
  <FORM method="POST" name="LoadInsOEInterna" action="<%= IWebConstants.PG_MAIN%>">
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.misurasicurezza.action.ActInserisciOrdineLiberazione">
    <table>
      <tr>
     	...
     	<tr>
		     ...
		     <td class="L" colspan="3">
		         ...
		         <input readonly title= "Nome Magistrato"    value="<%=StringUtils.toStringJSP(magistrato.getMagistrato().getNome() )%>" type="text" name="<%= ICostantiMagistrato.CAMPO_NOME %>"        maxlength="35" size="35">
		           <a href="Javascript:ListaMagistrati('LoadInsOEInterna');">
...
<!--Notifica per ESECUZIONE - ISTITUTO	-->    
 		<tr><td class="Titolo" colspan=6>Destinatari per Esecuzione</td></tr>
 		<tr>	
 			<td class="L">Istituto di Detenzione</td>
<%
		  if(lLuogoDetenzione == null || lLuogoDetenzione.getIstDetIdIstitutoDetenzione() == null || lLuogoDetenzione.getIstDetIdIstitutoDetenzione().equals("") || lLuogoDetenzione.getIstDetIdIstitutoDetenzione().equals("-"))
		  {
		%>
		    <td class="l" colspan="3">
		      <input readonly  Title="Istituto" name="Comune" value="" size=100>
		      <input type="hidden"  Title="Istituto" name="<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>" value="" size=30>
		      <a href="Javascript:ListaIstitutoDetenzione('LoadInsOEInterna','<%= ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE %>','Comune');">
		        <img src="/images/filefolder.gif" border=0>
		      </a>
		    </td>
		<%
		  ...
		  { // Precaricato con la struttura di detenzione attuale se presente
			  if(codposizione.equals("01") || codposizione.equals("03") ||		/* detenuto */
				 codposizione.equals("22") || codposizione.equals("24") ||		/* detenuto */
				 codposizione.equals("06") || codposizione.equals("09") )		/* internato */
			  	{
		%>
				    <td class="l" colspan="3">
				      <input readonly Title="Istituto" name="Comune" value="<%=StringUtils.toStringJSP(lLuogoDetenzione.getIstitutoDetenzione().getDescrTipoIstituto())%> di <%=StringUtils.toStringJSP(lLuogoDetenzione.getIstitutoDetenzione().getDescrizione())%> - <%=StringUtils.toStringJSP(lLuogoDetenzione.getIstitutoDetenzione().getIndirizzo())%>" size=100>
				      <input type="hidden"  Title="Istituto" name="<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>" value="<%=lLuogoDetenzione.getIstDetIdIstitutoDetenzione()%>" size=30>
				      <a href="Javascript:ListaIstitutoDetenzione('LoadInsOEInterna','<%= ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE %>','Comune');">
				        <img src="/images/filefolder.gif" border=0>
				      </a>
				    </td>
...
<!--Notifica per ESECUZIONE  - Altra Autorità -->
    ...
    <tr>
      ...
      <td class="L">
        ...
        <a href="Javascript:ListaComuni('LoadInsOEInterna','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E %>');">
          <img src="/images/filefolder.gif" border=0>
        </a>
      </td>
	</tr>
...
<!--Destinatario per l'Ente di Sorveglianza (Revisione del 24/10/2014)-->
  <tr>
    ...
    <td class="L">
      ...
      <a href="Javascript:ListaUfficiComuni('LoadInsOEInterna','<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>'
                                           ,document.LoadInsOEInterna.tipoUDS[document.LoadInsOEInterna.tipoUDS.options.selectedIndex].value);">
         <img src="/images/filefolder.gif" border=0>
      </a> 
    </td>
  </tr>   	
...
<%		}
		...
		{%>
			<td class="l" colspan=3 style="color:red">
				<input type="checkbox" name="Difesa" disabled >Notifiche Atti (Difensore - Condannato) Procedimento privo di Avvocato &nbsp;&nbsp;
			</td>
<%		} %>      		
      			
      		<td class="l" colspan=3>
      			<input type="checkbox" name="<%= ICostantiMisuraSicurezza.CAMPO_FLAG_ESECUZIONE_IMMEDIATA %>" value="D" >Disposta l'esecuzione Immediata &nbsp;&nbsp;
       		</td>
  		</tr>
    </table>
		...
		%>
				<table width="90%">
					...
					<%	}
						...
						{ 	%>
							<a href="Javascript:ListaComuni('LoadInsOEInterna','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>');">
								<img src="/images/filefolder.gif" border=0>
							</a>
					<%	} %>				
						</td>
...
<script language="JavaScript" type="text/javascript">
  var frmvalidator  = new Validator("LoadInsOEInterna");
