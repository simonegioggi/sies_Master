...
%>
<!--	 LoadModificaArchiviazionePerProvvSorveglianzaMS	 -->
<html>
<head>
...
<script language="JavaScript">
    	...
    	{
   	      <% if (avvocati.size()==1) {%>
				document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
    	      <% } else { %>
    	      for (var i=0; i< <%=avvocati.size()%>; i++){ 
  				document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
    	      }
   	     <% } %>
    	}
        ...
        function pulisciAutorita (nomeTipo,nomeComune,note)
        {
            ...
            if(note=='1')
            	document.ModificaArchSorvMS.<%=ICostantiNotifica.CAMPO_NOTE_E%>.value = "";
        ...
        function VediNotificaAvvocati()
        {
        	...
        	if(NotificaAvvocati == "-")
        	{
        		document.ModificaArchSorvMS.Difesa.checked = false;
        	}
        	else
        	{
        		document.ModificaArchSorvMS.Difesa.checked = true;
        		if(NotificaAvvocati == "C0")
        		{
        			document.ModificaArchSorvMS.SiNoTe.checked = true;
        		}
        		else
        		{
        			document.ModificaArchSorvMS.SiNoTe.checked = false;
        		}
        	...
        	if(lnote!="")
        	{
        		document.ModificaArchSorvMS.<%=ICostantiNotifica.CAMPO_NOTE_E%>.value=lnote;
        	}
    	...
    	function Difensore()
        {
            ...
            if( document.ModificaArchSorvMS.Difesa.checked )
         	{ 
           		nodequan.style.display='block';
           		
       	         if(numavvocati==1) 
       	         { 
            		document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.disabled=false;
           			document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
               		document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.disabled=false;
               		document.ModificaArchSorvMS.<%=ICostantiNotifica.CAMPO_NOTE%>.disabled=false;
          	     } 
       	         else
       	         { 
        	      	for (var i=0; i< numavvocati; i++)
        	      	{ 
	            		document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].disabled=false;
	    				document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
	               		document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].disabled=false;
	               		document.ModificaArchSorvMS.<%=ICostantiNotifica.CAMPO_NOTE%>[i].disabled=false;
					}
   	   	      	 } 
       	  	}
            else
            {
            	nodequan.style.display='none';
       	         ...
       	         if (numavvocati==1) 
       	         { 
	        		document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value="";
	        		document.ModificaArchSorvMS.<%=ICostantiNotifica.CAMPO_NOTE%>.value="";
        			document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
          	     }
       	         else 
       	         { 
   	      			for (var i=0; i< numavvocati; i++)
   	      			{ 
		        		document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value="";
		        		document.ModificaArchSorvMS.<%=ICostantiNotifica.CAMPO_NOTE%>[i].value="";
	    				document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
					}
   	   	      	} 
            }
    	...
    	function AutEsterna()
        {
            ...
            if( document.ModificaArchSorvMS.SiNoTe.checked )
         	{ 
...
// VERIFICHE ---    	
   ...
   // CONTROLLO DATA EMISSIONE
		  	if (document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
			  	document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
		  	if (document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
			  	document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value;

		  	var data_to_verify = document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('Data di emissione non valida');
        		document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.focus();
			   	return false;
		  	}
...
// CONTROLLO DATA RICEZIONE	
		  	if (document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_RICEZIONE_ATTI%>.value.length==1)
			  	document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_RICEZIONE_ATTI%>.value='0'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_RICEZIONE_ATTI%>.value;
		  	if (document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_RICEZIONE_ATTI%>.value.length==1)
			  	document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_RICEZIONE_ATTI%>.value='0'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_RICEZIONE_ATTI%>.value;

		  	var data_to_verify = document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_RICEZIONE_ATTI%>.value+'/'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_RICEZIONE_ATTI%>.value+'/'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_ANNO_DATA_RICEZIONE_ATTI%>.value;

		  	if (!ControllaData(data_to_verify) )
			{
        		alert('Data Ricezione NON Valida');
        		document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_RICEZIONE_ATTI%>.focus();
			   	return false;
		  	}
	        	
		    //1) Controllo : data di sistema deve essere >= Data Ricezione .
		    if( !CompareDate( data_to_verify, data_sistema) )
		    {
 ...
 // CONTROLLO DATA DEFINIZIONE (Data Archiviazione)
		  	if (document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value.length==1)
			  	document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value='0'+document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value;
		  	if (document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_MESE_DATA_DEFINIZIONE%>.value.length==1)
			  	document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_MESE_DATA_DEFINIZIONE%>.value='0'+document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_MESE_DATA_DEFINIZIONE%>.value;

		  	var data_to_verify = document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value+'/'+document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_MESE_DATA_DEFINIZIONE%>.value+'/'+document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_ANNO_DATA_DEFINIZIONE%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('Data Definizione NON Valida');
        		document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_GIORNO_DATA_DEFINIZIONE%>.focus();
			   	return false;
		  	}
 ...
 // CONTROLLO DATA TRASMISSIONE
		  	if (document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value.length==1)
			  	document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value='0'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value;
		  	if (document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value.length==1)
			  	document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value='0'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value;

		  	var data_to_verify = document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value+'/'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value+'/'+document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_ANNO_DATA_TRASMISSIONE_ATTI%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('Data Trasmissione NON Valida');
        		document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.focus();
			   	return false;
		  	}
	...
	// CONTROLLO AUTORITA' EMITTENTE  	
			if(document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_COD_UFFICIO_EMITTENTE %>.value == "" || 
			   document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_COD_UFFICIO_EMITTENTE %>.value == "-" )
			{
					alert('ATTENZIONE : Inserire Tipo Autorità Emittente');
          			document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_COD_UFFICIO_EMITTENTE%>.focus();
		 			return false;
			}
			
			if(document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_COD_LUOGO_EMITTENTE %>.value == "" || 
			   document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_COD_LUOGO_EMITTENTE %>.value == "-" )
			{
					alert('ATTENZIONE : Inserire Luogo Autorità Emittente');
          			document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_COD_LUOGO_EMITTENTE%>.focus();
		 			return false;
			}			

	// CONTROLLO TIPO PROVVEDIMENTO SORVEGLIANZA
			if(document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_COD_TIPO_PROVVEDIMENTO %>.value == "" || 
			   document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_COD_TIPO_PROVVEDIMENTO %>.value == "-" )
			{
				alert('ATTENZIONE : Inserire Tipo Provvedimento');
       			document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_COD_TIPO_PROVVEDIMENTO%>.focus();
	 			return false;
			}				

	// CONTROLLO ANNO e NUMERO PROVVEDIMENTO
			if( (document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO %>.value == "" || 
				 document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO %>.value.length == 0 )
				&& 
			    document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO %>.value != "" )
			{
				alert('ATTENZIONE : Inserire Anno Provvedimento');
       			document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO%>.focus();
	 			return false;
			}
		
			if( (document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO %>.value == "" || 
				 document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO %>.value.length == 0 )
				&& 
			    document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO %>.value != "" )
			{
				alert('ATTENZIONE : Inserire Numero Provvedimento');
       			document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO%>.focus();
	 			return false;
			}		

	// CONTROLLO ANNO e NUMERO FASCICOLO SIUS
			if( (document.ModificaArchSorvMS.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_ANNO %>.value == "" || 
				 document.ModificaArchSorvMS.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_ANNO %>.value.length == 0 )
				&& 
			    document.ModificaArchSorvMS.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_PROGR %>.value != "" )
			{
				alert('ATTENZIONE : Inserire Anno Fascicolo Sius');
       			document.ModificaArchSorvMS.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_ANNO%>.focus();
	 			return false;
			}
		
			if( (document.ModificaArchSorvMS.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_PROGR %>.value == "" || 
				 document.ModificaArchSorvMS.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_PROGR %>.value.length == 0 )
				&& 
			    document.ModificaArchSorvMS.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_ANNO %>.value != "" )
			{
				alert('ATTENZIONE : Inserire Numero Fascicolo Sius');
       			document.ModificaArchSorvMS.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_PROGR%>.focus();
	 			return false;
			}	
			
	// OGGETTO DEFINIZIONE
			if(document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_COD_RICH_MISURA_SIC%>.value == "-"  || 
				document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_COD_RICH_MISURA_SIC%>.value == "" )
			{
					alert('ERRORE : Inserire Oggetto Definizione');
					document.ModificaArchSorvMS.<%=ICostantiMisuraSicurezza.CAMPO_COD_RICH_MISURA_SIC%>.focus();
					return false;
			}
			
			
// AUTORITA' ESTERNA 
			if(document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.value != "-" 
				&& document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.value == "" )
			{
				alert('ERRORE : Inserire la descrizione SEDE ');
       			document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.focus();
	 			return false;		
			}
			...
			&& document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.value != "" )
			{
				alert('ERRORE : Inserire il Tipo Autorità');
       			document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.focus();
	 			return false;				
			}
						
	<%// CONTROLLO DESTINATARIO SORVEGLIANZA %>
			if(document.ModificaArchSorvMS.tipoUDS.value != "-")
			{
				if(document.ModificaArchSorvMS.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.value == "")
				{
					alert(' Inserire Sede Destinatario Sorveglianza');
					document.ModificaArchSorvMS.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.focus();
					return false;
				}	
			}
			...
			if(document.ModificaArchSorvMS.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.value != "")
			{
				if(document.ModificaArchSorvMS.tipoUDS.value == "-")
				{
					alert(' Inserire Tipo Ufficio Destinatario Sorveglianza');
					document.ModificaArchSorvMS.tipoUDS.focus();
					return false;
				}
			}	
				...
				if( document.ModificaArchSorvMS.Difesa.checked )
				{
					if( document.ModificaArchSorvMS.SiNoTe.checked )
					{
		       	     <% if(avvocati.size()==1) 
	       	        	{ %>
							document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value = "";
			        		document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=0;
			        		document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value='C0';
			        		document.ModificaArchSorvMS.<%=ICostantiNotifica.CAMPO_NOTE%>.value="";
	       	   <% 		}
	       	        	else
	       	        	{ %>
			    	    	...
			    	    	{ 
								document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value = "-";
				        		document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=0;
				        		document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value='C0';
				        		document.ModificaArchSorvMS.<%=ICostantiNotifica.CAMPO_NOTE%>[i].value="";
							}
					<%	} %>						
					}
					...
					{	
	       	        <%	if (avvocati.size()==1)
	       	        	{%>
							if(document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value != "" 
							&& document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value == "" )
							{
								alert('Attenzione : Inserire la Sede Destinatario notifica Difensore ');
			          			document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.focus();
					 			return false;		
							}
		       	   ...
		       	   <% 	}
	       	        	...
	       	        	{ %>
			    	    	for (var i=0; i< <%=avvocati.size()%>; i++) 
			    	    	{ 
								if(document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value != "" 
									&& document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value == "" )
								{
									alert('Attenzione : Inserire la Sede Destinatario notifica Difensore ');
				          			document.ModificaArchSorvMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].focus();
						 			return false;			
								}
							}
					<%	} %>
 ...
 <table>
	<tr>
	 ...
	 <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp; 
	 				<font class="campo">Modifica Archiviazione Misure di Sicurezza per provvedimento della Sorveglianza</font>
	 </td>
	</tr>
 </table>
	<br>
	...
	<br>
<FORM method="POST" name="ModificaArchSorvMS" action="<%=IWebConstants.PG_MAIN%>">
 <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.misurasicurezza.action.ActModificaArchiviazionePerProvvSorveglianzaMS">
...
<!-- 				Dati provvedimento della SORVEGLIANZA 			--> 
	...
	<tr>
      ...
      <td class="l" colspan="2">
        <a href="Javascript:ListaProvvSIUS('ModificaArchSorvMS');">
          provvedimento di Sorveglianza dalla lista <img src="/images/filefolder.gif" border=0>
        </a>
      </td>
    </tr>
    <tr>
  ...
  <tr>
	...
	<td class="L">
		<input value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(archiviazione.getDataRicezione(),"dd") )%>" 
			 type="text" size="2" maxlength="2" name="<%=ICostantiEvento.CAMPO_GIORNO_DATA_RICEZIONE_ATTI%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)" onBlur="javascript:value=FillDM(value)"> - 
		<input value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(archiviazione.getDataRicezione(),"MM") )%>"
			 type="text" size="2" maxlength="2" name="<%=ICostantiEvento.CAMPO_MESE_DATA_RICEZIONE_ATTI%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)" onBlur="javascript:value=FillDM(value)"> - 
		<input value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(archiviazione.getDataRicezione(),"yyyy") )%>"
			 type="text" size="4" maxlength="4" name="<%=ICostantiEvento.CAMPO_ANNO_DATA_RICEZIONE_ATTI%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)" onBlur="javascript:value=FillYear(value)">
	</td>
  </tr>
</table>
<table style="width: 95%;">	  
  <tr>
	...
	<td class="L" width="40%">
		...
		<input title="Sede Autorita Emittente" value="<%= StringUtils.toStringJSP(archiviazione.getDescrLuogoEmittente() )%>" type="text" 
											   name="<%=ICostantiEvento.CAMPO_COD_LUOGO_EMITTENTE%>" size="35" >
 		 <a href="Javascript:ListaUfficiComuni('ModificaArchSorvMS','<%=ICostantiEvento.CAMPO_COD_LUOGO_EMITTENTE %>',document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_COD_UFFICIO_EMITTENTE%>[document.ModificaArchSorvMS.<%=ICostantiEvento.CAMPO_COD_UFFICIO_EMITTENTE %>.options.selectedIndex].value);">
 		  <img src="/images/filefolder.gif" border=0>
 		 </a>
   		 <a href="Javascript:pulisciAutorita('<%=ICostantiEvento.CAMPO_COD_UFFICIO_EMITTENTE%>','<%=ICostantiEvento.CAMPO_COD_LUOGO_EMITTENTE%>','0');">
...
<table style="width: 95%;">    
  <tr>
	...
	<td class="l" width="25%" >Anno / Numero Fascicolo SIUS </td>
    <td class="L" >
      	<input Title="Anno FascicoloSius"   value="<%=StringUtils.toStringJSP(archiviazione.getChiaveAnno(),"")%>" 
       			name="<%=ICostantiFascicoloSius.CAMPO_CHIAVE_ANNO %>"  type="text" size="4" maxlength="4" onkeypress="return TicTabNumField(this,event)" onBlur="javascript:value=FillYear(value)"> /
       	<input Title="Numero FascicoloSius" value="<%=StringUtils.toStringJSP(archiviazione.getChiaveProgr(),"")%>" 
      			name="<%=ICostantiFascicoloSius.CAMPO_CHIAVE_PROGR %>" type="text" size="6" maxlength="6" onkeypress="return TicTabNumField(this,event)">
   	</td>         
  </tr>
</table>
<br>
  ...
  <tr>
	...
	<td class="L" colspan=3>
		...
		<input readonly title= "Nome Magistrato"    value="<%=StringUtils.toStringJSP(magistrato.getNome() )%>" type="text" name="<%= ICostantiMagistrato.CAMPO_NOME %>"        maxlength="35" size="35">
	     <a href="Javascript:ListaMagistrati('ModificaArchSorvMS');">
	       <img src="/images/filefolder.gif" border=0>
	     </a>
    ...
    <tr>
      ...
      <td class="l" colspan="3">
   		...
   		<input type="hidden"  Title="Istituto" name="<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>" value="<%= lIdIstDete %>" size=90>
   		 <a href="Javascript:ListaIstitutoDetenzione('ModificaArchSorvMS','<%= ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE %>','Comune');">
...
<%} %>
 ...
 <% if(lPrimaAutoritaEsterna!=null && lPrimaAutoritaEsterna.getIdAutoritaEsterna()!=null) 
	...
	<td class="L"><input title="Sede Autorita Esterna" value="<%= StringUtils.toStringJSP(lPrimaAutoritaEsterna.getDescrSede())%>" type="text" 
	 	...
	 	<a href="Javascript:ListaComuni('ModificaArchSorvMS','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>');"> 
	 	<img src="/images/filefolder.gif" border=0>
	 	</a>
 ...
 <%	} %>
  ...
  <tr>
    ...
    <td class="L">
      ...
      <a href="Javascript:ListaUfficiComuni('ModificaArchSorvMS','<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>'
	                                       ,document.ModificaArchSorvMS.tipoUDS[document.ModificaArchSorvMS.tipoUDS.options.selectedIndex].value);">
         <img src="/images/filefolder.gif" border=0></a>
	...
	{	
		while (lItxAvv.hasNext())
		{
			...
			<table width="90%">
		     ...
		     <%	if(avvocati.size()>1)
        		{	%>   				 
					<a href="Javascript:ListaComuni('ModificaArchSorvMS','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[<%=lIdxAvv%>]');">
					<img src="/images/filefolder.gif" border=0>
					</a>
			<%	}
		   		else
		   		{%>
		   			<a href="Javascript:ListaComuni('ModificaArchSorvMS','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>');">
					<img src="/images/filefolder.gif" border=0>
					</a>
	       	<%	} %>				
				</td>
		...
		for (int ind = 0;ind < eventonotifica.getNotifiche().length; ind++ )
		{
			...
			{	%>
				<table width="90%">
			       ...
			       <%	if(avvocati.size()>1)
	   					{	%>   				 
							<a href="Javascript:ListaComuni('ModificaArchSorvMS','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[<%=IndSede%>]');">
							<img src="/images/filefolder.gif" border=0>
							</a>
					<%	}
			       		else
