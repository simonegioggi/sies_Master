...
/*  siesLogger.debug("CC>"+strContenuto+"<" );
    // [FT] - 05/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger()
    siesLogger.debug(" >  F I N E  <" );
    // [FT] - 05/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger()
    siesLogger.debug("OG>"+strOggetto+"<" );
    // [FT] - 05/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger()
    siesLogger.debug(" >  F I N E  <" );
    // [FT] - 05/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger()
    siesLogger.debug("ES>"+strEsito+"<" );		*/
%>
...
//Funzione utile per impostare la data corrente.
 ...
 {    
  ...
  {
	  	//alert("caricatuttecombo() ");
	  	
	  	var nodeRegGen = document.getElementById('divRG');
	  	if( document.f.<%=ICostantiDecretoOrdinanzaSiep.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>.value == "CSS") // Corte Suprema Cassazione
	  	{
		...
		if( document.f.<%=ICostantiDecretoOrdinanzaSiep.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>.value == "CSS" ||		// Corte Suprema Cassazione
  			document.f.<%=ICostantiDecretoOrdinanzaSiep.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>.value == "RIE" ||		// Tribunale del Riesame
    ...
    caricaCombo(strEsito,';','#',document.f.<%=ICostantiDecretoOrdinanzaSiep.CAMPO_COD_OGGETTO_PROCEDIMENTO%>.value,document.f.<%= ICostantiDecretoOrdinanzaSiep.CAMPO_COD_ESITO%>);
  	...
  	function accertaEsito()
  	{
    	...
    	{	
    		...
    		//alert('Accerta - Codesito = '+Codesito+' - Descr = '+Descesito);
    		
    		if(Codesito=='0403' || Codesito=='0057' || Codesito=='0189')		// 0043 / 0189 - Sostituisce la Misura
    		{																	// 0057 - Trasforma la Misura
    		...
    		else if(Codesito=='0051' || Codesito=='0053')				// 0051 - Esecuzione della Misura
    		{															// 0053 - Proroga della Misura
    			node1=document.getElementById("ChangeMisura");
...
<table style="width: 95%;">	
  <tr>
    ...
    <td class="l">Anno/Numero Provvedimento </font></td>
    <td class="L">
...
<table style="width: 95%;">
  <tr>
    ...
    <td class="l">
      <select Title="Contenuto Decisione" name="<%=ICostantiDecretoOrdinanzaSiep.CAMPO_COD_OGGETTO_PROCEDIMENTO%>" onchange="javascript:caricatuttecomboContenuto()">
      </select>
    </td>
  </tr>
  <tr>
    ...
    <td class="l">
      <select Title="Oggetto Decisione"  name="<%=ICostantiDecretoOrdinanzaSiep.CAMPO_COD_OGGETTO_DECISIONE%>">
      </select>
    </td>
  </tr>
  <tr>
   <td class="l" nowrap>
     Esito Decisione<font class="ob">(*)</font>
   </td>
   <td class="l">
     <select Title="Tipologia Decisione" name="<%=ICostantiDecretoOrdinanzaSiep.CAMPO_COD_ESITO%>" onchange="javascript:accertaEsito()">
     </select>
   </td>
  </tr>
  <tr>
