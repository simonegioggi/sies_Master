...
%>
<!--  DettaglioComunicazionepolizia -->
<html>
  <head>
    <title>[S.I.E.S.] -Gestione Misure sicurezza- Comunicazione Forze di Polizia</title>
  ...
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Dettaglio Comunicazione per Esecuzione Misure di Sicurezza</font>
      </td>
<%
...
<%-- 				<jsp:param name="ActionLink" value="<%= "/jsp/Main.jsp?Action=siap.siep.misurasicurezza.action.ActStampaComunicazionePoliziaEsecMS&MisIdMisuraSicurezza="+MisidMisura+"&IdEvento="+eventonotifica.getEvento().getIdEvento()+"&CodMotivo="+eventonotifica.getEvento().getCodMotivo()+"&CodPosizioneGiuridica="+CodPos+"&CodTipo="+CodMisura%>"/> --%>
<%-- 			</jsp:include> --%>
			<%-- MEV_39: aggiunti pulsanti di validazione e cancellazione --%>
			<!-- BOTTONE DI STAMPA -->
			<td class="LBG">
       			<a  href="Javascript:stampaSiep('/jsp/Main.jsp?Action=siap.siep.misurasicurezza.action.ActStampaComunicazionePoliziaEsecMS&MisIdMisuraSicurezza=<%=MisidMisura%>&IdEvento=<%=eventonotifica.getEvento().getIdEvento()%>&CodMotivo=<%=eventonotifica.getEvento().getCodMotivo()%>&CodPosizioneGiuridica=<%=CodPos%>&CodTipo=<%=CodMisura%>')" onclick="javascript:lookUpload();">
			...
			<!-- BOTTONE DI VALIDAZIONE -->
     		<td class="LBG">
       			<a href="/jsp/Main.jsp?Action=siap.siep.misurasicurezza.action.ActUploadEsecuzioneMS&IdEvento=<%=eventonotifica.getEvento().getIdEvento()%>&<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>=siap.siep.misurasicurezza.action.ActDettaglioComunicazionePolizia&IdEvento=<%=eventonotifica.getEvento().getIdEvento()%>&<%=ICostantiEvento.CAMPO_VALIDA%>=S&noblob=S">
...
<table>
	<tr>
		<td class=C>Misura di Sicurezza da espiare</td>
	    <td class=L><font class="campo"><%=StringUtils.toStringJSP(MisuraModel.getDescrTipo())%>&nbsp;</font></td>
	    <td class=C> Anni</td>
	    <td class=L><font class="campo"><%=StringUtils.toStringJSP(MisuraModel.getNumAnni(),"0")%>&nbsp;</font></td>
	    <td class=C> Mesi</td>
	    <td class=L><font class="campo"><%=StringUtils.toStringJSP(MisuraModel.getNumMesi(),"0")%>&nbsp;</font></td>
	    <td class=C> Giorni</td>
	    <td class=L><font class="campo"><%=StringUtils.toStringJSP(MisuraModel.getNumGiorni(),"0")%>&nbsp;</font></td>	
	</tr>  		
</table>
...
<%		if(istitutodetenzione != null && istitutodetenzione.getDescrTipoIstituto() != null && 
		...
		{ 
				%>
		   <tr>
			    <td class="l" width="20%">Istituti di Detenzione</td>
			    <td class="L" colspan=2 width="60%">
      ...
      <table>
        ...
        <tr>
          <td class="L">
            ...
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>" value="siap.siep.misurasicurezza.action.ActDettaglioComunicazionePolizia">
          </td>
        </tr>
      </table>
    </FORM>
