...
%>
    		...
    		<tr>
    			...
    			<%if(provvSorv!=null && provvSorv.getDecreto()!= null && provvSorv.getDecreto().getDataScadenzaSospensioneSS() != null){ 
    				dataRinvioCalcolata = DateUtils.getDateToString(provvSorv.getDecreto().getDataScadenzaSospensioneSS(), "dd-MM-yyyy");
...
if (provvSorv!= null && provvSorv.getMisuraAlternativa() != null) {
	if (!"S".equals(provvSorv.getMisuraAlternativa().getFlagDecisioneTribunale())) { // flag decisione TDS
		if (provvSorv.getDecreto() != null
				&& ((provvSorv.getDecreto().getSospensioneAASS() != null && !provvSorv.getDecreto().getSospensioneAASS().equals(new BigDecimal(0)))
					|| (provvSorv.getDecreto().getSospensioneMMSS() != null && !provvSorv.getDecreto().getSospensioneMMSS().equals(new BigDecimal(0)))
					|| (provvSorv.getDecreto().getSospensioneGGSS() != null && !provvSorv.getDecreto().getSospensioneGGSS().equals(new BigDecimal(0))))) {
%>
			<tr>
    			<td class="l">Durata</td>
       			<td class="L">
      				<font class="campo">
      					Anni&nbsp;<%=StringUtils.toStringJSP(provvSorv.getDecreto().getSospensioneAASS())%>&nbsp;
      					Mesi&nbsp;<%=StringUtils.toStringJSP(provvSorv.getDecreto().getSospensioneMMSS())%>&nbsp;
      					Giorni&nbsp;<%=StringUtils.toStringJSP(provvSorv.getDecreto().getSospensioneGGSS())%>
      				</font>
        		</td>
    		</tr>
<%
...
%>
    		<tr>
    			<td class="l">Casa o Altro Luogo di Cura</td>
       			<td class="L">
      				<font class="campo">
      					<%=StringUtils.toStringJSP(provvSorv!=null && provvSorv.getDecreto() != null ? provvSorv.getDecreto().getLuogoSvolgimentoProva() : misuraAlternativa.getDescrLuogoProva(), "-")%>
      				</font>
        		</td>
    		</tr>
<%
