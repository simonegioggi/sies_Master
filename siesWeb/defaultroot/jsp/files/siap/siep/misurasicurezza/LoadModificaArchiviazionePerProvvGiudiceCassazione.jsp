...
%>
<!--	 LoadModificaArchiviazionePerProvvGiudiceCassazione	 -->
<html>
<head>
<title>[S.I.E.S.] -Gestione Misure sicurezza - Definizione Mis Sic per procedimento Giudice/Cassazione </title>
...
<script language="JavaScript">
    ...
    //	var lnote = '< %=lnote%>';
    	...
    	{
   	      <% if (avvocati.size()==1) {%>
				document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
    	      <% } else { %>
    	      for (var i=0; i< <%=avvocati.size()%>; i++){ 
  				document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
    	      }
   	     <% } %>
    	}
     ...
     //       if(note=='0')
     //       	document.ModArcGiuCas.< %=ICostantiArchiviazione.CAMPO_INDIRIZZO_EMITTENTE%>.value="";
     //      	else if(note=='1')
    	 
           	document.ModArcGiuCas.<%=ICostantiNotifica.CAMPO_NOTE_E%>.value = "";
        }
        ...
        function VediNotificaAvvocati()
        {
        	...
        	if(NotificaAvvocati == "-")
        	{
        		document.ModArcGiuCas.Difesa.checked = false;
        	}
        	else
        	{
        		document.ModArcGiuCas.Difesa.checked = true;
        		if(NotificaAvvocati == "C0")
        		{
        			document.ModArcGiuCas.SiNoTe.checked = true;
        		}
        		else
        		{
        			document.ModArcGiuCas.SiNoTe.checked = false;
        		}
        ...
        //	if(lnote!="")
        //	{
        //		document.ModArcGiuCas.< %=ICostantiNotifica.CAMPO_NOTE_E%>.value=lnote;
        //	}
    	...
    	function Difensore()
        {
            ...
            if( document.ModArcGiuCas.Difesa.checked )
         	{ 
           		nodequan.style.display='block';
           		
       	         if(numavvocati==1) 
       	         { 
            		document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.disabled=false;
           			document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
               		document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.disabled=false;
               		document.ModArcGiuCas.<%=ICostantiNotifica.CAMPO_NOTE%>.disabled=false;
          	     } 
       	         else
       	         { 
        	      	for (var i=0; i< numavvocati; i++)
        	      	{ 
	            		document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].disabled=false;
	    				document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
	               		document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].disabled=false;
	               		document.ModArcGiuCas.<%=ICostantiNotifica.CAMPO_NOTE%>[i].disabled=false;
					}
   	   	      	 } 
       	  	}
            else
            {
            	nodequan.style.display='none';
       	         ...
       	         if (numavvocati==1) 
       	         { 
	        		document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value="";
	        		document.ModArcGiuCas.<%=ICostantiNotifica.CAMPO_NOTE%>.value="";
        			document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
          	     }
       	         else 
       	         { 
   	      			for (var i=0; i< numavvocati; i++)
   	      			{ 
		        		document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value="";
		        		document.ModArcGiuCas.<%=ICostantiNotifica.CAMPO_NOTE%>[i].value="";
	    				document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
					}
   	   	      	} 
            }
    	...
    	function AutEsterna()
        {
            ...
            if( document.ModArcGiuCas.SiNoTe.checked )
         	{ 
    	...
    	// Caricamento POPUP	
        function ListaProvvGIUCAS(a_formname)
	  {
...
// VERIFICHE ---    	
   ...
   // CONTROLLO DATA EMISSIONE
		  	if (document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
			  	document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
		  	if (document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
			  	document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_EMISSIONE%>.value;

		  	var data_to_verify = document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_ANNO_DATA_EMISSIONE%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('ERRORE: Data Emissione non valida');
        		document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_EMISSIONE%>.focus();
			   	return false;
		  	}
 ...
 // CONTROLLO DATA RICEZIONE			    
	        if (document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_RICEZIONE%>.value.length==1)
	          	document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_RICEZIONE%>.value='0'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_RICEZIONE%>.value;
	        if (document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_RICEZIONE%>.value.length==1)
	          	document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_RICEZIONE%>.value='0'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_RICEZIONE%>.value;
		
	        var data_to_verify = document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_RICEZIONE%>.value+'/'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_RICEZIONE%>.value+'/'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_ANNO_DATA_RICEZIONE%>.value;
		
	        if (!ControllaData(data_to_verify) )
	        {
	          	alert('ERRORE: Data Ricezione non valida');
	          	document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_RICEZIONE%>.focus();
...
// CONTROLLO DATA DEFINIZIONE (Data Archiviazione)
		  	if (document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value.length==1)
			  	document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value='0'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value;
		  	if (document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value.length==1)
			  	document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value='0'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value;

		  	var data_to_verify = document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value+'/'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value+'/'+document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_ANNO_DATA_DEFINIZIONE%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('ERRORE: Data Definizione NON Valida');
        		document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_DEFINIZIONE%>.focus();
			   	return false;
		  	}
 ...
 // CONTROLLO DATA TRASMISSIONE
		  	if (document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value.length==1)
			  	document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value='0'+document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value;
		  	if (document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value.length==1)
			  	document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value='0'+document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value;

		  	var data_to_verify = document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value+'/'+document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value+'/'+document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_ANNO_DATA_TRASMISSIONE_ATTI%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('ERRORE: Data Trasmissione NON Valida');
        		document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.focus();
			   	return false;
		  	}
	...
	// CONTROLLO AUTORITA' EMITTENTE  	
			if(document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_AUTORITA_EMITTENTE %>.value == "" || 
			   document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_AUTORITA_EMITTENTE %>.value == "-" )
			{
					alert('ERRORE: Inserire Tipo Autorità Emittente');
          			document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>.focus();
		 			return false;
			}
			
			if(document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_COD_LUOGO_EMITTENTE %>.value == "" || 
			   document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_COD_LUOGO_EMITTENTE %>.value == "-" )
			{
					alert('ERRORE: Inserire Luogo Autorità Emittente');
          			document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_COD_LUOGO_EMITTENTE%>.focus();
		 			return false;
			}			

	// CONTROLLO TIPO PROVVEDIMENTO SORVEGLIANZA
			if(document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_PROVVEDIMENTO_ARC %>.value == "" || 
				document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_PROVVEDIMENTO_ARC %>.value == "-") 
			{
				alert("ERRORE: TIPO Provvedimento OBBLIGATORIO");
	        	document.ModArcGiuCas.<%= ICostantiArchiviazione.CAMPO_COD_TIPO_PROVVEDIMENTO_ARC %>.focus();
	        	return false;
			}	


	// CONTROLLO ANNO e NUMERO PROVVEDIMENTO
			if( (document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO %>.value == "" || 
				 document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO %>.value.length == 0 )
				&& 
			    document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO %>.value != "" )
			{
				alert('ERRORE: Inserire Anno Provvedimento');
       			document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO%>.focus();
	 			return false;
			}
		
			if( (document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO %>.value == "" || 
				 document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO %>.value.length == 0 )
				&& 
			    document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO %>.value != "" )
			{
				alert('ERRORE: Inserire Numero Provvedimento');
       			document.ModArcGiuCas.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO%>.focus();
	 			return false;
			}		

	// OGGETTO DEFINIZIONE
			if(document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_COD_OGGETTO_DEFINIZIONE%>.value == "-"  || 
				document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_COD_OGGETTO_DEFINIZIONE%>.value == "" )
			{
					alert('ERRORE:  Inserire Oggetto Definizione');
					document.ModArcGiuCas.<%=ICostantiArchiviazione.CAMPO_COD_OGGETTO_DEFINIZIONE%>.focus();
					return false;
			}
			
			
// AUTORITA' ESTERNA 
			if(document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.value != "-" 
				&& document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.value == "" )
			{
				alert('ERRORE: Inserire la descrizione SEDE ');
       			document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.focus();
	 			return false;		
			}
			...
			&& document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.value != "" )
			{
				alert('ERRORE: Inserire il Tipo Autorità');
       			document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.focus();
	 			return false;				
			}
						
	<%// CONTROLLO DESTINATARIO SORVEGLIANZA %>
			if(document.ModArcGiuCas.tipoUDS.value != "-")
			{
				if(document.ModArcGiuCas.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.value == "")
				{
					alert('ERRORE: Inserire Sede Destinatario Sorveglianza');
					document.ModArcGiuCas.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.focus();
					return false;
				}	
			}
			...
			if(document.ModArcGiuCas.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.value != "")
			{
				if(document.ModArcGiuCas.tipoUDS.value == "-")
				{
					...
					document.ModArcGiuCas.tipoUDS.focus();
					return false;
				}
			}
				...
				if( document.ModArcGiuCas.Difesa.checked )
				{
					if( document.ModArcGiuCas.SiNoTe.checked )
					{
		       	     <% if(avvocati.size()==1) 
	       	        	{ %>
							document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value = "";
			        		document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=0;
			        		document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value='C0';
			        		document.ModArcGiuCas.<%=ICostantiNotifica.CAMPO_NOTE%>.value="";
	       	   <% 		}
	       	        	else
	       	        	{ %>
			    	    	...
			    	    	{ 
								document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value = "-";
				        		document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=0;
				        		document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value='C0';
				        		document.ModArcGiuCas.<%=ICostantiNotifica.CAMPO_NOTE%>[i].value="";
							}
					<%	} %>						
					}
					...
					{	
	       	        <%	if (avvocati.size()==1)
	       	        	{%>
							if(document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value != "" 
							&& document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value == "" )
							{
								alert('Attenzione : Inserire la Sede Destinatario notifica Difensore ');
			          			document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.focus();
					 			return false;		
							}
		       	   ...
		       	   <% 	}
	       	        	...
	       	        	{ %>
			    	    	for (var i=0; i< <%=avvocati.size()%>; i++) 
			    	    	{ 
								if(document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value != "" 
									&& document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value == "" )
								{
									alert('Attenzione : Inserire la Sede Destinatario notifica Difensore ');
				          			document.ModArcGiuCas.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].focus();
						 			return false;			
								}
							}
					<%	} %>
 ...
 <table>
	<tr>
	 ...
	 <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp; 
	 				<font class="campo">Modifica Archiviazione Misure di Sicurezza per provvedimento del Giudice/Cassazione</font>
	 </td>
	</tr>
 </table>
	<br>
	...
	<br>
<FORM method="POST" name="ModArcGiuCas" action="<%=IWebConstants.PG_MAIN%>">
 <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.misurasicurezza.action.ActModificaArchiviazionePerProvvGiudiceCassazione">
...
<br>
 ...
 <table width="90%"> 
<!-- 				Dati provvedimento Del Giudice / Cassazione 			--> 
	<tr><td class="Titolo" colspan=6>Dati provvedimento del Giudice / Cassazione</td></tr>
	<tr>
      <td class="l" colspan="2">
        <a href="Javascript:ListaProvvGIUCAS('ModArcGiuCas');">
          provvedimento del Giudice/Cassazione da Lista  <img src="/images/filefolder.gif" border=0>
        </a>
      </td>
    </tr>
	<tr><td>&nbsp;</td></tr>
...
<table width="90%">	
  <tr>
	...
	<td class="L">
		<input readonly value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(archiviazione.getDataEmissione(),"dd"))%>" 
			type="text" size="2" maxlength="2" name="<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_EMISSIONE %>" > - 
		<input readonly value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(archiviazione.getDataEmissione(),"MM"))%>" 
			type="text" size="2" maxlength="2" name="<%=ICostantiArchiviazione.CAMPO_MESE_DATA_EMISSIONE %>" > - 
		<input readonly value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(archiviazione.getDataEmissione(),"yyyy"))%>" 
			 type="text" size="4" maxlength="4" name="<%=ICostantiArchiviazione.CAMPO_ANNO_DATA_EMISSIONE %>" > &nbsp;
	</td>
	<td class="l" width="20%">Data Ricezione <font class="ob">(*)</font></td>
	<td class="L">
		<input readonly value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(archiviazione.getDataRicezione(),"dd") )%>" 
			 type="text" size="2" maxlength="2" name="<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_RICEZIONE %>" > - 
		<input readonly value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(archiviazione.getDataRicezione(),"MM") )%>"
			 type="text" size="2" maxlength="2" name="<%=ICostantiArchiviazione.CAMPO_MESE_DATA_RICEZIONE %>" > - 
		<input readonly value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(archiviazione.getDataRicezione(),"yyyy") )%>"
			 type="text" size="4" maxlength="4" name="<%=ICostantiArchiviazione.CAMPO_ANNO_DATA_RICEZIONE %>" >
	</td>
  </tr>
<!-- 	Dati Autorità Emittente ( Giudice / Cassazione)  --> 
  <tr>
	...
	<td class="L" >
 		<input readonly type="text" Title="Autorita Emittente" name="<%=ICostantiArchiviazione.CAMPO_DESC_TIPO_AUTORITA_EMITTENTE%>" 
 						size="35" value="<%= StringUtils.toStringJSP(archiviazione.getDescrTipoAutoritaEmittente())%>">
 		<input type="hidden" title="Cod Aut Emi" name="<%=ICostantiArchiviazione.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>" value="<%= StringUtils.toStringJSP(archiviazione.getCodTipoAutoritaEmittente())%>" >				
	</td>
	<td class="l" width="10%" >Sede <font class="ob">(*)</font></td>
	<td class="L" width="35%">
		<%-- // [FT] - 05/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger() --%>
		<% //siesLogger.debug("--XX-- Trattino = "+archiviazione.getDescrLuogoEmittente()); %>
		<input readonly title="Sede Autorita Emittente" value="<%= StringUtils.toStringJSP(archiviazione.getDescrLuogoEmittente() )%>" type="text" 
  ...
  <tr>
    ...
    <td class="L" colspan=3 >
      <input readonly type="text" title="Tipo Provvedimento" name="<%= ICostantiArchiviazione.CAMPO_DESC_TIPO_PROVVEDIMENTO_ARC %>" 
      		value="<%= StringUtils.toStringJSP(archiviazione.getDescrTipoProvvedimentoArc())%>">
      <input type="hidden" title="Cod Tipo Provv" name="<%= ICostantiArchiviazione.CAMPO_COD_TIPO_PROVVEDIMENTO_ARC %>" value="<%= StringUtils.toStringJSP(archiviazione.getCodTipoProvvedimentoArc())%>" >		
    </td>    
  </tr>
  ...
  <tr>
  	...
  	<td class="L" colspan=3 >
      	<input readonly Title="Anno Provvedimento" value="<%=StringUtils.toStringJSP(archiviazione.getAnnoProvvedimento(),"")%>"
       			 name="<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO%>"  type="text" size="4" maxlength="4" > /
       	<input readonly Title="Numero Provvedimento" value="<%=StringUtils.toStringJSP(archiviazione.getNumProvvedimento(),"")%>" 
       			name="<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO%>" type="text" size="6" maxlength="6" >
   	</td>        
  </tr>
</table>
<br>
...
<table width="90%">
  <tr><td class="Titolo" colspan=6>Dati Definizione Modificabili</td></tr>
  <tr>
 ...
 <tr>
	...
	<td class="L" colspan=3>
		...
		<input readonly title= "Nome Magistrato"    value="<%=StringUtils.toStringJSP(magistrato.getNome() )%>" type="text" name="<%= ICostantiMagistrato.CAMPO_NOME %>"        maxlength="35" size="35">
	     <a href="Javascript:ListaMagistrati('ModArcGiuCas');">
    ...
    <tr>
      ...
      <td class="l" colspan="3">
   		...
   		<input type="hidden"  Title="Istituto" name="<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>" value="<%= lIdIstDete %>" size=90>
   		 <a href="Javascript:ListaIstitutoDetenzione('ModArcGiuCas','<%= ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE %>','Comune');">
...
<% if(lPrimaAutoritaEsterna!=null && lPrimaAutoritaEsterna.getIdAutoritaEsterna()!=null) 
	...
	<td class="L"><input title="Sede Autorita Esterna" value="<%= StringUtils.toStringJSP(lPrimaAutoritaEsterna.getDescrSede())%>" type="text" 
	 	...
	 	<a href="Javascript:ListaComuni('ModArcGiuCas','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>');"> 
	 	<img src="/images/filefolder.gif" border=0>
	 	</a>
 ...
 <tr>
  ...
  <tr>
    ...
    <td class="L">
      ...
      <a href="Javascript:ListaUfficiComuni('ModArcGiuCas','<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>'
	                                       ,document.ModArcGiuCas.tipoUDS[document.ModArcGiuCas.tipoUDS.options.selectedIndex].value);">
         <img src="/images/filefolder.gif" border=0></a>
	...
	{	
		while (lItxAvv.hasNext())
		{
			...
			<table width="90%">
		     ...
		     <%	if(avvocati.size()>1)
        		{	%>   				 
					<a href="Javascript:ListaComuni('ModArcGiuCas','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[<%=lIdxAvv%>]');">
					<img src="/images/filefolder.gif" border=0>
					</a>
			<%	}
		   		else
		   		{%>
		   			<a href="Javascript:ListaComuni('ModArcGiuCas','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>');">
					<img src="/images/filefolder.gif" border=0>
					</a>
	       	<%	} %>				
				</td>
		...
		for (int ind = 0;ind < eventonotifica.getNotifiche().length; ind++ )
		{
			...
			{	%>
				<table width="90%">
			       ...
			       <%	if(avvocati.size()>1)
	   					{	%>   				 
							<a href="Javascript:ListaComuni('ModArcGiuCas','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[<%=IndSede%>]');">
							<img src="/images/filefolder.gif" border=0>
							</a>
					<%	}
			       		else
