...
<script language="JavaScript">
    	...
    	{
   	      <% if (avvocati.size()==1) {%>
				document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
    	      <% } else { %>
    	      for (var i=0; i< <%=avvocati.size()%>; i++){ 
  				document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
    	      }
   	     <% } %>
    	}
    	...
    	function Difensore()
        {
            ...
            if( document.LoadInsOrdCons.Difesa.checked )
         	{ 
           		nodequan.style.display='block';
           		
       	        <% if (avvocati.size()==1) {%>
            		document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.disabled=false;
           			document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
               		document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.disabled=false;
               		document.LoadInsOrdCons.<%=ICostantiNotifica.CAMPO_NOTE%>.disabled=false;
          	    <% } else { %>
        	      	for (var i=0; i< <%=avvocati.size()%>; i++){ 
	            		document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].disabled=false;
	    				document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
	               		document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].disabled=false;
	               		document.LoadInsOrdCons.<%=ICostantiNotifica.CAMPO_NOTE%>[i].disabled=false;
					}
   	   	     <% } %>
       	  	}
            else
            {
            	nodequan.style.display='none';
       	        ...
       	        <% if (avvocati.size()==1) {%>
	        		document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value="";
	        		document.LoadInsOrdCons.<%=ICostantiNotifica.CAMPO_NOTE%>.value="";
        			document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
            		//document.LoadInsOrdCons.<%=ICostantiNotifica.CAMPO_NOTE%>.disabled=true;
          	    <% } else { %>
   	      			for (var i=0; i< <%=avvocati.size()%>; i++){ 
		        		document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value="";
		        		document.LoadInsOrdCons.<%=ICostantiNotifica.CAMPO_NOTE%>[i].value="";
	    				document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
	            		//document.LoadInsOrdCons.<%=ICostantiNotifica.CAMPO_NOTE%>[i].disabled=true;
					}
   	   	     <% } %>
            }
        }
    	...
    	function AutEsterna()
        {
            ...
            if( document.LoadInsOrdCons.SiNoTe.checked )
         	{ 
    	...
    	function Verify()
  		{
		  	...
		  	if (document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value.length==1)
			  	document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value='0'+document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value;
		  	if (document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value.length==1)
			  	document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value='0'+document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value;

		  	var data_to_verify = document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value+'/'+document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value+'/'+document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_ANNO_DATA_TRASMISSIONE_ATTI%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('Data Trasmissione NON Valida');
        		document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.focus();
			   	return false;
		  	}
		    ...
		    // CONTROLLO DATA EMISSIONE
		  	if (document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
			  	document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
		  	if (document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
			  	document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value;

		  	var data_to_verify = document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('Data di emissione non valida');
        		document.LoadInsOrdCons.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.focus();
			   	return false;
		  	}
					...
					&& !penaresidua.getFlagErgastolo().equals("S") && !penaresidua.getFlagErgastolo().equals("D")))) 
			{
				...
				{%>
					      if (document.LoadInsOrdCons.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value.length==1)
							  document.LoadInsOrdCons.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value='0'+document.LoadInsOrdCons.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value;
						  if (document.LoadInsOrdCons.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value.length==1)
							  document.LoadInsOrdCons.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value='0'+document.LoadInsOrdCons.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value;
				
						  var data_to_verifica = document.LoadInsOrdCons.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value+'/'+document.LoadInsOrdCons.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value+'/'+document.LoadInsOrdCons.<%=ICostantiPenaResidua.CAMPO_ANNO_DATA_FINE%>.value;
		
				      	  if (!ControllaData(data_to_verifica) )
						  {
			          			alert('Data fine pena non valida');
			          			document.LoadInsOrdCons.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.focus();
					 			return false;
				  		  }
		<%		}
			}	%>
				...
				if(document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>.value == "-" 
					&& document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.value == "-" )
				{
						alert('ERRORE : Inserire almeno un Destinatario');
	          			document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.focus();
			 			return false;
				}
				else
				{
						if(document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.value != "-" 
							&& document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.value == "" )
						{
								alert('ERRORE : Inserire la descrizione SEDE ');
			          			document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.focus();
					 			return false;		
						}
						else
						{	
								if(document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>.value != "-" 
									&& document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>.value == "" )
								{
										alert('ERRORE : Inserire la descrizione SEDE ');
					          			document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>.focus();
							 			return false;		
								}
						}	
				}
				...
				{
	       	        <% if (avvocati.size()==1) {%>
							if(document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value == "-" 
							&& document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value == "" )
							{
								alert('Attenzione : Inserire Dati Destinatario notifica Difensore ');
			          			document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.focus();
					 			return false;		
							}

							if(document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1
								&& document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value == "" )
							{
								alert('Attenzione : Inserire la Sede Destinatario notifica Difensore ');
			          			document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.focus();
					 			return false;		
							}

							if(document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value != ""
								&& document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value == "-" )
							{
		       	   ...
		       	   <% } else { %>
			    	    for (var i=0; i< <%=avvocati.size()%>; i++) { 
							if(document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value == "-" 
								&& document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value == "" )
							{
								alert('Attenzione : Inserire Dati Destinatario notifica Difensore '+i);
			          			document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].focus();
					 			return false;		
							}

							if(document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1
							&& document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value == "" )
							{
								alert('Attenzione : Inserire la Sede Destinatario notifica Difensore ');
			          			document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].focus();
					 			return false;		
							}

							if(document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value != ""
								&& document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value == "-" )
							{
								alert('Attenzione : Inserire il tipo di  Destinatario notifica Difensore  ');
					          	document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].focus();
							 	return false;		
				...
				if( document.LoadInsOrdCons.SiNoTe.checked ) {
	       	        <% if (avvocati.size()==1) {%>
							document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value = "-";
			        		document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=0;
			        		document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value='C0';
							var codAut=document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value;
							var indAut=document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex;
	       	   <% } else { %>
			    	    for (var i=0; i< <%=avvocati.size()%>; i++) { 
							document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value = "-";
			        		document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=0;
			        		document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value='C0';
							var codAut2=document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value;
							var indAut2=document.LoadInsOrdCons.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex;
						}
				<%	} %>
				}
				else
				...
				if(document.LoadInsOrdCons.tipoUDS.value != "-")
				{
					if(document.LoadInsOrdCons.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.value == "")
					{
						alert(' Inserire Sede Destinatario Sorveglianza');
						document.LoadInsOrdCons.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.focus();
						return false;
					}	
				}
...
<body class="corpo">
	<table>
		<tr>
			...
			<td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp; <font class="campo">Ordine di Consegna per esecuzione Misure Sicurezza</font></td>
		</tr>
	</table>
	<br>
	<jsp:include
	...
	<br>
	<FORM method="POST" name="LoadInsOrdCons" action="<%=IWebConstants.PG_MAIN%>">
		<input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.misurasicurezza.action.ActInserisciOrdinediConsegna">
		<table>
			<tr>
 ...
 %>		
		...
		<table>
			...
			<tr>
				...
				<td class="L" colspan="3">
					...
					<input readonly title="Nome Magistrato" value="<%=StringUtils.toStringJSP(magistrato.getMagistrato().getNome())%>" type="text" name="<%=ICostantiMagistrato.CAMPO_NOME%>" maxlength="35" size="35"> 
						<a	href="Javascript:ListaMagistrati('LoadInsOrdCons');"> <img src="/images/filefolder.gif" border=0> </a>
				</td>
			</tr>
			<tr>
				<td>
			...
			<tr>
				...
				<td class="L"><input title="Sede Autorita Esterna" value="" type="text" name="<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>" size="35"> 
					<a href="Javascript:ListaComuni('LoadInsOrdCons','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>');"> 
			...
			<tr>
				...
				<td class="L"><input title="Sede Autorita Esterna 1" value="" type="text" name="<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>" size="35"> 
					<a href="Javascript:ListaComuni('LoadInsOrdCons','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>');">
						<img src="/images/filefolder.gif" border=0>
					</a>
				</td>
			</tr>
		  ...
		  <tr>
		    ...
		    <td class="L">
		      ...
		      <a href="Javascript:ListaUfficiComuni('LoadInsOrdCons','<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>'
		                                           ,document.LoadInsOrdCons.tipoUDS[document.LoadInsOrdCons.tipoUDS.options.selectedIndex].value);">
		         <img src="/images/filefolder.gif" border=0>
		      </a> 
		    </td>
		  </tr> 
		    	...
		    	<td class="l" width="30%">Istituto di Detenzione</td> 
		     	<td class="l" colspan="2">
		      		...
		      		<input type="hidden"  Title="Istituto" name="<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>" value="<%= posizioneluogoaltra.getLuogoDetenzione().getIstDetIdIstitutoDetenzione()%>" size=90>
		      			<a href="Javascript:ListaIstitutoDetenzione('LoadInsOrdCons','<%= ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE %>','Comune');">
						...
						<tr>
							...
							<td class="L">
							 <%-- MEV_21 (avvocati) Sostituzione di getAvvocato().getForo() con getAvvocato().getDescComuneSedeForo() --%>
								<input title="Sede Autorita per Avvocato" value="<%=StringUtils.toStringJSP(lAvv.getAvvocato().getDescComuneSedeForo())%>" type="text" name="<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>" maxlength="35" size="35">
				       <%	if(avvocati.size()>1)
        					{	%>   				 
								<a href="Javascript:ListaComuni('LoadInsOrdCons','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[<%=lIdxAvv%>]');">
									<img src="/images/filefolder.gif" border=0>
								</a>
						<%	}
				       		else
				       		{%>
				       			<a href="Javascript:ListaComuni('LoadInsOrdCons','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>');">
										<img src="/images/filefolder.gif" border=0>
								</a>
				       	<%	} %>				
							</td>
  ...
  var frmvalidator  = new Validator("LoadInsOrdCons");
