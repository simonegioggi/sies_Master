...
<jsp:useBean id="emessoda" 				scope="request" class="java.lang.String"/>
...
<jsp:useBean id="autoritaDestGE" 		scope="request" class="java.lang.String" />
<jsp:useBean id="sedeautoritaDestGE"	scope="request" class="java.lang.String" />
...
// Gestione Autorità Esterna sulle Notifiche  
	...
	if(eventonotifica.getNotifiche() != null && eventonotifica.getNotifiche().length >0)
	{
		for (int i = 0; i < eventonotifica.getNotifiche().length; i++)
		{
			...
			if(eventonotifica.getNotifiche()[i] != null 
			   && ("NG").equals(eventonotifica.getNotifiche()[i].getCodTipoNotifica() ) )
			{
				if(eventonotifica.getNotifiche()[i].getNote()!= null)
				{	
					lnoteGE = eventonotifica.getNotifiche()[i].getNote();
				}	
			}
...
%>
<!--	 LoadModificaArchiviazionePerProvvGEsecuzioneMS	 -->
<html>
<head>
...
<script language="JavaScript">
    ...
    //	var lnote = '< %=lnote%>';
    	...
    	{
   	      <% if (avvocati.size()==1) {%>
				document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
    	      <% } else { %>
    	      for (var i=0; i< <%=avvocati.size()%>; i++){ 
  				document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
    	      }
   	     <% } %>
    	}
        ...
        function pulisciAutorita (nomeTipo,nomeComune,note)
        {
            ...
            if(note=='0')
            	document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_INDIRIZZO_EMITTENTE%>.value="";
           	else if(note=='1')
            	document.ModificaArchGEsecMS.<%=ICostantiNotifica.CAMPO_NOTE_E%>.value = "";
           	else if(note=='2')
           		document.ModificaArchGEsecMS.<%=ICostantiNotifica.CAMPO_NOTE_C%>.value = "";	
        ...
        function VediNotificaAvvocati()
        {
        	...
        	if(NotificaAvvocati == "-")
        	{
        		document.ModificaArchGEsecMS.Difesa.checked = false;
        	}
        	else
        	{
        		document.ModificaArchGEsecMS.Difesa.checked = true;
        		if(NotificaAvvocati == "C0")
        		{
        			document.ModificaArchGEsecMS.SiNoTe.checked = true;
        		}
        		else
        		{
        			document.ModificaArchGEsecMS.SiNoTe.checked = false;
        		}
        ...
        //	if(lnote!="")
        //	{
        //		document.ModificaArchGEsecMS.< %=ICostantiNotifica.CAMPO_NOTE_E%>.value=lnote;
        //	}
    	...
    	function Difensore()
        {
            ...
            if( document.ModificaArchGEsecMS.Difesa.checked )
         	{ 
           		nodequan.style.display='block';
           		
       	         if(numavvocati==1) 
       	         { 
            		document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.disabled=false;
           			document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
               		document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.disabled=false;
               		document.ModificaArchGEsecMS.<%=ICostantiNotifica.CAMPO_NOTE%>.disabled=false;
          	     } 
       	         else
       	         { 
        	      	for (var i=0; i< numavvocati; i++)
        	      	{ 
	            		document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].disabled=false;
	    				document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
	               		document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].disabled=false;
	               		document.ModificaArchGEsecMS.<%=ICostantiNotifica.CAMPO_NOTE%>[i].disabled=false;
					}
   	   	      	 } 
       	  	}
            else
            {
            	nodequan.style.display='none';
       	         ...
       	         if (numavvocati==1) 
       	         { 
	        		document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value="";
	        		document.ModificaArchGEsecMS.<%=ICostantiNotifica.CAMPO_NOTE%>.value="";
        			document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=1;
          	     }
       	         else 
       	         { 
   	      			for (var i=0; i< numavvocati; i++)
   	      			{ 
		        		document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value="";
		        		document.ModificaArchGEsecMS.<%=ICostantiNotifica.CAMPO_NOTE%>[i].value="";
	    				document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=1;
					}
   	   	      	} 
            }
    	...
    	function AutEsterna()
        {
            ...
            if( document.ModificaArchGEsecMS.SiNoTe.checked )
         	{ 
...
// VERIFICHE ---    	
   ...
   // CONTROLLO DATA EMISSIONE
		  	if (document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
			  	document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
		  	if (document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
			  	document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_EMISSIONE%>.value;

		  	var data_to_verify = document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_ANNO_DATA_EMISSIONE%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('ERRORE: Data Emissione non valida');
        		document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_EMISSIONE%>.focus();
			   	return false;
		  	}
 ...
 // CONTROLLO DATA RICEZIONE			    
	        if (document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_RICEZIONE%>.value.length==1)
	          	document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_RICEZIONE%>.value='0'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_RICEZIONE%>.value;
	        if (document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_RICEZIONE%>.value.length==1)
	          	document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_RICEZIONE%>.value='0'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_RICEZIONE%>.value;
		
	        var data_to_verify = document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_RICEZIONE%>.value+'/'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_RICEZIONE%>.value+'/'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_ANNO_DATA_RICEZIONE%>.value;
		
	        if (!ControllaData(data_to_verify) )
	        {
	          	alert('ERRORE: Data Ricezione non valida');
	          	document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_RICEZIONE%>.focus();
...
// CONTROLLO DATA DEFINIZIONE (Data Archiviazione)
		  	if (document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value.length==1)
			  	document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value='0'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value;
		  	if (document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value.length==1)
			  	document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value='0'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value;

		  	var data_to_verify = document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value+'/'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value+'/'+document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_ANNO_DATA_DEFINIZIONE%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('ERRORE: Data Definizione NON Valida');
        		document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_DEFINIZIONE%>.focus();
			   	return false;
		  	}
 ...
 // CONTROLLO DATA TRASMISSIONE
		  	if (document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value.length==1)
			  	document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value='0'+document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value;
		  	if (document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value.length==1)
			  	document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value='0'+document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value;

		  	var data_to_verify = document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.value+'/'+document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_MESE_DATA_TRASMISSIONE_ATTI%>.value+'/'+document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_ANNO_DATA_TRASMISSIONE_ATTI%>.value;

        	if (!ControllaData(data_to_verify) )
		  	{
        		alert('ERRORE: Data Trasmissione NON Valida');
        		document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_GIORNO_DATA_TRASMISSIONE_ATTI%>.focus();
			   	return false;
		  	}
	...
	// CONTROLLO AUTORITA' EMITTENTE  	
			if(document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_AUTORITA_EMITTENTE %>.value == "" || 
			   document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_AUTORITA_EMITTENTE %>.value == "-" )
			{
					alert('ERRORE: Inserire Tipo Autorità Emittente');
          			document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>.focus();
		 			return false;
			}
			
			if(document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_LUOGO_EMITTENTE %>.value == "" || 
			   document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_LUOGO_EMITTENTE %>.value == "-" )
			{
					alert('ERRORE: Inserire Luogo Autorità Emittente');
          			document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_LUOGO_EMITTENTE%>.focus();
		 			return false;
			}			

	// CONTROLLO TIPO PROVVEDIMENTO SORVEGLIANZA
			if(document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_PROVVEDIMENTO_ARC %>.value == "" || 
				document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_PROVVEDIMENTO_ARC %>.value == "-") 
			{
				alert("ERRORE: TIPO Provvedimento OBBLIGATORIO");
	        	document.ModificaArchGEsecMS.<%= ICostantiArchiviazione.CAMPO_COD_TIPO_PROVVEDIMENTO_ARC %>.focus();
	        	return false;
			}	


	// CONTROLLO ANNO e NUMERO PROVVEDIMENTO
			if( (document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO %>.value == "" || 
				 document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO %>.value.length == 0 )
				&& 
			    document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO %>.value != "" )
			{
				alert('ERRORE: Inserire Anno Provvedimento');
       			document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO%>.focus();
	 			return false;
			}
		
			if( (document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO %>.value == "" || 
				 document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO %>.value.length == 0 )
				&& 
			    document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO %>.value != "" )
			{
				alert('ERRORE: Inserire Numero Provvedimento');
       			document.ModificaArchGEsecMS.<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO%>.focus();
	 			return false;
			}		

	// OGGETTO DEFINIZIONE
			if(document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_OGGETTO_DEFINIZIONE%>.value == "-"  || 
				document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_OGGETTO_DEFINIZIONE%>.value == "" )
			{
					alert('ERRORE:  Inserire Oggetto Definizione');
					document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_OGGETTO_DEFINIZIONE%>.focus();
					return false;
			}
			
			
// AUTORITA' ESTERNA 
			if(document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.value != "-" 
				&& document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.value == "" )
			{
				alert('ERRORE: Inserire la descrizione SEDE ');
       			document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.focus();
	 			return false;		
			}
			...
			&& document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>.value != "" )
			{
				alert('ERRORE: Inserire il Tipo Autorità');
       			document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_E%>.focus();
	 			return false;				
			}
						
	<%// CONTROLLO DESTINATARIO SORVEGLIANZA %>
			if(document.ModificaArchGEsecMS.tipoUDS.value != "-")
			{
				if(document.ModificaArchGEsecMS.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.value == "")
				{
					alert('ERRORE: Inserire Sede Destinatario Sorveglianza');
					document.ModificaArchGEsecMS.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.focus();
					return false;
				}	
			}
			...
			if(document.ModificaArchGEsecMS.<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>.value != "")
			{
				if(document.ModificaArchGEsecMS.tipoUDS.value == "-")
				{
					...
					document.ModificaArchGEsecMS.tipoUDS.focus();
					return false;
				}
			}
			
// Controllo su AUTORITA ESECUZIONE (destinatari)	
			if(document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>.value != "-" && 
				document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>.value == "")
			{
				alert('ERRORE: Inserire Sede Destinatario Giudice Esecuzione');
				document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>.focus();
				return false;

			}
		
			if(document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>.value != ""  &&
				document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>.value == "-" )
			{
					alert('ERRORE: Inserire Tipo Autorità Destinatario Giudice Esecuzione');
					document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>.focus();
					return false;
			}			
				...
				if( document.ModificaArchGEsecMS.Difesa.checked )
				{
					if( document.ModificaArchGEsecMS.SiNoTe.checked )
					{
		       	     <% if(avvocati.size()==1) 
	       	        	{ %>
							document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value = "";
			        		document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.selectedIndex=0;
			        		document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value='C0';
			        		document.ModificaArchGEsecMS.<%=ICostantiNotifica.CAMPO_NOTE%>.value="";
	       	   <% 		}
	       	        	else
	       	        	{ %>
			    	    	...
			    	    	{ 
								document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value = "-";
				        		document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].selectedIndex=0;
				        		document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value='C0';
				        		document.ModificaArchGEsecMS.<%=ICostantiNotifica.CAMPO_NOTE%>[i].value="";
							}
					<%	} %>						
					}
					...
					{	
	       	        <%	if (avvocati.size()==1)
	       	        	{%>
							if(document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value != "" 
							&& document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value == "" )
							{
								alert('Attenzione : Inserire la Sede Destinatario notifica Difensore ');
			          			document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.focus();
					 			return false;		
							}
		       	   ...
		       	   <% 	}
	       	        	...
	       	        	{ %>
			    	    	for (var i=0; i< <%=avvocati.size()%>; i++) 
			    	    	{ 
								if(document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>[i].value != "" 
									&& document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].value == "" )
								{
									alert('Attenzione : Inserire la Sede Destinatario notifica Difensore ');
				          			document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[i].focus();
						 			return false;			
								}
							}
					<%	} %>
 ...
 <table>
	<tr>
	 ...
	 <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp; 
	 				<font class="campo">Modifica Archiviazione Misure di Sicurezza per provvedimento del Giudice di Esecuzione</font>
	 </td>
	</tr>
 </table>
	<br>
	...
	<br>
<FORM method="POST" name="ModificaArchGEsecMS" action="<%=IWebConstants.PG_MAIN%>">
 <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.misurasicurezza.action.ActModificaArchiviazionePerProvvGEsecuzioneMS">
...
<br>
 ...
 <table width="80%"> 
<!-- 				Dati provvedimento Del Giudice dell'Esecuzione 			--> 
	<tr><td class="Titolo" colspan=6>Dati provvedimento del Giudice Di esecuzione</td></tr>
	<tr>
      ...
      <td class="l" colspan="2">
        <a href="Javascript:ListaProvvGE('ModificaArchGEsecMS');">
          provvedimento del Giudice dell'Esecuzione dalla lista <img src="/images/filefolder.gif" border=0>
        </a>
      </td>
    </tr>
    <tr>
      <td class="l" width="25%">Provvedimento emesso da :&nbsp;<font class=ob>(*)</font></td>
      <td class="L" colspan="3">
        <select  Title="ufficio GE" name="<%=ICostantiArchiviazione.CAMPO_COD_PROVVEDIMENTO %>">
         <%=emessoda%>
         </select>
      </td>
    </tr>
...
<table style="width: 95%;">	  
  <tr>
	<td class="L" width="12%">Autorità Emittente <font class="ob">(*)</font></td>
	<td class="L" width="15%">
 		<select Title="Autorita Emittente" name="<%=ICostantiArchiviazione.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>">
  		<%=autorita%>
 		</select>
	</td>
	<td class="l" width="15%" style="text-align:center" >Sede <font class="ob">(*)</font></td>
	<td class="L" width="40%">
		...
		<input title="Sede Autorita Emittente" value="<%= StringUtils.toStringJSP(archiviazione.getDescrLuogoEmittente() )%>" type="text" 
 		 ...
 		 <a href="Javascript:ListaUfficiComuni('ModificaArchGEsecMS','<%=ICostantiArchiviazione.CAMPO_COD_LUOGO_EMITTENTE %>',document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>[document.ModificaArchGEsecMS.<%=ICostantiArchiviazione.CAMPO_COD_TIPO_AUTORITA_EMITTENTE %>.options.selectedIndex].value);">
  ...
  <tr>    
    <td class="l" width="15%">Indirizzo</td>
    <td class="L" width="25%">
  ...
  <tr>
   	<td class="l" width="25%">Tipo Provvedimento<font class="ob">(*)</font></td>
  ...
  <tr>
  	...
  	<td class="L" colspan=2 width="20%">
      	<input Title="Anno Provvedimento" value="<%=StringUtils.toStringJSP(archiviazione.getAnnoProvvedimento(),"")%>"
       			 name="<%=ICostantiEvento.CAMPO_ANNO_PROTOCOLLO%>"  type="text" size="4" maxlength="4" onkeypress="return TicTabNumField(this,event)" onBlur="javascript:value=FillYear(value)"> /
       	<input Title="Numero Provvedimento" value="<%=StringUtils.toStringJSP(archiviazione.getNumProvvedimento(),"")%>" 
       			name="<%=ICostantiEvento.CAMPO_PROGR_PROTOCOLLO%>" type="text" size="6" maxlength="6" onkeypress="return TicTabNumField(this,event)">
   	</td>        
  </tr>
</table>
<br>
 ...
 <tr>
	...
	<td class="L" colspan=3>
		...
		<input readonly title= "Nome Magistrato"    value="<%=StringUtils.toStringJSP(magistrato.getNome() )%>" type="text" name="<%= ICostantiMagistrato.CAMPO_NOME %>"        maxlength="35" size="35">
	     <a href="Javascript:ListaMagistrati('ModificaArchGEsecMS');">
	       <img src="/images/filefolder.gif" border=0>
	     </a>
    ...
    <tr>
      ...
      <td class="l" colspan="3">
   		...
   		<input type="hidden"  Title="Istituto" name="<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>" value="<%= lIdIstDete %>" size=90>
   		 <a href="Javascript:ListaIstitutoDetenzione('ModificaArchGEsecMS','<%= ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE %>','Comune');">
...
<% if(lPrimaAutoritaEsterna!=null && lPrimaAutoritaEsterna.getIdAutoritaEsterna()!=null) 
	...
	<td class="L"><input title="Sede Autorita Esterna" value="<%= StringUtils.toStringJSP(lPrimaAutoritaEsterna.getDescrSede())%>" type="text" 
	 	...
	 	<a href="Javascript:ListaComuni('ModificaArchGEsecMS','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_E%>');"> 
	 	<img src="/images/filefolder.gif" border=0>
	 	</a>
...
<!-- 	Destinatario per ufficio Esecuzione -->
  <tr>
    <td class="l" width="15%">Giudice Esecuzione</td>
	<td class="L" width="25%">
        <select id="AutoritaCdest" Title="AutoritaCdest" name="<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>">
          <%=autoritaDestGE%>
         </select>
    </td>
<%
if(sedeautoritaDestGE.compareTo("")==0)
{	%>    
    <td class="l" width="10%">Sede </td>
    <td class="L" width="25%">
          <input title="Sede AutoritaCdest"  type="text" name="<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>"  maxlength="35" size="35">
          <a href="Javascript:ListaUfficiComuni('ModificaArchGEsecMS','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>',document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>[document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>.selectedIndex].value);">
          <img src="/images/filefolder.gif" border=0>
        </a>
        <a href="Javascript:pulisciAutorita('<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>',
   	 									'<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>','2');">
	  &nbsp;<img src="/images/delete.gif" border=0></a>
    </td>
<%
}
...
{	%> 
    <td class="l" width="10%">Sede </td>
    <td class="L" width="25%">
          <input title="Sede AutoritaCdest"  type="text" name="<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>"  
          		 value="<%=StringUtils.toStringJSP(sedeautoritaDestGE,"") %>" 	maxlength="35" size="35">
          <a href="Javascript:ListaUfficiComuni('ModificaArchGEsecMS','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_C%>',document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>[document.ModificaArchGEsecMS.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>.selectedIndex].value);">
          <img src="/images/filefolder.gif" border=0>
        </a>
        <a href="Javascript:pulisciAutorita('<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA_C%>',
...
%>           
  ...
  <tr>    
    <td class="l" width="15%">Indirizzo</td>
    <td class="L" width="25%">
      <TEXTAREA title="IndirizzoCdest" name="<%=ICostantiAutoritaEsterna.CAMPO_NOTE_C%>" cols=50 ><%=StringUtils.toStringJSP(lnoteGE)%></textarea>
    </td>
  </tr>  
  ...
  <tr>
    ...
    <td class="L">
      ...
      <a href="Javascript:ListaUfficiComuni('ModificaArchGEsecMS','<%=ICostantiUfficio.CAMPO_SEDE_UFFICIO%>'
	                                       ,document.ModificaArchGEsecMS.tipoUDS[document.ModificaArchGEsecMS.tipoUDS.options.selectedIndex].value);">
         <img src="/images/filefolder.gif" border=0></a>
	...
	{	
		while (lItxAvv.hasNext())
		{
			...
			<table width="90%">
		     ...
		     <%	if(avvocati.size()>1)
        		{	%>   				 
					<a href="Javascript:ListaComuni('ModificaArchGEsecMS','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[<%=lIdxAvv%>]');">
					<img src="/images/filefolder.gif" border=0>
					</a>
			<%	}
		   		else
		   		{%>
		   			<a href="Javascript:ListaComuni('ModificaArchGEsecMS','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>');">
					<img src="/images/filefolder.gif" border=0>
					</a>
	       	<%	} %>				
				</td>
		...
		for (int ind = 0;ind < eventonotifica.getNotifiche().length; ind++ )
		{
			...
			{	%>
				<table width="90%">
			       ...
			       <%	if(avvocati.size()>1)
	   					{	%>   				 
							<a href="Javascript:ListaComuni('ModificaArchGEsecMS','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>[<%=IndSede%>]');">
							<img src="/images/filefolder.gif" border=0>
							</a>
					<%	}
			       		else
