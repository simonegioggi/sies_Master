...
<%      if ((fascicolo.getSoggetto().getDataNascita())==null || fascicolo.getSoggetto().getDataNascita().equals(""))
      ...
      <td class=c>            
	        <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.fascicolo.action.ActRicercaFascicoloPerSoggettoCodCui&<%=ICostantiSoggetto.CAMPO_COD_AFIS%>=<%=fascicolo.getSoggetto().getCodAfis()%>&<%="tipoRicerche"%>=<%=tipoRicerche%>" title="Soggetto">
	          <%=StringUtils.toStringJSP(fascicolo.getSoggetto().getCodAfis())%>&nbsp;
	        </a>      	
	  </td>
