...
%>
            ...
            <a class="cliccabile" href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.fascicolo.action.ActLoadDettaglioFascicolo&<%=ICostantiFascicoloSiep.CAMPO_ID_FASCICOLO_SIEP%>=<%=FascMSMod.getFasSieIdFascicoloCollegato()%>" title="Procedimento">
              ...
              <%=StringUtils.toStringJSP(FascMSMod.getChiaveProgrSiepCollegato() )%></a>
            <%} else {%>
    ...
    <td class="L">
      ...
      if (   fascicolo.getFlagAltraCausa() != null && fascicolo.getFlagAltraCausa().equals("S")
          && (   dettagliofascicolo.getPosizioneGiuridica().getCodPosizioneGiuridica().equals("10") // libero
              ...
              || elencoPosGiurAltraCausa.contains(dettagliofascicolo.getPosizioneGiuridica().getCodPosizioneGiuridica()) // libero - altra causa
             )
         )
      {
      %>
...
<% if (isAvvocatoForoSoppresso){%>
  ...
  if (document.addEventListener) document.addEventListener("DOMContentLoaded", blinkForo, false);
  else if (window.addEventListener) window.addEventListener("load", blinkForo, false);
  else if (window.attachEvent) window.attachEvent("onload", blinkForo);
  else window.onload = blinkForo;
...
{ 
	...
	{%> 
	 <tr>      
	   <td class="L" colspan=1>
		  ...
		  <% if ( richiestaconversione.getImportoMulta()!=null && richiestaconversione.getImportoMulta().compareTo(new BigDecimal(0))!=0){%> 
		      <font class="label">Multa </font>
		      <font class="campo"><%=StringUtils.toEuroFormat(richiestaconversione.getImportoMulta())%></font>&nbsp;&euro;&nbsp;&nbsp;&nbsp;&nbsp;
		  <%}
		    if ( richiestaconversione.getImportoAmmenda()!=null && richiestaconversione.getImportoAmmenda().compareTo(new BigDecimal(0))!=0){%> 
		      <font class="label">Ammenda </font>
		      <font class="campo"><%=StringUtils.toEuroFormat(richiestaconversione.getImportoAmmenda())%></font>&nbsp;&euro;&nbsp;&nbsp;&nbsp;&nbsp;
		<%}%>
	...
	if ((richiestaconversione.getCodTipoSanzione()!=null) &&
		...
		(richiestaconversione.getCodTipoSanzione().compareTo("02")==0 )	)
		// 17/02/2016 Fine
	{
...
%>
    <tr>
      <td class="L">
        ...
        <% } %>
        <% if(ggConcLS != 0 || ggConcLI != 0) {%><font class="label">&nbsp;di cui</font><%}%>
      </td>
    </tr>
<%
  }
...
{ 
  if(dettagliofascicolo.getGiorniLibConcessaLA() != null && dettagliofascicolo.getGiorniLibConcessaLA().compareTo(new BigDecimal(0)) !=0)
  {
  %>
      <tr>
        <td class="L">
...
<!--  LIBERAZIONE  ANTICIPATA  SPECIALE  GIA' CONCESSA  -->
  <%
  if(dettagliofascicolo.getGiorniLibConcessaLS() != null && dettagliofascicolo.getGiorniLibConcessaLS().compareTo(new BigDecimal(0)) !=0)
  {
  %>
      <tr>
        <td class="L">
...
<!--  INTEGRAZIONE  LIBERAZIONE  ANTICIPATA  GIA' CONCESSA  -->
  <%
  if(dettagliofascicolo.getGiorniLibConcessaLI() != null && dettagliofascicolo.getGiorniLibConcessaLI().compareTo(new BigDecimal(0)) !=0)
  {
  %>
      <tr>
        <td class="L">
...
%>
    <tr>
      <td class="L">
        ...
        <% } %>
        <% if(ggDaConcLS != 0 || ggDaConcLI != 0) {%><font class="label">&nbsp;di cui</font><%}%>
      </td>
    </tr>
<%
}
...
{
  if(dettagliofascicolo.getGiorniLibNONConcessaLA() != null && dettagliofascicolo.getGiorniLibNONConcessaLA().compareTo(new BigDecimal(0))!=0)
  {
  %>
      <tr>
        <td class="L">
...
<!--  LIBERAZIONE  ANTICIPATA  SPECIALE  DA  CONCEDERE  -->
  <%
  if(dettagliofascicolo.getGiorniLibNONConcessaLS() != null && dettagliofascicolo.getGiorniLibNONConcessaLS().compareTo(new BigDecimal(0))!=0)
  {
  %>
      <tr>
        <td class="L">
...
<!--   INTEGRAZIONE  LIBERAZIONE  ANTICIPATA  DA  CONCEDERE   -->
  <%
  if(dettagliofascicolo.getGiorniLibNONConcessaLI() != null && dettagliofascicolo.getGiorniLibNONConcessaLI().compareTo(new BigDecimal(0))!=0)
  {
    // [FT] - 05/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger()
    //siesLogger.debug("XXXXXXXXXXXXXXXXXXXXX -- DENTRO LI tot = "+dettagliofascicolo.getGiorniLibNONConcessaLI().intValue());
  %>
      <tr>
        <td class="L">
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
//      if(    UtenteConnesso.getUfficioUtente().getCodUfficio().equalsIgnoreCase(lEveMod.getCodUfficioInserimento()) 
