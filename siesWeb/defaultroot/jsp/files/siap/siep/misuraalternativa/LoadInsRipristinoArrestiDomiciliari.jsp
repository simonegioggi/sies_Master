...
<jsp:useBean id="motivoProvvMagistrato"   scope="request" class="java.lang.String"/>
<jsp:useBean id="motivoProvvTribunale"   scope="request" class="java.lang.String"/>
<jsp:useBean id="motivoProvvVuoto"   scope="request" class="java.lang.String"/>
...
<title>[S.I.E.S.] - Gestione evento </title>
    ...
    function VerificaCampiObbligatori () { 
    	...
    	if((campoConUfficioSorveglianza=="" || campoConUfficioSorveglianza=="-") 
    			&& (campoConUfficioSorveglianza=="" || campoConUfficioSorveglianza=="-"))
    	{
    	...
    	if((campoSedeTdsEmitt=="" || campoSedeTdsEmitt=="-") 
    			&& (campoSedeTdsEmitt=="" || campoSedeTdsEmitt=="-"))
    	{
    		alert("La Sede Ufficio Emittente è obbligatoria");
    		return false;
    	}
    	
    	var campoCodTipoDecisione = document.LoadInserisciMisuraAlternativa.<%= ICostantiMisuraAlternativa.CAMPO_COD_TIPO_DECISIONE %>.value;
    	if((campoCodTipoDecisione=="" || campoCodTipoDecisione=="-") 
    			&& (campoCodTipoDecisione=="" || campoCodTipoDecisione=="-"))
    	{
    		alert("Il Tipo Provvedimento è obbligatorio");
    		return false;
    	}
    	
    	var decisioneMagistrato = document.LoadInserisciMisuraAlternativa.<%= ICostantiEvento.CAMPO_COD_MOTIVO_MAGISTRATO %>.value;
    	var decisioneTribunale = document.LoadInserisciMisuraAlternativa.<%= ICostantiEvento.CAMPO_COD_MOTIVO_TRIBUNALE %>.value;   	
    	if((decisioneMagistrato=="" || decisioneMagistrato=="-")
    			&& (decisioneTribunale=="" || decisioneTribunale=="-"))
    	{
    	...
    	var campoCodTipoAutorita = document.LoadInserisciMisuraAlternativa.<%= ICostantiMisuraAlternativa.CAMPO_COD_POLIZIA_E %>.value;
    	if (campoCodTipoAutorita=="" || campoCodTipoAutorita=="-")
    	{
    	...
    	{
    		var campoIstDetIdIstDet = document.LoadInserisciMisuraAlternativa.<%= ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE %>.value;
        	if((campoIstDetIdIstDet=="" || campoIstDetIdIstDet=="-") )
        	{   
    	...
    	var campoIdCssa = document.LoadInserisciMisuraAlternativa.<%= ICostantiCSSA.CAMPO_ID_CSSA %>.value;
    	if((campoIdCssa=="" || campoIdCssa=="-") 
    			&& (campoIdCssa=="" || campoIdCssa=="-"))
    	{
    	...
    	var campoCodUds = document.LoadInserisciMisuraAlternativa.<%= ICostantiMisuraAlternativa.CAMPO_COD_UDS %>.value;
    	if((campoCodUds=="" || campoCodUds=="-") 
    			&& (campoCodUds=="" || campoCodUds=="-"))
    	{
    		// MEV10-s3: modificato msg
    	...
    	if((campoSedeTds=="" || campoSedeTds=="-") 
    			&& (campoSedeTds=="" || campoSedeTds=="-"))
    	{
    ...
    function onchangeUfficioEmittente() {
    	//alert('onchangeUfficioEmittente: ');
    	var a_typename = document.getElementById('<%=MinorMask.ComboEmittenteId%>').value;
    	//TDS =	Tribunale di Sorveglianza
		//TDSM=	Tribunale per i minorenni in funzione di Tribunale Sorveglianza
		//UDS =	Ufficio di Sorveglianza
		//UDSM=	Magistrato di Sorveglianza presso il Tribunale per minorenni
    	if ( a_typename == "UDS" || a_typename == "UDSM" || a_typename == "TDS" || a_typename == "TDSM")  {
    	  ...
    	  nodetipoOggDeciTribunaleDiv=document.getElementById('oggettoDecisioneTribunaleDiv');
    	  nodetipoOggDeciMagistratoDiv=document.getElementById('oggettoDecisioneMagistratoDiv');
    	  
    	  nodetipoOggDeciVuotoDiv=document.getElementById('oggettoDecisioneVuotoDiv');
    	  
    	  if ( a_typename == "TDS" || a_typename == "TDSM") 
    	  {   //alert('Tribunale: ' + a_typename);
	    	  nodetipoOggDeciTribunaleDiv.style.display='block';   		 
	    	  nodetipoOggDeciMagistratoDiv.style.display='none';
	    	  nodetipoOggDeciVuotoDiv.style.display='none';
	    	  
	    	  nodetipoOggDeciMagistratoDiv.disabled =true;
	    	  nodetipoOggDeciTribunaleDiv.disabled =false;
    	  } 
    	  else  if ( a_typename == "UDS" || a_typename == "UDSM")  
    	  {   //alert('Magistrato: ' + a_typename);
	    	  nodetipoOggDeciTribunaleDiv.style.display='none';    		
	    	  nodetipoOggDeciMagistratoDiv.style.display='block';
	    	  nodetipoOggDeciVuotoDiv.style.display='none';
	    	  
	    	  nodetipoOggDeciMagistratoDiv.disabled =false;
	    	  nodetipoOggDeciTribunaleDiv.disabled =true;
          }
    	   else   
    	  {   //alert('Tribunale/Magistrato VUOTA: ' + a_typename);
	    	  nodetipoOggDeciTribunaleDiv.style.display='none';   		 
	    	  nodetipoOggDeciMagistratoDiv.style.display='none';
	    	  nodetipoOggDeciVuotoDiv.style.display='block';
   ...
   // Chiamata lista Avvocati.
    ...
    function ListaDocumentiSius(a_formname)
    {
      ...
      else if(document.LoadInserisciMisuraAlternativa.tipomisura.value=='DETENZIONE')
      {
    	<%-- MERGE v10 COLLAUDO: modificata casistica per gestione arresti domiciliari --%>
  		...
  		naturaMA='<%=ICostantiMisuraAlternativa.RIPRISTINO%>';
      }
    ...
    function verificaUfficioEmittente()
    {
...
<%}
  ...
  if(document.LoadInserisciMisuraAlternativa.flagmisura.value=="N")
  {
    ...
    if(!document.LoadInserisciMisuraAlternativa.tipo[0].checked && !document.LoadInserisciMisuraAlternativa.tipo[1].checked )
    {
   ...
   {		
       ...
       var nodeIstituto;
        ...
        if(document.LoadInserisciMisuraAlternativa.tipo[0].checked)
        {
          ...
          // Modifica del 13/04/2016 commentata riga successiva
          // per layout errato nella maschera di "Ripristino Arresti Domiciliari"
          
          // MEV10-s3: modificato valore proprietà top
...
<%}%>
  ...
  <jsp:include page="/jsp/files/siap/siep/misuraalternativa/MinorScript.jsp"/>
       ...
       <body class="corpo" onLoad="radio(), onchangeUfficioEmittente();">
    <table>
...
<%
  ...
  {
%>
    <font class="campo">Ripristino Degli Arresti Domiciliari</font>
<%
  }
...
%>
 ...
 <br>
  ...
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.misuraalternativa.action.ActInsRipristinoArrestiDomiciliari">
...
<table width ="90%">
   ...
   <tr>
    ...
    <tr>		
	    ...
	    <td class="l" colspan="3">	      
	      <%=MinorMask.comboEmittente(filtroMinorenni, MinorMask.EmittenteAutoritaUff, "onchange='onchangeUfficioEmittente();'", ICostantiMisuraAlternativa.CAMPO_COD_UFFICIO_SORVEGLIANZA)%>
	      <INPUT type="hidden" name="ufficioEmittente" value="<%=ICostantiMisuraAlternativa.CAMPO_COD_UFFICIO_SORVEGLIANZA%>">
    ...
    <div id="oggettoDecisioneTribunaleDiv" style="width: 100%; display:none;" >
		<table cellspacing=1 cellpadding=1 width=90%>
		    <tr>
		      ...
		      <td class="L" colspan="3">
				...
				{
				%>
				        <font class="campo"><%=motivoProvvTribunale%></font>
				        <INPUT type="hidden" name="<%=ICostantiEvento.CAMPO_COD_MOTIVO_TRIBUNALE%>" value="<%=codicemotivo%>">
				<%
				}else{
				%>
				        <select  Title="Codice Motivo" name="<%=ICostantiEvento.CAMPO_COD_MOTIVO_TRIBUNALE%>" onChange="pulisciId();">
				          <%=motivoProvvTribunale%>
				        </select>
				<%}%>
		      </td>
		    </tr>
	   ...
	   <div id="oggettoDecisioneVuotoDiv" style="width: 100%; display:blok;" >
		<table cellspacing=1 cellpadding=1 width=100%>
		    <tr>
		      <td class="l"  width="24%"> Oggetto  Decisione <font class=ob> (*) </font></td>
		      <td class="L" colspan="3" width="100%">
				<%
				if(codicemotivo != null && !codicemotivo.equals(""))
				{
				%>
				        <font class="campo"><%=motivoProvvVuoto%></font>
				        <INPUT type="hidden" name="" value="<%=codicemotivo%>">
				<%
				}else{
				%>
				        <select  Title="Codice Motivo" name="" onclick="verificaUfficioEmittente();">
				          <%=motivoProvvVuoto%>
				        </select>
				<%}%>
		      </td>
	...
	<div id="oggettoDecisioneMagistratoDiv" style="width: 100%; display:none;" >
		<table cellspacing=1 cellpadding=1 width=90%>
		    <tr>
		      <td class="l">Oggetto Decisione <font class=ob>(*)</font></td>
		      <td class="L" colspan="3">
				...
				{
				%>
				        <font class="campo"><%=motivoProvvMagistrato%></font>
				        <INPUT type="hidden" name="<%=ICostantiEvento.CAMPO_COD_MOTIVO_MAGISTRATO%>" value="<%=codicemotivo%>">
				<%
				}else{
				%>
				        <select  Title="Codice Motivo" name="<%=ICostantiEvento.CAMPO_COD_MOTIVO_MAGISTRATO%>" onChange="pulisciId();">
				          <%=motivoProvvMagistrato%>
				        </select>
				<%}%>
		      </td>
		    </tr>
  ...
  <table width ="90%"> 
    <tr>
      <td class="l">Data Emissione Decisione</td>
      <td class="l" colspan="3">
        <font class="campo">
     ...
     <tr>
     <%-- 20170907: [SG] cambio etichetta --%>
<!--       <td  class="l">Luogo della Detenzione</td> -->
      <td  class="l">Luogo Espiazione Misura</td>
      <td  class="L" colspan="3">
        <TEXTAREA title="Luogo Espiazione Misura" name="<%=ICostantiMisuraAlternativa.LUOGO_DELLA_DETENZIONE%>" cols=100 rows=1></textarea>
      </td>
    </tr>
    <tr>
      <td  class="l">Note</td>
...
<%
    if(   tipoSospensione.equals("AFFIDAMENTO")
       || tipoSospensione.equals("DETENZIONE")
       || tipoSospensione.equals("SEMILIBERTA")
       || tipoSospensione.equals("INDULTINO"))
    {
%>
      <td class="l">Data Scarcerazione &nbsp;
        <input title = "Giorno Data Ingresso in carcere" value="" type="text" size="2" maxlength="2" name="<%= ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_SCARCERAZIONE%>" <%=IWebConstants.UTIL_DATA%> > -
        <input title = "Mese Data Ingresso in carcere" value="" type="text" size="2" maxlength="2" name="<%= ICostantiMisuraAlternativa.CAMPO_MESE_DATA_SCARCERAZIONE%>" <%=IWebConstants.UTIL_DATA%> > -
        <input title = "Anno Data Ingresso in carcere" value="" type="text" size="4" maxlength="4" name="<%= ICostantiMisuraAlternativa.CAMPO_ANNO_DATA_SCARCERAZIONE%>" <%=IWebConstants.UTIL_DATA_ANNO%> >
      </td>
<%
    }
%>
...
<!--autorità esterna e-->
    <tr>
     ...
     <td class="L">
       <input title="Sede Autorita Esterna" type="text" name="<%=ICostantiMisuraAlternativa.CAMPO_COD_SEDE_E%>"  maxlength="35" size="35">
        <a href="Javascript:ListaComuni('LoadInserisciMisuraAlternativa','<%=ICostantiMisuraAlternativa.CAMPO_COD_SEDE_E%>');">
          <img src="/images/filefolder.gif" border=0>
        </a>
      </td>
           <td class="l">Indirizzo</td>
           <td class="L">
              <TEXTAREA title="Note" name="<%= ICostantiMisuraAlternativa.CAMPO_NOTE_POL_E %>"  cols=50></textarea>
            </td>
    </tr>
</table>
...
<!-- <div id="istituto" style="width=100%; position:relative; top:-75px" > --> 
<div id="istituto" style="width: 100%; visibility:visible; position:relative;" >
<table width="100%">
<tr>
