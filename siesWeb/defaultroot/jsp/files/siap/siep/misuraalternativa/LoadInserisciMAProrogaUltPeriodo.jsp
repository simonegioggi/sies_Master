...
<jsp:useBean id="detdomSpecconcessa"       scope="request" class="siap.sico.misuraalternativa.model.MisuraAlternativaModel"/>
...
if(document.LoadInserisciMisuraAlternativa.<%= ICostantiMisuraAlternativa.CAMPO_SEDE_TDS_EMITT %>.value=="")
      {
        alert("La sede Ufficio emittente è obbligatorio");
        return false;
      }
	  }
...
if(document.LoadInserisciMisuraAlternativa.<%= ICostantiMisuraAlternativa.CAMPO_SEDE_TDS %>.value=="")
  {
    alert("La sede Tribunale di Sorveglianza è obbligatoria");
    return false;
  }

if(document.LoadInserisciMisuraAlternativa.<%= ICostantiMisuraAlternativa.CAMPO_SEDE_POL_C %>.value=="")
  {
    alert("Autorità di polizia preposta al controllo è obbligatorio");
    return false;
  }
...
%>
    <font class="campo">Proroga Ulteriore Periodo Detenzione Domiciliare Speciale</font>
 ...
 <br>
  ...
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.misuraalternativa.action.ActInserisciMAProrogaUltPeriodo">
...
%>
         ...
         <td class="L" colspan=2>
           <input value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(penaresidua.getDataFine(), "dd") )%>" type="text" size="2" maxlength="2" name="<%= ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE %>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)" onBlur="javascript:value=FillDM(value)">
           -
           <input value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(penaresidua.getDataFine(), "MM") )%>" type="text" size="2" maxlength="2" name="<%= ICostantiPenaResidua.CAMPO_MESE_DATA_FINE %>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)" onBlur="javascript:value=FillDM(value)">
           -
           <input value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(penaresidua.getDataFine(), "yyyy") )%>" type="text" size="4" maxlength="4" name="<%= ICostantiPenaResidua.CAMPO_ANNO_DATA_FINE %>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)" onBlur="javascript:value=FillYear(value)">
         </td>
<%
...
<%if(detdomSpecconcessa != null){%>
    <tr>
        <td class="l">Data decorrenza Det.Dom Speciale</td>

    <td class="l"><font class="campo"> <%=StringUtils.toStringJSP(DateUtils.getDateToString(detdomSpecconcessa.getDataDecisione(),"dd-MM-yyyy"))%></font></td>
    </tr>



  <tr>
      <td class="l">Luogo Detenzione Domiciliare </td>

      <td class="l"><font class="campo"><%=StringUtils.toStringJSP(detdomSpecconcessa.getDescrLuogoProva())%>&nbsp;
      <input type="hidden" name="<%=ICostantiMisuraAlternativa.CAMPO_DESCR_LUOGO_PROVA%>" value="<%=StringUtils.toStringJSP(detdomSpecconcessa.getDescrLuogoProva())%>">
         </font>
    </td>
   </tr>
<%}%>
...
<!--UEPE competente-->
  ...
  <tr>
      <td class="l">UEPE Competente</td>
         <td class="l">
         <input type="hidden" name="cssaDetenuto" value="C">
