...
<jsp:useBean id="inserimentoCessazione"	scope="request" class="java.lang.String"/>
...
<%
//==============================================================================
// Form che gestista la Cessazione per le seguenti misure
...
function caricaComboOggetto() {
	if (typeof (document.LoadInserisciMisuraAlternativa.<%=ICostantiMisuraAlternativa.CAMPO_COD_UFFICIO_SORVEGLIANZA%>) != "undefined") {
		caricaCombo(strOggettiDecisione,';','#',document.LoadInserisciMisuraAlternativa.<%=ICostantiMisuraAlternativa.CAMPO_COD_UFFICIO_SORVEGLIANZA%>.value,document.LoadInserisciMisuraAlternativa.<%=ICostantiEvento.CAMPO_COD_MOTIVO%>);
    }
}
...
<%
// Fase di inserimento dell'ordinanza
if (inserimentoCessazione == null || (inserimentoCessazione != null && inserimentoCessazione.compareTo("SI") != 0)) {
%>
	// Va indicata o a data Cessazione o i quantum ridetermnati. Non Etrambi
...
%>
	...
	if (document.LoadInserisciMisuraAlternativa.flagmisura.value == "N") {
		...
		<%-- 
		29/11/2010 Controllo eliminato e ripristinato su indicazioni di Nunzia Alfieri (Test Esecuzione Pena presso Domicilio).  Riprist. 16/06/2011
		if (tipoRevoca != null && tipoRevoca.equals("AFFIDAMENTO")) {
		 	if (document.LoadInserisciMisuraAlternativa.<%=ICostantiMisuraAlternativa.CAMPO_ANNO_DATA_INIZIO_REVOCA%>.value == ""
				    ...
				    && document.LoadInserisciMisuraAlternativa.<%=ICostantiMisuraAlternativa.CAMPO_NUM_MESI_REVOCA_ARRESTO%>.value == ""
				    && document.LoadInserisciMisuraAlternativa.<%=ICostantiMisuraAlternativa.CAMPO_NUM_GIORNI_REVOCA_ARRESTO%>.value == "") {
		  		alert("Data Cessazione o Pena Residua Rideterminata Obbligatori");
		  		return false;
			}
		}
		fine eliminazione controllo 
		--%>
	} // end if(document.LoadInserisciMisuraAlternativa.flagmisura.value == "N")
<%
if (lPosizione!= null && lPosizione.getCodPosizioneGiuridica() != null && !lPosizione.isLibero()) {
%>
	// Check Eventuale data ingressi in istituto se si è selezionata l'opzione "detenuto"
	if (document.LoadInserisciMisuraAlternativa.tipo[1].checked == true) {
  		if (document.LoadInserisciMisuraAlternativa.<%=ICostantiMisuraAlternativa.CAMPO_ANNO_DATA_INGRESSO_ISTITUTO%>.value == "") {
			alert("La data di Detenzione è obbligatoria");
...
if (nuovapenaresidua != null && nuovapenaresidua.getIdPenaResidua() != null) {
%>
	// Check Magistrato firnatario e Destinatarei
...
<%
	if ((tipoRevoca != null && tipoRevoca.equals("AFFIDAMENTO") && lPosizione!= null && lPosizione.getCodPosizioneGiuridica() != null
    		...
    		&& lPosizione.getCodPosizioneGiuridica()!= null && (lPosizione.getCodPosizioneGiuridica().equals("51")
...
%>
		...
		alert("Il " + descrTipoUfficio + " è obbligatorio");
...
<%
// <div id="istituto"
// <div id="autorita"  Polizia per l'esecuzione
// <div id="altri"     TDS/UDS + Avvocati
// <div id="bottone" 
%>
...
<%     
	// Se è in sospensione (Cautelativa o provvisoria) della misura si manda:
	// - All'autorità di polizia per l'esecuzione: se soggetto non detenuto
	// - All'istituto se soggetto detenuto (esegue SORV)
	// Attenzione per ora gestisce solo le posizione "in sospensione"
	// NEL SOLO CASO DI DET DOM TERM si gestisce anche la posizione 'IN MISURA'
 	if ((tipoRevoca.equals("AFFIDAMENTO")
 			&& (lPosizione.getCodPosizioneGiuridica().equals("32") // sosp 51 ter
				|| lPosizione.getCodPosizioneGiuridica().equals("37") // sosp 51 bis
				|| lPosizione.getCodPosizioneGiuridica().equals("13"))) // in Misura!!
			|| (tipoRevoca.equals("INDULTINO")
			...
			||( tipoRevoca.equals(ICostantiMisuraAlternativa.ESP_PRESSO_DOM)
					&& (lPosizione.getCodPosizioneGiuridica().equals("50") // in Misura!!!
							|| lPosizione.getCodPosizioneGiuridica().equals("51") // In sospoensione 51 ter
							|| lPosizione.getCodPosizioneGiuridica().equals("52")))) { // In sospoensione 51 bis
		// Se in sospensione Cautelativa 51 ter o Sospensione Provvisoria 51 bis 
%>
...
<%
	} else { // Soggetto non in sospensione: dest is Autorità per l'esecuzione
%> 
	nodeist.style.display = 'none'; 
	nodeaut.style.display = 'block';
	document.LoadInserisciMisuraAlternativa.<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>.disabled = true;          
...
} // chiudo if (nuovapenaresidua != null && nuovapenaresidua.getIdPenaResidua() != null) 
...
<body class="corpo" onload="caricaComboOggetto();VisualizzaCalcoli();functionRadio();">
<table>
	<tr>
    	<td class="LBG">
...
%>
			<font class="campo">Cessazione L.207/2003</font>
<%
} else if (tipoRevoca.equals("AFFIDAMENTO")) {
%>
        	<font class="campo">Cessazione Affidamento in Prova</font>
<%
} else if (tipoRevoca.equals(ICostantiMisuraAlternativa.ESP_PRESSO_DOM)) {
%>
        	<font class="campo">Cessazione Espiazione Pena presso Domicilio</font>
<%
}
%>
		</td>
...
%>
<input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.misuraalternativa.action.ActInserisciCessazioneMAAffProv">
...
String disabilitaData = null;  // null o readonly
String scarcerato = null;  // checked o null
String scarcerare = null;  // checked o null
...
//==============================================================================
// Posizione Giuridica - Luogo di Detenzione - Pena Residua
//==============================================================================
%>
<table>
	<tr>
...
<%
// Visualizzo decorrenza e scadenza della misura in corso solo in caso di
// misura concessa o in ammissione provvisoria ovvero se sto "Cessando" 
// una misura in corso
...
%>       
  		...
  		<td class="L">
  			<font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(misuraconcessa.getDataInizioMisura(),"dd-MM-yyyy"))%></font>
  		</td>
<%
	}
...
%>       
		<td class="l">Data Fine Misura</td>
 		<td class="L">
 			<font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(misuraconcessa.getDataFineMisura(),"dd-MM-yyyy"))%></font>
 		</td>
<%
	}
%>
...
%>
  		<td class="l">Data Inizio Misura</td>
  		<td class="L">
  			<font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(misuraammissioneprovv.getDataInizioMisura(),"dd-MM-yyyy"))%></font>
  		</td>
<%
	}
...
%>       
  		<td class="l">Data Fine Misura</td>
  		<td class="L">
  			<font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(misuraammissioneprovv.getDataFineMisura(),"dd-MM-yyyy"))%></font>
  		</td>
<%
	}
%>
...
%>
		<td class="Titolo" colspan="4"> Dati Ordinanza di Cessazione del <%=descrTipoUfficio%> </td>
<%
} else if ("02".equals(misuraalternativa.getCodTipoDecisione())) {
%>
    	<td class="Titolo" colspan="4"> Dati Decreto di Cessazione del <%=descrTipoUfficio%> </td>
<%
} else {
%>
    	<td class="Titolo" colspan="4"> Dati del Provvedimento di Cessazione del Tribunale di Sorveglianza </td>
<%
}
%>
	</tr>
<%
if (nuovapenaresidua != null && nuovapenaresidua.getIdPenaResidua() != null) {
	// n.b. nuovapenaresidua != null se Sto inserendo il provvedimento SIEP
	// n.b. misuraalternativa = null se ho selezionato l'ordinanza dalla lista e SIUS non ha inserito le informazioni su MISURA_ALTERNATIVA
%>
  	<tr>
...
%>
      		...
      		<input type="text" value="<%=StringUtils.toStringJSP(misuraalternativa.getChiaveProgrFascicoloSius())%>" name="<%=ICostantiMisuraAlternativa.CAMPO_CHIAVE_PROGR_FASCICOLO_SIUS%>" size="6" maxlength="6">
<%
	} else {
%>
...
<%
	if (misuraalternativa == null || misuraalternativa.getIdMisuraAlternativa() == null) {
%>
			<input type="text" value="<%=StringUtils.toStringJSP(misuraalternativa.getAnnoRegistro())%>" name="<%=ICostantiMisuraAlternativa.CAMPO_ANNO_REGISTRO%>" size="4" maxlength="4">/
        	<input type="text" value="<%=StringUtils.toStringJSP(misuraalternativa.getNumeroRegistro())%>" name="<%=ICostantiMisuraAlternativa.CAMPO_NUMERO_REGISTRO%>" size="6" maxlength="6">
<%
	} else {
%>
...
<%
	if (misuraalternativa == null || misuraalternativa.getChiaveUfficioFascicoloSius() == null
			|| misuraalternativa.getIdMisuraAlternativa() == null) {
		if (tipoRevoca != null && (tipoRevoca.equals("INDULTINO")
				|| tipoRevoca.equals(ICostantiMisuraAlternativa.ESP_PRESSO_DOM) 
                || tipoRevoca.equals("AFFIDAMENTO"))) { // 17/02/2013
			// Nel caso di INDULTINO o ESP_PRESSO_DOM o AFFIDAMENTO, l'Uff emittente può essere sia il TDS che il MDS
%>
    ...
    <tr>
		...
		<td class="l" colspan="3">
            ...
            <a href="Javascript:ListaComuniEmitUTMinor('LoadInserisciMisuraAlternativa','<%=ICostantiMisuraAlternativa.CAMPO_COD_UFFICIO_SORVEGLIANZA%>');">
...
<%
} // chiude if (nuovapenaresidua != null && nuovapenaresidua.getIdPenaResidua() != null)
else {
	// if(nuovapenaresidua != null && nuovapenaresidua.getIdPenaResidua() != null)
	// nuovapenaresidua = null sto caricando per la prima volta la form
%>
	<tr>
    ...
    <tr>
      	...
      	<td class="l">
        	<input Title="Anno Fascicolo Sius" name="<%=ICostantiMisuraAlternativa.CAMPO_CHIAVE_ANNO_FASCICOLO_SIUS%>" type="text" size="4" maxlength="4" onkeypress="return TicTabNumField(this,event)" onBlur="javascript:value=FillYear(value)" onChange="pulisciId();">
        	/
      	...
      	<td class="l">
        	<input Title="Anno Provvedimento" name="<%=ICostantiMisuraAlternativa.CAMPO_ANNO_REGISTRO%>" type="text" size="4" maxlength="4" onkeypress="return TicTabNumField(this,event)" onBlur="javascript:value=FillYear(value)" onChange="pulisciId();">
        	/
        	<input Title="Numero Provvedimento" name="<%=ICostantiMisuraAlternativa.CAMPO_NUMERO_REGISTRO%>" type="text" size="6" maxlength="6" onkeypress="return TicTabNumField(this,event)" onChange="pulisciId();">
      	</td>
	</tr>
    <tr>
      	<td class="l">Ufficio Emittente</td>
...
<%
	if (tipoRevoca.equals("AFFIDAMENTO") || tipoRevoca.equals("ESP_PRESSO_DOM") /*|| tipoRevoca.equals("INDULTINO")*/) {
%>
	<tr>
...
%>
	<input type="hidden" value="03" name="<%=ICostantiMisuraAlternativa.CAMPO_COD_TIPO_DECISIONE%>">
<%
	}
%>
	<tr>
...
<%
} // Fine sezione coni dati del Provvedimento di cessazione della Sorveglianza. Quello da eseguire
...
%>
	<tr>
		...
		<td class="l" colspan="3">
			<font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(misuraalternativa.getDataInizioRevoca(), "dd-MM-yyyy"))%>
...
if ((misuraalternativa == null || misuraalternativa.getIdMisuraAlternativa() == null)
		|| (nuovapenaresidua == null || nuovapenaresidua.getIdPenaResidua() == null)) { // Sto inserendo l'ordinanza
%>
	<tr>
      	<td class="l">
...
<%
	// Se soggetto in MISURA visualizza i giorni espiati da detrarre
    if ((tipoRevoca.equals("AFFIDAMENTO") && lPosizione.getCodPosizioneGiuridica().equals("13")) // Espiazione Pena in Regime di Affidamento in Prova
			|| (tipoRevoca.equals("INDULTINO") && lPosizione.getCodPosizioneGiuridica().equals("27")) // Sospensione Pena ex L. 207/03
			|| (tipoRevoca.equals(ICostantiMisuraAlternativa.ESP_PRESSO_DOM) && lPosizione.getCodPosizioneGiuridica().equals("50"))) { // Esecuzione presso domicilio della pena detentiva
%>
	<tr id="idTR3" style="display:none; position:relative;">
...
<%
		...
		&& nuovapenaresidua != null && nuovapenaresidua.getIdPenaResidua() != null) {
	// Fase di inserimento del Provvedimento SIEP, visualizzo i dati dell'ordinanza precedentemente inseriti
%>
	<tr>
<%
	// Verificare se in caso di cessazione questi campi sono valorizzati
	if (misuraalternativa.getNumAnniRevocaReclusione() != null || misuraalternativa.getNumMesiRevocaReclusione() != null
	         || misuraalternativa.getNumGiorniRevocaReclusione() != null || misuraalternativa.getNumAnniRevocaArresto() != null 
...
// Se non libero 
if (lPosizione != null && lPosizione.getCodPosizioneGiuridica() != null && !lPosizione.isLibero()) {
	// La sezione con "Esecuzione contro soggetto non detenuto/detenuto" + data detenuto dal
  	// viene visualizzata sia in fase di inserimento dell'ordinanza sia in fase di inserimento del provvedimento SIEP
%>
<table>
	<tr>
<%
	if (nuovapenaresidua != null && nuovapenaresidua.getIdPenaResidua() != null) {
%>
		<td class="l">Esecuzione contro soggetto non detenuto &nbsp;<input type="radio" name="tipo" value="nondetenuto" <%=StringUtils.toStringJSP(dascarceraredisabilita)%> <%=StringUtils.toStringJSP(scarcerare)%> onclick="functionRadio();">
			&nbsp; Esecuzione contro soggetto detenuto &nbsp; <input type="radio" name="tipo" value="detenuto" <%=StringUtils.toStringJSP(scarceratodisabilita)%> <%=StringUtils.toStringJSP(scarcerato)%> onclick="functionRadio();">
      	</td>
      	<td class="l">Dal &nbsp;
<%
		if (misuraalternativa != null && misuraalternativa.getIdMisuraAlternativa() != null
...
//==============================================================================
// Sezione con la pena residua rideterminata.
// n.b. visualizzata anche se no si è proceduto al calcolo
//==============================================================================
if (nuovapenaresidua != null && nuovapenaresidua.getIdPenaResidua() != null) {
%>
<table width ="95%">
...
//==============================================================================
// Sezione con il Magistrato Firmatario e i destinatari
//==============================================================================
%>
<table width ="95%">
<%
...
<%-- Autorità competente per il territorio (x esecuzione) --%>
...
<%
// - TDS/UDS (ufficio emittente il provvedimento di cessazione)
// - Avvocati 
%>
...
%>
frmvalidator.addValidation("<%=ICostantiMisuraAlternativa.CAMPO_NUM_ANNI_REVOCA_RECLUSIONE%>","numeric","Pena Residua Rideterminata : Il Campo Anni Reclusione è numerico");
frmvalidator.addValidation("<%=ICostantiMisuraAlternativa.CAMPO_NUM_MESI_REVOCA_RECLUSIONE%>","numeric","Pena Residua Rideterminata : Il Campo Mesi Reclusione è numerico");
frmvalidator.addValidation("<%=ICostantiMisuraAlternativa.CAMPO_NUM_GIORNI_REVOCA_RECLUSIONE%>","numeric","Pena Residua Rideterminata : Il Campo Giorni Reclusione è numerico");

frmvalidator.addValidation("<%=ICostantiMisuraAlternativa.CAMPO_NUM_ANNI_REVOCA_ARRESTO%>","numeric","Pena Residua Rideterminata : Il Campo Anni Arresto è numerico");
frmvalidator.addValidation("<%=ICostantiMisuraAlternativa.CAMPO_NUM_MESI_REVOCA_ARRESTO%>","numeric","Pena Residua Rideterminata : Il Campo Mesi Arresto è numerico");
frmvalidator.addValidation("<%=ICostantiMisuraAlternativa.CAMPO_NUM_GIORNI_REVOCA_ARRESTO%>","numeric","Pena Residua Rideterminata : Il Campo Giorni Arresto è numerico");
<%
}
if (misuraalternativa == null || misuraalternativa.getIdMisuraAlternativa() == null) {
%>
...
if ((tipoRevoca != null && lPosizione != null && lPosizione.getCodPosizioneGiuridica() != null)
		&& ((tipoRevoca.equals("AFFIDAMENTO") && lPosizione.getCodPosizioneGiuridica().equals("13"))
				|| (tipoRevoca.equals("INDULTINO") && lPosizione.getCodPosizioneGiuridica().equals("27"))
				|| (tipoRevoca.equals(ICostantiMisuraAlternativa.ESP_PRESSO_DOM)
						&& lPosizione.getCodPosizioneGiuridica().equals("50")))) {
	if (misuraalternativa == null || misuraalternativa.getIdMisuraAlternativa() == null) {
%>
frmvalidator.addValidation("<%=ICostantiMisuraAlternativa.CAMPO_FLAG_PERIODO_ESPIATO%>","numeric","Il Campo Periodo espiato in Istituto da detrarre è numerico");
<%
	}
}
%>
