...
<%	
	if((!lPosizione.isLibero()) || (((lFascicoloAssociato.getFlagAltraCausa()!=null && lFascicoloAssociato.getFlagAltraCausa().equals("S")) && (penaresidua.getDataFinePresunta()!= null && penaresidua.getDataFine() == null) ) ))
	{
 		if ( ((penaresidua.getFlagErgastolo() == null) || (penaresidua.getFlagErgastolo() != null && !penaresidua.getFlagErgastolo().equals("S") && !penaresidua.getFlagErgastolo().equals("D"))))
 		{
  			...
  			{
%>  				
    			if (document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value.length==1)
			  		document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value='0'+document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value;
		  		if (document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value.length==1)
			  		document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value='0'+document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value;

		  		var data_to_verifica = document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiPenaResidua.CAMPO_GIORNO_DATA_FINE%>.value+'/'+document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiPenaResidua.CAMPO_MESE_DATA_FINE%>.value+'/'+document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiPenaResidua.CAMPO_ANNO_DATA_FINE%>.value;
...
%>
    ...
    if (document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiNotifica.CAMPO_GIORNO_DATA_INVIO%>.value.length==1)
      	document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiNotifica.CAMPO_GIORNO_DATA_INVIO%>.value='0'+document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiNotifica.CAMPO_GIORNO_DATA_INVIO%>.value;
    if (document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiNotifica.CAMPO_MESE_DATA_INVIO%>.value.length==1)
      	document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiNotifica.CAMPO_MESE_DATA_INVIO%>.value='0'+document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiNotifica.CAMPO_MESE_DATA_INVIO%>.value;

    	var data_to_verify = document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiNotifica.CAMPO_GIORNO_DATA_INVIO%>.value+'-'+document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiNotifica.CAMPO_MESE_DATA_INVIO%>.value+'-'+document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiNotifica.CAMPO_ANNO_DATA_INVIO%>.value;
    	...
    	var campo = document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiPosizioneGiuridica.CAMPO_COD_POSIZIONE_GIURIDICA %>.value;

    	if(document.LoadVariaMisuraAlternativaDetDom.flagmisura.value=="N")
    	{
      		if (document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>.value.length==1)
        		document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>.value='0'+document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>.value;
      		if (document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>.value.length==1)
        		document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>.value='0'+document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>.value;

      		var data_to_verify = document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>.value+'-'+document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>.value+'-'+document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_ANNO_DATA_DECISIONE%>.value;
      		if (!ControllaData(data_to_verify) )
      		{
        		alert('Data di emissione Decreto non valida');
        		return false;
      		}
      		if(document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_SEDE_TDS_EMITT %>.value=="")
      		{
        		alert("La Sede dell'Ufficio Emittente è obbligatoria");
        		document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_SEDE_TDS_EMITT %>.focus();
        		return false;
      		}
    	}
...
<%	
    	...
    	{
%>			
    		if(document.LoadVariaMisuraAlternativaDetDom.tipo[1].checked == true)
    		{	
					...
					else
					{
       				...
       				document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_SCARCERAZIONE%>.focus();
        			return false;
...
%>
  		...
  		if(document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiMagistrato.CAMPO_COGNOME %>.value=="" && document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiMagistrato.CAMPO_NOME %>.value=="")
  		{ 
	    	alert("Il  Magistrato Firmatario è obbligatorio");
	    	document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiMagistrato.CAMPO_COGNOME %>.focus();
	    	return false;
  		}

		if(!document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiMisuraAlternativa.CAMPO_COD_POLIZIA_E%>.disabled)
    	{
      		if (document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiMisuraAlternativa.CAMPO_COD_POLIZIA_E%>[document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiMisuraAlternativa.CAMPO_COD_POLIZIA_E%>.selectedIndex].value == '-')
      		{
<%      		
        		if ((tipomisura.equals("DETENZIONE"))&& (lPosizione.isLibero()) 
            		&& (verbale.getIdVerbale()== null))
        		{
%>       
					document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiMisuraAlternativa.CAMPO_COD_POLIZIA_E%>.focus(); 			
...
%>
		...
		if (document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiMisuraAlternativa.CAMPO_SEDE_POL_E%>.value=="")
      	{
      			document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiMisuraAlternativa.CAMPO_SEDE_POL_E%>.focus();
       			alert("Il campo sede Autorità Destinazione è Obbligatorio");
        		return false;
      	}
		
		if (document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiMisuraAlternativa.CAMPO_COD_UDS%>.value=="")
      	{
      			document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiMisuraAlternativa.CAMPO_COD_UDS%>.focus();
       			alert("Il campo sede Ufficio di Sorveglianza è Obbligatorio");
        		return false;
      	}
      	
      	if (document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_SEDE_TDS%>.value=="")
      	{
      			document.LoadVariaMisuraAlternativaDetDom.<%= ICostantiMisuraAlternativa.CAMPO_SEDE_TDS%>.focus();
       			alert("Il campo sede Tribunale di Sorveglianza è Obbligatorio");
        		return false;
      	}
      	
      	if (document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE %>.value=="")
      	{
      			document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE %>.focus();
       			alert("Il campo sede UNEP Obbligatorio");
        		return false;
      	}

      	if(document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiCSSA.CAMPO_ID_CSSA%>.value=="" || 
      		document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiCSSA.CAMPO_ID_CSSA%>.value=="-" || 
      		document.LoadVariaMisuraAlternativaDetDom.<%=ICostantiCSSA.CAMPO_ID_CSSA%>.value.length==0 )
      	{
      			alert("Il campo sede UEPE/USSM è Obbligatorio");
        		return false;
      	}

}  // Chiude Functio Verifica()
...
<body class="corpo">
  <table>
    ...
    <font class="campo">COMUNICAZIONE VARIAZIONE DATA INIZIO MISURA</font>
  </td>
</tr>
</table>
 ...
 <br>
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadVariaMisuraAlternativaDetDom">
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.misuraalternativa.action.ActVariazioneMADecSca">
  ...
  <tr>
   ...
   <td class="L">
       ...
       <input readonly title= "Nome Magistrato"    value="<%=StringUtils.toStringJSP(magistratocompetente.getMagistrato().getNome() )%>" type="text" name="<%= ICostantiMagistrato.CAMPO_NOME %>"   maxlength="35" size="25">
        <a href="Javascript:ListaMagistrati('LoadVariaMisuraAlternativaDetDom','<%=ICostantiMagistrato.CAMPO_COD_MAGISTRATO%>','<%= ICostantiMagistrato.CAMPO_COGNOME %>','<%= ICostantiMagistrato.CAMPO_NOME %>');">
...
<table width="100%">  
 <tr>
     <td class="Titolo" width="100%" colspan=6> Notifica al Condannato</td>
 </tr>
...
<%	}
  ...
  <%}%>
          ...
          <a href="Javascript:ListaComuni('LoadVariaMisuraAlternativaDetDom','<%=ICostantiMisuraAlternativa.CAMPO_SEDE_POL_E%>');">
          <img src="/images/filefolder.gif" border=0>
        </a>
      </td>
           <td class="l">Indirizzo</td>
...
<table width="100%">
 	<tr>
     <td class="Titolo" width="100%" colspan=6> Notifica Servizio Sociale</td>
    </tr>

    <tr>
      <td class="l">Destinatario</td>
      <td class="L" colspan="3">
        <%=MinorMask.comboCSSA(filtroMinorenni)%>
      </td>
    </tr> 


  <tr>
      <td class="l"width=30%>Sede <%if(tipomisura.equals("DETENZIONE")){%><font class=ob>(*)</font><%}%></td>

      <td class="l">
        <input title="UEPE Competente" name="Indirizzo" value="" size=35 readonly>
        ...
        <input type="hidden" name="cssaE" value="S">
       		<a href="Javascript:ListaCSSAMinor('LoadVariaMisuraAlternativaDetDom','<%=ICostantiCSSA.CAMPO_ID_CSSA%>','Indirizzo');">
        		<img src="/images/filefolder.gif" border=0>
       		</a>
      </td>
 		...
 		<td class="l">Note</td>
        <td class="L">
              <TEXTAREA title="Note" name="<%=ICostantiMisuraAlternativa.CAMPO_NOTE_CSSA%>"  cols=30 ></textarea>
        </td>
    </tr>
</table>

<!-----------------------------Notifica Magistrato di Sorveglianza--------------------------------------->

<table width="100%">
 	<tr>
     <td class="Titolo" width="100%" colspan=6> Notifica al Magistrato di Sorveglianza</td>
    </tr>

    <tr>
      <td class="l">Destinatario</td>
      <td class="L" colspan="3">
        <%=MinorMask.comboMagistrato(filtroMinorenni, MinorMask.SorveglianzaUfficio)%>
      </td>
    </tr> 
  ...
  <tr>
      <td class="l" width=30%>Sede <%if(tipomisura.equals("DETENZIONE")){%><font class=ob>(*)</font><%}%></td>
      <td class="L">
       	<input title="ufficio" value="<%=UffUDS%>" type="text" name="<%=ICostantiMisuraAlternativa.CAMPO_COD_UDS%>" maxlength="35" size="35">
       		<a href="Javascript:ListaComuniMagiSorvMinor('LoadVariaMisuraAlternativaDetDom','<%=ICostantiMisuraAlternativa.CAMPO_COD_UDS%>');">
              <img src="/images/filefolder.gif" border=0> 
            </a>
      </td>
 		...
 		<td class="l">Note</td>
        <td class="L">
              <TEXTAREA title="Note" name="<%=ICostantiMisuraAlternativa.CAMPO_NOTE_UDS%>"  cols=30 ></textarea>
        </td>
  </tr>
</table>  
<!-----------------------------Notifica Ente di Sorveglianza--------------------------------------->    
<table width="100%">  
 	<tr>
     <td class="Titolo" width="100%" colspan=6> Notifica Ente di Sorveglianza</td>
    </tr>

    <tr>
      <td class="l">Destinatario</td>
      <td class="L" colspan="3">
        <%=MinorMask.comboTribunale(filtroMinorenni)%>
      </td>
    </tr> 
  ...
  <tr> 
      ...
      <td class="L">
            <input title="Sede Tribunale Sorveglianza" value="<%=StringUtils.toStringJSP(sedeUfficioEmittente.getDescrComune())%>" type="text" name="<%= ICostantiMisuraAlternativa.CAMPO_SEDE_TDS %>"  maxlength="35" size="35">
        		<a href="Javascript:ListaComuniTribSorvMinor('LoadVariaMisuraAlternativaDetDom','<%= ICostantiMisuraAlternativa.CAMPO_SEDE_TDS%>');">
         			<img src="/images/filefolder.gif" border=0>
         		</a>
      </td>
 		<td class="l">Note</td>
        <td class="L">
              <TEXTAREA title="Note" name="<%=ICostantiMisuraAlternativa.CAMPO_NOTE_TDS%>"  cols=30 ></textarea>
     </td>
      
  </tr>      
</table>

<!-----------------------------Destinatari per la Notifica  (prima era Autorità Competente per territorio)-------------->    
     		...
     		<tr>
      			<td class="l">Sede </td><td class="L">
        			<input title="Sede Foro Avvocato" value="<%=StringUtils.toStringJSP(lAvv.getAvvocato().getForo())%>" type="text" name="<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>" maxlength="35" size="35">
        				<a href="Javascript:ListaComuni('LoadVariaMisuraAlternativaDetDom','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE %>[<%=lIdxAvv%>]');">
          					<img src="/images/filefolder.gif" border=0>
       					</a>
      			</td>
        		<td class="l">Note</td>
...
<script language="JavaScript" type="text/javascript">
  var frmvalidator  = new Validator("LoadVariaMisuraAlternativaDetDom");
