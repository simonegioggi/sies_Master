...
<html>
<head>
<title>[S.I.E.S.] - Gestione MA Rigetto </title>
...
<%}
 ...
 <%if(lPosizione.getCodPosizioneGiuridica() != null && !lPosizione.getCodPosizioneGiuridica().equals("03"))
   {%>
...
%>
 ...
 if(document.LoadInserisciMisuraAlternativa.<%= ICostantiMisuraAlternativa.CAMPO_COD_POLIZIA_E %>[document.LoadInserisciMisuraAlternativa.<%=ICostantiMisuraAlternativa.CAMPO_COD_POLIZIA_E%>.selectedIndex].value == '-')
      {
        alert("L'Autorità Competente è obbligatoria");
...
<table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
     <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp;
      <font class="campo">Inserimento Ordinanza Rigetto Misure Alternative</font>
     </td>
</tr>
</table>
 <br>
...
<FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciMisuraAlternativa">
  ...
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.misuraalternativa.action.ActInserisciRigetto">
...
<%------------------------------------------------------------------------------
------ VECCHIA GESTIONE (anche se su JBUILDER non sembra è tutto commentato) ---
------------------------------------------------------------------------------ -
 <tr>
...
<%
 ...
 <tr>
    ...
    <tr>
      <td class="l">Sede Ufficio Emittente<font class=ob>(*)</font></td><td class="l"  colspan="3"><font class="campo">
        <input Title="Luogo Ufficio Sorveglianza" name="<%= ICostantiMisuraAlternativa.CAMPO_SEDE_TDS_EMITT %>" value="<%=distretto%>" size=35 type="text">
      ...
      <%}else
     {%>
    <td class="l" colspan="3"> <font class="campo"> <%=StringUtils.toStringJSP(UfficioEmittente.getDescrTipoUfficio())%> DI <%=StringUtils.toStringJSP(UfficioEmittente.getDescrComune())%></font></td>
    <%}%>
    </tr>
<tr>
     ...
     <%
       ...
       {%>
        ...
        <td class="l"  colspan="3">
         <input type="hidden" name="<%=ICostantiEvento.CAMPO_COD_MOTIVO%>" value="<%=misuraalternativa.getCodTipoMisura()%>">
        <font class="campo"> <%=StringUtils.toStringJSP(misuraalternativa.getDescrTipoMisura())%></font>&nbsp;</td>
...
<input type="HIDDEN" name="<%=ICostantiMisuraAlternativa.CAMPO_ID_MISURA_ALTERNATIVA%>" value="<%=misuraalternativa.getIdMisuraAlternativa()%>">
       <td class="l"  colspan="3"><font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(misuraalternativa.getDataDecisione(),"dd"))%> -
          <%=StringUtils.toStringJSP(DateUtils.getDateToString(misuraalternativa.getDataDecisione(),"MM"))%> -
          <%=StringUtils.toStringJSP(DateUtils.getDateToString(misuraalternativa.getDataDecisione(),"yyyy"))%>
          </font>
...
%>
  ...
  <tr>
   ...
   <tr>
<!--autorità esterna e-->
          <td class="l" width ="30%">Destinatario per l'Esecuzione <font class=ob>(*)</font>
    ...
    <tr>
      <td class="Titolo" colspan=8>Destinatario per Notifica </td>
    </tr>
    <tr>
<%
      int lIdxAvv = 0;
