...
function eseguiAzione(tipoAzione, id) {
	if (tipoAzione == 'Dettaglio') {
		azione = "siap.siep.sanzionesostitutiva.action.ActVerificaStatoElencoBollettini";
		document.VerificaStatoBollettino.<%=ICostantiEvento.CAMPO_ID_EVENTO%>.value = id;
		document.VerificaStatoBollettino.<%=IWebConstants.ACTION_FIELD%>.value = azione;
		document.VerificaStatoBollettino.<%=IWebConstants.LINK_RITORNO%>.value = "<%=TornaQui%>";
		document.VerificaStatoBollettino.submit();
	}
}

function tornaIndietro(action) {
	document.VerificaStatoBollettino.<%=IWebConstants.ACTION_FIELD%>.value = action;
	document.VerificaStatoBollettino.submit();
}
</script>
</head>
...
<br>
<FORM action="<%=IWebConstants.PG_MAIN%>" method="post" name="VerificaStatoBollettino">
...
%>
	<tr>
		<td class="titolo" colspan="5">Richiesta Generazione Bollettini PagoPA relativi a:</td>
	</tr>
	<tr><td>&nbsp;</td></tr>
	<tr>
