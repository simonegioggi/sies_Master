...
<jsp:useBean id="ListaEspulsione" 			scope="request" class="java.util.Vector"/>

<%
//==================================================================================================================
// Form per la visualizzazione dell'elenco di Provvedimenti di Espulsione legate a un certo Titolo.
//
// La form presenta un elenco dei Provvedimenti di Espulsioni già presenti con la possibilità di 
...
<html>
<head>
  <title> Elenco Provvedimenti di Espulsione </title>
  ...
  <script language="JavaScript">
    ...
    function eseguiAzione(aTipoAzione, aIdStat, aIdComp, aStato, aMotivoModifica)
    {
      if (aTipoAzione=='Dettaglio'){
        lAzione = "siap.siep.modulocumulo.action.ActDettaglioEspulsioneCumulo";
        document.ListaEspulsione.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaEspulsione.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaEspulsione.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaEspulsione.submit();
      }
      else if (aTipoAzione=='Modifica'){
        lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciEspulsioneCumulo";
        document.ListaEspulsione.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaEspulsione.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaEspulsione.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaEspulsione.modalita.value = 'M';
        ...
        if (window.confirm(msgConfirm)) {
          lAzione = "siap.siep.modulocumulo.action.ActInserisciEspulsioneCumulo";
          document.ListaEspulsione.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;

          document.ListaEspulsione.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
          document.ListaEspulsione.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
          document.ListaEspulsione.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_FLAG_STATO%>.value = aStato;
          document.ListaEspulsione.modalita.value = 'C';
    ...
    function nuovoProvvedimento(){
      lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciEspulsioneCumulo";
      document.ListaEspulsione.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
      document.ListaEspulsione.modalita.value = 'I';
      document.ListaEspulsione.submit();
    }
    ...
    {
      document.ListaEspulsione.<%=IWebConstants.ACTION_FIELD%>.value = action;
      document.ListaEspulsione.submit();
    }
  ...
  <table>
    <tr>
      ...
      <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp;
                      <font class="campo">Elenco Provvedimenti di Espulsione &nbsp;</font>
      </td>
...
<FORM action="<%=IWebConstants.PG_MAIN%>" method="post" name="ListaEspulsione">
  ...
  <table cellspacing="2" cellpadding="2" align="center" width="95%">
    ...
    <%
      ...
      Iterator itx = ListaEspulsione.iterator();
      while ( itx.hasNext()) {
