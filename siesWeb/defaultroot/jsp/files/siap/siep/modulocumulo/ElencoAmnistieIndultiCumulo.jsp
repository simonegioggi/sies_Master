...
<jsp:useBean id="ListaAmnistieIndulti" scope="request" class="java.util.Vector"/>

<%
//==============================================================================
// Form per la visualizzazione delle Amnistie / Cumuli legati a un certo Titolo.
//
...
<html>
<head>
  <title> Elenco Provvedimenti di Amnistia / Indulto</title>
  ...
  <script language="JavaScript">
    ...
    function eseguiAzione(aTipoAzione, aIdStat, aIdComp, aStato, aMotivoModifica)
    {
      if (aTipoAzione=='Dettaglio'){
        lAzione = "siap.siep.modulocumulo.action.ActDettaglioAmnistiaIndultoCumulo";
        document.ListaAmnistieIndulti.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaAmnistieIndulti.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaAmnistieIndulti.submit();
      }
      else if (aTipoAzione=='Modifica'){
        lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciAmnistiaIndultoCumulo";
        document.ListaAmnistieIndulti.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaAmnistieIndulti.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaAmnistieIndulti.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaAmnistieIndulti.modalita.value='M';
        document.ListaAmnistieIndulti.submit();
      }
        ...
        if (window.confirm(msgConfirm)) {
          ...
          document.ListaAmnistieIndulti.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
          document.ListaAmnistieIndulti.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
          document.ListaAmnistieIndulti.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_FLAG_STATO%>.value = aStato;
          document.ListaAmnistieIndulti.modalita.value = "C";
    ...
    function nuovaAnnotazione(){
      lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciAmnistiaIndultoCumulo";
      document.ListaAmnistieIndulti.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
      document.ListaAmnistieIndulti.modalita.value = "I";
      document.ListaAmnistieIndulti.submit();
    }
    ...
    {
      document.ListaAmnistieIndulti.<%=IWebConstants.ACTION_FIELD%>.value = action;
      document.ListaAmnistieIndulti.submit();
    }
  ...
  <table>
    <tr>
      ...
      <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp;
                      <font class="campo">Elenco Provvedimenti di Amnistia / Indulto &nbsp;</font>
      </td>
...
<FORM action="<%=IWebConstants.PG_MAIN%>" method="post" name="ListaAmnistieIndulti">
  ...
  <table cellspacing="2" cellpadding="2" align="center" width="95%">
   ...
   <%
      ...
      Iterator itx = ListaAmnistieIndulti.iterator();
      while ( itx.hasNext()) {
    ...
    <% if (ListaAmnistieIndulti.size()==0){ %>
    <tr>
