...
//============================================================================== 
// Form per l'inserimento e la modifica dei provv. di Espulsione - 
...
<html>
<head>
  <title> Gestione Provvedimento Espulsione </title>
  ...
  <script language="JavaScript" >
    ...
    function Verify()
    {
        ...
        if ($.inArray(codOggetto, arrayUDS) >-1) {
        	// Controllo data decisione
	        if (document.f.<%=ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>[0].value.length==1)
	            document.f.<%=ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>[0].value='0'+document.f.<%=ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>[0].value;
	        if (document.f.<%=ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>[0].value.length==1)
	            document.f.<%=ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>[0].value='0'+document.f.<%=ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>[0].value;
	
	        var data_to_verify = document.f.<%=ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>[0].value+'/'+document.f.<%=ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>[0].value+'/'+document.f.<%=ICostantiMisuraAlternativa.CAMPO_ANNO_DATA_DECISIONE%>[0].value;
	
	        if (!ControllaDataPassaVuota(data_to_verify) ||
	        	data_to_verify.length < 10 )
	        {
	          alert('Data Decisione non valida');
	          document.f.<%=ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>[0].focus();
	
	          return false;
	        }

        	// Controllo dei quantum di espulsione non valorizzati.
        	if (document.f.<%=ICostantiMisuraAlternativa.CAMPO_NUM_ANNI_MISURA%>.value.length==0    &&
			    document.f.<%=ICostantiMisuraAlternativa.CAMPO_NUM_MESI_MISURA%>.value.length==0    &&
			    document.f.<%=ICostantiMisuraAlternativa.CAMPO_NUM_GIORNI_MISURA%>.value.length==0 )
	        {
	          alert('Valorizzare i quantum di rinvio');
	          document.f.<%=ICostantiMisuraAlternativa.CAMPO_NUM_ANNI_MISURA %>.focus();
	          return false;
	        }
        ...
        if ($.inArray(codOggetto, arrayTDS) >-1) {
        	// Controllo data decisione
	        if (document.f.<%=ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>[1].value.length==1)
	            document.f.<%=ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>[1].value='0'+document.f.<%=ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>[1].value;
	        if (document.f.<%=ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>[1].value.length==1)
	            document.f.<%=ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>[1].value='0'+document.f.<%=ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>[1].value;
	
	        var data_to_verify = document.f.<%=ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>[1].value
	                        +'/'+document.f.<%=ICostantiMisuraAlternativa.CAMPO_MESE_DATA_DECISIONE%>[1].value
	                        +'/'+document.f.<%=ICostantiMisuraAlternativa.CAMPO_ANNO_DATA_DECISIONE%>[1].value;
	
	        if (!ControllaDataPassaVuota(data_to_verify) ||
	        	data_to_verify.length < 10 )
	        {
	          alert('Data Decisione non valida');
	          document.f.<%=ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>[1].focus();
  ...
  <table>
    <tr>
      ...
      <td class="LBG">
        ...
        <font class="campo">Inserimento Espulsione &nbsp;</font>
        <% } else if( modalita.equals("M") || modalita.equals("NP") ) { %>
        <font class="campo">Modifica Espulsione &nbsp;</font>
        <%}%>
      </td>
      <td class="LBG">
        <a href="javascript:eseguiFunzione('siap.siep.modulocumulo.action.ActRicercaEspulsioneCumulo')">
...
<FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name="f">
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.modulocumulo.action.ActInserisciEspulsioneCumulo">
  ...
  <input type="HIDDEN" name="<%=ICostantiComputiCumulo.CAMPO_COD_TIPO_MISURA%>" >
  ...
  <table width="95%" align="center">
	...
	<div id="divUDS" style="display:block;">
	  <table width="95%" align="center" >
  		...
  		<tr>
		    <td class="l" width="240" >
		        Data Emissione Decreto <font class=ob>(*)</font></td>

		    <td class="l" colspan="3">
		        <input type="text" Title="Giorno data Emissione Decreto" 
		          			name="<%=ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_DECISIONE%>" maxlength="2" size="2"  
		          			value="<%=StringUtils.toStringJSP (DateUtils.getDateToString(aComputo.getDataEmissioneProvv(),"dd"))%>"  
		          			<%=IWebConstants.UTIL_DATA%>> /
		        <input type="text" Title="Mese data Emissione Decreto"
							...
							<%=IWebConstants.UTIL_DATA%>> /
		        <input type="text" Title="Anno data Emissione Decreto" 
		          			name="<%=ICostantiMisuraAlternativa.CAMPO_ANNO_DATA_DECISIONE%>" maxlength="4" size="4"  
	   ...
	   <tr>
	    ...
	    <td class="l" colspan="3">
	      <textarea cols="60" rows="2" name="<%=ICostantiMisuraAlternativa.CAMPO_NOTE%>"><%=StringUtils.toStringJSP( aComputo.getNote() )%></textarea>
	    </td>
	  </tr>
	...
	<div id="divTDS" style="display:block;">
	  <table width="95%" align="center" >
  		...
  		<tr>
		    <td class="l" width="240" >
		        Data Emissione Ordinanza <font class=ob>(*)</font></td>
