...
<jsp:useBean id="ListaRichiesteBeneficiGE" scope="request" class="java.util.Vector"/>

<%
//==============================================================================
// Form per la visualizzazione delle Richieste al GE dei benefici
// Amnistia/Indulto/Depenalizzazione/Incostituzionalita'
...
<html>
<head>
  <title> Elenco Provvedimenti Richieste al GE Applicazione/Revoca Benefici</title>
  ...
  <script language="JavaScript">
    ...
    function eseguiAzione(aTipoAzione, aIdStat, aIdComp, aStato, aMotivoModifica)
    {
      if (aTipoAzione=='Dettaglio'){
        lAzione = "siap.siep.modulocumulo.action.ActDettaglioRichBenGECumulo";
        document.ListaRichBeneficiGEi.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaRichBeneficiGEi.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaRichBeneficiGEi.submit();
      }
      else if (aTipoAzione=='Modifica'){
        lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciRichBenGECumulo";
        document.ListaRichBeneficiGEi.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaRichBeneficiGEi.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaRichBeneficiGEi.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaRichBeneficiGEi.modalita.value='M';
        document.ListaRichBeneficiGEi.submit();
      }
        ...
        if (window.confirm(msgConfirm)) {
          lAzione = "siap.siep.modulocumulo.action.ActInserisciRichBenGECumulo";
          document.ListaRichBeneficiGEi.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;

          document.ListaRichBeneficiGEi.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
          document.ListaRichBeneficiGEi.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
          document.ListaRichBeneficiGEi.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_FLAG_STATO%>.value = aStato;
          document.ListaRichBeneficiGEi.modalita.value = "C";
    ...
    function nuovaAnnotazione(){
      lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciRichBenGECumulo";
      document.ListaRichBeneficiGEi.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
      document.ListaRichBeneficiGEi.modalita.value = "I";
      document.ListaRichBeneficiGEi.submit();
    }
    ...
    {
      document.ListaRichBeneficiGEi.<%=IWebConstants.ACTION_FIELD%>.value = action;
      document.ListaRichBeneficiGEi.submit();
    }
  ...
  <table>
    <tr>
      ...
      <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp;
                      <font class="campo">Elenco Provvedimenti Richieste al GE Amnistia/Indulto/Depenelizzazione/Incostituzionalita' &nbsp;</font>
      </td>
...
<FORM action="<%=IWebConstants.PG_MAIN%>" method="post" name="ListaRichBeneficiGEi">
  ...
  <table cellspacing="2" cellpadding="2" align="center" width="95%">
    <tr>
      ...
      <td class="int" title="Anticipazione degli effetti">Anticip.</td>
   ...
   <%
      ...
      Iterator itx = ListaRichiesteBeneficiGE.iterator();
      while ( itx.hasNext()) {
...
if (lListaComputi!=null) {        
    ...
    <tr>
      ...
      <td class="l"><%=StringUtils.toStringJSP(lComputo.getDescrTipoAnnotazione())%></td>
	  ...
	  <td class="c" title="Anticipazione degli effetti"><img src="/images/V.gif"></td>
	  <%  } else { %> 
	  <td class="c" title="Anticipazione degli effetti">&nbsp;</td>
	  <%  } %> 
...
<% } else { %> 
<%-- Vecchi cumulo che non estraevano i computi --%>
    <tr>
        <td class="l"><%=StringUtils.toStringJSP(lStatoEsec.getDescrTipoProvvedimento())+" "+StringUtils.toStringJSP(lStatoEsec.getDescrMotivo())%></td>
        <td class="l" colspan="7">&nbsp;Dettaglio dei computi non disponibile</td>
        <td class="c" title="<%=lDescStato%>">&nbsp;<%=lStato%></td>
      <td class="c" style="text-align:center"> &nbsp;
      <%
        //======================================================================
      ...
      %>
        ...
        //======================================================================
        if(IstruttoriaCumulo.getFlagStato().equals("A") && !lStatoEsec.getFlagStato().equals("C")){ %>
        <a href="javascript:eseguiAzione('Modifica',<%=lStatoEsec.getIdStatoEsecTitoloCumulato() %>,'')">
          <img src="/images/modifica.gif" width="12" height="12" alt="Modifica" border="0"></a>
        <a href="javascript:eseguiAzione('Cancella',<%=lStatoEsec.getIdStatoEsecTitoloCumulato() %>,'','<%=lStatoEsec.getFlagStato() %>','<%=StringUtils.toStringJSP(lStatoEsec.getMotivoModifica(),"") %>')">
          <img src="/images/delete.gif" width="12" height="12" alt="Elimina" border="0"></a>
        <% } %>
      </td>        
    </tr>
<% } %>    
    ...
    <% if (ListaRichiesteBeneficiGE.size()==0){ %>
    <tr>
    ...
    <% if (isRevoche){ %>
    <tr>
      <td colspan="3"><font class="cRosso">I dati evidenziati in rosso si riferiscono a benefici revocati</font></td>
    </tr>
    <% } %>
