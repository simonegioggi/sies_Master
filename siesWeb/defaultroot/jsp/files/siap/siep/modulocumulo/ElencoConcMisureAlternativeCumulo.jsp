...
<jsp:useBean id="ListaConcMisureAlt"   scope="request" class="java.util.Vector"/>

<%
//====================================================================================================
// Form per la visualizzazione dell'elenco di Concessioni Misure Alternative legate a un certo Titolo.
//
...
<html>
<head>
  <title> Elenco Provvedimenti di Concessione Misura Alternativa </title>
  ...
  <script language="JavaScript">
    ...
    function eseguiAzione(aTipoAzione, aIdStat, aIdComp, aStato, aMotivoModifica)
    {
      if (aTipoAzione=='Dettaglio'){
        lAzione = "siap.siep.modulocumulo.action.ActDettaglioConcMisuraAlternativaCumulo";
        document.ListaConcMisureAlt.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaConcMisureAlt.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaConcMisureAlt.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaConcMisureAlt.submit();
      }
      else if (aTipoAzione=='Modifica'){
        lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciConcMisuraAlternativaCumulo";
        document.ListaConcMisureAlt.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaConcMisureAlt.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaConcMisureAlt.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaConcMisureAlt.modalita.value = 'M';
        ...
        if (window.confirm(msgConfirm)) {
          lAzione = "siap.siep.modulocumulo.action.ActInserisciConcMisuraAlternativaCumulo";
          document.ListaConcMisureAlt.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;

          document.ListaConcMisureAlt.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
          document.ListaConcMisureAlt.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
          document.ListaConcMisureAlt.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_FLAG_STATO%>.value = aStato;
          document.ListaConcMisureAlt.modalita.value = 'C';
    ...
    function nuovoProvvedimento(){
      lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciConcMisuraAlternativaCumulo";
      document.ListaConcMisureAlt.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
      document.ListaConcMisureAlt.modalita.value = 'I';
      document.ListaConcMisureAlt.submit();
    }
    ...
    {
      document.ListaConcMisureAlt.<%=IWebConstants.ACTION_FIELD%>.value = action;
      document.ListaConcMisureAlt.submit();
    }
  ...
  <table>
    <tr>
      ...
      <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp;
                      <font class="campo">Elenco Provvedimenti di Concessione Misura Alternativa &nbsp;</font>
      </td>
...
<FORM action="<%=IWebConstants.PG_MAIN%>" method="post" name="ListaConcMisureAlt">
  ...
  <table cellspacing="2" cellpadding="2" align="center" width="95%">
    ...
    <%
      ...
      Iterator itx = ListaConcMisureAlt.iterator();
      while ( itx.hasNext()) {
