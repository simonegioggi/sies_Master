...
<html>
<head>
<title> Gestione Richieste Applicazione Benefici</title>
...
<script language="JavaScript" >
	function checkInteroTitolo (checkObject) {
      	var jQueryObj = $(checkObject);
      	var idTitolo = jQueryObj.val();

      	if (jQueryObj.prop('checked')){
       		$("input[type=checkbox][id="+idTitolo+"]").prop('checked',false);
        	$("input[type=checkbox][id="+idTitolo+"]").prop('disabled',true);
      	} else {
        	$("input[type=checkbox][id="+idTitolo+"]").prop('disabled',false);
      	}
	}
...
%>
    ...
    function Verify() { 
      ...
      // Controllo sulla selezione dei checkBox relativa alla MisuraSicurezza
      var SpuntaMisura = "NO";
      for (var i = 0; i < document.getElementsByName("idMisuraSel").length; i++ )
      {
        //alert ("idReatoSelezionato = "+document.getElementsByName("idMisuraSel")[i].checked);
        if(document.getElementsByName("idMisuraSel")[i].checked )
        { 
          SpuntaMisura = "SI";
        }
      } 
      ...
      // Controllo sulla selezione dei checkBox relativa alla PenaAccessoria
      var SpuntaPena = "NO";
      for (var i = 0; i < document.getElementsByName("idPenaAccSel").length; i++ )
      {
        //alert ("idReatoSelezionato = "+document.getElementsByName("idPenaAccSel")[i].checked);
        if(document.getElementsByName("idPenaAccSel")[i].checked )
        { 
          SpuntaPena = "SI";
        }
      }       
      ...
      alert("SpuntaMisura = "+SpuntaMisura);
      alert("SpuntaPena = "+SpuntaPena);
      */
      
      // Controllo che ci sia almeno un check selezionato
      if(SpuntaTitolo == "NO" && SpuntaReato == "NO" && SpuntaMisura == "NO" && SpuntaPena == "NO" )
      {
        alert("Attenzione: selezionare almeno un Elemento nella form \n(Titolo o Reato o Mis.Sic. o Pena Acc.)");
        return false; //PER TEST ESCLUDO 
      }
     ...
     // Quantum
      ...
      // 10/04/2019 Segnalazione da collaudo MEV42 (Bisogna obbligare l'inserimento del segno solo se è selezionato almeno un checkBox non di Pena Accessoria)
      var titoloCompletoSel = $('input[type=checkbox][id=selTitoloCompleto]:checked');
      var reatoSel = $('input[type=checkbox][id=selReato]:checked');
      var misuraSel = $('input[type=checkbox][id=selMisura]:checked');
      
      if(document.formName.<%=ICostantiRichiestePmInCumulo.CAMPO_FLAG_PIU_MENO_R%>.value == ''	&&  
		(titoloCompletoSel.size()>0	||
		 reatoSel.size()>0			||
		 misuraSel.size()>0			 ) )
      {
  ...
  <table>
    <tr>
      ...
      <td class="LBG">
        ...
        <font class="campo">Inserimento Richiesta Applicazione Beneficio &nbsp;</font>
...
<%
  ...
  { %>  
    ...
    <%-- input type="HIDDEN" name="<%=ICostantiRichiestePmInCumulo.CAMPO_FLAG_STATO %>"           value="<%=StringUtils.toStringJSP(RichiestaAlGE.getf) %>" --%>
<%} %>  
...
%>        
        		...
        		<td class="c">
          			<input type="checkbox" id="selTitoloCompleto" name="idTitoloCompleto" value="<%=StringUtils.toStringJSP(lTitolo.getIdTitoloCumulato()) %>" onClick="Javascript:checkInteroTitolo(this);">                 
        		</td>
			</tr>
		</table>
<%  //  ------------    REATI   --------------
...
<%
    ...
    for (int kr = 0; kr < lTitolo.getReatoCircostanzaCumulo().size(); kr++) {
		...
		if (lReato.getAnnoFonte() != null
				&& !lReato.getAnnoFonte().toString().equals("")
...
<%  
				boolean lFlagAnnoNumeroCirc = false;
             	if (lCirc.getAnnoFonte() != null
             			&& !lCirc.getAnnoFonte().toString().equals("")
  ...
  %>      
     ...
     <font class="campo"><%=StringUtils.toStringJSP(lStringPenaReato ,"")%></font>&nbsp; 
      ...
      <font class="campo"><%=StringUtils.toStringJSP(lStringaSanzione ,"")%></font>&nbsp; 
        ...
        <td class="c">
          <input type="checkbox" name="idReatoSelezionato" id="selReato"
                 value="<%=StringUtils.toStringJSP(lReato.getIdReatoCum() ) %>" 
                 id="<%=StringUtils.toStringJSP(lTitolo.getIdTitoloCumulato()) %>"
                 checked>
        </td>
      </tr>
...
<% // -------     MISURA di SICUREZZA   -------------------
  ...
  {     %>
    <table width="95%" align="center">
      <tr>
        <td class="titolo" width="90%">Misura di Sicurezza</td>
        <td class="titolo" width="5%">sel.</td>
      </tr>          
<%    for(int km = 0; km < lTitolo.getMisureSicurezzaCumulo().size(); km++ )
	...
	<tr>
        <td class="l">
        	<input type="hidden" name="<%=ICostantiMisuraSicurezzaCumulo.CAMPO_ID_MISURA_SICUREZZA_CUMULO %>" value="<%=StringUtils.toStringJSP(lMisura.getIdMisuraSicurezzaCumulo()) %>" >
         &nbsp;&nbsp;<font class="label"> MISURA: </font>
         &nbsp;<font class="campoLow"><%=StringUtils.toStringJSP(lMisura.getDescrTipo(),"") %></font>&nbsp;
         
         <font class="label"> per la durata di </font>
          Anni&nbsp;<font class="campoLow"><%=StringUtils.toStringJSP(lMisura.getNumAnni(), "0") %></font>&nbsp;&nbsp;
          Mesi&nbsp;<font class="campoLow"><%=StringUtils.toStringJSP(lMisura.getNumMesi(), "0") %></font>&nbsp;&nbsp;
          Giorni&nbsp;<font class="campoLow"><%=StringUtils.toStringJSP(lMisura.getNumGiorni(), "0") %></font>&nbsp;&nbsp;
        
        </td>
        <td class="c">
          <input type="checkbox" id="selMisura"
                 name="idMisuraSel" 
                 id="<%=StringUtils.toStringJSP(lTitolo.getIdTitoloCumulato()) %>"
                 value="<%=StringUtils.toStringJSP(lMisura.getIdMisuraSicurezzaCumulo()) %>" >                 
        </td>
      </tr>
   <% } %>  
    </table>
...
<%  // -----------------------  PENE ACCESSORIE -----------------------------------------  
  ...
  { %>
    <table width="95%" align="center">
      <tr>
        <td class="titolo" width="90%">Pena Accessoria</td>
        <td class="titolo" width="5%">sel.</td>
      </tr>
<%    for(int kp = 0; kp < lTitolo.getPeneAccessorieCumulo().size(); kp++ )
	...
	<tr>
		<td class="l">
        	<input type="hidden" name="<%=ICostantiPenaAccessoriaCumulo.CAMPO_ID_PENA_ACCESSORIA_CUMULO %>" value="<%=StringUtils.toStringJSP(lPenAcc.getIdPenaAccessoriaCumulo()) %>" >
         &nbsp;&nbsp;<font class="label"> PENA ACCESSORIA: </font>
         ...
         <% } else { %>
         <font class="label"> per la durata di </font>
          Anni&nbsp;<font class="campoLow"><%=StringUtils.toStringJSP(lPenAcc.getNumAnni(), "0") %></font>&nbsp;&nbsp;
          Mesi&nbsp;<font class="campoLow"><%=StringUtils.toStringJSP(lPenAcc.getNumMesi(), "0") %></font>&nbsp;&nbsp;
          Giorni&nbsp;<font class="campoLow"><%=StringUtils.toStringJSP(lPenAcc.getNumGiorni(), "0") %></font>&nbsp;&nbsp;
         <% } %>
        </td>
        <td class="c">
          <input type="checkbox" 
                 name="idPenaAccSel" 
                 id="<%=StringUtils.toStringJSP(lTitolo.getIdTitoloCumulato()) %>"
                 value="<%=StringUtils.toStringJSP(lPenAcc.getIdPenaAccessoriaCumulo()) %>">                 
        </td>
      </tr>
<%    }   %>
