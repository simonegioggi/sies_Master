...
<jsp:useBean id="ListaDifferimentoPena" 	scope="request" class="java.util.Vector"/>

<%
//==================================================================================================================
// Form per la visualizzazione dell'elenco di Provvedimenti di Differimento pena legate a un certo Titolo.
//
// La form presenta un elenco dei Provvedimenti di Differimento pena già presenti con la possibilità di 
...
<html>
<head>
  <title> Elenco Provvedimenti di Differimento Pena </title>
  ...
  <script language="JavaScript">
    ...
    function eseguiAzione(aTipoAzione, aIdStat, aIdComp, aStato, aMotivoModifica)
    {
      if (aTipoAzione=='Dettaglio'){
        lAzione = "siap.siep.modulocumulo.action.ActDettaglioDifferimentoPenaCumulo";
        document.ListaDifferimentoPena.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaDifferimentoPena.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaDifferimentoPena.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaDifferimentoPena.submit();
      }
      else if (aTipoAzione=='Modifica'){
        lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciDifferimentoPenaCumulo";
        document.ListaDifferimentoPena.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaDifferimentoPena.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaDifferimentoPena.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaDifferimentoPena.modalita.value = 'M';
        ...
        if (window.confirm(msgConfirm)) {
          lAzione = "siap.siep.modulocumulo.action.ActInserisciDifferimentoPenaCumulo";
          document.ListaDifferimentoPena.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;

          document.ListaDifferimentoPena.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
          document.ListaDifferimentoPena.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
          document.ListaDifferimentoPena.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_FLAG_STATO%>.value = aStato;
          document.ListaDifferimentoPena.modalita.value = 'C';
    ...
    function nuovoProvvedimento(){
      lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciDifferimentoPenaCumulo";
      document.ListaDifferimentoPena.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
      document.ListaDifferimentoPena.modalita.value = 'I';
      document.ListaDifferimentoPena.submit();
    }
    ...
    {
      document.ListaDifferimentoPena.<%=IWebConstants.ACTION_FIELD%>.value = action;
      document.ListaDifferimentoPena.submit();
    }
  ...
  <table>
    <tr>
      ...
      <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp;
                      <font class="campo">Elenco Provvedimenti di Differimento Pena &nbsp;</font>
      </td>
...
<FORM action="<%=IWebConstants.PG_MAIN%>" method="post" name="ListaDifferimentoPena">
  ...
  <table cellspacing="2" cellpadding="2" align="center" width="95%">
    ...
    <%
      ...
      Iterator itx = ListaDifferimentoPena.iterator();
      while ( itx.hasNext()) {
    ...
    <% if (ListaDifferimentoPena.size()==0){ %>
    <tr>
