...
<jsp:useBean id="ListaDepenalizzazioni" scope="request" class="java.util.Vector"/>

<%
//==============================================================================
// Form per la visualizzazione delle Depenalizzazioni legate a un certo Titolo.
//
//
...
<html>
<head>
  <title> Elenco Provvedimenti di Depenalizzazione</title>
  ...
  <script language="JavaScript">
    ...
    function eseguiAzione(aTipoAzione, aIdStat, aIdComp, aStato, aMotivoModifica)
    {
      if (aTipoAzione=='Dettaglio'){
        lAzione = "siap.siep.modulocumulo.action.ActDettaglioDepenalizzazioneCumulo";
        document.ListaDepenalizzazioni.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaDepenalizzazioni.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaDepenalizzazioni.submit();
      }
      else if (aTipoAzione=='Modifica'){
        lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciDepenalizzazioneCumulo";
        document.ListaDepenalizzazioni.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaDepenalizzazioni.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaDepenalizzazioni.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaDepenalizzazioni.modalita.value='M';
        document.ListaDepenalizzazioni.submit();
      }
        ...
        if (window.confirm(msgConfirm)) {
          lAzione = "siap.siep.modulocumulo.action.ActInserisciDepenalizzazioneCumulo";
          document.ListaDepenalizzazioni.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;

          document.ListaDepenalizzazioni.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
          document.ListaDepenalizzazioni.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
          document.ListaDepenalizzazioni.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_FLAG_STATO%>.value = aStato;
          document.ListaDepenalizzazioni.modalita.value = "C";
    ...
    function nuovaAnnotazione(){
      lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciDepenalizzazioneCumulo";
      document.ListaDepenalizzazioni.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
      document.ListaDepenalizzazioni.modalita.value = "I";
      document.ListaDepenalizzazioni.submit();
    }
    ...
    {
      document.ListaDepenalizzazioni.<%=IWebConstants.ACTION_FIELD%>.value = action;
      document.ListaDepenalizzazioni.submit();
    }
  ...
  <table>
    <tr>
      ...
      <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp;
                      <font class="campo">Elenco Provvedimenti di Depenalizzazione &nbsp;</font>
      </td>
...
<FORM action="<%=IWebConstants.PG_MAIN%>" method="post" name="ListaDepenalizzazioni">
  ...
  <table cellspacing="2" cellpadding="2" align="center" width="95%">
   ...
   <%
      ...
      Iterator itx = ListaDepenalizzazioni.iterator();
      while ( itx.hasNext()) {
    ...
    <% if (ListaDepenalizzazioni.size()==0){ %>
    <tr>
