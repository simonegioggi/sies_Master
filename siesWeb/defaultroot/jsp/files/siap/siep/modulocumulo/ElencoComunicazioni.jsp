...
<html>
  <head>
    <title>[S.I.E.S.] - Elenco Comunicazioni </title>
	...
	if (existFC == 'false') {
	  	...
	  	if (window.confirm('Confermi la cancellazione ?')) {
	  		if (documentoRegistrato=="S") {
	  			var  desktop = window.open("<%= IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=" + a_action + "&IdIstruttoriaCumulo="+"<%=IstruttoriaCumulo.getIdIstruttoriaCumulo()%>" + "&nextAction="+"<%=nextAction%>" +"&idEventoCumulo="+"<%=ProvvedimentoCumulo.getIdEvento()%>" + "&" + a_parameter + "=" +a_entityname +"&lOrdinamento="+valoreordinamento, "Cancella_provvedimento","  top="+eval(screen.height/2-200/2)+",left="+eval(screen.width/2-450/2)+", toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=200");
	         	window.parent.close();
	    	} else {
	    		str = "/jsp/Main.jsp?Action=siap.siep.ordineesecuzione.action.ActCancellaProvvedimento&nextAction="+"<%=nextAction%>"+"&IdIstruttoriaCumulo="+"<%=IstruttoriaCumulo.getIdIstruttoriaCumulo()%>" +"&idEventoCumulo="+"<%=ProvvedimentoCumulo.getIdEvento()%>" + "&" +a_parameter +"=" + a_entityname+"&lOrdinamento=<%=ordinamento%>";
...
function pr()
{
  ...
  <table cellspacing=2 cellpadding=2>
    <tr>
      <td class="titolo" colspan="6">Provvedimenti di Comunicazione Emessi</td>
...
<%
 ...
 String modificabile = "";
   if (UtenteConnesso.getUfficioUtente().getCodUfficio().equals(fascicolo.getChiaveUfficio()))
      ...
      {modificabile = "NO";}
%>
        <jsp:include page="/jsp/files/siap/siep/modulocumulo/buttonsComunicazioni.jsp">
