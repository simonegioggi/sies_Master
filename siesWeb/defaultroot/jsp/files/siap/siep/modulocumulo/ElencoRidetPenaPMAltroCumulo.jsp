...
<jsp:useBean id="ListaRidetPena" scope="request" class="java.util.Vector" />

<%
//================================================================================================
// Form per la visualizzazione delle Annotazioni di Ridet. Pena PM Altro legati a un certo Titolo.
//
...
<html>
<head>
<title>Elenco Provvedimenti di Rideterminazione Pena PM Altro</title>
...
<script language="JavaScript">
    ...
    function eseguiAzione(aTipoAzione, aIdStat, aIdComp, aStato, aMotivoModifica)
    {
      if (aTipoAzione=='Dettaglio'){
        lAzione = "siap.siep.modulocumulo.action.ActDettaglioRidetPenaPMAltroCumulo";
        document.ListaRidetPena.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaRidetPena.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaRidetPena.submit();
      }
      else if (aTipoAzione=='Modifica'){
        lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciRidetPenaPMAltroCumulo";
        document.ListaRidetPena.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaRidetPena.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaRidetPena.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaRidetPena.modalita.value='M';
        document.ListaRidetPena.submit();
      }
        ...
        if (window.confirm(msgConfirm)) {
          ...
          document.ListaRidetPena.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
          document.ListaRidetPena.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
          document.ListaRidetPena.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_FLAG_STATO%>.value = aStato;
          document.ListaRidetPena.modalita.value = "C";
    ...
    function nuovaAnnotazione(){
      lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciRidetPenaPMAltroCumulo";
      document.ListaRidetPena.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
      document.ListaRidetPena.modalita.value = "I";
      document.ListaRidetPena.submit();
    }
    ...
    {
      document.ListaRidetPena.<%=IWebConstants.ACTION_FIELD%>.value = action;
      document.ListaRidetPena.submit();
    }
...
<body class="corpo">
	<table>
		<tr>
			...
			<td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp;
				<font class="campo">Elenco Provvedimenti di Rideterminazione Pena PM Altro &nbsp;</font></td>
			<td class="LBG">
				<!-- Tasto indietro alla Griglia dei dati analitici --> <a
				href="javascript:tornaIndietro('siap.siep.modulocumulo.action.ActLoadGrigliaDatiAnalitici')">
					<img align="middle"
	...
	<FORM action="<%=IWebConstants.PG_MAIN%>" method="post"
		name="ListaRidetPena">
		...
		<input type="hidden"
			...
			value="<%=TitoloInCumulo.getIdTitoloCumulato()%>">
  ...
  %>
		<table cellspacing="2" cellpadding="2" align="center" width="95%">
			<tr>
				...
				<td class="int"
					title="Indica la natura dei dati rispetto a quelli importati">Stato</td>
			...
			<%
      int id_record = 0;
      boolean isRevoche = false; 
      Iterator itx = ListaRidetPena.iterator();
      while ( itx.hasNext()) {
        ...
        while ( itxComputi.hasNext()) 
        {
          ...
          if (!lComputo.isQuantumReclusioneZero()){
            lComputo.calcolaStringaReclusione();
            lStringaReclusione = lComputo.getStringaReclusione();
          }
          
          String lStringaArresto = "&nbsp;";
          if (!lComputo.isQuantumArrestoZero()){
            lComputo.calcolaStringaArresto();
            lStringaArresto = lComputo.getStringaArresto();
          }          
    %>
			<tr>
				<% if (contaComputi==1) {%>
				...
				<td class="c" nowrap><%=StringUtils.toStringJSP(DateUtils.getDateToString(lStatoEsec.getDataEmissione(),"dd-MM-yyyy"))%></td>
				<% } else {%>
				<td class="c"></td>
				<td class="c"></td>
				<% } %>
				<td class="<%=lClassComputi%>"><%=lStringaReclusione%></td>
				<td class="<%=lClassComputi%>"><%=StringUtils.toStringJSP(lComputo.getImportoMulta() )%>&nbsp;</td>
				<td class="<%=lClassComputi%>"><%=lStringaArresto%></td>
				<td class="<%=lClassComputi%>"><%=StringUtils.toStringJSP(lComputo.getImportoAmmenda() )%>&nbsp;</td>
        ...
        //======================================================================
      %> <a
					href="javascript:eseguiAzione('Dettaglio',<%=lStatoEsec.getIdStatoEsecTitoloCumulato() %> )">
						<img src="/images/dettagli.gif" width="12" height="12"
						alt="Dettaglio Misura" border="0"></a> <% 
        //======================================================================
        ...
        if(IstruttoriaCumulo.getFlagStato().equals("A") && !lStatoEsec.getFlagStato().equals("C")){ %>
					<a
					href="javascript:eseguiAzione('Modifica',<%=lStatoEsec.getIdStatoEsecTitoloCumulato() %>,<%=lComputo.getIdComputiCumulo() %>)">
						<img src="/images/modifica.gif" width="12" height="12"
						alt="Modifica" border="0"></a> 
        <a
					href="javascript:eseguiAzione('Cancella',<%=lStatoEsec.getIdStatoEsecTitoloCumulato() %>,<%=lComputo.getIdComputiCumulo() %>,'<%=lStatoEsec.getFlagStato() %>','<%=StringUtils.toStringJSP(lStatoEsec.getMotivoModifica(),"") %>')">
						<img src="/images/delete.gif" width="12" height="12" alt="Elimina"
						border="0"></a> <% } %>
				</td>
			</tr>
			<!-- Record Hidden con le note -->
			<tr style="display: none" id="rec_<%=id_record%>">
				<td class="l" colspan="100%"><font class="campoSmall"> <%=StringUtils.toStringJSP(lStatoEsec.getMotivoModifica(),"&nbsp;")%>
				</font></td>
			</tr>
			...
			<% if (isRevoche){ %>
			<tr>
				<td colspan="3"><font class="cRosso">I dati evidenziati
						in rosso si riferiscono a benefici revocati</font></td>
			</tr>
			<% } %>
			...
			<tr>
				<td><INPUT class="bottone" type="button" name="AGGIUNGI"
					value="Inserisci nuovo Provvedimento"
					onClick="javascript:nuovaAnnotazione();"></td>
			</tr>
			<% } %>
		</table>
	</FORM>
