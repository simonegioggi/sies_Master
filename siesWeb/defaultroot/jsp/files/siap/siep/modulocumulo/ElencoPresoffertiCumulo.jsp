...
<jsp:useBean id="ListaPresofferti"     scope="request" class="java.util.Vector"/>

<%
//==============================================================================
// Form per la visualizzazione della Presofferti legati a un certo Titolo.
//
// La form presenta un elenco dei Presofferti già presenti con la possibilità di 
// modificarle, cancellarle o inserirne delle nuovi 
//
//  FORM DA IMPLEMENTARE. PER ORA IN DRAFT
//==============================================================================
%>

<html>
<head>
  <title> Elenco Provvedimenti computo Presofferto</title>
  ...
  <script language="JavaScript">
    ...
    function eseguiAzione(aTipoAzione, aIdStat, aIdComp, aStato, aMotivoModifica)
    {
      if (aTipoAzione=='Dettaglio'){
        lAzione = "siap.siep.modulocumulo.action.ActDettaglioPresoffertoCumulo";
        document.ListaPresoffertiCumulo.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaPresoffertiCumulo.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaPresoffertiCumulo.submit();
      }
      else if (aTipoAzione=='Modifica'){
        lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciPresoffertoCumulo";
        document.ListaPresoffertiCumulo.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaPresoffertiCumulo.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaPresoffertiCumulo.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaPresoffertiCumulo.modalita.value='M';
        document.ListaPresoffertiCumulo.submit();
      }
        ...
        if (window.confirm(msgConfirm)) {
          ...
          document.ListaPresoffertiCumulo.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
          document.ListaPresoffertiCumulo.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
          document.ListaPresoffertiCumulo.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_FLAG_STATO%>.value = aStato;
          document.ListaPresoffertiCumulo.modalita.value = "C";
    ...
    function nuovaAnnotazione(){
      lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciPresoffertoCumulo";
      document.ListaPresoffertiCumulo.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
      document.ListaPresoffertiCumulo.modalita.value = "I";
      document.ListaPresoffertiCumulo.submit();
    }
    ...
    {
      document.ListaPresoffertiCumulo.<%=IWebConstants.ACTION_FIELD%>.value = action;
      document.ListaPresoffertiCumulo.submit();
    }
  ...
  <table>
    <tr>
      ...
      <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp;
                      <font class="campo">Elenco Provvedimenti computo Presofferto &nbsp;</font>
      </td>
...
<FORM action="<%=IWebConstants.PG_MAIN%>" method="post" name="ListaPresoffertiCumulo">
  ...
  <table cellspacing="2" cellpadding="2" align="center" width="95%">
   ...
   <%
      ...
      Iterator itx = ListaPresofferti.iterator();
      while ( itx.hasNext()) {
    ...
    <tr>
      ...
      <% } else {%>
        ...
        <td class="c"></td>
      <% } %>
      <td class="c" nowrap><%=StringUtils.toStringJSP(DateUtils.getDateToString (lComputo.getDataReclusioneDa(),"dd-MM-yyyy"))%></td>
      <td class="c" nowrap><%=StringUtils.toStringJSP(DateUtils.getDateToString (lComputo.getDataReclusioneA(),"dd-MM-yyyy"))%></td>
