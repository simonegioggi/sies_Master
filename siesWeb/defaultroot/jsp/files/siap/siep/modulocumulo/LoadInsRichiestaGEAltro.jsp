...
<!-- 				LoadInsRichiestaGEAltro				 --> 
<% 
//============================================================================== 
// Form per l'inserimento e la modifica delle richieste al GE - Altre Richieste
//============================================================================== 
...
<html>
<head>
  <title> Gestione Altre Richieste al GE</title>
  ...
  <script language="JavaScript" >
    ...
    {
      document.InsRicGEaltro.<%=IWebConstants.ACTION_FIELD%>.value = action;
      document.InsRicGEaltro.submit();
    }
    ...
    function ElencoTitoliPopup (a_form_name, a_form_type)
    {
		//alert('ElencoTitoliPopup - lmodo = '+lmodo);   	 	
   	 	if(lmodo == "I")
   	 	{	
	   	 	if(document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_ESTREMI_TITOLO_CUMULATO%>.value != "")
	 	 	{	
	 	 		$('#divdatiTitolo').hide();
	 	 		$('#tabdatiTitolo').hide();
	 	 		document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_ESTREMI_TITOLO_CUMULATO%>.value = "";
	 	 	}
	 	 	else
	 	 	{
	 	 		$('#divdatiTitolo').show();
    ...
    { 
   		// Data Emissione
       if (document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
         	 document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
       if (document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
           	document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_MESE_DATA_EMISSIONE%>.value;

       var data_to_verify = document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_ANNO_DATA_EMISSIONE%>.value;
       if (data_to_verify=='//' )
       {
           alert('Indicare la Data Richiesta');
           document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_GIORNO_DATA_EMISSIONE%>.focus();
           return false;
       }
       ...
       var data_od = '<%=DateUtils.getSysDate("dd/MM/yyyy")%>';
 	   if(!CompareDate(data_to_verify, data_od))
 	   {
	 	...
	 	if(document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_MOTIVAZIONI %>.value == "" || 
	   	   document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_MOTIVAZIONI %>.value.length == 0	)
	    {
	   		alert("Descrizione Richiesta Obbligatoria");
	   		document.InsRicGEaltro.<%=ICostantiRichiestePmInCumulo.CAMPO_MOTIVAZIONI%>.focus();
	   		return false;
	    }
 		
		// Titolo Cumulato di Riferimento
	 	if(document.InsRicGEaltro.<%=ICostantiTitoloCumulato.CAMPO_ID_TITOLO_CUMULATO %>.value == "" || 
	   	   document.InsRicGEaltro.<%=ICostantiTitoloCumulato.CAMPO_ID_TITOLO_CUMULATO %>.value.length == 0	)
	    {
  ...
  <table>
    <tr>
      ...
      <td class="LBG">
        ...
        <font class="campo">Inserimento Richiesta al G.E. - Altre Richieste &nbsp;</font>
        <% } else if( modalita.equals("M") ) { %>
        <font class="campo">Modifica Richiesta al G.E. - Altre Richieste &nbsp;</font>
        <% } %>
      </td>
      <td class="LBG">
...
<FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name="InsRicGEaltro">
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.modulocumulo.action.ActInserisciRichiestaGEAltro">
  ...
  {	%>
  	...
  	<input type="hidden" name="<%=ICostantiTitoloCumulato.CAMPO_ID_TITOLO_CUMULATO %>"	value="<%=RichiestaAlGE.getTitIdTitoloCumulato() %>" >
  ...
  <table width="95%" align="center">
    <tr>
      <td class="l" colspan="1">
        <a href="Javascript:ElencoTitoliPopup('InsRicGEaltro','<%=ICostantiRichiestePmInCumulo.FORM_TYPE_RIC_APPL_PA%>');">
...
<script language="JavaScript" type="text/javascript">
  var frmvalidator  = new Validator("InsRicGEaltro");
