...
<jsp:useBean id="ListaRevocheMisureAlt"	scope="request" class="java.util.Vector"/>

<%
//====================================================================================================
// Form per la visualizzazione dell'elenco di Revoche Misure Alternative legate a un certo Titolo.
//
// La form presenta un elenco di revoche Mis. Alt. già presenti con la possibilità di 
...
<html>
<head>
  <title> Elenco Provvedimenti di Revoca Misura Alternativa </title>
  ...
  <script language="JavaScript">
    ...
    function eseguiAzione(aTipoAzione, aIdStat, aIdComp, aStato, aMotivoModifica)
    {
      if (aTipoAzione=='Dettaglio'){
        lAzione = "siap.siep.modulocumulo.action.ActDettaglioRevocaMisuraAlternativaCumulo";
        document.ListaRevocheMisureAlt.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaRevocheMisureAlt.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaRevocheMisureAlt.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaRevocheMisureAlt.submit();
      }
      else if (aTipoAzione=='Modifica'){
        lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciRevocaMisuraAlternativaCumulo";
        document.ListaRevocheMisureAlt.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
        document.ListaRevocheMisureAlt.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
        document.ListaRevocheMisureAlt.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
        document.ListaRevocheMisureAlt.modalita.value = 'M';
        ...
        if (window.confirm(msgConfirm)) {
          lAzione = "siap.siep.modulocumulo.action.ActInserisciRevocaMisuraAlternativaCumulo";
          document.ListaRevocheMisureAlt.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;

          document.ListaRevocheMisureAlt.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_ID_STATO_ESEC_TITOLO_CUMULATO%>.value = aIdStat;
          document.ListaRevocheMisureAlt.<%=ICostantiComputiCumulo.CAMPO_ID_COMPUTI_CUMULO%>.value = aIdComp;
          document.ListaRevocheMisureAlt.<%=ICostantiStatoEsecTitoloCumulato.CAMPO_FLAG_STATO%>.value = aStato;
          document.ListaRevocheMisureAlt.modalita.value = 'C';
    ...
    function nuovoProvvedimento(){
      lAzione = "siap.siep.modulocumulo.action.ActLoadInserisciRevocaMisuraAlternativaCumulo";
      document.ListaRevocheMisureAlt.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
      document.ListaRevocheMisureAlt.modalita.value = 'I';
      document.ListaRevocheMisureAlt.submit();
    }
    ...
    {
      document.ListaRevocheMisureAlt.<%=IWebConstants.ACTION_FIELD%>.value = action;
      document.ListaRevocheMisureAlt.submit();
    }
  ...
  <table>
    <tr>
      ...
      <td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp;
                      <font class="campo">Elenco Provvedimenti di Revoca Misura Alternativa &nbsp;</font>
      </td>
...
<FORM action="<%=IWebConstants.PG_MAIN%>" method="post" name="ListaRevocheMisureAlt">
  ...
  <table cellspacing="2" cellpadding="2" align="center" width="95%">
    ...
    <%
      ...
      Iterator itx = ListaRevocheMisureAlt.iterator();
      while ( itx.hasNext()) {
    ...
    <% if (ListaRevocheMisureAlt.size()==0){ %>
    <tr>
