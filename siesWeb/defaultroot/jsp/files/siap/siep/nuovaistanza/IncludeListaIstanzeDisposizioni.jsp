...
<jsp:useBean id="descr_data"  scope="request" class="java.lang.String"/>
...
<script language="JavaScript">
 ...
 function dettaglioIstanza(idIstanza,idEventoIstanza)
 {
	   ...
	   document.getElementById('containerIst').style.height = '50px';
...
function conferma(a_action, idIstanza, eventodacanc, tipo)
 {
  if (window.confirm('Confermi la cancellazione ?'))
  {
    ...
    }else
    {
     	...
     	var str = "/jsp/Main.jsp?Action=siap.siep.nuovaistanza.action.ActCancellaInoltroNuovaIstanza&IdEvento="+eventodacanc+"&IdIstanza="+idIstanza+"&TipoOp="+tipo+"&TipoProvvedimento=Disposizione";
...
%>	
  	...
  	<input type="hidden"  name="idEventoIstanza" value="" />     
		<tr>
			<td colspan="6" class="L">Seleziona istanza dalla lista:</td>
	</tr>	
	<tr>
...
%>
	...
	<td class="<%=colorTr%>">&nbsp;
	<% if (!lModel.getDescrStatoIstanza().equals("Disposizione da Validare")&&
			!lModel.getDescrStatoIstanza().equals("Da compilare e da Validare")) {%>
	...
	{%>
		<a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.nuovaistanza.action.ActLoadDettaglioNuovaIstanza&IdEvento=<%=lModel.getCodEsito()%>&TipoVis=Disposizione">
			<img src="/images/dettagli.gif" width="12" height="12" alt="Dettaglio Disposizione PM" border="0">
		</a>		
<%}	
	...
	{%>
		<a href="Javascript:stampa2('<%=ISIAPCostantiWeb.PG_STAMPA%>','<%=IWebConstants.ACTION_FIELD%>=siap.sico.evento.action.ActLoadDocumento&IdEvento=<%=lModel.getCodEsito()%>');">
			<img src="/images/print.gif" alt="Stampa Disposizione PM" width="12" height="12" border="0">
		</a>
<%}
	if (!lModel.getDescrStatoIstanza().equals("Da compilare e da Validare")&& !lModel.getDescrStatoIstanza().equals("Disposizione da Validare")) 
	{%>
...
<!-- ADESSO devo inserire eventuali precedenti annullamenti  -->
<% 
...
%>
	...
	<td class="cGrigio" style="font-size: 11px; text-align=left" >
			<a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.nuovaistanza.action.ActLoadDettaglioNuovaIstanza&IdEvento=<%=leveModel.getIdEvento()%>&TipoVis=Disposizione">
			    <img src="/images/dettagli.gif" width="12" height="12" alt="Dettaglio Disposizione PM" border="0">
			</a>		
	</td>
<%}
