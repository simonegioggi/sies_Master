...
<jsp:useBean id="IdIstanza"     scope="request" class="java.lang.String"/>
...
String nAction ="";
  if( request.getParameter("nextAction") == null){	  
	if( request.getParameter("TipoProvvedimento").equals("Inoltro")){
      	nAction="&nextAction=siap.siep.nuovaistanza.action.ActLoadInoltroPM";
	}else{
      	nAction="&nextAction=siap.siep.nuovaistanza.action.ActLoadDisposizioniPM";
	}
  }
%>
  ...
  <%}%>
    ...
    <script language="JavaScript">
      function verify()
      {
        ...
        <% if( request.getParameter("TipoProvvedimento").equals("Inoltro")){ %>
        	window.opener.top.frames['centrale'].frames['body'].location.href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.nuovaistanza.action.ActCancellaInoltroNuovaIstanza&nextAction=siap.siep.nuovaistanza.action.ActLoadInoltroPM&IdEvento="+"<%=IdEvento%>"+"&motivazioni="+lmotivazioni+"&IdIstanza="+"<%=IdIstanza%>"+"&TipoProvvedimento="+"<%=TipoProvvedimento%>"+"&TipoOp=A&TornaQui=10"+"<%=nAction%>&TornaQui=10";
        <% } else { %>
        	window.opener.top.frames['centrale'].frames['body'].location.href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.nuovaistanza.action.ActCancellaInoltroNuovaIstanza&nextAction=siap.siep.nuovaistanza.action.ActLoadDisposizioniPM&IdEvento="+"<%=IdEvento%>"+"&motivazioni="+lmotivazioni+ "&IdIstanza="+"<%=IdIstanza%>"+ "&TipoProvvedimento="+"<%=TipoProvvedimento%>"+"&TipoOp=A";
        <% }%>
        window.close();
      }
    </script>
  ...
  <!--  input type="HIDDEN" name="Action" value="" -->
   ...
   <tr>
     <td class="lNoBord" colspan="2">
       ...
       <%}else{ %>
       <INPUT class="bottone" type="submit" name="I" value="Conferma" onClick="javascript:verify()">
       <%}%>
     </td>
   </tr>
