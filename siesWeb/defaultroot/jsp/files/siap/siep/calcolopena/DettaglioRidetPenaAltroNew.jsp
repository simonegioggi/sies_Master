...
<jsp:useBean id="penaResUltimaValidata"         scope="request" class="siap.siep.penaresidua.model.PenaResiduaModel"/>

<!-- DettaglioRidetPenaAltroNew -->
<%

//==============================================================================
// Form per la visualizzazione del dettaglio del "provvedimento" di computo
// della 'Rideterminazione pena - Altro' NUOVA VERSIONE
// La vecchia versione resta valida in quanto i vecchi provvedimenti hanno
// struttura dati differenti e necessitano di una pagina di dettaglio apposita.
//==============================================================================
...
<head>
  ...
  <script>
    ...
    function caricaStampe(idEvento) {
      document.location.href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.calcolopena.action.ActLoadGrigliaRidetPenaAltro&<%=ICostantiEvento.CAMPO_ID_EVENTO%>="+idEvento;
    }
  </script>
</head>
...
<%
	// MEV_66: gestita casistica che se destinatario è udsm allora cambio il nome
  	String descrTipoUfficio = aEventoAltraAut.getDescrUfficioEmittente();
  	if ("UDSM".equals(aEventoAltraAut.getCodTipoUfficioEmittente()))
...
<form action="<%=IWebConstants.PG_MAIN%>" method="post" name="f">
  ...
  <%
  if (   penaResUltimaValidata.getDataFine()!=null 
      && DateUtils.isGreater(DateUtils.getSysDate() ,penaResUltimaValidata.getDataFine())
     ) 
  {
    // new 07/2014 d.f. se il fine pena è già trascorso si utilizza per il calcolo
    //                  della fungibilità come data scarcerazione
  %>
  <!-- Data per il calcolo della fungibilità: in questo caso data fine pena -->
  <input type="HIDDEN" name="<%=ICostantiAnnotazioneManuale.CAMPO_ANNO_DATA_SCARCERAZIONE%>"   value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(penaResUltimaValidata.getDataFine(), "yyyy") )%>">
  <input type="HIDDEN" name="<%=ICostantiAnnotazioneManuale.CAMPO_MESE_DATA_SCARCERAZIONE%>"   value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(penaResUltimaValidata.getDataFine(), "MM") )%>">
  <input type="HIDDEN" name="<%=ICostantiAnnotazioneManuale.CAMPO_GIORNO_DATA_SCARCERAZIONE%>" value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(penaResUltimaValidata.getDataFine(), "dd") )%>">
  <% } else { %>
  ...
  <table>
    <tr>
      ...
      //if (   aEventoComputo.getFlagDocumentoRegistrato()==null 
      //    || aEventoComputo.getFlagDocumentoRegistrato().equals("N")
      //   ) 
      if (   penaresidua==null  
