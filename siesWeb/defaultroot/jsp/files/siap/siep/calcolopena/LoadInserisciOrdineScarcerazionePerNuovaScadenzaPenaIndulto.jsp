...
<html>
<head>
  <!-- LoadInserisciOrdineScarcerazionePerNuovaScadenzaPenaIndulto -->
  ...
  <script language="JavaScript">
    ...
    function ListaComuni(a_formname,a_fieldname) {
    	<%-- MERGE v10 COLLAUDO: aggiunto controllo preventivo --%>
  	  	var a_typename = document.getElementById('<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>').value;
		if (a_typename == "-") {
			alert("Selezionare l'Autorita' di destinazione");
		} else {
	...
	function Verify() {
		...
		// MERGE v10 COLLAUDO: aggiunti controlli di verifica
		if (document.LoadInserisciOrdineScarcerazione.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value == "") {
        	alert("Il campo Sede è obbligatorio");
        	document.LoadInserisciOrdineScarcerazione.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.focus();
        	return false;
      	}
		if (document.LoadInserisciOrdineScarcerazione.<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>.value == "-" &&
				document.LoadInserisciOrdineScarcerazione.<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>.value != "") {
        	alert("Se valorizzato il campo Sede, valorizzare il campo Autorita' di Destinazione");
        	document.LoadInserisciOrdineScarcerazione.<%=ICostantiCSSA.CAMPO_ID_CSSA%>.focus();
        	return false;
      	}
		if (document.LoadInserisciOrdineScarcerazione.<%=ICostantiCSSA.CAMPO_ID_CSSA%>.value == "" &&
				(document.LoadInserisciOrdineScarcerazione.<%=MinorMask.ComboCSSAId%>.value != "" &&
						document.LoadInserisciOrdineScarcerazione.<%=MinorMask.ComboCSSAId%>.value != "-")) {
        	alert("Se valorizzato il campo Destinatario, valorizzare il campo Sede UEPE Competente");
        	document.LoadInserisciOrdineScarcerazione.<%=ICostantiCSSA.CAMPO_ID_CSSA%>.focus();
        	return false;
      	}
		if (document.LoadInserisciOrdineScarcerazione.<%=MinorMask.ComboCSSAId%>.value == "-" &&
				document.LoadInserisciOrdineScarcerazione.<%=ICostantiCSSA.CAMPO_ID_CSSA%>.value != "") {
        	alert("Se valorizzato il campo Sede UEPE Competente, valorizzare il campo Destinatario");
        	document.LoadInserisciOrdineScarcerazione.<%=MinorMask.ComboCSSAId%>.focus();
        	return false;
      	}
		if (document.LoadInserisciOrdineScarcerazione.<%=ICostantiNotifica.CAMPO_UFF_COD_UFFICIO%>.value == "" &&
				(document.LoadInserisciOrdineScarcerazione.<%=MinorMask.ComboMagistratoId%>.value != "" &&
						document.LoadInserisciOrdineScarcerazione.<%=MinorMask.ComboMagistratoId%>.value != "-")) {
        	alert("Se valorizzato il campo Destinatario, valorizzare il campo Sede Ufficio di Sorveglianza");
        	document.LoadInserisciOrdineScarcerazione.<%=ICostantiNotifica.CAMPO_UFF_COD_UFFICIO%>.focus();
        	return false;
      	}
		if (document.LoadInserisciOrdineScarcerazione.<%=MinorMask.ComboMagistratoId%>.value == "-" &&
				document.LoadInserisciOrdineScarcerazione.<%=ICostantiNotifica.CAMPO_UFF_COD_UFFICIO%>.value != "") {
        	alert("Se valorizzato il campo Sede Ufficio di Sorveglianza, valorizzare il campo Destinatario");
        	document.LoadInserisciOrdineScarcerazione.<%=MinorMask.ComboMagistratoId%>.focus();
        	return false;
      	}
	}
...
//     }
	...
	function ListaUDS(a_formname,a_fieldname) {
		...
		if (a_typename == "-") {
			alert("Selezionare il Destinatario dell'Ufficio preposto al controllo");
		} else {
			desktop = window.open("<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.sico.ufficio.action.ActLoadListaUDS&formname="+a_formname+"&fieldname="+a_fieldname,
					"Ricerca_UDS", "toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=no, width=370, height=500");
		}
    }
...
%>
    ...
    <tr>
      <%//MEV29 si visualizzano anno e numero procedimento SIGE e non anno e enumero Ordinanza%>
      <% if (annotazioneordinanza.getAnnotazioneManuale().getChiaveAnnoSige()!=null) {%>
      <td class="l">
        <font class="label">Anno / Numero Procedimento SIGE</font>
      </td>
      <td class="l">
        <font class="campo">
          <%=StringUtils.toStringJSP( annotazioneordinanza.getAnnotazioneManuale().getChiaveAnnoSige() )%>/<%=StringUtils.toStringJSP(annotazioneordinanza.getAnnotazioneManuale().getChiaveNumeroSige())%>
        </font>
      </td>      
      <% } else {%>
      <td class="l">
...
%>
      <tr>
      	<td>
       		<li>
<%		 // ACCOGLIE
		 if(annotazioneordinanza.getEvento().getCodEsito().equals("C"))
		 {		 	 
%>	
				<font class="label" >ESITO : </font>&nbsp;
          		<font style="color:red; font-size: 11pt"> ACCOGLIE IN CONFORMITA' </font>&nbsp;
	       	   	<font class="campo">
          				<%=StringUtils.toStringJSP(lAnn.getDescrDpr())%>
        		</font>
<%		 } 	
		 else if(annotazioneordinanza.getEvento().getCodEsito().equals("-"))
		 {	 
%>
				<font class="label" >ESITO : </font>&nbsp;
          		<font style="color:red; font-size: 11pt"> RICHIESTA ACCOLTA </font>&nbsp;
        		<font class="campo">
          			<%=StringUtils.toStringJSP(lAnn.getDescrDpr())%>
        		</font>
<%		 } 	
		 else if(annotazioneordinanza.getEvento().getCodEsito().equals("D"))
		 {	 
%>
				<font class="label" >ESITO : </font>&nbsp;
          		<font style="color:red; font-size: 11pt"> ACCOGLIE IN DIFFORMITA' </font>&nbsp;
	        	<font class="campo">
    	      		<%=StringUtils.toStringJSP(lAnn.getDescrDpr())%>
        		</font>        		
<%		 } 	
		 else if(annotazioneordinanza.getEvento().getCodEsito().equals("R"))
		 {	 
			// RIGETTA
%>
				<font class="label" >ESITO : </font>&nbsp;
          		<font style="color:red; font-size: 11pt"> RIGETTO </font>&nbsp;
           		<font class="campo">
          			<%=StringUtils.toStringJSP(lAnn.getDescrDpr())%>
        		</font>

<%		 } 	
		 else if(annotazioneordinanza.getEvento().getCodEsito().equals("I"))
		 {	 
%>
				<font class="label" >ESITO : </font>&nbsp;
          		<font style="color:red; font-size: 11pt"> RICHIESTA INAMMISSIBILE  </font>&nbsp;
        		<font class="campo">
          			<%=StringUtils.toStringJSP(lAnn.getDescrDpr())%>
        		</font>
<%		 } 	
		 else if(annotazioneordinanza.getEvento().getCodEsito().equals("U"))
		 {	 
%>
				<font class="label" >ESITO : </font>&nbsp;
          		<font style="color:red; font-size: 11pt"> RIUNISCE </font>&nbsp;
	        	<font class="campo">
    	      		<%=StringUtils.toStringJSP(lAnn.getDescrDpr())%>
        		</font>
<%		 } 	%>
...
<!-- PRIMA ERA CONTROLLATA LA ANNOTAZIONE, ORA L'EVENTO -->   
<% // IN CONFORMITA'
      if(lAnn.getFlagConforme() != null && lAnn.getFlagConforme().equals("C"))
      {
%>
        <font class="campo">
          <font class="label"> -- </font>IN CONFORMITA' RICHIESTA PM
        </font>
<%
      } // IN DIFFORMITA'
...
%>
        <font class="campo">
          <font class="label"> -- </font>IN DIFFORMITA' RICHIESTA PM
        </font>
<%
      }
      else if(lAnn.getFlagConforme() != null && lAnn.getFlagConforme().equals("-"))
      { // senza richiesta
%>
        <font class="campo">
          <font class="label"> -- </font>DI UFFICIO
        </font>
<%
      }
%>
...
<!--autorità di polizia-->
    ...
    <tr>
      	...
      	<td class="L">
        	<input readonly title="Sede Autorita Esterna" type="text" name="<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE%>" maxlength="35" size="35">
...
<!--UEPE competente-->
	...
	<tr>
      	<td class="l">Sede UEPE Competente</td>
      	<td class="l">
...
<!-- Ufficio di Sorveglianza -->
	...
	<tr>
      	<td class="l">Sede Ufficio di Sorveglianza</td>
      	<td class="L">
	        <input type="hidden" name="magSorv" value="S">
	        <input type="hidden" name="notificaMagistrato" value="C">
	        <input readonly title="ufficio" value="<%=UffUDS%>" type="text" name="<%=ICostantiNotifica.CAMPO_UFF_COD_UFFICIO%>" maxlength="35" size="25">
...
<!-- Istituto di Detenzione -->
    ...
    <tr>
		<td class="l">Sede Istituto di Detenzione</td>
<%
