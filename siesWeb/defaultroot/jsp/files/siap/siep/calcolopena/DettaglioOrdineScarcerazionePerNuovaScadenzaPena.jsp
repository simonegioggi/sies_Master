...
<jsp:useBean id="cssa"         scope="request" class="siap.siep.notifica.model.NotificaModel"/>
...
<body class="corpo">
  <table>
    <tr>
      ...
      <font class="campo">Dettaglio Ordine Scarcerazione Per Nuova Scadenza Pena</font>
    </td>
<%
    //==========================================================================
    // Visualizzao il tasto di stampa se l'evento non è validato
    //==========================================================================
    ...
    %>
      ...
      <jsp:include page="<%= ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIEP%>">
        <jsp:param name="ActionLink" value="<%="/jsp/Main.jsp?Action=siap.siep.calcolopena.action.ActStampaOrdineScarcerazionePerNuovaScadenzaPena&IdEvento="+eventonotifica.getEvento().getIdEvento()%>"/>
      </jsp:include>
      <%
    }
...
%>
  <td class="LBG">
       <%if(eventonotifica != null && eventonotifica.getEvento() != null && eventonotifica.getEvento().getCodMotivo()!= null &&
            (eventonotifica.getEvento().getCodMotivo().equals("0161") || eventonotifica.getEvento().getCodMotivo().equals("0162") ||
             eventonotifica.getEvento().getCodMotivo().equals("0163")))
       {%>
...
<%
  	...
  	if (cssa.getIdNotifica() != null) {
%>
    <tr>
    	<%-- MERGE v10 COLLAUDO: modificata stringa fissa (UEPE) con dinamica --%>
      	<td class="l"><%=StringUtils.toStringJSP(cssa.getCodUffUepeUssmSS())%> Competente</td>
      	<td class="l">
        	<font class="campo"><%=StringUtils.toStringJSP(cssa.getCSSA().getComune())%>-<%=StringUtils.toStringJSP(cssa.getCSSA().getIndirizzo())%></font>
      	</td>
	</tr>
    <tr>
      	<td class="l">Note</td>
      	<td  class="l">
        	<font class="campo"><%=StringUtils.toStringJSP(cssa.getNote())%>&nbsp;</font>
      	</td>
	</tr>
<%
	}
  	if (ufficio.getIdNotifica() != null) {
  		String descrUfficioDestinatario = "UFFICIO DI SORVEGLIANZA";
  		if ("UDSM".equals(ufficio.getCodUffUdsUdsm()))
  			descrUfficioDestinatario = "MAGISTRATO DI SORVEGLIANZA PER I MINORENNI";
%>
    <tr>
      <td class="l">Ufficio Preposto al controllo</td >
      <%-- MERGE v10 COLLAUDO: modificata stringa fissa (UFFICIO DI SORVEGLIANZA) con dinamica --%>
      <td class="l">
      	<font class="campo"><%=descrUfficioDestinatario%></font>
      </td>
    <tr>
    ...
    <tr>
      <td class="l">Note</td>
      <td  class="l">
        <font class="campo"><%=StringUtils.toStringJSP(ufficio.getNote())%>&nbsp;</font>
      </td>
    </tr>
<%
  }
      ...
      <table>
        ...
        <tr>
          <td class="L">
            ...
            <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.calcolopena.action.ActUploadOrdineScarcerazionePerNuovaScadenzaPena">
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_ID_EVENTO%>" value="<%= eventonotifica.getEvento().getIdEvento() %>">
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>" value="siap.siep.calcolopena.action.ActDettaglioOrdineScarcerazionePerNuovaScadenzaPena">
          </td>
        </tr>
      </table>
</form>
