...
<jsp:useBean id="istanza" scope="request"
	class="siap.sico.evento.model.EventoModel" />
<jsp:useBean id="fogliocomplementare" scope="request"
	class="java.lang.String" />
<jsp:useBean id="misuraalternativa" scope="request"
	class="siap.sico.misuraalternativa.model.MisuraAlternativaModel" />
<jsp:useBean id="UfficioEmittente" scope="request"
	class="siap.sico.ufficio.model.UfficioModel" />
<jsp:useBean id="motivoevento" scope="request"
	class="siap.siep.motivoevento.model.MotivoEventoModel" />
...
<head>
<title>[S.I.E.S.] - Dettaglio Revoca Decreto di Sospensione-</title>
...
<BODY class="corpo">
	...
	<table>
		<tr>
			...
			<td class="LBG"><font class="label">Funzione :</font>&nbsp; <font
				class="campo">Dettaglio Revoca Decreto di Sospensione (legge
					165/98)</font></td>
			...
			<jsp:include page="<%=ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIEP%>">
				<jsp:param name="ActionLink"
					value="<%="/jsp/Main.jsp?Action=siap.siep.ordineesecuzione.action.ActStampaRevocaLSAltrePosizioni&fc="
							+ fogliocomplementare + "&IdEvento=" + lEve.getEvento().getIdEvento()%>" />
			</jsp:include>
			<%
				}
			%>
	...
	<table>
		<tr>
			...
			<td class="L" colspan=5><font class="campo"> <%// Se detenuto altra causa, visualizzo Istituito di detenzione o Indirizzo
		        if (lFascicoloAssociato.getFlagAltraCausa() != null && lFascicoloAssociato.getFlagAltraCausa().equals("S")
		          && lPosizione!=null && lPosizione.getCodPosizioneGiuridica()!= null 
		          && lAltraCausa!=null  )
		      	{ %> <%=lAltraCausa.getDescrTipoPosGiuridica()%> <% } else { %> <%=lPosizione.getDescrPosizioneGiuridica()%>
					<% } %>
			</font></td>
		</tr>
		<%
		...
		<%
			...
			if (penaresidua.getIdPenaResidua() != null
					&& ((penaresidua.getFlagErgastolo() == null) || (penaresidua.getFlagErgastolo() != null
							&& !penaresidua.getFlagErgastolo().equals("S") && !penaresidua.getFlagErgastolo()
							.equals("D")))) {
		...
		<tr>
			...
			<%
				...
				if (((penaresidua.getFlagErgastolo() == null) || (penaresidua.getFlagErgastolo() != null
						&& !penaresidua.getFlagErgastolo().equals("S") && !penaresidua.getFlagErgastolo().equals(
						"D")))
						&& penaresidua.getDataFine() != null) {
					if (penaresidua.getDataFine().equals(penaresidua.getDataFinePresunta())) {
			%>
			<td class="l">Data Fine Pena</td>
			<td class="L"><font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(penaresidua.getDataFine(),
							"dd"))%> - <%=StringUtils.toStringJSP(DateUtils.getDateToString(penaresidua.getDataFine(),
							"MM"))%> - <%=StringUtils.toStringJSP(DateUtils.getDateToString(penaresidua.getDataFine(),
							"yyyy"))%></font></td>
			<%
				} else {
			%>
			<td class="l">Data Fine Pena</td>
			<td class="lRosso"><font class="lRosso"><%=StringUtils.toStringJSP(DateUtils.getDateToString(penaresidua.getDataFine(),
							"dd"))%> - <%=StringUtils.toStringJSP(DateUtils.getDateToString(penaresidua.getDataFine(),
							"MM"))%> - <%=StringUtils.toStringJSP(DateUtils.getDateToString(penaresidua.getDataFine(),
							"yyyy"))%></font></td>
		...
		<tr>
			...
			<td class="L" colspan=1><font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(lEve.getEvento().getDataEmissione(),
					"dd-MM-yyyy"))%></font></td>
			...
			<td class="L" colspan=1><font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(
						lEve.getNotifiche()[0].getDataInvio(), "dd-MM-yyyy"))%></font></td>
			<%
				}
			%>
		</tr>
		<tr>
			...
			<td class="l"><font class="campo"> <%=StringUtils.toStringJSP(motivoevento.getDescrMotivoRevoca())%>
			</font></td>
		</tr>
		<tr>
 ...
 %>
			...
			<td class="l">
				...
				%> <font class="campo"> <%=StringUtils.toStringJSP(lEve.getNotifiche()[i].getAutoritaEsterna()
								.getDescrSede())%>
			</font> <%
 	}
 		}
 %>
	...
	<table>
		<%
			if (motivoevento != null && motivoevento.getCodMotivoRevoca() != null
					&& motivoevento.getCodMotivoRevoca().equals("0002") && misuraalternativa != null
		...
		<tr>
			...
			</font> <font class="campo"><%=StringUtils.toStringJSP(misuraalternativa.getChiaveProgrFascicoloSius())%></font></td>
			<%
				}
			%>
			<%
			...
			<td class="l"><font class="campo"><%=StringUtils.toStringJSP(misuraalternativa.getAnnoRegistro())%>
					/</font> <font class="campo"> <%=StringUtils.toStringJSP(misuraalternativa.getNumeroRegistro())%></font>
			</td>
			<%
				}
			%>
		...
		<tr>
			...
			<td class="l"><font class="campo"><%=StringUtils.toStringJSP(UfficioEmittente.getDescrTipoUfficio())%>&nbsp;di&nbsp;<%=StringUtils.toStringJSP(UfficioEmittente.getDescProvincia())%></font></td>
		</tr>
		<tr>
			...
			<td class="l"><font class="campo"> <%=StringUtils.toStringJSP(DateUtils.getDateToString(
						misuraalternativa.getDataDecisione(), "dd-MM-yyyy"))%>
			</font></td>
		</tr>
		...
		<tr>
			...
			<td class="l"><font class="campo"> <%=StringUtils.toStringJSP(DateUtils.getDateToString(
							misuraalternativa.getDataInizioMisura(), "dd-MM-yyyy"))%>
			</font></td>
			<%
				}
			%>
			...
			<td class="l"><font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(
							misuraalternativa.getDataFineMisura(), "dd-MM-yyyy"))%></font></td>
			<%
				}
			%>
		</tr>
		...
		<tr>
			<td class="l">Note</td>
			<td class="l"><font class="campo"><%=StringUtils.toStringJSP(misuraalternativa.getNote())%>&nbsp;</font>
			<td>
		</tr>

		<%
			}
			} else if (motivoevento != null && motivoevento.getCodMotivoRevoca() != null
					&& motivoevento.getCodMotivoRevoca().equals("0001")) {
		%>

		<tr>
			<td class="l">Motivo Revoca PM</td>
			<td class="l"><font class="campo"><%=StringUtils.toStringJSP(motivoevento.getDescrMotivoRevocaPm())%>&nbsp;</font>
			<td>
		</tr>

		<tr>
			<td class="l">Motivazioni</td>
			<td class="l"><font class="campo"><%=StringUtils.toStringJSP(motivoevento.getMotivazioni())%>&nbsp;</font>
			<td>
		</tr>
		...
		<%
			if (lEve.getNotifiche() != null && lEve.getNotifiche().length > 0
					&& lEve.getNotifiche()[0].getIstitutoDetenzione() != null) {
		%>
		<tr>
			<td class="l">Istituto Detenzione</td>
			<td class="L" colspan=2><font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[0].getIstitutoDetenzione()
						.getDescrTipoIstituto())%></font>&nbsp; di <font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[0].getIstitutoDetenzione()
						.getDescrComune())%></font>&nbsp;</td>
		</tr>
		<%
			} else if (lEve.getNotifiche() != null && lEve.getNotifiche().length > 0
					&& lEve.getNotifiche()[0].getAutoritaEsterna() != null) {
		%>
		<tr>
			<td class="l">Destinatario per l'Esecuzione</td>
			<td class="L" colspan=2><font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[0].getAutoritaEsterna()
						.getDescrTipoAutorita())%></font>&nbsp; di <font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[0].getAutoritaEsterna().getDescrSede())%></font>&nbsp;</td>
		</tr>
		...
		<%
			...
			int count = 1;
			String lTrasmetti = "N";
			String lSedeTribunale = null;
			if (lEve.getNotifiche() != null && lEve.getNotifiche().length > 1) {
				if (lEve.getNotifiche()[count] != null
						&& lEve.getNotifiche()[count].getCodTipoNotifica() != null
						&& lEve.getNotifiche()[count].getCodTipoNotifica().equals("TS")
						&& lEve.getNotifiche()[count].getUfficio() != null) {
		%>
		<tr>
			<td class="l">Destinatario</td>
			<td class="L"><font class="campo"><%=StringUtils.toStringJSP(lEve.getEvento().getDescrUfficioEmittente())%></font>
				di <font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[count].getUfficio().getDescrComune())%></font>
		</tr>
		<%
			lTrasmetti = "S";
					lSedeTribunale = lEve.getNotifiche()[count].getUfficio().getDescrComune();
					count = 2;
				}
			}
		...
		<tr>
			...
			<td class="L" colspan="2"><font class="campo"><%=StringUtils.toStringJSP(lEve.getAvvocati()[lIndex].getAvvocato().getCognome())
							+ " "
							+ StringUtils.toStringJSP(lEve.getAvvocati()[lIndex].getAvvocato().getNome())%></font>&nbsp; &nbsp;Foro di&nbsp; <font class="campo">
					<%=StringUtils.toStringJSP(lEve.getAvvocati()[lIndex].getAvvocato().getForo())%>
			</font> &nbsp;Difensore di&nbsp; <font class="campo"> <%=StringUtils.toStringJSP(lEve.getAvvocati()[lIndex].getAvvocato().getDescrTipo())%>
			</font></td>
		</tr>
		<%
			if (lEve.getNotifiche() != null && lEve.getNotifiche().length > count
							&& lEve.getNotifiche()[count].getAutoritaEsterna() != null) {
		%>
		<tr>
			<td class="l">Autorita Notifica</td>
			<td class="L" colspan=2><font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[count].getAutoritaEsterna()
								.getDescrTipoAutorita())%></font>&nbsp; di <font
				class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[count].getAutoritaEsterna()
								.getDescrSede())%></font>&nbsp;</td>
		</tr>
		<tr>
			<td class="l">Note</td>
	...
	<table>
		<tr>
			...
			<td class="l"><font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(istanza.getDataEmissione(),
						"dd-MM-yyyy"))%></font>&nbsp;</td>
		</tr>
		<tr>
			<td class="l">Oggetto dell'Istanza</td>
			<td class="l"><font class="campo"><%=StringUtils.toStringJSP(istanza.getDescrMotivo())%>
			</font>&nbsp;</td>
		</tr>
	</table>
	<%
			...
			<table>
				...
				<tr>
					<td class="L"><input class=bottone type="submit"
						...
						value="siap.siep.ordineesecuzione.action.ActUploadRS"> <input
						type="HIDDEN" name="<%=ICostantiEvento.CAMPO_ID_EVENTO%>"
						value="<%=lEve.getEvento().getIdEvento()%>"> <%
 	if (motivoevento != null && motivoevento.getCodMotivoRevoca() != null
 			&& motivoevento.getCodMotivoRevoca().equals("0001") && lTrasmetti.equals("S")) {
 %> <input type="HIDDEN"
						name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>"
						value="siap.siep.ordineesecuzione.action.ActLoadTrasferisciProvvedimentoLS">
						<input type="HIDDEN" name="titolo"
						value="Revoca Decreto Sospensione"> <input type="HIDDEN"
						name="sedetribunale" value="<%=lSedeTribunale%>"> <%
 	} else {
 %> <input type="HIDDEN"
						name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>"
						value="siap.siep.ordineesecuzione.action.ActDettaglioRevocaLSAltrePosizioni">
						<%
							}
						%></td>
				</tr>
