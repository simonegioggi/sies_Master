...
<%
  ...
  String lTitolo = "Decreto LEGGE 78/2013 - Dettaglio Ordine di Esecuzione - " + lPosizione.getDescrPosizioneGiuridica();
%>

<html>

  <head>
    <title> [S.I.E.S.] - Dettaglio L78/2013 - Emissione OE </title>
...
<%
    ...
    {%>
			...
			<jsp:include page="<%= ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIEP%>">
				<jsp:param name="ActionLink" value="<%=  "/jsp/Main.jsp?Action=siap.siep.ordineesecuzione.action.ActStampaOrdineEsecuzioneL78del2013&autorita="+lCodTipoAutorita+"&fc="+fogliocomplementare+"&IdEvento="+lEve.getEvento().getIdEvento()+"&IdPosizioneGiuridica="+lPosizione.getCodPosizioneGiuridica()%>"/>
			</jsp:include>
<%   }
	 ...
	 if((lEve.getEvento().getFlagDocumentoRegistrato()!=null) &&
  	 ...
  	 {%>
		    ...
		    <td class="LBG">
		    	<a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.ordineesecuzione.action.ActLoadTrasferisciProvvedimentoL78del2013&autorita=<%=lCodTipoAutorita%>&fc=<%=fogliocomplementare%>&IdEvento=<%=lEve.getEvento().getIdEvento()%>&titolo=<%=lTitolo%>">
...
%>
			      	<font class="campo">
			        	<%=StringUtils.toStringJSP( lEve.getNotifiche()[i].getAutoritaEsterna().getDescrSede() )%>
			      	</font>
<%
			  }
		}
...
%>
  ...
  <div align=left style="visibility:hidden" id="upld" >
    <FORM name="comandiOE" enctype="multipart/form-data" method="post" onSubmit="return controllaUpload();">
      <table>
        ...
        <tr>
          <td class="L">
            ...
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>" value="siap.siep.ordineesecuzione.action.ActDettaglioOrdineEsecuzioneL78del2013">
          </td>
        </tr>
      </table>
    </FORM>
