    ...
    <table>
      <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
        <td class="LBG">
          ...
          <font class="campo">Dettaglio Legge 165/98 Arresti Domiciliari</font>
        </td>
...
<!-- BOTTONE DI STAMPA -->
   <jsp:include page="<%= ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIEP%>">
     <jsp:param name="ActionLink" value="<%=  "/jsp/Main.jsp?Action=siap.siep.ordineesecuzione.action.ActStampaLSArrestiDomiciliari&fc="+fogliocomplementare+"&IdEvento="+lEve.getEvento().getIdEvento()%>"/>
   </jsp:include>
<%}%>
...
<%}%>
 ...
 <%if ("S".equals(lEve.getEvento().getFlagDocumentoRegistrato()))
 {%> 
      <td class="LBG">
          <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.ordineesecuzione.action.ActLoadTrasferisciProvvedimentoLS&<%=ICostantiEvento.CAMPO_ID_EVENTO%>=<%=lEve.getEvento().getIdEvento()%>">
            <img align="middle" src="<%=IWebConstants.IMAGES_DIR%>net24.gif" alt="Trasmissione" width="24" height="24" border="0">
          </a> 
       </td>  
 <%} %>
...
<%if(lEve.getNotifiche()[0].getNote() != null){%>
     <tr>
...
%>
    ...
    if(lEve.getNotifiche() != null && lEve.getNotifiche().length >0)
	{
     if( lEve.getNotifiche()[count].getAutoritaEsterna()!=null){%>
...
<%
  if(posizioneluogoaltra.getPosizioneGiuridica().getCodPosizioneGiuridica().equals("04") || posizioneluogoaltra.getPosizioneGiuridica().getCodPosizioneGiuridica().equals("02"))
  {
  if(lEve.getNotifiche() != null && lEve.getNotifiche().length >0)
  {
  if(lEve.getNotifiche()[count+1].getCSSA()!=null)
   {%>
   <tr>
        <td class="l">Destinatario</td>
         <td class="l"><font class="campo">UFFICIO DI ESECUZIONE PENALE ESTERNA</font> di <font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[count+1].getCSSA().getComune())+ " " +StringUtils.toStringJSP(lEve.getNotifiche()[count+1].getCSSA().getIndirizzo())%></font></td>
   </td>
   </tr>
<%}%>
<%if(lEve.getNotifiche()[count+1].getNote()!=null){%>
        <tr>
          <td class="l">Note</td>
          <td class="L" colspan="2"><font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[count+1].getNote())%></font>&nbsp;</td>
        </tr>
        <input type="hidden" value="SSPA" >
 <%}%>
<%if(lEve.getNotifiche()[count+2].getUfficio()!=null){%>

   <tr>
        <td class="l">Destinatario</td>
        <td class="l"><font class="campo">UFFICIO DI SORVEGLIANZA</font> di <font class="campo">
       <%=StringUtils.toStringJSP(lEve.getNotifiche()[count+2].getUfficio().getDescrComune())%></font></td>
     </tr>
<%}%>
<%if(lEve.getNotifiche()[count+2].getNote()!= null){%>
   <tr>
      <td class="l">Note</td>
      <td class="L" colspan="2"><font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[count+2].getNote())%></font>&nbsp;</td>
     </tr>
 ...
 <%//modifica relativa al tipo istituto
   ...
   if(((istanza != null) && (istanza.getIdEvento()!=null))&&(lEve.getNotifiche() != null && lEve.getNotifiche().length >0 && lEve.getNotifiche()[count+3].getUfficio()!= null))
   {
   %>

   <tr>
         <td class="l">Destinatario</td >
         <td class="L"><font class="campo">TRIBUNALE DI SORVEGLIANZA</font> di <font class="campo">
         <%=StringUtils.toStringJSP(lEve.getNotifiche()[count+3].getUfficio().getDescrComune())%>
         </font></td>
   </tr>
<% }%>
<% if(lEve.getNotifiche() != null && lEve.getNotifiche().length >0 && lEve.getNotifiche()[count+3].getNote() != null)
   {%>
   <tr>
      <td class="l">Note</td>
      <td class="L" colspan="2"><font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[count+3].getNote())%></font>&nbsp;</td>
     </tr>
   <% }%>
   <tr>
  </tr>
      ...
      <table>
        ...
        <tr>
          <td class="L">
            ...
            <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.ordineesecuzione.action.ActUploadLS">
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_ID_EVENTO%>" value="<%= lEve.getEvento().getIdEvento() %>">
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>" value="siap.siep.ordineesecuzione.action.ActLoadTrasferisciProvvedimentoLS">
            <input type="HIDDEN" name="titolo" value="Ordine Esecuzione">
          </td>
        </tr>
      </table>
    </FORM>
