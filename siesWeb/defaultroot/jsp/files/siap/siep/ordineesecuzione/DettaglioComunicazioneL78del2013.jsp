...
<jsp:useBean id="tornasuelenco" 		scope="request" class="java.lang.String"/>
...
<%
  ...
  //IstitutoDetenzioneModel lIstDet = new IstitutoDetenzioneModel();
  IstitutoDetenzioneModel lIstDet = istitutodetenzione;

  if(lEve.getNotifiche() != null && lEve.getNotifiche().length >0)
  {
    if (lEve.getNotifiche()[0].getAutoritaEsterna() != null)
    {
      lCodTipoAutorita = lEve.getNotifiche()[0].getAutoritaEsterna().getCodTipoAutorita();
    }
  ...
  // String lTitolo = "Ordine esecuzione legge 199/2010 - condannato IN CUSTODIA CAUTELARE  AGLI ARRESTI DOMICILIARI ";  
  String lTitolo = "Gestione decreto LEGGE 78/2013 - Dettaglio Comunicazione - " + lPosizione.getDescrPosizioneGiuridica();
%>

<html>

  <head>
    <title> [S.I.E.S.] - Dettaglio L78/2013 - Comunicazione al MDS - </title>
  ...
  if((lEve.getEvento().getFlagDocumentoRegistrato()==null) ||
    ...
    {%>
		...
		<jsp:include page="<%= ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIEP%>">
			<jsp:param name="ActionLink" value="<%=  "/jsp/Main.jsp?Action=siap.siep.ordineesecuzione.action.ActStampaComunicazioneL78del2013&autorita="+lCodTipoAutorita+"&IdEvento="+lEve.getEvento().getIdEvento()+"&IdPosizioneGiuridica="+lPosizione.getCodPosizioneGiuridica()%>"/>
		</jsp:include>
	<%}

	  if((lEve.getEvento().getFlagDocumentoRegistrato()!=null) &&
  		 (lEve.getEvento().getFlagDocumentoRegistrato().compareTo("S")==0) )
  	  {%>
			<!-- BOTTONE DI TRASFERIMENTO -->
		    <td class="LBG">
		    	<a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.ordineesecuzione.action.ActLoadTrasferisciProvvedimentoL78del2013&autorita=<%=lCodTipoAutorita%>&IdEvento=<%=lEve.getEvento().getIdEvento()%>&titolo=<%=lTitolo%>">
		        <img src="<%=IWebConstants.IMAGES_DIR%>net24.gif" alt="Trasferisci" width="24" height="24" border="0">
		      	</a>
		    </td>
	<%}%> 
	  
<%	  if(tornasuelenco.equals("RicercaProcedimentiTrasmessiL78del2013")  )
	  {%>	  
...
<%
        ...
        if ( ((penaresidua.getFlagErgastolo() == null) || 
        			...
        			!penaresidua.getFlagErgastolo().equals("D"))) && penaresidua.getDataFine()!=null )
        {
...
<%
  ...
  if(lIstDet != null && lIstDet.getDescrTipoIstituto() != null &&  !lIstDet.getDescrTipoIstituto().isEmpty() )
  { 
		String lNotificaIstituto = lIstDet.getDescrTipoIstituto();
		if (lIstDet.getDescrComune() != null)
			lNotificaIstituto += " di " + lIstDet.getDescrComune();
		if (lIstDet.getIndirizzo() != null)
			lNotificaIstituto += " - " + lIstDet.getIndirizzo();
	    %>
	     <tr>
...
<%
				if( lEve.getNotifiche() != null && (lEve.getNotifiche().length >= lEve.getAvvocati().length) )
				{
					for (int i = 0; i < lEve.getNotifiche().length; i++)
					{
					  
						if  (lEve.getNotifiche()[i].getAvvIdAvvocatoFascicoloSiep() != null 
					    && lEve.getNotifiche()[i].getAvvIdAvvocatoFascicoloSiep().equals
					    	(lEve.getAvvocati()[lIndex].getAvvocatoFascicoloSiepModel().getIdAvvocatoFascicoloSiep() ) )
					  	{
	...
	%>
						        <tr>
						        	...
						        	<td class="L" colspan=2>
						          	<font class="campo"><%=StringUtils.toStringJSP( lEve.getNotifiche()[i].getAutoritaEsterna().getDescrTipoAutorita() )%></font>&nbsp;
						           		di
						          	<font class="campo"><%=StringUtils.toStringJSP( lEve.getNotifiche()[i].getAutoritaEsterna().getDescrSede())%></font>&nbsp;
						        	</td>
						        </tr>
	<%					    
						  	}
	...
	%>
					        	<tr>
					          	<td class="l">Note</td>
					          	<td class="L" colspan="2"><font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[i].getNote())%></font>&nbsp;</td>
					        	</tr>
	<%					      
						    }				      
						}
    ...
    if( lEve.getNotifiche() != null && lEve.getNotifiche().length > 0 )
	{
		for (int i = 0; i < lEve.getNotifiche().length; i++)
		{
				...
				if( lEve.getNotifiche()[i].getUffCodUfficio() != null )
				{
					// NotificaModel lUfficio = lEve.getNotifiche()[i].getUfficio();
			%>				 
					 <tr>
						<td class="l">Ufficio Preposto</td >
							 <td class="l">
							 <font class="campo"><%=StringUtils.toStringJSP( lEve.getNotifiche()[i].getUfficio().getDescrTipoUfficio())%></font>
						</td>
						 </tr>
						  <tr>
				        <td class="l">Sede</td><td class="L">
				          <font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[i].getUfficio().getDescrComune())%></font>
				        </td>
				      </tr>
				       <%if(lEve.getNotifiche()[i].getNote() != null)
						 {%>
								<tr>
						        	<td class="l">Note</td>
						        	<td  class="l">
						          	<font class="campo"><%=StringUtils.toStringJSP(lEve.getNotifiche()[i].getNote() )%>&nbsp;</font>
						        	</td>
						      	</tr>	 
			...
			<%			}
				...
				{
						if( lEve.getNotifiche()[i].getCodTipoNotifica() != null &&
							lEve.getNotifiche()[i].getAutEstIdAutoritaEsterna() != null &&
							lEve.getNotifiche()[i].getCodTipoNotifica().equals("PC"))
						{
							%>
							<tr>
      ...
      <table>
        ...
        <tr>
          <td class="L">
            ...
            <input type="HIDDEN" name="motivo" value="<%=lEve.getEvento().getCodMotivo()%>">
            ...
            <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.ordineesecuzione.action.ActUploadL78del2013">
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_ID_EVENTO%>" value="<%= lEve.getEvento().getIdEvento() %>">
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>" value="siap.siep.ordineesecuzione.action.ActDettaglioComunicazioneL78del2013">
