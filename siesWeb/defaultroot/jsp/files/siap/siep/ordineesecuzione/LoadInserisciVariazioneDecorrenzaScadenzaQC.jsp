...
<jsp:useBean id="ggdataperv"       scope="request" class="java.lang.String"/>
<jsp:useBean id="mmdataperv"       scope="request" class="java.lang.String"/>
<jsp:useBean id="aadataperv"       scope="request" class="java.lang.String"/>
<jsp:useBean id="motivazioni"       scope="request" class="java.lang.String"/>


<%
//==============================================================================
// Form per l'inserimento della comunicazione Variazione decorrenza/scadenza
// pena per soggetti in espizione questa causa
//==============================================================================
  ...
  <head>
    ...
    <script language="JavaScript">
      ...
      function Verify()
      {
        ...
        // Data Fine Pena - Solo se non è ERGASTOLO
     ...
     <% } %>	
        ...
        <% 	if( lPosizione.getCodPosizioneGiuridica().equals("04") ) 
        	{%>
			        ...
			        {
			          alert("Sede Ufficio Esecuzione Penale Esterna Obbligatoria");
			          return false;
			        }

			        if(document.LoadInserisciOrdineEsecuzione.<%=ICostantiOrdineEsecuzione.CAMPO_COD_SEDE_UDS%>.value == "")
			        {
			          alert("Sede Ufficio di Sorveglianza Obbligatoria");
			          return false;
			        }
          
			        if(document.LoadInserisciOrdineEsecuzione.<%=ICostantiOrdineEsecuzione.CAMPO_COD_SEDE_TDS%>.value == "")
			        {
			          alert("Sede Tribunale di Sorveglianza Obbligatoria");
			          return false;
			        }
        <% 	} 
        	else 
...
<body class="corpo">
  ...
  <form method="POST" name="LoadInserisciOrdineEsecuzione" action="<%=IWebConstants.PG_MAIN%>">
    <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.ordineesecuzione.action.ActInserisciVariazioneDecorrenzaScadenzaQC">
    <input type="HIDDEN" value="<%=StringUtils.toStringJSP(penaresidua.getIdPenaResidua())%>"        type="text" name="<%= ICostantiPenaResidua.CAMPO_ID_PENA_RESIDUA %>">
    <input type="HIDDEN" value="<%=StringUtils.toStringJSP(lPosizione.getCodPosizioneGiuridica())%>" type="text" name="<%=ICostantiPosizioneGiuridica.CAMPO_COD_POSIZIONE_GIURIDICA%>">
    
    <input type="HIDDEN" name="<%= ICostantiOrdineEsecuzione.CAMPO_GIORNO_PERVENIMENTO_VARIAZIONE %>" value="<%=ggdataperv%>">
    <input type="HIDDEN" name="<%= ICostantiOrdineEsecuzione.CAMPO_MESE_PERVENIMENTO_VARIAZIONE %>"   value="<%=mmdataperv%>">
    <input type="HIDDEN" name="<%= ICostantiOrdineEsecuzione.CAMPO_ANNO_PERVENIMENTO_VARIAZIONE %>"   value="<%=aadataperv%>">
    
    <input type="HIDDEN" name="<%= ICostantiPenaResidua.CAMPO_GIORNO_DATA_INIZIO%>" value="<%=DateUtils.getDateToString(penaresidua.getDataInizio(), "dd") %>">
    <input type="HIDDEN" name="<%= ICostantiPenaResidua.CAMPO_MESE_DATA_INIZIO%>"   value="<%=DateUtils.getDateToString(penaresidua.getDataInizio(), "MM") %>">
    <input type="HIDDEN" name="<%= ICostantiPenaResidua.CAMPO_ANNO_DATA_INIZIO%>"   value="<%=DateUtils.getDateToString(penaresidua.getDataInizio(), "yyyy") %>">
    
    
    <input type="HIDDEN" value="<%=motivazioni%>" type="text" name="<%=ICostantiOrdineEsecuzione.CAMPO_MOTIVAZIONI_VARIAZIONE%>">
        
    <table>
      <tr>
        <td class="l">Posizione Giuridica </td>
        <td class="L" colspan=5><font class="campo"><%=lPosizione.getDescrPosizioneGiuridica()%></font>
        </td>        
      </tr>
      ...
      <tr>
        ...
        <td class="L" colspan="1">
          <% if (ggdataperv.length()>0) {%>
          <font class="campo"><%=ggdataperv%>-<%=mmdataperv%>-<%=aadataperv%></font>
          <% } %>
        </td>
      </tr>
      <tr>
        <td class="l">Motivazioni</td>
        <td class="L" colspan="100%"><font class="campo"><%=motivazioni%></font></td>
      </tr>   
      ...
      <% } else { %>
        <% if(penaresidua.getDataFinePresunta() != null ) { %>
          <td class="l">Nuova Data Fine Pena</td>
          <td class="L" colspan=2>
    ...
    <table width=100%>
      ...
      <!-- ================================================================= -->
      <tr>
        <td class="Titolo" colspan=6> Notifica Servizio Sociale</td>
      </tr>
      <tr>
        <td class="l">Destinatario</td>
        <td class="l" colspan=3>UFFICIO DI ESECUZIONE PENALE ESTERNA</td>
<input type="hidden" value="SSPA" >
      </tr>
      <tr>
        <td class="l">Sede<font class=ob>(*)</font></td>
        <td class="l">
      ...
      <!-- ================================================================= -->
      <tr>
        <td class="Titolo" colspan=6> Notifica Magistrato di Sorveglianza</td>
      </tr>
      <tr>
        <td class="l">Destinatario</td>
        <td class="l" colspan=3>UFFICIO DI SORVEGLIANZA</td>
<input type="hidden" value="UDS" >
      ...
      <tr>
         <td class="l">Destinatario</td >
         <td class="L" colspan=3>TRIBUNALE DI SORVEGLIANZA</td>
      </tr>
      <tr>
        <td class="l">Sede <font class=ob>(*)</font></td>
        <td class="L">
      ...
      <tr>
        ...
        <% 
        if (lLuogoDetenzione != null && lLuogoDetenzione.getIdLuogoDetenzione()!=null && lLuogoDetenzione.getIstitutoDetenzione()!=null)
        {%>
        <td class="l">
          <input type="hidden"  name="<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>" value="<%=lLuogoDetenzione.getIstDetIdIstitutoDetenzione()%>" >
    ...
    %>
        ...
        <table width=100%>
          <tr>
            ...
            <td class="L" colspan=3>
               <select Title="Autorita Destinazione" class="small" name="<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>" >
                 <%=autoritaEsternaN%>
               </select>
            </td>
          </tr>
  ...
  <%
       ...
       )
    {
      ...
      {%>
          ...
          frmvalidator.addValidation("<%= ICostantiPenaResidua.CAMPO_ANNO_DATA_FINE%>","lt=2100");
    <%}
    }
  %>
