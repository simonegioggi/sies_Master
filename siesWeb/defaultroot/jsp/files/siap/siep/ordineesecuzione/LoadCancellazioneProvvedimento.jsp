...
//09/04/2019 in caso di annullamento Comunicazione provvedimento il parametro 'IdIstruttoriaCumulo' e 'idEventoCumulo' consente di eseguire correttamente la nextAction
 if( request.getParameter("IdIstruttoriaCumulo") != null)
     nAction+="&IdIstruttoriaCumulo="+request.getParameter("IdIstruttoriaCumulo").toString();

 if( request.getParameter("idEventoCumulo") != null)
     nAction+="&idEventoCumulo="+request.getParameter("idEventoCumulo").toString();

//cambia l'ordinamento della ricerca nella visualizzazione
String nOrdinamento ="";
  if( request.getParameter("lOrdinamento") != null)
      nOrdinamento=request.getParameter("lOrdinamento").toString();
%>
  ...
  <%}%>
    ...
    <script language="JavaScript">
      function verify()
      {
        ...
        <% if( request.getParameter("campoSIUS") != null) { %>
        window.opener.top.frames['centrale'].frames['body'].location.href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.sius.provvedimento.action.ActAnnullaProvvedimento&IdEvento="+<%=IdEvento%>+"&motivazioni="+lmotivazioni+"<%=nAction%>&TornaQui=10";
        <% } else { %>
        window.opener.top.frames['centrale'].frames['body'].location.href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.ordineesecuzione.action.ActCancellaProvvedimento&IdEvento="+<%=IdEvento%>+"&motivazioni="+lmotivazioni+"<%=nAction%>&lOrdinamento=<%=nOrdinamento%>";
        <% }%>
