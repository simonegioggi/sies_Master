...
<html>
<head>
  <title>[S.I.E.S.] - Gestione Legge 78 del 2013  </title>
  ...
  <script language="JavaScript">
    ...
    {
        var codTipoSede = document.LoadTrasferisciProvvedimentoL78del2013.<%=ICostantiNuovaIstanza.CAMPO_COD_TIPO_UFFICIO_DESTINATARIO%>.value
    ...
    {
      var ritorno = true;
      if ((document.LoadTrasferisciProvvedimentoL78del2013.<%=ICostantiNuovaIstanza.CAMPO_COD_TIPO_UFFICIO_DESTINATARIO%>.value.length == 0)  || 
      		(document.LoadTrasferisciProvvedimentoL78del2013.<%=ICostantiNuovaIstanza.CAMPO_COD_LUOGO_DESTINATARIO%>.value.length == 0))
      {
        alert("Selezionare l'ufficio destinatario");
        ritorno = false;
      }
			var tipUff = document.LoadTrasferisciProvvedimentoL78del2013.<%=ICostantiNuovaIstanza.CAMPO_COD_TIPO_UFFICIO_DESTINATARIO%>.value;
      		document.LoadTrasferisciProvvedimentoL78del2013.tipoUfficioDestinatario.value = tipUff;
      		
      		var tipUff2 = document.LoadTrasferisciProvvedimentoL78del2013.tipoUfficioDestinatario.value;
  ...
  <body class="corpo">
  <form method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadTrasferisciProvvedimentoL78del2013" onsubmit="document.forms[0].go.disabled=true">
  <table>
...
%>
		...
		<%-- BOTTONE DI STAMPA -->
			<td class="LBG">
    		<%--a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.ordineesecuzione.action.ActStampaTrasferimentoProvvedimento&IdEvento=<%=IDEvento%>&CodLuogoDestinatario=Javascript:document.LoadTrasferisciProvvedimentoL78del2013.<%=ICostantiNuovaIstanza.CAMPO_COD_LUOGO_DESTINATARIO%>.value&CodTipoUfficioDestinatario='Javascript:document.LoadTrasferisciProvvedimentoL78del2013.<%=ICostantiNuovaIstanza.CAMPO_COD_TIPO_UFFICIO_DESTINATARIO%>.value'"-->
      ...
      <tr>
        <td class="l">Sede Destinatario </td><td class="L">
          <input title="Sede Destinatario"  value="<%=sedetribunale%>" type="text" name="<%= ICostantiNuovaIstanza.CAMPO_COD_LUOGO_DESTINATARIO %>"  maxlength="35" size="35">
            <a href="Javascript:ListaComuniTds('LoadTrasferisciProvvedimentoL78del2013','<%= ICostantiNuovaIstanza.CAMPO_COD_LUOGO_DESTINATARIO %>');">
