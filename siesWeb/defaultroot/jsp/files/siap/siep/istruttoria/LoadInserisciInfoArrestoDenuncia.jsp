...
<html>
  <head>
    <title>[S.I.E.S.] - Gestione Richiesta Arresto e/o Denuncia </title>
    ...
    function Verify()
	  {
		  if (document.LoadInserisciInfoArrestoDenuncia.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
			  document.LoadInserisciInfoArrestoDenuncia.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciInfoArrestoDenuncia.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
		  if (document.LoadInserisciInfoArrestoDenuncia.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
			  document.LoadInserisciInfoArrestoDenuncia.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciInfoArrestoDenuncia.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value;

		  var data_to_verify = document.LoadInserisciInfoArrestoDenuncia.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciInfoArrestoDenuncia.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciInfoArrestoDenuncia.<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>.value;
    ...
    if(!CompareDate("<%=datairrevocabilita%>",data_to_verify))
      {
        ...
        document.LoadInserisciInfoArrestoDenuncia.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.focus();
        return false;
      }
 ...
 <body class="corpo" >
   <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
      <font class="label">Funzione :</font> &nbsp;&nbsp;<font class="campo">Richiesta Informazioni Arresto e/o Denuncia</font>
...
<br>
  ...
  <br>
  <FORM method="POST" name="LoadInserisciInfoArrestoDenuncia" action="<%= IWebConstants.PG_MAIN%>">
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.istruttoria.action.ActInserisciInfoArrestoDenuncia">
 ...
 <tr><td class="Titolo" colspan=6>Destinatario  </td></tr>
    ...
    <tr><td class="l">Luogo Destinazione <font class="ob">(*)</font></td><td class="L">
       ...
       <a href="Javascript:ListaComuni('LoadInserisciInfoArrestoDenuncia','<%= ICostantiIstruttoria.SEDE_AUTORITA_DESTINATARIO %>');">
       <img src="/images/filefolder.gif" border=0></a></td>
    </tr>
    <tr><td class="Titolo" colspan=6>Oggetto  </td></tr>
   <tr>
      <td class="l">Data Arresto / Denuncia <font class="ob">(*)</font></td>
...
<script language="JavaScript" type="text/javascript">
  var frmvalidator  = new Validator("LoadInserisciInfoArrestoDenuncia");
