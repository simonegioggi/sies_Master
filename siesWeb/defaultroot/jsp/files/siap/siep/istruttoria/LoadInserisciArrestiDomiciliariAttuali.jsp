...
<html>
  <head>
    <title>[S.I.E.S.] - Gestione Richiesta Arresti Domiciliari Attuali </title>
    ...
    function Verify()
    {
      if (document.LoadInserisciArrestiDomiciliariAttuali.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
              document.LoadInserisciArrestiDomiciliariAttuali.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciArrestiDomiciliariAttuali.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
      if (document.LoadInserisciArrestiDomiciliariAttuali.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
              document.LoadInserisciArrestiDomiciliariAttuali.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciArrestiDomiciliariAttuali.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value;

      var data_to_verify = document.LoadInserisciArrestiDomiciliariAttuali.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciArrestiDomiciliariAttuali.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciArrestiDomiciliariAttuali.<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>.value;
    ...
    if(!CompareDate("<%=datairrevocabilita%>",data_to_verify))
      {
        ...
        document.LoadInserisciArrestiDomiciliariAttuali.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.focus();
        return false;
      }
    }
  </script>
 ...
 <body class="corpo" >
   <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
       <font class="label">Funzione :</font> &nbsp;&nbsp;<font class="campo">Richieste Informazioni Arresti Domiciliari Attuali</font>
      </td>
    </tr>
  </table>
  <br>
  ...
  <br>
  <FORM method="POST" name="LoadInserisciArrestiDomiciliariAttuali" action="<%= IWebConstants.PG_MAIN%>">
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.istruttoria.action.ActInserisciArrestiDomiciliariAttuali">
   <input type="hidden" name="<%=ICostantiIstruttoriaCumulo.CAMPO_ID_ISTRUTTORIA_CUMULO %>"	value="<%= IdIstruttoriaCumulo%>" >
    ...
    <tr><td class="l">Luogo Destinazione <font class="ob">(*)</font></td><td class="L">
       ...
       <a href="Javascript:ListaComuni('LoadInserisciArrestiDomiciliariAttuali','<%= ICostantiIstruttoria.SEDE_AUTORITA_DESTINATARIO %>');">
       <img src="/images/filefolder.gif" border=0></a></td>
    </tr>
    <tr>
      <td class="l">Arresti Domiciliari Attuali<font class="ob">(*)</font></td>
  ...
  <script language="JavaScript" type="text/javascript">
      var frmvalidator  = new Validator("LoadInserisciArrestiDomiciliariAttuali");
