...
<html>
<head>
<title>[S.I.E.S.] - Dettaglio Scadenzario Vane Ricerche</title>
...
<script language="JavaScript">
    ...
    //==========================================================================
    // Richiama la funzione di inserimento Sollecito
    //==========================================================================
    function InserisciSollecito(aIdEve,aIdFas)
    {
    
	      lAzione = "siap.siep.notifica.action.ActLoadInserisciSollecitiVerbVaneRicerche";
	      document.DettaglioScadeVaneRice.<%=ICostantiScadenzario.CAMPO_EVE_ID_EVENTO %>.value = aIdEve;
	      document.DettaglioScadeVaneRice.<%=ICostantiScadenzario.CAMPO_FAS_SIE_ID_FASCICOLO_SIEP %>.value = aIdFas;
	      document.DettaglioScadeVaneRice.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
	      document.DettaglioScadeVaneRice.submit();
    }

    //==========================================================================
    // Richiama la funzione di inserimento Rinnovo
    //==========================================================================
    function InserisciRinnovo(aIdEve,aIdFas)
    {
	      lAzione = "siap.siep.notifica.action.ActLoadOEOmessaNotifica";
	      document.DettaglioScadeVaneRice.<%=ICostantiScadenzario.CAMPO_EVE_ID_EVENTO%>.value = aIdEve;
	      document.DettaglioScadeVaneRice.<%=ICostantiScadenzario.CAMPO_FAS_SIE_ID_FASCICOLO_SIEP %>.value = aIdFas;
	      document.DettaglioScadeVaneRice.<%=IWebConstants.ACTION_FIELD%>.value = lAzione;
	      document.DettaglioScadeVaneRice.submit();
   	...
   	{
   			var a_fieldname_1 = "FP";
    		desktop = window.open("/jsp/Main.jsp?<%=IWebConstants.ACTION_FIELD%>=siap.siep.notifica.action.ActLoadRicercaSolleciti&formname="+a_formname+"&fieldname="+a_fieldname+"&fieldname_1="+a_fieldname_1, "DettaglioScadeVaneRice","toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=600,height=500");
  	}
  	
  	function ListaRinnovi(a_formname,a_fieldname)
   	{
   			var a_fieldname_1 = "FP";
    		desktop = window.open("/jsp/Main.jsp?<%=IWebConstants.ACTION_FIELD%>=siap.siep.notifica.action.ActLoadRicercaNotificaOmessa&formname="+a_formname+"&fieldname="+a_fieldname+"&fieldname_1="+a_fieldname_1, "DettaglioScadeVaneRice","toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=600,height=500");
  	}
...
<FORM action="<%=IWebConstants.PG_MAIN%>" method="post" name="DettaglioScadeVaneRice" >
	<input type="hidden" name="<%=IWebConstants.ACTION_FIELD%>" value="">
	<input type="hidden" name="<%=ICostantiScadenzario.CAMPO_EVE_ID_EVENTO %>" value="">
	<input type="hidden" name="<%=ICostantiScadenzario.CAMPO_FAS_SIE_ID_FASCICOLO_SIEP %>" value="">
    <table>
      <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
        <td class="LBG">
        <font class="label">Funzione :</font>&nbsp;
<% 	if(scadenzario.getCodStatoNotifica().equals("PP") )
	{	 	
%>        
        <font class="campo">Dettaglio Scadenzario Vane Ricerche - Pervenuti</font>
<%	} 	
	else if (scadenzario.getCodStatoNotifica().equals("NP") )
	{	
%>   
     	<font class="campo">Dettaglio Scadenzario Vane Ricerche - Non Pervenuti</font>
<%	}
	else
	{	
%>	   
  		<font class="campo">Dettaglio Scadenzario Vane Ricerche</font>
<%	} %>  		
      ...
      <td class="L">
        ...
        <font class="campo">
        	<a class="cliccabile" href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.sentenza.action.ActLoadDettaglioSentenza&<%=ICostantiSentenza.CAMPO_ID_SENTENZA%>=<%=lSentMod.getIdSentenza()%>" title="Sentenza">
          		<%=lSentMod.getAnnoSentenza()%> / <%=lSentMod.getNumeroSentenza()%></a>&nbsp;
...
%>      <table>  
			<tr>
          		<td colspan="2">
              		<INPUT class="bottone" type="button"   name="INSERISCIRINN" value="Inserisci Rinnovo" onClick="javascript:InserisciRinnovo('<%=evento.getIdEvento() %>','<%=lFas.getIdFascicoloSiep()%>');">
          		</td>
        	</tr>
        </table>
        <br>
        <table>	
      		<tr>
            	<td class="LGB">
          			<a href="Javascript:ListaRinnovi('DettaglioScadeVaneRice',<%=notifica.getIdNotifica()%>);">Elenco Notifiche Precedenti</a>
        		</td>
        	</tr>
        </table>
        <br>        
...
%>   	<table>
			<tr>
          		<td colspan="2">
               		<INPUT class="bottone" type="button" name="INSERISCISOLL" value="Inserisci Sollecito" onClick="javascript:InserisciSollecito('<%=evento.getIdEvento() %>','<%=lFas.getIdFascicoloSiep()%>');">
          		</td>
        	</tr>
      	</table>    
        <br>
       	<table>
      		<tr>
            	<td class="LGB">
          			<a href="Javascript:ListaSolleciti('DettaglioScadeVaneRice',<%=notifica.getIdNotifica()%>);">Elenco Solleciti Precedenti</a>
        		</td>
        	</tr>
        </table>
        <br>
