...
<%
	// MEV 16: aggiunto recupero di attributi dalla request e passati come parametro alla popup
	    ...
	    <script language="JavaScript" src="<%=IWebConstants.JS_DATE_CONTROL%>"></script>
	   	<script language="JavaScript">
			...
			function verify() {
	   		 	...
	   		 	window.opener.top.frames['centrale'].frames['body'].location.href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=nextAction%>&IdEvento=" +
	   		 			"<%=IdEvento%>" + "&IdImpugnazione=" + "<%=IdImpugnazione%>" +
	   		 			"&<%=ICostantiImpugnazione.CAMPO_MOTIVO_ANNULLAMENTO%>=" + lmotivazioni + "&TornaQui=10" +
	   		 			// MEV 16: aggiunte impostazioni variabili
	   		 			"&tipoWS=" + "<%=tipoWS%>" + "&idSoggetto=" + "<%=idSoggetto%>" +
	   		 			"&idSentenza=" + "<%=idSentenza%>" + "&idFascicoloSiep=" + "<%=idFascicoloSiep%>";
	 			window.close();
			}
		</script>
 	</head>
