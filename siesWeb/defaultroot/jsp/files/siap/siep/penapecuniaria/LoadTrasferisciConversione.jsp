...
<html>
<head>
  <title>[S.I.E.S.] - Gestione Conversione Pene Pecuniarie </title>
  ...
  <script language="JavaScript">
    ...
    {
      codTipoUfficio = document.LoadTrasferisciConversione.<%=ICostantiUfficio.CAMPO_TIPO_UFFICIO%>.value;
      if (codTipoUfficio=='-'){
          alert("Selezionare il tipo di ufficio emittente");
      		document.LoadTrasferisciConversione.<%=ICostantiUfficio.CAMPO_TIPO_UFFICIO%>.focus();
      }
      else {
...
<%
        ...
        lAction = "siap.siep.penapecuniaria.action.ActTrasferisciConversione";
%>
        <font class="campo">Trasferimento Conversione Pene Pecuniarie</font>
    </tr>
  </table>
  <br>
    <jsp:include page="/jsp/files/siap/siep/fascicolo/DettaglioSoggettoSentenza.jsp"/>
  <br>
  <form method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadTrasferisciConversione" onsubmit="document.forms[0].go.disabled=true">
    <table cellspacing=2 cellpadding=2>
      <tr>
        <td class="l">Destinatario </td >
        
      </tr>
	  <!-- 28/07/2015  Notifica per l'Ente di Sorveglianza -->
	  <tr>
	    ...
	    <td class="L">
	      ...
	      <input type="text" title="ufficio" value="<%=StringUtils.toStringJSP(lEve.getNotifiche()[0].getUfficio().getDescrComune())%>"  name="<%=ICostantiEvento.CAMPO_COD_LUOGO_DESTINATARIO%>" maxlength="35" size="25">
    			  <a href="Javascript:ListaComuniUfficio('LoadTrasferisciConversione','<%=ICostantiEvento.CAMPO_COD_LUOGO_DESTINATARIO%>');">
