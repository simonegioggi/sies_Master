...
<table>
	<tr>
		...
		<td class="LBG"><font class="label">Funzione :</font>&nbsp; <font
			class="campo">Dettaglio Sentenza</font></td>
		<%
...
<table cellspacing=2 cellpadding=2>
	...
	<tr>
		...
		<td class="L" colspan=5><font class="campo"> <%=anno_reg%>
		/ <%=num_reg%>&nbsp;&nbsp;&nbsp; <%=reg%></font></td>
		<%
		}
		%>
	...
	<tr>
		...
		<td class="L" colspan=5>
			<font class="campo">
			<%=StringUtils.toStringJSP(lSentenza.getDescrTipoProvvedimentoRif())%>
			</font>&nbsp;
		</td>
	</tr>
	<tr <%=vis%>>
		...
		<td class="L" colspan=5><font class="campo"><%=StringUtils.toStringJSP(lSentenza
									.getDescrTipoProvvRif())%></font>&nbsp;</td>
	</tr>
	<tr>
		<td class="l">Data <%=tipoProvvedimento%></td>
		<td class="L" colspan=5><font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(
							lSentenza.getDataProvvRif(), "dd-MM-yyyy"))%></font>&nbsp;</td>
	</tr>
	<tr>
		<td class="l">Anno/Numero <%=tipoProvvedimento%></td>
		<td class="L" colspan=5><font class="campo"><%=StringUtils.toStringJSP(lSentenza
									.getAnnoProvvRif())%></font>&nbsp; / <font class="campo"><%=StringUtils.toStringJSP(lSentenza.getNumeroProvvRif())%></font>&nbsp;
		</td>
	</tr>
	<tr>
		<td class="l">Autorità Emittente</td>
		<td class="L" colspan=5><font class="campo"><%=StringUtils.toStringJSP(lSentenza
							.getDescrTipoAutoritaProvvRif())%></font>&nbsp;</td>
	</tr>
	<%
				if (lSentenza.getCodTipoAutoritaProvvRif().equals("DIB")
	...
	<tr>
		<td class="l">Luogo Emittente</td>
		<td class="L" colspan=5><font class="campo"><%=StringUtils
							.toStringJSP(lSentenza.getDescrLuogoProvvRif())%></font>&nbsp;</td>
	</tr>
	<tr>
		<td class="l">Sezione Autorità Emittente</td>
		<td class="L" colspan=5><font class="campo"><%=StringUtils.toStringJSP(lSentenza
							.getNumSezioneAutoritaProvvRif())%></font>&nbsp;</td>
	</tr>	
	<tr <%=vis%>>
	...
	<tr <%=vis%>>
		...
		<td class="L" colspan=5>
			<font class="campo">
			<%=StringUtils.toStringJSP(lSentenza.getDescrTipoProvvedimentoAltro())%>
			</font>&nbsp;
		</td>
	</tr>
	<tr <%=visCas%>>
		...
		<td class="L" colspan=5><font class="campo"><%=StringUtils.toStringJSP(lSentenza
							.getNote1DecisioneCassazione())%></font>&nbsp; / <font class="campo"><%=StringUtils.toStringJSP(lSentenza
							.getNote2DecisioneCassazione())%></font>&nbsp;</td>
	</tr>
	<tr <%=vSpec %>>
		<td class="l">Anno/Numero <%=tipoProvvedimentoCas %></td>
		<td class="L" colspan=5><font class="campo"><%=StringUtils.toStringJSP(lSentenza
							.getAnnoSentenzaCassazione())%></font>&nbsp; / <font class="campo"><%=StringUtils.toStringJSP(lSentenza
							.getNumeroSentenzaCassazione())%></font>&nbsp;</td>
	</tr>
	<tr <%=visCas%>>
		<td class="l">Anno/Numero Raccolta Generale</td>
		<td class="L" colspan=5><font class="campo"><%=StringUtils.toStringJSP(lSentenza
							.getAnnoRaccoltaGenerale())%></font>&nbsp; / <font class="campo"><%=StringUtils.toStringJSP(lSentenza
							.getNumeroRaccoltaGenerale())%></font>&nbsp;</td>
	</tr>
	<tr <%=visCas%>>
	...
	<tr>
		...
		<FORM method="POST" name="FascSiep"
			action="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.fascicolo.action.ActLoadInserisciFascicolo&lTipoFunzione=<%=lTipoFunzione%>">
		<br>
		<INPUT class="bottone" type="button" name="NS" value="Prosegui"
			onclick="Javascript:Disabilita();"></FORM>
		</td>

		<td class="lNoBord" colspan="2">
		<FORM method="POST" name="Abbandona"
			action="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.sentenza.action.ActLoadDettaglioSentenza&<%=ICostantiSentenza.CAMPO_ID_SENTENZA%>=<%=lSentenza.getIdSentenza()%>&lTipoFunzione=ritornodettaglio">
		<br>
		<INPUT class="bottone" type="button" name="A" value="Abbandona"
			onclick="Javascript:return Verify();"></FORM>
