...
<jsp:useBean id="autPoliziaNotifica"  scope="request" class="siap.siep.notifica.model.NotificaModel"/>
...
<%
   ...
   {
%>
     <font class="campo">Dettaglio Ordine di Scarcerazione a seguito di ordinanza liberazione anticipata - DETENUTO PER ALTRA CAUSA</font>
<%
   }
   else
   {
%>
     <font class="campo">Dettaglio Ordine di Scarcerazione a seguito di ordinanza liberazione anticipata - <%=lPosizione.getDescrPosizioneGiuridica()%></font>
<%
   }
%>
</td>
...
<!-- BOTTONE DI STAMPA -->
   <jsp:include page="<%= ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIEP%>">
     <jsp:param name="ActionLink" value="<%="/jsp/Main.jsp?Action=siap.siep.ordinescarcerazione.action.ActStampaOSLiberazioneAnticipata&IdEvento="+eventonotifica.getEvento().getIdEvento()+"&codposizionegiuridica="+posizioneluogoaltra.getPosizioneGiuridica().getCodPosizioneGiuridica()%>"/>
   </jsp:include>
<%
    }
%>
...
<%
  int cont=0;
  //la lunghezza delle notifiche è -3 perchè due sono gli avvocati e uno è un destinatario nel caso ci sia fungibilità
%>
   <tr>
     <td class="l">Destinatario</td>
     <td class="L">
      <font class="campo"><%=StringUtils.toStringJSP(tiposorveglianza)%></font>
      di
      <font class="campo"><%=StringUtils.toStringJSP(sedesorveglianza)%></font>
     </td>
   </tr>
...
%>
  ...
  <%
  // MEV29 - 13/03/2015 - Se pos 02 è prevista l'autorità di polizia al posto 
  //                      dell'istituto
  ...
  <% if (autPoliziaNotifica.getIdNotifica()!=null ) { %>
  <tr>
    <td class="l">Autorità Preposta al Controllo</td>
    <td class="L">
      <font class="campo"><%=StringUtils.toStringJSP( autPoliziaNotifica.getAutoritaEsterna().getDescrTipoAutorita())%></font>&nbsp;
      di
      <font class="campo"><%=StringUtils.toStringJSP( autPoliziaNotifica.getAutoritaEsterna().getDescrSede())%></font>&nbsp;
      <font class="campo"><%=StringUtils.toStringJSP( autPoliziaNotifica.getNote())%></font>&nbsp;
    </td>
  </tr>
  <% } %>
...
while(count < eventonotifica.getNotifiche().length)
{
  ...
  if(lNotMod.getCodTipoNotifica().equals("E") && lNotMod.getAutoritaEsterna() != null)
  {
...
<%
  ...
  {
    if (eventonotifica.getNotifiche()!=null && eventonotifica.getNotifiche().length > cont && eventonotifica.getNotifiche()[cont].getAutoritaEsterna()!=null)
    {
%>
      <table>
       <tr>
      ...
      <table>
        ...
        <tr>
          <td class="L">
            ...
            <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.ordinescarcerazione.action.ActUploadOSLibAnt">
            <input type="HIDDEN" name="tipoMisura" value="OS_LIBERAZIONE_ANTICIPATA">
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_ID_EVENTO%>" value="<%= eventonotifica.getEvento().getIdEvento() %>">
            <input type="HIDDEN" name="IdPosizioneGiuridica" value="<%=lPosizione.getIdPosizioneGiuridica()%>">
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>" value="siap.siep.ordinescarcerazione.action.ActDettaglioOSLiberazioneAnticipata">
          </td>
        </tr>
      </table>
</form>
