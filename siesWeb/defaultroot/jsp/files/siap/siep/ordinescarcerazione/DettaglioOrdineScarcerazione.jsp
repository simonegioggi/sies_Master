...
%>
    ...
    <%if(lFascicoloAssociato.getFlagAltraCausa()!=null && lFascicoloAssociato.getFlagAltraCausa().equals("S"))
        {%>
          <font class="campo">Dettaglio Ordine Scarcerazione </font>
        <%}else
         {%>
          <font class="campo">Dettaglio Ordine Scarcerazione </font>
        <%}%>
       </td>
 <%if(lPosizione.getCodPosizioneGiuridica().equals("03") || lPosizione.getCodPosizioneGiuridica().equals("12") 
	  || lPosizione.getCodPosizioneGiuridica().equals("04") || lPosizione.getCodPosizioneGiuridica().equals("14") 
...
<!-- BOTTONE DI STAMPA -->
   <jsp:include page="<%= ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIEP%>">
     <jsp:param name="ActionLink" value="<%="/jsp/Main.jsp?Action=siap.siep.ordinescarcerazione.action.ActStampaOrdineScarcerazione&IdEvento="+lEve.getEvento().getIdEvento()%>"/>
   </jsp:include>
<%}%>
<%if (lEve.getEvento().getFlagDocumentoRegistrato()==null)
 {%>
...
<!-- BOTTONE DI STAMPA -->
   <jsp:include page="<%= ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIEP%>">
     <jsp:param name="ActionLink" value="<%="/jsp/Main.jsp?Action=siap.siep.ordinescarcerazione.action.ActStampaOSAltrePosizioni&IdEvento="+lEve.getEvento().getIdEvento()%>"/>
   </jsp:include>
<%}%>
<%if (lEve.getEvento().getFlagDocumentoRegistrato()==null)
 {%>
...
<%}else
  if(lEve.getNotifiche()!= null && lEve.getNotifiche().length > 0 && lEve.getNotifiche()[0].getAutoritaEsterna() != null)
   {
    %>
     <tr>
      <td class="l">Autorità Destinazione</td>
      ...
      <table>
        ...
        <tr>
          <td class="L">
            ...
            <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.ordinescarcerazione.action.ActUploadOS">
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_ID_EVENTO%>"  value="<%= lEve.getEvento().getIdEvento() %>">
           <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>"  value="siap.siep.ordinescarcerazione.action.ActDettaglioOrdineScarcerazione">
          </td>
        </tr>
      </table>
    </FORM>
