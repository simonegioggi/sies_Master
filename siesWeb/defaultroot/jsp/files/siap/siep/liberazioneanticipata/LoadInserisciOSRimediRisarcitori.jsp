...
<%
//==============================================================================
// Form per l'inserimento dell'Ordine di scarcerazione nuovo fine pena x soggetto
// 'In Espiazione': Detenuto o in Misura
//==============================================================================
%>
  ...
  <head>
    ...
    function Verify()
    {
      ...
      if (document.LoadInserisciOrdineScarcerazione.<%=ICostantiNotifica.CAMPO_MESE_DATA_INVIO%>.value.length==1)
        document.LoadInserisciOrdineScarcerazione.<%=ICostantiNotifica.CAMPO_MESE_DATA_INVIO%>.value='0'+document.LoadInserisciOrdineScarcerazioneLoadInserisciOrdineScarcerazione.<%=ICostantiNotifica.CAMPO_MESE_DATA_INVIO%>.value;
      ...
      if (!ControllaData(data_to_verify) )
      {
        ...
        document.LoadInserisciOrdineScarcerazione.<%=ICostantiNotifica.CAMPO_GIORNO_DATA_INVIO%>.focus();
        return false;
      }
      else if (CompareDate(data_to_verify,dataOdierna)== false) {
      ...
      //=====================================
      // Destinatari x ora Tutti obbligatori 
      //=====================================
      var contaDest = 0;
      ...
      if (typeof (document.LoadInserisciOrdineScarcerazione.<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>)!="undefined") {
        if(document.LoadInserisciOrdineScarcerazione.<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>.value == "")
        {
          ...
          document.LoadInserisciOrdineScarcerazione.Comune.focus();
          return false;
        }
        else {contaDest++;}
      }
      ...
      // UEPE
      if (typeof (document.LoadInserisciOrdineScarcerazione.<%=ICostantiCSSA.CAMPO_ID_CSSA%>)!="undefined") {
        if(document.LoadInserisciOrdineScarcerazione.<%=ICostantiCSSA.CAMPO_ID_CSSA%>.value == "")
        {
          alert("UEPE Competente obbligatorio");
          document.LoadInserisciOrdineScarcerazione.Indirizzo.focus();
          return false;
        }
        else { contaDest++;}
      }
      ...
      // TDS
      if (typeof (document.LoadInserisciOrdineScarcerazione.<%=ICostantiOrdineScarcerazione.CAMPO_SEDE_TRIBUNALE %>)!="undefined") {
        if(document.LoadInserisciOrdineScarcerazione.<%=ICostantiOrdineScarcerazione.CAMPO_SEDE_TRIBUNALE %>.value == "")
        {
          ...
          document.LoadInserisciOrdineScarcerazione.<%=ICostantiOrdineScarcerazione.CAMPO_SEDE_TRIBUNALE %>.focus();
          return false;
        }
        else {contaDest++;}
      }
      ...
      // MDS
      if (typeof (document.LoadInserisciOrdineScarcerazione.<%=ICostantiOrdineScarcerazione.CAMPO_SEDE_UDS %>)!="undefined") {
        if(document.LoadInserisciOrdineScarcerazione.<%=ICostantiOrdineScarcerazione.CAMPO_SEDE_UDS %>.value == "")
        {
          ...
          document.LoadInserisciOrdineScarcerazione.<%=ICostantiOrdineScarcerazione.CAMPO_SEDE_UDS %>.focus();
          return false;
        }
        else {contaDest++;}
      }
      
      //alert ("contaDest = "+contaDest);
      return true;
    }
...
<body class="corpo">
  <table>
    <tr>
      ...
      <td class="LBG">
        <font class="label">Funzione :</font>&nbsp;&nbsp;<%//ticket 20190805017%>
        <font class="campo">Ordine di Scarcerazione A Seguito di <% if ("02".equals(EventoSIUS.getCodTipoProvvedimento())) { %>Decreto<% } else { %>Ordinanza<% } %> <% if (EventoSIUS.getCodMotivo()!= null && "9027".equals(EventoSIUS.getCodMotivo())) {%> Reclamo<%}%> Rimedi Risarcitori D.L. 92/2014 Per condannato <% if (lPosizione.isMisuraAlternativa()) {%>in Misura<% } else { %>Detenuto<% } %></font>
      </td>
    </tr>
  </table>
  ...
  <FORM method="POST" name="LoadInserisciOrdineScarcerazione" action="<%= IWebConstants.PG_MAIN%>">
    <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sico.libertaanticipata.action.ActInserisciOSRimediRisarcitori">
    <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_EVE_ID_EVENTO%>" value="<%=StringUtils.toStringJSP(EventoSIUS.getIdEvento())%>">

    <input type="HIDDEN" name="isMisura" value="<%= (lPosizione.isMisuraAlternativa())?"S":"N"%>">
    
    <input type="HIDDEN" name="<%=ICostantiFungibilita.CAMPO_ID_FUNGIBILITA%>" value="<%=StringUtils.toStringJSP(IdFungibilita,"")%>">
...
%>
  <table>
    ...
    <tr>
      ...
      <td class="L" colspan="3"> <font class="campo"><%=StringUtils.toStringJSP(lSommaLiquidata,"&nbsp;")%> </font> &euro;</td>
    </tr>
    <% } %>
...
// - Se Detenuto:  Istituto di detenzione
//                 Ufficio che ha concesso il DL92 TDS o UDS
// - Se In Misura: UEPE
//                 Istituto di detenzione (se semilibero)
//                 Ufficio che ha concesso il DL92 TDS o UDS
...
%> 
  ...
  <table style="width: 95%;">
    ...
    <%
    //==================================
    // Istituto x Detenuto o Semilibero
    //==================================
    if (  !lPosizione.isMisuraAlternativa()
        || "14".equals(lPosizione.getCodPosizioneGiuridica()) // Semilibertà 
       ) 
    { 
    %>    
    <tr>
    ...
    %>
    <tr>
      <td class="l">UEPE Competente <font class=ob>(*)</font></td>
      <td class="l">
        <input readonly Title="UEPE Competente" name="Indirizzo"  value="" size="60">
        <input type="hidden" Title="IdCSSA" name="<%=ICostantiCSSA.CAMPO_ID_CSSA %>" value="" >
      ...
      <td class="l">Note</td>
      <td class="L">
        <TEXTAREA title="Note" name="<%= ICostantiMisuraAlternativa.CAMPO_NOTE_CSSA %>"  cols=35 rows=2 ></textarea>
      </td>
    </tr>
    ...
    <%
    //============================================================
    // Autorità di Sorveglianza che ha emesso il provvedimento (C)
    // [per ora prevista per tutte le posizioni]
    //============================================================
    %> 
