...
<jsp:useBean id="stato_fascicoli" scope="request" class="java.lang.String"/>

<html>
<head>
  <title>[S.I.E.S.] - Presa in Carico - Ricerca Atti ricevuti per Competenza</title>
  ...
  <FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name='LoadRicercaProvSius'>
    ...
    <input type="HIDDEN" name="stato_fascicoli" value="<%=stato_fascicoli%>">
   ...
   <br>
    ...
    <table cellspacing=2 cellpadding=2>
      <tr>
        <td class="l">Stato&nbsp;&nbsp;&nbsp;</td>
        <td>
    <%--input type="HIDDEN" name="<%=ICostantiMessaggio.CAMPO_COD_ESITO%>" value="<%=ICostantiJMS.PRESAINCARICO%>" --%>
          <select title="Stato" name="<%=ICostantiMessaggio.CAMPO_COD_ESITO%>" >
           <option value="<%=ICostantiJMS.PRESAINCARICO%>" selected > Presi in carico </option>
           <option value="<%=ICostantiJMS.RESTITUITO%>" > Restituiti </option>           
          </select>
        </td>
      </tr>
    </table>
  ...
  <input type="HIDDEN" name="<%=ICostantiPresaincarico.CAMPO_ANNO_FASCICOLO_SIEP%>" value="">
  <input type="HIDDEN" name="<%=ICostantiPresaincarico.CAMPO_ANNO_FASCICOLO_SIUS%>" value="" >
