...
<table>
  ...
  <%if(Notifica != null && Notifica.equals("FP"))
    {%>
        <font class="campo">Dettaglio Sollecito Verbale Vane Ricerche - Forza di Polizia</font>
  <%}else if(Notifica != null && Notifica.equals("UG")){%>
        <font class="campo">Dettaglio Sollecito Verbale Vane Ricerche - Ufficiali Giudiziari</font>
     <%}%>
      </td>
<%if ((rinnovo.getFlagDocumentoRegistrato()!=null && rinnovo.getFlagDocumentoRegistrato().compareTo("N")==0)
     ...
     <%-- MEV NUOVA INFRASTRUTTURA: refactoring --%>
      <%--td class="LBG">
        <a href="/jsp/Main.jsp?Action=siap.siep.notifica.action.ActStampaSolleciti&idrinnovo=<%=rinnovo.getIdRinnovo()%>&notifica=<%=Notifica%>" onclick="javascript:lookUpload();">
           <img  align="middle" src="/images/print24.gif" alt="Generazione Stampa" width="24" height="24" border="0">
        </a>
      </td--%>
<!-- BOTTONE DI STAMPA -->
   <jsp:include page="<%= ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIEP%>">
     <jsp:param name="ActionLink" value="<%="/jsp/Main.jsp?Action=siap.siep.notifica.action.ActStampaSollecitiVerbVaneRicerche&idrinnovo="+rinnovo.getIdRinnovo()+"&notifica="+Notifica%>"/>
   </jsp:include>
     <%}%>
     </tr>
</table>
      ...
      <table>
        ...
        <tr>
          <td class="L">
            ...
            <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.notifica.action.ActUploadSolleciti">
            ...
            <input type="HIDDEN" name="<%=ICostantiNotifica.CAMPO_AZIONE_DETTAGLIO%>" value="siap.siep.notifica.action.ActLoadDettaglioSollecitiVerbVaneRicerche">
          </td>
        </tr>
      </table>
</form>
