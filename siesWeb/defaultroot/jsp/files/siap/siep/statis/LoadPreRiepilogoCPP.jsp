...
<html>
  <head>
    ...
    {	
	    ...
	    if (document.PreCPP.<%= ICostantiStatis.CAMPO_GIORNO_INIZIALE%>.value.length==1)
	       document.PreCPP.<%= ICostantiStatis.CAMPO_GIORNO_INIZIALE%>.value='0'+document.PreCPP.<%= ICostantiStatis.CAMPO_GIORNO_INIZIALE%>.value;
	    if (document.PreCPP.<%= ICostantiStatis.CAMPO_MESE_INIZIALE%>.value.length==1)
	       document.PreCPP.<%= ICostantiStatis.CAMPO_MESE_INIZIALE%>.value='0'+document.PreCPP.<%= ICostantiStatis.CAMPO_MESE_INIZIALE%>.value;
	
	    if (document.PreCPP.<%= ICostantiStatis.CAMPO_GIORNO_FINALE%>.value.length==1)
	       document.PreCPP.<%= ICostantiStatis.CAMPO_GIORNO_FINALE%>.value='0'+document.PreCPP.<%= ICostantiStatis.CAMPO_GIORNO_FINALE%>.value;
	    if (document.PreCPP.<%= ICostantiStatis.CAMPO_MESE_FINALE%>.value.length==1)
	        document.PreCPP.<%= ICostantiStatis.CAMPO_MESE_FINALE%>.value='0'+document.PreCPP.<%= ICostantiStatis.CAMPO_MESE_FINALE%>.value;
	
	    var data_inizio=document.PreCPP.<%=ICostantiStatis.CAMPO_GIORNO_INIZIALE%>.value+'/'+document.PreCPP.<%=ICostantiStatis.CAMPO_MESE_INIZIALE%>.value+'/'+document.PreCPP.<%=ICostantiStatis.CAMPO_ANNO_INIZIALE%>.value;
	    var data_fine=document.PreCPP.<%=ICostantiStatis.CAMPO_GIORNO_FINALE%>.value+'/'+document.PreCPP.<%=ICostantiStatis.CAMPO_MESE_FINALE%>.value+'/'+document.PreCPP.<%=ICostantiStatis.CAMPO_ANNO_FINALE%>.value;
		var data_sistema="<%=sysdate%>";
		
// Controllo Form Senza Nessun parametro di ricerca selezionato
		if(document.PreCPP.<%= ICostantiStatis.CAMPO_SOLO_ANNO_INIZIALE%>.value.length==0 &&  
			document.PreCPP.<%= ICostantiStatis.CAMPO_SOLO_ANNO_FINALE%>.value.length==0 &&
			document.PreCPP.<%= ICostantiStatis.CAMPO_ANNO_SEMESTRE%>.value.length==0 && 
			document.PreCPP.<%= ICostantiStatis.CAMPO_ANNO_TRIMESTRE%>.value.length==0 && 
			!ControllaData(data_inizio) && 
			!ControllaData(data_fine)  )	
		{
			alert('Nessun Parametro di Ricerca è presente nella Form ');
			document.PreCPP.<%= ICostantiStatis.CAMPO_GIORNO_INIZIALE%>.focus();
	    	return false;
		}
		...
		{	
			if(!ControllaData(data_inizio))
	      	...
	      	if(!ControllaData(data_fine))
	      	{
	        	...
	        	document.PreCPP.<%= ICostantiStatis.CAMPO_GIORNO_FINALE%>.focus();
	        	return false;
	      	}
...
// Controllo Parametri Solo Anno Iniziale/Solo Anno Finale Solo se NON sono VUOTI
		...
		else
		{
			if(document.PreCPP.<%= ICostantiStatis.CAMPO_SOLO_ANNO_INIZIALE%>.value.length==0)
			{
	        	...
	        	document.PreCPP.<%= ICostantiStatis.CAMPO_SOLO_ANNO_INIZIALE%>.focus();
	        	return false;
			}
			
			if(document.PreCPP.<%= ICostantiStatis.CAMPO_SOLO_ANNO_FINALE%>.value.length==0)
			{
				alert('Inserire Anno Finale');
	        	document.PreCPP.<%= ICostantiStatis.CAMPO_SOLO_ANNO_FINALE%>.focus();
	        	return false;
			}
			
			if(document.PreCPP.<%= ICostantiStatis.CAMPO_SOLO_ANNO_FINALE%>.value < document.PreCPP.<%= ICostantiStatis.CAMPO_SOLO_ANNO_INIZIALE%>.value )
			{
...
// Controllo parametri Semestre
		if(document.PreCPP.<%= ICostantiStatis.CAMPO_ANNO_SEMESTRE%>.value.length>0)
		{
			if(!document.PreCPP.Semestre[0].checked && 
				!document.PreCPP.Semestre[1].checked	)
			{
				alert('Selezionare Primo e/o Secondo Semestre');
	        	document.PreCPP.<%= ICostantiStatis.CAMPO_ANNO_SEMESTRE%>.focus();
	        	return false;				
			}	
...
// Controllo parametri Trimestre
		if(document.PreCPP.<%= ICostantiStatis.CAMPO_ANNO_TRIMESTRE%>.value.length>0)
		{
			// Selezionare almeno 1 trimestre
			if(!document.PreCPP.Trimestre[0].checked && 
				!document.PreCPP.Trimestre[1].checked &&
				!document.PreCPP.Trimestre[2].checked &&
				!document.PreCPP.Trimestre[3].checked)
			{
				alert('Selezionare il Trimestre \n (oppure i Trimestri purchè CONSECUTIVI)');
	        	document.PreCPP.<%= ICostantiStatis.CAMPO_ANNO_TRIMESTRE%>.focus();
	        	return false;				
			}
			...
			if(document.PreCPP.Trimestre[1].checked && 
			   document.PreCPP.Trimestre[3].checked && 
			  !document.PreCPP.Trimestre[2].checked  )
			{
				alert('Selezionare SOLO Trimestri CONSECUTIVI)');
...
// Scelta della classe Procedimento da elaborare
			if(!document.PreCPP.tipo[0].checked && 
				!document.PreCPP.tipo[1].checked &&
				!document.PreCPP.tipo[2].checked &&
				!document.PreCPP.tipo[3].checked &&
				!document.PreCPP.tipo[4].checked && 
				!document.PreCPP.tipo[5].checked &&
				!document.PreCPP.tipo[6].checked &&
				!document.PreCPP.tipo[7].checked )
			{
				alert('Inserire il check su Classe Procedimenti');
	        	document.PreCPP.tipo[0].blur();
	        	return false;						
			}	
    ...
    function enableBtn()
    {
    	document.PreCPP.btnconf.disabled = false;
    }
    
    function SelezionaClasse()
    {
    	if(document.PreCPP.tipo[0].checked)			// Classe I - Pena Detentiva
    	{
    		document.PreCPP.<%=IWebConstants.ACTION_FIELD%>.value="siap.sico.web.ActionUnderConstruction";
    	}
    	else if(document.PreCPP.tipo[1].checked)		// Classe II - Pena Pecuniaria
    	{
    		document.PreCPP.<%=IWebConstants.ACTION_FIELD%>.value="siap.sico.web.ActionUnderConstruction";
    	}
    	else if(document.PreCPP.tipo[2].checked)		// Classe III  - Pena Sospesa
    	{
    		document.PreCPP.<%=IWebConstants.ACTION_FIELD%>.value="siap.sico.web.ActionUnderConstruction";
    	}
    	else if(document.PreCPP.tipo[3].checked)		// Classe IV - Misure Sicurezza
    	{
    		document.PreCPP.<%=IWebConstants.ACTION_FIELD%>.value="siap.sico.web.ActionUnderConstruction";
    	}
    	else if(document.PreCPP.tipo[4].checked)		// Classe V - Persona Giuridica
    	{
    		document.PreCPP.<%=IWebConstants.ACTION_FIELD%>.value="siap.sico.web.ActionUnderConstruction";
    	}
    	else if(document.PreCPP.tipo[5].checked)		// Classe VI - Giudice di pace
    	{
    		document.PreCPP.<%=IWebConstants.ACTION_FIELD%>.value="siap.sico.web.ActionUnderConstruction";
    	}
    	else if(document.PreCPP.tipo[6].checked)		// Classe VII - Conversione Pene Pecuniarie
    	{
    		//document.PreCPP.< %=IWebConstants.ACTION_FIELD%>.value="siap.siep.statis.action.ActCreaStatisticaRiepilogoPreCPP";
    		document.PreCPP.<%=IWebConstants.ACTION_FIELD%>.value="siap.siep.statis.action.ActLoadCreaRiepilogo_CPP";
    	}
    	else if(document.PreCPP.tipo[7].checked)		// Registro Istanze
    	{
    		document.PreCPP.<%=IWebConstants.ACTION_FIELD%>.value="siap.sico.web.ActionUnderConstruction";
    	}
  ...
  <table>
  	<tr>
        ...
        <td class="LBG">
          ...
          <font class="campo">STATISTICHE UFFICIO - RIEPILOGO PROCEDIMENTI PENDENTI</font>
        </td>
     </tr>
   </table>
<br>
<FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name="PreCPP">
...
<script language="JavaScript" type="text/javascript">
  	var frmvalidator  = new Validator("PreCPP");
