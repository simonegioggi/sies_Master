...
<%
//solo nel caso che venga richiamata dalla sanzione sostitutiva
String lFlagSanzione = "N";
if(request.getAttribute("lFlagSanzione") != null && "S".equals(request.getAttribute("lFlagSanzione")))
{
    lFlagSanzione = "S";
}
...
%>
        <tr>
          <td width="32%" class="menulines" nowrap>
            <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFunModel.getNameAction()%>&lFlagSanzione=<%=lFlagSanzione%>"><%=lFunModel.getLabelFunction()%></a>
          </td>
          <td width="32%" class="menulines" nowrap>
            <%if (itx.hasNext()) { lFunModel = (FunctionModel)itx.next();%><a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFunModel.getNameAction()%>&lFlagSanzione=<%=lFlagSanzione%>"><%=lFunModel.getLabelFunction()%></a><% } %>
          </td>
          <td width="32%" class="menulines" nowrap>
            <%if (itx.hasNext()) { lFunModel = (FunctionModel)itx.next();%><a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFunModel.getNameAction()%>&lFlagSanzione=<%=lFlagSanzione%>"><%=lFunModel.getLabelFunction()%></a><% } %>
          </td>
