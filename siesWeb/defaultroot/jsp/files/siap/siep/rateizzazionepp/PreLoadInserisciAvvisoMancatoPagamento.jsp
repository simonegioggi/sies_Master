...
function eseguiAzione(tipoAzione, id) {
	...
	} else if (tipoAzione == 'Inserisci') {
		azione = "siap.siep.rateizzazionepp.action.ActLoadInserisciAvvisoMancatoPagamento";
    	document.PreLoadInserisciAvvisoMancatoPagamento.<%="test"%>.value = id;
    	tornaIndietro(azione);
	}
}

function tornaIndietro(action) {
	document.PreLoadInserisciAvvisoMancatoPagamento.<%=IWebConstants.ACTION_FIELD%>.value = action;
	document.PreLoadInserisciAvvisoMancatoPagamento.submit();
}
</script>
</head>
<body class="corpo">
...
<br>
<FORM action="<%=IWebConstants.PG_MAIN%>" method="post" name="PreLoadInserisciAvvisoMancatoPagamento">
...
<%
	...
	while (itx.hasNext()) {
		...
		if ("U".equals(bpm.getTipoRateizzazione()) || "R".equals(bpm.getTipoRateizzazione())) {
%>
	<tr>
