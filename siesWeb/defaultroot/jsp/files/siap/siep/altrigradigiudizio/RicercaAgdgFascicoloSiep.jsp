...
<%@ page import="siap.siep.agdgfascicolosiep.model.AgdgFascicoloSiepModel" %>

<jsp:useBean id="UtenteConnesso" scope="session" class="siap.sico.utente.model.UtenteModel" />

<jsp:useBean id="agdgfascicolosiep" scope="request" class="java.util.Vector" />
...
<%
  ...
  SentenzaModel sentenza = fascicolo.getSentenza();
	String lNota = "N.B.: Per Associare/Deassociare un 'Altro grado di Giudizio' al Procedimento, selezionare/deselezionare la relativa casella e Confermare.";
%>
  ...
  <FORM method="POST" name="elenco" action="<%=IWebConstants.PG_MAIN%>">
    ...
    <table>
      <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
        <td class="LBG"><font class="label">Funzione :</font>&nbsp;<font class="campo">Elenco Altri Gradi Giudizio</font></td>
      </tr>
    </table>
...
%>
    ...
    <tr>
      ...
      <td class="int">Anno/ <br>Numero</td>
      ...
      <td class="int">Anno/<br>Num.<br>Reg.Gen.</td>
      <td class="int">Anno/<br>Num.<br>Sentenza</td>
      <td class="int">Anno/<br>Num.<br>Racc.Gen.</td>
      <td class="int">Dispositivo</td>
      <td class="int">Azioni</td>
    </tr>

<%
    Iterator itx = agdgfascicolosiep.iterator();
	String flagassociato="";
	int numagdgfasc=agdgfascicolosiep.size();
	int conto=0;
    while ( itx.hasNext())
    {
    	AgdgFascicoloSiepModel lSen = (AgdgFascicoloSiepModel)itx.next();
      flagassociato="";
...
%>
    <tr>
	  ...
	  <td class=C><%=StringUtils.toStringJSP(DateUtils.getDateToString(lSen.getAltriGradiGiudizioModel().getDataSentenzaIGrado(),"dd-MM-yyyy"))%></td>
      <td class=C><%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getAnnoSentenzaIGrado())%>/<%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getNumeroSentenzaIGrado())%></td>
      <td class=C><%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getDescrAutEmittSentIGrado())%></td>
      <td class=C><%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getDescrLuoEmittSentIGrado())%></td>
      
	  <td class=C><%=StringUtils.toStringJSP(DateUtils.getDateToString(lSen.getAltriGradiGiudizioModel().getDataSentenzaIiGrado(),"dd-MM-yyyy"))%></td>
      <td class=C><%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getAnnoSentenzaIiGrado())%>/<%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getNumeroSentenzaIiGrado())%></td>
      <td class=C><%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getDescrAutEmittSentIiGrado())%></td>
      <td class=C><%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getDescrLuoEmittSentIiGrado())%></td>
            
      <td class=C><%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getAnnoRegGenCassaz())%>/<%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getNumeroRegGenCassaz())%></td>
      <td class=C><%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getAnnoSentenzaCassaz())%>/<%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getNumeroSentenzaCassaz())%></td>
	  <td class=C><%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getAnnoRaccGenealeIiGrado())%>/<%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getNumeroRaccGenealeIiGrado())%></td>
      <td class=C><%=StringUtils.toStringJSP(lSen.getAltriGradiGiudizioModel().getDescrTipoDecisioneCassazione())%></td>
      <td class=C>
        <jsp:include page="<%=ICostantiSentenza.PG_BUTTONS_SENTENZA%>">
           ...
           <jsp:param name="ValoreIdEntita" value="<%=lSen.getAltriGradiGiudizioModel().getIdAltrigradigiudizio()%>"/>
        </jsp:include>
      </td>
  	  <td class="l"><input type="HIDDEN" name="IdAltriGradiGiudizio<%=conto%>" value="<%=lSen.getAltriGradiGiudizioModel().getIdAltrigradigiudizio()%>"></td>
  	  <td class="l"><input type="HIDDEN" name="IdAgdgFascicoloSiep<%=conto%>" value="<%=lSen.getIdAgdgFascicoloSiep()%>"></td>
...
%>
  ...
  <input type="HIDDEN" name="Action" value="siap.siep.altrigradigiudizio.action.ActAssociaFascicoloAltriGradiGiudizio" >
  ...
  <input type="HIDDEN" name="numagdgfasc" value="<%=numagdgfasc%>" >
