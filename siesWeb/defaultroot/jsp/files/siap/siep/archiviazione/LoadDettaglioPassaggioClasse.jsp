...
<jsp:useBean id="fascicoloSiepModel"	scope="request" class="siap.siep.fascicolo.model.FascicoloSiepModel"/>
...
String[] caep = ICostantiEvento.CODICI_AVVENUTA_ESECUZIONE_PENA;
List<String> codMotivi = Arrays.asList(caep);
String codMotivo = eventonotifica.getEvento().getCodMotivo();
boolean isPresentCodMotivo = false;
if (codMotivo != null && codMotivo.length() > 0)
	isPresentCodMotivo = codMotivi.contains(codMotivo);
%>
<html>
<head>
...
<table>
	<tr>
		...
		<td class="LBG"><font class="label">Funzione :</font>&nbsp;&nbsp;
			<font class="campo">Dettaglio Definizione Procedimento - Passaggio di Classe</font>
		</td>
<%
...
%>
		<!-- BOTTONE DI VALIDAZIONE -->
     	<td class="LBG">
      		<a  href="/jsp/Main.jsp?Action=siap.siep.archiviazione.action.ActUploadPassaggioClasse&IdEvento=<%=eventonotifica.getEvento().getIdEvento()%>&<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>=siap.siep.archiviazione.action.ActLoadDettaglioPassaggioClasse&IdEvento=<%=eventonotifica.getEvento().getIdEvento()%>&<%=ICostantiEvento.CAMPO_VALIDA%>=S&noblob=S">
		...
		<td class="LBG">
   			<jsp:include page="<%= ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIEP%>">
       			<jsp:param name="ActionLink" value="<%=  "/jsp/Main.jsp?Action=siap.siep.archiviazione.action.ActStampaPassaggioClasse&IdEvento="+eventonotifica.getEvento().getIdEvento()%>"/>
   			</jsp:include>
   		</td>
<%
	}
...
%>
	<tr>
	  	...
	  	<td class="L" colspan="5">
	  		<font class="campo"><%=acm.getIstitutoDetenzione().getDescrTipoIstituto()%></font>
...
%>
		...
		<td class="<%=lClassTd%>">
			<font class="<%=lClassFont%>"><%=StringUtils.toStringJSP(DateUtils.getDateToString(penaresidua.getDataFine(),"dd-MM-yyyy"))%></font>
		</td>
<%
}
%>
	<tr>
<%
if (penaresidua.getIdPenaResidua() != null && ((penaresidua.getFlagErgastolo() == null) || (penaresidua.getFlagErgastolo() != null
...
%>
	<tr>
	  	...
	  	<td class="L"><font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(archiviazione.getDataDefinizione(), "dd-MM-yyyy"))%></font></td>
	</tr>
<%
}
%>
...
%>
	...
	<tr>
		<td class="l">Note
		<td class="L"><font class="campo"><%=StringUtils.toStringJSP(archiviazione.getNote())%></font></td>
	</tr>
<%
}
...
%>
	<tr>
		...
		<td class="l">
			<font class="campo"><%=StringUtils.toStringJSP(ufficio.getUfficio().getDescrTipoUfficio())%></font>&nbsp;
			<font class="campo"><%=StringUtils.toStringJSP(ufficio.getUfficio().getDescrComune())%></font>
		</td>
	</tr>
<%
}
...
%>
	<tr>
		...
		<td class="l">
			<font class="campo"><%=StringUtils.toStringJSP(altraautorita.getAutoritaEsterna().getDescrTipoAutorita())%></font>&nbsp;
			<font class="campo"><%=StringUtils.toStringJSP(altraautorita.getAutoritaEsterna().getDescrSede())%></font>
		</td>
	</tr>
	<tr>
		<td class="l">Note</td>
		<td class="L"><font class="campo"><%=StringUtils.toStringJSP(altraautorita.getNote())%></font></td>
	</tr>
<%
}
%>
	...
	<tr>
		<td class="L">
        	<font class="label">Procedimento : N. </font>
          	<a class="cliccabile" href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.fascicolo.action.ActLoadDettaglioFascicolo&<%=ICostantiFascicoloSiep.CAMPO_ID_FASCICOLO_SIEP%>=<%=fascicoloSiepModel.getIdFascicoloSiep()%>" title="Procedimento">
            	<%=fascicoloSiepModel.getChiaveAnno()%>/<%=fascicoloSiepModel.getChiaveProgr()%>
          	</a>
		</td>
	</tr>
</table>
...
<table>
	...
	<tr>
		<td class="L">
			...
			<input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.archiviazione.action.ActUploadPassaggioClasse">
			<input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_ID_EVENTO%>" value="<%= eventonotifica.getEvento().getIdEvento()%>">
			<input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>" value="siap.siep.archiviazione.action.ActLoadDettaglioPassaggioClasse">
		</td>
	</tr>
</table>
</form>
