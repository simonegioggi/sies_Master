...
UtenteModel um = new UtenteModel((UtenteModel) session.getAttribute(ICostantiSecurity.SESSION_UTENTE_CONNESSO));
UfficioModel ufm = um.getUfficioUtente();
String casellario = ufm.getDescrComune();
if (sm != null && (sm.getCodStatoNascita() == null
		||  "".equals(sm.getCodStatoNascita()) 
		|| "-".equals(sm.getCodStatoNascita()))) {
	casellario = "-";
}
%>
...
<title>[S.I.E.S.] - Definizione Procedimento - Passaggio di Classe</title>
...
function Verify() {
	...
	if (document.f.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value.length == 1)
		document.f.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value = '0' +
		document.f.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value;
	var data_to_verify = document.f.<%=ICostantiArchiviazione.CAMPO_GIORNO_DATA_DEFINIZIONE%>.value + '/' +
		document.f.<%=ICostantiArchiviazione.CAMPO_MESE_DATA_DEFINIZIONE%>.value + '/' +
		document.f.<%=ICostantiArchiviazione.CAMPO_ANNO_DATA_DEFINIZIONE%>.value;
...
<table>
	<tr>
        ...
        <td class=lbg>
			...
			<font class="campo">Definizione Procedimento - Passaggio di Classe</font>
        </td>
	</tr>
</table>
<br>
...
<input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.archiviazione.action.ActLoadSceltaPassaggioClasse">
<table>
	<tr>
	  	<td class="l">Posizione Giuridica </td>
	  	<td class="L" colspan="8">
	    	<font class="campo"><%=StringUtils.toStringJSP(pgm.getDescrPosizioneGiuridica())%></font>
	  </td>
	</tr>
<%
...
if (pgm.getCodPosizioneGiuridica() != null
		&& (!pgm.getCodPosizioneGiuridica().equals("07")
      	&& !pgm.getCodPosizioneGiuridica().equals("10"))) {
%>
	<tr>
<%
	if (flagergastolo.equals("N") && penaresidua.getDataInizio() != null) {
%>
...
<%
    ...
    if (flagergastolo.equals("N") && penaresidua.getDataFine() != null) {
		String lClassTd = "l";
...
%>
		...
		<td class="<%=lClassTd%>"><font class="<%=lClassFont%>"><%=StringUtils.toStringJSP(DateUtils.getDateToString(penaresidua.getDataFine(), "dd-MM-yyyy"))%></font></td>
<%
	}
}
%>
...
<table width="100%">
    ...
    <tr>
      	<td class="l" width="30%">Data Definizione <font class="ob">(*)</font></td>
      	<td class="l">
    ...
    <tr>
      	...
      	<td class="l">
        	<select Title="Oggetto Definzione" name="<%=ICostantiArchiviazione.CAMPO_COD_OGGETTO_DEFINIZIONE%>" onChange="gestisciDest()"><%=oggettodefinzione%></select>
      	</td>
    </tr>
    <tr>
      	<td class="l">Note</td>
      	<td class="l"><textarea cols="60" rows="2" name="<%=ICostantiNotifica.CAMPO_NOTE_E%>"></textarea></td>
    </tr>
    <tr>
    ...
    <tr>
      	...
      	<td class="L">
			...
			<a href="Javascript:ListaMagistrati('f');"><img src="/images/filefolder.gif" border="0"></a>
		</td>
       	<td>
         	<input type="HIDDEN" value="<%=StringUtils.toStringJSP(magistratocompetente.getMagistrato().getCodMagistrato())%>" name="<%=ICostantiMagistrato.CAMPO_COD_MAGISTRATO%>">
       </td>
    </tr>
    <tr>
    ...
    <tr>
     	...
     	<td class="l" id="inputCasellario">
       		<input title="Sede Casellario Giudiziale" value="<%=StringUtils.toStringJSP(casellario)%>" type="text" name="<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_CAS%>" maxlength="35" size="35">
       		<a href="Javascript:ListaUfficiPerTipo('f','<%=ICostantiAutoritaEsterna.CAMPO_COD_SEDE_CAS%>','DIB');"><img src="/images/filefolder.gif" border="0"></a>
     	</td>
	</tr>
</table>
...
%>
	        <input readonly Title="Istituto" name="Comune" value="<%=StringUtils.toStringJSP(ldm.getIstitutoDetenzione().getDescrTipoIstituto())%> di <%=StringUtils.toStringJSP(ldm.getIstitutoDetenzione().getDescrComune())%>" size="50">
	        <input type="hidden" name="<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>" value="<%=ldm.getIstDetIdIstitutoDetenzione()%>">
	        <a href="Javascript:ListaIstitutoDetenzione('f','<%=ICostantiLuogoDetenzione.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>', 'Comune');">
	          	<img src="/images/filefolder.gif" border="0">
	        </a>
<%
...
%>
	...
	<tr>
      	...
      	<td class="L"><select Title="Ufficio Recupero Crediti" name="UfficioRecuperCrediti"><%=uffrecrediti%></select></td>
     	<td class="l">di</td>
     	<td class="L">
    ...
    <tr>
      	...
      	<td class="L" colspan="3"><select Title="Altra Autorità" name="<%=ICostantiAutoritaEsterna.CAMPO_COD_TIPO_AUTORITA%>"><%=codiceAutorita%></select></td>
	</tr>
    <tr>
      	<td class="l">Sede</td>
      	<td class="L">
      	...
      	<td class="L"><TEXTAREA title="Indirizzo" name="<%=ICostantiNotifica.CAMPO_NOTE%>" cols="30"></textarea></td>
	</tr>    
</table>
</div>
<table>
