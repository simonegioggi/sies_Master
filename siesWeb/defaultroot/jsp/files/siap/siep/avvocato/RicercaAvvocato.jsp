...
function avvocati() {
	if ("<%=avvocato.size()%>" == 0) {
		// MEV_21 in caso di chiusura infruttuosa della ricerca Avv. su SIES, si chiude la popup di ricerca
		// e si abilita il pulsante di inserimento manuale dell'avvocato
...
%>
		<td class=c>
			<a href="Javascript:insertIT('<%=lAvv.getIdAvvocato()%>','<%=StringUtils.cStrForJS(lAvv.getCognome())%>',
					...
					'<%=StringUtils.cStrForJS(lAvv.getDescrComuneStudio() )%>','<%=stato%>');">
