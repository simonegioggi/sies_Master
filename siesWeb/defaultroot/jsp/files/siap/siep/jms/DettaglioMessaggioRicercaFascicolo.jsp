...
<%
	...
	{
	  if (Messaggio.getMessaggioCorrelato().getCodEsito().compareTo("00000") == 0)
	  {%><td class="cVerde">
	<%}else
	  {%><td class="lRosso"><%}%>
          <%=Messaggio.getMessaggioCorrelato().getDescrEsito()%>&nbsp;</td>
	<%}%>
      </tr>
      <tr>
...
<%
	  if (Messaggio.getMessaggioCorrelato() != null &&
	      Messaggio.getMessaggioCorrelato().getCodEsito().compareTo("00000") == 0)
	  {%>
	    <font class="campo"><%=Messaggio.getMessaggioCorrelato().getDescrEsito()%>&nbsp;</font>
	    </td> </tr></table>
	<%}

	  if(Messaggio.getCodTipoOperazione().equals("00050"))
	  {%>
	    <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="ConfermaTrasferimentoFascicolo">
	      <table>
		<tr>
		  <td>
