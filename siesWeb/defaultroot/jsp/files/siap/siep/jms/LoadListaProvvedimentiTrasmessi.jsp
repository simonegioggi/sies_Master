...
<html>
<head>
  <title>[S.I.E.S.] - Riscontro Trasmissioni - Ricerca Provvedimenti Trasmessi</title>
  ...
  <script language="JavaScript">
    ...
    function Verify()
    {
      var data_inizio=document.LoadListaProvvedimentiTrasmessi.<%=ICostantiSicoJMS.CAMPO_GIORNO_DATA_TRASMISSIONE_INIZIO%>.value+'/'+document.LoadListaProvvedimentiTrasmessi.<%=ICostantiSicoJMS.CAMPO_MESE_DATA_TRASMISSIONE_INIZIO%>.value+'/'+document.LoadListaProvvedimentiTrasmessi.<%=ICostantiSicoJMS.CAMPO_ANNO_DATA_TRASMISSIONE_INIZIO%>.value;
      var data_fine=document.LoadListaProvvedimentiTrasmessi.<%=ICostantiSicoJMS.CAMPO_GIORNO_DATA_TRASMISSIONE_FINE%>.value+'/'+document.LoadListaProvvedimentiTrasmessi.<%=ICostantiSicoJMS.CAMPO_MESE_DATA_TRASMISSIONE_FINE%>.value+'/'+document.LoadListaProvvedimentiTrasmessi.<%=ICostantiSicoJMS.CAMPO_ANNO_DATA_TRASMISSIONE_FINE%>.value;
      ...
      if( (document.LoadListaProvvedimentiTrasmessi.<%=ICostantiSiepJMS.CAMPO_TIPO_UFFICIO%>.value.length > 1)
           && (document.LoadListaProvvedimentiTrasmessi.<%=ICostantiSiepJMS.CAMPO_SEDE_UFFICIO%>.value.length == 0) )
      {
      ...
      if( (document.LoadListaProvvedimentiTrasmessi.<%=ICostantiSiepJMS.CHIAVE_PROGR_SIEP%>.value.length != 0)
           && (document.LoadListaProvvedimentiTrasmessi.<%=ICostantiSiepJMS.CHIAVE_ANNO_SIEP%>.value.length == 0) )
      {
  ...
  <body class="corpo">
    <table>
      <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
        <td class="LBG"><font class="label">Funzione :</font>&nbsp;<font class="campo">Ricerca Provvedimenti Trasmessi: </font>
        </td>
      </tr>
    </table>
    <FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name='LoadListaProvvedimentiTrasmessi'>
      <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.jms.action.ActListaProvvedimentiTrasmessi">
      ...
      <tr>
        ...
        <td class="l">
          <input Title="Sede Ufficio" name="<%=ICostantiSiepJMS.CAMPO_SEDE_UFFICIO%>" type="text" maxlength="35" size="35">
            <a href="Javascript:ListaUffici('LoadListaProvvedimentiTrasmessi','<%= ICostantiSiepJMS.CAMPO_SEDE_UFFICIO %>');">
  ...
  <input type="HIDDEN" name="<%=IWebConstants.LINK_RITORNO%>" value="<%=TornaQui%>" >
    ...
    <script language="JavaScript" type="text/javascript">
      ...
      var frmvalidator  = new Validator("LoadListaProvvedimentiTrasmessi");
