...
<html>
<head>
<title>[S.I.E.S.] - Dettaglio Sospensione Pena</title>
...
<FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciSosp">
  ...
  <table>
    <tr>
      ...
      <font class="campo">Dettaglio Sospensione Pena</font>
    </td>
<%if (eventonotifica.getEvento().getFlagDocumentoRegistrato()!=null)
 if (eventonotifica.getEvento().getFlagDocumentoRegistrato().compareTo("N")==0) {%>
 <%-- MEV NUOVA INFRASTRUTTURA: refactoring --%>
     <%--td class="LBG">
          <a href="/jsp/Main.jsp?Action=siap.siep.sospensione.action.ActStampaSospensionePena&IdEvento=<%= eventonotifica.getEvento().getIdEvento() %>" onclick="javascript:lookUpload();">
            <img  align="middle" src="/images/print24.gif" alt="Generazione Stampa" width="24" height="24" border="0">
          </a>
        </td--%>
<!-- BOTTONE DI STAMPA -->
   <jsp:include page="<%= ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIEP%>">
     <jsp:param name="ActionLink" value="<%="/jsp/Main.jsp?Action=siap.siep.sospensione.action.ActStampaSospensionePena&IdEvento="+eventonotifica.getEvento().getIdEvento()%>"/>
   </jsp:include>
   <td class="LBG">
...
<tr>
  ...
  <tr>
   ...
   <td class="L">
       <font class="campo"><%=StringUtils.toStringJSP(magistratocompetente.getCognome())%> &nbsp; <%=StringUtils.toStringJSP(magistratocompetente.getNome() )%>
      </font>
      </td>
      <td>
      </td>
...
<%if(autoritaE != null && autoritaE.getIdAutoritaEsterna()!=  null){%>

 <tr>
     <td class="l" width=30%>Autorita di Polizia </td>
  <td class="l"><font class="campo"><%=StringUtils.toStringJSP(autoritaE.getDescrTipoAutorita())%> di <%=StringUtils.toStringJSP(autoritaE.getDescrSede())%>
</font>
</td>
</tr>
<%}%>
<%if(autoritaC != null && autoritaC.getIdAutoritaEsterna()!=  null){%>

 <tr>
     <td class="l" width=30%>Ufficiali Giudiziari notifica al Condannato </td>
  <td class="l"><font class="campo"><%=StringUtils.toStringJSP(autoritaC.getDescrTipoAutorita())%> di <%=StringUtils.toStringJSP(autoritaC.getDescrSede())%>
</font>
</td>
</tr>
<%}%>
      ...
      <table>
        ...
        <tr>
          <td class="L">
            ...
            <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.siep.sospensione.action.ActUploadSospensionePena">
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_ID_EVENTO%>" value="<%= eventonotifica.getEvento().getIdEvento() %>">
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>" value="siap.siep.sospensione.action.ActLoadDettaglioSospensionePena">
          </td>
        </tr>
      </table>
</form>
