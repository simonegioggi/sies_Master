...
<%
int NumeroTotalePagine=new BigDecimal(CountRisultati.doubleValue()/2).intValue();
...
<%
  ...
  double lNumLayer = Math.ceil((CountRisultati.doubleValue()/(PagesPerLayer*2)));
//  double MaxPage= Math.floor(CountRisultati.doubleValue()/IWebConstants.RESULT_PER_PAGE)+1;
  int MaxPage = CountRisultati.intValue()/2; // troncamento
  int resto = CountRisultati.intValue()%2;
   ...
   // Luigi 26-2-2004
     tmp+="<td bgcolor=#000080 colspan=29 align=center><font color=#FFFFFF>Pagina <b><%=ThisPage%></b> di <%=MaxPage%> ,  per un totale di <b><%=CountRisultati.intValue()%></b> Risultati</font></td>"
    	...
    	{%>
        	tmp+="<tr bordercolor=#DEDEDE>";
            if(nextPage>1)
				tmp+="<td width=15 align=center><a class=pager href=javascript:VisibleIT('"+Lpage+"');><strong>&laquo;&laquo;</strong></a></td>";
			if (ThisPage > 1)
    		{
       			tmp+="<td width=15 align=center><a href='<%=RequestForPaging%>&<%=IWebConstants.NUM_PAGE%>=<%=ThisPage-1%>&CountRisultati=<%=CountRisultati%>' class=pager><strong>Precedente</strong></a></td>";
   			}
			for (var i=parseInt(Lbound);i<=parseInt(Ubound);i++)
			{
				if (i==ThisPage)
				{
					tmp+="<td width=15 align=center bgcolor=#000080><font color=#FFFFFF>"+i+"</font></td>";
				}else
				{
					tmp+="<td width=15 align=center><a href='<%=RequestForPaging%>&<%=IWebConstants.NUM_PAGE%>="+i+"&CountRisultati=<%=CountRisultati.intValue()%>' class=pager>"+i+"</a></td>";
				}
			}
			if (ThisPage< MaxPage)
			{
				tmp+="<td width=15  align=center><a href='<%=RequestForPaging%>&<%=IWebConstants.NUM_PAGE%>=<%=ThisPage+1%>&CountRisultati=<%=CountRisultati%>' class=pager><strong>Successiva</strong></a></td>";
			}
            if(NumOfLayer!=nextPage)
		    	tmp+="<td width=15 align=center><a class=pager href=javascript:VisibleIT('"+Upage+"');><strong>&raquo;&raquo;</strong></a></td>";
		tmp+="</tr>"
		<% } %>
		tmp+="</table>";
		node.innerHTML=tmp;
