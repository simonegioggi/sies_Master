...
<!-- 
		Maschera di Dettaglio deputata alla visualizzazione dei dati afferenti 
		alla Relazione della Richiesta UEPE Ricevuta.
-->

<html>
  <head>
    <title>[S.I.E.S.] - Dettaglio Relazione di Richiesta UEPE Ricevuta</title>
      ...
      <table>
        <tr>
          ...
          <td class="LBG">
            ...
            <font class="campo">Dettaglio Relazione di Richiesta UEPE Ricevuta</font>
          </td>

      		<!-- TOOLBAR HEADER -->
      		<td class="LBG">
          	<jsp:include page="<%=ICostantiRicezioneAtti.PG_TOOLBAR_HEADER_RELAZIONE_RICHIESTA_RICEVUTA%>">
...
<%
  				...
  				if (Messaggio.getFlagVisto().compareTo("N") == 0)
  					lStato+="Ricevuto";
  				else if (Messaggio.getFlagVisto().compareTo("V") == 0) 
  		 			lStato+="Preso in Visione";
  				else if (Messaggio.getFlagVisto().compareTo("S") == 0) 
  		  		lStato+="Preso in Carico";
...
%>
    ...
    <jsp:include page="<%=ICostantiRicezioneAtti.PG_SINTESI_PROCEDIMENTO_SIUS_RICEVUTO %>"/>
    ...
    <table cellspacing=2 cellpadding=2>
      ...
      <!--  Sezione Dati della Richiesta SIEPE -->
    	
    	<tr>
    		<td class="Titolo" colspan=4>Richiesta</td>
    	</tr>
 			
 			<tr>
   			<td class="l">
   				<font class="l">Data Richiesta</font>
   			</td>
				<td class="l">	
					<font class="campo"><%=DateUtils.getDateToString(richiesta.getDataRichiesta(),"dd-MM-yyyy")%></font>
				</td>
    	</tr>
			<tr>
      		<td class="l"><font class="label">Tipo Richiesta</font></td>
        	<td class="l"><font class="campo"><%=richiesta.getDescrTipoRichiesta()%></font></td>
			</tr>
			<tr>
					<td class="l"><font class="label">Presentata da</font></td>
					<td class="l">
						<font class="campo"><%=StringUtils.toStringJSP( richiesta.getDescrTipoRichiedente())%></font>
    			</td>
			</tr>
			
			<!-- Fine Sezione Dati della Richiesta SIEPE/UEPE -->
    	...
    	<tr><td class="Titolo" colspan=4>Relazione </td></tr>
 			...
 			<tr>
   			<td class="L"><font class="l">Data Emissione </font></td>
				<td class="l">
					<font class="campo"><%=DateUtils.getDateToString(relazione.getDataEmissione(),"dd-MM-yyyy")%></font>
				</td>
    	</tr>
			
			<tr>
				<td class="L"><font class="l">Nota relazione </font></td>
      	<td class="l">
        	<font class="campo"><%=StringUtils.toStringJSP(relazione.getNote(),"-") %></font>
        </td>
			</tr>
		
			<!-- Fine Sezione dati della Relazione -->
      
      <tr>
      	<td>&nbsp;</td>
      </tr>
   		
   		<!--  Sezione dati del Messaggio -->
...
%>
      ...
      <!--  Fine Sezione dati del Messaggio -->
      
    </table>
    <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="DettgalioRelazioneRichiestaRicevuta">
    	<!-- <tr> -->
