...
%>
          <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%>&CodTipoOperazione=<%=ICostantiJMS.ESITO_TRASFERIMENTO_RELAZIONE_ATTIVITA%>&TornaQui=<%=TornaQui%>">
            <img align="middle" src="<%=IWebConstants.IMAGES_DIR%>new24.gif" alt="Presa in Carico Relazione della Attività UEPE Iscrizione Procedimento SIEPE" width="24" height="24" border="0">
          </a>
<%				}
        }
...
%>
          <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%>&CodTipoOperazione=<%=ICostantiJMS.ESITO_TRASFERIMENTO_RELAZIONE_ATTIVITA%>&TornaQui=<%=TornaQui%>">
            <img align="middle" src="<%=IWebConstants.IMAGES_DIR%>PresaVisione.gif" alt="Presa Visione Relazione dell'Attività Ricevuta" width="24" height="24" border="0">
          </a>
<%				}
        }
        ...
        {
					String nameAction = lFun.getNameAction().concat("&CodTipoOperazione="+ICostantiJMS.ESITO_TRASFERIMENTO_RELAZIONE_ATTIVITA);
          if (flagVisto.compareTo("N") == 0) {
%>
          <a href="Javascript:confermaRestituzione('<%=nameAction%>','<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>','<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%>','<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA_PROVV)%>','<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA_PROVV)%>', 'Confermi la restituzione ?');">
            <img align="middle" src="<%=IWebConstants.IMAGES_DIR%>Restituzione.gif" alt="Restituzione Relazione dell'Attività UEPE al Mittente" width="24" height="24" border="0">
          </a>
<%				}
        }
...
%>
          <a href="Javascript:stampa2( '<%=ISIAPCostantiWeb.PG_STAMPA%>', '<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%>&TornaQui=<%=TornaQui%>')">
            <img align="middle" src="<%=IWebConstants.IMAGES_DIR%>print24.gif" alt="Stampa Relazione dell'Attività UEPE" width="24" height="24" border="0">
          </a>
<%
        }
