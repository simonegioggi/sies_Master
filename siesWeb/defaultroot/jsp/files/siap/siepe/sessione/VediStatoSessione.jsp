...
<%
  FascicoloSiepeEstesoModel lFascicoloEsteso=(FascicoloSiepeEstesoModel)session.getAttribute("FascicoloSiepeEsteso");
  FascicoloSiepeModel fasSiepe=new FascicoloSiepeModel();
  ...
  FascicoloGPModel fasSiusGP = new FascicoloGPModel();
	if (lFascicoloEsteso!= null)
  {
  	fasSiepe=lFascicoloEsteso.getFascicoloSiepe();
  	fascicolo=lFascicoloEsteso.getFascicoloSiep();
  	sentenza=lFascicoloEsteso.getFascicoloSiep().getSentenza();
  	soggetto=lFascicoloEsteso.getSoggetto();
  	fasSiusGP =lFascicoloEsteso.getFascicoloSius();
  }
...
<%-- MEV NUOVA INFRASTRUTTURA: refactoring --%>
    ...
    window.close();
        ...
        function DettaglioSIUS(id)
        {
    window.opener.top.frames['centrale'].frames['body'].location.href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.sius.fascicolo.action.ActLoadDettaglioFascicolo&<%=ICostantiFascicoloSius.CAMPO_ID_FASCICOLO_SIUS%>="+id;
    window.close();
        }

        function DettaglioSIEPE(id)
        {
    window.opener.top.frames['centrale'].frames['body'].location.href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siepe.fascicolo.action.ActLoadDettaglioFascicoloSiepe&<%=ICostantiFascicoloSiepe.CAMPO_ID_FASCICOLO_SIEPE%>="+id;
    window.close();
        }
  ...
  function CleanIT()
    ...
    document.location.href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.sico.sessionstate.action.ActCleanWorkSessionSIEPE";
        }
</script>
</head>
...
<!-- procedimento siepe -->
  <table cellspacing=0 cellpadding=0 width=95%>
    <tr><td class=LBG><font class="label">Ultimo Procedimento SIEPE Trattato</font></td></tr>
  </table>
<%
  if (lFascicoloEsteso !=null && fasSiepe !=null)
  {
%>
          <table cellspacing=0 cellpadding=0 width=95%>
            <tr>
            ...
            <a class="cliccabile" href="javascript:DettaglioSIEPE('<%=fasSiepe.getIdFascicoloSiepe()%>');" title="Procedimento SIEPE">
              <%=fasSiepe.getChiaveAnno()%>
              /
              <%=fasSiepe.getChiaveProgr()%>
            </a>
              </td>
            </tr>
            <tr>
...
%>
                <table cellspacing=0 cellpadding=0 width=95%>
                <tr><td><font class="label">Nessun procedimento SIEPE attualmente disponibile</font></td></tr>
                </table>
<%
        }
%>
...
<!-- procedimento sius -->
  <table cellspacing=0 cellpadding=0 width=95%>
    <tr><td class=LBG><font class="label">Ultimo Procedimento SIUS Trattato</font></td></tr>
  </table>
<%
  if (lFascicoloEsteso!=null && fasSiusGP !=null)
  {
%>
          <table cellspacing=0 cellpadding=0 width=95%>
            <tr>
            ...
            <a class="cliccabile" href="javascript:DettaglioSIUS('<%=fasSiusGP.getFascicoloSiusModel().getIdFascicoloSius()%>');" title="Procedimento">
              <%=fasSiusGP.getFascicoloSiusModel().getChiaveAnno()%>
              /
              <%=fasSiusGP.getFascicoloSiusModel().getChiaveProgr()%>
            </a>
              </td>
            </tr>
            <tr>
              <td class="L" width=100%><font class="label">Soggetto :</font>
              <font class="campo">
          <%=fasSiusGP.getFascicoloSiusModel().getSoggetto().getCognome()%>&nbsp;<%=fasSiusGP.getFascicoloSiusModel().getSoggetto().getNome()%>
              </font>&nbsp;
<%
                if (fasSiusGP.getFascicoloSiusModel().getSoggetto().getSesso().compareTo("F")==0)
                {
%>
                  <font class="label">nata il :</font>&nbsp;
<%
...
%>
              <font class="campo"><%=DateUtils.getDateToString(fasSiusGP.getFascicoloSiusModel().getSoggetto().getDataNascita(),"dd-MM-yyyy")%></font>&nbsp;
              <font class="label">in : </font>
              <font class="campo">
<%
              if (fasSiusGP.getFascicoloSiusModel().getSoggetto().getDescrComuneNascita().compareTo("-")==0)
              {
%>
                <%=fasSiusGP.getFascicoloSiusModel().getSoggetto().getDescrStatoNascita()%>
<%
              }
              else
              {
%>
                <%=fasSiusGP.getFascicoloSiusModel().getSoggetto().getDescrComuneNascita()+ "  ("+fasSiusGP.getFascicoloSiusModel().getSoggetto().getCodProvinciaNascita()+")" %>
<%
              }
%>
              </font>
...
%>
                <table cellspacing=0 cellpadding=0 width=95%>
                <tr><td><font class="label">Nessun procedimento SIUS attualmente disponibile</font></td></tr>
                </table>
<%
        }
%>
  <table><tr><td><br>&nbsp;</td></tr></table>
<!-- fine procedimento sius -->
...
<%
  if (lFascicoloEsteso!=null && fascicolo !=null)
  {
...
<%
 if (lFascicoloEsteso!=null && sentenza !=null)
 {
%>
  <table cellspacing=0 cellpadding=0 width=95%>
    <tr>
...
<%
  if (lFascicoloEsteso!=null && soggetto !=null)
  {
%>
    <table cellspacing=0 cellpadding=0 width=95%>
    <tr>
