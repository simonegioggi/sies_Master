...
%>
              <td>
                <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%>&hufUtConnesso=<%=ufUtConnesso%>&hcodDistretto=<%=codDistretto%>&htipoUfficio=<%=tipoUfficio%><%=retParam%>">
                  <img src="/images/dettagli.gif" width="12" height="12" alt="Elenco Fascicoli del Soggetto" border="0">
                </a>
              </td>
<%
            }
...
%>
              <td>
                <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%>&hufUtConnesso=<%=ufUtConnesso%>&hcodDistretto=<%=codDistretto%>&htipoUfficio=<%=tipoUfficio%><%=retParam%>">
                  <img src="/images/new24.gif" width="12" height="12" alt="Iscrizione Procedimento" border="0">
                </a>
              </td>
<%
            }
