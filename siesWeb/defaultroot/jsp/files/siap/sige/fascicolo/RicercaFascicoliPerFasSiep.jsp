...
<%}%>
  ...
  <table cellspacing=2 cellpadding=2>
      ...
      <td>
        ...
        <font class="campo">
            <a class="cliccabile" href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.siep.fascicolo.action.ActLoadDettaglioFascicolo&<%=ICostantiFascicoloSiep.CAMPO_ID_FASCICOLO_SIEP%>=<%=fascicoloUno.getFascicoloSiep().getIdFascicoloSiep()%><%=retParam%>">
              <%=fascicoloUno.getFascicoloSiep().getChiaveAnno()%>/<%=fascicoloUno.getFascicoloSiep().getChiaveProgr()%>
            </a>
           &nbsp;&nbsp;<%=fascicoloUno.getFascicoloSiep().getDescrTipoUfficio()%>&nbsp;<%=fascicoloUno.getFascicoloSiep().getDescrComuneUfficio()%>
         </font>&nbsp;
      </td>
    </tr>
  </table>
