...
<%
    ...
    while ( itx.hasNext())
    {
      ScadenzarioSigeModel lSca = (ScadenzarioSigeModel)itx.next();
      FascicoloSigeModel lFas = lSca.getFascicoloSige();
      SoggettoModel lSog = lSca.getSoggetto();
%>
 <tr>
<%
    if(tipo.equals("tutti"))
...
%>
       ...
       <% if( lSca!=null && lSca.getFlagVisto()!=null && lSca.getFlagVisto().equals("S"))
          ...
          {%>
            <td class=C> &nbsp;</td>
            <td class=C>
              <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.sige.scadenzario.action.ActVistoScadenzarioSige&<%=ICostantiScadenzarioSige.CAMPO_ID_SCADENZARIO_SIGE%>=<%=lSca.getIdScadenzarioSige()%>&T=<%=tipo%>&A=<%=anni%>&M=<%=mesi%>&G=<%=giorni%>&TS=<%=tipoScadenzario%>&DS=<%=descTipoScadenzario%>  ">
