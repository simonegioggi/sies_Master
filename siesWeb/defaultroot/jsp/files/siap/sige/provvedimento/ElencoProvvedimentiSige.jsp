...
<table>
	<tr>
      	<td class="LBG"><font class="label"> Non ci sono provvedimenti riferiti al procedimento indicato </font></td>
	</tr>
</table>
<%
} else {
...
%>
			<font class="label">-&nbsp;<%=lTenore.getDescrOggettoSige()%><br></font>
<%
			}
		}
%>
...
<%
			if ((lProvEve.getEventoNotifica().getEvento().getFlagDocumentoRegistrato() != null)
					&& (lProvEve.getEventoNotifica().getEvento().getFlagDocumentoRegistrato().compareTo("S") == 0)) {
           		...
           		if (lProvEve.getEventoNotifica().getEvento().getNumAllValidati() < 1
           				...
           				&& (modificabile || lProvEve.getEventoNotifica().getEvento().getCodEsito().compareToIgnoreCase("0600") == 0)) {
%>
...
<%
       		...
       		// ANNULLATO
        	} else if (lProvEve.getEventoNotifica().getEvento().getFlagDocumentoRegistrato() != null
        			&& lProvEve.getEventoNotifica().getEvento().getFlagDocumentoRegistrato().compareTo("A") == 0) {
%>
...
%>
		<!-- casella stato notifica -->
	  	<td class="l">
	  		<center><%=(lProvEve.isAllNotified()?"<img src=\"/images/TickRed.gif\">" : "-")%></center>
	  	</td>
		<td class="l">
<%
			String isBlob = "SI"; 
	        ...
	        if (lProvEve.getEventoNotifica().getEvento().getNumAllegati() > 0
	        		// aggiunto controllo altrimenti si deposita senza validazione del provvedimento
	        		&&  "S".equals(lProvEve.getEventoNotifica().getEvento().getFlagDocumentoRegistrato())) {
	        	isAllegato = "SI";
	        }
%>
<!--
...
%>
				<jsp:include page="<%=ICostantiProvvedimentoSige.PG_BUTTONS_PROVVEDIMENTO%>">
					...
					<jsp:param name="ValoreIdEntitaProvv" value="<%=lProvEve.getEventoNotifica().getEvento().getFlagDocumentoRegistrato()%>"/>
					<jsp:param name="CodTipoProvvedimento" value="<%=lProvEve.getEventoNotifica().getEvento().getCodTipoProvvedimento()%>"/>
					<jsp:param name="CodTipoProvvedimentoSige" value="<%=lProvEve.getProvvedimento().getCodTipoProvvedimentoSige()%>"/>
...
%>
				<jsp:include page="<%=ICostantiProvvedimento.PG_BUTTONS_NOTIFICHE%>">
					...
					<jsp:param name="FLAG_PIU_MENO" value="<%=lProvEve.getEventoNotifica().getEvento().getFlagPiuMeno()%>"/>
				</jsp:include>
<%
			}
%>
