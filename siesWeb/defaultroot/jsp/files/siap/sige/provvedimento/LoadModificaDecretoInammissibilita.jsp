...
<%
	...
	// Estrazione della data iscrizione
	...
	String giornoEmissione=DateUtils.getDateToString(ProvvedimentoEvento.getEventoNotifica().getEvento().getDataEmissione(),"dd");
	String meseEmissione=DateUtils.getDateToString(ProvvedimentoEvento.getEventoNotifica().getEvento().getDataEmissione(),"MM");
	String annoEmissione=DateUtils.getDateToString(ProvvedimentoEvento.getEventoNotifica().getEvento().getDataEmissione(),"yyyy");
...
<html>
  <head>
    <title>[S.I.E.S.] - Emissione Decreto di Inammissibilità </title>
...
<%--         	<% if (defTipoGiudizio) { %> --%>
<%--         		Visualizza(document.LoadEmissioneDecretoInammissibilita.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value); --%>
<%--         	 <%} %> --%>
        	Verifica();
        	<%

        	Vector <MotivazioneProvvedimentoSigeModel>motivi=ProvvedimentoEvento.getMotiviProvvedSige();
            for (MotivazioneProvvedimentoSigeModel motivo : motivi) {
            	if (motivo.getCodTipoMotivazione().equals("90") || motivo.getCodTipoMotivazione().equals("91")) {
            		out.println ("document.getElementById('AltraMotivazione"+motivo.getCodTipoMotivazione()+"').value='"+StringUtils.cStrForJS(motivo.getDescrMotivazione())+"'");
            	}
            	out.println ("document.getElementById('CheckMotivo"+motivo.getCodTipoMotivazione()+"').checked='checked'");
            }
    ...
    function Verify()
    {
      ...
      var data_emissione = document.LoadEmissioneDecretoInammissibilita.<%=ICostantiProvvedimentoSige.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadEmissioneDecretoInammissibilita.<%=ICostantiProvvedimentoSige.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadEmissioneDecretoInammissibilita.<%=ICostantiProvvedimentoSige.CAMPO_ANNO_DATA_EMISSIONE%>.value;
      ...
      // Controllo data emissione < data iscrizione
      else if( !CompareDate( data_iscrizione, data_emissione ) )
      {
...
<%     	if (defTipoGiudizio) { %>
	    	...
	    	if ( document.LoadEmissioneDecretoInammissibilita.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value != "-" )
	    	{
	    		if (document.LoadEmissioneDecretoInammissibilita.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value == "C"){
...
<%  	} %>
	...
	{
		var ritorno = true;
		var collegio = document.LoadEmissioneDecretoInammissibilita.<%=ICostantiCollegio.CAMPO_ID_COLLEGIO%>.value;
		if ( collegio == '' ) 
    {
    ...
    //   
  	function InserisciUdienza( tipoRito ){
  		var dataUdienza = document.LoadEmissioneDecretoInammissibilita.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA%>.value +'/'+
                          document.LoadEmissioneDecretoInammissibilita.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA%>.value +'/'+
                          document.LoadEmissioneDecretoInammissibilita.<%=ICostantiUdienzaSige.CAMPO_ANNO_DATA_UDIENZA%>.value;
  		...
  		if( tipoRito == 'M' ){
  			...
  			var idUdienza=document.LoadEmissioneDecretoInammissibilita.<%=ICostantiUdienzaSige.CAMPO_ID_UDIENZA_SIGE %>.value;
	...
	function setUdienza (idUdienza, dataUdienza, idCollegio, idSezione, codMagistrato) {
	 	if (dataUdienza != "null") {
		     ...
		     document.LoadEmissioneDecretoInammissibilita.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA %>.value=dataSplitted[0];
		     document.LoadEmissioneDecretoInammissibilita.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA %>.value=dataSplitted[1];
		     document.LoadEmissioneDecretoInammissibilita.<%=ICostantiUdienzaSige.CAMPO_ANNO_DATA_UDIENZA %>.value=dataSplitted[2];
		     document.LoadEmissioneDecretoInammissibilita.<%=ICostantiUdienzaSige.CAMPO_ID_UDIENZA_SIGE %>.value=idUdienza;
		     document.LoadEmissioneDecretoInammissibilita.<%=ICostantiCollegio.CAMPO_ID_COLLEGIO%>.value=idCollegio;
		     if (codMagistrato != null) {
		    		document.LoadEmissioneDecretoInammissibilita.<%=ICostantiMagistrato.CAMPO_COD_MAGISTRATO%>.value = codMagistrato;
		     }
	 	} 
	 }
  ...
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG"><font class="label">Funzione :  </font>&nbsp;
        ...
        <font class="campo">Modifica Emissione Decreto Inammissibilità</font>
      </td>
         <!-- BOTTONE DI RITORNO -->
    ...
    <jsp:include page="<%=ICostantiMagistratoAssegnatario.PG_SINTESIMAGISTRATOASSEGNATARIO%>">
          <jsp:param name="MagAssRitorno" value="siap.sige.provvedimento.action.ActLoadModificaDecretoInammissibilita"/>
    </jsp:include>

    </tr>

      <jsp:include page="<%=ICostantiAvvocatoFascicoloSige.PG_INCLUDE_AVVOCATI%>">
      <jsp:param name="AvvRitorno" value="siap.sige.provvedimento.action.ActLoadModificaDecretoInammissibilita"/>
      </jsp:include>

  </table>


  <FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name="LoadEmissioneDecretoInammissibilita">
  
    <table cellspacing="2" cellpadding="2">
    <tr>
      <td class="l">Data Emissione<font class="ob">(*)</font></td>
      <td class="L">
        <input type="text" size="2" maxlength="2" name="<%=ICostantiProvvedimentoSige.CAMPO_GIORNO_DATA_EMISSIONE%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillDM(value)" value = "<%=giornoEmissione%>"> /
        <input type="text" size="2" maxlength="2" name="<%=ICostantiProvvedimentoSige.CAMPO_MESE_DATA_EMISSIONE%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillDM(value)" value = "<%=meseEmissione%>" > /
        <input type="text" size="4" maxlength="4" name="<%=ICostantiProvvedimentoSige.CAMPO_ANNO_DATA_EMISSIONE%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillYear(value)" value = "<%=annoEmissione%>">

		<!-- MEV 15 - Revisione SIGE -->
		<a href="javascript:calendario('LoadEmissioneDecretoInammissibilita','<%=ICostantiProvvedimentoSige.CAMPO_ANNO_DATA_EMISSIONE%>','<%=ICostantiProvvedimentoSige.CAMPO_MESE_DATA_EMISSIONE%>','<%=ICostantiProvvedimentoSige.CAMPO_GIORNO_DATA_EMISSIONE%>');">
   ...
   <tr>
    <td class="label">Tipo Rito &nbsp;
  	...
  	<jsp:include page="<%=ICostantiUdienzaCollegiale.PG_INCLUDE_INSERISCI_COLLEGIO%>">
          <jsp:param name="form_name" value="LoadEmissioneDecretoInammissibilita"/>
          <jsp:param name="tipo_ufficio" value="<%=tipoUfficioUtente%>"/>
     </jsp:include>
    --%>   

    <%-- Modifica del 08/03/2017 --%>
    <jsp:include page="<%=ICostantiProvvedimentoSige.INC_TIPO_GIUDIZIO_COLLEGIO_DECRETO_INAMMISSIBILITA%>" />

  <%-- Motivi di Inammissibilità            --%> 
  <br>
 		<jsp:include page="<%=ICostantiProvvedimentoSige.INC_MOTIVI_INAMMISSIBILITA%>"/>
       		...
       		<td class="L">
         		<select title="tipoUfficioCompetente" class=small name="<%=ICostantiFascicoloSige.CAMPO_CHIAVE_UFFICIO%>">
          		...
          		<input Title="Sede Procura" name="<%=ICostantiFascicoloSige.CAMPO_DESCR_COMUNE_UFFICIO%>" type="text" maxlength="35" size="35"  value="<%=lSedeUDS%>">
             		<a href="Javascript:ListaComuni('LoadEmissioneDecretoInammissibilita','<%=ICostantiFascicoloSige.CAMPO_DESCR_COMUNE_UFFICIO%>',document.LoadEmissioneDecretoInammissibilita.<%=ICostantiFascicoloSige.CAMPO_CHIAVE_UFFICIO%>[document.LoadEmissioneDecretoInammissibilita.<%=ICostantiFascicoloSige.CAMPO_CHIAVE_UFFICIO%>.selectedIndex].value);">
  ...
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sige.provvedimento.action.ActModificaDecretoInammissibilita" >
 ...
 <input type="HIDDEN" name="<%=ICostantiProvvedimentoSige.CAMPO_COD_TIPO_PROV_SIGE%>" value="<%=ICostantiProvvedimentoSige.COD_DECRETO_INAMMISSIBILITA%>">
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("LoadEmissioneDecretoInammissibilita");
