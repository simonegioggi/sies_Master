    ...
    <tr>
      ...
      <td class="L">
       ...
       <input value="<%=StringUtils.toStringJSP( DateUtils.getDateToString(notifica.getDataInvio(),"yyyy"))%>" type="text" size="4" maxlength="4" name="<%= ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillYear(value)" >
		...
		<a href="javascript:calendario('<%=NomeForm%>','<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>','<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>','<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>');">
...
%>
          <td class="l" colspan="3"> Per la notifica all' Avv. <font class="campo"><%=StringUtils.toStringJSP(notifica.getAvvSige().getAvvocato().getCognome() +" "+notifica.getAvvSige().getAvvocato().getNome())%></font>&nbsp; </td>
<%
		}
      }
%>
...
%>
		<td class="l" colspan="1" ><font class="campo"><%=notifica.getUfficio().getDescrTipoUfficio()%></font></td>
         <td class="campo" nowrap>
			<input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>" value="<%=notifica.getUfficio().getDescrComune()%>" type="text" maxlength="35" size="35">
<%
...
%>
          ...
          <td class="campo" nowrap>
			<input Title="Sede del Comune di" name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>" value="<%=notifica.getAutoritaEsterna().getDescrSede()%>" type="text" maxlength="35" size="35">
<%
...
<%
		// Istituto Detenzione
      	} else if (notifica.getIstitutoDetenzione() != null) {
%>
		<td class="l"><font class="l">Tipo Istituto</font></td>
          <td class="campo" nowrap>
...
%>
          ...
          <td class="campo" nowrap>
		  	<input Title="Sede UEPE" name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>" value="<%=notifica.getCSSA().getComune()%>" type="text" maxlength="35" size="35">
...
<%
       	...
       	// MAC20200107012-MG-09/01/2020: formattazione layout
       	else {
%>
		<td class="l" colspan="2"></td>
		<input name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>" type="hidden" value="" title="VUOTO">
<%
       }
       	// FINE MAC20200107012-09/01/2020: formattazione layout
%>
       <td class=l nowrap>
