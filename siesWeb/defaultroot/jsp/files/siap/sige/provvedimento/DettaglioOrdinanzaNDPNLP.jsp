...
<%
  ...
  String titolo = "Dettaglio Ordinanza NDP/NLP";
  if (modificaOrdinanza)
	  titolo = "Modifica Ordinanza NDP/NLP";
...
<html>
  <head>
    <title>[S.I.E.S.] - Dettaglio Ordinanza NDP/NLP</title>
  ...
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG"><font class="label">Funzione :  </font>&nbsp;
          <font class="campo">Dettaglio Ordinanza NDP/NLP</font>
      </td>
...
<%  if (Modificabile.compareTo("SI")==0) {%> 
  ...
  <table width="90%">
    <tr>
      ...
      <%-- [EC] - 21082018 : intervento post collaudo
		 emissione ordinzanza di NDPNLP(segnalazione Nunzia vedi documento esiti procedimento.docx).	Aggiungo la colonna azioni
       --%>
    ...
    <tr>
  			...
  			<%-- [EC] - 21082018 : intervento post collaudo
		 emissione ordinzanza di NDPNLP(segnalazione Nunzia vedi documento esiti procedimento.docx).	Includo la jsp ElencoGestioneTenori.jsp (prima includeva erroneamente ElencoDettaglioTenori)
       	    --%>
...
<% 
   if( ProvvedimentoEvento != null && ProvvedimentoEvento.getMotiviProvvedSige() != null  && ProvvedimentoEvento.getMotiviProvvedSige().size() > 0)
  {
	...
	if( motivoProvvedimento != null  && motivoProvvedimento.getAltraMotivazione() != null )
   	{ 
%>  		
	  <tr>
	    <td class="Titolo">Motivazioni</td>
	  </tr>

	  <tr>
	    <td class="l" colspan=2><%=motivoProvvedimento.getAltraMotivazione()%></td>
	  </tr>
<%
   	}
 }
%>
	  <tr>
	    <td class="Titolo" colspan=2>Dispone</td>
	  </tr>
<%
    if( ProvvedimentoEvento.getMotiviProvvedSige() != null && ProvvedimentoEvento.getMotiviProvvedSige().size() > 0)
    {
      ...
      for( int x=0; x<ProvvedimentoEvento.getMotiviProvvedSige().size(); x++ )
      {
        // Stampa la descrizione motivazione se è != null
        if( ProvvedimentoEvento != null &&
        		ProvvedimentoEvento.getMotiviProvvedSige() != null &&
      			ProvvedimentoEvento.getMotiviProvvedSige().elementAt(x) != null )
        {
          motivoProvvedimento = (MotivazioneProvvedimentoSigeModel) ProvvedimentoEvento.getMotiviProvvedSige().elementAt(x) ;
%>
          <tr>
            <td class="l" colspan=2>
<%          if(motivoProvvedimento != null && motivoProvvedimento.getDescrMotivazione() != null){ %>            
            	<%=motivoProvvedimento.getDescrMotivazione()%>
<%          } %>            
            </td>
          </tr>
<%
...
<%	if (provvedimento.getDescrUfficioDestinatario()!=null &&
		provvedimento.getDescrUfficioDestinatario().length()>2)
	{
%>
  	  <table  width="90%" >
	  	<tr>
	    	<td class="Titolo">Ufficio Competente</td>
	  	</tr>
	  	<tr>
	    	<td class="l"><%=provvedimento.getDescrUfficioDestinatario()%></td>
	  	</tr>
	  </table>
<%   } %>
    ...
    <table>
        ...
        <tr>
          <td class="L">
            ...
            <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>"  value="siap.sige.provvedimento.action.ActDettaglioOrdinanzaNDPNLP">
          </td>
        </tr>
        </table>
        </FORM>
