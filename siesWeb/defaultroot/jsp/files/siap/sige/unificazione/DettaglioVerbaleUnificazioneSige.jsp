...
<html>
<head>
  <title>[S.I.E.S.] - Dettaglio Verbale di Unificazione Sige</title>
  ...
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Dettaglio Verbale Unificazione Sige</font>&nbsp;
      </td>
<%
...
%>
  <!-- BOTTONE DI STAMPA -->
    <jsp:include page="<%=ISIAPCostantiWeb.PG_BUTTONS_STAMPA_SIGE%>">
          <jsp:param name="CampoIdEntita" value="<%=ICostantiVerbaleUnificazioneSige.CAMPO_ID_VERBALE_UNIFICAZIONE%>"/>
          <jsp:param name="ValoreIdEntita" value="<%=provvedimentoUnificazione.getProvvedimento().getIdProvvedimentoSige()%>"/>
    </jsp:include>

    <!-- BOTTONE DI CANCELLAZIONE -->
    <td class="LBG">
      <a href="Javascript:conferma('siap.sige.unificazione.action.ActCancellaVerbaleUnificazioneSige','<%=ICostantiVerbaleUnificazioneSige.CAMPO_ID_VERBALE_UNIFICAZIONE%>','<%=provvedimentoUnificazione.getProvvedimento().getIdProvvedimentoSige()%>','TornaQui','<%=TornaQui%>', '<%=ICostantiFascicoloSige.CAMPO_ID_FASCICOLO_SIGE%>', '<%=idFascicoloUnificato%>')">
      <img align="middle" src="<%=IWebConstants.IMAGES_DIR%>delete24.gif" alt="Cancella" width="24" height="24" border="0">
      </a>
    </td>
    
    <!-- BOTTONE DI RITORNO -- lo includo solo se non è stata gia fatta la validazione -->
    <%-- <jsp:include page="<%=IWebConstants.PG_RETURN_BUTTON%>"/>  --%>
<%}%>
    <!-- Modifica del 03/03/2017 sposto il tasto "Indietro" fuori dall'if, affinchè
    	 sia visibile anche dopo la validazione.
    	 
    	 Se provengo dall'inserimento del Verbale di Unificazione, il tasto Indietro
  ...
  <table>
    ...
    <tr>
      <td class="L">
       ...
       <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>"  value="siap.sige.unificazione.action.ActLoadDettaglioVerbaleUnificazioneSige">
       <input type="HIDDEN" name="<%=IWebConstants.ACTION_DOPO_CANCELLAZIONE%>"  value="siap.sige.unificazione.action.ActLoadVerificaVerbaleUnificazioneSige">    
      </td>
    </tr>
  </table>
