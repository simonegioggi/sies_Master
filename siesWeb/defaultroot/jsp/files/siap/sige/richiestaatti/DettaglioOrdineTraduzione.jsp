...
<%
  ...
  String titolo = "Dettaglio Rogatoria MdS";
  if (modificaOrdinanza)
	  titolo = "Modifica Rogatoria MdS";
  ...
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG"><font class="label">Funzione :  </font>&nbsp;
          <font class="campo">Dettaglio Ordine Traduzione</font>
      </td>
   ...
   //  		} /* endif esistenza ElencoTemplate */
      ...
      <td class="LBG">
       <a href="Javascript:stampa2( '<%=ISIAPCostantiWeb.PG_STAMPA%>', '<%=IWebConstants.ACTION_FIELD%>=siap.sico.evento.action.ActLoadDocumento&<%=ICostantiEvento.CAMPO_ID_EVENTO%>=<%=ProvvedimentoEvento.getEventoNotifica().getEvento().getIdEvento()%>')">
        <img src="/images/print24.gif" alt="Visualizza Ordine Traduzione" width="24" height="24" border="0">
        </a>
      </td> 
  	<%}
...
<table >
<tr>
      <td class="l">Data Richiesta del Condannato</td>
      <td class="L" colspan=5>
        <font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(ProvvedimentoEvento.getEventoNotifica().getEvento().getDataRichiesta(),"dd-MM-yyyy"))%></font>&nbsp;
      </td>
    </tr>
    <tr>
...
<%
   ...
   // Preleva le note
   if (ProvvedimentoEvento.getEventoNotifica().getNotifiche()!= null && ProvvedimentoEvento.getEventoNotifica().getNotifiche().length > 0 )
    {
    %>
    </table>   
 	 <table style="width: 95%;">
    <tr>
      <td class="Titolo">Ulteriori Informazioni</td>
    </tr>
    </table><table>
      <%
	  int lSizeNotifiche = ProvvedimentoEvento.getEventoNotifica().getNotifiche().length;
      for(int count = 0;count < lSizeNotifiche; count++)
      {
		// Gestione Note
		    if (ProvvedimentoEvento.getEventoNotifica().getNotifiche()[count].getIstitutoDetenzione() != null ) { %>
	            <tr>
	            <td class="l">Destinatario </td>
	            <td class="L" colspan="5">
	              <font class="campo"><%=ProvvedimentoEvento.getEventoNotifica().getNotifiche()[count].getIstitutoDetenzione().getDescrTipoIstituto()%></font>&nbsp;
	            </td>
	            <td class="l">Luogo </td>
	            <td class="L" colspan="5">
	              <font class="campo"><%=ProvvedimentoEvento.getEventoNotifica().getNotifiche()[count].getIstitutoDetenzione().getDescrizione()%></font>&nbsp;
	            </td>
		   ...
		   <% }
            ...
            if (ProvvedimentoEvento.getEventoNotifica().getNotifiche()[count].getNote() != null )
            {
                %>
                  <tr>
                    
                    <td class="L" colspan="6">
                      <font class="campo"><%=ProvvedimentoEvento.getEventoNotifica().getNotifiche()[count].getNote()%></font>&nbsp;
                    </td>
                  </tr>
                <%
  ...
  <div align=left style="visibility:hidden" id="upld">
    ...
    <table>
        ...
        <tr>
          ...
          <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>"  value="siap.sige.provvedimento.action.ActDettaglioOrdineTraduzione">
