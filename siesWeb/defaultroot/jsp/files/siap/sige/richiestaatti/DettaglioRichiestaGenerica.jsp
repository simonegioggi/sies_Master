...
<jsp:useBean id="notifiche" scope="request" class="siap.sico.evento.model.EventoNotificaModel"/>
...
<html>
  ...
  <head>
    <title> [S.I.E.S.] - Dettaglio Richiesta Generica - </title>
...
<%
  ...
  if (tipoRichiesta == null || tipoRichiesta.trim().length() < 1)
	  tipoRichiesta = " ";
      ...
      <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
        <td class="LBG">
        	<font class="label">Funzione :</font>&nbsp;
          	<font class="campo">Dettaglio <%=tipoRichiesta %></font>
        </td>
...
%>
     ...
     <%
    // Gestione Autorità *****************************************************************************
    int lSizeNotifiche = notifiche.getNotifiche().length;
    for(int count = 0;count < lSizeNotifiche; count++)
    {%>
    
    <% if (notifiche.getNotifiche()[count].getUfficio() != null )
        {
        %>
          <tr>
            <td class="l">Destinatario n° <%=count+1%></td>
            <td class="L" colspan="5">
              <font class="campo"><%=notifiche.getNotifiche()[count].getUfficio().getDescrTipoUfficio()%></font>&nbsp;
            </td>

            <td class="l">Luogo </td>
            <td class="L" colspan="5">
              <font class="campo"><%=notifiche.getNotifiche()[count].getUfficio().getDescrComune()%></font>&nbsp;
            </td>
          </tr>

        <%
        } // Gestione Autorità Esterna
        else if (notifiche.getNotifiche()[count].getAutoritaEsterna() != null )
        {
        %>
          <tr>
            <td class="l">Destinatario n° <%=count+1%></td>
            <td class="L" colspan="5">
              <font class="campo"><%=notifiche.getNotifiche()[count].getAutoritaEsterna().getDescrTipoAutorita()%></font>&nbsp;
            </td>

            <td class="l">Luogo </td>
            <td class="L" colspan="5">
              <font class="campo"><%=notifiche.getNotifiche()[count].getAutoritaEsterna().getDescrSede()%></font>&nbsp;
            </td>

          </tr>
        <%
        } // Gestione CSSA
        else if (notifiche.getNotifiche()[count].getCSSA() != null )
        {
        %>
          <tr>
            <td class="l">Destinatario n° <%=count+1%></td>
            <td class="L" colspan="5">
              <font class="campo"><%=notifiche.getNotifiche()[count].getCSSA().getTipo()%></font>&nbsp;
            </td>
            <td class="l">Luogo </td>
            <td class="L" colspan="5">
              <font class="campo"><%=notifiche.getNotifiche()[count].getCSSA().getComune()%></font>&nbsp;
            </td>

          </tr>
        <%
        } // Gestione istituto di detenzione.
        else if (notifiche.getNotifiche()[count].getIstitutoDetenzione() != null )
        {
        %>
          <tr>
            <td class="l">Destinatario n° <%=count+1%></td>
            <td class="L" colspan="5">
              <font class="campo"><%=notifiche.getNotifiche()[count].getIstitutoDetenzione().getDescrTipoIstituto()%></font>&nbsp;
            </td>
            <td class="l">Luogo </td>
            <td class="L" colspan="5">
              <font class="campo"><%=notifiche.getNotifiche()[count].getIstitutoDetenzione().getDescrizione()%></font>&nbsp;
            </td>
        ...
        <%
         ...
         // Gestione Note
            if (notifiche.getNotifiche()[count].getNote() != null )
            {
                %>
                  <tr>
                    <td class="l"> </td>
                    <td class="L" colspan="5">
                      <font class="campo"><%=notifiche.getNotifiche()[count].getNote()%></font>&nbsp;
                    </td>
                  </tr>
                <%
             }
         ...
         <FORM name="comandi" enctype="multipart/form-data" method="post">
          ...
          <jsp:include page="<%=ISIAPCostantiWeb.CAMPI_VALIDA_UPLOAD%>" />
             <tr><td class="L">
                 ...
                 <input type="HIDDEN" name="<%=ICostantiEvento.CAMPO_AZIONE_DETTAGLIO%>"  value="siap.sige.richiestaatti.action.ActDettaglioRichiestaFascicoloPenale">
