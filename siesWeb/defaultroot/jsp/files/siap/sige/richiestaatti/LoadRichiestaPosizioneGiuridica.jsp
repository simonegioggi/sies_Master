...
<html>
  <head>
    <title>[S.I.E.S.] - Richiesta Posizione Giuridica</title>
    ...
    function Verify()
	  {
		  if (document.LoadRichiestaPosizioneGiuridica.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
			  document.LoadRichiestaPosizioneGiuridica.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadRichiestaPosizioneGiuridica.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
		  if (document.LoadRichiestaPosizioneGiuridica.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
			  document.LoadRichiestaPosizioneGiuridica.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadRichiestaPosizioneGiuridica.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value;

		  var data_to_verify = document.LoadRichiestaPosizioneGiuridica.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadRichiestaPosizioneGiuridica.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadRichiestaPosizioneGiuridica.<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>.value;
 ...
 <body class="corpo" >
   <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
      <font class="label">Funzione :</font> &nbsp;&nbsp;<font class="campo">Richiesta Posizione Giuridica Ad Autorità Penitenziaria</font>
        ...
        if( modalita.equals("I") )
         {
          ...
          lAzione = "siap.sige.richiestaatti.action.ActInserisciPosizioneGiuridica";
      %>
          <font class="campo">Richiesta Posizione Giuridica Ad Autorità Penitenziaria</font>
      <%}
...
<br>
  ...
  <br>
  <FORM method="POST" name="LoadRichiestaPosizioneGiuridica" action="<%= IWebConstants.PG_MAIN%>">
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sige.richiestaatti.action.ActInserisciPosizioneGiuridica">
    <table>
	...
	<tr>
        ...
        <td class="L">
		  ...
		  <a href="javascript:calendario('LoadRichiestaPosizioneGiuridica','<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>','<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>','<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>');">
    ...
    <tr><td class="l">Sede <font class="ob">(*)</font></td><td class="L">
       ...
       <a href="Javascript:ListaComuni('LoadRichiestaPosizioneGiuridica','<%= ICostantiIstruttoria.SEDE_AUTORITA_DESTINATARIO %>');">
    ...
    <tr>
        ...
        <td class="L">
           <input value="" type="text" size="35" maxlength="40" name="<%= ICostantiIstruttoria.INDIRIZZO_DESTINATARIO %>"  >
        </td>
      </tr>
...
<script language="JavaScript" type="text/javascript">
  var frmvalidator  = new Validator("LoadRichiestaPosizioneGiuridica");
