...
<html>
  <head>
    <title>[S.I.E.S.] - Ordine Traduzione </title>
        ...
        function Verify()
    {
      ...
      var data_emissione = document.LoadOrdineTraduzione.<%=ICostantiProvvedimentoSige.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadOrdineTraduzione.<%=ICostantiProvvedimentoSige.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadOrdineTraduzione.<%=ICostantiProvvedimentoSige.CAMPO_ANNO_DATA_EMISSIONE%>.value;
...
<%	if (defTipoGiudizio) { %>
     //Controllo obbligatorietà Collegio in caso di Tipo Giudizio impostato.
    	if ( document.LoadOrdineTraduzione.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value != "-" )
    	{
    	if (document.LoadOrdineTraduzione.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value == "C")
  	...
  	<%}%>
		...
		{
			var ritorno = true;
			var collegio = document.LoadOrdineTraduzione.<%=ICostantiCollegio.CAMPO_ID_COLLEGIO%>.value;
			if ( collegio == '' ) 
	    {
  ...
  function ListaUDS(a_formname,a_fieldname)
  {
    ...
    function Verify()
	  {
		  if (document.LoadOrdineTraduzione.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
			  document.LoadOrdineTraduzione.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadOrdineTraduzione.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
		  if (document.LoadOrdineTraduzione.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
			  document.LoadOrdineTraduzione.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadOrdineTraduzione.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value;

		  var data_to_verify = document.LoadOrdineTraduzione.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadOrdineTraduzione.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadOrdineTraduzione.<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>.value;
      ...
      {
        var a_codnum = document.LoadOrdineTraduzione.<%=ICostantiMagistrato.CAMPO_COD_MAGISTRATO_VECCHIO %>.value;
 ...
 <body class="corpo" >
   <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
      <font class="label">Funzione :</font> &nbsp;&nbsp;<font class="campo">Ordine Traduzione</font>
<%
        String lAction = new String();
        lAction = "siap.sige.richiestaatti.action.ActInserisciordineTraduzione";
%>
      <%
        ...
        if( modalita.equals("I") )
         {
          ...
          lAzione = "siap.sige.richiestaatti.action.ActInserisciOrdineTraduzione";
      %>
 ...
 <br>
  ...
  <FORM method="POST" name="LoadOrdineTraduzione" action="<%= IWebConstants.PG_MAIN%>">
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sige.richiestaatti.action.ActInserisciOrdineTraduzione">
   ...
   <table>
	<tr>
        ...
        <td class="L">
		  ...
		  <a href="javascript:calendario('LoadOrdineTraduzione','<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>','<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>','<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>');">
   ...
   <tr>
        ...
        <td class="L">
		  ...
		  <!-- MEV 15 - Revisione SIGE -->
		  <a href="javascript:calendario('LoadOrdineTraduzione','<%=ICostantiEvento.CAMPO_ANNO_DATA_INSERIMENTO%>','<%=ICostantiEvento.CAMPO_MESE_DATA_INSERIMENTO%>','<%=ICostantiEvento.CAMPO_GIORNO_DATA_INSERIMENTO%>');">
       	 	  <img src="/images/calendario.gif" border=0>
          </a>
        </td>
   </tr>
   ...
   <!-- Terzo destinatario + luogo -->
	  <tr>
             <td class="l">Istituto Detenzione <font class=ob>(*)</font></td>
             ...
             <td class="l">
               ...
               <a href="Javascript:ListaIstitutoDetenzione('LoadOrdineTraduzione','<%= ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>','Comune');">
...
<script language="JavaScript" type="text/javascript">
  var frmvalidator  = new Validator("LoadOrdineTraduzione");
  ...
  frmvalidator.addValidation("<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>", "req","Il campo Istituto Detenzione è obbligatorio");
