...
<html>
  <head>
    <title>[S.I.E.S.] - Gestione Richiesta AL PM </title>
    ...
    function Verify()
	  {
		  if (document.LoadRichiestaPM.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
			  document.LoadRichiestaPM.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadRichiestaPM.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
		  if (document.LoadRichiestaPM.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
			  document.LoadRichiestaPM.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadRichiestaPM.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value;

		  var data_to_verify = document.LoadRichiestaPM.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadRichiestaPM.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadRichiestaPM.<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>.value;
 ...
 <body class="corpo" >
   <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
      <font class="label">Funzione :</font> &nbsp;&nbsp;<font class="campo">Richiesta Al PM</font>
        ...
        if( modalita.equals("I") )
         {
          ...
          lAzione = "siap.sige.richiestaatti.action.ActInserisciRichiestaPM";
      %>
          <font class="campo">Inserisci Richiesta Al PM</font>
      <%}
        else if( modalita.equals("M") )
        {
          lProvvedimento = new EventoModel(evento);
          lAzione = "siap.sige.richiestaatti.action.ActInserisciRichiestaPM";
      %>  <font class="campo">Modifica Richista Al PM</font>
      <%}
      %>
      </td>
      <!-- BOTTONE DI RITORNO -->
...
<br>
  ...
  <br>
  <FORM method="POST" name="LoadRichiestaPM" action="<%= IWebConstants.PG_MAIN%>">
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sige.richiestaatti.action.ActInserisciRichiestaPM">
    <table>
	...
	<tr>
        ...
        <td class="L">
		  ...
		  <a href="javascript:calendario('LoadRichiestaPM','<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>','<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>','<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>');">
 ...
 <%//ICostantiIstruttoria.AUTORITA_DESTINATARIO %>
    ...
    <tr><td class="l">Sede <font class="ob">(*)</font></td><td class="L">
       ...
       <a href="Javascript:ListaComuni('LoadRichiestaPM','<%= ICostantiIstruttoria.SEDE_AUTORITA_DESTINATARIO %>');">
       <img src="/images/filefolder.gif" border=0></a></td>
    </tr>

    <tr>
        <td class="l">Tipo di Richiesta <font class="ob">(*)</font></td>
        <td class="L">
   ...
   <!-- Campo Note  -->
        <tr>
            <td class="l">Descrizione Richiesta </td>
...
<script language="JavaScript" type="text/javascript">
  var frmvalidator  = new Validator("LoadRichiestaPM");
  ...
  frmvalidator.addValidation("<%= ICostantiIstruttoria.TIPO_DOCUMENTO%>","req","Il campo Tipo Richiesta è obbligatorio");
