...
<jsp:useBean id="listaTipoProvvDecretoArchiviazione" scope="request" class="java.lang.String"/>

<!-- segnalazioni 4: modificato il titolo -->
<title>[S.I.E.S.] - Inserimento Decreto di Archiviazione</title>
<link rel="STYLESHEET" type="text/css" href="<%=IWebConstants.PG_STYLE%>">
<script language="JavaScript" src="<%=IWebConstants.JS_DATE_CONTROL%>"></script>

<script language="JavaScript">
	// segnalazioni 4: modificato il nome funzione
	function VerifyDECRETO() {
		if (document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_PROVVEDIMENTO%>.value.length==1)
          	document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_PROVVEDIMENTO%>.value='0'+document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_PROVVEDIMENTO%>.value;
        if (document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value.length==1)
          	document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value='0'+document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value;
        if (document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value.length==1)
          	document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value='0'+document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value;
        // segnalazioni 4: aggiunti controlli preventivi
		if (document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_IRREVOCABILITA%> &&
				document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_IRREVOCABILITA%>.value.length==1)
			document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_IRREVOCABILITA%>.value='0'+document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_IRREVOCABILITA%>.value;
        if (document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_MESE_DATA_IRREVOCABILITA%> &&
        		document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_MESE_DATA_IRREVOCABILITA%>.value.length==1)
          	document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_MESE_DATA_IRREVOCABILITA%>.value='0'+document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_MESE_DATA_IRREVOCABILITA%>.value;

        // Data Emissione Provvedimento
        var d2=document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_PROVVEDIMENTO%>.value+'/'+document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value+'/'+document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_ANNO_DATA_PROVVEDIMENTO%>.value;
        if (!ControllaData(d2)) {
...
<FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name="LoadInserisciSentenzaDecretoArchiviazione">
 	...
 	<tr>
        ...
        <td class="l">
          <select title="tipoProvvedimento">
			  <%=listaTipoProvvDecretoArchiviazione%>          
		  </select>
        </td>
    </tr>

	<!-- segnalazioni 4: aggiunte due righe -->
	<tr>
		<td class="l">Anno/Numero R.G.N.R.</td>
		<td class="L">
			<input Title="Anno Re.Ge. PM" value="<%=StringUtils.toStringJSP( lSentenza.getAnnoRegePm()) %>" type="text" name="<%= ICostantiSentenza.CAMPO_ANNO_REGE_PM %>" maxlength="4" size="4"> /
			<input Title="Numero Re.Ge. PM" value="<%=StringUtils.toStringJSP(lSentenza.getNumeroRegePm()) %>" type="text" name="<%= ICostantiSentenza.CAMPO_NUMERO_REGE_PM %>" maxlength="6" size="6">
		</td>
		<td class="l"></td>
		<td class="L"></td>
	</tr>
	...
	<tr>
		...
		<td class="L">
			<input Title="Anno Re.Ge. GIP" value="<%=StringUtils.toStringJSP( lSentenza.getAnnoRegeGip()) %>" type="text" name="<%= ICostantiSentenza.CAMPO_ANNO_REGE_GIP %>" maxlength="4" size="4"> /
    ...
    <tr>
      <td class="l">Data Emissione Provvedimento <font class="ob">(*)</font></td>
         <td class="L" colspan=3>
			...
			<a href="javascript:calendario('LoadInserisciSentenzaDecretoArchiviazione','<%=ICostantiSentenza.CAMPO_ANNO_DATA_PROVVEDIMENTO%>','<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>','<%=ICostantiSentenza.CAMPO_GIORNO_DATA_PROVVEDIMENTO%>');">
      ...
      <td class="L"  colspan=3>
         <input Title="Luogo Emittente" name="<%=ICostantiSentenza.CAMPO_COD_LUOGO_EMITTENTE%>"
            value="<%=lSentenza.getDescrLuogoEmittente()%>" type="text" maxlength="35" size="35">
 		      <a href="Javascript:ListaUfficiPerTipo('LoadInserisciSentenzaDecretoArchiviazione','<%=ICostantiSentenza.CAMPO_COD_LUOGO_EMITTENTE%>',document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_COD_TIPO_AUTORITA_EMITTENTE %>[document.LoadInserisciSentenzaDecretoArchiviazione.<%=ICostantiSentenza.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>.selectedIndex].value);">
...
<table cellspacing=2 cellpadding=2>	
	<tr>
		<td colspan=2>
			...
			<INPUT class="bottone" type="submit" name="INSERISCI" value="Conferma" onclick="Javascript: return VerifyDECRETO();">
      	</td>
	</tr>
</table>
	...
	<input type="HIDDEN" name="<%=ICostantiFasSigeSentenza.RADIO_TIPO_PROVVEDIMENTO%>" value="63">
</form>

<script language="JavaScript" type="text/javascript">
  	var frmvalidator  = new Validator("LoadInserisciSentenzaDecretoArchiviazione");
	...
	frmvalidator.addValidation("<%= ICostantiSentenza.CAMPO_ANNO_PROVVEDIMENTO %>","gt=1900");
  	...
  	// segnalazioni 4: aggiunti controlli
    ...
    frmvalidator.addValidation("<%= ICostantiSentenza.CAMPO_ANNO_REGE_GIP%>","numeric");
    frmvalidator.addValidation("<%= ICostantiSentenza.CAMPO_ANNO_REGE_GIP%>","gt=1900");
    frmvalidator.addValidation("<%= ICostantiSentenza.CAMPO_ANNO_REGE_GIP%>","lt=2999");
    frmvalidator.addValidation("<%= ICostantiSentenza.CAMPO_NUMERO_REGE_GIP%>","numeric");

  	// segnalazioni 4: modificato il nome funzione
  	frmvalidator.setAddnlValidationFunction("VerifyDECRETO");
</script>