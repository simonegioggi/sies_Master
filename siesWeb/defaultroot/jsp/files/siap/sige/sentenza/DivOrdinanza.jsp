...
<jsp:useBean id="autoritaEmiOrdinanza" scope="request" class="java.lang.String"/>
...
<jsp:useBean id="listaTipoProvvOrdinanza" scope="request" class="java.lang.String"/>

<!-- segnalazioni 4: modificato il titolo -->
<title>[S.I.E.S.] - Inserimento Ordinanza</title>
<link rel="STYLESHEET" type="text/css" href="<%=IWebConstants.PG_STYLE%>">
<script language="JavaScript" src="<%=IWebConstants.JS_DATE_CONTROL%>"></script>

<script language="JavaScript">
	function loadUfficiAccorpati(chiamataDa,competente){	  
		if (chiamataDa=='elencoOrdinanzeNelDistretto') {
			var objsel = document.getElementById("autoritaEmittente");  	
			document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>.value=competente;  
		}
	}

	// segnalazioni 4: modificato il nome funzione
	function VerifyORDINANZA() {
		if (document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_PROVVEDIMENTO%>.value.length==1)
			document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_PROVVEDIMENTO%>.value='0'+document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_PROVVEDIMENTO%>.value;
		if (document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value.length==1)
			document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value='0'+document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value;
		if (document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value.length==1)
			document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value='0'+document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value;
		// segnalazioni 4: aggiunti controlli preventivi
		if (document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_IRREVOCABILITA%> &&
				document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_IRREVOCABILITA%>.value.length==1)
			document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_IRREVOCABILITA%>.value='0'+document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_IRREVOCABILITA%>.value;
		if (document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_MESE_DATA_IRREVOCABILITA%> &&
				document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_MESE_DATA_IRREVOCABILITA%>.value.length==1)
			document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_MESE_DATA_IRREVOCABILITA%>.value='0'+document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_MESE_DATA_IRREVOCABILITA%>.value;

        //Data Emissione Provvedimento
        var d2=document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_GIORNO_DATA_PROVVEDIMENTO%>.value+'/'+document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>.value+'/'+document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_ANNO_DATA_PROVVEDIMENTO%>.value;
        if (! ControllaData(d2))
        {
...
%>
  ...
  <FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name="LoadInserisciSentenzaOrdinanza">
  <table cellspacing=2 cellpadding=2>
    ...
    <tr>	
      ...
      <a href="Javascript:ListaOrdinanzeNelDistretto('LoadInserisciSentenzaOrdinanza','<%=ICostantiSentenza.CAMPO_COD_LUOGO_EMITTENTE%>','<%= ICostantiSentenza.CAMPO_ANNO_PROVVEDIMENTO %>','<%= ICostantiSentenza.CAMPO_NUMERO_PROVVEDIMENTO %>','<%= ICostantiSentenza.CAMPO_GIORNO_DATA_PROVVEDIMENTO %>','<%= ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO %>','<%= ICostantiSentenza.CAMPO_ANNO_DATA_PROVVEDIMENTO %>',document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>[document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>.selectedIndex].value);">
		 Elenco Ordinanze nel Distretto 
		</a>
	  </td>
	</tr>
    <tr>
      <td class="l">Data Emissione Provvedimento <font class="ob">(*)</font></td>
         <td class="L" colspan=3>
			...
			<a href="javascript:calendario('LoadInserisciSentenzaOrdinanza','<%=ICostantiSentenza.CAMPO_ANNO_DATA_PROVVEDIMENTO%>','<%=ICostantiSentenza.CAMPO_MESE_DATA_PROVVEDIMENTO%>','<%=ICostantiSentenza.CAMPO_GIORNO_DATA_PROVVEDIMENTO%>');">
    ...
    <tr>
      <td class="l">Anno/Numero SIGE<font class="ob">(*)</font></td>
      <td class="L">
      ...
      <td class="L"  colspan=3>
         <input Title="Luogo Emittente" name="<%=ICostantiSentenza.CAMPO_COD_LUOGO_EMITTENTE%>"
            value="<%=lSentenza.getDescrLuogoEmittente()%>" type="text" maxlength="35" size="35">
 		      <a href="Javascript:ListaUfficiPerTipo('LoadInserisciSentenzaOrdinanza','<%=ICostantiSentenza.CAMPO_COD_LUOGO_EMITTENTE%>',document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_COD_TIPO_AUTORITA_EMITTENTE %>[document.LoadInserisciSentenzaOrdinanza.<%=ICostantiSentenza.CAMPO_COD_TIPO_AUTORITA_EMITTENTE%>.selectedIndex].value);">
...
<table cellspacing=2 cellpadding=2>	
	<tr>
		<td colspan=2>
        	...
        	<INPUT class="bottone" type="submit" name="INSERISCI" value="Conferma" onclick="Javascript: return VerifyORDINANZA();">
      	</td>
	</tr>
</table>
	...
	<input type="HIDDEN" name="<%=ICostantiFasSigeSentenza.RADIO_TIPO_PROVVEDIMENTO%>" value="03">
</form>

<script language="JavaScript" type="text/javascript">
	var frmvalidator  = new Validator("LoadInserisciSentenzaOrdinanza");
...
<%-- 	frmvalidator.addValidation("<%= ICostantiSentenza.CAMPO_COD_LUOGO_EMITTENTE%>","alphabetic"); --%>
	...
	frmvalidator.addValidation("<%= ICostantiSentenza.CAMPO_ANNO_PROVVEDIMENTO %>","req","L'Anno SIGE è obbligatorio");
	...
	frmvalidator.addValidation("<%= ICostantiSentenza.CAMPO_NUMERO_PROVVEDIMENTO %>","req","Il Numero SIGE è obbligatorio");
	frmvalidator.addValidation("<%= ICostantiSentenza.CAMPO_NUMERO_PROVVEDIMENTO %>","alfanumeric");

	// segnalazioni 4: modificato il nome funzione
  	frmvalidator.setAddnlValidationFunction("VerifyORDINANZA");
</script>