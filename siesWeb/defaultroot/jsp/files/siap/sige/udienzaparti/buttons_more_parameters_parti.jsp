...
%>
              <td>
                <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%><%=retParam%><%=totParam%>">
                  <img src="/images/modifica.gif" alt="Modifica Parte" width="12" height="12" border="0">
                </a>
              </td>
<%
            }
...
<%
            ...
            if(lFun.getFunctionType().equals(ICostantiFunzioni.TIPO_MODIFICA_DIFENSORE) && (lModificabile.equals("SI") && lAnnullato.equals("N")))
            {
%>
              <td>
                <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%><%=retParam%><%=totParam%>">
                  <img src="/images/modificaDif.gif" alt="Modifica Difensore e Convocazione Parte" width="12" height="12" border="0">
                </a>
              </td>
<%
            }
