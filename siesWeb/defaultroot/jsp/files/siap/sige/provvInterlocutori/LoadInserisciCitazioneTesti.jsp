...
<html>
  <head>
    <title>[S.I.E.S.] - Citazione Testi </title>
...
<%-- 		<% if (defTipoGiudizio) { %> --%>
<%-- 		Visualizza(document.LoadCitazioneTesti.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value); --%>
<%-- 		<%} %> --%>
        	Verifica();
        }
    ...
    function Verify() {
      ...
      var data_emissione = document.LoadCitazioneTesti.<%=ICostantiProvvedimentoSige.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadCitazioneTesti.<%=ICostantiProvvedimentoSige.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadCitazioneTesti.<%=ICostantiProvvedimentoSige.CAMPO_ANNO_DATA_EMISSIONE%>.value;
...
<%	if (defTipoGiudizio) { %>
     //Controllo obbligatorietà Collegio in caso di Tipo Giudizio impostato.
    	if ( document.LoadCitazioneTesti.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value != "-" )
    	{
    	if (document.LoadCitazioneTesti.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value == "C"){
  	...
  	<%}%>
		...
		{
			var ritorno = true;
			var collegio = document.LoadCitazioneTesti.<%=ICostantiCollegio.CAMPO_ID_COLLEGIO%>.value;
			if ( collegio == '' ) 
	    {
    ...
    if ( document.LoadCitazioneTesti.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value == "-"
      || document.LoadCitazioneTesti.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value == "" )
    {
      alert('Inserire il Tipo Rito');
      return false;
    }
		
		if (document.LoadCitazioneTesti.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
			  document.LoadCitazioneTesti.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadCitazioneTesti.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
		  if (document.LoadCitazioneTesti.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
			  document.LoadCitazioneTesti.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadCitazioneTesti.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value;

		  var data_to_verify = document.LoadCitazioneTesti.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadCitazioneTesti.<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadCitazioneTesti.<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>.value;
  ...
  function ListaUDS(a_formname,a_fieldname)
  {
    ...
    function ListaComuni(a_formname,a_fieldname)
    {
     desktop = window.open("/jsp/Main.jsp?<%=IWebConstants.ACTION_FIELD%>=siap.sico.decodifiche.action.ActLoadRicercaComune&formname="+a_formname+"&fieldname="+a_fieldname, "Ricerca_Comune","toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=300,height=500");
      ...
      {
        var a_codnum = document.LoadCitazioneTesti.<%=ICostantiMagistrato.CAMPO_COD_MAGISTRATO_VECCHIO %>.value;
    ...
    //   
  	function InserisciUdienza( tipoRito ){
  		var dataUdienza= document.LoadCitazioneTesti.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA%>.value +'/'+
                         document.LoadCitazioneTesti.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA%>.value +'/'+
                         document.LoadCitazioneTesti.<%=ICostantiUdienzaSige.CAMPO_ANNO_DATA_UDIENZA%>.value;
      	var lLink;
  		if( tipoRito == 'M' ){
  			...
  			var idUdienza=document.LoadCitazioneTesti.<%=ICostantiUdienzaSige.CAMPO_ID_UDIENZA_SIGE %>.value;
	...
	function setUdienza (idUdienza, dataUdienza, idCollegio, idSezione, codMagistrato) {
	 	if (dataUdienza != "null") {
		     ...
		     document.LoadCitazioneTesti.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA %>.value=dataSplitted[0];
		     document.LoadCitazioneTesti.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA %>.value=dataSplitted[1];
		     document.LoadCitazioneTesti.<%=ICostantiUdienzaSige.CAMPO_ANNO_DATA_UDIENZA %>.value=dataSplitted[2];
		     document.LoadCitazioneTesti.<%=ICostantiUdienzaSige.CAMPO_ID_UDIENZA_SIGE %>.value=idUdienza;
		     document.LoadCitazioneTesti.<%=ICostantiCollegio.CAMPO_ID_COLLEGIO%>.value=idCollegio;
			if (codMagistrato != null) {
				document.LoadCitazioneTesti.<%=ICostantiMagistrato.CAMPO_COD_MAGISTRATO%>.value = codMagistrato;
	 	} 
			// 20190519 [SG]: aggiunta gestione idUdienzaSige
			if (idUdienza != null) {
				document.LoadCitazioneTesti.idUdiSig.value = idUdienza;
	 }
		}
	}
	...
	function gestioneOggetti() {
    	var lLink;
		var idUdienza = document.LoadCitazioneTesti.idUdiSig.value;
 ...
 <body class="corpo" onLoad="Init();" >
   <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
      <font class="label">Funzione :</font> &nbsp;&nbsp;<font class="campo">Citazione Testi</font>
<%
        String lAction = new String();
        lAction = "siap.sige.provvInterlocutori.action.ActInserisciCitazioneTesti";
%>
      <%
        ...
        if( modalita.equals("I") )
         {
          ...
          lAzione = "siap.sige.provvInterlocutori.action.ActInserisciCitazioneTesti";
      %>
          <font class="campo">Citazione Testi</font>
      <%}
        else if( modalita.equals("M") )
        {
          lProvvedimento = new EventoModel(evento);
          lAzione = "siap.sige.provvInterlocutori.action.ActInserisciCitazioneTesti";
      %>  <font class="campo">Modifica Citazione Testi</font>
      <%}
      %>
      </td>
    </tr>
 ...
 <br>
  ...
  <FORM method="POST" name="LoadCitazioneTesti" action="<%= IWebConstants.PG_MAIN%>">
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sige.provvInterlocutori.action.ActInserisciCitazioneTesti">
   ...
   <jsp:include page="<%=ICostantiUdienzaCollegiale.PG_INCLUDE_INSERISCI_COLLEGIO%>">
          <jsp:param name="form_name" value="LoadCitazioneTesti"/>
          <jsp:param name="tipo_ufficio" value="<%=tipoUfficioUtente%>"/>
     </jsp:include>
    --%>  
    
    <%-- Modifica del 08/03/2017 --%>
    <jsp:include page="<%=ICostantiProvvedimentoSige.INC_TIPO_GIUDIZIO_COLLEGIO_CITAZIONE_TESTI%>" />
...
<table>
	...
	<tr>
		...
		<a href="javascript:calendario('LoadCitazioneTesti','<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>','<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>','<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>');">
    ...
    <tr>
      ...
      <td class="l">
           <input Title="Sede Procura" name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
              ...
              <a href="Javascript:ListaUffici('LoadCitazioneTesti','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[<%=num_sede%>]');">
    ...
    <tr>
      ...
      <td class="l">
         <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
            ...
            <a href="Javascript:ListaComuni('LoadCitazioneTesti','<%=ICostantiRichiestaAtti.CAMPO_SEDE%><% if (num_sede > 0) { %> [<%=num_sede%>]<%}%>');">
            <img src="/images/filefolder.gif" border=0> </a>
      </td>
    </tr>
    <tr>
...
<script language="JavaScript" type="text/javascript">
  var frmvalidator  = new Validator("LoadCitazioneTesti");
