...
<html>
	<head>
    	<title>[S.I.E.S.] - Decreto Irreperibilità </title>
	...
	<script language="JavaScript">
		...
		function Verify() {
	        ...
	        var data_sistema='<%=DateUtils.getSysDate("dd/MM/yyyy")%>';
      		...
      		if (document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length == 1)
				document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value = '0' + document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
			if (document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length == 1)
				document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value = '0' + document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;
			var data_to_verify = document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value +
				'/' + document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value +
				'/' + document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
			if (!ControllaData(data_to_verify)) {
  		       	alert('Data di emissione non valida');
				return false;
			}
      		var data_emissione = document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value +
      			'/' + document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value +
      			'/' + document.LoadDecretoIrreperibilita.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
...
//       		}
			<% if (defTipoGiudizio) { %>
    			...
    			if (document.LoadDecretoIrreperibilita.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value != "-" &&
    					document.LoadDecretoIrreperibilita.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value == "C"){
  ...
  <body class="corpo" onLoad="Init();" >
   <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
      <font class="label">Funzione :</font> &nbsp;&nbsp;<font class="campo">Modifica Decreto Irreperibilità</font>
<%
...
%>
      <%
	   EventoModel lProvvedimento = new EventoModel();
          ...
          lAction = "siap.sige.provvInterlocutori.action.ActModificaLatitanzaIrreperibilita";
      %>
 ...
 <br>
  ...
  <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sige.provvInterlocutori.action.ActModificaLatitanzaIrreperibilita">
 	 ...
 	 <table style="width: 95%;">
     	<tr> 
    		...
    		<td class="L" colspan=3>
    			<TEXTAREA title="Note" name="<%= ICostantiIstruttoria.CAMPO_NOTE %>"  cols=80 rows=2 ><%=StringUtils.toStringJSP(ProvvedimentoEvento.getProvvedimento().getNote())%></textarea>
    		</td>
    	</tr>
