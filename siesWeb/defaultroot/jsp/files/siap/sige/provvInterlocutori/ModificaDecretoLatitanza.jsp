    	...
    	function Verify() {
	        ...
	        var data_sistema='<%=DateUtils.getSysDate("dd/MM/yyyy")%>';
	     	// Controllo della data emissione.
	   		if (document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length == 1)
	   			document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value = '0' +
	   				document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;
			if (document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length == 1)
				document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value = '0' +
					document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;
			var data_to_verify = document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value +
				'/' + document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value +
				'/' + document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
			if (!ControllaData(data_to_verify)) {
				alert('Data di emissione non valida');
	    		return false;
	   		}	
		   	var data_emissione = document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value +
		   		'/'	+ document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value +
		   		'/' + document.LoadDecretoLatitanza.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
			...
			<% if (defTipoGiudizio) { %>
		    	...
		    	if (document.LoadDecretoLatitanza.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value != "-" &&
		    			document.LoadDecretoLatitanza.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value == "C"){
  ...
  <body class="corpo" onLoad="Init();" >
   <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
      <font class="label">Funzione :</font> &nbsp;&nbsp;<font class="campo">Modifica Decreto Latitanza</font>
<%
