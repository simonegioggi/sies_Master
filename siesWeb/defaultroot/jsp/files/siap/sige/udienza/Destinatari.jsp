...
<table cellspacing=2 cellpadding=2 width="95%">
	...
	<tr>
		...
		%>
		<td class="l">
			<font class="campo"> <%=notifica.getUfficio().getDescrTipoUfficio()%>
			</font>&nbsp;di&nbsp; <font class="campo"><%=notifica.getUfficio().getDescrComune()%> </font>
		</td>
		<%
		...
		<td class="l">
			...
			<%	if (notifica.getNote() != null) {%> <font class="campo">-&nbsp;&nbsp;<%=notifica.getNote()%></font> <%}%>
		</td>
		<%
			} else if (notifica.getIstitutoDetenzione() != null) {
					// Istituto Detenzione
		%>
		<td class="l"><font class="campo"><%=notifica.getIstitutoDetenzione().getDescrTipoIstituto()%> </font> &nbsp;di&nbsp; <font class="campo"><%=notifica.getIstitutoDetenzione().getDescrComune()%></font></td>
		<%
			}
		%>
		...
		<%
				...
				// Avvocato SIGE
				if (notifica.getAvvSige() != null && notifica.getAvvSige().getAvvocato() != null) {
					if (notifica.getAutoritaEsterna() == null) {
						// notifica telematica
		%>
		<td class="l"><font class="campo">S.N.T. (Sistema Notifiche Telematiche)</font></td>
		<%
					}
		%>
		<td class="l">Per la notifica all' Avvocato &nbsp;<%=notifica.getAvvSige().getAvvocato().getDescrTipo()%>&nbsp;:&nbsp; <font class="campo"> <%=StringUtils.toStringJSP(notifica.getAvvSige().getAvvocato().getCognome() + " " + notifica.getAvvSige().getAvvocato().getNome())%></font>&nbsp;</td>
		<%
				}
		...
		%>
		<td class="l" colspan="2"><font class="campo">UEPE &nbsp;</font> <font class="campo"><%=StringUtils.toStringJSP(notifica.getCSSA().getIndirizzo() + " " + notifica.getCSSA().getComune())%></font>&nbsp;</td>
		<%
				}
			}
		%>
