...
<jsp:useBean id="newIdUdienzaSige"    scope="request" class="java.lang.String"/>
<%-- 20171018: [EC] aggiunto useBean --%>
...
<html>
<head>
  <title>[S.I.E.S.] - Gestione Ordinanza Rinvio Udienza </title>
  ...
  <script language="JavaScript">
	...
	function controllaRuolo()
	{
	 	...
	 	var a='<%=flagRinvio %>';
	 	if(a == 'N'){
	 		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaProcedimentoSige.CAMPO_CHECK_NUOVO_RUOLO%>.checked = true;	 		
	 	}
	 	else{
	 		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaProcedimentoSige.CAMPO_CHECK_NUOVO_RUOLO%>.checked = false;
	 	}
	}
  	...
  	//
		...
		{
	  	if ( document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value == "-" )
	  	{
	  		alert('Selezionare il tipo rito!');
	  		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.focus();
	    	return false;
	  	}
	  	return true;
		}
  	...
  	//
		...
		function controlloLuogoUdienza()
		{
			...
			if( !document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaProcedimentoSige.CAMPO_CHECK_NUOVO_RUOLO%>.checked )
			{	
	    	if ( document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiProvvedimentoSige.CAMPO_LUOGO_SVOLGIMENTO%>.value == "-"
		  		|| document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiProvvedimentoSige.CAMPO_LUOGO_SVOLGIMENTO%>.value == "" )
		  	{
		  		alert('Inserire un Luogo svolgimento udienza!');
		  		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiProvvedimentoSige.CAMPO_LUOGO_SVOLGIMENTO%>.focus();
		    	return false;
		  	}
			}
		  return true;
		...
		function controlloDataEmissione()
		{
	    	...
	    	var dataEmissione = 
	    		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.value +'/'+ 
		    	...
		    	if (!ControllaData(dataEmissione))
		      	{
				  	...
				  	document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>.focus();
		        	ritorno = false;
		      	}
		...
		function controlloDataUdienza()
		{
		  ...
		  var dataUdienzaFissata = '<%=lDataUdienza%>';
	    var dataUdienza = 
	    		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA%>.value +'/'+ 
	    		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA%>.value +'/'+ 
	    		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_ANNO_DATA_UDIENZA%>.value;
	    ...
	    if (dataUdienza.length > 2)
	    {
	    	if (!ControllaData(dataUdienza))
	      {
	      	alert('Data udienza non corretta!');
			  	document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA%>.focus();
	        ritorno = false;
	      }
	    }  
	    else if( !document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaProcedimentoSige.CAMPO_CHECK_NUOVO_RUOLO%>.checked ) 
	    {
	    	...
	    	document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA%>.focus();
	     	ritorno = false;
	    	    	 ...
	    	    	 document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaProcedimentoSige.CAMPO_CHECK_NUOVO_RUOLO%>.checked ) 
	    {
  	...
  	function Verify()
  	{
  	  ...
  	  // controlloMagistrato.	
			...
			if( !controlloTipoRito() )
	      	...
	      	// 20171018: [EC] aggiunto controllo che per verificare che per i riti collegiali sia stata inserito un'udienza con il relativo collegio 
	      	
	      	if (!document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaProcedimentoSige.CAMPO_CHECK_NUOVO_RUOLO%>.checked
	      			&& document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_ID_UDIENZA_SIGE%>.value == ""
	      			&& dataUdienza != "//"
	      			&& document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value == 'C'
	      			&& <%=ufficioConSezioni%>) {	
				alert("Attenzione prima di confermare occorre inserire l'udienza con il collegio da apposita funzione (Inserimento Udienza - Visualizza)");
				return false;
		    }
...
/* 			if( !controlloLuogoUdienza() )
  ...
  <script language="JavaScript">
	...
	function InserisciUdienza(dataUdienza, idSezione) {		
	    var tipoRito=document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiFascicoloSige.CAMPO_COD_TIPO_GIUDIZIO%>.value;	
	    var lLink;
	    if( tipoRito == 'M' ){	 
  ...
  <script language="JavaScript">
    ...
    function CheckUncheck()
    {
      if (document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaProcedimentoSige.CAMPO_CHECK_NUOVO_RUOLO%>.checked )
      {
        if (document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA%> !=null)
          document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA%>.value = "";
        if (document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA%> !=null)
          document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA%>.value = "";
        if (document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_ANNO_DATA_UDIENZA%> !=null)
          document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_ANNO_DATA_UDIENZA%>.value = "";
        if (document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiCollegio.CAMPO_ID_COLLEGIO %> !=null)
          document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiCollegio.CAMPO_ID_COLLEGIO%>.value = "";
      }
    }

    function Uncheck()
    {
      document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaProcedimentoSige.CAMPO_CHECK_NUOVO_RUOLO%>.value = "1";
      document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaProcedimentoSige.CAMPO_CHECK_NUOVO_RUOLO%>.checked = false;
    }
    ...
    function loadAula (aIdUdienza) {
    	...
    	$.ajax({
    	    ...
    	    success: function(data){ 
    	    	...
    	    	document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_COD_SEZIONE_UDIENZA%>.value=obj.idSezione;
    	    },
    	    error: function(data) {
    	        // alert('ERROR @@@@');
    	    }
    ...
    function ListaAule() {
		...
		idSezione=document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_COD_SEZIONE_UDIENZA%>.value;
    	if ( idSezione == "" || idSezione == "-" ) {
			alert('Selezionare una sezione!');
	    } else {
			desktop = window.open("<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.sige.aula.action.ActRicercaAula&<%=IWebConstants.POPUP_PAGE%>=yes&formname=LoadInserisciOrdinanzaRinvioUdienza&<%=ICostantiAula.CAMPO_ID_SEZIONE%>="+idSezione, "Ricerca_Aule", "toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=500,height=500");
	    }
	}
    ...
    function VerificaChiamataInserisciUdienza(){
	    ...
	    if (document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA%>.value.length==1)
	        document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA%>.value='0'+document.LoadInserisciFissazioneUdienza.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA%>.value;

	    if (document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA%>.value.length==1)
	        document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA%>.value='0'+document.LoadInserisciFissazioneUdienza.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA%>.value;
    	
	    var dataUdienza= document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA%>.value +'/'+
	                     document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA%>.value +'/'+
	                     document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_ANNO_DATA_UDIENZA%>.value;
	                     
   	    // [EC] 20171019 : recuero la sezione e se specificata la passo alla funzione InserisciUdienza
   	    var idSezione = '';
   	    if(document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_COD_SEZIONE_UDIENZA%>.value != '-'){
   	    	idSezione = document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_COD_SEZIONE_UDIENZA%>.value;
   	    }	  
    ...
    function setUdienza (idUdienza, dataUdienza, idCollegio, idSezione, codMagistrato) {
    	document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_ID_UDIENZA_SIGE %>.value=idUdienza;
    	...
    	document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_GIORNO_DATA_UDIENZA %>.value=dataSplitted[0];
    	document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_MESE_DATA_UDIENZA %>.value=dataSplitted[1];
    	document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_ANNO_DATA_UDIENZA %>.value=dataSplitted[2];    	 
		}
		// AGGIUNGO PER INTERVENTO PER PROBLEMATICA ORDINANZE DI RINVIO UDIENZE SU ESERCIZION 11.2.1
		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiCollegio.CAMPO_ID_COLLEGIO%>.value = idCollegio;
		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_ID_UDIENZA_SIGE%>.value = idUdienza;
    	if (idSezione != null) {
    		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiUdienzaSige.CAMPO_COD_SEZIONE_UDIENZA%>.value = idSezione;
    		// svuoto i campi aula, ingresso e piano
    		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiAula.CAMPO_DESCRIZIONE_AULA%>.value = "";
    		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiAula.CAMPO_DESCRIZIONE_INGRESSO%>.value = "";
    		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiAula.CAMPO_NUMERO_PIANO%>.value = "";
    	}
    	<%-- intervento per 11.2.1 --%>
    	if (codMagistrato != null) {
    		document.LoadInserisciOrdinanzaRinvioUdienza.<%=ICostantiMagistrato.CAMPO_COD_MAGISTRATO%>.value = codMagistrato;    		
    	}
    	// 20190519 [SG]: aggiunta gestione idUdienzaSige
		if (idUdienza != null) {
			document.LoadInserisciOrdinanzaRinvioUdienza.idUdiSig.value = idUdienza;
    }
    }
	...
	function gestioneOggetti() {
    	var lLink;
		var idUdienza = document.LoadInserisciOrdinanzaRinvioUdienza.idUdiSig.value;
...
<%
    			String lAzione = new String();
					...
					if( UdienzaPreFissata.getIdUdienzaSige() != null  )
  					...
  					lUdienza = UdienzaSige;
    			
					lAzione = "siap.sige.udienzaprocedimento.action.ActInserisciOrdinanzaRinvioUdienza";
%>
       	<font class="campo">Inserimento Ordinanza Rinvio Udienza</font>
      </td>
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" 
				name="LoadInserisciOrdinanzaRinvioUdienza">
				
	<input type="hidden"  id="<%=ICostantiCollegio.CAMPO_ID_COLLEGIO%>" name="<%=ICostantiCollegio.CAMPO_ID_COLLEGIO%>" />
	<input type="hidden"  name="newIdUdienzaSige" value="<%=newIdUdienzaSige%>" />			
	  <table cellspacing="2" cellpadding="2" width="100%">
  	  <tr>
	  	  <jsp:include page="<%=ICostantiMagistratoAssegnatario.PG_SINTESIMAGISTRATOASSEGNATARIO%>">
      		<jsp:param name="MagAssRitorno" value="siap.sige.udienzaprocedimento.action.ActLoadInserisciOrdinanzaRinvioUdienza"/>
    		</jsp:include>
    	</tr>
    	
      <jsp:include page="<%=ICostantiAvvocatoFascicoloSige.PG_INCLUDE_AVVOCATI%>">
      	<jsp:param name="AvvRitorno" value="siap.sige.udienzaprocedimento.action.ActLoadInserisciOrdinanzaRinvioUdienza"/>
      </jsp:include>
  	</table>
	<br>
  ...
  <table cellspacing=2 cellpadding=2 width="100%">
    ...
    <tr>
      ...
      <td class="L">
        ...
        <input value="" type="text" size="4" 
			...
			<a href="javascript:calendario('LoadInserisciOrdinanzaRinvioUdienza','<%=ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE%>','<%=ICostantiEvento.CAMPO_MESE_DATA_EMISSIONE%>','<%=ICostantiEvento.CAMPO_GIORNO_DATA_EMISSIONE%>');">
    ...
    <tr>
  		<td class="L">
  		  <table cellspacing=1 cellpadding=1  width="100%" style="border: 0;">
  			<tr>
...
<table>
    ...
    <input type="HIDDEN" value="<%=StringUtils.toStringJSP( lUdienza.getIdUdienzaSige(), "" )%>" name="<%= ICostantiUdienzaSige.CAMPO_ID_UDIENZA_SIGE %>"  >
...
%>
    <input type="HIDDEN" value="<%=StringUtils.toStringJSP( request.getAttribute(ICostantiUdienzaProcedimentoSige.CAMPO_ID_UDIENZA_PROCEDIMENTO_SIGE), "0" )%>" name="<%=ICostantiUdienzaProcedimentoSige.CAMPO_ID_UDIENZA_PROCEDIMENTO_SIGE%>"  >
<% } %>
