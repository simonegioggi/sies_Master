...
<html>
  <head>
    ...
    <title>[S.I.E.S.] - ListaProcedimentixUdienzaTuttiCollegi</title>
  ...
  <table>
    <tr>
      ...
      <td class="LBG">
      	...
      	<font class="campo">Elenco Procedimenti Fissati all'Udienza del <%=DateUtils.getDateToString(data_udi,"dd-MM-yyyy")%> </font></td>
     	</td>
     ...
     <td class="LBG" nowrap>
       <jsp:include page="<%=ICostantiUdienzaProcedimentoSige.PG_COMBOTEMPLATE_RUOLOUDIENZA%>">
        ...
        <jsp:param name="ValoreIdEntita" value="<%=DateUtils.getDateToString(data_udi,"yyyyMMdd")%>" />
...
%>
    <tr>
     <td class=l><%=i%></td>
     <td class=l><%=procedimento.getNumCollegio()%></td>
     <td class="l">
...
%>       
					<td class=l>
						<%=Utils.arrayToString(procedimento.getDescrOggettiProcedimento(),"<br>")%>
					</td>
					<td class=l>
						<%=procedimento.getEsitoProvvedimento()%>
					</td>
...
%>
    <td class=l>
       <a href="Javascript:stampa2('<%=ISIAPCostantiWeb.PG_STAMPA%>',
																	 '<%=IWebConstants.ACTION_FIELD%>=siap.sige.udienzaprocedimento.dettaglioruolo.action.ActStampaFlashVerbaleUdienza&<%=ICostantiFascicoloSige.CAMPO_ID_FASCICOLO_SIGE%>=<%=procedimento.getIdFasSIGE()%>&<%=ICostantiUdienzaProcedimentoSige.CAMPO_UDI_ID_UDIENZA_SIGE%>=<%=procedimento.getIdUdienza()%>');">
