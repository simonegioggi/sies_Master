    ...
    <script language="JavaScript">
      ...
      function initDataIni(data)
      {
        DataIni = data;
			...
			function checkObblGiudiciPopolari()
			{
				if( document.LoadInserisciCollegio.<%=ICostantiGiudicePopolare.CAMPO_ID_GIUDICE_POPOLARE%>[0].value.length == 0 )
				{
					alert("Occorre selezionare il Giudice Popolare.");
					return false;
				}			
				return true;
			}			
...
<% 
			for( int i=0; i<=ICostantiCollegio.NUM_GIUDICI_POPOLARI; i++ ){
%>
				<tr>
    			<td class="l">Giudice Popolare</td>
       		<td class="L">
        		<input title="Cognome" readonly type="text" 
								 	 name="<%=ICostantiGiudicePopolare.CAMPO_COGNOME_GIU_POP%>" maxlength="35" size="25"
								 	 value="<%=CollegioUtils.isValidSize( collegio.getCollegioGiudiciPopolari(), i) ? 
								 	     collegio.getCollegioGiudiciPopolari()[i].getGiudicePopolare().getCognome() : "" %>">
         		<input title="Nome" readonly type="text" 
								 	 name="<%=ICostantiGiudicePopolare.CAMPO_NOME_GIU_POP%>" maxlength="35" size="25"
								 	 value="<%=CollegioUtils.isValidSize( collegio.getCollegioGiudiciPopolari(), i) ? 
								 	     collegio.getCollegioGiudiciPopolari()[i].getGiudicePopolare().getNome() : "" %>">
						<input title="IdGiudicePopolare" type="hidden" 
								 	 name="<%=ICostantiGiudicePopolare.CAMPO_ID_GIUDICE_POPOLARE%>"
								 	 value="<%=CollegioUtils.isValidSize( collegio.getCollegioGiudiciPopolari(), i) ? 
								 	     collegio.getCollegioGiudiciPopolari()[i].getGiudicePopolare().getIdGiudicePopolare().toString() : "" %>">
         		<a href="Javascript:ListaGiudiciPopolari('LoadInserisciCollegio','<%=i%>');">
           		<img src="/images/filefolder.gif" border=0>
         		</a>
       		</td>
    		</tr>
