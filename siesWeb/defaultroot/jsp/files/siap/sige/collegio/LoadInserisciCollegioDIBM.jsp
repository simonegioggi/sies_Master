  ...
  <head>
    ...
    <script language="JavaScript">
		...
		%>
			...
			// Elenco popup degli Esperti
			
			function ListaEsperti(a_formname, a_idfieldnum)
			{
			 var desktop;
			 desktop = window.open("/jsp/Main.jsp?<%=IWebConstants.ACTION_FIELD%>=siap.sius.esperto.action.ActLoadRicercaEspertoLista&formname="+a_formname+"&idfieldnum="+a_idfieldnum, 
					 									 "Lista_Esperti",
					 									 "toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=500,height=500");
			} 
      ...
      function initDataIni(data)
      {
        DataIni = data;
			...
			// Controllo obbligatorietà dei giudici popolari 
			function checkObblEsperti()
			{
				if( document.LoadInserisciCollegio.<%=ICostantiEsperto.CAMPO_ID_ESPERTO%>[0].value.length == 0 )
				{
					alert("Occorre selezionare l' Esperto.");
					return false;
				}			
				return true;
			}			
...
%>
			...
			<tr>
				<td class="l">Esperto</font></td>
				<td class="L">
					<input title="Cognome" readonly type="text" 
								 name="<%=ICostantiEsperto.CAMPO_COGNOME%>" maxlength="35" size="25" 
								 value="<%=CollegioUtils.isValidSize( collegio.getCollegioEsperti(), 0) ? 
								     collegio.getCollegioEsperti()[0].getEsperto().getCognome() : "" %>">
					<input title="Nome" readonly type="text" 
								 name="<%=ICostantiEsperto.CAMPO_NOME%>" maxlength="35" size="25" 
								 value="<%=CollegioUtils.isValidSize( collegio.getCollegioEsperti(), 0) ? 
								     collegio.getCollegioEsperti()[0].getEsperto().getNome() : "" %>">
					<input title="CodEsperto" type="hidden" 
								 name="<%=ICostantiEsperto.CAMPO_ID_ESPERTO%>" 
								 value="<%=CollegioUtils.isValidSize( collegio.getCollegioEsperti(), 0) ? 
								     collegio.getCollegioEsperti()[0].getEsperto().getIdEsperto().toString() : "" %>">
					<a href="Javascript:ListaEsperti('LoadInserisciCollegio', '0');">
						<img src="/images/filefolder.gif" border=0>
					</a>
				</td>
			</tr>
			...
			<tr>
				...
				<td class="L">
					<input title="Cognome" readonly type="text" 
								 name="<%=ICostantiEsperto.CAMPO_COGNOME%>" maxlength="35" size="25" 
								 value="<%=CollegioUtils.isValidSize( collegio.getCollegioEsperti(), 1) ? 
								     collegio.getCollegioEsperti()[1].getEsperto().getCognome() : "" %>">
					<input title="Nome" readonly type="text" 
								 name="<%=ICostantiEsperto.CAMPO_NOME%>" maxlength="35" size="25" 
								 value="<%=CollegioUtils.isValidSize( collegio.getCollegioEsperti(), 1) ? 
								     collegio.getCollegioEsperti()[1].getEsperto().getNome() : "" %>">
					<input title="CodEsperto" type="hidden" 
								 name="<%=ICostantiEsperto.CAMPO_ID_ESPERTO%>" 
								 value="<%=CollegioUtils.isValidSize( collegio.getCollegioEsperti(), 1) ? 
								     collegio.getCollegioEsperti()[1].getEsperto().getIdEsperto().toString() : "" %>">
					<a href="Javascript:ListaEsperti('LoadInserisciCollegio', '1');">
						<img src="/images/filefolder.gif" border=0>
					</a>
				</td>
			</tr>
