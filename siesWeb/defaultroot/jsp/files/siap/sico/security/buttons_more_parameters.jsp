...
<%
  ...
  String Param3 = "";
  if (request.getParameter(ICostantiSecurity.CAMPO_ENTITA_CHIAVE_QUATTRO) != null && request.getParameter(ICostantiSecurity.CAMPO_ENTITA_CHIAVE_QUATTRO).length() > 0 )
  if (request.getParameter(ICostantiSecurity.CAMPO_VALORE_CHIAVE_QUATTRO) != null && request.getParameter(ICostantiSecurity.CAMPO_VALORE_CHIAVE_QUATTRO).length() > 0 )
      Param3 = "&" + request.getParameter(ICostantiSecurity.CAMPO_ENTITA_CHIAVE_QUATTRO) + "=" + request.getParameter(ICostantiSecurity.CAMPO_VALORE_CHIAVE_QUATTRO);
  ...
  if (request.getParameter(ICostantiSecurity.VALORE_ID_ENTITAPP) != null && request.getParameter(ICostantiSecurity.VALORE_ID_ENTITAPP).length() > 0 )
      Param4 = "&" + request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITAPP) + "=" + request.getParameter(ICostantiSecurity.VALORE_ID_ENTITAPP);
...
%>
            ...
            <%if(request.getParameter("ValoreAzioneChiamante")!=null && !request.getParameter("ValoreAzioneChiamante").equals(""))
              {%>
                <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%><%=retParam%>&<%=request.getParameter("CampoAzioneChiamante")%>=<%=request.getParameter("ValoreAzioneChiamante")%><%=Param2%><%=Param3%><%=Param4%>">
                  <img src="/images/dettagli.gif" width="12" height="12" alt="Dettagli" border="0">
                </a>
            <%}else
              {%>
                <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%><%=retParam%><%=Param2%><%=Param3%>">
                  <img src="/images/dettagli.gif" width="12" height="12" alt="Dettagli" border="0">
                </a>
