...
//ordinanza
      ...
      {
          if(window.parent.opener.document.<%=request.getParameter("formname")%>.<%=ICostantiEvento.CAMPO_COD_TIPO_PROVVEDIMENTO%>_AA.options[k].value=="03")
          {
...
// Giorni e periodi L.A.      
      if (giorniLA>0) {
        ...
        window.parent.opener.document.<%=request.getParameter("formname")%>.NumGiorniLibanticipataSorv.disabled=false;
      }
      else {
        window.parent.opener.document.getElementById("datiSorvLA").style.display = 'none';
        window.parent.opener.document.<%=request.getParameter("formname")%>.NumGiorniLibanticipataSorv.disabled=true;
      }
      ...
      if (giorniLA_spe>0) {
        ...
        window.parent.opener.document.<%=request.getParameter("formname")%>.NumGiorniLibanticipataSorv_SPE.disabled=false;
      }
      else {
        window.parent.opener.document.getElementById("datiSorvLA_SPE").style.display = 'none';
        window.parent.opener.document.<%=request.getParameter("formname")%>.NumGiorniLibanticipataSorv_SPE.disabled=true;
      }
      ...
      if (giorniLA_int>0) {
        ...
        window.parent.opener.document.<%=request.getParameter("formname")%>.NumGiorniLibanticipataSorv_INT.disabled=false;
      }
      else {
        window.parent.opener.document.getElementById("datiSorvLA_INT").style.display = 'none';
        window.parent.opener.document.<%=request.getParameter("formname")%>.NumGiorniLibanticipataSorv_INT.disabled=true;
      }
...
//  < --  
  ...
  LicenzaLibAnticipataModel AppolicenzeMod = null; 
    for (int i = 0; itx.hasNext(); i++)
    {
      ...
      LicenzaLibAnticipataModel licenzeMod = (LicenzaLibAnticipataModel)licPerMod.getLicenza();
      ...
      //siesLogger.debug("--XX-- Elemento "+i);
...
//  -- >  DL 146
    if(licenzeMod.getDescrStatoPermesso() != null)
    {
      ...
      {
    	 // [FT] - 05/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger()
    	 // siesLogger.debug("--XX-- LS Elemento "+i);
          for (int ii = 0; ii <lPeriodiMod.length; ii++)
          {
        	  if(lPeriodiMod[ii] != null && lPeriodiMod[ii].getIdPeriodoLibanticipata() != null)
        	  {	  
      ...
      {
    	 // [FT] - 05/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger()
    	 // siesLogger.debug("--XX-- LI Elemento "+i);
          for (int ii = 0; ii <lPeriodiMod.length; ii++)
          {
        	  if(lPeriodiMod[ii] != null && lPeriodiMod[ii].getIdPeriodoLibanticipata() != null)
        	  {	
      ...
      { 
    	 // [FT] - 05/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger()
    	 // siesLogger.debug("--XX-- LA Elemento "+i);
    	  for (int ii = 0; ii <lPeriodiMod.length; ii++)
          {
    		  if(lPeriodiMod[ii] != null && lPeriodiMod[ii].getIdPeriodoLibanticipata() != null)
        	  {	
...
//  < -- End    
    if (!lRipetizione )
      ...
      { 
        //if (i == licenze.size()-1) Appostrperiodi+=strperiodi;
        //if (i == licenze.size()-1) TotNumeroGiorni+=licenzeMod.getNumeroGiorni().intValue();
        strperiodiTot="";
        strperiodiTot_spe="";
        strperiodiTot_int="";
...
%>
        <tr>
          ...
          <td class="l"><%=StringUtils.toStringJSP(DateUtils.getDateToString(AppolicenzeMod.getDataEmissioneOrdinanza(),"dd-MM-yyyy"),"-")%></td>
