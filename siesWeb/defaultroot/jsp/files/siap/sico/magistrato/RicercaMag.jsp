...
function insertIT(cod,cognome,nome) {
	window.parent.opener.document.<%=formname%>.<%=fieldname%>.value=cod;
...
function checkListaComuni() {
	if ("<%=magistrati.size()%>" == 0)
  		alert('Attenzione! Nessun Magistrato trovato.');
	if ("<%=magistrati.size()%>" == 200)
    	alert('Attenzione! Visualizzati solo i primi 200 Magistrati individuati. Perfezionare la ricerca!');
}
</script>
<%
}
...
%>
	<tr>
        ...
        <td class="l"><%=StringUtils.toStringJSP(DateUtils.getDateToString(lMag.getDataNascita(), "dd-MM-yyyy"), "-")%></td>
<%
		if (!modalita.equals("NoPop")) {
%>
        <td class="c">
        	<a href="Javascript:insertIT('<%=StringUtils.cStrForJS(lMag.getCodMagistrato())%>','<%=StringUtils.cStrForJS(lMag.getCognome())%>','<%=StringUtils.cStrForJS(lMag.getNome())%>');">
