...
<jsp:useBean id="codProvinciaUtenteConnesso" scope="request"
...
<body class="corpo" onload="javascript:focus();document.f.submit();">
	<FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="f"
		...
		<input type="HIDDEN" name="formname"
			value="<%=request.getParameter("formname")%>"> <input
			type="HIDDEN" name="fieldname"
			value="<%=request.getParameter("fieldname")%>"> <input
			type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>"
			value="siap.sico.decodifiche.action.ActLoadProvinciaComuneNascita">
		<br>
		...
		<table cellpadding=0 cellspacing=0>
			<tr>
				...
				<td valign="middle"><select
					name="<%=ICostantiComune.CAMPO_COD_PROVINCIA%>"
					onclick="javascript:document.f.go.disabled=false;">
						<%
...
%>
				...
				<td valign="middle"><input type="submit" name="go"
					value="Seleziona"></td>
			</tr>
		</table>
	</form>
