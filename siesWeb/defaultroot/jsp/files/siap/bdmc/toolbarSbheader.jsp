...
if ((request.getParameter(ICostantiSbPren.CAMPO_ENTITA_CHIAVE_TRE) != null
  && request.getParameter(ICostantiSbPren.CAMPO_ENTITA_CHIAVE_TRE).length() > 0 )
  && (request.getParameter(ICostantiSbPren.CAMPO_VALORE_CHIAVE_TRE) != null
  && request.getParameter(ICostantiSbPren.CAMPO_VALORE_CHIAVE_TRE).length() > 0 ))
  {
      treeParam = true;
      lParam3 = "&" + request.getParameter(ICostantiSbPren.CAMPO_ENTITA_CHIAVE_TRE) + "=" +
                     request.getParameter(ICostantiSbPren.CAMPO_VALORE_CHIAVE_TRE);
  }
...
<%
        ...
        if(lFun.getFunctionType().equals(ICostantiFunzioni.TIPO_CANCELLA))
        {
         ...
         if((lParameter != null) && (lParameter.length()>0) && !treeParam) {%>
                ...
                <%} else //tre parametri
         if(treeParam)  {   %>
                <a href="Javascript:conferma('<%=lFun.getNameAction()%>','<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>','<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%>','<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA_PROVV)%>','<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA_PROVV)%>','<%=request.getParameter(ICostantiSbPren.CAMPO_ENTITA_CHIAVE_TRE)%>','<%=request.getParameter(ICostantiSbPren.CAMPO_VALORE_CHIAVE_TRE)%>');">
                   <img src="/images/delete24.gif" width="24" height="24" alt="Cancella" border="0">
                </a>
...
if(   request.getParameter(ICostantiSbPren.CAMPO_TORNA_INDIETRO) != null
     && !request.getParameter(ICostantiSbPren.CAMPO_TORNA_INDIETRO).equals(""))
  {%>
    <td class="LBG">
