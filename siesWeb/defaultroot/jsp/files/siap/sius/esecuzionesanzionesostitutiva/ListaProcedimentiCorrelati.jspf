...
%>
      <tr>
        ...
        <td class="c"><font class="label">
          <a class="CliccabileFermo" href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.sius.fascicolo.action.ActLoadDettaglioFascicolo&<%=ICostantiFascicoloSius.CAMPO_ID_FASCICOLO_SIUS%>=<%=prCorrelato.getFascicoloSiusModel().getIdFascicoloSius()%>&TornaQui=<%=TornaQui%>" Title="Dettaglio Procedimento SIUS">
...
<%        if (prCorrelato.getFascicoloSiusModel().getNumeroFascicoliUnificati() != null &&
          ...
          {%>
          <a href="#1" onClick="return effettoTree(<%=jESS%>)"><img name="image<%=jESS%>" src="<%=IWebConstants.IMAGES_DIR%>expand.gif"  alt="" border="0" Title="Elenco Procedimenti del Tribunale Correlati" ></a>
        <%}%>
        </font></td>
        ...
        <font class="label"><%=prCorrelato.getFascicoloSiusModel().getDescrTipoUfficio()%>&nbsp;<%=prCorrelato.getFascicoloSiusModel().getDescrComuneUfficio()%></font></td>
