...
<%
//==============================================================================
// jsp per l'inserimento dell'opposizione TDS e MDS 
//==============================================================================
%>
...
<html>
  <head>
    ...
    function Verify()
    {
      ...
      var data_atto = document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_GIORNO_DATA_RICORSO%>.value
                 +'/'+document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_MESE_DATA_RICORSO%>.value
                 +'/'+document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_ANNO_DATA_RICORSO%>.value;
      ...
      var data_canc = document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_GIORNO_DATA_ARRIVO_CANCELLERIA%>.value
                 +'/'+document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_MESE_DATA_ARRIVO_CANCELLERIA%>.value
                 +'/'+document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_ANNO_DATA_ARRIVO_CANCELLERIA%>.value;
      ...
      var data_decisione = document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_GIORNO_DATA_DECISIONE%>.value
                      +'/'+document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_MESE_DATA_DECISIONE%>.value
                      +'/'+document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_ANNO_DATA_DECISIONE%>.value;

      if (! ControllaDataPassaVuota(data_decisione))
      {
        alert('Data Decisione non valida');
        document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_GIORNO_DATA_DECISIONE%>.focus();
        return false;
      }
      
      var data_restituzione = document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_GIORNO_DATA_RESTITUZIONE_ATTI%>.value
                         +'/'+document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_MESE_DATA_RESTITUZIONE_ATTI%>.value
                         +'/'+document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_ANNO_DATA_RESTITUZIONE_ATTI%>.value;
      if (! ControllaDataPassaVuota(data_restituzione))
      {
        alert('Data Restituzione Atti non valida');
        document.LoadInserisciOpposizione.<%=ICostantiImpugnazione.CAMPO_GIORNO_DATA_RESTITUZIONE_ATTI%>.focus();
        return false;
      }
    ...
    <table>
      <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
        <td class=LBG><font class="label">Funzione :</font>&nbsp;
            <font class="campo">Iscrizione Opposizione</font>  
        </td>
  ...
  <FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name='LoadInserisciOpposizione'>
    <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sius.impugnazione.action.ActInserisciOpposizione" >
    ...
    <input type="HIDDEN" name="<%=ICostantiImpugnazione.CAMPO_ID_IMPUGNAZIONE%>" value="<%=StringUtils.toStringJSP(impugnazione.getIdImpugnazione(),"")%>" >

    <input type="HIDDEN" name="<%=ICostantiImpugnazione.CAMPO_COD_TIPO_IMPUGNAZIONE%>" value="04" >
...
--%>
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator  = new Validator("LoadInserisciOpposizione");
