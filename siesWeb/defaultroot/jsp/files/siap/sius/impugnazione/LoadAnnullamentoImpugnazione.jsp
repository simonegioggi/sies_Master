...
function verify()
{
    ...
    window.opener.top.frames['centrale'].frames['body'].location.href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=nextAction%>&IdEvento=" + "<%=IdEvento%>" + "&IdImpugnazione=" + "<%=IdImpugnazione%>" + "&<%=ICostantiImpugnazione.CAMPO_MOTIVO_ANNULLAMENTO%>=" + lmotivazioni + "&TornaQui=10";
 window.close();
}
