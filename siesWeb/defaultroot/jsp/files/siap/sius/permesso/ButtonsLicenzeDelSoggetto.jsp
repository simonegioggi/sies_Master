...
%>
              <td>
                <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%>&hufUtConnesso=<%=ufUtConnesso%>&hufOTribunale=<%=ufOTribunale%>&hcodDistretto=<%=codDistretto%>&hlIncludeRigettati=<%=lIncludeRigettati%>&hcodLicenza=<%=codLicenza%>&hdescrLicenza=<%=descrLicenza%>&htipoUfficio=<%=tipoUfficio%>&hdataDalInCancelleria=<%=dataDalInCancelleria%>&hdataAlInCancelleria=<%=dataAlInCancelleria%> ">
                  <img src="/images/dettagli.gif" width="12" height="12" alt="Elenco dei Procedimenti relativi a licenze" border="0">
                </a>
              </td>
<%
            }
