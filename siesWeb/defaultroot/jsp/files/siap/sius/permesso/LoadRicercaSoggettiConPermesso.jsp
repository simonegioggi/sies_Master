...
<head>
  ...
  function Verify()
  {
    ...
    if (! ControllaData(data_to_verify) && data_to_verify.length>2)
    {
      ...
      document.LoadRicercaSoggettiConPermesso.<%=ICostantiSoggetto.CAMPO_GIORNO_DATA_NASCITA%>.focus();
      return false;
    }
    if (document.LoadRicercaSoggettiConPermesso.<%=ICostantiSoggetto.CAMPO_COGNOME%>.value.length<2)
    {
      alert("Valorizzare almeno i primi 2 caratteri del Cognome");
      document.LoadRicercaSoggettiConPermesso.<%=ICostantiSoggetto.CAMPO_COGNOME%>.focus();
      return false;
    }

		var data_to_verify=document.LoadRicercaSoggettiConPermesso.<%=ICostantiFascicoloSius.CAMPO_GIORNO_DATA_INSERIMENTO%>.value+'/'+document.LoadRicercaSoggettiConPermesso.<%=ICostantiFascicoloSius.CAMPO_MESE_DATA_INSERIMENTO%>.value+'/'+document.LoadRicercaSoggettiConPermesso.<%=ICostantiFascicoloSius.CAMPO_ANNO_DATA_INSERIMENTO%>.value ;
		if (! ControllaData(data_to_verify) && data_to_verify.length>2)
		{
			alert('Data non valida');
      document.LoadRicercaSoggettiConPermesso.<%=ICostantiFascicoloSius.CAMPO_GIORNO_DATA_INSERIMENTO%>.focus();
			return false;
		}
		var data_to_verify=document.LoadRicercaSoggettiConPermesso.<%=ICostantiFascicoloSius.CAMPO_GIORNO_DATA_AGGIORNAMENTO%>.value+'/'+document.LoadRicercaSoggettiConPermesso.<%=ICostantiFascicoloSius.CAMPO_MESE_DATA_AGGIORNAMENTO%>.value+'/'+document.LoadRicercaSoggettiConPermesso.<%=ICostantiFascicoloSius.CAMPO_ANNO_DATA_AGGIORNAMENTO%>.value ;
		if (! ControllaData(data_to_verify) && data_to_verify.length>2)
		{
			alert('Data non valida');
      document.LoadRicercaSoggettiConPermesso.<%=ICostantiFascicoloSius.CAMPO_GIORNO_DATA_AGGIORNAMENTO%>.focus();
			return false;
		}
    return true;
  }
