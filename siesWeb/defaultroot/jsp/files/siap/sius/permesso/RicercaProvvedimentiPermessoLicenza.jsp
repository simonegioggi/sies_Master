...
<!--  jsp:useBean id="elenco" scope="request" class="java.util.Collection"/-->
...
<html>
        ...
        <td class="LBG">
	 	...
	 	<table>
	 		<tr>
			    ...
			    <tr>    	
			    	<td class="L" >
			      	...
			      	<td class="l">
			      		<font class="campo"><%=DateUtils.getDateToString(criteriRicerca.getDataDepositoFinale(),"dd-MM-yyyy")%></font>
			      	</td>
			    ...
			    <tr>    	
			    	<td class="L">
			      	<font class="label">Tipo Ricerca</font>
			      </td>
			    	<td class="L" >
			     		<font class="campo"><%=criteriRicerca.getDescrTipoRicerca()%></font>
			      </td>
			    </tr>
			  	<tr>    	
			    	<td>
		  ...
		  <td> &nbsp; </td>
		    ...
		    <tr>    	
		    	<td class="L">
		      	<font class="label">Totale Permessi Necessità </font>
		      </td>
		    	<td class="l">
		     		<font class="campo"><%=totali.getNumPN()%></font>
		      </td>
		    </tr>
		    <tr>    	
		    	<td class="L">
		      	<font class="label">Totale Permessi Premio </font>
		      </td>
		    	<td class="l">
		      	<font class="campo"><%=totali.getNumPP()%></font>
		      </td>
		    </tr>

		    <tr>    	
		    	<td class="L">
		      	<font class="label">Totale Permessi Internati </font>
		      </td>
		    	<td class="l">
		      	<font class="campo"><%=totali.getNumPI()%></font>
		      </td>
		    </tr>

		    <tr>    	
		    	<td class="L">
		      	<font class="label">Totale Licenze </font>
		      </td>
		    	<td class="l">
		     		<font class="campo"><%=totali.getNumLC()%></font>
		      </td>
		    </tr>

		    <tr>    	
		    	<td class="L">
		      	<font class="label">Totale Licenze Internati </font>
		      </td>
		    	<td class="l">
		     		<font class="campo"><%=totali.getNumLI()%></font>
		      </td>
		    </tr>
    
		    <tr>    	
		    	<td class="L">
		      	<font class="label">Totale Provvedimenti</font>
		      </td>
		    	<td class="l">
		      	<font class="campo"><%=totali.getNumTot()%></font>
		      </td>
		    </tr>
   		</table>
   	</td>
 ...
 <jsp:include page="<%=IWebConstants.PAGINAZIONE_RICERCA%>"></jsp:include>
	...
	<table cellspacing=2 cellpadding=2 width="95%">
  	<tr>
      <td class="int" width=15%>Numero SIUS</td>
      <td class="int" width=30%>Cognome Nome</td>
      <td class="int" width=18%>Data di nascita</td>
      <td class="int" width=20%>Luogo Detenzione</td>
      <td class="int" width=18%>Data Deposito</td>
      <td class="int" width=15%>Oggetto</td>
...
<% 
			...
			lRedirDettFasSius.setParameter(ICostantiFascicoloSius.CAMPO_ID_FASCICOLO_SIUS, 
			    lModel.getFascicolo().getIdFascicoloSius().toString() );
			lRedirDettFasSius.setParameter( IWebConstants.LINK_RITORNO, "20" );
%>      
      	<a class="cliccabile" href="<%=lRedirDettFasSius%>">
      	<%=StringUtils.toStringJSP(lModel.getFascicolo().getChiaveAnno())%>/
      	<%=StringUtils.toStringJSP(lModel.getFascicolo().getChiaveProgr())%>
      	</a>
      </td>
      <td class=l>
      	<%=StringUtils.toStringJSP(lModel.getSoggetto().getCognome())%>&nbsp;
      	<%=StringUtils.toStringJSP(lModel.getSoggetto().getNome())%>
      </td>
      <td class=l>
      	<%=StringUtils.toStringJSP(
      	    DateUtils.getDateToString(lModel.getSoggetto().getDataNascita(),"dd-MM-yyyy"))%>
      </td>
			<td class=l>
			...
			<td class=l>
				<%=StringUtils.toStringJSP(
				    DateUtils.getDateToString(lModel.getDepositoDecreto().getDataDeposito(),"dd-MM-yyyy"))%>
			</td>
			<td class=c>
...
<% 
			...
			if( lModel.getLicenza().getCodTipoLicenza().equalsIgnoreCase("PP") )
			  ...
			  lRedirDettEsec.setAction("siap.sius.permesso.action.ActLoadDettaglioEsecuzioneLicenza");			
			*/
			lRedirDettEsec.setParameter(ICostantiFascicoloSius.CAMPO_CHIAVE_ANNO, 
			    lModel.getFascicolo().getChiaveAnno().toString() );
			lRedirDettEsec.setParameter(ICostantiFascicoloSius.CAMPO_CHIAVE_PROGR, 
			    lModel.getFascicolo().getChiaveProgr().toString() );
