...
function Verify() {
	...
	var data_to_verify = document.LoadRicercaSoggettiConLicenza.<%=ICostantiFascicoloSius.CAMPO_GIORNO_DATA_INSERIMENTO%>.value
		+ '/' + document.LoadRicercaSoggettiConLicenza.<%=ICostantiFascicoloSius.CAMPO_MESE_DATA_INSERIMENTO%>.value
		+ '/' + document.LoadRicercaSoggettiConLicenza.<%=ICostantiFascicoloSius.CAMPO_ANNO_DATA_INSERIMENTO%>.value;
	if (!ControllaData(data_to_verify) && data_to_verify.length > 2) {
		alert('Data non valida');
		document.LoadRicercaSoggettiConLicenza.<%=ICostantiFascicoloSius.CAMPO_GIORNO_DATA_INSERIMENTO%>.focus();
		return false;
	}
	var data_to_verify = document.LoadRicercaSoggettiConLicenza.<%=ICostantiFascicoloSius.CAMPO_GIORNO_DATA_AGGIORNAMENTO%>.value
		+ '/' + document.LoadRicercaSoggettiConLicenza.<%=ICostantiFascicoloSius.CAMPO_MESE_DATA_AGGIORNAMENTO%>.value
		+ '/' + document.LoadRicercaSoggettiConLicenza.<%=ICostantiFascicoloSius.CAMPO_ANNO_DATA_AGGIORNAMENTO%>.value;
	if (!ControllaData(data_to_verify) && data_to_verify.length > 2) {
		alert('Data non valida');
...
function ListaComuni(a_formname,a_fieldname) {
    ...
    function cancellaCodComuneReale() {
      	document.LoadRicercaSoggettiConLicenza.<%=ICostantiComune.CAMPO_COD_COMUNE_REALE%>.value = "";      	
    }
...
<table>
	<tr>
        ...
        <td class="LBG">
        	...
        	<font class="campo">Ricerca Soggetti con Procedimenti di Sorveglianza relativi a licenze</font>
        </td>
	</tr>
</table>
<br>
<table cellspacing="2" cellpadding="2">
	...
	<tr>
		...
		<td class="l">
			<input Title="Comune di nascita" name="<%=ICostantiSoggetto.CAMPO_COD_COMUNE_NASCITA%>" value="" type="text" maxlength="35" size="35" onChange="cancellaCodComuneReale();" > 
			<a href="Javascript:ListaComuniNascita('LoadRicercaSoggettiConLicenza','<%= ICostantiSoggetto.CAMPO_COD_COMUNE_NASCITA %>');">
	...
	<tr>
        ...
        <td class="L">
			<select  title="Stato di Nascita" name="<%=ICostantiSoggetto.CAMPO_COD_STATO_NASCITA%>"><%=nazioni%></select>
		</td>
	</tr>
	<tr>
...
<table cellspacing="2" cellpadding="2">
	<tr>
		<td class="lVerdeNB">N.B.: Di norma vengono visualizzati i soggetti con provvedimenti di licenza di competenza dell'ufficio senza limitazione del periodo di pervenimento in cancelleria. Per variare i criteri selezionare una o più delle seguenti opzioni:</td>
	</tr>
</table>
<br>
<table cellspacing="2" cellpadding="2">
	<tr>
		<td class="Cliccabile">Modifica competenza territoriale dei procedimenti di licenza visualizzati</td>
	</tr>
	<tr>
		<td class="label">Visualizza solo le licenze dell'Ufficio</td>
        <td class="label"><input type=radio name="<%=ICostantiFascicoloSius.CAMPO_INCLUDE_UFFICIO%>" value="0" CHECKED></td>
	</tr>
	<tr>
		<td class="label">Visualizza le licenze dell'intero Distretto</td>
        <td class="label"><input type=radio name="<%=ICostantiFascicoloSius.CAMPO_INCLUDE_UFFICIO%>" value="2"></td>
	</tr>
	<tr>
        <td class="label">Visualizza anche le licenze di altri Distretti presenti in BDI</td>
        <td class="label"><input type=radio name="<%=ICostantiFascicoloSius.CAMPO_INCLUDE_UFFICIO%>" value="3"></td>
	</tr>
	<tr><td>&nbsp;</td></tr>
	<tr>
	...
	<tr>
        <td class="label">Visualizza anche le licenze rigettate</td>
        <td class="label"><input type=checkbox name="<%=ICostantiPermesso.CAMPO_INCLUDE_RIGETTATI%>" value="1"></td>
	</tr>
</table>
<table cellspacing="2" cellpadding="2">
	<tr>
		<td class="label">Visualizza solo i provvedimenti relativi a &nbsp;&nbsp;
	  		<select title="contenuto" class=small name="<%=ICostantiPermesso.CAMPO_COD_LICENZA%>"><%=licenza%></select>
	  	</td>
	</tr>
	<tr><td>&nbsp;</td></tr>
	<tr>
