...
<jsp:useBean id="elencoCertificati"  scope="request" class="java.lang.String"/>

<%
  // Azione da chiamare per l'inserimento dei dati.
  String lAzione = "siap.sius.misuresicurezzarichiestaatti.action.ActInserisciCertificatiComune";
%>
...
<html>
<head>
  <title>[S.I.E.S.] - Richiesta Certificati al Comune</title>
  ...
  <script language="JavaScript" src="<%=IWebConstants.JS_VALIDATOR%>"></script>
    ...
    function Verify()
    {
      if (document.LoadInserisciCertificatiComune.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciCertificatiComune.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciCertificatiComune.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciCertificatiComune.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciCertificatiComune.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciCertificatiComune.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciCertificatiComune.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciCertificatiComune.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciCertificatiComune.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
...
<body onLoad="document.forms[0].elements[0].focus()" class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Richiesta Certificati al Comune</font>
      </td>
      <td class="LBG">
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciCertificatiComune">
    <table cellspacing=2 cellpadding=2>
      ...
      <tr>
        <td class="l">Comune <font class=ob>(*)</font></td>
        <td class="l">
           <input Title="Comune" name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
              value="" type="text" maxlength="35" size="35">
              <a href="Javascript:ListaComuni('LoadInserisciCertificatiComune','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>');">
          ...
          <tr>
            ...
            <td class="L" colspan=3>
             <input title="Indirizzo" name="<%=ICostantiIstruttoria.TIPO_DOCUMENTO %>" value="" type="text" maxlength="80" size="80">
      		...
      		<tr>
      		<td class="l">Certificati Richiesti</td>
      		<td class="l">
        		<select multiple title="Tipo" name="<%=ICostantiRichiestaAtti.CAMPO_AGGIUNTIVO%>" >
        		<%=elencoCertificati%>
        		</select>
						<br>
						<font class=cVerde>Per selezionare/deselezionare più certificati <br> premere tasto CTRL + tasto sinistro del mouse.</font> 
      		</td>
    		</tr>
      
      <tr>
           <td class="l">Ulteriore Tipo Certificato</td>
           <td class="L" colspan=3>
             <input title="Note" name="<%= ICostantiRichiestaAtti.CAMPO_NOTE %>" type="text" maxlength="80" size="80">
          </td>
          </tr>
        ...
        <tr>
            <td class="l">Note</td>
            <td class="L" colspan=3>
             <TEXTAREA title="Note" name="<%= ICostantiRichiestaAtti.CAMPO_NOTE %>"  cols=40 rows=4 ></textarea>
            </td>
            <input name="Certificato" value="note" type="hidden" >
        </tr>
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("LoadInserisciCertificatiComune");
