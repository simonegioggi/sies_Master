...
<%
  ...
  String lAzione = "siap.sius.misuresicurezzarichiestaatti.action.ActInserisciInfDSMUOSM";
%>
...
<script language="JavaScript">
    function Verify()
    {
      // Controlla che le coppie di campi Destinatario/Sede siano riempiti ( destinatario 1 )
      if (document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[0].value != '-'
          && document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[0].value == '')
      {
      	alert('La Sede del destinatario n°1 è obbligatoria');
        return false;
      }
      // Controlla che le coppie di campi Destinatario/Sede siano riempiti ( destinatario 2 )
      if (document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[1].value != '-'
          && document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[1].value == '')
      {
      ...
      if (document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[0].value == '-'
          && document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[1].value == '-')
      {
      	alert('Inserire almeno un Destinatario con la relativa Sede.');
        return false;
      }

      if (document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciInformazioniDSMUOSM.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
...
<html>
  <head>
    <title>[S.I.E.S.] - Richiesta Informazioni D.S.M./U.O.S.M.</title>
  ...
  <body onLoad="document.forms[0].elements[0].focus()" class="corpo">
    <table>
      <tr>
        ...
        <td class="LBG">
          ...
          <font class="campo">Richiesta Informazioni D.S.M./U.O.S.M.</font>
        </td>
      	<td class="LBG">
    ...
    <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciInformazioniDSMUOSM">
          	...
          	<tr>
          		...
          		<td class="l">
             		<input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
                		...
                		<a href="Javascript:ListaComuni('LoadInserisciInformazioniDSMUOSM','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[0]');">
          ...
          <table>
          	<tr>
          		...
          		<td class="l">
            		<input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
                		value="" type="text" maxlength="35" size="35">
                		<a href="Javascript:ListaComuni('LoadInserisciInformazioniDSMUOSM','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[1]');">
                		<img src="/images/filefolder.gif" border=0> </a>
          		</td>
          	</tr>
          	<tr>
    ...
    <script language="JavaScript" type="text/javascript">
      var frmvalidator = new Validator("LoadInserisciInformazioniDSMUOSM");
