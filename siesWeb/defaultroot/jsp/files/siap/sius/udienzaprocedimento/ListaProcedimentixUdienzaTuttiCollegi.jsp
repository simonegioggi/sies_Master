    ...
    <tr>
     ...
     <td class="LBG" nowrap>
       <jsp:include page="<%=ICostantiUdienzaProcedimento.PG_COMBOTEMPLATE_RUOLOUDIENZA%>">
        ...
        <jsp:param name="CampoIdEntita" value="<%=ICostantiUdienza.CAMPO_DATA_UDIENZA%>" />
...
%>
    <tr>
    	...
    	<td class=c><%=i%></td>
			<td class=c><%=procedimento.getNumCollegio()%></td>
...
%>
    <td class=l>
       <a href="Javascript:stampa2('<%=ISIAPCostantiWeb.PG_STAMPA%>','<%=IWebConstants.ACTION_FIELD%>=siap.sius.udienza.action.ActStampaFlashVerbaleUdienza&<%=ICostantiFascicoloSius.CAMPO_ID_FASCICOLO_SIUS%>=<%=procedimento.getIdFasSIUS()%>&dataUdienza=<%=DateUtils.getDateToString(data_udi,"dd-MM-yyyy")%>');">
