...
<%
  String lAzione = "siap.sius.produzioneatti.action.ActInserisciRichiestaParere";
%>
...
<script language="JavaScript">
    ...
    <script language="JavaScript">
    function CambiaParere(value)
    {
      var codMotDec = "<%=ICostantiProduzioneAtti.PARERE_INAMMISSIBILITA%>";
      //alert (value);
      if (value == codMotDec)
      {
        //alert("motivazioniDecreto");
        disabilitaDiv("motivazioni");
        abilitaDiv("motivazioniDecreto");
      }
      else
      {
        //alert("motivazioneSemplice");
        disabilitaDiv("motivazioniDecreto");
        abilitaDiv("motivazioni");
      }
      
      if (<%=filtroMinorenni%>)
  	    document.LoadInserisciRichiestaParere.<%=ICostantiProduzioneAtti.CAMPO_COD_DESTINATARIO%>.selectedIndex=2; 
    }
</script>
    <script language="JavaScript">
 var node;
    function abilitaDiv(value)
    {
       //alert("abilito " + value);
    ...
    function disabilitaDiv(value)
    {
      // alert("disabilito " + value);
           node=document.getElementById(value);
...
<head>
  ...
  <script language="JavaScript" src="<%=IWebConstants.JS_VALIDATOR%>"></script>
    ...
    function Verify()
    {
      ...
      var data_emissione=document.LoadInserisciRichiestaParere.<%=ICostantiProduzioneAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciRichiestaParere.<%=ICostantiProduzioneAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciRichiestaParere.<%=ICostantiProduzioneAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
...
<body class="corpo" onload ="Javascript:return CambiaParere(document.LoadInserisciRichiestaParere.<%=ICostantiProduzioneAtti.CAMPO_COD_MOTIVO%>.value);" >
  <table>
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciRichiestaParere">
    <table cellspacing=2 cellpadding=2>
      ...
      <tr>
        ...
        <td class="L">
          <input Title="Giorno" value="<%=DateUtils.getSysDate("dd")%>" type="text" size="2" maxlength="2" name="<%= ICostantiProduzioneAtti.CAMPO_GIORNO_DATA_EMISSIONE %>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillDM(value)" > /
          <input Title="Mese" value="<%=DateUtils.getSysDate("MM")%>" type="text" size="2" maxlength="2" name="<%= ICostantiProduzioneAtti.CAMPO_MESE_DATA_EMISSIONE %>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillDM(value)" > /
          <input Title="Anno" value="<%=DateUtils.getSysDate("yyyy")%>" type="text" size="4" maxlength="4" name="<%= ICostantiProduzioneAtti.CAMPO_ANNO_DATA_EMISSIONE %>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillYear(value)" >
        </td>
      </tr>
      ...
      <tr>
        <td class="l">Destinatario <font class=ob>(*)</font></td>
    		...
    		<td class="L">
      		<select title="Destinatario" class=small name="<%=ICostantiProduzioneAtti.CAMPO_COD_DESTINATARIO%>" >
        <%= codTipoUfficioS %>
      </select>
      </tr>
      ...
      <tr>
        ...
        <td class="l">
           <input Title="Sede Procura" name="<%=ICostantiProduzioneAtti.CAMPO_SEDE%>"
              value="<%=UtenteConnesso.getUfficioUtente().getDescrComune()%>" type="text" maxlength="35" size="35">
              <a href="Javascript:ListaUffici('LoadInserisciRichiestaParere','<%=ICostantiProduzioneAtti.CAMPO_SEDE%>');">
        ...
        <tr>
          ...
          <td class="L">
          <select title="Tipo parere" name="<%=ICostantiProduzioneAtti.CAMPO_COD_MOTIVO%>" onchange="Javascript:return CambiaParere(value);">
            <%= colMotivoParere %>
          </select>
        </tr>
  ...
  <table>
   <%
    ...
    while( count <  4 )
    {
      ...
      <tr>
        <td >
          <TEXTAREA title="Motivo n° <%=count%>" name="<%= ICostantiProduzioneAtti.CAMPO_AGGIUNTIVO%>"  cols=80 rows=3 ></textarea>
        </td>
      </tr>
     <%
     count+=1;
...
%>
      <tr>
        <td class="l"><input value="<%=lDecod.getCode()%>" type="checkbox" name="<%=ICostantiMotivazioneDecreto.CAMPO_CK_01%>"></td>
...
%>
          <input value="" type="text" name="<%=ICostantiMotivazioneDecreto.CAMPO_ALTRA_MOTIVAZIONE+lDecod.getCode()%>" size=35>
<%
            }
...
%>
      <tr>
        <td class="l"><input value="90" type="checkbox" name="<%= ICostantiMotivazioneDecreto.CAMPO_CK_01%>"></td>
        <td class="l">
          <textarea title="AltraMotivazione90" name=<%=ICostantiMotivazioneDecreto.CAMPO_ALTRA_MOTIVAZIONE+"90"%> cols=90 rows=5 ></textarea>
        </td>
      </tr>
 <table cellspacing=2 cellpadding=2>
      <tr>
...
<%-- 	    frmvalidator.addValidation("<%=ICostantiProduzioneAtti.CAMPO_SEDE%>","alpha"); --%>
	    ...
	    frmvalidator.setAddnlValidationFunction("Verify");
	 	...
	 	// MEV10-s3: aggiunta impostazione iniziale nella combo per i minori
	 	var codTipoUfficio = '<%=UtenteConnesso.getUfficioUtente().getCodTipoUfficio()%>';
	 	if ("TDSM" == codTipoUfficio || "UDSM" == codTipoUfficio) {
	 		document.LoadInserisciRichiestaParere.<%=ICostantiProduzioneAtti.CAMPO_COD_MOTIVO%>.value = "0750";
	 	}
  	</script>
