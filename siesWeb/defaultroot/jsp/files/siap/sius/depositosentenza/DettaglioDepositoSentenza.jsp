...
<html>
<head>
  <title>[S.I.E.S.] - Dettaglio Deposito Sentenza </title>
  ...
  <body class="corpo">
    <table>
      <tr>
        ...
        <td class=LBG><font class="label">Funzione : </font><font class="campo">Dettaglio Deposito Sentenza</font>&nbsp;
...
%>    
  	<td class="LBG">
   		<a href="/jsp/Main.jsp?Action=siap.sius.depositosentenza.action.ActLoadInserisciDataDeposito&Aggiungi=yes&<%=ICostantiEvento.CAMPO_ID_EVENTO%>=<%=depositosentenza.getIdEventoGenerato()%><%=retParam%>">
      		<img align="middle" src="/images/new24.gif" alt="Iscrizione Altri Destinatari" width="24" height="24" border="0">
        </a>
    </td>
   	
   	<td class="LBG">
   		<a href="/jsp/Main.jsp?Action=siap.sius.depositosentenza.action.ActLoadModificaDataDepositoSentenza&Aggiungi=yes&<%=ICostantiEvento.CAMPO_ID_EVENTO%>=<%=depositosentenza.getIdEventoGenerato()%><%=retParam%>">
      		<img  align="middle" src="/images/modifica24.gif" alt="Modifica Deposito Sentenza" width="24" height="24" border="0">
        </a>
    </td>    	
<%
	}
...
%>
	    ...
	    <td class="LBG">
	    	<a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.sius.depositosentenza.action.ActLoadTrasferisciSentenza&<%=ICostantiEvento.CAMPO_ID_EVENTO%>=<%=depositosentenza.getIdEventoGenerato()%>">
...
%>
			<td class="LBG">
					<a href="/jsp/Main.jsp?Action=siap.sico.webservice.action.ActUDSTrasferimentoEsecuzione&<%=ICostantiEvento.CAMPO_ID_EVENTO%>=<%=depositosentenza.getIdEventoGenerato()%>&IdDocumentoAllegato=<%=documentoAllegato.getIdDocumentoAllegato()%>">
...
<table cellspacing=4 cellpadding=4>
  <tr>
    <td>&nbsp;</td>
    <input Title="Id Evento" type="hidden" name="<%= ICostantiEvento.CAMPO_ID_EVENTO %>" value="<%=depositosentenza.getIdEventoGenerato()%>" >
  </tr>

  <tr>
    <td class="l"> Anno / Numero Sentenza</td>
    <td class="l"><font class="campo"> <%=StringUtils.toStringJSP(depositosentenza.getAnnoSentenza())%> / <%=StringUtils.toStringJSP(depositosentenza.getNumSentenza())%></font></td>
  </tr>

  <tr>
    <td class="l"> Data Emissione</td>
    <td class="l"> <font class="campo"><%=DateUtils.getDateToString(depositosentenza.getDataEmissione(),"dd/MM/yyyy")%></font></td>
  </tr>
  <tr>
    <td class="l"> Data Deposito in Cancelleria</td>
    <td class="l"> <font class="campo"><%=DateUtils.getDateToString(depositosentenza.getDataDeposito(),"dd/MM/yyyy")%></font></td>
  </tr>
  <tr>
 ...
 <div align=left style="visibility:hidden" id="upld">
  ...
  <table>
    ...
    <tr>
        ...
        <input type="HIDDEN" name="<%=ICostantiDocumentoAllegato.CAMPO_AZIONE_DETTAGLIO%>"  value="siap.sius.depositosentenza.action.ActLoadDettaglioDataDepositoSentenza">
      </td> 
    </tr>
  </table>
