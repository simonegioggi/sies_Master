...
<html>
  <head>
    ...
    function Verify()
    {
      ...
      //alert("Verify");
      var data_deposito = document.LoadInserisciDataDeposito.<%=ICostantiDepositoSentenza.CAMPO_GIORNO_DATA_DEPOSITO%>.value+'/'+document.LoadInserisciDataDeposito.<%=ICostantiDepositoSentenza.CAMPO_MESE_DATA_DEPOSITO%>.value+'/'+document.LoadInserisciDataDeposito.<%=ICostantiDepositoSentenza.CAMPO_ANNO_DATA_DEPOSITO%>.value;
    ...
    if (document.LoadInserisciDataDeposito.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%> != undefined )
     ...
     //if (ritorno) {
     //	 var sel="";
     //	 var lNotificaFax=document.LoadInserisciDataDeposito.<//%=ICostantiRichiestaAtti.CAMPO_NOTIFICA_VIA_FAX %>;
    	    
     //	 for(var notifica=0; notifica < lNotificaFax.length ; notifica++ ) {
     //		    var val=(lNotificaFax[notifica].checked ? "1" : "0");
     //		    sel += (sel=="" ? val : ","+val);
     //	 }
    	    
     //	 var numAvvocati =<//%=avvocato.size() %>
    		  
     //	 if (sel == "") {
     //	     for(var notifica=0; notifica < numAvvocati ; notifica++ ) {
     //	  		  sel += (sel=="" ? "0" : ",0");
     //	  	 }
     //	 }
    	 
     //	 document.LoadInserisciDataDeposito.<//%=ICostantiRichiestaAtti.CAMPO_NOTIFICHE_VIA_FAX %>.value=sel;
     //}
...
%>
        <font class="campo">Deposito Sentenza SIUS &nbsp;del&nbsp;<%=DateUtils.getDateToString(lDepositoSentenza.getDataEmissione(), "dd/MM/yyyy") %></font>
      </td>
   <!-- BOTTONE DI RITORNO -->
...
%>      </td>
        <td class="L">
        <%= StringUtils.toStringJSP( DateUtils.getDateToString(lDepositoSentenza.getDataDeposito(),"dd/MM/yyyy"))%>
        <input value="<%=StringUtils.toStringJSP( DateUtils.getDateToString(lDepositoSentenza.getDataDeposito(),"dd"))%>" type="hidden" size="2" maxlength="2" name="<%= ICostantiDepositoSentenza.CAMPO_GIORNO_DATA_DEPOSITO%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillDM(value)">
        <input value="<%=StringUtils.toStringJSP( DateUtils.getDateToString(lDepositoSentenza.getDataDeposito(),"MM"))%>" type="hidden" size="2" maxlength="2" name="<%= ICostantiDepositoSentenza.CAMPO_MESE_DATA_DEPOSITO%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillDM(value)">
        <input value="<%=StringUtils.toStringJSP( DateUtils.getDateToString(lDepositoSentenza.getDataDeposito(),"yyyy"))%>" type="hidden" size="4" maxlength="4" name="<%= ICostantiDepositoSentenza.CAMPO_ANNO_DATA_DEPOSITO%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillYear(value)" >
<%    }else{%>
        <font class="ob">(*)</font></td>
        <td class="L">
        <input value="" type="text" size="2" maxlength="2" name="<%= ICostantiDepositoSentenza.CAMPO_GIORNO_DATA_DEPOSITO%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillDM(value)"> /
        <input value="" type="text" size="2" maxlength="2" name="<%= ICostantiDepositoSentenza.CAMPO_MESE_DATA_DEPOSITO%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillDM(value)"> /
        <input value="" type="text" size="4" maxlength="4" name="<%= ICostantiDepositoSentenza.CAMPO_ANNO_DATA_DEPOSITO%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillYear(value)" >
<%    }%>
      </td>
    </tr>
  </table>
...
%>
  <table cellspacing="0" cellpadding="0" width="100%">
   <tr>
     <td class="l"><input type="radio" name="tipo" value="Dep" CHECKED onClick="javascript:CambiaTipo();">Deposito Sentenza</td>
     <td class="l"><input type="radio" name="tipo" value="DepTrasm" onClick="javascript:CambiaTipo();">Deposito Sentenza e Trasmissione</td>
   </tr>
  </table>
...
<jsp:include page="<%=ICostantiDepositoDecreto.PG_INSERIMENTO_NOTIFICA_AVVOCATI_ALTRO%>">
 <jsp:param name="NomeForm" value="LoadInserisciDataDeposito"/>
    ...
    <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sius.depositosentenza.action.ActInserisciDataDeposito" >
    </form>
