...
<%
      ...
      if( (lFunFiglie != null) && (lFunFiglie.size() != 0) )
      {
        ...
        while(lIterBottoni.hasNext())
        {
          ...
          {
            String idSoggetto = request.getParameter(ICostantiEsecuzioneMA.CAMPO_ID_SOGGETTO);
            String idFascicoloSIUS = request.getParameter(ICostantiFascicoloSius.CAMPO_ID_FASCICOLO_SIUS);
            String idFascicoloSIEP = request.getParameter(ICostantiEsecuzioneMA.CAMPO_ID_FASCICOLO_SIEP);
            if(lFun.getFunctionType().equals(ICostantiFunzioni.TIPO_DETTAGLIO))
            {
%>
              <td>
                <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%>&<%=ICostantiEsecuzioneMA.CAMPO_ID_SOGGETTO%>=<%=idSoggetto%>&<%=ICostantiEsecuzioneMA.CAMPO_ID_FASCICOLO_SIEP%>=<%=idFascicoloSIEP%>&<%=ICostantiFascicoloSius.CAMPO_ID_FASCICOLO_SIUS%>=<%=idFascicoloSIUS%>&TornaQui=<%=TornaQui%>">
                  <img src="/images/dettagli.gif" width="12" height="12" alt="Elenco dei Procedimenti relativi all'Esecuzione della Misura Alternativa " border="0">
                </a>
              </td>
<%
            }
            // STUB 07/05/2004 In Presenza del fascicolo SIEP viene richiamata l'iscrizione Procedimento da UDS (ActLoadInserisciFascicoloUDS).
