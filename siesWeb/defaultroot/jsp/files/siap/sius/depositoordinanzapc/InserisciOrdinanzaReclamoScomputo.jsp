...
<html>
  <head>
<% if(tipo_decreto != null && tipo_decreto.equals("OR")) { %>
    <title>[S.I.E.S.] - Emissione Ordinanza Reclamo per Revoca Licenza/Permesso</title>
<% } else { %>
	<title>[S.I.E.S.] - Emissione Ordinanza Reclamo Scomputo</title>
<% } %>
    ...
    {
      var ritorno = true;
      var esito = document.InserisciOrdinanzaReclamoScomputo.<%=ICostantiTenore.CAMPO_COD_ESITO_TENORE%>;
    ...
    function ControlliDate()
    {
     ...
     var data_emissione_decreto = document.InserisciOrdinanzaReclamoScomputo.<%=ICostantiDepositoOrdinanzaPc.CAMPO_GIORNO_DATA_TRASMISSIONE%>.value+'/'+document.InserisciOrdinanzaReclamoScomputo.<%=ICostantiDepositoOrdinanzaPc.CAMPO_MESE_DATA_TRASMISSIONE%>.value+'/'+document.InserisciOrdinanzaReclamoScomputo.<%=ICostantiDepositoOrdinanzaPc.CAMPO_ANNO_DATA_TRASMISSIONE%>.value;
  ...
  <script language="JavaScript">
    ...
    {
       document.InserisciOrdinanzaReclamoScomputo.<%=ICostantiDepositoOrdinanzaPc.CAMPO_GIORNO_DATA_TRASMISSIONE%>.value = "<%=DateUtils.getDateToString(decreto.getDataEmissione() ,"dd")%>";
       document.InserisciOrdinanzaReclamoScomputo.<%=ICostantiDepositoOrdinanzaPc.CAMPO_MESE_DATA_TRASMISSIONE%>.value = "<%=DateUtils.getDateToString(decreto.getDataEmissione(),"MM")%>";
       document.InserisciOrdinanzaReclamoScomputo.<%=ICostantiDepositoOrdinanzaPc.CAMPO_ANNO_DATA_TRASMISSIONE%>.value = "<%=DateUtils.getDateToString(decreto.getDataEmissione(),"yyyy")%>";
       document.InserisciOrdinanzaReclamoScomputo.<%=ICostantiDepositoOrdinanzaPc.CAMPO_COD_UFFICIO_MAGISTRATO_COMP%>.value = "<%=decreto.getDescrUfficioInserimento()%>";
    }
 ...
 <%
  ...
  if(CodUff.equals("TDSM") || CodUff.equals("UDSM")){
	  labelUfficio = "Ufficio di Sorveglianza presso Tribunale per Minorenni";
  } else {
...
<% if(tipo_decreto != null && tipo_decreto.equals("OR")) { %>
      Emissione Ordinanza Reclamo per Revoca Licenza/Permesso
<% } else { %>
	  Emissione Ordinanza Reclamo per scomputo Licenza/Permesso
<% } %>
      </font>&nbsp;</td>
    </tr>
    <tr>
  ...
  <FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name="InserisciOrdinanzaReclamoScomputo">
    <table width=35%>
   <tr>
...
<% if(tipo_decreto != null && tipo_decreto.equals("OR")) { %>    
    	Estremi provvedimento revoca licenza/permesso reclamato:
<% } else { %> 
		Estremi provvedimento scomputo licenza/permesso reclamato:
<% } %>
    <td/>
  </tr>
    <tr>
      ...
      <td class="l">
        ...
        <a href="Javascript:ListaUDS('InserisciOrdinanzaReclamoScomputo','<%=ICostantiDepositoOrdinanzaPc.CAMPO_COD_UFFICIO_MAGISTRATO_COMP%>','<%=CodUff%>');">
  ...
  <tr>
    ...
    <tr>
      <td class="l">Numero giorni da computare &nbsp;</td>
      <td class="l">
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("InserisciOrdinanzaReclamoScomputo");
