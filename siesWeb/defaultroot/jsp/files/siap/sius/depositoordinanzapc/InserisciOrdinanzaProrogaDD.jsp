...
<html>
  <head>
    <title>[S.I.E.S.] - Emissione Ordinanza di Proroga detenzione domiciliare</title>
  ...
  <script language="JavaScript">
   function init()
   {
      document.InserisciOrdinanzaProrogaDD.<%=ICostantiDepositoOrdinanzaPc.CAMPO_GIORNO_DATA_FINE_MISURA%>.focus();
   }

    // STUB 21/07/2004 Controllo obbligatorietà esiti.
    function Verify()
    {
      var lEsiti=document.InserisciOrdinanzaProrogaDD.<%=ICostantiTenore.CAMPO_COD_ESITO_TENORE%>;
    ...
    {
     var ritorno = true;
     var data_termine_periodo = document.InserisciOrdinanzaProrogaDD.<%=ICostantiDepositoOrdinanzaPc.CAMPO_GIORNO_DATA_FINE_MISURA%>.value+'/'+document.InserisciOrdinanzaProrogaDD.<%=ICostantiDepositoOrdinanzaPc.CAMPO_MESE_DATA_FINE_MISURA%>.value+'/'+document.InserisciOrdinanzaProrogaDD.<%=ICostantiDepositoOrdinanzaPc.CAMPO_ANNO_DATA_FINE_MISURA%>.value;
     var cod_esito = document.InserisciOrdinanzaProrogaDD.<%=ICostantiTenore.CAMPO_COD_ESITO_TENORE%>.value;
     var aammgg = document.InserisciOrdinanzaProrogaDD.<%=ICostantiDepositoOrdinanzaPc.CAMPO_NUM_ANNI_DETENZIONE_DOM%>.value + document.InserisciOrdinanzaProrogaDD.<%=ICostantiDepositoOrdinanzaPc.CAMPO_NUM_MESI_DETENZIONE_DOM%>.value + document.InserisciOrdinanzaProrogaDD.<%=ICostantiDepositoOrdinanzaPc.CAMPO_NUM_GIORNI_DETENZIONE_DOM%>.value;
     if (cod_esito == '0099' && (data_termine_periodo.length == 2 && aammgg == "" ))
     {
         ritorno = false;
         alert( "In caso di Proroga inserire data fine beneficio o la sua durata !");
 ...
 %>
  ...
  <body class="corpo" onLoad="Javascript:init();">
    ...
    <tr>
      ...
      <td class=LBG><font class="label">Funzione : </font> <font class="campo">Emissione Ordinanza Proroga Detenzione Domiciliare</font>&nbsp;
      </td>
    </tr>
    <tr>
  ...
  <FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name="InserisciOrdinanzaProrogaDD">
    <table width=35%>
   <tr>
...
<br>
 <table cellspacing="2" cellpadding="2" style="width: 90%;">
  ...
  <tr>
    <td class="Titolo" colspan=6> In caso di proroga beneficio indicare: <td>
  </tr>
    <tr>
  ...
  <tr>
    ...
    <tr>
      <td class="l">Durata beneficio <br> (AA-MM-GG)</td>
      <td class="L">
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("InserisciOrdinanzaProrogaDD");
