...
<html>
	<head>
  	<title>[S.I.E.S.] - Trasferimento Ordinanza</title>
...
<%
String lAction = "siap.sius.depositoordinanzapc.action.ActConfermaTrasmissione";
if (postTitle.compareTo("") == 0) {
%>
          		<font class="campo">Trasferimento Ordinanza</font>
<%
} else {
%>
...
%>
	...
	<jsp:include page="<%=ICostantiFascicoloSius.PG_LOAD_SINTESIPROCEDIMENTOSIUS%>"/>
  	<br>
  	<form method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadTrasferisciOrdinanza">
    	<td>
    		<font class="campo">Ordinanza N. <%=StringUtils.toStringJSP(datiOrdinanza.getOrdinanza().getAnnoS3())%>/<%=StringUtils.toStringJSP(datiOrdinanza.getOrdinanza().getNumS3())%></font>
    		<font class="Label"> del </font>
    		<font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(datiOrdinanza.getEvento().getDataEmissione(), "dd-MM-yyyy"))%></font>
    		<font class="Label"> depositata il </font>
    		<font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(datiOrdinanza.getOrdinanza().getDataDeposito(), "dd-MM-yyyy"))%></font>
    	</td>
    	<br>
   	<table width="100%" cellspacing=2 cellpadding=2>
      	<tr>
	        <td class="l"><font class="label">Data Deposito</font></td>
	        <td class="l"><font class="campo"><%=StringUtils.toStringJSP(DateUtils.getDateToString(datiOrdinanza.getOrdinanza().getDataDeposito(), "dd-MM-yyyy"))%>&nbsp;</font></td>
      	</tr>
      	<tr>
...
<%
// STUB 25/06/2004 Se l'ordinanza riguarda un procedimento di EMA o unificante, è stato preventivamente
		...
		<tr>
            <td class="l" colspan="2">
            	<font class="campo">
              	...
              	<% if (Utils.isPresent(lUffProvModel.getChiaveAnnoSiep()) && !"".equals(lUffProvModel.getChiaveAnnoSiep())
            		  && Utils.isPresent(lUffProvModel.getChiaveProgrSiep()) && !"".equals(lUffProvModel.getChiaveProgrSiep())) {
              		checked = true;
            	%>
      	...
      	<tr>
        	...
        	<td class="L">
          		<input title="Sede Destinatario"  type="text" name="<%= ICostantiEvento.CAMPO_COD_LUOGO_DESTINATARIO %>"  maxlength="35" size="35">
           		<a href="Javascript:ListaUfficiPerTipo('LoadTrasferisciOrdinanza','<%=ICostantiEvento.CAMPO_COD_LUOGO_DESTINATARIO%>[0]',document.LoadTrasferisciOrdinanza.<%=ICostantiEvento.CAMPO_COD_TIPO_UFFICIO_DESTINATARIO%>[0][document.LoadTrasferisciOrdinanza.<%=ICostantiEvento.CAMPO_COD_TIPO_UFFICIO_DESTINATARIO%>[0].selectedIndex].value);">
      	...
      	<tr>
	        ...
	        <td class="L">
          		<input title="Sede UEPE Destinatario"  type="text" name="<%= ICostantiEvento.CAMPO_COD_LUOGO_DESTINATARIO %>"  maxlength="35" size="35">
           		<a href="Javascript:ListaUfficiPerTipo('LoadTrasferisciOrdinanza','<%=ICostantiEvento.CAMPO_COD_LUOGO_DESTINATARIO%>[1]',document.LoadTrasferisciOrdinanza.<%=ICostantiEvento.CAMPO_COD_TIPO_UFFICIO_DESTINATARIO%>[1][document.LoadTrasferisciOrdinanza.<%=ICostantiEvento.CAMPO_COD_TIPO_UFFICIO_DESTINATARIO%>[1].selectedIndex].value);">
              		<img src="/images/filefolder.gif" border=0>
            	</a>
        	</td>
		</tr>
