...
if(decreto.compareTo("SI") == 0)
  ...
  lAction = "siap.sius.depositoordinanzapc.action.ActInserisciOrdinanzaReclamiCEDU";
...
<html>
  <head>
    <title>[S.I.E.S.] - Emissione Ordinanza per Reclamo in materia di rimedi risarcitori per violazione art.3 CEDU</title>
...
//
		...
		function AbilitaPeriodo() {
			var retValue = true;
		  	retValue = Chiusura();
		  	if (retValue) {
		  		// MERGE v10: aggiunta impostazione per abilitare disabilitare alcuni campi
		  		node = document.getElementById("reclamo");
		 	...
		 	// MERGE v10: aggiunta chiamata a funzione per abilitare disabilitare alcuni campi
			abilitaDisabilitaGiorniSomma();
...
//
	...
	function Verify() {
        ...
        if (retValue) {
			var data_emissione = '<%=DateUtils.getDateToString(data_emissione,"dd/MM/yyyy")%>';
			var data_emissione_ordinanza = document.InserisciOrdinanzaArt3Cedu.<%=ICostantiDepositoOrdinanzaPc.CAMPO_GIORNO_DATA_TRASMISSIONE%>.value+'/'+document.InserisciOrdinanzaArt3Cedu.<%=ICostantiDepositoOrdinanzaPc.CAMPO_MESE_DATA_TRASMISSIONE%>.value+'/'+document.InserisciOrdinanzaArt3Cedu.<%=ICostantiDepositoOrdinanzaPc.CAMPO_ANNO_DATA_TRASMISSIONE%>.value;
   			// Controlli su Data Emissione Ordinanza
    		if (data_emissione_ordinanza.length > 2) {
    			if (!ControllaData(data_emissione_ordinanza)) {
    				alert('Data Emissione Ordinanza Reclamata non valida');
			        ritorno =  false;
				} else if (!CompareDate(data_emissione_ordinanza, data_emissione)) {
					alert("Data Emissione Ordinanza Reclamata deve precedere " + data_emissione + " !");
			        ritorno =  false;
		       }
	    	}
           	...
           	if (retValue) {
            	...
            	if (!retValue) {
           			...
           			if (document.InserisciOrdinanzaArt3Cedu.<%=ICostantiTenore.CAMPO_COD_ESITO_TENORE%>.value != "2073"
						...
						if (document.InserisciOrdinanzaArt3Cedu.<%=ICostantiTenore.CAMPO_COD_ESITO_TENORE%>.value == "2764"
								|| document.InserisciOrdinanzaArt3Cedu.<%=ICostantiTenore.CAMPO_COD_ESITO_TENORE%>.value == "2765"
								|| document.InserisciOrdinanzaArt3Cedu.<%=ICostantiTenore.CAMPO_COD_ESITO_TENORE%>.value == "2766") {
...
// ---->
	...
	// MERGE v10: aggiunta funzione per abilitare disabilitare alcuni campi
	function abilitaDisabilitaGiorniSomma() {
		var reclamo = document.getElementById("reclamo");
		var periodo = document.getElementById("periodo");
		var somma = document.getElementById("somma");
		if (document.InserisciOrdinanzaArt3Cedu.<%=ICostantiTenore.CAMPO_COD_ESITO_TENORE%>.value in { '2761':1, '2762':1, '2763':1 }) {
			reclamo.style.display='block';
			periodo.style.display='block';
     		somma.style.display='block';
   		} else {
   			reclamo.style.display='none';
   			periodo.style.display = 'none';
     		somma.style.display = 'none';
   		}
	}
    </script>
    ...
    <table>
    	<tr>
     	...
     	<%	}
      		...
      		{	%>
      				<font class="campo">Emissione Ordinanza per Reclamo in materia di rimedi risarcitori per violazione art.3 CEDU</font>&nbsp;
      	<%	} %>		
      		</td>
    	</tr>
    </table>
   ...
   for (int i=0; i< tenori.length;i++)
    ...
    %>
		<tr>
	        ...
	        <td class="l"colspan=3 width="45%">
	        	<!-- MERGE v10: aggiunta chiamata a funzione per abilitare disabilitare alcuni campi -->
	           	<select Title="Cod Esito" name="<%=ICostantiTenore.CAMPO_COD_ESITO_TENORE%>" onchange="Javascript: abilitaDisabilitaGiorniSomma();">
	             	<%=esiti[i]%>
	         	 </select>
	        </td>
      	</tr>
	...
	<table cellspacing="3" cellpadding="2" width=95%>
		<tr>
		    <td class="Titolo" colspan="4"> 
		    	Estremi provvedimento di rimedi risarcitori per violazione art. 3 CEDU
		    <td/>
		</tr>
		<tr>
		    <td class="l">Data Emissione <br> (gg-mm-aaaa)</td>
		    <td class="L">
		        <input type="text" size="2" maxlength="2" name="<%=ICostantiDepositoOrdinanzaPc.CAMPO_GIORNO_DATA_TRASMISSIONE%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillDM(value)" > /
		        <input type="text" size="2" maxlength="2" name="<%=ICostantiDepositoOrdinanzaPc.CAMPO_MESE_DATA_TRASMISSIONE%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillDM(value)" > /
		        <input type="text" size="4" maxlength="4" name="<%=ICostantiDepositoOrdinanzaPc.CAMPO_ANNO_DATA_TRASMISSIONE%>" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillYear(value)">
		    </td>
		    <td class="l">Ufficio Sorveglianza  </td>
		    <td class="l">
		        <input Title="Magistrato" name="<%=ICostantiDepositoOrdinanzaPc.CAMPO_COD_UFFICIO_MAGISTRATO_COMP_RECLA %>" size=35 >
		        <a href="Javascript:ListaUDS('InserisciOrdinanzaArt3Cedu','<%=ICostantiDepositoOrdinanzaPc.CAMPO_COD_UFFICIO_MAGISTRATO_COMP_RECLA %>');">
...
<!--  < < < < < <  INSERISCI L.A. - LIB. ANT. NORMALE 45gg 	> > > > > > >	-->
	...
	<!-- MERGE v10: aggiunta div per abilitare disabilitare alcuni campi -->
	<div id="reclamo" style="position: relative; top: 0; left: 0; display:none;">
		<table width="70%">
	    	<tr>
	        	<td class="Titolo" colspan=3 >In Caso di Accoglimento del Reclamo, Indicare: </td>
	    	</tr>
	    </table>
    </div>
