...
<html>
  <head>
    <title>[S.I.E.S.] - Emissione Decreto Ricoveri</title>
...
<%
    ...
    if (Action.trim().length() > 1)
    {
    	lAction = Action;
      lFlagOrdinanza = true;
      lFunctionName = "Emissione Ordinanza Ricoveri";
    }
    else
    {
      lAction = "siap.sius.depositodecreto.action.ActInserisciEmissioneDecretoDeposito";
      lFunctionName = "Emissione Decreto Ricoveri";
    }
%>
  ...
  <body class="corpo">
    ...
    <tr>
			<td class="LBG">
				<a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
  ...
  <FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name="InserisciEmissioneDecretoSosp">
   ...
   for (int i=0; i< tenori.length;i++)
    ...
    <tr><td>&nbsp;</td></tr>
      <tr>
        <td class="l">Tipo di esame clinico o intervento</td>
        <td class="l"><Textarea title="Note" name="<%=ICostantiDepositoDecreto.CAMPO_NOTE%>" cols=80 rows=3></Textarea></td>
      </tr>
    <tr>
      <td class="l">Descrizione struttura ospedaliera</td>
      <td class="l">
        <input Title="Luogo" name="<%= ICostantiDepositoDecreto.CAMPO_LUOGO_SVOLGIMENTO_PROVA %>" value="" size=80 >
      </td>
    </tr>
 </table>
