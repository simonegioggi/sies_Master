...
if (tipo_decreto.compareTo(ICostantiDepositoDecreto.ESCLUSIONE_COMPUTO) == 0) {
	...
	nomeFunzione = "Emissione Decreto Esclusione Computo Licenza" + tipoContenuto;
	nomeCampoGiorni = "Periodo scomputato";
...
<html>
<head>
<title>[S.I.E.S.] - <%=nomeFunzione%></title>
...
function ListaUffici(a_formname, a_fieldname) {
	var desktop = window.open("/jsp/Main.jsp?<%=IWebConstants.ACTION_FIELD%>=siap.sico.ufficio.action.ActLoadRicercaUfficio&formname="+a_formname+"&fieldname="+a_fieldname, "Ricerca_Ufficio","toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=370,height=500");
}
</script>
</head>
<body class="corpo">
<table>
	<tr>
		<td class="LBG">
      	...
      	<td class=LBG>
      		<font class="label">Funzione : </font><font class="campo"><%=nomeFunzione%></font>
      	</td>
	</tr>
    <tr>
...
<table cellspacing="2" cellpadding="2" style="width: 90%;">
	...
	<tr>
        ...
        <td class="l">
			<input Title="<%=labelUfficioProc%>" name="<%=ICostantiDepositoDecreto.CAMPO_COD_UFFICIO_PROCURA_COMP%>" value="<%=comuneProcura%>" type="text" maxlength="35" size="35">
