...
<%
	// Imposta variabili se Decreto o Ordinanza
	...
	if (Action.trim().length() > 1)
	{
		lAction = Action;
  	lFlagOrdinanza = true;
  	lFunctionName = "Emissione Ordinanza Modifica Permanente su Sanzione Sostitutiva";
	}
	else
	{
   	lAction = "siap.sius.depositodecreto.action.ActInserisciEmissioneDecretoDeposito";
    lFunctionName = "Emissione Decreto Modifica Permanente su Sanzione Sostitutiva";
	}
%>
<html>
  <head>
...
<%  
		// Se trattasi di ordinanza, presenta i campi di ulteriore descrizione. Mentre per decreto
		// presenta i campi Motivazione Provvedimento.
		// Il dato Ulteriore Descrizione -> ULTERIORE_DESCRIZIONE
		// Il dato Motivazione Provvedimento -> LUOGO_SVOLGIMENTO_PROVA
    if ( lFlagOrdinanza ) // Test flag ordinanza
    {
%>        
    <tr>
      <td class="l">Ulteriore Descrizione</td>
      <td class="l">
        <Textarea title="Ulteriore Descrizione" name="<%=ICostantiDepositoOrdinanzaPc.CAMPO_ULTERIORE_DESCRIZIONE%>" cols=68 rows=3></Textarea></td>
      </td>
    </tr>
<% 
    }
    else // Trattasi di Decreto
    {
%>
    <tr>
      <td class="l">Motivazione Provvedimento</td>
      <td class="l">
        <Textarea title="Motivazione" name="<%=ICostantiDepositoDecreto.CAMPO_LUOGO_SVOLGIMENTO_PROVA%>" cols=68 rows=3></Textarea></td>
      </td>
    </tr>
<%
    }
