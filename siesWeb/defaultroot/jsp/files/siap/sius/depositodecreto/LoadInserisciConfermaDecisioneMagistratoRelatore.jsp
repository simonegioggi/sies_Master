...
<html>
<head>
<title>[S.I.E.S.] - Emissione Decreto Conferma Decisione Magistrato Relatore</title>
...
%>
	...
	if (document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiMagistrato.CAMPO_COD_MAGISTRATO%>.value == ""
			|| document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiMagistrato.CAMPO_COD_MAGISTRATO%>.value == "-") {
	    alert('Magistrato Relatore obbligatorio!');
	    document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiMagistrato.CAMPO_COGNOME%>.focus();
	    return false;
  	} else {
<%
...
%>
		if (document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiMagistrato.CAMPO_COD_MAGISTRATO%>.value != '<%=codMagistratoOld%>') {
...
%>
	...
	var dataTermineEmissione =
		document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiDepositoDecreto.CAMPO_GIORNO_DATA_TERMINE_EMISSIONE%>.value + '/' +
		document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiDepositoDecreto.CAMPO_MESE_DATA_TERMINE_EMISSIONE%>.value + '/' +
		document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiDepositoDecreto.CAMPO_ANNO_DATA_TERMINE_EMISSIONE%>.value;

	if (dataTermineEmissione != "//") {
	  	if (!ControllaData(dataTermineEmissione)) {
		    alert('Data Termine Emissione non valida!');
		    document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiDepositoDecreto.CAMPO_GIORNO_DATA_TERMINE_EMISSIONE%>.focus();
		    return false;
	  	}
...
String azione = "siap.sius.depositodecreto.action.ActInserisciConfermaDecisioneMagistratoRelatore";
%>
...
<table>
	<tr>
		<td class="LBG">
      	...
      	<td class="LBG"><font class="label">Funzione:</font>&nbsp;
        	<font class="campo">Emissione Decreto Conferma Decisione Magistrato Relatore</font>
      	</td>
	</tr>
    <tr>
    ...
    <tr>
    	<jsp:include page="<%=ICostantiMagistratoRelatore.PG_SINTESIMAGISTRATORELATORE%>">
       		<jsp:param name="MagRelRitorno" value="siap.sius.depositodecreto.action.ActLoadInserisciConfermaDecisioneMagistratoRelatore"/>
    	</jsp:include>
	</tr>
</table>
<jsp:include page="<%=ICostantiAvvocatoFascicoloSius.PG_INCLUDE_AVVOCATI%>">
	<jsp:param name="AvvRitorno" value="siap.sius.depositodecreto.action.ActLoadInserisciConfermaDecisioneMagistratoRelatore"/>
</jsp:include>
...
<!-- Sezione Contenuto Oggetti -->
    	...
    	<td class="l">Oggetto <font class="ob">(*)</font></td>
      	<td class="l" colspan="3">
        	<Textarea title="Oggetto" name="<%=ICostantiFascicoloSius.CAMPO_DESCR_OGGETTO%>" cols=100 rows=3 readonly><%=descOggetti%></Textarea>
 			<a href="Javascript:ListaOggetti('LoadInserisciConfermaDecisioneMagistratoRelatore',document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiFascicoloSius.CAMPO_COD_CONTENUTO%>.value, '<%= ICostantiFascicoloSius.CAMPO_DESCR_OGGETTO %>', '<%=ICostantiFascicoloSius.CAMPO_COD_OGGETTO%>', '<%=ICostantiFascicoloSius.CAMPO_COD_DETTAGLIO_OGGETTO%>', document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiFascicoloSius.CAMPO_COD_OGGETTO%>.value, document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiFascicoloSius.CAMPO_COD_DETTAGLIO_OGGETTO%>.value );">
 				<img src="/images/fileselected.gif" title="Oggetti per il Contenuto selezionato" border="0">
 			</a>
 			&nbsp;
 			<a href="Javascript:ListaOggetti('LoadInserisciConfermaDecisioneMagistratoRelatore','-', '<%= ICostantiFascicoloSius.CAMPO_DESCR_OGGETTO %>', '<%=ICostantiFascicoloSius.CAMPO_COD_OGGETTO%>', '<%=ICostantiFascicoloSius.CAMPO_COD_DETTAGLIO_OGGETTO%>', document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiFascicoloSius.CAMPO_COD_OGGETTO%>.value, document.LoadInserisciConfermaDecisioneMagistratoRelatore.<%=ICostantiFascicoloSius.CAMPO_COD_DETTAGLIO_OGGETTO%>.value );">
	...
	<tr>
		...
		<td class="l"><input value="" type="text" name="<%=ICostantiMagistrato.CAMPO_NOME%>" readonly="readonly" size="25"></td>
	  	<td class="l">
	    	<a href="Javascript:ListaMagistratiRelatori('LoadInserisciConfermaDecisioneMagistratoRelatore');">
