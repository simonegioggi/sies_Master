...
<%
//Hashtable <BigDecimal, Vector <ImpugnazioneModel> > = Hashtable <idEvento,lista impugnazioni/opposizioni dell'evento>
%>
...
<% } else  { %>
  <div align=center>
    <tr>
      ...
      <td class="int" width=14%>Altre Informazioni</td>
...
//      siesLogger.debug("JSP - dataRicorso ="+dataRicorso);
// [FT] - 05/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger()
//      siesLogger.debug("JSP - dataRicorso ="+dataRicorso.length());
%>
    <tr>
        ...
        <% if( numImpugnazioni>0 || numOpposizioni>0 || presenzaFoglio )  { %>
            ...
            <% if (presenzaFoglio ) {%>
            	<%-- MEV NUOVA INFRASTRUTTURA: refactoring --%>
              	...
              	<a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.sius.provvedimento.action.ActLoadDettaglioCompFoglioComp&Provenienza=DettaglioFC&IdEvento=<%=lProv.getIdEvento()%>&CodTipoProvvedimento=<%=StringUtils.toStringJSP(lProv.getCodTipoProvvedimento())%>&TornaQui=<%=0%>" title="Foglio Complementare <%=lEstremiFoglioComp%>"> <img src="/images/dettaglioFC24.gif" width="22" height="22" border="0"> </a>
            <% } %>
        <% } else {%>-<%}%>
      ...
      <td class="c">
       ...
       <td class="c">
        <%
           if (lProv.getFlagDocumentoRegistrato()!=null && lProv.getFlagDocumentoRegistrato().compareTo("S")==0 && lProv.getNumAllValidati() > 0)
           { %>
          <img src="/images/TickRed.gif">
        <% }else{%>
          -
