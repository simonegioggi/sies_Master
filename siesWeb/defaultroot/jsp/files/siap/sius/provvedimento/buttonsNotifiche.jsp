...
%>
              <td>
                <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%><%=retParam%>">
                  <% if(lFun.getNameAction() != null && lFun.getNameAction().contains("ActDettaglioNotificheEventoSige") || lFun.getNameAction() != null && lFun.getNameAction().contains("ActDettaglioNotificheEvento")){ %>
                  		<img src="/images/modificaDif.gif" width="12" height="12" alt="Inserimento Date Notifica" border="0">
                  <% } else { %>
