...
<jsp:useBean id="elencoEsperti"        scope="request" class="java.lang.String"/>
...
<script language="JavaScript">
  ...
  {
    var ritorno = true;
    dataU = document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_GIORNO_DATA_UDIENZA%>.value+'/'+document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_MESE_DATA_UDIENZA%>.value+'/'+document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_ANNO_DATA_UDIENZA%>.value;
    if (dataU.length < 3 || ControllaData(dataU) == false)
    {
      ...
      ritorno = false;
       ...
       if ((document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_COD_PRESIDENTE%>.value != '-')&&
        (document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_COD_PRESIDENTE%>.value == document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_COD_PG%>.value))
    {
  ...
  {
    //document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_MESE_DATA_UDIENZA %>.disabled=true;
    return;
  }
  function SelezionaCollegio( i)
  {
    if (i >= 0)
    if (i < document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_NUM_COLLEGIO%>.options.length)
        document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_NUM_COLLEGIO%>.options[i].selected = true;
    return;
  }
  ...
  function controlloOra(ora, minuti)
  {
   ...
   {
      var ret 			= true;
      var oraInizio = document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_ORA_INIZIO%>.value;    
      var minInizio = document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_INIZIO%>.value;
      var oraFine 	= document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_ORA_FINE%>.value;
      var minFine 	= document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_FINE%>.value;
      ...
      ret = controlloOra(oraInizio, minInizio);
      	...
      	ret = controlloOra(oraFine,minFine);
			...
			{
      	document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_INIZIO%>.value = "00";
      	minInizio = document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_INIZIO%>.value;
      }
			// Se il campo OraFine è valorizzato e il campo minFine no
			// provvede a valorizzare quest'ultimo con 00      
      if (oraFine != "" && minFine == "")
      {
      	document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_FINE%>.value = "00";
      	minFine = document.LoadInserisciUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_FINE%>.value;
			}
...
%>
    ...
    <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciUdienzaUDS">
    ...
    <tr>
      ...
      <td class="l">
        ...
        <input value="<%=StringUtils.toStringJSP( DateUtils.getDateToString(lUdienza.getDataUdienza(),"yyyy")) %>" type="text" name="<%=ICostantiUdienza.CAMPO_ANNO_DATA_UDIENZA%>" maxlength="4" size="4" <%  if( modalita.equals("M") ) { %> disabled="disabled" <% } %> onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillYear(value)">
            <a href="javascript:calendario('LoadInserisciUdienzaUDS','<%=ICostantiUdienza.CAMPO_ANNO_DATA_UDIENZA%>','<%=ICostantiUdienza.CAMPO_MESE_DATA_UDIENZA%>','<%=ICostantiUdienza.CAMPO_GIORNO_DATA_UDIENZA%>');">
  ...
  <input type="HIDDEN" name="<%=ICostantiUdienza.CAMPO_CHECK_INS_FISS_UDIENZA%>" value="<%=checkInsFissUdienza%>" >
    ...
    var frmvalidator  = new Validator("LoadInserisciUdienzaUDS");
