...
<html>
<head>
<title>[S.I.E.S.] - GestioneCopiaUdienza UDS</title>
...
<script language="JavaScript">
  ...
  {
    var ritorno = true;
    dataU = document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_GIORNO_DATA_UDIENZA%>.value+'/'+document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_MESE_DATA_UDIENZA%>.value+'/'+document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_ANNO_DATA_UDIENZA%>.value;
    if (dataU.length < 3 || ControllaData(dataU) == false)
    {
      alert('Data Udienza è un campo obbligatorio ');
      ritorno = false;
    }
    if (document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_COD_ID_ESPERTO_1%>.value != 0 &&
        (document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_COD_ID_ESPERTO_1%>.value == document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_COD_ID_ESPERTO_2%>.value))
    {
  ...
  {
    if (i >= 0)
    if (i < document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_NUM_COLLEGIO%>.options.length)
        document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_NUM_COLLEGIO%>.options[i].selected = true;
    return;
  }
  ...
  function controlloOra(ora, minuti)
  {
   ...
   {
      var ret 			= true;
      var oraInizio = document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_ORA_INIZIO%>.value;    
      var minInizio = document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_INIZIO%>.value;
      var oraFine 	= document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_ORA_FINE%>.value;
      var minFine 	= document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_FINE%>.value;
      ...
      ret = controlloOra(oraInizio, minInizio);
      	...
      	ret = controlloOra(oraFine,minFine);
			...
			{
      	document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_INIZIO%>.value = "00";
      	minInizio = document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_INIZIO%>.value;
      }
			// Se il campo OraFine è valorizzato e il campo minFine no
			// provvede a valorizzare quest'ultimo con 00      
      if (oraFine != "" && minFine == "")
      {
      	document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_FINE%>.value = "00";
      	minFine = document.LoadInserisciCopiaUdienzaUDS.<%=ICostantiUdienza.CAMPO_MIN_FINE%>.value;
			}


			// Controllo compilazione campi di ora	
...
<%
    ...
    if( modalita.equals("I") )
    {
      ...
      lAzione = "siap.sius.udienza.action.ActInserisciUdienzaUDS";
%>
...
<%
    ...
    else if( modalita.equals("M") )
    {
      ...
      lAzione = "siap.sius.udienza.action.ActModificaUdienzaUDS";
%>
...
%>
    ...
    <font class="campo">Inserimento con copia di un'Udienza (Ufficio di Sorveglianza)</font>
<%
    }
%>
    </td>
    ...
    <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciCopiaUdienzaUDS">
...
<!--  </tr> -->
    ...
    <tr>
      ...
      <td class="l">
        ...
        <input value="<%=StringUtils.toStringJSP( DateUtils.getDateToString(lUdienza.getDataUdienza(),"yyyy")) %>" type="text" name="<%=ICostantiUdienza.CAMPO_ANNO_DATA_UDIENZA%>" maxlength="4" size="4" <%  if( modalita.equals("M") ) { %> disabled="disabled" <% } %> onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)"  onBlur="javascript:value=FillYear(value)">
            <a href="javascript:calendario('LoadInserisciCopiaUdienzaUDS','<%=ICostantiUdienza.CAMPO_ANNO_DATA_UDIENZA%>','<%=ICostantiUdienza.CAMPO_MESE_DATA_UDIENZA%>','<%=ICostantiUdienza.CAMPO_GIORNO_DATA_UDIENZA%>');">
    ...
    <tr>
      <td class="l">Luogo</td>
      <td class="l"><input value="<%=StringUtils.toStringJSP( lUdienza.getLuogoUdienza(), "" )%>" type="text" name="<%= ICostantiUdienza.CAMPO_LUOGO_UDIENZA %>"  maxlength='99'></td>
    </tr>

    <tr>
      <td class="l">Orario Inizio(ora:min)</td>
      <td class="l">
      	<input value="<%=StringUtils.toStringJSP( lUdienza.getOraInizio(), "" )%>" type="text" name="<%=ICostantiUdienza.CAMPO_ORA_INIZIO%>" size="2" maxlength="2" onBlur="javascript:value=FillDM(value)">
      	:
      	<input value="<%=StringUtils.toStringJSP( lUdienza.getMinInizio(), "" )%>" type="text" name="<%=ICostantiUdienza.CAMPO_MIN_INIZIO%>" size="2" maxlength="2" onBlur="javascript:value=FillDM(value)">
      </td>
    </tr>
    <tr>
      <td class="l">Orario Fine(ora:min)</td>
      <td class="l">
      	<input value="<%=StringUtils.toStringJSP( lUdienza.getOraFine(), "" )%>" type="text" name="<%=ICostantiUdienza.CAMPO_ORA_FINE%>" size="2" maxlength="2" onBlur="javascript:value=FillDM(value)">
      	:
      	<input value="<%=StringUtils.toStringJSP( lUdienza.getMinFine(), "" )%>" type="text" name="<%=ICostantiUdienza.CAMPO_MIN_FINE%>" size="2" maxlength="2" onBlur="javascript:value=FillDM(value)">
      </td>
    </tr>
  ...
  <input type="HIDDEN" value="<%=StringUtils.toStringJSP( lUdienza.getIdUdienza(), "" )%>" name="<%= ICostantiUdienza.CAMPO_ID_UDIENZA %>"  >
    ...
    var frmvalidator  = new Validator("LoadInserisciCopiaUdienzaUDS");
