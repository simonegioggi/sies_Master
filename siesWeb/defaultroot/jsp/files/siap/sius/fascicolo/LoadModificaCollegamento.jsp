...
<%
  ...
  if( modalita.equals("M")  )
  {
  	...
  	lChiaveAnno=fascicoloPadre.getFascicoloSiusModel().getChiaveAnno().toString();
  	lChiaveProgr=fascicoloPadre.getFascicoloSiusModel().getChiaveProgr().toString();
  }
  ...
  aDataNas = fascicoloSiusGP.getFascicoloSiusModel().getSoggetto().getDataNascita().toString();
	if(fascicoloPadre!=null && fascicoloPadre.getFascicoloSiusModel()!=null && fascicoloPadre.getFascicoloSiusModel().getSoggetto()!=null &&  fascicoloPadre.getFascicoloSiusModel().getSoggetto().getIdSoggetto()!=null)
  {
	  aCognomePadre = fascicoloPadre.getFascicoloSiusModel().getSoggetto().getCognome();
  	aNomePadre = fascicoloPadre.getFascicoloSiusModel().getSoggetto().getNome();
  	aComuneNasPadre = fascicoloPadre.getFascicoloSiusModel().getSoggetto().getDescrComuneNascita();
  	aDataNasPadre = fascicoloPadre.getFascicoloSiusModel().getSoggetto().getDataNascita().toString();
  }
...
<html>
  <head>
    ...
    <script language="JavaScript">
      ...
      function VerifyControlli(aProvenienza)
      {
        ...
        if ( document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_ANNO%>.value.length<4 || document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_ANNO %>.value<1900 || isNaN(document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_ANNO%>.value) )
        {
          alert ("Anno Fascicolo SIUS Non Valido");
          document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_ANNO%>.focus();
          return false;
        }
        if ( document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_PROGR%>.value.length<=0 || document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_PROGR %>.value<0 || isNaN(document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_PROGR%>.value) )
        {
          alert ("Numero Fascicolo SIUS Non Valido");
          document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_CHIAVE_PROGR%>.focus();
          return false;
        }
        // Controllo obbligatorietà campi relativi all'autorità.
        if (document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_COD_MITTENTE_ATTO%>.value=="-")
        {
          alert("Il tipo autorità è un campo obbligatorio");
          document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_COD_MITTENTE_ATTO%>.focus();
          return false;
        }
        if (document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_DESCR_SEDE_MITTENTE%>.value=="")
        {
          alert("La sede dell'autorità è un campo obbligatorio");
          document.LoadModificaCollegamento.<%=ICostantiFascicoloSius.CAMPO_DESCR_SEDE_MITTENTE%>.focus();
          return false;
        }
      ...
      function Ricerca()
      {
      	if (VerifyControlli("D"))
        {
        	...
        	document.LoadModificaCollegamento.<%=IWebConstants.ACTION_FIELD%>.value="siap.sius.fascicolo.action.ActRicercaCollegamento"
      		return true;
        }else{
      		return false;
        }
      ...
      function Modifica()
      {
        ...
        document.LoadModificaCollegamento.<%=IWebConstants.ACTION_FIELD%>.value="siap.sius.fascicolo.action.ActModificaCollegamento";
        VerifyConferma();
        return true;
  		}
  </script>
  ...
  <FORM method="POST" name="LoadModificaCollegamento" action="<%=IWebConstants.PG_MAIN%>">
    ...
    <br>
<%
    String lAction = "siap.sius.fascicolo.action.ActModificaCollegamento";
%>
    <table>
      <tr>
      ...
      <tr>
        ...
        <td class="l">
	        <input Title="Anno SIUS" value="<%=lChiaveAnno%>" name="<%=ICostantiFascicoloSius.CAMPO_CHIAVE_ANNO%>"type="text" maxlength="4" size="4">/
  	      <input Title="Numero SIUS" value="<%=lChiaveProgr%>" name="<%=ICostantiFascicoloSius.CAMPO_CHIAVE_PROGR%>"type="text" maxlength="14" size="14">
				</td>
      </tr>
      <tr>
        <td class="l">Autorità</td>
    		<td class="L">
      		<select title="Autorità" class=small name="<%=ICostantiFascicoloSius.CAMPO_COD_MITTENTE_ATTO%>">
        	<%= tipoUfficioSIUS %>
      </select>
      </tr>
      <tr>
        <td class="l">Sede</td>
        <td class="l">
      		<input Title="Sede Autorità" name="<%=ICostantiFascicoloSius.CAMPO_DESCR_SEDE_MITTENTE%>"	value="<%=StringUtils.toStringJSP(fascicoloPadre.getFascicoloSiusModel().getDescrComuneUfficio())%>" type="text" maxlength="35" size="35">
      					<a href="Javascript:ListaComuni('LoadModificaCollegamento','<%=ICostantiFascicoloSius.CAMPO_DESCR_SEDE_MITTENTE%>');">
    ...
    <table cellspacing=2 cellpadding=2>
	  ...
	  <input type="HIDDEN" name="IdFascicoloPadre" value="<%=fascicoloPadre.getFascicoloSiusModel().getIdFascicoloSius()%>" >
		</form>
  </body>
</html>