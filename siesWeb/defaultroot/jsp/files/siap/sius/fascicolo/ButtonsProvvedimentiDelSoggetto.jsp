...
%>
              <td>
                <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%>&dCognome=<%=request.getParameter("dCognome")%>&dNome=<%=request.getParameter("dNome")%>&dDataNascita=<%=request.getParameter("dDataNascita")%>&hufUtConnesso=<%=ufUtConnesso%>&hufOTribunale=<%=ufOTribunale%>&hcodDistretto=<%=codDistretto%>&hlIncludeArchiviati=<%=lIncludeArchiviati%>&hcodContenuto=<%=codContenuto%>&hdescrContenuto=<%=descrContenuto%>&htipoUfficio=<%=tipoUfficio%>&hdataDalInCancelleria=<%=dataDalInCancelleria%>&hdataAlInCancelleria=<%=dataAlInCancelleria%><%=retParam%>">
                  <img src="/images/dettagli.gif" width="12" height="12" alt="Elenco Procedimenti del Soggetto" border="0">
                </a>
              </td>
<%
            }
