...
<%
String lNota = "N.B.: Di norma vengono visualizzati i soggetti con procedimenti pendenti di competenza dell'ufficio senza limitazione del periodo di pervenimento in cancelleria. Per variare i criteri selezionare una o più delle seguenti opzioni:";
String lCheck ="";

if (TipoUfficioConnesso.substring(1,3).compareTo("DS") != 0) {
	lNota = "N.B.: Di norma vengono visualizzati i soggetti con procedimenti pendenti.  Per variare i criteri selezionare una o più delle seguenti opzioni:";
	lCheck="CHECKED";
}
%>
<html>
...
function Verify() {
	var data_to_verify = document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_GIORNO_DATA_NASCITA%>.value + '/'
		+ document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_MESE_DATA_NASCITA%>.value + '/'
		+ document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_ANNO_DATA_NASCITA%>.value;
	if (document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_COGNOME%>.value.length < 2
			&& document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_NOME%>.value == ""
			&& document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_GIORNO_DATA_NASCITA%>.value.length == 0
			&& document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_MESE_DATA_NASCITA%>.value.length == 0
			&& document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_ANNO_DATA_NASCITA%>.value.length == 0
			&& document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_COD_COMUNE_NASCITA%>.value == ""
			&& document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_COD_STATO_NASCITA%>.value == "-"
			&& document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_PATERNITA%>.value ==""
			<%-- 20190301 [SG]: modifica per il cui; NO CAMPO_COD_CS SI CAMPO_COD_AFIS --%>
			&& document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_COD_AFIS%>.value == "") {
 	        alert("Inserire il Cognome del Soggetto (almeno 2 lettere)");
 	        document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiSoggetto.CAMPO_COGNOME%>.focus();
...
<body class="corpo">
<FORM method="POST" action="<%=IWebConstants.PG_MAIN%>" name="LoadRicercaSoggettiConProcedimenti">
<input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sius.fascicolo.action.ActRicercaSoggettiConProcedimenti">
<table>
	<tr>
		<td class="LBG">
		...
		<td class="LBG"><font class="label">Funzione:</font>&nbsp;<font class="campo">Ricerca Soggetti con Procedimenti di Sorveglianza</font></td>
	</tr>
</table>
<br>
<table cellspacing="2" cellpadding="2">
      ...
      <tr>
        ...
        <td class="l">
          ...
          <a href="Javascript:ListaComuni('LoadRicercaSoggettiConProcedimenti','<%= ICostantiSoggetto.CAMPO_COD_COMUNE_NASCITA %>');">
...
<table cellspacing="2" cellpadding="2">
      <tr>
		<td class="Cliccabile">Modifica competenza territoriale dei procedimenti visualizzati</td>
      </tr>
<%
if ("DS".equals(TipoUfficioConnesso.substring(1,3))) {
%>
      <tr>
        <td class="label" >
<%
	if ("UDS".equals(TipoUfficioConnesso) || "UDSM".equals(TipoUfficioConnesso)) {
%>
...
%>
        ...
        <td class="label" >
          	<input type=radio name="<%=ICostantiFascicoloSius.CAMPO_INCLUDE_UFFICIO%>" value="0" CHECKED>
        </td>
      </tr>
...
<%
	if ("UDS".equals(TipoUfficioConnesso) || "UDSM".equals(TipoUfficioConnesso)) {
%>
            Visualizza anche i procedimenti del Tribunale (Sede)
<%
	} else {
%>
...
%>
        ...
        <td class="label" >
          	<input type=radio name="<%=ICostantiFascicoloSius.CAMPO_INCLUDE_UFFICIO%>" value="1">
        </td>
      </tr>
<%
}
%>
      <tr>
        <td class="label">Visualizza i procedimenti dell' intero Distretto</td>
        <td class="label">
          	<input type=radio name="<%=ICostantiFascicoloSius.CAMPO_INCLUDE_UFFICIO%>" value="2" <%=lCheck%>>
        </td>
      </tr>

      <tr>
        <td class="label">Visualizza anche i procedimenti pervenuti da altri Distretti</td>
        <td class="label">
          	<input type=radio name="<%=ICostantiFascicoloSius.CAMPO_INCLUDE_UFFICIO%>" value="3">
        </td>
      </tr>
      ...
      <tr>
        ...
        <td class="label">
          	<input type=checkbox name="<%=ICostantiFascicoloSius.CAMPO_INCLUDE_ARCHIVIATI%>" value="1" onclick="javascript:document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiFascicoloSius.CAMPO_SOLO_ARCHIVIATI%>.checked=false">
        </td>
      </tr>

      <tr>
        <td class="label">Visualizza solo i procedimenti definiti</td>
        <td class="label">
          	<input type=checkbox name="<%=ICostantiFascicoloSius.CAMPO_SOLO_ARCHIVIATI%>" value="1" onclick="javascript:document.LoadRicercaSoggettiConProcedimenti.<%=ICostantiFascicoloSius.CAMPO_INCLUDE_ARCHIVIATI%>.checked=false">
        </td>
      </tr>
...
<table cellspacing="2" cellpadding="2">
  ...
  <script language="JavaScript" type="text/javascript">
    ...
    var frmvalidator  = new Validator("LoadRicercaSoggettiConProcedimenti");
