...
<jsp:useBean id="PeriodoAltraSanzione" scope="request"
	class="siap.sius.sanzionesostitutiva.model.PeriodoAltraSanzioneModel" />
...
<jsp:useBean id="lESSModel" scope="request"
	class="siap.sius.esecuzionesanzionesostitutiva.model.EsecuzioneSanzioneSostitutivaModel" />

<html>
<head>
<title>[S.I.U.S.] - Valida Inizio Sanzioni Sostitutive</title>
...
<table>
	<tr>
		...
		<td class=LBG><font class="label">Funzione : </font>&nbsp;
 			<font class="campo">Valida Inizio Sanzioni Sostitutive</font></td>
	</tr>
...
<table>
	<tr>
		...
		<td class="l"><font class="campo"> <%=StringUtils.toStringJSP(DateUtils.getDateToString(
							PeriodoAltraSanzione.getDataInizioEsecuzione(),
							"dd/MM/yyyy"))%></font></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td class="l"><font class="campo"> <%=StringUtils.toStringJSP(PeriodoAltraSanzione
								.getDescrTipoAutorita())%> di <%=StringUtils.toStringJSP(PeriodoAltraSanzione
...
<FORM method="POST" action="<%=IWebConstants.PG_MAIN%>"
	name='LoadValidaInizioSanzioneSostitutivaUDS'>
	...
	<tr>
      <td class="l">Data Fine Pena <font class="ob">(*)</font></td>
     	<td class="L">
			...
			<input value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(lESSModel
					.getDataTermineAttuale(), "dd") )%>" type="text" size="2" maxlength="2"
					 name="<%= ICostantiSanzioneSostitutiva.CAMPO_GIORNO_DATA_SCADENZA%>" 
					 <%=IWebConstants.UTIL_DATA%>>
				/ 
			<input value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(lESSModel
					.getDataTermineAttuale(), "MM") )%>" type="text" size="2" maxlength="2"
					name="<%= ICostantiSanzioneSostitutiva.CAMPO_MESE_DATA_SCADENZA%>"
					<%=IWebConstants.UTIL_DATA%>>
				/ 
			<input value="<%=StringUtils.toStringJSP(DateUtils.getDateToString(lESSModel
					.getDataTermineAttuale(), "yyyy") )%>" type="text" size="4" maxlength="4" 
					name="<%= ICostantiSanzioneSostitutiva.CAMPO_ANNO_DATA_SCADENZA %>" 
      ...
      <tr>
        <td>
          ...
          <input type="HIDDEN" name="<%=ICostantiSanzioneSostitutiva.CAMPO_ID_PERIODO_ALTRA_SANZIONE%>"
           value="<%=PeriodoAltraSanzione.getIdPeriodoAltraSanzione()%>" >
          <input type="HIDDEN" name="data_sanzione" 
