...
<title>[S.I.U.S.] - Inizio Sanzione Sostitutiva</title>
...
<table>
	<tr>
		...
		<td class=LBG><font class="label">Funzione : </font>&nbsp; <font class="campo">inizio/ripresa sanzione sostitutiva</font></td>
	</tr>
</table>
<br>
...
%>
				<input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>"
					value="siap.sius.sanzionesostitutiva.action.ActModificaInizioSanzioneSostitutivaUDS">
...
<%
				if (listaSanzioniSius.size() > 1) { PeriodoAltraSanzioneModel lPerMod1 = (PeriodoAltraSanzioneModel) listaSanzioniSius.get(listaSanzioniSius.size() - 2);
%>
...
%> </td>
		...
		<td class="L"> Quantum Sanzione Sostitutiva :
...
<%			}%>
			...
			name="<%=ICostantiSanzioneSostitutiva.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE%>"
<%			if (istitutodetenzione != null && istitutodetenzione.getIdIstitutoDetenzione().trim().length() > 0) {%>
...
<%			}%>
	...
	<tr>
		...
		<td class="l"><select title="Destinatario"
			name="<%=ICostantiSanzioneSostitutiva.CAMPO_COD_TIPO_AUTORITA%>">
			<%=tipoAutorita%>
		</select></td>
	</tr>
	<tr>
