...
int contaRegInde = 0;
    ...
    while ( itx.hasNext())
    {
      ...
      if ("NO".equals(lAvvocato.getFlagRegInde())) {
         ...
         contaRegInde++;
         strAlertAvvocatoReginde+= " L'anagrafica dell'Avvocato "+StringUtils.toStringJSP(lAvvocato.getCognome())+" "
                           +StringUtils.toStringJSP(lAvvocato.getNome())
...
%>
       <tr <%=attributeForo%> <%=attributeForoReginde%>>
         <td class="L" colspan=1 width=60%>
          <font class="campo"><%=StringUtils.toStringJSP(lAvvocato.getCognome()) +" "+ StringUtils.toStringJSP(lAvvocato.getNome()) %></font><%=lSoppresso%><%=lCertRegInde%>
         </td>
