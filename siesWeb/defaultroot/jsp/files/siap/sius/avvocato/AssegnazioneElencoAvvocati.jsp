...
if(document.LoadRicercaAvvocato.tipo.checked==false)
 {
   ...
   if(id==1)
  {
          document.LoadRicercaAvvocato.<%=IWebConstants.ACTION_FIELD%>.value = "siap.sius.avvocato.action.ActDeassegnaDifensore";

  }else if(id==2)
  {

          document.LoadRicercaAvvocato.<%=IWebConstants.ACTION_FIELD%>.value = "siap.sius.avvocato.action.ActLoadSostituzioneDifensore";

  }else if(id==3)
  {
          document.LoadRicercaAvvocato.<%=IWebConstants.ACTION_FIELD%>.value = "siap.sius.avvocato.action.ActLoadInserisciAssegnaAvvocato";

  } /*else if(id==4)
  {
          document.LoadRicercaAvvocato.<%=IWebConstants.ACTION_FIELD%>.value = "siap.sius.reato.action.ActLoadInserisciReato";
  } */
...
<%}%>
 ...
 int i =0;
	...
	<tr>
        ...
        <td class=c><%=StringUtils.toStringJSP(DateUtils.getDateToString(lAvv.getAvvocatoFascicoloSiusModel().getDataInizioValidita(),"dd-MM-yyyy"))%></td>
        <td class="c"><input type="radio" name="tipo" value="<%=lAvv.getAvvocato().getIdAvvocato()%>" >
        <jsp:include page="<%=ICostantiAvvocatoFascicoloSius.PG_BUTTONS_AVVOCATO%>">
