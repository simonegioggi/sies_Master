...
<%@ page import="f3b.util.DateUtils"%>
 ...
 <% 
 // Il default è ricerca x Estremi Foglio Complementare
 String testo1 = "Estremi Foglio Complementare";
 String testoDate = "Date Emissione";
 String valoreDate = ICostantiStatistiche.DATA_EMISSIONE_INTERVALLO;
 String testo2 = "Tutti";
 String testo3 = "Validati";
 String testo4 = "Non Validati";
 String testo5 = "Annullati"; 		
 boolean isFoglioComplementare = true;
 %>
  
  <script language="JavaScript" src="<%=ICostantiStatistiche.RICERCA_ORDINANZA_JS%>"></script>
  <form method="POST" action="<%=IWebConstants.PG_MAIN%>" name="RicercaFCTrasmessi" >
    <table  width=96%>
    ...
    <br>
    <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sius.statistiche.action.ActRicercaFCTrasmessi">
    <input type="HIDDEN" name="<%=ICostantiStatistiche.CAMPO_MODALITA_RICERCA%>" value="<%=modalitaRicerca%>">
  </FORM>
 
 
   <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("RicercaFCTrasmessi");
  ...
  {
    var ritorno = true;
	var data_iniziale = document.RicercaFCTrasmessi.<%=ICostantiStatistiche.CAMPO_GIORNO_DATA_DEPOSITO_INI%>.value + '/' + document.RicercaFCTrasmessi.<%=ICostantiStatistiche.CAMPO_MESE_DATA_DEPOSITO_INI%>.value + '/'+ document.RicercaFCTrasmessi.<%=ICostantiStatistiche.CAMPO_ANNO_DATA_DEPOSITO_INI%>.value;
			
	var data_finale = document.RicercaFCTrasmessi.<%=ICostantiStatistiche.CAMPO_GIORNO_DATA_DEPOSITO_FINE%>.value + '/' + document.RicercaFCTrasmessi.<%=ICostantiStatistiche.CAMPO_MESE_DATA_DEPOSITO_FINE%>.value + '/'+ document.RicercaFCTrasmessi.<%=ICostantiStatistiche.CAMPO_ANNO_DATA_DEPOSITO_FINE%>.value;
