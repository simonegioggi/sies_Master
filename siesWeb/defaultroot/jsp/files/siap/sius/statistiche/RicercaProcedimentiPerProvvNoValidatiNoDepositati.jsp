...
<html>
  <head>
    ...
    <title>[S.I.E.S.] - Procedimenti per provvedimenti Non validati no depositati </title>
	  ...
	  <table>
	    <tr>
	       ...
	       <td class="LBG">
	       	 ...
	       	 <font class="campo">Elenco Procedimenti Per Provvedimenti non validati/depositati</font>
	       </td>
	       <jsp:include page="<%=IWebConstants.PG_RETURN_BUTTON%>"/>
	       <!-- NUOVO BOTTONE PER STAMPA EXCEL --> 
					<td class=l>
						<a class="cliccabile" 
							 href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.sius.statistiche.action.ActRicercaProcPerProvvNoValidatiNoDepositoExcel">
...
<% 			if( ricercaProcedimenti.getStatoProcedimento() == 0 ) { %>
        	<tr>
          	<td class="lVerdeNB">Privi di Provvedimenti </td>
        	</tr>
<%    	} else if ( ricercaProcedimenti.getStatoProcedimento() == 1 ) { %>
        	<tr>
          	<td class="lVerdeNB">Emessi ma non validati </td>
        	</tr>
<%    	} else if ( ricercaProcedimenti.getStatoProcedimento() == 2 ) { %>
        	<tr>
          	<td class="lVerdeNB">Provvedimenti non depositati </td>
        	</tr>
<%    	} else if ( ricercaProcedimenti.getStatoProcedimento() == 3 ) { %>
        	<tr>
          	<td class="lVerdeNB">Provvedimenti depositati non validati </td>
        	</tr>
<%			}	%>
...
%>          	
        ...
        <td class="c">
        	<font class="label">
        		<%=StringUtils.toStringJSP(DateUtils.getDateToString(procedimento.getGeneraleProcedimento().getDataCameraConsiglio() ,"dd-MM-yyyy"),"-")%>
        	</font>
        </td>
        <td class="c">
...
<% 
				...
				if( procedimento.getDocumentoAllegato() != null && procedimento.getDocumentoAllegato().getDataEmissione() != null)
					lDataDeposito = 
						StringUtils.toStringJSP(DateUtils.getDateToString(procedimento.getDocumentoAllegato().getDataEmissione(), "dd-MM-yyyy"),"-") ;				
%>      
        	<font class="label"><%=lDataDeposito%></font>
        </td>
