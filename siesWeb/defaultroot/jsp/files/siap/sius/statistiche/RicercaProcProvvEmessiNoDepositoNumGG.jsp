...
<html>
  <head>
    ...
    <title>[S.I.E.S.] - Ricerca procedimenti pendenti con provvedimenti emessi non depositati entro un determinato numero di giorni </title>
	  ...
	  <table>
	    <tr>
	       ...
	       <td class="LBG">
	       	 ...
	       	 <font class="campo"> Elenco procedimenti pendenti con provvedimenti emessi non depositati entro un determinato numero di giorni </font>
	       </td>
	       <jsp:include page="<%=IWebConstants.PG_RETURN_BUTTON%>"/>
	       <!-- NUOVO BOTTONE PER STAMPA EXCEL --> 
					<td class=l>
						<a class="cliccabile" href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=siap.sius.statistiche.action.ActRicercaProcProvvEmessiNoDepositoNumGGExcel">
...
<% 		if( ricercaProcedimenti.getDataEmissioneInizio() != null || ricercaProcedimenti.getDataEmissioneFine() != null ) { %>
        	<tr>
          		<td class="lVerdeNB">Procedimenti con Data Emissione Provvedimento :&nbsp;&nbsp;
<%					if( ricercaProcedimenti.getDataEmissioneInizio() != null ) { %>
            			Dal <%=DateUtils.getDateToString(ricercaProcedimenti.getDataEmissioneInizio(), "dd-MM-yyyy")%>&nbsp;&nbsp;
<%      			} %>          
<% 					if( ricercaProcedimenti.getDataEmissioneFine() != null ) { %>
            			Al&nbsp;&nbsp;<%=DateUtils.getDateToString(ricercaProcedimenti.getDataEmissioneFine(), "dd-MM-yyyy")%>
<%        			} %>
          		</td>
       		</tr>
<%      } %>
...
<% 				if(ricercaProcedimenti.getNumeroGiorni() != null) { %>
          			con numero giorni trascorsi dalla data di Emissione Provvedimento maggiore o uguale a <%=ricercaProcedimenti.getNumeroGiorni()%>
<%    			} %>
...
%>
  <table cellspacing="2" cellpadding="2">
    <tr>
      ...
      <td class="int">N.ro Giorni</td>
      <td class="int">Azione</td>
    </tr>
<%
	RedirectTo lRedirect = null;
	EveFasGepSogProvModel procedimento;
   	while ( itx.hasNext()) {
	   	procedimento = (EveFasGepSogProvModel)itx.next();
%>
      <tr>
        <td class="c">
        	<font class="label">
        ...
        <td class="c">
        	<font class="label"><%=StringUtils.toStringJSP(DateUtils.getDateToString(procedimento.getEvento().getDataEmissione() ,"dd-MM-yyyy"),"-")%></font>
        </td>
        <td class="c">
