...
<%@ page import="f3b.util.DateUtils"%>
 ...
 <% 
 // Il default è ricerca Provvedimenti privi di Foglio Complementare
 String testo1 = "Estremi Provvedimento";
...
%>
  ...
  <form method="POST" action="<%=IWebConstants.PG_MAIN%>" name="RicercaProvvedimentiPriviDiFC" >
    <table  width=96%>
    ...
    <table width=96% >
      <tr>
      	...
      	<td class="c" width="60%" >
            <%=testo1%> <input type="radio" name="<%=ICostantiStatistiche.CAMPO_TIPO_INTERVALLO%>" value="<%=ICostantiStatistiche.ESTREMI_PROVVEDIMENTO_INTERVALLO %>"  onClick="VisualizzaEstremiProvvedimento();"  checked >&nbsp;
    ...
    <div id="EstremiProvvedimento" style="position:relative;  top: 0; left: 0; " >  
      <table  width=95%>
      ...
      <tr>
        ...
        <td class="l">
          <input type="text" title="Anno Provvedimento Iniziale" name="<%=ICostantiStatistiche.CAMPO_ANNO_INI %>" maxlength="4" size="4" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)">/
          <input type="text" title="Numero Provvedimento Iniziale" name="<%=ICostantiStatistiche.CAMPO_NUM_INI %>" maxlength="6" size="6" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)">
        </td>
        <td class="c" >
          <font class="label">Anno/Numero Finale (*) </font>
        </td>
        <td class="l">
          <input type="text" title="Anno Provvedimento Finale" name="<%=ICostantiStatistiche.CAMPO_ANNO_FINE %>" maxlength="4" size="4" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)">/
          <input type="text" title="Numero Provvedimento Finale" name="<%=ICostantiStatistiche.CAMPO_NUM_FINE %>" maxlength="6" size="6" onFocus="javascript:textboxSelect(this)" onkeypress="return TicTabNumField(this,event)">
        </td>
        </tr>      
         </table>
   </div>     
    ...
    <br>
    <input type="HIDDEN" name="<%=IWebConstants.ACTION_FIELD%>" value="siap.sius.statistiche.action.ActRicercaProvvedimentiPriviDiFC">
    <input type="HIDDEN" name="<%=ICostantiStatistiche.CAMPO_MODALITA_RICERCA%>" value="<%=modalitaRicerca%>">
  </FORM>
 
 
   <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("RicercaProvvedimentiPriviDiFC");

	// Viene usata questa funzione per definire i campi da controllare che cambiano dalle opzioni scelte.
     function Validazione() {
      	if (isEstremiProvvedimento()) {
...
function VerificaDateDeposito() {
    var ritorno = true;
	var data_iniziale = document.RicercaProvvedimentiPriviDiFC.<%=ICostantiStatistiche.CAMPO_GIORNO_DATA_DEPOSITO_INI%>.value + '/' + document.RicercaProvvedimentiPriviDiFC.<%=ICostantiStatistiche.CAMPO_MESE_DATA_DEPOSITO_INI%>.value + '/'+ document.RicercaProvvedimentiPriviDiFC.<%=ICostantiStatistiche.CAMPO_ANNO_DATA_DEPOSITO_INI%>.value;
	var data_finale = document.RicercaProvvedimentiPriviDiFC.<%=ICostantiStatistiche.CAMPO_GIORNO_DATA_DEPOSITO_FINE%>.value + '/' + document.RicercaProvvedimentiPriviDiFC.<%=ICostantiStatistiche.CAMPO_MESE_DATA_DEPOSITO_FINE%>.value + '/'+ document.RicercaProvvedimentiPriviDiFC.<%=ICostantiStatistiche.CAMPO_ANNO_DATA_DEPOSITO_FINE%>.value;
