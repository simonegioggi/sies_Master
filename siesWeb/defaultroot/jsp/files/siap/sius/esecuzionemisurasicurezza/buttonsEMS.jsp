...
<%
      ...
      if( (lFunFiglie != null) && (lFunFiglie.size() != 0) )
      {
        ...
        while(lIterBottoni.hasNext())
        {
          ...
          {
            String idSoggetto = request.getParameter(ICostantiEsecuzioneMS.CAMPO_ID_SOGGETTO);
            String idFascicoloSIUS = request.getParameter(ICostantiFascicoloSius.CAMPO_ID_FASCICOLO_SIUS);
            String idFascicoloSIEP = request.getParameter(ICostantiEsecuzioneMS.CAMPO_ID_FASCICOLO_SIEP);
            if(lFun.getFunctionType().equals(ICostantiFunzioni.TIPO_DETTAGLIO))
            {
%>
              <td>
                <a href="<%=IWebConstants.PG_MAIN%>?<%=IWebConstants.ACTION_FIELD%>=<%=lFun.getNameAction()%>&<%=request.getParameter(ICostantiSecurity.CAMPO_ID_ENTITA)%>=<%=request.getParameter(ICostantiSecurity.VALORE_ID_ENTITA)%>&<%=ICostantiEsecuzioneMS.CAMPO_ID_SOGGETTO%>=<%=idSoggetto%>&<%=ICostantiEsecuzioneMS.CAMPO_ID_FASCICOLO_SIEP%>=<%=idFascicoloSIEP%>&<%=ICostantiFascicoloSius.CAMPO_ID_FASCICOLO_SIUS%>=<%=idFascicoloSIUS%>&TornaQui=<%=TornaQui%>">
