...
//siesLogger.debug("JSP: "+evento.getEvento());
...
<%
            String strTitle = "";
            String strObject = "";
            if (Messaggio.getCodTipoOperazione().compareTo(Messaggio.TRASFERIMENTO_ISTANZA)==0)
            {
              strTitle="Dettaglio Istanza Ricevuta";
              strObject="Oggetto dell'Istanza";
            }
            else if (Messaggio.getCodTipoOperazione().compareTo(Messaggio.TRASFERIMENTO_PROVVEDIMENTO)==0)
            {
              strTitle="Dettaglio Provvedimento Ricevuto";
              strObject="Oggetto del Provvedimento";
            }
            else if (Messaggio.getCodTipoOperazione().compareTo(Messaggio.TRASFERIMENTO_SANZIONE_SOSTITUTIVA)==0)
            {
              strTitle="Dettaglio Sanzione Sostitutiva Ricevuta";
              strObject="Oggetto della Sanzione";
            }
            else if (Messaggio.getCodTipoOperazione().compareTo(Messaggio.TRASFERIMENTO_ATTI_CONVERSIONE)==0)
            {
              strTitle="Dettaglio Richiesta Conversione Pene Pecuniarie Ricevuta";
              strObject="Oggetto del Provvedimento";
            }
            else if (Messaggio.getCodTipoOperazione().compareTo(Messaggio.TRASFERIMENTO_RICHIESTA_ACCERTA_PERICOLO_SOCIALE)==0)
            {
              strTitle="Dettaglio Richiesta Accertamento Pericolosità Sociale";
              strObject="Oggetto del Provvedimento";
            }
            else if (Messaggio.getCodTipoOperazione().compareTo(Messaggio.RICHIESTA_CESSAZIONE_MISURA)==0)
            {
              strTitle="Dettaglio Richiesta Cessazione Misura Per Sopravvenienza Nuovo Titolo Esecutivo (ex Art. 51 Bis)";
              strObject="Oggetto del Provvedimento";
            }
            else if (Messaggio.getCodTipoOperazione().compareTo(Messaggio.RICHIESTA_PROSECUZIONE_MISURA)==0)
            {
...
<!----------- SOGGETTO --------------------->
 <tr><td class="Titolo" colspan=4><%=strObject%></td></tr>
      <tr>
...
%>
        ...
        <tr><td>&nbsp;</td></tr>
          <tr>
            ...
            <td class="l"><font class="campo"><%=StringUtils.toStringJSP(richiestaconversione.getAnnoPartita()) %></font>&nbsp;
                   &nbsp;/&nbsp;<font class="campo"><%=StringUtils.toStringJSP(richiestaconversione.getNumPartita()) %></font>&nbsp;</td>
          </tr>
          <tr>
        ...
        <tr>
            ...
            <td class="l"><font class="campo"><%=StringUtils.toEuroFormat(richiestaconversione.getImportoMulta()) %></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                          Data Prescrizione
        ...
        <tr>
            ...
            <td class="l"><font class="campo"><%=StringUtils.toEuroFormat(richiestaconversione.getImportoAmmenda()) %></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                          Data Prescrizione     
...
if (  Messaggio.getCodTipoOperazione().compareTo(Messaggio.TRASFERIMENTO_RICHIESTA_ACCERTA_PERICOLO_SOCIALE)==0
    && misureSicurezza!=null
    && misureSicurezza.size()>0
    )
{
%>
