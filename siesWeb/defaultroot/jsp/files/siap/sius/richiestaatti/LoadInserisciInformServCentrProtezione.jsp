...
<jsp:useBean id="sedeProcura"       scope="request" class="java.lang.String"/>

<%
  // Azione da chiamare per l'inserimento dei dati.
  String lAzione = "siap.sius.richiestaatti.action.ActInserisciInformServCentrProtezione";
%>
...
<html>
<head>
  <title>[S.I.E.S.] - Informazioni Servizi Centro Protezione</title>
  ...
  <script language="JavaScript" src="<%=IWebConstants.JS_VALIDATOR%>"></script>
    ...
    function Verify()
    {
      if (document.LoadInserisciInformServCentrProtezione.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciInformServCentrProtezione.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciInformServCentrProtezione.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciInformServCentrProtezione.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciInformServCentrProtezione.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciInformServCentrProtezione.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciInformServCentrProtezione.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciInformServCentrProtezione.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciInformServCentrProtezione.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
...
<body onLoad="document.forms[0].elements[0].focus()" class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Informazioni Servizi Centro Protezione</font>
      </td>
      <td class="LBG">
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciInformServCentrProtezione">
    ...
    <col width="35%"></col>
      ...
      <tr>
        ...
        <td class="l" colspan="3"><input type="text" Title="Destinatario" name="<%=ICostantiRichiestaAtti.CAMPO_DES_DESTINATARIO%>" value="<%=descTipoUfficioS%>" size="80" readonly ></td>
      ...
      <tr>
        ...
        <td class="l" colspan="3">
           <input Title="Sede Procura" name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
              value="<%=sedeProcura%>" type="text" maxlength="35" size="35" readonly>
        </td>
      </tr>
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("LoadInserisciInformServCentrProtezione");
