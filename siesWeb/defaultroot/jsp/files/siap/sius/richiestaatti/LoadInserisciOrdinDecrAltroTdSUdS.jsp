...
<%
  ...
  String lAzione = "siap.sius.richiestaatti.action.ActInserisciOrdinDecrAltroTdSUdS";
...
<script language="JavaScript">
	...
	function abilitaCampiDataRestituzione() {
		if (document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CHECK_DATA_RESTITUZIONE%>.checked){
	   		document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.disabled = false;
	   		document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.disabled = false;
	   		document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.disabled = false;
		}
		else {
	   		document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.disabled = true;
	   		document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.disabled = true;
	   		document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.disabled = true;
		}    	
	}
    ...
    function Verify()
    {
      ...
      if (document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_AGGIUNTIVO%>[0].value == '')
          {
              alert('Estremi del provvedimento è un campo obbligatorio');
              return false;
          }

      // Controlla che le coppie di campi Destinatario/Sede siano riempiti
      if (document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>.value != '-'
          && document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>.value == '')
          {
              alert('La Sede del destinatario è obbligatoria');
              return false;
          }

      if (document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
      ...
      <%-- INIZIO: MEV_9 (D.lgs. 123/2018) --%>
      if (document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CHECK_DATA_RESTITUZIONE%>.checked){
	      var data_restituzione = document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.value
	                        +'/'+ document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.value
	                        +'/'+ document.LoadInserisciOrdinDecrAltroTdSUdS.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.value;
...
<html>
<head>
    <title>[S.I.E.S.] - Richiesta stampa Ordin/Decr. altro TdS UdS</title>
...
<body onLoad="document.forms[0].elements[0].focus()" class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Richiesta stampa Ordin/Decr. altro TdS UdS</font>
      </td>
      <td class="LBG">
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciOrdinDecrAltroTdSUdS">
      ...
      <tr>
        ...
        <td class="l">
           <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
              ...
              <a href="Javascript:ListaUDS('LoadInserisciOrdinDecrAltroTdSUdS','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>');">
              <img src="/images/filefolder.gif" border=0> </a>
        </td>
      </tr>

      <tr>
        <td class="l">Estremi del provvedimento <font class=ob>(*)</font></td>
        <td class="l"><input type="text" Title="Estremi del provvedimento" name="<%=ICostantiRichiestaAtti.CAMPO_AGGIUNTIVO%>" size="40"></td>
      </tr>
    ...
    <script language="JavaScript" type="text/javascript">
      var frmvalidator = new Validator("LoadInserisciOrdinDecrAltroTdSUdS");
