...
source4.style.borderStyle=state
  ...
  <title>[S.I.E.S.] - Generazione Modelli Richiesta Atti </title>
 ...
 <table>
  <tr>
    ...
    <td class="LBG">
      ...
      <font class="campo">Generazione Modelli Richiesta Atti</font>
    </td>
   </tr>
 </table>
</FORM>
...
%>
      <tr>
        ...
        <td width="22%" class="menulines">
          <%if (itx.hasNext()) {i++;lFunModel = (FunctionModel)itx.next();%><a href="Javascript:stampa2('<%=ISIAPCostantiWeb.PG_STAMPA%>',  '<%=IWebConstants.ACTION_FIELD%>=<%=lFunModel.getNameAction()%>')"><%=lFunModel.getLabelFunction()%></a><%}else{%><td width="18%"></td> <% }%>
        </td>
        <td width="22%" class="menulines">
          <%if (itx.hasNext()) {i++;lFunModel = (FunctionModel)itx.next();%>  <a href="Javascript:stampa2('<%=ISIAPCostantiWeb.PG_STAMPA%>',  '<%=IWebConstants.ACTION_FIELD%>=<%=lFunModel.getNameAction()%>')"><%=lFunModel.getLabelFunction()%></a><%}else{%><td width="18%"></td> <% }%>
        </td>
