...
<%
  ...
  String lAzione = "siap.sius.richiestaatti.action.ActInserisciSentenzaIntegrale";
...
<html>
<head>
  <title>[S.I.E.S.] - Richiesta Sentenza Integrale</title>
  ...
  <script language="JavaScript" src="<%=IWebConstants.JS_VALIDATOR%>"></script>
    ...
    function abilitaCampiDataRestituzione() {
    	if (document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CHECK_DATA_RESTITUZIONE%>.checked){
	   		document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.disabled = false;
	   		document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.disabled = false;
	   		document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.disabled = false;
    	}
    	else {
	   		document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.disabled = true;
	   		document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.disabled = true;
	   		document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.disabled = true;
    	}    	
   	}
    ...
    function Verify()
    {
      if (document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
      ...
      <%-- INIZIO: MEV_2019-09 (D.lgs. 123/2018) --%>
      if (document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CHECK_DATA_RESTITUZIONE%>.checked){
	      var data_restituzione = document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.value
	                        +'/'+ document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.value
	                        +'/'+ document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.value;
      ...
      if (document.LoadInserisciSentenzaIntegrale.<%=ICostantiRichiestaAtti.CAMPO_AGGIUNTIVO%>[0].value == '')
          {
              alert('Estremi sentenza è un campo obbligatorio');
              return false;
          }
...
<body onLoad="document.forms[0].elements[0].focus()" class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Richiesta stampa Sentenza Integrale</font>
      </td>
      <td class="LBG">
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciSentenzaIntegrale">
    <table cellspacing=2 cellpadding=2>
      ...
      <tr>
        ...
        <td class="l">
           <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
              ...
              <a href="Javascript:ListaUffici('LoadInserisciSentenzaIntegrale','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>');">
      ...
      <tr>
        ...
        <td class="l"><input type="text" Title="Estremi sentenza" name="<%=ICostantiRichiestaAtti.CAMPO_AGGIUNTIVO%>" size="40"></td>
      </tr>
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("LoadInserisciSentenzaIntegrale");
