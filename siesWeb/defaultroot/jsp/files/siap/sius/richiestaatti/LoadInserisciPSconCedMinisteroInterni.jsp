...
<%
  ...
  String lAzione = "siap.sius.richiestaatti.action.ActInserisciPSconCedMinisteroInterni";
%>
...
<html>
<head>
  <title>[S.I.E.S.] - Richiesta P.S.con CED Ministero Interni </title>
  ...
  <script language="JavaScript" src="<%=IWebConstants.JS_VALIDATOR%>"></script>
    ...
    function Verify()
    {
      if (document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
      ...
      // Destinatario campo obbligatorio
      if (document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>.value == '-')
      {
        alert('Destinatario campo obbligatorio');
        return false;
      }
      // Controlla che la coppia di campi Destinatario/Sede sia riempita
      if (document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>.value != '-'
       && document.LoadInserisciPSconCedMinisteroInterni.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>.value == '')
      {
...
<body onLoad="document.forms[0].elements[0].focus()" class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Richiesta P.S. con CED Ministero Interni</font>
      </td>
      <td class="LBG">
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciPSconCedMinisteroInterni">
      ...
      <tr>
        ...
        <td class="l">
           <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
              ...
              <a href="Javascript:ListaComuni('LoadInserisciPSconCedMinisteroInterni','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>');">
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("LoadInserisciPSconCedMinisteroInterni");
