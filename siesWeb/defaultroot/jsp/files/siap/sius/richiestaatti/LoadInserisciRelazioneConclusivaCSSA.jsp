...
<%
  ...
  String lAzione = "siap.sius.richiestaatti.action.ActInserisciRelazioneConclusivaCSSA";
%>
...
<script language="JavaScript">
    ...
    function scegliTipo() {
    	var tipo = document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>.value;
    	if ("B5" == tipo)
    		ListaUSSM('LoadInserisciRelazioneConclusivaCSSA','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>');
    	else
    		ListaUffici('LoadInserisciRelazioneConclusivaCSSA','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>');
    }
...
<html>
<head>
  <title>[S.I.E.S.] - Richiesta Relazione Conclusiva UEPE/USSM</title>
  ...
  <script language="JavaScript" src="<%=IWebConstants.JS_VALIDATOR%>"></script>
    ...
    function Verify()
    {
      if (document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
      ...
      if (document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>.value == '-')
      {
         alert('Il campo Destinatario è obbligatorio');
         return false;
      }

      if (document.LoadInserisciRelazioneConclusivaCSSA.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>.value == '')
      {
...
<body onLoad="document.forms[0].elements[0].focus()" class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Richiesta Relazione Conclusiva</font>
      </td>
      <td class="LBG">
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciRelazioneConclusivaCSSA">
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("LoadInserisciRelazioneConclusivaCSSA");
