...
<%
  ...
  String lAzione = "siap.sius.richiestaatti.action.ActInserisciCertifCampionePenale";
%>
...
<script language="JavaScript">
	function Verify()
  {
  	// Controlla che le coppie di campi Destinatario/Sede siano riempiti
   	if (document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[0].value != '-'
        && document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[0].value == '')
    {
    	alert('La Sede del destinatario n°1 è obbligatoria');
      return false;
    }
    
    if (document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[1].value != '-'
        && document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[1].value == '')
    {
    ...
    if (document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[0].value == '-'
        && document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[1].value == '-')
    {
    	alert('Inserire almeno un Destinatario con la relativa Sede.');
      return false;
    }
      
    if (document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
        document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

    if (document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
         document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciCertifCampionePenale.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
...
<html>
  <head>
    <title>[S.I.E.S.] - Richiesta Certificato Recupero Credito (ex Campione Penale)</title>
  ...
  <body onLoad="document.forms[0].elements[0].focus()" class="corpo">
    <table>
      <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
        <td class="LBG">
          ...
          <font class="campo">Richiesta Certificato Recupero Credito (ex Campione Penale)</font>
        </td>
      <td class="LBG">
    ...
    <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciCertifCampionePenale">
      <table cellspacing=2 cellpadding=2>
        ...
        <tr>
          ...
          <td class="l">
             <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>" value="" type="text" maxlength="35" size="35">
								<a href="Javascript:ListaComuni('LoadInserisciCertifCampionePenale','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[0]');">
        ...
        <tr>
          ...
          <td class="l">
            <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>" value="" type="text" maxlength="35" size="35">
                <a href="Javascript:ListaComuni('LoadInserisciCertifCampionePenale','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[1]');">
                <img src="/images/filefolder.gif" border=0> </a>
          </td>
        </tr>
    ...
    <script language="JavaScript" type="text/javascript">
      var frmvalidator = new Validator("LoadInserisciCertifCampionePenale");
