...
<%
  ...
  String lAzione = "siap.sius.richiestaatti.action.ActInserisciRelazComportVariPeriodi";
%>
...
<html>
<head>
  <title>[S.I.E.S.] - Richiesta Relazione Comportamentale Vari Periodi</title>
  ...
  <script language="JavaScript" src="<%=IWebConstants.JS_VALIDATOR%>"></script>
    ...
    function Verify()
    {
      ...
      if (document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[0].value != '-'
          && document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[0].value == '')
          {
              alert('La Sede del destinatario n°1 è obbligatoria');
              return false;
          }
      if (document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[1].value != '-'
          && document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[1].value == '')
          {
      ...
      if (document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[0].value == '-'
          && document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[1].value == '-')
          {
              alert('Inserire almeno un Destinatario con la relativa Sede.');
              return false;
          }

      if (document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciRelazCompVariPeriodi.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
...
<body onLoad="document.forms[0].elements[0].focus()" class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Richiesta stampa relazione comportamentale vari periodi</font>
      </td>
      <td class="LBG">
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciRelazCompVariPeriodi">
    <table cellspacing=2 cellpadding=2>
        ...
        <select title="Destinatario" name="<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>">
          ...
          <td class="l">
           <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
              ...
              <a href="Javascript:ListaComuni('LoadInserisciRelazCompVariPeriodi','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[0]');">
      ...
      <!-- secondo destinatario + luogo -->
        ...
        <select title="Destinatario" name="<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>">
          ...
          <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
              value="" type="text" maxlength="35" size="35">
              <a href="Javascript:ListaComuni('LoadInserisciRelazCompVariPeriodi','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[1]');">
              <img src="/images/filefolder.gif" border=0> </a>
          </td>
          </tr>
          <tr>
      ...
      <tr>
        <td class="l" colspan="4"> Indicare i periodi di cui si richiede la condotta:</td>
      </tr>
   ...
   <%
    ...
    while( count <  10 )
    {
      %>
      <tr>
        <td class="l">Periodo <%=count%>) </font></td>
        <td class="l"><input type="text" Title="Periodo n° <%=count%>" name="<%=ICostantiRichiestaAtti.CAMPO_AGGIUNTIVO%>" size="40"></td>

        <td class="l">Periodo <%=count + 1%>) </font></td>
        <td class="l"><input type="text" Title="Periodo n° <%=count + 1%>" name="<%=ICostantiRichiestaAtti.CAMPO_AGGIUNTIVO%>" size="40"></td>
      </tr>
     <%
      count+=2;
    }
    ...
    <script language="JavaScript" type="text/javascript">
      var frmvalidator = new Validator("LoadInserisciRelazCompVariPeriodi");
