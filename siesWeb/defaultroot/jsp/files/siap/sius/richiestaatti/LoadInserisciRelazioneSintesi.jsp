...
<%
  ...
  String lAzione = "siap.sius.richiestaatti.action.ActInserisciRelazioneSintesi";
%>
...
<html>
<head>
  <title>[S.I.E.S.] - Richiesta stampa Relazione Sintesi</title>
  ...
  <script language="JavaScript" src="<%=IWebConstants.JS_VALIDATOR%>"></script>
    ...
    function Verify()
    {
      if (document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
      ...
      if (document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[0].value != '-'
          && document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[0].value == '')
      {
        alert('La Sede del destinatario 1 è obbligatoria');
        return false;
      }
      if (document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[1].value != '-'
          && document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[1].value == '')
      {
        alert('La Sede del destinatario 1 è obbligatoria');
        return false;
      }
      ...
      if (document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[0].value == '-'
       && document.LoadInserisciRelazioneSintesi.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[1].value == '-')
      {
...
<body onLoad="document.forms[0].elements[0].focus()" class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Richiesta stampa Relazione Sintesi</font>
      </td>
      <td class="LBG">
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciRelazioneSintesi">
           ...
           <table>
             ...
             <td class="l">
               <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
                ...
                <a href="Javascript:ListaComuni('LoadInserisciRelazioneSintesi','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[0]');">
          ...
          <td class="L" colspan=3>
           <table>
             ...
             <td class="l">
               <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
                value="" type="text" maxlength="35" size="35">
                <a href="Javascript:ListaComuni('LoadInserisciRelazioneSintesi','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[1]');">
                <img src="/images/filefolder.gif" border=0> </a>
          </td>
          </tr>
          <tr>
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("LoadInserisciRelazioneSintesi");
