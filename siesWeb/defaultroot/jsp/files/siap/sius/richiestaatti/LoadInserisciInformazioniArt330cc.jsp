...
<%
  String lAzione = "siap.sius.richiestaatti.action.ActInserisciInformazioniArt330cc";
%>
...
<html>
<head>
  <title>[S.I.E.S.] - Richiesta Informazioni Art. 330 cc</title>
  ...
  <script language="JavaScript" src="<%=IWebConstants.JS_VALIDATOR%>"></script>
    ...
    function Verify()
    {
      if (document.LoadInserisciInformazioniArt330cc.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciInformazioniArt330cc.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciInformazioniArt330cc.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciInformazioniArt330cc.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciInformazioniArt330cc.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciInformazioniArt330cc.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciInformazioniArt330cc.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciInformazioniArt330cc.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciInformazioniArt330cc.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
...
<body onLoad="document.forms[0].elements[0].focus()" class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Richiesta Informazioni Art. 330 cc</font>
      </td>
      <td class="LBG">
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciInformazioniArt330cc">
      ...
      <tr>
        ...
        <td class="l" colspan="3">
           <input Title="Sede" name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
              ...
              <a href="Javascript:ListaUffici('LoadInserisciInformazioniArt330cc','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>');">
    ...
    while( count <  10 )
    {
      %>
      <tr>
        <td class="l" nowrap>Figlio n° <%=count%>) </font></td>
        <td class="l"><input type="text" Title="Figlio n° <%=count%>" name="<%=ICostantiRichiestaAtti.CAMPO_AGGIUNTIVO%>" size="40"></td>

        <td class="l" nowrap>Figlio n° <%=count + 1%>) </font></td>
        <td class="l"><input type="text" Title="Figlio n° <%=count + 1%>" name="<%=ICostantiRichiestaAtti.CAMPO_AGGIUNTIVO%>" size="40"></td>
      </tr>
     <%
      count+=2;
    }
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("LoadInserisciInformazioniArt330cc");
