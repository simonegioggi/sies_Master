...
<%
  ...
  String lAzione = "siap.sius.richiestaatti.action.ActInserisciVisitaMedica";
...
<script language="JavaScript">
	...
	function abilitaCampiDataRestituzione() {
		if (document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CHECK_DATA_RESTITUZIONE%>.checked){
	   		document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.disabled = false;
	   		document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.disabled = false;
	   		document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.disabled = false;
		}
		else {
	   		document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.disabled = true;
	   		document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.disabled = true;
	   		document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.disabled = true;
		}    	
	}
    ...
    function Verify()
    {
      ...
      if (document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[0].value != '-'
          && document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[0].value == '')
          {
              alert('La Sede del destinatario n°1 è obbligatoria');
              return false;
          }
      if (document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[1].value != '-'
          && document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[1].value == '')
          {
      ...
      if (document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[0].value == '-'
          && document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>[1].value == '-')
          {
              alert('Inserire almeno un Destinatario con la relativa Sede.');
              return false;
          }

      if (document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
      ...
      <%-- INIZIO: MEV_2019-09 (D.lgs. 123/2018) --%>
      if (document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CHECK_DATA_RESTITUZIONE%>.checked){
	      var data_restituzione = document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.value
	                        +'/'+ document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.value
	                        +'/'+ document.LoadInserisciVisitaMedica.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.value;
...
<html>
  <head>
    <title>[S.I.E.S.] - Richiesta Visita Medica</title>
  ...
  <body onLoad="document.forms[0].elements[0].focus()" class="corpo">
    <table>
      <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
        <td class="LBG">
          ...
          <font class="campo">Richiesta Visita Medica</font>
        </td>
      <td class="LBG">
    ...
    <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciVisitaMedica">
        ...
        <tr>
          ...
          <td class="l">
             <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
                ...
                <a href="Javascript:ListaComuni('LoadInserisciVisitaMedica','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[0]');">
...
<div id="elenco1" style="display:none">
        ...
        <tr>
          ...
          <td class="l">
            <input Title="Sede " name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
                value="" type="text" maxlength="35" size="35">
                <a href="Javascript:ListaComuni('LoadInserisciVisitaMedica','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>[1]');">
                <img src="/images/filefolder.gif" border=0> </a>
          </td>
        </tr>
    ...
    <script language="JavaScript" type="text/javascript">
      var frmvalidator = new Validator("LoadInserisciVisitaMedica");
