...
// Chiamata all'elenco degli UEPE
    ...
    function scegliTipo() {
    	var tipo = document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>.value;
    	if ("B5" == tipo)
    		ListaUSSM('LoadInserisciRelazioneCSSA','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>');
    	else
    		ListaUffici('LoadInserisciRelazioneCSSA','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>');
    }
...
<html>
<head>
  <title>[S.I.E.S.] - Richiesta Relazione UEPE/USSM</title>
  ...
  <script language="JavaScript" src="<%=IWebConstants.JS_VALIDATOR%>"></script>
    ...
    <script language="JavaScript">
	  ...
	  function abilitaCampiDataRestituzione() {
	  	if (document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CHECK_DATA_RESTITUZIONE%>.checked){
	   		...
	   		document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.disabled = false;
	   		document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.disabled = false;
	  	}
	  	else {
	   		document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.disabled = true;
	   		document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.disabled = true;
    ...
    function Verify()
    {
      if (document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
      ...
      if (document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CHECK_DATA_RESTITUZIONE%>.checked){
	      var data_restituzione = document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.value
	                        ...
	                        +'/'+ document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.value;
      ...
      if (document.LoadInserisciRelazioneCSSA.<%=ICostantiRichiestaAtti.CAMPO_SEDE%>.value == '')
      {
...
<body onLoad="document.forms[0].elements[0].focus()" class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Richiesta Relazione</font>
      </td>
      <td class="LBG">
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciRelazioneCSSA">
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("LoadInserisciRelazioneCSSA");
    ...
    frmvalidator.addValidation("<%=ICostantiRichiestaAtti.CAMPO_COD_DESTINATARIO%>", "req","Il campo Destinatario è obbligatorio");
