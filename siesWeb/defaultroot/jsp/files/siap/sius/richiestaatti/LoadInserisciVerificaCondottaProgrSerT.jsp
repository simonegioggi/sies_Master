...
<%
  ...
  String lAzione = "siap.sius.richiestaatti.action.ActInserisciVerificaCondottaProgrSerT";
...
<html>
<head>
  <title>[S.I.E.S.] - Richiesta Verifica condotta e programma ser.t.</title>
  ...
  <script language="JavaScript" src="<%=IWebConstants.JS_VALIDATOR%>"></script>
    ...
    function abilitaCampiDataRestituzione() {
    	if (document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CHECK_DATA_RESTITUZIONE%>.checked){
	   		document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.disabled = false;
	   		document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.disabled = false;
	   		document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.disabled = false;
    	}
    	else {
	   		document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.disabled = true;
	   		document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.disabled = true;
	   		document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.disabled = true;
    	}    	
   	}
    ...
    function Verify()
    {
      if (document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value='0'+document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value;

      if (document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value.length==1)
          document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value='0'+document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value;

      // Controllo validita' della data emissione
      var data_emissione=document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_EMISSIONE%>.value+'/'+document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_EMISSIONE%>.value;
      ...
      <%-- INIZIO: MEV_9 (D.lgs. 123/2018) --%>
      var data_restituzione = document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_GIORNO_DATA_RESTITUZIONE%>.value
                        +'/'+ document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_MESE_DATA_RESTITUZIONE%>.value
                        +'/'+ document.LoadInserisciVerificaCondottaProgrSerT.<%=ICostantiRichiestaAtti.CAMPO_ANNO_DATA_RESTITUZIONE%>.value;
...
<body onLoad="document.forms[0].elements[0].focus()" class="corpo">
  <table>
    <tr><td class="LBG"><a href="Javascript:window.print();"><img align="middle" src="<%=IWebConstants.IMAGES_DIR%>quickprint24.gif" alt="Stampa questa videata" border=0></a></td>
      <td class="LBG">
        ...
        <font class="campo">Richiesta Verifica condotta e programma ser.t.</font>
      </td>
      <td class="LBG">
  ...
  <FORM method="POST" action="<%= IWebConstants.PG_MAIN%>" name="LoadInserisciVerificaCondottaProgrSerT">
      ...
      <tr>
        ...
        <td class="l">
           <input Title="Sede" name="<%=ICostantiRichiestaAtti.CAMPO_SEDE%>"
              ...
              <a href="Javascript:ListaComuni('LoadInserisciVerificaCondottaProgrSerT','<%=ICostantiRichiestaAtti.CAMPO_SEDE%>');">
  ...
  <script language="JavaScript" type="text/javascript">
    var frmvalidator = new Validator("LoadInserisciVerificaCondottaProgrSerT");
