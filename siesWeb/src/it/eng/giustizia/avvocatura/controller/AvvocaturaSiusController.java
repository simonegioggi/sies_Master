...
public class AvvocaturaSiusController extends GenericController implements IAvvocaturaSius {
	...
	public DATIPROCEDIMENTOOUTPUT callRicercaFascicoloSius(String codDistretto, String codTipoUfficio,
		...
		try {
			...
			stmt = connection
					.prepareCall("{call AVVOCATURA_SIUS.CERCA_FASIUS_PER_ESTREMI(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}");
			...
			Object[] itemAtributes = new Object[] { new Integer(annnoProcedimento), new Integer(numeroProcedimento),
					new String(codTipoUfficio), new String(codDistretto), new String(codFiscaleAvvocato) };
			...
			// info per il log
			avvocaturaLogger
					.info("########## Tempo di Esecuzione x Store Procedure 'AVVOCATURA_SIUS.CERCA_FASIUS_PER_ESTREMI': "
			...
			if (udienze != null) {
				...
				for (int i = 0; i < children.length; i++) {
					elencoMovimentiUdienza[i] = (MOVIMENTIUDIENZATYPE) TypeFactory
							.createMovimentiUdienza((Struct) children[i]);
				}
			...
			codiceErrore = (String) stmt.getString(14);
			descrErrore = (String) stmt.getString(15);
		...
		// fvender 02/08/2018: aggiunto blocco finally per la gestione della problematica della chiusura delle connessioni	
		} finally {
			try {
