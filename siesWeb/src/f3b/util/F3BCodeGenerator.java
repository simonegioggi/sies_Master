...
public class F3BCodeGenerator {
	...
	private static void writeAction() {
		...
		try {
			...
			lFileAction
					.write("\t\t setRequestAttribute(IWebConstants.MESSAGE_TEXT,\"Inserimento Avvenuto Correttamente!\");\n");
			...
			lFileAction.write("public String processRequest() throws F3BException \n{\n\n");
			lFileAction.write(" \t\t String lId = getRequestStringParameter(CAMPO_" + mKey.toUpperCase()
					+ ");\n");
	...
	private static void writeController() {
		...
		try {
			...
			lFileController.write("\t\t        {\n");
			lFileController.write("\t\t l" + mNomeTabella.substring(0, mNomeTabella.length() - 1)
					+ "i.add( (" + mNomeTabella + "Model)" + lLocalDao + ".getModel() );\n");
			lFileController.write("\t\t        }\n");
	...
	private static void writeJsp() {
		...
		try {
			...
			PrintWriter lFileJSP = new PrintWriter(new BufferedWriter(new FileWriter(lNomeFile + "Load"
					+ mNomeTabella + ".jsp")));
