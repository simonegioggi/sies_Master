 ...
 * </p>
 * <p>
 * Description: Classe di gestione della lettura dei files di properties. Infatti questa classe espone i
 * metodi di lettura dei parametri e la fase di load del file stesso.
 * </p>
 * <p>
...
public class PropertiesMgr {
	...
	/**
	 * Metodo che ritorna il valore corrispondente alla chiave passata come attributo, inoltre nel caso in cui
	 * non esistesse, il valore corrispondente ritorna quello di default, anch'esso passato come attributo.
	 * <p>
	 * 
	 * @param aKey
	 *            chiave per valore.
	...
	protected void init() throws F3BException {
		...
		try {
			...
			throw new F3BException("Errore nella fase di load del file di properties");
		} finally {
			try {
				if (lFis != null)
					lFis.close();
			} catch (IOException ioex) {
				throw new F3BException("Errore nella fase di chiusura del file");
			}
		}
	}
	 ...
	 * @return il valore corrispondente alla chiave.
	 */
	protected synchronized int readIntProperty(String aKey) throws F3BException {
		int lIntValue = 0;

		try {
			lIntValue = Integer.parseInt(mProps.getProperty(aKey));
		} catch (NumberFormatException nfex) {
			throw new F3BException("Non è possibile convertire la chiave " + aKey + " con valore = "
	...
	/**
	 * Metodo che ritorna il valore corrispondente alla chiave come intero passata come attributo, inoltre nel
	 * caso in cui non esistesse, il valore corrispondente ritorna quello di default, anch'esso passato come
	 * attributo.
	 * <p>
	 * 
	 * @param aKey
	 ...
	 * @return il valore corrispondente alla chiave.
	 */
	protected int readIntProperty(String aKey, int aDefaultValue) throws F3BException {
		int lIntValue = 0;
		...
		try {
			...
			throw new F3BException("Non è possibile convertire la stringa in intero!");
		}
