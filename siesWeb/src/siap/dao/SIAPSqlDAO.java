...
*/
 ...
 /**
  * Il metodo ritorna il n.ro di record restituiti da una select
  *  già precedentemente impostata.
  *
  */
 public BigDecimal getNumRowsSelected () throws DAOException
 {
  ...
  /**
   * Il metodo  a partire dal numero di pagina calcola il range
   * di record da passare alla funzione start(int, int)
   * per inizializzare l'operazine di fetch.
   * @param aPageNum
   * @throws Exception
   */
  ...
  /**
   * Il metodo trasforma la select già impostata nello Statement in
   * una select paginata.
   *
   * @param aPageNum
   * @throws F3BException
  ...
  /**
   * Il metodo  a partire dal numero di pagina riscrive la select
   * in modo da ritornare la ricerca su una sola pagina.
   * Il metodo è alternativo al metodo startPage() perchè restituisce
   * lo stesso risultato ma con un metodo diverso.
   * La prima startPage() lascia inalterata la select ma agisce sulla fetch
   * dei record utili.
   * startPage1(), invece, genera una nuova select innestata per ricavare
   * i record utili senza alterare la fetch.
   * Questa ultima potrebbe essere più efficiente ( da verificare ??) perchè non
