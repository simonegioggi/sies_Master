...
public class ActDettaglioRichiestaGenerica extends ActSIESDettaglioProvvedimento {
	...
	public String processRequest() throws Exception {
		...
		String tipoRichiestaParameter = "";
		if (!isRequestParameterNullObj("tipoRichiesta")) {
			tipoRichiestaParameter = getRequestStringParameter("tipoRichiesta");
		}
		...
		EventoNotificaModel lEveNot = lCtrl.ExRicercaEventoNotificaByKeyForRichiestaAtti(lProvEvento
		...
		//CampoNotaModel[] campoNotaModelVett = lProvEvento.getEventoNotifica().getCampoNote();
		String tipoRichiesta = "Richiesta ";

		
//		 if (campoNotaModelVett!=null && campoNotaModelVett.length>1){
//			 int dim = campoNotaModelVett.length-1;
//			 tipoRichiesta = tipoRichiesta+campoNotaModelVett[dim].getDescr();
//		 } else if (campoNotaModelVett!=null && campoNotaModelVett.length>0){
//			 tipoRichiesta = tipoRichiesta+campoNotaModelVett[0].getDescr();
//		 } else {
//			 tipoRichiesta = tipoRichiesta+" Generica";
//		 }

		if (tipoRichiestaParameter != null && !tipoRichiestaParameter.equalsIgnoreCase("")) {
			tipoRichiesta = tipoRichiesta + tipoRichiestaParameter;
		} else {
			tipoRichiesta = tipoRichiesta + " Generica";
		}
		...
		if (lEveMod.getEvento().getTemIdTemplate() == null) {
			...
			siesLogger
					.debug(">>> Il CodMotivo nell'evento è : " + lEveMod.getEvento().getCodMotivo());
...
//			ITemplate lCtrlTemplate = SICOLookupRemote.getTemplateRemote();
			...
			// VERSIONE 11: modifica per visione template --> SIGE_IS_011
			lDecodifiche.add(new DecodificheModel("SIGE_IS_011", tipoRichiesta, "", "", "", "", "", "", ""));
			Option lOptionTemp = new Option(lDecodifiche);

			setRequestAttribute(ICostantiTemplate.CAMPO_COMBO_TEMPLATE, lOptionTemp.toString());

		}

		return ICostantiRichiestaAtti.PG_DETTAGLIO_RICHIESTAGENERICA;
	}
