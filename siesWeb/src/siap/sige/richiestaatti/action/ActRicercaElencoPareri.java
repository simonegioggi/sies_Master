...
/**
 * <p>Title: ActRicercaElencoPareri </p>
 * <p>Description: Attiva la Ricerca delle richieste di parere sia per la sua visualizzazione
 * che per la stampa.</p>
 ...
 * @version 1.0
 */
public class ActRicercaElencoPareri  extends ActionSige
implements ICostantiRichiestaAtti
{
  ...
  private Vector <ParereModel>mVect = null;
  ...
  public String processRequest() throws Exception
  {
    ...
    // AA IFascicoloSius lCtrl = SIUSLookupRemote.getFascicoloSiusRemote();
    ...
    else
    {
      ...
      String lInTesta = " nel periodo " + DateUtils.getDateToString(mParMod.getDataEmissione(), "dd/MM/yyyy") + " - " +  DateUtils.getDateToString(mParMod.getDataEmissione2(), "dd/MM/yyyy");
  ...
  private String Stampa() throws Exception
  {
    ...
    ByteArrayOutputStream lReport =   lCtrlSta.ExPreStampaPareri(mParMod, mVect, lUtenteMod); //lCtrlSta.ExPreStampaRichiestaAtti(aEvento, lUfficio.getCodUfficio(), lUtenteMod);
  ...
  private void letturaFiltroRicerca() throws Exception
  {
    ...
    // [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di LogF3B.getLogger()
    siesLogger.info( "Cod Tipo Parere : " + lCodTipoParere);

    mParMod.setCodMotivo(lCodTipoParere);
    if (lCodTipoParere.length() > 1)
      mParMod.setDescrMotivo(DecodificheUtils.getDescbyCode(DecodificheManager.getInstance().getMotivoProvvedimento(),mParMod.getCodMotivo()));
    else
    ...
    if (lCodContenuto.length() > 1)
      mParMod.setDescrOggettoProcedimento(DecodificheUtils.getDescbyCode(DecodificheManager.getInstance().getOggettoProcedimento(),mParMod.getCodOggettoProcedimento()));
    else*/
    ...
    mParMod.setMessage( getRequestStringParameter(ICostantiProduzioneAtti.CAMPO_TIPO_ORDINAMENTO));


    // Lettura intervallo data di emissione
    Date lDataIni = null, lDataFin = null;

    lDataIni = getRequestDateParameter( CAMPO_ANNO_DATA_EMISSIONE,CAMPO_MESE_DATA_EMISSIONE,CAMPO_GIORNO_DATA_EMISSIONE );
    lDataFin = getRequestDateParameter( ICostantiProduzioneAtti.CAMPO_ANNO_DATA_EMISSIONE2,ICostantiProduzioneAtti.CAMPO_MESE_DATA_EMISSIONE2,ICostantiProduzioneAtti.CAMPO_GIORNO_DATA_EMISSIONE2 );
