...
/**
 * TenoreSentenzaReatoDAO - Classe DAO che rappresenta la tabella TenoreSentenzaReato; Tabella di relazione
 * tra Oggetto SIGE (TENORE_SIGE), Sentenza e Reato.
 *
 * @version 1.0
 */
public class TenoreSentenzaReatoDAO extends TableDAO {
	...
	public BigDecimal getIdTenSenRea() throws DAOException {
		return getBigDecimal("ID_TEN_SEN_REA");
	}
	...
	public void setIdTenSenRea(BigDecimal aValore) {
		setBigDecimal("ID_TEN_SEN_REA", aValore);
	}

	public void setTenIdTenoreSige(BigDecimal aValore) {
		setBigDecimal("TEN_ID_TENORE_SIGE", aValore);
	}
	...
	public GenericModel getModel() throws DAOException {
		...
		return new TenoreSentenzaReatoModel(getTenIdTenoreSige(), getSenIdSentenza(), getReaIdReato(),
				getDataInserimento(), getCodOperatoreInserimento(), getCodUfficioInserimento(), "",
				getIdTenSenRea(), getCodEsito(), "", getDataAggiornamento(), getCodOperatoreAggiornamento(),
	...
	/**
	 * Predispone la condizione di filtro per cancellare tutti i record di relazione individuati dalla chiave
	 * TEN_ID_TENORE_SIGE.
	 */
	...
	/**
	 * Predispone la condizione di filtro per cancellare l'associazione Titolo Esecutivo/Oggetto.
	 */
	...
	public void selCondizioneDeleteProvvedimento(BigDecimal aIdProvSige) {
		...
		setCondition(" TEN_ID_TENORE_SIGE IN ( SELECT ID_TENORE_SIGE FROM TENORE_SIGE WHERE"
				+ " PROV_ID_PROVVEDIMENTO_SIGE = " + aIdProvSige + " )");
	}
	...
	public void selCondizioneSentenzaFascicolo(BigDecimal aIdSentenza, BigDecimal aIdFasSige) {
		...
		lCondizione += " AND TEN_ID_TENORE_SIGE IN (SELECT ID_TENORE_SIGE FROM TENORE_SIGE WHERE"
				+ " FAS_ID_FASCICOLO_SIGE = " + aIdFasSige + " and data_fine is null)"; // 20251202
	...
	public void setUpdateForDeleteProvvedimento(ProvvedimentoSigeModel aProvModel) throws DAOException {
		...
		setCondition(" TEN_ID_TENORE_SIGE IN ( SELECT ID_TENORE_SIGE FROM TENORE_SIGE WHERE"
				+ " PROV_ID_PROVVEDIMENTO_SIGE = " + aProvModel.getIdProvvedimentoSige() + " )");
	}
