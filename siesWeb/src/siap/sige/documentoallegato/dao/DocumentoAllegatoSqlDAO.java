...
public class DocumentoAllegatoSqlDAO extends SIAPSqlDAO {
	...
	public void ricercaDocumentoAllegatoFCByIdFascicolo (BigDecimal aKey)throws DAOException {
	    String lSql = "select doc.* from documento_allegato doc, PROVVEDIMENTO_SIGE prov where " +
	    		      "doc.cod_tipo_documento='06' and " +
	    		      "nvl (doc.FLAG_DOCUMENTO_REGISTRATO, 'S') <> 'A' and " +
	    		      "doc.eve_id_evento=prov.id_evento_generato and " +
	    		      "prov.COD_TIPO_PROVVEDIMENTO_SIGE IN ('02','03') AND " +
	    		      "prov.fas_id_fascicolo_sige=" + aKey;
       setStatement(lSql);
   }
   ...
   protected String getSqlQuery()  {
	    ...
	    //lStatement += " WHERE DOCUMENTO_ALLEGATO.COD_TIPO_DOCUMENTO = CODTIPDOC.RV_LOW_VALUE AND CODTIPDOC.RV_DOMAIN = 'TIPO_DOCUMENTO_ALLEGATO'";
   ...
   public String setCondizioniFCByIdEvento(BigDecimal aKey){
     return " AND EVE_ID_EVENTO = " + aKey + " and " +
            " COD_TIPO_DOCUMENTO='06'"; 
   }
   ...
   public BigDecimal getProgressivoByUff(String  aKey, BigDecimal aAnno) throws DAOException
   {
     String  lStatement = "SELECT DISTINCT nvl(MAX(PROGR_FOGLIO_COMPLEMENTARE),0) AS MAX_PROG FROM DOCUMENTO_ALLEGATO";
             lStatement += " WHERE COD_TIPO_DOCUMENTO = '06' AND COD_UFFICIO_INSERIMENTO = '" + aKey +"' AND ANNO_FOGLIO_COMPLEMENTARE = '" + aAnno.toString() + "'";
   ...
   public BigDecimal countDecretiDepositoFissazioneUdienzaNonValidati (BigDecimal idFascicolo) throws DAOException {
	   String sql = "select count (ev.id_evento) as numDec from UDIENZA_PROCEDIMENTO_SIGE ud,evento ev  where " +
		        "ud.fas_id_fascicolo_sige=" +idFascicolo + " and " + 
		        "ev.id_evento=ud.eve_id_evento and " + 
		        "ev.cod_tipo_evento='01' and " + 
		        "ev.cod_tipo_provvedimento='02' and " +
   ...
   public BigDecimal countDecretiDepositoFissazioneUdienzaValidati (BigDecimal idFascicolo) throws DAOException {
	   String sql = "select count (ev.id_evento) as numDec from UDIENZA_PROCEDIMENTO_SIGE ud,evento ev  where " +
			        "ud.fas_id_fascicolo_sige=" +idFascicolo + " and " + 
			        "ev.id_evento=ud.eve_id_evento and " + 
			        "ev.cod_tipo_evento='01' and " + 
