...
/**
 * <p>
 * Title: ActLoadDettaglioDecretoUnificazioneSige
 * </p>
 * <p>
 * Description: Classe Action per la load di Dettaglio Decreto Unificazione Sige
 * </p>
 * <p>
...
public class ActLoadDettaglioDecretoUnificazioneSige extends ActionSiap
	...
	public String processRequest() throws Exception {
		...
		String lId = "";
		if (!isRequestParameterNullObj(CAMPO_ID_DECRETO_UNIFICAZIONE))
			lId = getRequestStringParameter(CAMPO_ID_DECRETO_UNIFICAZIONE);
		if ((lId.compareTo("") == 0)
		...
		if (lId.compareTo("") == 0)
			throw new SIGEException(SIGEException.USER_MESSAGE, "Decreto di Unificazione inesistente.");
		...
		lFasUnificato = lCtrl
				.ExRicercaEstesaFascicoloSigeByKey(lProvvEveMod.getProvvedimento().getFasIdFascicoloSige());
		lFasUnificante = lCtrl.ExRicercaEstesaFascicoloSigeByKey(
				lFasUnificato.getFascicoloSige().getFasSigIdFascicoloSige());
		...
		setRequestAttribute("idFascicoloUnificato",
				lFasUnificato.getFascicoloSige().getIdFascicoloSige().toString());
		setRequestAttribute("annoFascicoloUnificato",
				lFasUnificato.getFascicoloSige().getChiaveAnno().toString());
		setRequestAttribute("progrFascicoloUnificato",
				lFasUnificato.getFascicoloSige().getChiaveProgr().toString());
		setRequestAttribute("dataUnificazione", DateUtils
				.getDateToString(lFasUnificato.getFascicoloSige().getDataDefinizione(), "dd/MM/yyyy"));
		...
		return PG_DETTAGLIODECRETOUNIFICAZIONESIGE;
	}
