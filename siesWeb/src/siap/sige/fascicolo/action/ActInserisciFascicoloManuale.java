...
/**
 * ActInserisciFascicoloManuale - Classe Azione di inserimento manuale del Fascicolo SIGE
 *
 * @version 1.0
 */
public class ActInserisciFascicoloManuale extends ActionSiap implements ICostantiFascicoloSige {
	...
	public String processRequest() throws Exception {
		...
		mFascicolo = lCtrl.ExInserisciFascicoloSigeManuale(mFascicolo, mRichiesta, mMagAssegnatario,
				lDetenzione);
	...
	protected FascicoloSigeModel leggiDatiFascicolo() throws Exception {
		...
		lFascicolo.setChiaveAnno(getRequestBigDecimalParameter(CAMPO_CHIAVE_ANNO)); // Anno impostato nella
																					// form
		lFascicolo.setChiaveProgr(getRequestBigDecimalParameter(CAMPO_CHIAVE_PROGR)); // Progressivo impostato
																						// nella form
		...
		if (!isRequestParameterNullObj(CAMPO_ANNO_DATA_ISCRIZIONE)
				&& !isRequestParameterNullObj(CAMPO_MESE_DATA_ISCRIZIONE)
				&& !isRequestParameterNullObj(CAMPO_GIORNO_DATA_ISCRIZIONE))
			lFascicolo.setDataIscrizione(getRequestDateParameter(CAMPO_ANNO_DATA_ISCRIZIONE,
