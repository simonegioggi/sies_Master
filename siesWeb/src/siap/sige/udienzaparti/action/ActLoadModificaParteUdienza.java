...
/**
 * <p>Title: ActLoadModificaParteUdienza</p>
 * <p>Description: Classe Action per la load Modifica Parte Offesa/Civile associata ad una udienza</p>
 * <p>Copyright: Copyright (c) 2008</p>
 * <p>Company: Engineering S.p.A.</p>
 * @version 1.0
*/
public class ActLoadModificaParteUdienza extends ActionSige implements ICostantiPartiUdienza, ICostantiEvento {

	public String processRequest() throws Exception {

		// Prepara la pagina di destinazione
		String lPage = PG_LOAD_MODIFICA_PARTE_UDIENZA;
		...
		AnagraficaPartiUdienzaModel anagParteUdienzaRet = lCtrl.ExRicercaParteUdienzaByKey(new BigDecimal(lIdSoggetto));
		...
		Option lOption = new Option(DecodificheManager.getInstance().getSesso(), anagParteUdienzaRet.getSesso());
		setRequestAttribute("sesso", "" + lOption);

		if (anagParteUdienzaRet.getRagSociale() != null && !anagParteUdienzaRet.getRagSociale().equals("")) {
			lOption = new Option(DecodificheManager.getInstance().getRagioneSociale(), anagParteUdienzaRet.getRagSociale());
		} else {
			lOption = new Option(DecodificheManager.getInstance().getRagioneSociale(), "-");
		}
		setRequestAttribute("ragioneSociale", "" + lOption);

		if (anagParteUdienzaRet.getCodProvincia() != null && !anagParteUdienzaRet.getCodProvincia().equals("")) {
			lOption = new Option(DecodificheManager.getInstance().getProvincie(), anagParteUdienzaRet.getCodProvincia());
		} else {
		...
		if (anagParteUdienzaRet.getCodStatoNascita() != null) {
			lOption = new Option(DecodificheUtils.getDecodesWithoutCode(DecodificheManager.getInstance().getNazioni(), "-"), anagParteUdienzaRet.getCodStatoNascita());
		} else {
		...
		if (anagParteUdienzaRet.getResidenza() != null && anagParteUdienzaRet.getResidenza().getCodStato() != null) {
			lOption = new Option(DecodificheUtils.getDecodesWithoutCode(DecodificheManager.getInstance().getNazioni(), "-"), anagParteUdienzaRet.getResidenza().getCodStato());
		} else {
			lOption = new Option(DecodificheUtils.getDecodesWithoutCode(DecodificheManager.getInstance().getNazioni(), "-"), "039");
		}
		...
		if (notificaSoggetto != null && notificaSoggetto.getAutoritaEsterna() != null && notificaSoggetto.getAutoritaEsterna().getIdAutoritaEsterna() != null) {
			lOptionAut = new Option(DecodificheUtils.getDecodesWithoutCode(DecodificheManager.getInstance().getTipoAutorita(), "-"), notificaSoggetto.getAutoritaEsterna().getIdAutoritaEsterna().toString());
		} else {
		...
		if (anagParteUdienzaRet != null && anagParteUdienzaRet.getFlagConvUdienza() != null && anagParteUdienzaRet.getFlagConvUdienza().equals("S")) {
