...
//import siap.sige.richiesta.dao.RichiestaSigeSqlDAO;
...
public class RichiestaSigeController extends SiapController implements IRichiestaSige
{
	...
	//  Valutare se usare RichiestaSigeSqlDAO o RichiestaSigeDAO nell'implementazione della Ricerca
/* 

 * (non-Javadoc)
 * @see siap.sige.richiesta.controller.IRichiestaSige#ExRicercaRichiestaSige(siap.sige.richiesta.model.RichiestaSigeModel)
 *
public Vector ExRicercaRichiestaSige (RichiestaSigeModel aRichiestaSige )
 		        throws F3BException
{
 		  Connection lConn = null;
 		  Vector lRichiestaSigi = new Vector();
 		  RichiestaSigeSqlDAO lRicDao = null;
 		


    try {
		       lConn = getDBConnection();
		       lRicDao = new RichiestaSigeSqlDAO(lConn);
		       lRicDao.ricercaRichiestaSige(aRichiestaSige);
		       lRichiestaSigi = new Vector(lRicDao.getModels());
		       if ( lRichiestaSigi.size() == 0 )
		        {
		      ...
		      catch (DAOException daoEx)
		      {
		          ...
		          throw new F3BException("RichiestaSigeController.ExRicercaRichiestaSige: Non posso leggere : " + daoEx);
		          }
		         catch (SQLException sqe)
		         {
		           // [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di mLog
		           siesLogger.error("SQLException: " + sqe);
		           throw new F3BException("RichiestaSigeController.ExRicercaRichiestaSige: Non posso leggere  : " + sqe);
		          }
		          finally 
		          {
		              cleanup(lRicDao);
