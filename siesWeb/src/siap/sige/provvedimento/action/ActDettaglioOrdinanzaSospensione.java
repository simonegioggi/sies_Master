...
/**
 * <p>
 * Title: ActDettaglioOrdinanzaSospensione
 * </p>
 * <p>
 * Description: Classe Action per il Dettaglio Ordinanza Sospensione
 * </p>
 * <p>
...
public class ActDettaglioOrdinanzaSospensione extends ActLoadEmissioneOrdinanzaSospensione implements
	...
	public String processRequest() throws Exception {
		...
		// LogF3B.getLogger()
		siesLogger.debug("ActDettaglioOrdinanzaSospensione: inizio");
		...
		String lRetPage = PG_DETTAGLIO_ORDINANZA_SOSPENSIONE;
		...
		if (lProvEvento == null || lProvEvento.getProvvedimento() == null
				|| lProvEvento.getProvvedimento().getProvvIdProvvedimentoSige() == null)
			throw new SIGEException(SIGEException.USER_MESSAGE, "Provvedimento di Sospensione non valido!");
		...
		// Ricerca del Provvedimento Sospeso
		ProvvedimentoSigeEventoModel provvSospeso = lCtrlProv.ExRicercaProvvedimentoById(lProvEvento
				.getProvvedimento().getProvvIdProvvedimentoSige());
		if (provvSospeso == null || provvSospeso.getProvvedimento() == null
				|| provvSospeso.getProvvedimento().getIdProvvedimentoSige() == null)
			throw new SIGEException(SIGEException.USER_MESSAGE, "Provvedimento Sospeso non valido!");

		setRequestAttribute("ProvvedimentoEventoSospeso", provvSospeso);

		// Ricerca tenori legati al provvedimento sospeso
		TenoreSigeModel lTenore = new TenoreSigeModel();
		lTenore.setProvIdProvvedimentoSige(lProvEvento.getProvvedimento().getProvvIdProvvedimentoSige());
		...
		lTempRic.setCodTipoProvvedimentoSige(ICostantiProvvedimentoSige.COD_ORDINANZA_SOSPENSIONE);
		...
		ModificabileStampabile(lFasEsteso, lProvEvento);
		...
		// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
		// LogF3B.getLogger()
		siesLogger.debug("ActDettaglioOrdinanzaSospensione: -> page: " + lRetPage);
	...
	private void ModificabileStampabile(FascicoloSigeEstesoModel lFasEsteso,
			ProvvedimentoSigeEventoModel lProvEvento) throws Exception {
		...
		if (getCodUfficioUtenteConnesso().equalsIgnoreCase(lFasEsteso.getFascicoloSige().getChiaveUfficio())
			...
			if ((lProvEvento.getProvvedimento().getDataDeposito() == null))
				lCancellabile = "SI";
