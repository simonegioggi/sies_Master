...
/**
 * Title: ActModificaCompFoglioComp Description: Azione per la modifica del Foglio Complementare.
 *
...
public class ActModificaCompFoglioComp extends ActionSige implements ICostantiFoglioComp {
	...
	public String processRequest() throws Exception {
		...
		aDocAllegato.setIdDocumentoAllegato(
				getRequestBigDecimalParameter(ICostantiDocumentoAllegato.CAMPO_ID_DOCUMENTO_ALLEGATO));
		aDocAllegato
				.setDataEmissione(getRequestDateParameter(ICostantiFoglioComp.CAMPO_ANNO_DATA_TRASMISSIONE,
		...
		// In fase di modifica trasmissione viene aggiornata la data di invio ultimo aggiornamento
		// (DATA_ULT_INVIO)
		// mentre la data trasmissione rimane invariata
		/*
		 * aDocAllegato.setDataTrasmissione(
		...
		if (flagMotivoNonInvio != null && flagMotivoNonInvio.equals("01")) {
			...
			aDocAllegato
					.setDataInsMan(getRequestDateParameter(ICostantiFoglioComp.CAMPO_ANNO_DATA_INS_MANUALE,
		...
		if (!isRequestParameterNullObj(ICostantiFoglioComp.CAMPO_DESCR_MOTIVO_NON_INVIO)) {
			aDocAllegato.setDescrizioneNonInvio(
					getRequestStringParameter(ICostantiFoglioComp.CAMPO_DESCR_MOTIVO_NON_INVIO));
		}

		if (!this.isRequestParameterNullObj("Provenienza")
				&& super.getRequestStringParameter("Provenienza").equalsIgnoreCase("tastoFunzione")) {
			aDocAllegato
					.setDataInsMan(getRequestDateParameter(ICostantiFoglioComp.CAMPO_ANNO_DATA_INS_MANUALE,
							ICostantiFoglioComp.CAMPO_MESE_DATA_INS_MANUALE,
							ICostantiFoglioComp.CAMPO_GIORNO_DATA_INS_MANUALE));
		 ...
		 */
		return IWebConstants.PG_MAIN + "?" + IWebConstants.ACTION_FIELD
				+ "=siap.sige.fogliocomplementare.action.ActLoadDettaglioFCTastoFunzione&"
				+ ICostantiDocumentoAllegato.CAMPO_ID_DOCUMENTO_ALLEGATO + "="
				+ aDocAllegato.getIdDocumentoAllegato();
