...
public class CollegioController extends SiapController implements ICollegio {
			...
			if ("0".equals(lColMod.getIdCollegio().toString())) {
				...
				try {
					...
					if (daoe.UNIQUE_CONSTRAINT_VIOLATED) {
						siesLogger.info(
								"Inserimento impossibile: Collegio già presente! --> " + daoe.getMessage());
						// ricerco id_collegio
	...
	public Collection ExElencoCbxCollegiByCodUfficio(String aCodUfficio) throws F3BException {
		...
		try {
			...
			throw new F3BException(
					"CollegioController.ExElencoCbxCuratoriByCodUfficio: Non posso leggere : " + daoEx);
		} finally {
			cleanup(lColDao);
			cleanup(lConn);
		}
	...
	public CollegioModel ExRicercaCollegioByKey(BigDecimal aKey) throws F3BException {
		...
		try {
			...
			throw new F3BException(
					"CollegioController.ExRicercaCollegioByKey : Non posso leggere : " + daoEx);
		} finally {
			cleanup(lColDao);
			cleanup(lColMagDao);
			cleanup(lColGiuPopDao);
		...
		try {
			...
			try {
				...
				if (daoe.getMessage().contains("COL_UFF_SEZ_DATA_UK")) {
					...
					siesLogger.info(
							"Impossibile modificare il collegio poichè già esistente: " + daoe.getMessage());
					// ricerco id_collegio
	...
	private BigDecimal ricercaCollegioBySezCodColl(BigDecimal sezIdSezione, String codCollegio,
		...
		try {
			...
			throw new F3BException(
					"CollegioController.ricercaCollegioBySezCodColl: Non posso leggere : " + daoEx);
		} finally {
			cleanup(csDAO);
			cleanup(c);
		}
	...
	public Vector ExRicercaCollegioByCodUfficio(String aCodUfficio) throws F3BException {
		...
		try {
			...
			throw new F3BException(
					"CollegioController.ExRicercaCollegioByCodUfficio: Non posso leggere : " + daoEx);
		} finally {
			cleanup(lCurDao);
			cleanup(lConn);
		}
	 ...
	 * @throws F3BException
	 */
	public String ExRicercaMaxCodCollegio(CollegioModel aCollegio, Date dataUdienza, String codMagis,
			String tipoGiudizio, String prove) throws F3BException {
		...
		try {
			...
			// 20171127: [EC] il CodGiudice è valorizzato solo per le MONOCRATICHE, pertanto in ricerca delle
			// collegiali non va passato
			...
			Vector udienze = lCtrlUdi.ExRicercaUdienzaCollegialeSige(usm, usm.getCodUfficioAppartenenza(),
					prove);
			...
			if (!Utils.isNullObj(udiTrovata) && !Utils.isNullObj(udiTrovata.getCollegio())
				...
				max = max + "#" + udiTrovata.getCollegio().getIdCollegio() + "#" + "P";
			} // P = COLLEGIO PRESENTE
			else {
				// 20171005: [SG] aggiunto codice magistrato
				lCurDao.ricercaMaxCodCollegio(aCollegio, codMagis,
				...
				if (lCurDao.next()) {
					...
					// ripeto la ricerca senza magistrato solo se non ha trovato un collegio con codice
					// magistrato
			...
			throw new F3BException(
					"CollegioController.ExRicercaMaxCodCollegio: Non posso leggere : " + daoEx);
		} finally {
			cleanup(lCurDao);
			cleanup(lConn);
		}
		...
		try {
			...
			if (collegioMagistrati != null) {
				for (int i = 0; i < collegioMagistrati.length; i++) {
					String idMagistrato = collegioMagistrati[i].getMagCodMagistrato();
			...
			rollback(lConn);
			throw new F3BException(
					"CollegioController.ExAggiornaCollegioMagistrati: Non posso modificare : " + daoex);
		} catch (Exception ex) {
			rollback(lConn);
			throw new F3BException(
					"CollegioController.ExAggiornaCollegioMagistrati: Non posso modificare : " + ex);
		} finally {
			cleanup(lColMagDao);
			cleanup(lConn);
		}
