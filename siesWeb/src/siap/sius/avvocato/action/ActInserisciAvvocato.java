...
public class ActInserisciAvvocato extends ActionSiap implements ICostantiAvvocato {
	...
	public String processRequest() throws F3BException {
		...
		if (!isRequestParameterNullObj(ICostantiAvvocatoFascicoloSius.CAMPO_COD_SEDE_AUTORITA_DIF) && (!this
	...
	private BigDecimal inserisciAggiornaAvvocato() throws F3BException {
		...
		if (getRequestStringParameter(CAMPO_ID_AVVOCATO).contains("COA") // Reginde
			...
			// **********************************************************************
			/*
			 * if (Utils.isPresent(descCodLuogoNascita)) { try { comuneNascita = new
			 * ComuneModel(getCodComuneByDescr(descCodLuogoNascita)); } catch (Exception e) {
			 * siesLogger.warn("Comune di nascita ["+descCodLuogoNascita+"] non trovato "+e.getMessage());
			 *
			 * if (getRequestStringParameter(CAMPO_ID_AVVOCATO).contains("COA")) { // Se reginde provo a
			 * decodificare dal CF
			 * siesLogger.debug("AVVOCATO REGINDE: Provo a decodificare il comune dal Codice Fiscale"); //
			 * algortimo di omocodia: comuneNascita = AvvocatoUtil
			 * .calcolaComuneNascita(getRequestStringParameter(CAMPO_CODICE_FISCALE));
			 * siesLogger.debug("comuneNascita da CF = "+comuneNascita); } else { // inserimento manuale:
			 * comune inesistente throw e; }
			 *
			 * }
			 *
			 * if (!Utils.isNullObj(comuneNascita)) { if
			 * ("039".equals(getRequestStringParameter(CAMPO_COD_STATO_NASCITA)) ) { codLuogoNascita =
			 * comuneNascita.getCodComune(); codProvincia = comuneNascita.getCodProvincia(); codCap =
			 * comuneNascita.getCap(); //?????? ce ci fai con il cap del comune di nascita???
			 * descCodLuogoNascita = comuneNascita.getDescrizione(); } } }
			 */
