...
public class ActSostituzioneDifensore extends ActionSiap implements ICostantiAvvocato {
	...
	public String processRequest() throws F3BException {
		...
		// ===========================================================
		BigDecimal idAvvocato = inserisciAggiornaAvvocato();
		...
		// n.b. in questo vettore c'è anche l'avvocato che sto sostituendo
		BigDecimal idVecchio = getRequestBigDecimalParameter("idAvvVecchio");

		// n.b. l'utente potrebbe aver selezionato lo stesso avvocato idVecchio = idAvvocato (inserito)
		// Caso vero SOLO se ha selezionato in avvocato REGINDE già a sistema. In questo caso il sistema
		// aggiorna i dati AVVOCATO.
		// In questo caso non cambia il collegamente AVVOCATO_FASCICOLO_SIUS per l'id avvocato ma potrebbero
		// cambiare gli altri dati (tipo avvocato ecc)
		// Caso particolare è il cambiamento del FORO. Viene inserito nuovamente l'avvocato (nuovo ID)
		if (lVectPrec.size() > 0) {
			Iterator iter = lVectPrec.iterator();
			while (iter.hasNext()) {
				AvvocatoModel lAvvModPrec = ((AvvocatoSiusModel) iter.next()).getAvvocato();

				if (lAvvModPrec.getIdAvvocato().compareTo(idVecchio) == 0) {
					continue;
				}
		...
		if (lAvvFascMod.getCodTipoAvvocato().equals("01")) {
			lAvvFascMod.setDataInizioValidita(
					getRequestDateParameter(ICostantiAvvocatoFascicoloSius.CAMPO_ANNO_DATA_DESIGNAZIONE,
		...
		if (lAvvFascMod.getCodTipoAvvocato().equals("02")) {
			lAvvFascMod.setDataInizioValidita(
					getRequestDateParameter(ICostantiAvvocatoFascicoloSius.CAMPO_ANNO_DATA_NOMINA,
		...
		if (!isRequestParameterNullObj(ICostantiAvvocatoFascicoloSius.CAMPO_COD_MOTIVO_DESIGNAZIONE)) {
			lAvvFascMod.setCodMotivoDesignazione(
					getRequestStringParameter(ICostantiAvvocatoFascicoloSius.CAMPO_COD_MOTIVO_DESIGNAZIONE));
		} else {
			lAvvFascMod.setCodMotivoDesignazione("-");
		}

		if (!isRequestParameterNullObj(ICostantiAvvocatoFascicoloSius.CAMPO_COD_TIPO_AUTORITA))
			lAvvFascMod.setCodTipoAutorita(
					getRequestStringParameter(ICostantiAvvocatoFascicoloSius.CAMPO_COD_TIPO_AUTORITA));
		else
			lAvvFascMod.setCodTipoAutorita("-");

		if (!isRequestParameterNullObj(ICostantiAvvocatoFascicoloSius.CAMPO_COD_SEDE_AUTORITA)
				&& (!isRequestParameterNullObj(ICostantiAvvocatoFascicoloSius.CAMPO_COD_TIPO_AUTORITA)
						&& !getRequestStringParameter(ICostantiAvvocatoFascicoloSius.CAMPO_COD_TIPO_AUTORITA)
								.equals("-"))) {
			ComuneModel lComMod = new ComuneModel(getCodComuneByDescr(
					getRequestStringParameter(ICostantiAvvocatoFascicoloSius.CAMPO_COD_SEDE_AUTORITA)));
			lAvvFascMod.setSedeAutorita(lComMod.getCodComune());
		} else {
			lAvvFascMod.setSedeAutorita("-");
		}

		if (!isRequestParameterNullObj(ICostantiAvvocatoFascicoloSius.CAMPO_INDIRIZZO_TIPO_AUTORITA))
			lAvvFascMod.setIndirizzoTipoAutorita(
					getRequestStringParameter(ICostantiAvvocatoFascicoloSius.CAMPO_INDIRIZZO_TIPO_AUTORITA));
		...
		if (!isRequestParameterNullObj(ICostantiAvvocatoFascicoloSius.CAMPO_COD_TIPO_AUTORITA_DIF))
			lAvvFascMod.setCodTipoAutoritaDif(
					getRequestStringParameter(ICostantiAvvocatoFascicoloSius.CAMPO_COD_TIPO_AUTORITA_DIF));
		else
			lAvvFascMod.setCodTipoAutoritaDif("-");
		...
		if (!isRequestParameterNullObj(ICostantiAvvocatoFascicoloSius.CAMPO_COD_SEDE_AUTORITA_DIF)
			...
			String lComneAutoritaDif = getRequestStringParameter(
					ICostantiAvvocatoFascicoloSius.CAMPO_COD_SEDE_AUTORITA_DIF);
		...
		// ======================================
		// Chiamo la funzione di Sostituzione
		// ======================================
		// idVecchio = getRequestBigDecimalParameter("idAvvVecchio");
	 ...
	 * @throws F3BException
	 * @deprecated vecchio codice sostituito per la MEV_21 dalla nuova processRequest
	 */
	...
	// }
	//
	// if (!isRequestParameterNullObj(ICostantiAvvocatoFascicoloSius.CAMPO_NOTE)) {
	...
	/**
	 * Metodo che effettua inserimento o aggiornamento dell'avvocato e restituisce l'id per collegarlo al
	 * fascicolo. Se avvocato selezionato da Reginde il sistema lo ceraca e se trovato lo aggiorna altrimenti
	 * lo inserisce. Se avvocato selezionato da SIES (non trovato su reginde) non fa nullea (già presente) e
	...
	private BigDecimal inserisciAggiornaAvvocato() throws F3BException {
		...
		if (getRequestStringParameter(CAMPO_ID_AVVOCATO).contains("COA") // Reginde
			...
			siesLogger.debug("Iscrizione manuale o Reginde: CAMPO_ID_AVVOCATO = "
					+ getRequestStringParameter(CAMPO_ID_AVVOCATO));
			...
			try {
				if (!isRequestParameterNullObj(ICostantiComune.CAMPO_COD_COMUNE_REALE)
					...
					descCodLuogoNascita = comuneNascita.getDescrizione();
					codLuogoNascita = comuneNascita.getCodComune();
					// altrimenti , in caso di Paese di Nascita Estero, dalla routine che ricava i dati dal
					// C.F.
			...
			if (Utils.isPresent(descLuogoResidenza)) {
				try {
					...
					siesLogger.warn("Comune di residenza [" + descLuogoResidenza + "] non trovato: "
							+ e.getMessage());
				}
		...
		if (getRequestStringParameter(CAMPO_ID_AVVOCATO).contains("COA")) {
			...
			/*
			 * Se l'avvocato certificato RegInde non è presente in SIES si inserisce Se è già presente in SIES
			 * si effettua l'aggiornamento con i dati da Reginde. FORO COMPRESO
			 */
			if (lAvvCertRegSies == null) {
				...
				siesLogger.debug("Avvocato Certificato Reginde già presente in SIES (con id:"
						+ lAvvCertRegSies.getIdAvvocato() + ") e certificato sul foro "
						+ lAvvCertRegSies.getForo() + ", lo aggiorno");
				...
				if (lAvvCertRegSies.getForo().equals(amReginde.getForo())) {
					...
					// n.b. aggiorno comunque l'avvocato devo testare comuqne che non sia già legato al
					// fascicolo
			...
			if (idAvvocato != null) {
				siesLogger.debug("Avvocato FORM non selezionato da reginde ma da SIEP con ID " + idAvvocato
						+ ", aggancio l'avvocato al fascicolo");
