 ...
 * *
 * <p>
 * Title: RicercaProvvedimentoModel
 * </p>
 * <p>
 * Description: La Classe Model rappresenta il filtro da utilizzare nella Ricerca di Procedimenti Sius per
 * Estremi Provvedimento. I possibili parametri utilizzati nella ricerca sono: l'intervallo delle date di
 * deposito oppure un intervallo di Estremi Provvedimento espresso come ANNO / NUM lo stato di validazione:
 * validato, non validato, annullato, tutti. La ricerca è sempre limitata ad un ufficio.
 *
 ...
 *
 */
public class RicercaProvvedimentoModel extends GenericModel implements ICostantiStatistiche {

	/**
	 *
	 */
	private static final long serialVersionUID = -5244537062877242263L;
	...
	// Intervallo per Riferimenti Provvedimenti
	...
	// Modalità di ricerca (Per: Provvedimento, Decreto, Impugnazione, Foglio Complementare.)
	...
	public RicercaProvvedimentoModel() {
		// Di default la Ricerca è per Estremi Provvedimento
		mModalitaRicerca = RICERCA_PROVVEDIMENTO;
		mCalcolatiTotali = false;
	}

	// Costruttore di copia
	public RicercaProvvedimentoModel(RicercaProvvedimentoModel aModel) {
	...
	public boolean isRicercaXProvvedimento() {
		boolean lRet = false;
		if (mModalitaRicerca != null && (mModalitaRicerca.equalsIgnoreCase(RICERCA_PROVVEDIMENTO)
				|| mModalitaRicerca.equalsIgnoreCase(RICERCA_PROVVEDIMENTI_PRIVI_DI_FC)))
			lRet = true;
		return lRet;
	}
	...
	public String getDescModalitaRicerca() {
		String lRet = "";
		if (isRicercaXProvvedimento())
			lRet = "Ricerca Procedimenti per estremi Provvedimento";
	...
	private String resetDescCalcoli() {
		...
		if (isRicercaStatoTutti() && mCalcolatiTotali) {
			...
			if (isRicercaXDecreto() || isRicercaXProvvedimento() || isRicercaXSentenza())
