...
public class StatisticheSiusController extends SiapController implements IStatisticheSius {
	...
	public BigDecimal ExGetNumRicercaProcSiusXProvvedimenti(RicercaOrdinanzaModel aModel) throws F3BException {
		...
		try {
			...
			throw new SIUSException(SIUSException.USER_MESSAGE, ICostantiStatistiche.MESSAGGIO_ERRORE_GENERICO);
	...
	/*
	 * 20140119 - Non utilizzato
	 */
	/*
	 * private Vector RicercaNumAllegatiValidati(Vector aProvvedimenti, Connection aConn) throws Exception {
	 * // Elemento corrente nel vector EveFasGepSogProvModel lProvvedimento = null;
	 * 
	 * // DAO per leggere sulla tabella DOCUMENTO_ALLEGATO EventoSqlDAO lEveDao = null; if (aProvvedimenti !=
	 * null) { try { Iterator itx = aProvvedimenti.iterator(); lEveDao = new EventoSqlDAO(aConn); int
	 * lNumValidati = -1; //Si Itera sugli elementi nel vector per calcolare il n.ro di // documenti allegati
	 * per ogni evento nella lista. while (itx.hasNext()) { lProvvedimento = ( EveFasGepSogProvModel)
	 * itx.next(); if (lProvvedimento.getEvento() != null) { lNumValidati =
	 * lEveDao.getNumAllegatiValidati(lProvvedimento.getEvento().getIdEvento());
	 * lProvvedimento.getEvento().setNumAllValidati(lNumValidati); } }
	 * 
	 * }
	 * 
	 * finally { cleanup(lEveDao); } } return aProvvedimenti; }
	 */
	...
	throws F3BException {
		...
		try {
			...
			if (lMagistrati.size() == 0) {
				...
				siesLogger
						.info("MagistratoFirmatarioController.ExRicercaMagistratiOggettiEstratti: Nessun magistrato !");
				// throw new F3BException(F3BException.USER_MESSAGE, "Nessun Elemento trovato");
			}
	 ...
	 * ISP_CONTEGGIO_OGGETTI. Tali dati sono stati memorizzati all'atto della creazione della
	 * "Statistica Procedimenti SIUS X Oggetti" e l'elenco fornito da questa funzione verrà utilizzato per la
	...
	/**
	 * La funzione restituisce l'elenco dei contatatori statistici contenuti nella tabella
	 * ISP_CONTEGGIO_MAGISTRATI. Tali dati sono stati memorizzati all'atto della creazione della
	 * "Statistica Procedimenti SIUS X Oggetti" e l'elenco fornito da questa funzione verrà utilizzato per la
	 * creazione del report.
	 * 
	 * @return
	...
	public Vector ExRicercaTempiEstratti(IspProcIntervalliModel aModel) throws F3BException {
		...
		Connection lConn = null;
		Vector<IspProcIntervalliModel> lOggetti = new Vector<IspProcIntervalliModel>();
	...
	public Collection<EveFasGepSogProvModel> ExRicercaProcFissatiNoDefPaginata(
		...
		try {
			...
			lProcedimenti = new ArrayList<EveFasGepSogProvModel>();
	...
	public Collection<EveFasGepSogModel> ExRicercaProcFissatiNoDefNumGGPaginata(
		...
		try {
			...
			lProcedimenti = new ArrayList<EveFasGepSogModel>();
	...
	public Collection<EveFasGepSogCancModel> ExRicercaProcPosizioneGiuridica(RicercaProcedimentoModel aRicerca)
			throws F3BException {
		Connection lConn = null;
		Collection<EveFasGepSogCancModel> lProcedimenti = new ArrayList<EveFasGepSogCancModel>();
		ProcPosizioneGiuridicaSqlDAO lProcPosGiuSqlDao = null;
		try {
			lConn = getDBConnection();
			lProcPosGiuSqlDao = new ProcPosizioneGiuridicaSqlDAO(lConn);
	...
	public Collection<EveFasGepSogCancModel> ExRicercaProcPosizioneGiuridicaPaginata(
		...
		try {
			...
			lProcedimenti = new ArrayList<EveFasGepSogCancModel>();
			// Necessario iterare per via della paginazione.
	...
	public Collection<IspConteggioRelatoriMagistratiModel> ExRicercaStatisticaComparataMagistrati(
		...
		Connection lConn = null;
		Collection<IspConteggioRelatoriMagistratiModel> lConteggi = new ArrayList<IspConteggioRelatoriMagistratiModel>();
		StatisSiusStoreProcedureDAO lSpDao = null;
		...
		try {
			...
			if (aRicerca.getCodTipoUfficio().equals("TDS") || aRicerca.getCodTipoUfficio().equals("TDSM")) {
				...
				DecodificheUtils.getFilteredByCodAlt(DecodificheManager.getInstance()
						.getMotivoProvvedimento(), "^[C]\\d{3}");
			} else if (aRicerca.getCodTipoUfficio().equals("UDS")
					|| aRicerca.getCodTipoUfficio().equals("UDSM")) {
				lMotivi = // DecodificheManager.getInstance().getMotivoProvvedimento();
				DecodificheUtils.getFilteredByCodAlt(DecodificheManager.getInstance()
						.getMotivoProvvedimento(), "^[U]\\d{3}");
			}
	...
	public ArrayList<IspMotivoOggettoSelezionatiModel> ExListaMotiviOggettiSelezionati() throws F3BException {
		...
		ArrayList<IspMotivoOggettoSelezionatiModel> lElenco = new ArrayList<IspMotivoOggettoSelezionatiModel>();
	...
	public Collection<IspEstrazioneOggettiModel> ExRicercaProcedimentiEstrattiOggettiSelezionatiOrdinati(
		...
		Connection lConn = null;
		Collection<IspEstrazioneOggettiModel> lOggetti = new Vector<IspEstrazioneOggettiModel>();
		IspEstrazioneOggettiTribDAO lDao = null;
		try {
			lConn = getDBConnection(); // Preleva connessione dal dbase
			lDao = new IspEstrazioneOggettiTribDAO(lConn);
	 ...
	 * @throws F3BException
	 */
	public Vector ExRicercaMagistratiProcIntervalli(String aCodUfficio)
	throws F3BException {
		Connection lConn = null;
		Vector lMagistrati = new Vector();
		MagistratoFirmatarioSqlDAO lMagSqlDao = null;
