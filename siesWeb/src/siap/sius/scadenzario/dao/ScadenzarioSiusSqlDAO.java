...
public class ScadenzarioSiusSqlDAO extends SIAPSqlDAO {
	...
	public void ricercaFinePenaProcedimentiPendentiPaginata(String riferimento, BigDecimal ai, BigDecimal ni,
		...
		if (!test) {
			lStatement += ", CP.DATA_SCARC_LA_FUNG FINE_PENA_VIRTUALE,"
					+ " TRUNC(CP.DATA_SCARC_LA_FUNG - SYSDATE) GIORNI_RESIDUI_VIRTUALI";
		}
		lStatement += " FROM FASCICOLO_SIUS FSIUS,"
				...
				+ " FASCICOLO_SIEP FSIEP, UFFICIO UFFSIEP, COMUNE DESCR_COMUNE_UFFICIO_SIEP";
		if (!test) {
			lStatement += ", CALCOLO_PENA_DL92 CP";
		}
		...
		if (!test) {
			lStatement += " AND CP.FAS_SIE_ID_FASCICOLO_SIEP = FSIEP.ID_FASCICOLO_SIEP"
					+ "	AND CP.DATA_SCARC_LA_FUNG IS NOT NULL AND CP.DATA_INSERIMENTO ="
					+ "	(SELECT MAX(DATA_INSERIMENTO) FROM CALCOLO_PENA_DL92"
					+ " WHERE FAS_SIE_ID_FASCICOLO_SIEP = FSIEP.ID_FASCICOLO_SIEP)";
		}
		lStatement += " AND PR.FLAG_VALIDATO = 'S'"
