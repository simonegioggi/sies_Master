 ...
 * @version 1.0
 */
public class ActConfermaTrasmissione extends ActionSiap implements ICostantiDepositoSentenza, ICostantiJMS {
	...
	public String processRequest() throws Exception {
		...
		// Si Prepara la trasmissione della Sentenza.
		// I destinatari aggiuntivi sono di 2 tipi (aggiunto l'UEPE)
		...
		if (lTipoUff[0].trim().compareTo("-") != 0) {
			...
			MessaggioModel lMessage = lCtrlMess.getMessageForSentenza(lEveId);
			...
			if (!this.isRequestParameterNullObj("codTipoOperazione"))
				...
				lMessage.setCodTipoOperazione(TRASFERIMENTO_SENTENZA);
		...
		if (!isRequestParameterNullObj("lCheckUffici")) {
			...
			for (int x = 0; x < lSize; x++) {
				if (!lCheckUffici[x].equals("1")) {
					...
					lMessage.setCodUfficioMittente(getCodUfficioUtenteConnesso());
					lMessage.setCodTipoMessaggio(RICHIESTA);
					// STUB 23/09/2004 Se valorizzato il parametro codTipoOperazione, lo si utilizza per il
					// messaggio.
					if (!this.isRequestParameterNullObj("codTipoOperazione"))
						lMessage.setCodTipoOperazione(getRequestStringParameter("codTipoOperazione"));
					else
					...
					if (lFasGPMod.getFascicoloSiusModel().getSoggetto() != null) {
						if (lFasGPMod.getFascicoloSiusModel().getSoggetto().getNome() != null)
							lMessage.setNomeSoggetto(
									lFasGPMod.getFascicoloSiusModel().getSoggetto().getNome());
						if (lFasGPMod.getFascicoloSiusModel().getSoggetto().getCognome() != null)
							lMessage.setCognomeSoggetto(
									lFasGPMod.getFascicoloSiusModel().getSoggetto().getCognome());
						if (lFasGPMod.getFascicoloSiusModel().getSoggetto().getDataNascita() != null)
							lMessage.setDataNascita(
									lFasGPMod.getFascicoloSiusModel().getSoggetto().getDataNascita());
						if (lFasGPMod.getFascicoloSiusModel().getSoggetto().getCodComuneNascita() != null)
							lMessage.setCodComuneNascita(
									lFasGPMod.getFascicoloSiusModel().getSoggetto().getCodComuneNascita());
						if (lFasGPMod.getFascicoloSiusModel().getSoggetto().getCodStatoNascita() != null)
					...
					// LogF3B.getLogger()
					siesLogger.debug(
							"MESSAGGIO DA SPEDIRE A " + lBDI.getDescrComune() + " : " + lMessage.toString());
		...
		setRequestAttribute(IWebConstants.MESSAGE_TEXT, "Trasmissione Sentenza sottomessa al Sistema!");
		...
		if (this.isRequestParameterNullObj("codTipoOperazione")) {
			// setta la risposta nella request
			lRedirigi.setAction("siap.sius.depositosentenza.action.ActLoadTrasferisciSentenza");
			setRequestAttribute(IWebConstants.MESSAGE_TEXT, "Trasmissione Sentenza sottomessa al Sistema!");
		}
		/*
		 * else { lRedirigi.setParameter(ICostantiEvento.CAMPO_COD_TIPO_PROVVEDIMENTO, "03"); String
		 * strMessage=""; if (lTipoUfficio.compareTo("UDS")==0) strMessage =
		 * "Trasmissione Impugnazione sottomessa al Sistema!"; else
		 * strMessage="Trasmissione Ricorso sottomessa al Sistema!";
		 * setRequestAttribute(IWebConstants.MESSAGE_TEXT, strMessage ); lRedirigi.setAction(
		 * "siap.sius.impugnazione.action.ActLoadDettaglioImpugnazione" ); }
		 */
