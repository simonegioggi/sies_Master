...
public class ActInserisciRogatoria extends ActInserisciRicAtti implements ICostantiRichiestaAtti {
	...
	public String processRequest() throws Exception {
		...
		lEveNot.getEvento().setCodMotivo("0533");
		...
		for (int x = 0; x < lSize; x++) {
			if (!lDestinatari[x].equals("-") && !lSedi[x].equals("")) {
				if (x == 0) // destinatario interno (CSSA)
				{
					...
					String lDescrComune = lSedi[x];
					String lCodTipoUfficioDest = lDestinatari[x];
