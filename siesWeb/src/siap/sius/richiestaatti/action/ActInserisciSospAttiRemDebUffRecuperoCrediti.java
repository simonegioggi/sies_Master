...
/**
 * <p>
 * Title: ActInserisciSospAttiRemDebUffRecuperoCrediti
 * </p>
 * <p>
...
public class ActInserisciSospAttiRemDebUffRecuperoCrediti extends ActInserisciRicAtti
	...
	public String processRequest() throws Exception {
		...
		lEveNot.getEvento().setCodMotivo("0583"); // CodMotivo - Sospensione Atti per Remissione Debito
		...
		for (int x = 0; x < lTipoDest.length; x++) {
			...
			if (lTipoDest[x].equalsIgnoreCase("UFF_REC_CRED")) // Gestione Autorità Esterna
			{
		...
		if (lCampiNoteReq != null) {
			...
			for (int x = 0; x < lSize; x++) {
				...
				if (!lCampiNoteReq[x].equals("")) {
					...
					if (x == 1)
						lCampoNotaMod.setDescr("Numero Campione Penale: " + lCampiNoteReq[x]);
					else
