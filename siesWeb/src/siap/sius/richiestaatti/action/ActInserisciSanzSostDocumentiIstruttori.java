...
/**
 * <p>
 * Title: ActInserisciSanzSostDocumentiIstruttori
 * </p>
 * <p>
 * Description: Classe di tipo Azione responsabile della raccolta dei dati inputati nella form d'inserimento.
 * Inoltre, creazione evento e relative notifiche. La classe eredita le funzionalità generalizzate dalla
 * ActInserisciRicAtti
 * </p>
 * <p>
...
public abstract class ActInserisciSanzSostDocumentiIstruttori extends ActInserisciRicAtti
	...
	public String processRequest() throws Exception {
		...
		// Esegue controllo che le classi fidlie abbiano impostato il codice motivo.
		// In caso contrario sollevano un Error exception.
		if (lEveNot.getEvento().getCodMotivo() == null)
			throw new SIUSException(
					"Errore il codice motivo ( CodMotivo ) dell'EventoModel non è valorizzato nella classe derivata.");
		...
		for (int x = 0; x < lTipoDest.length; x++) {
			...
			} else if (lTipoDest[x].equalsIgnoreCase("UEPE")) // Gestione UEPE
			{
				if (!lSedi[x].equals("")) {
					// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
					// LogF3B.getLogger()
					...
					lNotifica = creaNotifica(lDataEmissione, lCodiceOperatore, lCodiceUfficio);
					String lDescrCSSA = lSedi[x]; // Gestione UEPE.
			...
			} else if (lTipoDest[x].equalsIgnoreCase("AUT_EXT")) // Gestione Autorità Esterna
			{
