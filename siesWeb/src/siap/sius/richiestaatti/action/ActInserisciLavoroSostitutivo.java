...
public class ActInserisciLavoroSostitutivo extends ActInserisciRicAtti implements ICostantiRichiestaAtti {
	...
	public String processRequest() throws Exception {
		...
		lEveNot.getEvento().setCodMotivo("0582"); // CodMotivo - Invito x accordi preliminari x Lavoro
													// Sostitutivo
		...
		for (int x = 0; x < lTipoDest.length; x++) {
			...
			/*
			 * if( lTipoDest[x].equalsIgnoreCase("UFF_GIUD") ) // Gestione Ufficio Giudiziario. { // Gestione
			 * UFF_GIUD if( !lDestinatari[x].equals("-") && !lSedi[x].equals("") ) { // [FT] - 03/08/2016 -
			 ...
			 * 
			 * // Preleva il codice d'ufficio per la coppia comune e codice // tipo ufficio. String
			 * lCodUfficioDest = getCodUfficioByCodTipoUfficioDescrComune( lDestinatari[x], lSedi[x] );
			 * lNotifica.setUffCodUfficio( lCodUfficioDest ); } } else if(
			 * lTipoDest[x].equalsIgnoreCase("IST_DET") ) // Gestione Istituto di Detenzione { // Gestione
			 * IST_DET if( !lDestinatari[x].equals( "" ) ) { // [FT] - 03/08/2016 - MAC_LOG - Utilizzo la
			 * variabile di istanza siesLogger al posto di LogF3B.getLogger() siesLogger.debug(
			 * ">>> Lavorazione TipoDestinatario : " + lTipoDest[x] );
			 * 
			 * lNotifica = creaNotifica( lDataEmissione, lCodiceOperatore, lCodiceUfficio );
			 * lNotifica.setIstDetIdIstitutoDetenzione( lDestinatari[x] ); lNotifica.setUffCodUfficio("-"); }
			 * } else if( lTipoDest[x].equalsIgnoreCase("CSSA") ) // Gestione CSSA { if( !lSedi[x].equals("")
			 * ) { // [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
			 * LogF3B.getLogger() siesLogger.debug( ">>> Lavorazione TipoDest : " + lTipoDest[x] );
			 * 
			 * lNotifica = creaNotifica( lDataEmissione, lCodiceOperatore, lCodiceUfficio ); String lDescrCSSA
			 * = lSedi[x]; // Gestione CSSA. // Preleva l'id del CSSA attraverso la propria descrizione.
			 * BigDecimal lIdCSSA = getIdCSSAByDescrComune( lDescrCSSA ); // Imposta l'IdCSSA nella Notifica.
