...
/**
 * Action di presa in carico atti ricevuti. Utilizzata sia sa SIEP che da SIUS
 * sia per atti provenienti stessa BDI che altra BDI.
 * 
 * @author d.fiorletta
 *
 */
public class ActConfermaPresaInCaricoAttiSiep extends ActionSiap implements ICostantiJMS
{
  ...
  public String processRequest() throws Exception
  {
    ...
    try
    {
      ...
      // STUB 19/20/2008 Presa in carico Provvedimento di Sanzione Sostitututiva
      else if (lMessIns.getCodTipoOperazione().compareTo(ICostantiJMS.TRASFERIMENTO_SANZIONE_SOSTITUTIVA)==0)
    ...
    catch (F3BException ex)
    {
       ...
       throw new SIUSException(SIUSException.USER_MESSAGE, "Si è verificato un errore durante il caricamento dell'Atto pervenuto.<br> Riprovare in seguito.");
    }
    ...
    // STUB 19/20/2008 Presa in carico Provvedimento di Sanzione Sostitututiva
    else if (lMessIns.getCodTipoOperazione().compareTo(ICostantiJMS.TRASFERIMENTO_PROVVEDIMENTO)==0)
    ...
    // Si punta all'ultimo elemento della lista (invece del primo) perchè è il più recente, ed è quindi quello relativo all'istanza.
    //EventoNotificaModel lEventoInviato = (EventoNotificaModel)lPars.getDettaglioFascicoloSiep().getEventi().get(0);
    EventoNotificaModel lEventoInviato = (EventoNotificaModel)lPars.getDettaglioFascicoloSiep().getEventi().get(lPars.getDettaglioFascicoloSiep().getEventi().size()-1);
    ...
    // STUB 08/10/2004 Presa in carico Provvedimento LS va distinto dall'Istanza
    if (lMessIns.getCodTipoOperazione().compareTo(ICostantiJMS.TRASFERIMENTO_PROVVEDIMENTO)==0)
      strReturn = IWebConstants.ROOT_DIR + "files/siap/siep/ordineesecuzione/RapportoTrasferimentoProvvedimento.jsp";
    // STUB 19/02/2008 Presa in carico Provvedimento di Sanzione Sostitututiva
    else if (lMessIns.getCodTipoOperazione().compareTo(ICostantiJMS.TRASFERIMENTO_SANZIONE_SOSTITUTIVA)==0)
      strReturn = IWebConstants.ROOT_DIR + "files/siap/siep/sanzionesostitutiva/RapportoTrasferimentoSanzioneSostitutiva.jsp";
    // TRASFERIMENTO per COMPETENZA
    else if(lMessIns.getCodTipoOperazione().compareTo(ICostantiJMS.TRASFERIMENTO_COMPETENZA)==0)
    	strReturn = IWebConstants.ROOT_DIR + "files/siap/siep/istanza/RapportoTrasferimentoIstanza.jsp";
    // 16/03/2009 Presa in carico Provvedimento di Conversione Pene Pecuniarie
    else if (lMessIns.getCodTipoOperazione().compareTo(ICostantiJMS.TRASFERIMENTO_ATTI_CONVERSIONE)==0)
      strReturn = IWebConstants.ROOT_DIR + "files/siap/siep/penapecuniaria/RapportoTrasferimentoAttoConversionePP.jsp";
    else
      strReturn = IWebConstants.ROOT_DIR + "files/siap/siep/istanza/RapportoTrasferimentoIstanza.jsp";
