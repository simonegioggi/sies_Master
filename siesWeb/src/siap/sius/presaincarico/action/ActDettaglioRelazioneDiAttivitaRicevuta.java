...
/**
 * <p>
 * Title: ActDettaglioRelazioneDiAttivitaRicevuta
 * </p>
 * <p>
 * Description:
 * </p>
 ...
 * @version 1.0
 */
public class ActDettaglioRelazioneDiAttivitaRicevuta extends ActionSiap implements ICostantiPresaincarico {

	public String processRequest() throws Exception {

		gestioneRitorno();
		// Controllo che non si stia lavorando su una entità in modifica ad altri.
		LockModel lck = lockIfNotLocked("caricoattivita",
		...
		if (lck != null) {
			...
			setRequestAttribute(IWebConstants.MESSAGE_TEXT,
					"La presa in carico di questa Relazione Di Attività è in gestione ad un altro utente! <BR>Riprovare più tardi!");
		...
		if (lParser.getFascicoloSiepeEsteso() != null)
			...
			throw new SIEPEException(SIEPEException.USER_MESSAGE,
					"Errore nella Ricezione del Procedimento SIEPE Esteso. <BR>Rivolgersi all'amministratore di sistema! ");
		...
		// EVENTO
		/*
		 * EventoModel lEvento = new EventoModel(); if (lParser.getFascicoloSiepeEsteso().getEvento()!=null)
		 * lEvento = lParser.getFascicoloSiepeEsteso().getEvento(); else throw new SIEPEException(
		 * SIEPEException.USER_MESSAGE,
		...
		if (lParser.getAttivita() != null)
			this.setRequestAttribute("attivita", lParser.getAttivita());
		...
		return PG_DETTAGLIO_RELAZIONE_ATTIVITA_RICEVUTA;
	}
