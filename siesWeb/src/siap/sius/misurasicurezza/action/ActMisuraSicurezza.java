...
public class ActMisuraSicurezza extends ActionSius
	...
	protected  PeriodoAltraMisuraModel recuperoDatiMaschera(
			PeriodoAltraMisuraModel lPerMod) throws F3BException {

	    lPerMod.setDataInizioEsecuzione ( getRequestDateParameter       ( CAMPO_ANNO_DATA_INIZIO_ESECUZIONE,CAMPO_MESE_DATA_INIZIO_ESECUZIONE,CAMPO_GIORNO_DATA_INIZIO_ESECUZIONE) );
		...
		if(!this.isRequestParameterNullObj(ICostantiSiusMisuraSicurezza.CAMPO_SOSPENSIONE_GG))
			lPerMod.setSospensioneGG	  	(getRequestBigDecimalParameter	( CAMPO_SOSPENSIONE_GG) );
		if(!this.isRequestParameterNullObj(ICostantiSiusMisuraSicurezza.CAMPO_SOSPENSIONE_MM))
			lPerMod.setSospensioneMM		(getRequestBigDecimalParameter	( CAMPO_SOSPENSIONE_MM) );
		if(!this.isRequestParameterNullObj(ICostantiSiusMisuraSicurezza.CAMPO_SOSPENSIONE_AA))
			lPerMod.setSospensioneAA		(getRequestBigDecimalParameter	( CAMPO_SOSPENSIONE_AA) );
		// 12/05/2008 if(!this.isRequestParameterNullObj(ICostantiSiusMisuraSicurezza.CAMPO_NUMERO_GIORNI))
		//	lPerMod.setNumeroGiorni		(getRequestBigDecimalParameter	( CAMPO_NUMERO_GIORNI) );
		if(!this.isRequestParameterNullObj(ICostantiSiusMisuraSicurezza.CAMPO_ANNO_DATA_SCADENZA))
		...
		if(!this.isRequestParameterNullObj(ICostantiSiusMisuraSicurezza.CAMPO_DA_RECUPERARE))
			lPerMod.setDaRecuperare			(getRequestStringParameter		( CAMPO_DA_RECUPERARE) );
    if(!this.isRequestParameterNullObj(ICostantiSiusMisuraSicurezza.CAMPO_DA_RECUPERARE_GG))
    	lPerMod.setDaRecuperareGG		(getRequestBigDecimalParameter	( CAMPO_DA_RECUPERARE_GG) );
    if(!this.isRequestParameterNullObj(ICostantiSiusMisuraSicurezza.CAMPO_DA_RECUPERARE_MM))
      lPerMod.setDaRecuperareMM		(getRequestBigDecimalParameter	( CAMPO_DA_RECUPERARE_MM) );
    if(!this.isRequestParameterNullObj(ICostantiSiusMisuraSicurezza.CAMPO_DA_RECUPERARE_AA))
      lPerMod.setDaRecuperareAA		(getRequestBigDecimalParameter	( CAMPO_DA_RECUPERARE_AA) );
	    ...
	    if(!this.isRequestParameterNullObj(ICostantiSiusMisuraSicurezza.CAMPO_COD_TIPO_AUTORITA))
		{
		   	 lPerMod.setCodTipoAutorita(this.getRequestStringParameter(ICostantiSiusMisuraSicurezza.CAMPO_COD_TIPO_AUTORITA));
		}else{
		     lPerMod.setCodTipoAutorita("-");
		}

		 // controllo dell'istituto di detenzione
	    if(!this.isRequestParameterNullObj(ICostantiSiusMisuraSicurezza.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE))
		{
		   	 lPerMod.setIstDetIdIstitutoDetenzione(this.getRequestStringParameter(ICostantiSiusMisuraSicurezza.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE));
		}else{
	...
	protected PeriodoEsecuzioneMisuraModel calcoloDataScadenza(
				PeriodoAltraMisuraModel lPerMod,
				EsecuzioneMisuraSicurezzaModel lEMSModelPar) throws F3BException
	{
	...
	//	data scadenza = data inizio + periodo indicato
		...
		EsecuzioneMisuraSicurezzaModel lEMSModel = new EsecuzioneMisuraSicurezzaModel(lEMSModelPar);
		PeriodoEsecuzioneMisuraModel lPerEse = new PeriodoEsecuzioneMisuraModel();
		if (lPerMod.getDataScadenza() == null) {
			if (lPerMod.getSospensioneGG() != null
					|| lPerMod.getSospensioneMM() != null
				...
				//10/06/2011 lPerMod.setDataScadenza(lCal.exCalcolaNuovaDataFine(lPerMod.getDataInizioEsecuzione(), lCalModDurata, false));
				lPerMod.setDataScadenza(lCal.exCalcolaNuovaDataFine(lPerMod.getDataInizioEsecuzione(), lCalModDurata, true));

				if (lEMSModel == null
						|| lEMSModel.getIdEsecuzioneMisuraSicurezza().equals(null))
...
/*
		  ...
		  // Eliminato controllo
				...
				if (lPerMod.getDataScadenza().after(
						lEMSModel.getDataTermineAttuale()))
					throw new SIUSException(SIUSException.USER_MESSAGE,
							"Data Scadenza Sospensione deve essere inferiore alla Data Termine Misura'");
*/
			}
      ...
      // 12/05/2008 Controllo Periodo modificato.
        ...
        int giorniDaRecuperare = (AA*365) + (MM*30) + GG;
				...
				{
					throw new SIUSException(SIUSException.USER_MESSAGE, "Numero di giorni superiore alla Data Scadenza");
		 		}
				Date lDataTermineAttuale = DateUtils.moveDateTo(lEMSModel.getDataTermineAttuale(), Calendar.DAY_OF_MONTH, giorniDaRecuperare );
				lEMSModel.setDataTermineAttuale(lDataTermineAttuale);
			}
		}
		lPerEse.setPeriodoAltraMisura(lPerMod);
		lPerEse.setEsecuzioneMisuraSicurezza(lEMSModel);
		return lPerEse;
	}

	protected  CalendarModel quantumEspiata(PeriodoAltraMisuraModel lPerMod,
			EsecuzioneMisuraSicurezzaModel lEMSModel) throws F3BException {
		// calcolo dei quantum misura sicurezza espiata
		Date DataInizio = null;
		Date DataFine = null;
		if ((lPerMod.getDataScadenza() != null) && lPerMod.getFlagMotivo().equals("03")){
			// 10/06/2011 DataFine = lPerMod.getDataScadenza();
			DataFine = lPerMod.getDataInizioEsecuzione();
		} else {
			DataFine = lPerMod.getDataInizioEsecuzione();
		}
		DataInizio = lEMSModel.getDataInizioMisura();
	...
	protected EsecuzioneMisuraSicurezzaModel termineFinale(
				PeriodoAltraMisuraModel lPerMod,
				EsecuzioneMisuraSicurezzaModel lEMSModel) throws F3BException{
    
   
		CalendarModel lCalModDurata = new CalendarModel();
		lCalModDurata.setNumAnni(lEMSModel.getNumAnniMisura() );
		lCalModDurata.setNumMesi(lEMSModel.getNumMesiMisura() );
		lCalModDurata.setNumGiorni(lEMSModel.getNumGiorniMisura() );
    ...
    lCalModDurata = lCalUtil.ricalcolaGAM(lCalModDurata);
		...
		ICalcoloPena lCal = SIEPLookupRemote.getCalcoloPenaRemote();
		lEMSModel.setDataTermineIniziale((lCal.exCalcolaNuovaDataFine(lPerMod
				.getDataInizioEsecuzione(), lCalModDurata, true)));
		lEMSModel.setDataTermineAttuale((lCal.exCalcolaNuovaDataFine(lPerMod
				.getDataInizioEsecuzione(), lCalModDurata, true)));
	...
	protected  CalendarModel quantumTotale(EsecuzioneMisuraSicurezzaModel lEMSModel) throws F3BException {
		// calcolo dei quantum misura sicurezza residua da espiare
		Date DataInizio = null;
		Date DataFine = null;
		DataInizio = lEMSModel.getDataInizioMisura();
		DataFine = lEMSModel.getDataTermineAttuale();
	...
	protected  PeriodoAltraMisuraModel riempioquantumEspiataeResidua(
			PeriodoAltraMisuraModel lPerMod,
	...
	protected EventoModel preparoEvento(PeriodoAltraMisuraModel lPerMod
			) throws F3BException {
...
//	    lEve.setCodMagistrato(getRequestStringParameter(ICostantiMagistrato.CAMPO_COD_MAGISTRATO));
	...
	lEve.setCodTipoEvento("20");  // Comunicazione inizio misura sicurezza
	...
	if(!this.isRequestParameterNullObj(ICostantiDepositoDecreto.CAMPO_COD_UFFICIO_MAGISTRATO_COMP ))
		lEve.setCodMagistrato(getRequestStringParameter( ICostantiDepositoDecreto.CAMPO_COD_UFFICIO_MAGISTRATO_COMP ) );

    return lEve;
	}

	protected void caricoIstitutiDetenzione(PeriodoAltraMisuraModel lPerMod)
		throws F3BException {
	...
	protected ScadenzarioSiusModel riempioScadenzario(PeriodoAltraMisuraModel lPerMod,
		...
		if (lScaMod.getCodOperatoreInserimento() == null || lScaMod.getCodOperatoreInserimento().length()==0 ) {
		...
		if (lPerMod.getEveIdEvento() != null) lScaMod.setEveIdEvento(lPerMod.getEveIdEvento());
		return lScaMod;
	}
}
