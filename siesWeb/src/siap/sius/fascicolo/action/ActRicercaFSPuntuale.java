...
public class ActRicercaFSPuntuale extends ActionSius implements ICostantiFascicoloSius
{
  ...
  // 07/06/2004 Aggiunto attributo per parametrizzare il controllo dello stato fascicolo (=false evita i controlli per filtrare le operazioni sui procedimenti).
  ...
  public String processRequest() throws Exception
  {
    ...
    if (this.mControl)
      mFasGPMod = lCtrl.ExRicercaFascicoloByAnnoProgrCodUfficio(getRequestBigDecimalParameter(CAMPO_CHIAVE_ANNO), getRequestBigDecimalParameter(CAMPO_CHIAVE_PROGR), StrCodiceUfficioUtente );
    else
      mFasGPMod = lCtrl.ExRicercaFascicoloByAnnoProgrCodUfficioNoControl(getRequestBigDecimalParameter(CAMPO_CHIAVE_ANNO), getRequestBigDecimalParameter(CAMPO_CHIAVE_PROGR), StrCodiceUfficioUtente );


    // 30/04/2007 Si Consente alla fase di "Richiesta atti" di operare con i Procedimenti di ESECUZIONE MISURE ALTERNATIVE.
    // 03/08/2007 Si Consente alla fase di "Richiesta atti" di operare con i Procedimenti di ESECUZIONE SANZIONI SOSTITUTIVE.
    ...
    //
    // N.B.: 28/04/2008 Si esclude il controllo per le azioni ActRicercaFSPRichiestaAtti, ActRicercaStatoAtti e ActRicercaFSPSanzioniSostitutive
    //       riferite al menu di "Richiesta Atti".
    if ( mFasGPMod != null && mFasGPMod.getGeneraleProcedimentoModel() != null &&
        ( mFasGPMod.getGeneraleProcedimentoModel().getCodOggettoProcedimento().compareTo(COD_OGGETTO_PROCEDIMENTO_MA) == 0 ||
          mFasGPMod.getGeneraleProcedimentoModel().getCodOggettoProcedimento().compareTo(COD_OGGETTO_PROCEDIMENTO_SS) == 0 ||
          mFasGPMod.getGeneraleProcedimentoModel().getCodOggettoProcedimento().compareTo(COD_OGGETTO_PROCEDIMENTO_MS) == 0 ) &&
    ...
    {
        String lDescrOggettoProcedimento = mFasGPMod.getGeneraleProcedimentoModel().getDescrOggettoProcedimento();
        throw new F3BException(F3BException.USER_MESSAGE, "Operazione non consentita per il Procedimento di " + lDescrOggettoProcedimento );
    }
    else
    {
      ...
      FascicoloSiepModel lFasSIEP = lCtrlSIEP.ExRicercaFascicoloByKeyNoError( mFasGPMod.getFascicoloSiusModel().getFasSieIdFascicoloSiep());
