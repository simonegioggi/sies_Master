...
/**
 * ActInsFascicoloDaSoggettoUDS - Classe Azione di inserimento del Fascicolo SIUS da Soggetto
 */
public class ActInsFascicoloDaSoggettoUDS extends ActionSiap implements ICostantiFascicoloSius {
	...
	public String processRequest() throws Exception {
		...
		if (getRequestBigDecimalParameter(CAMPO_CHIAVE_ANNO_S22) != null
			...
			if (!isRequestParameterNullObj("CampoIdFascicoloPadreEsecuzioneHidden")) {
				...
				if (getRequestBigDecimalParameter("CampoIdFascicoloPadreEsecuzioneHidden") != null) {
					IdFascicoloPadreEsecuzione = getRequestBigDecimalParameter(
							"CampoIdFascicoloPadreEsecuzioneHidden");
		...
		// Foreign KEY del soggetto
		lFasGPMod.getFascicoloSiusModel().setSogIdSoggetto(lSoggettoMod.getIdSoggetto());
		// Foreign KEY del fascicolo SIEP
		...
		// Soggetto recuperato dalla sessione
		lFasGPMod.getFascicoloSiusModel().setSoggetto(lSoggettoMod);
		...
		if (getRequestStringParameter(CAMPO_COD_CONTENUTO).compareTo("U004") == 0
			...
			aDescrUfficioMittente = DecodificheUtils.getDescbyCode(
					DecodificheManager.getInstance().getMittenteAtto(),
					getRequestStringParameter(CAMPO_COD_MITTENTE_ATTO));
			aCodUfficioMittente = getCodUfficioByCodTipoUfficioDescrComune(DecodificheUtils
					.getCodebyDesc(DecodificheManager.getInstance().getTipoUfficio(), aDescrUfficioMittente),
		...
		if (!isRequestParameterNullObj(ID_LUOGO_DETENZIONE)
			...
			// setto a NULL, altrimenti SIEP lo vede tra le posiz. giuridiche
		...
		// lFasGPMod = lCtrl.ExInserisciFascicoloSiusUDS(lFasGPMod, null, IdFascicoloPadreEsecuzione);
			...
			lFasGPMod = lCtrl.ExInserisciFascicoloSiusUDS(lFasGPMod,
					(getSessionAttribute("IdEventoInviato") + ""), IdFascicoloSiusPadre, durataEsitoAnni,
					durataEsitoMesi, durataEsitoGiorni);
