   ...
   */
    ...
    //  lEvento.setFlagDocumentoRegistrato( lEveSqlDao.getFlagDocumentoRegistrato( aIdEvento ) );
      ...
      if( lDepDecrMotMod.getDepositoDecreto() == null )
        throw new SIUSException( SIUSException.USER_MESSAGE, "Nessun decreto per l'evento selezionato");
      else
        // Cerca eventuali Ufficio competente e Procura Esecuzione
      ...
      Vector lMotivazioni = new Vector( lMotSqlDao.getModels() );
			lDepDecrMotMod.setMotivazioniDecreto((MotivazioneDecretoModel[]) lMotivazioni
					.toArray(new MotivazioneDecretoModel[0]));
		} catch (DAOException daoEx) {
			// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
			// LogF3B.getLogger()
      siesLogger.debug("DAOException: " + daoEx);
   ...
   */
    ...
    SoggettoSqlDAO lSogDao = null;
      ...
      siesLogger.error("DAOException",daoEx);
			throw new SIUSException("DepositoDecretoController.ExRicercaDecretoFascicoloByIdSoggetto: "
					+ daoEx);
		} catch (Exception ex) {
			// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
			// LogF3B.getLogger()
    ...
    boolean lEsiste = false;
      ...
      if( lNum > 0 )
        lEsiste = true;
			...
			// LogF3B.getLogger()
      siesLogger.debug("DAOException: " + daoEx);
			throw new SIUSException("DepositoDecretoController.ExEsisteDepositoDecretoByGenProcCodEsito: "
					+ daoEx);
		} finally {
      cleanup(lDepSqlDao);
      cleanup(lConn);
   ...
   */
    ...
    if (lIdEvento == null)
     throw new SIUSException("ID Evento nullo");
	  ...
	  lMisAltDao.delete();	 
		siesLogger.debug(
				">>>> Cancellate misure alternative collegate aL depositoDecreto " + lIdEvento);
...
*/
  ...
  public  DepositoDecretoEventoModel  ExInserisciDecretoIrreperibilità( GPTenoreModel aGpTenoreModel,
    ...
    DepositoDecretoEventoModel lModelRet      = null;
      ...
      // Update GeneraleProcedimento.
			lGenProcDao.setCodOggettoProcedimento(lGPTenoreModel.getGeneraleProcedimentoModel()
					.getCodOggettoProcedimento());
			lGenProcDao.setDataAggiornamento(lGPTenoreModel.getGeneraleProcedimentoModel()
					.getDataAggiornamento());
			lGenProcDao.setCodUfficioAggiornamento(lGPTenoreModel.getGeneraleProcedimentoModel()
					.getCodUfficioAggiornamento());
			lGenProcDao.setCodOperatoreAggiornamento(lGPTenoreModel.getGeneraleProcedimentoModel()
					.getCodOperatoreAggiornamento());
			lGenProcDao.setCondizioneUpdate(lGPTenoreModel.getGeneraleProcedimentoModel()
      ...
      lTenore.setDataAggiornamento(lGPTenoreModel.getGeneraleProcedimentoModel().getDataAggiornamento());
      lTenore.setDataFine(lGPTenoreModel.getGeneraleProcedimentoModel().getDataAggiornamento());
			lTenore.setGenPridGeneraleProcedimento(lGPTenoreModel.getGeneraleProcedimentoModel()
					.getIdGeneraleProcedimento());
      lTenore.setCodUfficioInserimento(lDepDecrModel.getCodUfficioInserimento());
      lTenore.setDescrUfficioInserimento(lDepDecrModel.getDescrUfficioInserimento());
   ...
   */
    ...
    DepositoDecretoEventoModel lModelRet      = null;
		...
		try {
      lConn = getDBTransaction();
			lModelRet = ExInserisciDecreto(aGPTenoreModel, aDepDecrEveModel, aDepDecrEveModel.getEvento()
					.getCodEsito(), lConn);
      commit(lConn);
		} catch (F3BException e) {
      rollback(lConn);
   ...
   */
	public DepositoDecretoEventoModel ExInserisciDecretoPeriodoAltraSanzione(GPTenoreModel aGPTenoreModel,
			DepositoDecretoEventoModel aDepDecrEveModel,
  ...
  // con quelli del decreto da revocare.
	private void controlloDatiRevoca(DepositoDecretoEventoModel aDepDecrEveModel) throws F3BException {
		if (aDepDecrEveModel != null && aDepDecrEveModel.getEvento() != null
			...
			DepositoDecretoFascicoloModel lDepDecMod = ExRicercaDecretoFascicoloByIdEvento(aDepDecrEveModel
					.getEvento().getEveIdEvento());
  ...
  private TreeModel prelevaDatiDecreto( EventoNotificaModel aEvento, FascicoloGPModel lFasModel)
    ...
    BigDecimal lKeyFascicolo = new BigDecimal(0);
      ...
      lLuoDao = new LuogoDetenzioneSqlDAO(lConn);
			lLuoDao.ricercaLuogoDetenzioneCorrenteByFascicoloSius(lFasModel.getFascicoloSiusModel()
					.getIdFascicoloSius());
      ...
      //Crea il TreeModel con i dati che occorrono
			lTreeFasMod = lCtrlSta.ExAggiungiDatiStampa(lFasModel.getFascicoloSiusModel()
					.getIdFascicoloSius(), aTipoDati, lTreeFasMod);
    ...
    DepositoDecretoModel lDecMod = new DepositoDecretoModel(aDepositoDecreto);
      ...
      DecodificheModel lDecodifiche = new DecodificheModel();
        ...
        lDecodDao = new DecodificheDAO(lConn);
				lDecodDao.setCondizioneContestoRwLowValue("OGGETTO_PROCEDIMENTO", aFasGPMod
...
// 25/07/2014 DL 146/2013 - Revoca L.A. e  DL 92 2014 - Violazione CEDU
    ...
    // Update LicenzaLibAnticipata va aggiornata per FAS_SIE_ID_FASCICOLO_SIEP.
				lLicLibDAO.setDAOFromDecretoForUpdate(lDecMod, aFasGPMod.getFascicoloSiusModel()
						.getFasSieIdFascicoloSiep());
          lLicLibDAO.update();
      }
// End DL 146/2013      
     ...
     // boolean lGiaEsisteMA = lMADao.esisteMisuraAlternativaPerEvento(lEveMod.getIdEvento());
			...
			if (lExistFasSiep
					&& (lEveMod.getCodEsito() != null &&
					// STUB 24-02-2006 La Misura Alternativa si iscrive se il Deposito non riguarda la
					// Fissazione Udienza.
					(lEveMod.getCodEsito().compareTo("0601") != 0 && lEveMod.getCodEsito().compareTo("0602") != 0))
    ...
    DepositoDecretoSqlDAO lDecDao = null;
      ...
      // In caso di Decreto con Data Deposito valorizzata, è possibile caricare i destinatari.
        ...
        INotifica lNotCtrl = SIEPLookupRemote.getNotificaRemote();
				/* Vector lNotifica = */lNotCtrl.ExRicercaEstesaNotificaByKeyEvento(lDecMod
						.getIdEventoGenerato());
      }
		} catch (DAOException daoEx) {
			// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
  ...
  public DepositoDecretoModel ExRicercaDepositoDecretoByEveIdEventoNoDescTipoDecreto ( BigDecimal aEveKey)
	...
	private DepositoDecretoModel RicercaUffici(DepositoDecretoModel aDepDecreto) throws F3BException {
      ...
      lCodUff = aDepDecreto.getCodProcuraRevocato();
        ...
        lUfficio = lUff.getUfficioByKey(lCodUff);
        if (lUfficio != null)
				aDepDecreto.setDescrProcuraRevocato(lUfficio.getDescrTipoUfficio() + " di "
						+ lUfficio.getDescrComune());
      }
     ...
     lCodUff = aDepDecreto.getCodUfficioInserimento();
       ...
       lUfficio = lUff.getUfficioByKey(lCodUff);
       if (lUfficio != null)
				aDepDecreto.setDescrUfficioInserimento(lUfficio.getDescrTipoUfficio() + " di "
						+ lUfficio.getDescrComune());
     }
   ...
   lCodUff = aDepDecreto.getCodUfficioAggiornamento();
     ...
     lUfficio = lUff.getUfficioByKey(lCodUff);
     if (lUfficio != null)
				aDepDecreto.setDescrUfficioAggiornamento(lUfficio.getDescrTipoUfficio() + " di "
						+ lUfficio.getDescrComune());
   }
...
*/
  ...
  */
  ...
  public DepositoDecretoModel ExRicercaDepositoDecretoByAnnoNumUfficio (DepositoDecretoModel aDepositoDecreto )
			throws F3BException {
    Connection lConn = null;
    DepositoDecretoDAO lDepDao = null;
    DepositoDecretoModel lDepMod;
  ...
  throws F3BException {	  
	    ...
	    try {
	      ...
	      lDepDecrSqlDao.ricercaDepositoDecretoByIdEveGenerato(aDepDecrMod.getIdEventoGenerato());
			aDepDecrMod.setIdDepositoDecreto(((DepositoDecretoModel) lDepDecrSqlDao.getModelByKey())
...
//Inserisce LIBERAZIONE_ANTICIPATA SPECIALE   (Periodo Unico)
			if (aLicenzaC_SPE != null) {
				// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
				// LogF3B.getLogger()
				siesLogger.debug("--> DepositoDecretoController - aLicenzaC_SPE != null no semestri"
						+ aLicenzaC_SPE);
...
//Inserisce LIBERAZIONE_ANTICIPATA INTEGRAZIONE   (Periodo Unico)
			if (aLicenzaC_INT != null) {
				// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
				// LogF3B.getLogger()
				siesLogger.debug("---> DepositoDecretoController - aLicenzaC_INT != null no semestri"
						+ aLicenzaC_INT);
    	...
    	rollback(lConn);
			throw new F3BException("DepositoDecretoController.ExInserisciDecretoRevocaLiberazAnticipata: "
					+ ex);
    	...
    	//throw e;
			...
			// LogF3B.getLogger()
    	siesLogger.debug("Exception: " , e);
			throw new SIUSException("DepositoDecretoController.ExInserisciDecretoRevocaLiberazAnticipata:"
					+ e);
		} finally {
      cleanup(lConn);
    }
