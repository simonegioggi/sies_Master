 ...
 * @version 1.0
 */
public class ActConfermaTrasmissione extends ActionSiap implements ICostantiJMS {
	...
	public String processRequest() throws Exception {
		...
		// Si Prepara la trasmissione del decreto.
		// STUB 11/09/2006 I destinatari aggiuntivi sono di 2 tipi (aggiunto l'UEPE)
		...
		if (lTipoUff[0].trim().compareTo("-") != 0) {
			lCodiceUfficio = getCodUfficioByCodTipoUfficioDescrComune(lTipoUff[0], lSedeUff[0]);
			...
			MessaggioModel lMessage = lCtrlMess.getMessageForDecreto(lEveId);
			...
			// STUB 23/09/2004 Se valorizzato il parametro codTipoOperazione, lo si utilizza per il messaggio.
			if (!isRequestParameterNullObj("codTipoOperazione"))
				lMessage.setCodTipoOperazione(getRequestStringParameter("codTipoOperazione"));
			else
				lMessage.setCodTipoOperazione(TRASFERIMENTO_DECRETO);
			...
			if (lFasGPMod.getFascicoloSiusModel().getSoggetto() != null) {
				...
				if (lFasGPMod.getFascicoloSiusModel().getSoggetto().getCodComuneNascita() != null)
					lMessage.setCodComuneNascita(lFasGPMod.getFascicoloSiusModel().getSoggetto()
							.getCodComuneNascita());
				if (lFasGPMod.getFascicoloSiusModel().getSoggetto().getCodStatoNascita() != null)
					lMessage.setCodStatoNascita(lFasGPMod.getFascicoloSiusModel().getSoggetto()
							.getCodStatoNascita());
			}
			// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
			// LogF3B.getLogger()
		...
		// Invio eventuale all'UEPE di competenza.
		if (lTipoUff[1].trim().compareTo("-") != 0) {
			lCodiceUfficio = getCodUfficioByCodTipoUfficioDescrComune(lTipoUff[1], lSedeUff[1]);
			lLocal = getUfficioByCodUfficio(lCodiceUfficio);
			lBDI = getUfficioByCodUfficio(lLocal.getCodDistretto());
			...
			lMessage.setCodUfficioMittente(getCodUfficioUtenteConnesso());
			lMessage.setCodTipoMessaggio(RICHIESTA);
			// Se valorizzato il parametro codTipoOperazione, lo si utilizza per il messaggio.
			if (!isRequestParameterNullObj("codTipoOperazione"))
				lMessage.setCodTipoOperazione(getRequestStringParameter("codTipoOperazione"));
			else
				lMessage.setCodTipoOperazione(TRASFERIMENTO_DECRETO);
...
//
//			String lCodUfficio[] = getRequestStringParameters("lCodUfficio");
//			String lChiaveAnno[] = getRequestStringParameters("lChiaveAnno");
//			String lChiaveProgr[] = getRequestStringParameters("lChiaveProgr");
//
//			int lSize = lCodUfficio.length;
//
//			for (int x = 0; x < lSize; x++) {
//				if (!lCheckUffici[x].equals("1")) {
...
//
//					// SETTA RIFERIMENTI FASCICOLO SIEP/SIUS.
//					lMessage.setChiaveAnnoSiep(new BigDecimal(lChiaveAnno[x]));
//					lMessage.setChiaveProgrSiep(new BigDecimal(lChiaveProgr[x]));
//					lMessage.setChiaveAnnoSius(lFasGPMod.getFascicoloSiusModel().getChiaveAnno());
//					lMessage.setChiaveProgrSius(lFasGPMod.getFascicoloSiusModel().getChiaveProgr());
//
//					// 27/06/2006 UEPE
//					if (lFasGPMod.getFascicoloSiusModel().getSoggetto() != null) {
//						if (lFasGPMod.getFascicoloSiusModel().getSoggetto().getNome() != null)
//							lMessage.setNomeSoggetto(lFasGPMod.getFascicoloSiusModel().getSoggetto()
//									.getNome());
//						if (lFasGPMod.getFascicoloSiusModel().getSoggetto().getCognome() != null)
//							lMessage.setCognomeSoggetto(lFasGPMod.getFascicoloSiusModel().getSoggetto()
//									.getCognome());
//						if (lFasGPMod.getFascicoloSiusModel().getSoggetto().getDataNascita() != null)
//							lMessage.setDataNascita(lFasGPMod.getFascicoloSiusModel().getSoggetto()
//									.getDataNascita());
//						if (lFasGPMod.getFascicoloSiusModel().getSoggetto().getCodComuneNascita() != null)
//							lMessage.setCodComuneNascita(lFasGPMod.getFascicoloSiusModel().getSoggetto()
//									.getCodComuneNascita());
...
//		}
		 ...
		 //05/12/2019 :  ANOMALIA n° 3 del COLLAUDO 11.3 (terza sessione)
		...
		if (isRequestParameterNullObj("codTipoOperazione")) {
			...
			setRequestAttribute(IWebConstants.MESSAGE_TEXT, "Trasmissione Decreto sottomessa al Sistema!");
			lRedirigi.setAction("siap.sius.depositodecreto.action.ActLoadTrasferisciDecreto");
		} else {
			lRedirigi.setParameter(ICostantiEvento.CAMPO_COD_TIPO_PROVVEDIMENTO, "02");
			String strMessage = "";
			...
			lRedirigi.setAction("siap.sius.impugnazione.action.ActLoadDettaglioImpugnazione");
		}
