...
public class ActLoadDettaglioDecretoDeposito extends ActionSius
		implements ICostantiDepositoDecreto, ICostantiTemplate {
	...
	public String processRequest() throws Exception {
		...
		if ((lTipoDecreto.compareTo(INAMMISSIBILITA) == 0) && (lFasGPMod.getGeneraleProcedimentoModel()
			...
			if ((mDepDecrMotMod.getMotivazioniDecreto() != null)
				...
				for (int j = 0; j < mDepDecrMotMod.getMotivazioniDecreto().length; j++) {
					mDepDecrMotMod.getMotivazioniDecreto()[j]
							.setDescrMotivazione(mDepDecrMotMod.getMotivazioniDecreto()[j]
		...
		if (lTipoDecreto.compareTo(GENERICO) == 0 || lTipoDecreto.compareTo(GENERICO2) == 0
			...
			setFunctionsAvailableToRequest(
					"siap.sius.depositodecreto.action.ActLoadDettaglioDecretoInammissibilita");
			...
			setFunctionsAvailableToRequest(
					"siap.sius.depositodecreto.action.ActLoadDettaglioDecretoIncompetenza");
			...
			if (lDepDecreto.getIstDetIdIstitutoDetenzione() != null
				...
				lIstMod = lCtrlIst
						.ExRicercaIstitutoDetenzioneByKey(lDepDecreto.getIstDetIdIstitutoDetenzione());
			}
		...
		// ***** FINE INTERVENTO MEV_39 *****//
			...
			setRequestAttribute(IWebConstants.MESSAGE_TEXT,
					"Tipo di decreto non supportato. [" + lTipoDecreto + "]");
		}
		// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
		// LogF3B.getLogger()
	...
	private void gestioneAvvocati(FascicoloGPModel aFasGPMod) throws Exception {
		...
		Vector lAvvocato = lAvvCtrl
				.ExRicercaAvvocatiByFascicoloNoError(aFasGPMod.getFascicoloSiusModel().getIdFascicoloSius());
		...
		if (lDepDecrMotMod != null && lDepDecrMotMod.getDepositoDecreto() != null) {
			...
			if (lDepDecrMotMod.getEvento() != null
				...
				FascicoloGPModel lFascicoloSius = ricercaFascicoloSIUS(
						lDepDecrMotMod.getEvento().getFasSiuIdFascicoloSius());
	...
	private void ricercaPeriodoAltraSanzione() throws Exception {
		...
		PeriodoAltraSanzioneModel mPerAlSanz = lCtrlPAS
				.ExRicercaSanzioneSostitutivaByIdEvento(mDepDecrMotMod.getEvento().getIdEvento());
	...
	private void ricercaPeriodoAltraMisura() throws Exception {
		...
		PeriodoAltraMisuraModel mPerAlMisu = lCtrlPAM
				.ExRicercaMisuraSicurezzaByIdEvento(mDepDecrMotMod.getEvento().getIdEvento());
