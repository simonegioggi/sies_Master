...
public class ActConfermaPresaInCarico extends ActionSiap implements ICostantiJMS
{
  ...
  //Controllo che non si stia lavorando su una entità in modifica ad altri
      LockModel lck =
      lockIfNotLocked("caricodecreto", getRequestStringParameter(ICostantiMessaggio.CAMPO_ID_MESSAGGIO), getCodUtenteConnesso());
      if (lck != null)
      {
        setRequestAttribute (IWebConstants.MESSAGE_TEXT, "La presa in carico di questo decreto è in gestione ad un altro utente! <BR>Riprovare più tardi!");
    ...
    //lMisAlt.setNote(getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_NOTE));     // STUB 03/12/2007
...
//    }
    ...
    setRequestAttribute(IWebConstants.MESSAGE_TEXT, "Ricezione Decreto Completata e Esito rispedito al Mittente!");
  ...
  //----------------------------------------------------------------------------/
    ...
    lRedirigi.setAction( "siap.sius.depositodecreto.action.ActListaDecretiRicevuti" );
