...
* <p>Description: Classe Action per l'inserimento della data di Deposito Ordinanza</p>
 ...
 * Questo perchè l'inserimento dei destinatari è comune sia al Decreto che all'Ordinanza.</p>
 ...
 */
 public BigDecimal  aggiornaProvvedimento() throws Exception
 {
   ...
   // Se l'Ordinanza non è già depositata bisogna lockare per evitare che a 2 Ordinanze venga attribuito lo stesso progressivo.
   if(mDepMod.getNumS3()== null )
   {
	    // Lock
	    LockModel lck = LockController.lockIfNotLocked(getServletContext(),"DEPOSITO_ORDINANZA",getCodUfficioUtenteConnesso(),getCodUtenteConnesso(),getSession().getId());
	    if (lck!=null)
	    {
   ...
   mDepMod.setDataDeposito(getRequestDateParameter(ICostantiDepositoOrdinanzaPc.CAMPO_ANNO_DATA_DEPOSITO, ICostantiDepositoOrdinanzaPc.CAMPO_MESE_DATA_DEPOSITO, ICostantiDepositoOrdinanzaPc.CAMPO_GIORNO_DATA_DEPOSITO));
 ...
 public String  inserisciDati(EventoNotificaModel aEveNot) throws Exception
 {
   ...
   {   
	   ...
	   DocumentoAllegatoModel lDocAllMod = lCtrlDO.ExInserisciDataDepositoOrdinanza(mFasGP, mDepMod, aEveNot,  lCheck, lScadenzarioSiusModPrincipal, lScadenzarioSiusModSecond);

	   //restituisce la jsp di VIEW.
	   lPage = IWebConstants.PG_MAIN + "?" + IWebConstants.ACTION_FIELD + "=siap.sius.depositoordinanzapc.action.ActLoadDettaglioDataDepositoOrdinanza&"+ ICostantiDepositoOrdinanzaPc.CAMPO_ID_DOCUMENTO_ALLEGATO +"="+lDocAllMod.getIdDocumentoAllegato();
   	}
