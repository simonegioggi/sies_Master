...
public class ActInserisciOrdinanzaConversioneRevocaPPS extends ActInserisciOrdinanzaUDS {
	...
	public OrdinanzaEventoTenoriGProcModel inserimento(OrdinanzaEventoTenoriGProcModel aOrdEveTenGP)
		...
		// String lTipoRichiestaCPP = getRequestStringParameter(ICostantiTenore.CAMPO_COD_OGGETTO_TENORE);

		// Importo da convertire
		String lValoreIntRata = this
				.getRequestStringParameter(ICostantiSiusPenaPecuniaria.CAMPO_INTERO_IMPORTO_MULTA);
		String lValoreDecRata = this
				.getRequestStringParameter(ICostantiSiusPenaPecuniaria.CAMPO_DECIMALE_IMPORTO_MULTA);
		...
		String lNumGiorniDurataEsito = this
				.getRequestStringParameter(ICostantiSiusPenaPecuniaria.CAMPO_NUM_GIORNI_SS);
		String lNumMesiDurataEsito = this
				.getRequestStringParameter(ICostantiSiusPenaPecuniaria.CAMPO_NUM_MESI_SS);
		String lNumAnniDurataEsito = this
				.getRequestStringParameter(ICostantiSiusPenaPecuniaria.CAMPO_NUM_ANNI_SS);

		lRicConv.setDurataEsitoAnni(
				lNumAnniDurataEsito.trim().length() > 0 ? new BigDecimal(lNumAnniDurataEsito) : null);
		lRicConv.setDurataEsitoMesi(
				lNumMesiDurataEsito.trim().length() > 0 ? new BigDecimal(lNumMesiDurataEsito) : null);
		lRicConv.setDurataEsitoGiorni(
				lNumGiorniDurataEsito.trim().length() > 0 ? new BigDecimal(lNumGiorniDurataEsito) : null);

		// inserimento
		IDepositoOrdinanzaPc IDepOrdCtrl = SIUSLookupRemote.getDepositoOrdinanzaPcRemote();
		OrdinanzaEventoTenoriGProcModel lModRet = IDepOrdCtrl
				.ExInserisciOrdinanzaRevocaConversionePPS(aOrdEveTenGP, lRicConv);
