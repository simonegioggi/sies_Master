...
public class ActLoadEmissioneApplicazioneProvvisoriaMA extends ActLoadEmissioneDecreto {
	...
	public String processRequest() throws Exception {
		...
		// e che sia stato emesso un EVENTO con cod_tipo_provvedimento = 02, cod_esito = 0610 e flag_documento_registrato = ‘S’
		
		FascicoloGPModel lFascicoloGPModel = new FascicoloGPModel((FascicoloGPModel) getSessionAttribute("fascicoloSiusGP"));
		if (   !"C050".equals(lFascicoloGPModel.getGeneraleProcedimentoModel().getCodOggettoProcedimento())
			&& !"C051".equals(lFascicoloGPModel.getGeneraleProcedimentoModel().getCodOggettoProcedimento())
		   )
		{
			// throw new SIUSException(SIUSException.USER_MESSAGE, "Operazione non consentita per il Contenuto del fascicolo");	
			throw new SIUSException(SIUSException.USER_MESSAGE, "Operazione non consentita per il Procedimento di " + lFascicoloGPModel.getGeneraleProcedimentoModel().getDescrOggettoProcedimento());
		}
		...
		if (idd.ExVerificaEsistenzaDepositoDecretoByIdGenProcCodTipoDec(idGP,DECRETO_DESIGNAZIONE_MAGISTRATO_RELATORE_PER_MA)) {
