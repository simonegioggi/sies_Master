...
public class EsecuzioneMisuraAlternativaSqlDAO extends SIAPSqlDAO {
	...
	public void ricercaDataScadenzaProcEsecMA(RicercaProcedimentoModel rpm) {
		...
		statement = "SELECT distinct EMA.ID_ESECUZIONE_MISURA_ALTERNATI, EMA.ANNO_S07,"
				...
				+ " FASC.ID_FASCICOLO_SIUS_ORIGINE ID_FASCICOLO_SIUS_ORIGINE,"
				+ " MA.DATA_INIZIO_MISURA DATA_INIZIO_MISURA_MA,"
				+ " MA.DATA_FINE_MISURA, MA.COD_TIPO_MISURA COD_TIPO_MISURA_MA,"
				+ " MA.DATA_AGGIORNAMENTO DATA_AGGIORNAMENTO_MA, RA.RV_LOW_VALUE, RA.RV_ABBREVIATION"
				...
				+ " CG_REF_CODES RA WHERE " + getCondizione(rpm)
				...
				+ " AND RA.RV_LOW_VALUE = MA.COD_TIPO_MISURA AND RA.RV_DOMAIN = 'RICERCA_EMA'"
				+ " AND (EMA.COD_TIPO_MISURA = RA.RV_LOW_VALUE OR EMA.COD_TIPO_MISURA = RA.RV_ABBREVIATION)"
				+ " AND MA.COD_NATURA_DECISIONE = 'CO' AND MA.DATA_FINE_MISURA IS NOT NULL"
				+ " AND MA.DATA_FINE_MISURA IN (SELECT MAX(DATA_FINE_MISURA) FROM MISURA_ALTERNATIVA"
				+ " WHERE FAS_SIE_ID_FASCICOLO_SIEP(+) = FASC.FAS_SIE_ID_FASCICOLO_SIEP)"
				// + " AND MA.DATA_AGGIORNAMENTO IS NOT NULL"
				// + " ORDER BY CHIAVE_ANNO, CHIAVE_PROGR ASC";
				+ " ORDER BY MA.DATA_FINE_MISURA DESC";
