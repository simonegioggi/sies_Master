...
/**
* <p>Title: ActInserisciUlterioreIstanza</p>
* <p>Description: Classe Action per l'inserimento di UlterioreIstanza</p>
...
public class ActInserisciUlterioreIstanza extends ActionSius implements ICostantiUlterioreIstanza
{
  ...
  public String processRequest() throws Exception
  {
    ...
    lUltMod.setDataRichiesta( getRequestDateParameter(  CAMPO_ANNO_DATA_RICHIESTA,
    ...
    lUltMod.setDataArrivoCancelleria( getRequestDateParameter( CAMPO_ANNO_DATA_ARRIVO_CANCELLERIA,
                                                               ...
                                                               CAMPO_GIORNO_DATA_ARRIVO_CANCELLERIA) );
    ...
    if ((getRequestStringParameter(CAMPO_SEDE_MITTENTE)).equalsIgnoreCase(""))
      ...
      lUltMod.setSedeMittente( super.getCodComuneByDescr(getRequestStringParameter( CAMPO_SEDE_MITTENTE )).getCodComune() );
    ...
    UlterioreIstanzaModel lUltModRet = lCtrl.ExInserisciUlterioreIstanza(lUltMod, lTenori);		 // setta la risposta nella request
    ...
    lPage = IWebConstants.PG_MAIN + "?" + 
            IWebConstants.ACTION_FIELD  + "=siap.sius.ulterioreistanza.action.ActLoadDettaglioUlterioreIstanza&" +
  ...
  /**
   * Metodo che si occupa di leggere i tenori dalla Request, eseguire la scomposizione
   * secondo il Pattern PIPE, ed inserire l'elenco dei tenori, opportumante raccolti in 
   * forma di Array, all'interno dell'UlterioreIstanzaModel  
   * <p>
   * @param aUltIstMod Istnaza del model by reference.
   */
  private void letturaTenori( UlterioreIstanzaModel aUltIstMod ) throws F3BException
  {
    ...
    //String lCodOgg = getRequestStringParameter( ICostantiUlterioreIstanza.CAMPO_COD_OGGETTO_PROCEDIMENTO);
    StringTokenizer lCodOggetto = new StringTokenizer(getRequestStringParameter( ICostantiUlterioreIstanza.CAMPO_COD_OGGETTO_PROCEDIMENTO) ,"|");
    StringTokenizer lDescrOggetto = new StringTokenizer(getRequestStringParameter( ICostantiUlterioreIstanza.CAMPO_DESCR_OGGETTO_PROCEDIMENTO) ,"\n");
    ...
    while (lCodOggetto.hasMoreTokens())
    {
      ...
      lUltIstTenModel.setCodUfficioInserimento( getCodUfficioUtenteConnesso()); //Codice dell'ufficio dell'operatore che inserisce
      lUltIstTenModel.setCodOperatoreInserimento (getCodUtenteConnesso()); //Codice dell'operatore che inserisce
      lUltIstTenModel.setDataInserimento( DateUtils.getSysDate() );      
      lUltIstTenModel.setCodMagistrato( mFascicoloGP.getGeneraleProcedimentoModel().getCodAutoritaDelegata() );
      ...
      {
        String lCodDettaglioCorrente = lStCodiceDet.substring(lStCodiceDet.indexOf(lUltIstTenModel.getCodOggettoTenore()+"0")+4,
  ...
  /**
   * Metodo che preleva dalla lista dei tenori di Ulteriore istanza
   * l'array dei tenori, e contestualmente alimenta un secondo array 
   * in uscita dei Tenori generici.
   * <p>  
   * @param aUltIst model popolato con i dati inviati dall chiamante.
  ...
  private TenoreModel[] toArrayTenori( UlterioreIstanzaModel aUltIst ) 
  {
    ...
    for( int i=0; i<lSize; i++ )
    {
      ...
      lTenori[i].setGenPridGeneraleProcedimento( this.mFascicoloGP.getGeneraleProcedimentoModel().getIdGeneraleProcedimento());
