...
/**
 * <p>
 * Title: ActLoadInserisciImpugnazione
 * </p>
 * <p>
 * Description: Classe Action per la load inserisci di Impugnazione
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class ActLoadInserisciImpugnazione extends ActionSius implements ICostantiImpugnazione {
	...
	public String processRequest() throws Exception {
		...
		// MEV63 (PER UDSM DEVONO APPARIRE I CODICI 01,02 E 03 QUINDI ELIMINO IL CONTROLLO SU UDSM PER IL
		// FILTRO 01
		if (strCodTipoUfficio.compareTo("TDS") == 0 || strCodTipoUfficio.compareTo("TDSM") == 0)
			lOption.setFilter("01");
		else
			lOption.setFilter(new String[] { "01", "02", "03" });

		setRequestAttribute("tipoRicorso", "" + lOption);

		// Imposta ComboBOX Soggetto Impugnante.
		if (Utils.isNullObj(lImpMod) || isRequestParameterNullObj(CAMPO_ID_IMPUGNAZIONE)) {
		...
		String[] lFilterTDR = null;
		if (strCodTipoUfficio.compareTo("TDS") == 0) {
			lFilterTDR = new String[] { "-", "01", "02", "03", "04", "05", "06", "08" };
		} else {
			lFilterTDR = new String[] { "-", "01", "02", "03", "04", "05", "06", "07", "08", "09" };
		}
		...
		String[] lFilterUDSM = { "CSS", "TDSM" };
		if (strCodTipoUfficio.equals("TDS") || strCodTipoUfficio.equals("TDSM"))
			lOption.setFilter(lFilterTDS);
		else if (strCodTipoUfficio.equals("UDSM"))
			lOption.setFilter(lFilterUDSM);
		else
		...
		// 01/10/2007 Switch Inserimento/Aggiornamento.
		String lTipoOper = "";
		if (!(isRequestParameterNullObj("tipoOper")))
			lTipoOper = getRequestStringParameter("tipoOper");

		if (lTipoOper.compareTo("A") == 0)
			setRequestAttribute("modalita", "M");
		else if (lTipoOper.compareTo("I") == 0)
			setRequestAttribute("modalita", "I");
		else
			setRequestAttribute("modalita", "I");

		return PG_LOAD_INSERISCIIMPUGNAZIONE; // restituisce la jsp di VIEW
	}
