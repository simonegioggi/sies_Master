...
public class ActSanzioneSostitutiva extends ActionSius
	...
	protected  PeriodoAltraSanzioneModel recuperoDatiMaschera(
			PeriodoAltraSanzioneModel lPerMod) throws F3BException {
    ...
    if(!this.isRequestParameterNullObj(ICostantiSanzioneSostitutiva.CAMPO_DA_RECUPERARE_AA))
      lPerMod.setDaRecuperareAA		(getRequestBigDecimalParameter	( CAMPO_DA_RECUPERARE_AA) );
		...
		{
		   	 lPerMod.setCodTipoAutorita(this.getRequestStringParameter(ICostantiSanzioneSostitutiva.CAMPO_COD_TIPO_AUTORITA));
		}else{
		     lPerMod.setCodTipoAutorita("-");
		}

		 // controllo dell'istituto di detenzione
	    if(!this.isRequestParameterNullObj(ICostantiSanzioneSostitutiva.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE))
		{
		   	 lPerMod.setIstDetIdIstitutoDetenzione(this.getRequestStringParameter(ICostantiSanzioneSostitutiva.CAMPO_IST_DET_ID_ISTITUTO_DETENZIONE));
		}else{
	...
	protected PeriodoEsecuzioneSanzioneModel calcoloDataScadenza(
				PeriodoAltraSanzioneModel lPerMod,
	...
	//	data scadenza = data inizio + periodo indicato
      ...
      // 12/05/2008 Controllo Periodo modificato.
        ...
        int giorniDaRecuperare = (AA*365) + (MM*30) + GG;
				...
				Date lDataTermineAttuale = DateUtils.moveDateTo(lESSModel.getDataTermineAttuale(), Calendar.DAY_OF_MONTH, giorniDaRecuperare );
	...
	protected EsecuzioneSanzioneSostitutivaModel termineFinale(
				PeriodoAltraSanzioneModel lPerMod,
				EsecuzioneSanzioneSostitutivaModel lESSModel) throws F3BException{
    ...
    lCalModDurata = lCalUtil.ricalcolaGAM(lCalModDurata);
		...
		ICalcoloPena lCal = SIEPLookupRemote.getCalcoloPenaRemote();
		lESSModel.setDataTermineIniziale((lCal.exCalcolaNuovaDataFine(lPerMod
				.getDataInizioEsecuzione(), lCalModDurata, true)));
		lESSModel.setDataTermineAttuale((lCal.exCalcolaNuovaDataFine(lPerMod
				.getDataInizioEsecuzione(), lCalModDurata, true)));
	...
	protected  PeriodoAltraSanzioneModel riempioquantumEspiataeResidua(
			PeriodoAltraSanzioneModel lPerMod,
	...
	protected EventoModel preparoEvento(PeriodoAltraSanzioneModel lPerMod
			) throws F3BException {
...
//	    lEve.setCodMagistrato(getRequestStringParameter(ICostantiMagistrato.CAMPO_COD_MAGISTRATO));
	...
	protected void caricoIstitutiDetenzione(PeriodoAltraSanzioneModel lPerMod)
		throws F3BException {
