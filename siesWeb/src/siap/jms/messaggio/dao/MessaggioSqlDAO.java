...
/**
 * MessaggioSqlDAO - Classe SqlDAO che rappresenta la tabella Messaggio
 *
 * @version 1.0
 */
...
public class MessaggioSqlDAO extends SIAPSqlDAO {
	...
	public void ricercaMessaggioRicercaConFiltri(String aDeliveryMode, Vector<String> aListaTipoMessaggio,
		...
		if (aChiaveUfficioFasCumulante != null)
			lSql += " AND MESSAGGIO.CHIAVE_UFFICIO_FAS_CUMULANTE = '" + aChiaveUfficioFasCumulante + "'";
		lSql += " order by DATA_INVIO DESC "; // dal piu' recente
		// Se la ricerca e' paginata estraggo solo i risultati da visualizzare sulla pagina indicata
		if (aPage > 0) {
			lSql = " SELECT * FROM (SELECT INNER.* , Rownum rn FROM ( " + lSql
					+ " ) INNER ) WHERE rn between " + ((aPage - 1) * IWebConstants.RESULT_PER_PAGE + 1)
					+ " AND " + (aPage) * IWebConstants.RESULT_PER_PAGE;
	...
	public void ricercaMessaggioRichiestaPerTipoOperazioneFascicolo(String aUfficioRicevente,
		...
		// inserito controllo su "SIEPE"
				(aTipoOperazione.trim().length() == 5 && aTipoOperazione.compareTo("SIEPE") != 0))
			// poiche' la variabile TipoOperazione viene utilizzata anche come flag
			lSql += " AND COD_TIPO_OPERAZIONE = '" + aTipoOperazione + "'";
		// STUB 15/03/2005 Se si e' inclusi gli atti presi in carico, va esclusa la condizione sul FLAG_VISTO
		if (aIncludeInCarico.compareTo("S") != 0)
			lSql += " AND FLAG_VISTO='N' ";
	 ...
	 * aTipoOperazione, aAnnoFascicolo, aProgrFascicolo, aIncludeInCarico, null ); } public void
	 * ricercaMessaggioRichiestaPerTipoOperazioneFascicolo( String aUfficioRicevente, String aUfficioMittente,
	 * String aTipoOperazione, BigDecimal aAnnoFascicolo, BigDecimal aProgrFascicolo, String aIncludeInCarico,
	 * String aFlagVisto ) throws DAOException { String lSql = getSqlQuery(); lSql +=
	 * " AND COD_TIPO_MESSAGGIO = '01' AND COD_UFFICIO_DESTINATARIO = '" + aUfficioRicevente + "'"; if
	 * (!aUfficioMittente.equals("-")) // 26/06/2006 lSql += " AND COD_UFFICIO_MITTENTE = '" +
	 * aUfficioMittente + "'"; if (aTipoOperazione != null && (aTipoOperazione.trim().length() == 5 ) ) lSql
	 * += " AND COD_TIPO_OPERAZIONE = '" + aTipoOperazione + "'"; // STUB 15/03/2005 Se si e' inclusi gli atti
	 * presi in carico, va esclusa la condizione sul FLAG_VISTO. // 20070614 - Riadattato per impostare dati
	 * di flagVisto, per filtro su occorrenze if(aIncludeInCarico != null &&
	 * aIncludeInCarico.compareTo("S")!=0) lSql += " AND FLAG_VISTO='N' "; else if( aFlagVisto != null &&
	 * aFlagVisto.compareTo("-")!=0 ) lSql += " AND FLAG_VISTO='" + aFlagVisto + "' "; // Trasferimento
	 * ISTANZA, PROVVEDIMENTO, RIF_FAS_SIUS. if (aTipoOperazione != null &&
	 ...
	 * " AND CHIAVE_PROGR_SIUS = '" + aProgrFascicolo + "'"; } // Trasferimento ATTIVITA, RICHIESTA_UEPE,
	 * RELAZIONE_UEPE, RICHIESTA_RELAZIONE. else if (aTipoOperazione != null &&
	 * (aTipoOperazione.compareTo(ICostantiJMS.TRASFERIMENTO_ATTIVITA)==0 ||
	 * aTipoOperazione.compareTo(ICostantiJMS.TRASFERIMENTO_RICHIESTA_UEPE)==0 ||
	 * aTipoOperazione.compareTo(ICostantiJMS.TRASFERIMENTO_RELAZIONE_UEPE)==0 ||
	 * aTipoOperazione.compareTo(ICostantiJMS.TRASFERIMENTO_RICHIESTA_RELAZIONE)==0 ||
	 ...
	 * " AND CHIAVE_PROGR_SIEPE = '" + aProgrFascicolo + "'"; } lSql += setOrder(); setStatement(lSql); }
	 */
	...
	public void ricercaMessaggioRichiestaPerTipoOperazioneFascicolo(MessaggioModel aMessaggio,
		...
		// 26/06/2006
		if (!aMessaggio.getCodUfficioMittente().equals("-") && !aMessaggio.getCodUfficioMittente().equals(""))
			lSql += " AND COD_UFFICIO_MITTENTE = '" + aMessaggio.getCodUfficioMittente() + "'";
		// 2007-06-25 Patch Migliorabile
		if (aMessaggio.getCodTipoOperazione() != null
				&& aMessaggio.getCodTipoOperazione().trim().length() == 5
				// inserito controllo su "SIEPE"
	...
	protected String getSqlQuery(boolean aFlagBlob) {
		...
		lStatement += "SELECT ID_MESSAGGIO, COD_TIPO_MESSAGGIO, tipoMess.DESCRIZIONE DescrTipoMessaggio,"
				...
				+ " COD_UFFICIO_DESTINATARIO, comuffdest.DESCRIZIONE destSede,"
				+ " tipuffdest.RV_MEANING tipodest, comuff.DESCRIZIONE sede, tipuff.RV_MEANING tipouff,"
				...
				+ " COD_UFFICIO_INOLTRO, tipUffInoltro.RV_MEANING descrUffInoltro,"
				+ " comUffInoltro.DESCRIZIONE descrSedeUfficioInoltro, COD_BDI_INOLTRO,"
				+ " COD_UFFICIO_REPLY_TO, tipuffReplyTo.RV_MEANING descrUffReplyTo,"
				+ " comuffReplyTo.DESCRIZIONE descrSedeUfficioReplyTo,"
				+ " COD_BDI_REPLY_TO, JMS_CORRELATION_REPLY_TO, ID_MESSAGGIO_SOLLECITATO, ID_RICHIESTA,"
				...
				+ " CHIAVE_ANNO_SIEPE, CHIAVE_PROGR_SIEPE, COGNOME_SOGGETTO, NOME_SOGGETTO, DATA_NASCITA,"
				+ " COD_STATO_NASCITA, COD_COMUNE_NASCITA FROM MESSAGGIO,"
				+ " JMS_CODE mit, JMS_CODE dest, JMS_CODE ESITO, ufficio uff, comune comuff,"
				+ " cg_ref_codes tipuff, ufficio uffdest,comune comuffdest, cg_ref_codes tipuffdest,"
				+ " JMS_CODE tipoMess,JMS_CODE tipOperazione"
				...
				+ ", UFFICIO ufficio_fasc_siep_cum, COMUNE comune_fasc_siep_cum,"
				+ " CG_REF_CODES tipo_ufficio_fasc_siep_cum"
				...
				+ " and COD_TIPO_OPERAZIONE = tipOperazione.CODICE and tipOperazione.dominio ="
				+ " 'TIPO_OPERAZIONE' and COD_BDI_MITTENTE = mit.CODICE and mit.dominio = 'BDI'"
	...
	protected String getSqlQueryMS() {
		...
		lStatement = "SELECT ID_MESSAGGIO, COD_TIPO_MESSAGGIO, tipoMess.DESCRIZIONE DescrTipoMessaggio,"
				...
				+ " DELIVERY_MODE, COD_UFFICIO_INOLTRO, tipUffInoltro.RV_MEANING descrUffInoltro,"
				+ " comUffInoltro.DESCRIZIONE descrSedeUfficioInoltro, COD_BDI_INOLTRO, COD_UFFICIO_REPLY_TO,"
				+ " tipuffReplyTo.RV_MEANING descrUffReplyTo,"
				+ " comuffReplyTo.DESCRIZIONE descrSedeUfficioReplyTo,"
				+ " COD_BDI_REPLY_TO, JMS_CORRELATION_REPLY_TO, ID_MESSAGGIO_SOLLECITATO, ID_RICHIESTA,"
				...
				// Ticket#20220111018 - ottimizzazione recupero solleciti: si cerca subito se presenti
				// solleciti
				...
				// query inutili.
				// Si ricercano tra i messaggi ricevuti (DELIVERY_MODE = '00002'), di tipo richiesta
				// (COD_TIPO_MESSAGGIO = '01')
				...
				+ ", (select COUNT(*) from MESSAGGIO solleciti where 1=1"
				+ " AND solleciti.DELIVERY_MODE = '00002' AND solleciti.COD_TIPO_MESSAGGIO = '01'"
				+ " AND solleciti.COD_TIPO_OPERAZIONE in ('00073', '00076','00068')"
				// + " AND solleciti.COD_UFFICIO_DESTINATARIO = '00127202101' " // in questo metodo non lo
				// conosco s cui ometto il filtro
				...
				+ " FROM MESSAGGIO LEFT OUTER JOIN FASC_MS_TO_FASC_SIEP ON (MESSAGGIO.ID_MESSAGGIO ="
				+ " FASC_MS_TO_FASC_SIEP.MES_ID_MESSAGGIO)"
				...
				+ " AND COD_TIPO_OPERAZIONE = tipOperazione.CODICE and tipOperazione.dominio ="
				+ " 'TIPO_OPERAZIONE' AND COD_ESITO = esito.CODICE AND esito.DOMINIO = 'CODICE_ESITO'"
	...
	// Si elimina la join con FASC_MS_TO_FASC_SIEP non uitilizzata
	...
	protected String getSqlQueryCumulo() {
		...
		lStatement = "SELECT ID_MESSAGGIO, COD_TIPO_MESSAGGIO, tipoMess.DESCRIZIONE DescrTipoMessaggio,"
				...
				+ " COD_UFFICIO_MITTENTE, COD_BDI_MITTENTE, bdiMitt.DESCRIZIONE bdiMit,"
				+ " COD_UFFICIO_DESTINATARIO, comuffdest.DESCRIZIONE destSede,"
				+ " tipuffdest.RV_MEANING tipodest,"
				+ " comuffMitt.DESCRIZIONE sede, tipuffMitt.RV_MEANING tipouff,"
				+ " COD_BDI_DESTINATARIA, bdiDest.descrizione bdiDest, DATA_INVIO, DATA_ESITO,"
				+ " FLAG_VISTO, CODICE_UTENTE_MITTENTE, COD_ESITO, esito.DESCRIZIONE DESCR_ESITO,"
				+ " JMS_ID_MESSAGE, JMS_CORRELATION_ID_MESSAGE, MESSAGGIO.CHIAVE_ANNO_SIEP,"
				...
				+ " comUffInoltro.DESCRIZIONE descrSedeUfficioInoltro, COD_BDI_INOLTRO,"
				+ " COD_UFFICIO_REPLY_TO, tipuffReplyTo.RV_MEANING descrUffReplyTo,"
				+ " comuffReplyTo.DESCRIZIONE descrSedeUfficioReplyTo, COD_BDI_REPLY_TO,"
				+ " JMS_CORRELATION_REPLY_TO, ID_MESSAGGIO_SOLLECITATO, ID_RICHIESTA,"
				// MEV_2025-48 - 2.15 Gestione Annotazioni Trasmissioni
				...
				+ " OR solleciti.ID_MESSAGGIO_SOLLECITATO = MESSAGGIO.ID_MESSAGGIO)) contaSolleciti"
				// Ticket#20220111018 - ottimizzazione
				// MEV_2025-48 - 2.15 Gestione Annotazioni Trasmissioni
				// Se elimina la join con FASC_MS_TO_FASC_SIEP non utilizzata nel riscontro esiti
				+ " FROM MESSAGGIO"
				...
				+ ", UFFICIO ufficio_fasc_siep, COMUNE comune_fasc_siep, CG_REF_CODES tipo_ufficio_fasc_siep"
				+ " WHERE COD_TIPO_MESSAGGIO = tipoMess.CODICE and tipoMess.dominio = 'TIPO_MESSAGGIO'"
				+ " AND COD_TIPO_OPERAZIONE = tipOperazione.CODICE"
				+ " and tipOperazione.dominio = 'TIPO_OPERAZIONE'"
	...
	protected String getSqlQuery() {
		...
		/*
		 * String lStatement = new String(""); lStatement += " SELECT ID_MESSAGGIO, " +
		 ...
		 * " and comuffdest.COD_COMUNE = uffdest.COD_COMUNE" +
		 * " and uffdest.COD_TIPO_UFFICIO =tipuffdest.RV_LOW_VALUE" +
		 * " and tipuffdest.RV_DOMAIN ='TIPO_UFFICIO'"; return lStatement;
		 */
	}
	...
	public GenericModel getModel() throws DAOException {
		...
		// new d.f. NOV/2013: aggiunti 3 campi per gestire l'inoltro ad altro ufficio
		aModel.setChiaveUfficioSiep(getString("CHIAVE_UFFICIO_SIEP"));
		aModel.setDescrUfficioSiep(getString("descr_ufficio_siep"));
		aModel.setDescrSedeUfficioSiep(getString("sede_ufficio_siep"));
		...
		if (findColumn("CHIAVE_UFFICIO_FAS_CUMULANTE")) {
			// new d.f. MEV26 Cumulo: aggiunt 3 campi
			aModel.setChiaveUfficioFasCumulante(getString("CHIAVE_UFFICIO_FAS_CUMULANTE"));
			aModel.setDescrUfficioFasCumulante(getString("descr_ufficio_siep_cum"));
			aModel.setDescrSedeUfficioFasCumulante(getString("sede_ufficio_siep_cum"));
		}
		...
		// FINE MEV26
		aModel.setDeliveryMode(getString("DELIVERY_MODE"));
		// new d.f. NOV/2013: aggiunti 9 campi per gestire l'inoltro e la risposta ad altro ufficio
		aModel.setCodUfficioInoltro(getString("COD_UFFICIO_INOLTRO"));
		aModel.setDescrUfficioInoltro(getString("descrUffInoltro"));
		aModel.setDescrSedeUfficioInoltro(getString("descrSedeUfficioInoltro"));
		aModel.setCodBdiInoltro(getString("COD_BDI_INOLTRO"));
		aModel.setCodUfficioReplyTo(getString("COD_UFFICIO_REPLY_TO"));
		aModel.setDescrUfficioReplyTo(getString("descrUffReplyTo"));
		aModel.setDescrSedeUfficioReplyTo(getString("descrSedeUfficioReplyTo"));
		aModel.setCodBdiReplyTo(getString("COD_BDI_REPLY_TO"));
		aModel.setJmsCorrelationReplyTo(getString("JMS_CORRELATION_REPLY_TO"));
		// new d.f. NOV/2013 per gestire il sollecito
		aModel.setIdMessaggioSollecitato(getString("ID_MESSAGGIO_SOLLECITATO"));
	...
	public String setCondizioneDataInvio(Date aDaIni, Date aDaFine) {
		...
		// lCondizioni += " AND TO_CHAR(DATA_INVIO,'YYYYMMDD') >= '" + DateUtils.getDateToString(aDaIni,
		// "yyyyMMdd" )+"'" ;
		// lCondizioni += " AND TO_CHAR(DATA_INVIO,'YYYYMMDD') <= '" + DateUtils.getDateToString(aDaFine,
		// "yyyyMMdd" )+"'" ;
		if (aDaIni != null)
			lCondizioni += " AND TO_CHAR(DATA_INVIO,'YYYYMMDD') >= '"
	...
	protected String getSqlQueryPerRicerca(int aPage, String lCond) {
		...
		lStatement += "SELECT * FROM (SELECT ID_MESSAGGIO,"
				...
				+ " CHIAVE_UFFICIO_FAS_CUMULANTE, descr_ufficio_siep_cum, sede_ufficio_siep_cum, NOTE,"
				+ " CHIAVE_ANNO_SIEPE, CHIAVE_PROGR_SIEPE, COGNOME_SOGGETTO, NOME_SOGGETTO, DATA_NASCITA,"
				+ " COD_STATO_NASCITA, COD_COMUNE_NASCITA, Rownum rn FROM (" + getSqlQuery() + " "
				+ lCond +
	...
	public void ricercaXCruscotto(String aCodUfficio, String aOrd) throws DAOException {
		...
		String lSql = "SELECT TRUNC(DATA_INVIO) DATA_INVIO, FLAG_VISTO, COUNT(*) NUM from MESSAGGIO where"
				+ " MESSAGGIO.COD_UFFICIO_DESTINATARIO = '";
		lSql += aCodUfficio;
		lSql += "' AND COD_TIPO_MESSAGGIO = '01' ";
	...
	protected String getPagedSqlQuery(int aPage) {
		...
		if (aPage > 0) {
			lStatement += "SELECT ID_MESSAGGIO,"
					...
					+ " ID_RICHIESTA, CHIAVE_ANNO_SIUS, CHIAVE_PROGR_SIUS,"
					+ " CHIAVE_ANNO_FAS_CUMULANTE, CHIAVE_PROGR_FAS_CUMULANTE,"
					+ " CHIAVE_UFFICIO_FAS_CUMULANTE,"
					+ " tipo_ufficio_fasc_siep_cum.RV_MEANING descr_ufficio_siep_cum,"
					+ " comune_fasc_siep_cum.DESCRIZIONE sede_ufficio_siep_cum, NOTE,"
					+ " CHIAVE_ANNO_SIEPE, CHIAVE_PROGR_SIEPE, COGNOME_SOGGETTO, NOME_SOGGETTO,"
					+ " DATA_NASCITA, COD_STATO_NASCITA, COD_COMUNE_NASCITA";
		} else {
