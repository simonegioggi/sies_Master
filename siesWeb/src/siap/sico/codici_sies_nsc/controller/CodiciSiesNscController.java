...
public class CodiciSiesNscController extends SiapController implements ICodiciSiesNsc {
	...
	public Vector ExRicercaCodiciSiesNsc(CodiciSiesNscModel aCodiciSiesNsc) throws F3BException {
		...
		try {
			...
			lCodDao.start();
			while (lCodDao.next()) {
				lCodiciSiesNsi.add(lCodDao.getModel());
			}
			lCodDao.stop();
		} catch (DAOException daoEx) {
			throw new F3BException(
					"CodiciSiesNscController.ExRicercaCodiciSiesNsc: Non posso leggere : " + daoEx);
		} finally {
			cleanup(lCodDao);
			cleanup(lConn);
		}
	...
	public CodiciSiesNscModel ExRicercaCodiciSiesNscById(String aCoDomain, String aCoCodcentr)
		...
		try {
			...
			throw new F3BException(
					"CodiciSiesNscController.ExRicercaCodiciSiesNscById: Non posso leggere : " + daoEx);
		} finally {
