...
public class ActNscToSiesLoadSoggetto extends ActWsBase {
	...
	public SoggettoModel processRequest(ANAGRAFICADocument.ANAGRAFICA adatiAnagrafica,
		...
		/*
		 * if (adatiAnagrafica.getDATIANAGRAFICI().getCODILUOGONASCITA() != null) { CodiciSiesNscModel
		 * lCodiciSIESNSCModel = Decodifica("COMUNE",
		 * adatiAnagrafica.getDATIANAGRAFICI().getCODILUOGONASCITA()); lCodComuneNascita =
		 * lCodiciSIESNSCModel.getCoSies().trim(); } lSoggettoModel.setCodComuneNascita(lCodComuneNascita);
		 */
		try {
			...
			if (adatiAnagrafica.getDATIANAGRAFICI().getCODILUOGONASCITA() != null) {
				siesLogger.debug(
						"Codice da NSC = " + adatiAnagrafica.getDATIANAGRAFICI().getCODILUOGONASCITA());
				Vector<CodiciSiesNscModel> listaComuni = DecodificaComune("COMUNE",
						adatiAnagrafica.getDATIANAGRAFICI().getCODILUOGONASCITA());
				if (listaComuni.size() == 1) {
					lCodComuneNascita = listaComuni.elementAt(0).getCoSies().trim();
					siesLogger.debug(
							"Trovato un solo comune decodificato con codice SIES = " + lCodComuneNascita);
				} else {
					// Trovati più comuni
					siesLogger.debug("Trovati più comuni (" + listaComuni.size()
							+ ") testo la data di nascita per decidere quale comune usare...");
					...
					if (dataNascita == null && lSoggettoModel.getAnnoNascita() != null) {
						...
						dataNascita = DateUtils.getDate(lSoggettoModel.getAnnoNascita().toString(),
								lSoggettoModel.getMeseNascita() != null
										? lSoggettoModel.getMeseNascita().toString()
										: "01",
								"01");
					...
					if (dataNascita != null) {
						siesLogger.debug(
								"dataNascita disponibile (" + dataNascita + ") ciclo sui comuni trovati...");
						...
						for (int kk = 0; kk < listaComuni.size(); kk++) {
							...
							siesLogger.debug("comune (" + comuneNascita.getCoSies().trim() + ","
									+ comuneNascita.getCoVal3() + " )");
							...
							if (dataFineValidita != null) {
								if (DateUtils.isLower(dataNascita, dataFineValidita)
									...
									siesLogger.debug("comune trovato (cod, mindatafine)=(" + lCodComuneNascita
											+ "," + minDataFineValidita + ")");
								}
							} else {
								// mi trovo sul comune valido. Potrebbe comunque essere quello buono se non è
								// stato
		...
		if (adatiAnagrafica.getDATIANAGRAFICI().getCODISTATOESTERONAS() != null) {
			CodiciSiesNscModel lCodiciSIESNSCModel = Decodifica("NAZIONE",
					adatiAnagrafica.getDATIANAGRAFICI().getCODISTATOESTERONAS());
		...
		if (adatiAnagrafica.getPROCEDIMENTO().getTITOLOESECUTIVO().getDATIPROVVEDIMENTO()
				.getFLAGCITTADINANZAISD() != null) {
			lSoggettoModel.setNazionalita(adatiAnagrafica.getPROCEDIMENTO().getTITOLOESECUTIVO()
					.getDATIPROVVEDIMENTO().getFLAGCITTADINANZAISD());
		} else {
			lSoggettoModel.setNazionalita("-");
		}

		/*
		 * if (lSoggettoModel.getCodStatoNascita().equals("-")) { lSoggettoModel.setNazionalita("-"); } else {
		 * if (lSoggettoModel.getCodStatoNascita().equals("039")) { lSoggettoModel.setNazionalita("I"); } else
