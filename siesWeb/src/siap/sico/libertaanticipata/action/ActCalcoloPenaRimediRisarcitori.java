...
/**
 * <p>
 * Title: ActCalcoloPenaRimediRisarcitori
 * </p>
 * <p>
 * Description: Classe Action per il calcolo della pena nel caso di concessione Rimedi Risarcitori così come
 * previsti dal DL92/2014
 * </p>
 * 
 * @version 1.0
 * @since 10/2014
...
public class ActCalcoloPenaRimediRisarcitori extends ActionSiap implements ICostantiLibertaAnticipata {
	...
	public String processRequest() throws Exception {
		...
		if ("S".equals(lIsConfermaDataFine)) {
			...
			if ("DataFineAbInitio".equals(lRadioButtonScelto)) {
				lDataFineSelezionata = getRequestDateParameter("AnnoDataFineAbInitio", "MeseDataFineAbInitio",
						"GiornoDataFineAbInitio");
			} else {
		...
		// ==========================================================================
		BigDecimal lIdEveSorveglianza = getRequestBigDecimalParameter(
		...
		CalcoloPenaModel lCalcoloPena = lCtrlCal.exCalcoloLiberazioneAnticipata(lCalcoloPenaModel,
				lIdFascicolo, lIdEveSorveglianza, dataSistemaPerCalcoli, lDataFineSelezionata, "RD",
				lOperMod);
			...
			if (lDataFineAbFine != null && lCalcoloPena.getPenaResiduaRicalcolata() != null) {
				...
				if (lPenaResNuovoCalcolo != null && lPenaResNuovoCalcolo.getDataFine() != null) {
					if (!lDataFineAbFine.equals(lPenaResNuovoCalcolo.getDataFine())) {
						...
						setRequestAttribute("DataFineAbInitioSenzaLA",
								lPenaNuovoCalcoloSenzaLA.getDataFine());
		...
		if (lFunMod != null && ((lFunMod.getNumAnni() != null && lFunMod.getNumAnni().intValue() > 0)
			...
			lPage = ICostantiLicenzaLibanticipata.PG_DETTAGLIO_CALCOLO_PENA_DL92;
		}
