...
/**
 * TemplateSqlDAO - Classe SqlDAO che rappresenta la tabella Template
 *
 * @version 1.0
 */
...
public class TemplateSqlDAO extends SIAPSqlDAO {
	...
	public void ricercaTemplate(TemplateModel aModel) throws DAOException {
	...
	public void ricercaTemplateByCodMotivoFlagTemplate(String aCodMotivo, String aFlagTemplate)
			throws DAOException {

		String lSql = getSqlQuery();

		lSql += " WHERE " + setCondizioniByCodMotivo(aCodMotivo) + " AND "
				+ setCondizioniByFlagTemplate(aFlagTemplate);

		setStatement(lSql);
	}

	public void ricercaTemplateByTipEveTipoProvCodMotivoFlagTemplate(String aTipoEvento, String aTipoProv,
			String aCodMotivo, String aFlagTemplate) throws DAOException {
	...
	protected String getSqlQuery() {
		...
		lStatement += " SELECT " + "ID_TEMPLATE, " + "NOME_TEMPLATE, " + "DESCR, " + "PATH_RICERCA, "
				+ "COD_TIPO_EVENTO, " + "COD_TIPO_PROVVEDIMENTO, " + "COD_MOTIVO, " + "FLAG_TEMPLATE, "
				+ "COD_ESITO, " + "COD_OGGETTO_PROCEDIMENTO, " + "MAG_COD_MAGISTRATO, "
	...
	public String setCondizione(TemplateModel aModel) {
		...
		if (aModel.getFlagTemplate().length() > 0) {
			if (lInserito)
				...
				lInserito = true;
			// 01/06/2010 Solo per SIGE, in caso di valorizzazione di FLAG_TEMPLATE si includono anche i
			// template con il flag = null.
			if (aModel.getCodTipoProvvedimentoSige().length() > 1)
		...
		if (aModel.getCodMagistrato().length() > 1) {
			if (lInserito)
				...
				lInserito = true;
			lCondizioni += "( MAG_COD_MAGISTRATO = '" + aModel.getCodMagistrato()
					+ "' OR MAG_COD_MAGISTRATO IS NULL) ";
		} else {
			if (lInserito)
				lCondizioni += " AND ";
			else
