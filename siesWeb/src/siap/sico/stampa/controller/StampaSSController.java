	...
	public TreeModel prelevaDatiSanzioniSostitutive(EventoNotificaModel aEveModel, UtenteModel aUtenteModel)
		...
		try {
			...
			if (lFasModel != null && lFasModel.getCodUfficioInserimento() != null
				...
				siesLogger.error("lFasModel.getChiaveProgr()3: " + lFasModel.getChiaveProgr());
			...
			// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di mLog
			siesLogger.info("====> TRASMISSIONE SS - Prima di appendere lo Stato di Esecuzione = = = "
			...
			throw new F3BException("StampaSSController.prelevaDatiSanzioniSostitutive: " + sqe);
		} finally {
			cleanup(lFasDao);
			cleanup(lAvvDao);
