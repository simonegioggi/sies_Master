...
public class ActUploadDocument extends ActionSiap implements ICostantiEvento {
	...
	public String processRequest() throws Exception {
		...
		// **********************************************
		// Flag che segnala la necessit� di inserire o meno un record nella tabella
		// AVVISI_AVVOCATO
		...
		// Flag che segnala la necessit� del controllo della presenza del documento nel BLOB
		...
		// 20171011: [SG] controllo preventivo se l'evento sia gi� validato
		...
		if (lisUpdate) {
			...
			if ("S".equals(lFlagValidazioneEsito)) {
				// se � stata chiesta la validazione dell'esito, devo updatare
		...
		// Se c'� lo stack di ritorno effettua un ritorno in cima
	...
	/**
	 * Funzione di utilit�. Viene richiamata per controllare l'esistenza del documento nel BLOB della tabella
	...
	public void updateTabella(BigDecimal aId, String lFlgAvvocatura) throws Exception {
		...
		if (!lFlgAvvocatura.equals("")) {
			...
			if (lFasGPMod != null && lFasGPMod.getFascicoloSiusModel() != null) {
				// provo a verificare se � presente nell'oggetto FascicoloGPModel
