...
/**
 * <p>Title: CalendarController</p>
 * <p>Description: Classe Controller per il calcolo delle date</p>
...
public class CalendarUtil
{
  ...
  /*****************************************************************************
   * Calcola il quantum (aModel.mGG , mMM, mAA) a partire da data inizio e data 
   * fine del model passato in input.
   * 
   * @param aModel -  Model con data inizio e data fine valorizzati
   * @param lEscludiDiesaquo - indica se escludere il giorno inizio nel
   *        calcolo del quantum. Se true il giorno inizio non viene conteggiato.
   *        Se false, viene conteggiatao nel quantum anche il gg inizio.<br>
  ...
  /*****************************************************************************
   * Valorizza aModel.mGG , mMM, mAA a partire da data inizio e data fine 
   * conteggiando nel quantum anche il giorno di inizio.
   * 
  ...
  /*****************************************************************************
   * METODO PRIVATO : Valorizza aModel.mGG , mMM, mAA a partire da data inizio 
   * e data fine (considera nel quantum anche il gg iniziale)
   *  (USE CASE SIEP-UC-014-LV-AA)
   *  Il quantum restituito non è necessariamente normalizzato vale a dire che 
   *  può valere 30 giorni, 3 mesi e 2anni opure anche 31 giorni 2mesi e un anno
   *   
   ************************************************************************** */
  private CalendarModel contagiorniclassic(CalendarModel aModel)
  {
    ...
    { // Calcolo il numero di giorni ad arrivare alla 
      ...
      // giorno fine
       ...
       int lNrGiorni = Integer.parseInt(DateUtils.getDayToString(DateUtils.getEndOfMonth(aaI,mmI)))-ggI+ggF+1; // +1 per considerare anche il gg iniziale
       ...
       if (   lModRet.getNumGiorni()==Integer.parseInt(DateUtils.getDayToString(DateUtils.getEndOfMonth(aaI,mmI))) 
           && mmI==mmF && aaI!=aaF)
       {  // vera se ggF=ggI-1
       ...
       { //=====================================================================
         ...
         // es: data inizio 26/02/2001, data fine 25/04/2002 lNrGiorni = 28 convetiti in 0 giorni e 1 mese
         // es: data inizio 15/02/2000, data fine 14/07/2001 lNrGiorni = 29 (bisestile) convetiti in 0 giorni e 1 mese
         //=====================================================================
         ...
         // il mese di inizio 
         if(mmI == 2 && lNrGiorni >= 28) // Se Febbraio e nrgiorni >= 28 (n.b. può valere solo 28 o 29)
         {
           ...
           else if(   lNrGiorni == 28  
                   && Integer.parseInt(DateUtils.getDayToString(DateUtils.getEndOfMonth(aaI,mmI))) == 28)
           {
              lNrGiorni = 0; 
              lModRet.setNumGiorni(lNrGiorni);
           }
...
//           }
     ...
     { // ggI<=ggF
      ...
      if (mmI <= mmF)
      {
        ...
        if (Integer.parseInt(DateUtils.getDayToString(DateUtils.getEndOfMonth(aaI,mmI)))==lModRet.getNumGiorni())
        { // Il numero di GG calcolati coincide con il numero di gg del mese di 
          ...
          { // sono finito sul primo del mese di arrivo ma ho scalato 1 per il dies a quo
              lModRet.setNumGiorni(0);
   ...
   * METODO PRIVATO : Valorizza aModel.mGG , mMM, mAA a partire da data inizio 
   * e data fine
   * (USE CASE SIEP-UC-014-LV-AA) escludendo il dies a quo!!!
   * Identico al contagiorniclassic ma non aggiunge 1 al lNrGiorni
   * 
   * Il calcoli non viene eseguito come differenza in giorni tra due oggetti date,
   * quindi non calcola l'effettiva differenza di giorni tra due date
   */
  private CalendarModel contagiorni_escludidiesaquo(CalendarModel aModel)
  {
    ...
    if (ggI>ggF)
    {
       int lNrGiorni = Integer.parseInt(DateUtils.getDayToString(DateUtils.getEndOfMonth(aaI,mmI)))-ggI+ggF;
       lModRet.setNumGiorni(lNrGiorni);

       if (lModRet.getNumGiorni()==Integer.parseInt(DateUtils.getDayToString(DateUtils.getEndOfMonth(aaI,mmI))) && mmI==mmF && aaI!=aaF)
       {
          lModRet.setNumGiorni(0);
          lModRet.setNumMesi(0);
          lModRet.setNumAnni(aaF-aaI);
  ...
  /*****************************************************************************
   *  Restituisce un nuovo CalendarModel ottenuto sommando (mGG,mMM,mAA) del 
   *  aModelA con (mGG,mMM,mAA) del aModelB. I dati vengono restituiti 
   *  normalizzati (ricalcolaGAM)<br><br>
   *  n.b. Il calendar restituito contiene valorizzati i <b>soli</b> campi GG,MM,AA
   *       tutti gli altri campi sono valorizzati ai valori di default del 
   *       CalendarModel (null, e 0)
   ************************************************************************** */
  ...
  /**
   *  Valorizza mGG , mMM, mAA del aModel, sommando a questi
   *  i gg,mm,aa passati a parametro e i giorni di differenza tra datainizio e datafine.
   */
  ...
  /**
   *  Valorizza mGG , mMM, mAA del aModel, sommando soltanto
   *  i gg,mm,aa passati a parametro e i giorni di differenza tra datainizio e datafine.
   */
  ...
  /**
   *  Valorizza mGG , mMM, mAA del aModel, sommando a questi
   *  i gg,mm,aa passati a parametro.
   */
  ...
  /*****************************************************************************
   *  Restituisce un Calendar ottenuto dalla differenza: (aModelA-aModelB)
   *  n.b. il metodo effettua la sola differenza dei quantum (GG, MM, AA),
   *  non opera sugli altri campi dei model in input (valute e sulle date)<br>
   *  <b>Attenzione: non utilizzare questo metodo se aModelA&lt;aModelB</b>
   *  infatti dati vengono restituiti con valori negativi ma non sempre in una 
   *  forma corretta es:
   *  <br>aModelA = (10gg, 2mm, 2aa) e aModelB = (15gg, 2mm, 2aa)
   *  <br>Il model restituito è nella forma (25gg, 11mm, -1aa ) e non (-5gg, 0mm, 0aa)
   *  le due forme sono equivalenti ma la prima non è bella a vedersi e non 
   *  esistono metodi per convertirla. ricalcolaGAM infatti non agisce su quantum 
   *  negativi
   *  Per come vengono effettuati i calcoli, se aModelA&lt;aModelB, giorni e mesi
   *  risultano sempre positivi mentre è sempre l'anno che diviene negativo. Per
   *  cui si ottiene un quantum accettabile <b>SOLO</b> se i quantum in input sono 
   *  espressi in anni: es aModelA=(0,0,3), aModelA=(0,0,5) differenza
   *  calcolata = (0,0,-2)
   ************************************************************************** */
  ...
  /*****************************************************************************
   *  Ricalcola mGG , mMM, mAA di aModel riportando i giorni <30 e i mesi<12
   *  (si presuppone che il model di input possa trovarsi in una situazione tipo
   *  59 giorni , 21 mesi, 2 anni. In output viene prodotto : 29gg, 10mm,3anni)
   *  I conteggi sono effettuati usando gli algoritmi di calcolo della pena
   *  (USE CASE SIEP-UC-015-LV-AA)
   *  <b>Attenzione!! Non opera su quantum negativi. Per normalizzare un quantum 
   *  negativo è necessario passare i dati combiati di segno</b>
   ****************************************************************************/
 ...
 /******************************************************************************
  *  Valorizza mGG , mMM, mAA del aModelA, sommando a questi
  *  mGG , mMM, mAA del aModelB. Vengono sommati inoltre anche Ammende e Multe
  *  Il risultato viene restituito normalizzato (ricalcolaGAM)
  *  I quantum devono essere positvi.
  *************************************************************************** */
 ...
 /******************************************************************************
  *  Valorizza mGG , mMM, mAA , sottranedo a quelli del aModelA
  *  mGG , mMM, mAA del aModelB.<br>
  *  Inoltre sottrae anche le valute (Multa, Ammenda)<br>
  *  Non utilizza DataInizio e DataFine del calendar <br>
  *  n.b Se aModelA&lt;aModelB allora mGG , mMM, mAA risultano negativi (tutti)
  *      e ErrorMsg="Swapped"
  *  Questo metodo si differenzia dal sottraiGiorni per il fatto che il quantum 
  *  viene sempre restituito <b>normalizzato</b>, anche se la differenza porta a 
  *  un quantum negativo. I risultati ottenutosono del tutto equivalenti.
  *  Es: aModelA = (10gg, 2mm, 2aa) <  aModelB = (15gg, 2mm, 2aa)
  *  La sottraiGiorni restituisce (25gg, 11mm, -1aa ) 
  *  mentre questo metodo restituisce (-5gg, 0mm, 0aa)
  *  
  *************************************************************************** */
  ...
  /*****************************************************************************
   * Effettua la differenza sia sui quantum che sugli importi tra aModelA 
   * e aModelB. (aModelA-aModelB)
   *  I conteggi sono effettuati usando gli algoritmi di calcolo della pena
   *  (USE CASE SIEP-UC-015-LV-AA)
   * 
   * Se il quantum aModelA è minore del quantum di aModelB, il quantum ottenuto
   * sarebbe negativo ma risultato viene restituito in valore assoluto
   * (cioè aModelB-aModelA) con ErrorMsg="Swapped" (gli importi sono invece corretti)
   * @param aModelA
   * @param aModelB
   * @return
   ****************************************************************************/
  ...
  /*****************************************************************************
   * Verifica se il periodo (AA,MM,GG) del primo Calendar è STRETTAMENTE
   * maggiore del periodo del secondo<br>
  ...
  /*****************************************************************************
   * Restituisce true se giorni, mesi e anni del Calendar sono tutti nulli
   * Non effettua controlli sugli altri campi.
   * @param aModelA
   * @return 
   ************************************************************************** */
  ...
  /*****************************************************************************
   * Verifica se GG. MM e AA sono tutti >=0
   * Non effettua controlli sugli altri campi.
   * @param aModel
   * @return
   ************************************************************************** */
  ...
  /**
   * Metodo che effettua la differenza tra quantum passati in input con i due 
   * CalendaModel: aModelA-aModelB. Il risultato viene restituito su un terzo
   * calendar. I model in input NON vengono modificati, il risultato può essere 
   * negativo es (-5 giorni, -1 mese, -1 anno)
   * Questo metodo opera solo sui quantum.
   * I Calendar passati in input possono avere qualsiasi segno.
   * Il risultato è normalizzato 
   * @param aModelA
   * @param aModelB
   * @return
   */
...
//    siesLogger.debug("lDiff = "+lDiff);
   ...
   * calendar. I model in input NON vengono modificati, il risultato può essere 
   * negativo es (-5 giorni, -1 mese, -1 anno)
   * Questo metodo NON opera solo sui quantum.
   * I Calendar passati in input possono avere qualsiasi segno.
   * Il risultato è normalizzato 
   * @param aModelA
   * @param aModelB
  ...
  /**
   * Ritorna il quantum in giorni applicando la seguente conversione:
   * 1 mese = 30 gg
   * 1 anno = 12 mesi x 30 gg = 360gg 
   * @param aModelA
   * @return
   */
