...
/**
 * ActLoadTrasferisciRelazione - Classe Action per il caricamento dela form per il trasferimento Relazione
 *
 * @version 1.0
 */
public class ActLoadTrasferisciRelazione extends ActionSiap implements ICostantiRelazione {
	...
	public String processRequest() throws Exception {
		...
		if (lRelMod.getFlagDocumentoRegistrato() != null
				&& lRelMod.getFlagDocumentoRegistrato().equalsIgnoreCase("S")) {
			lModificabile = "NO";
			lTrasferibile = "SI";
			lUpload = "NO";
			lValidata = "NO";
		...
		/*
		 * 2007-06-23 Temporaneamente commentato, per gestione ufficio destinario diverso dal mittente del
		 * fascicolo siepe, in attesa di chiarimenti/confronti dell'analisi funzionale. Vedere il codice
		 * riportato di seguito al medesimo. // Dati aggregati relativi al Fascicolo SIEPE
		 * FascicoloSiepeEstesoModel lFascicoloEsteso = null; // Si risale ai dati in Sessione
		 * lFascicoloEsteso = (FascicoloSiepeEstesoModel) getSessionAttribute("FascicoloSiepeEsteso"); //
		 * L'ufficio destinatario è il mittente del Fascicolo SIEPE String lCodUfficioMittente =
		 * lFascicoloEsteso.getFascicoloSiepe().getCodUfficioMittente(); IUfficio lUffCtrl =
		 * SICOLookupRemote.getUfficioRemote(); UfficioModel lUffDestinatario =
		 * lUffCtrl.ExRicercaUfficioByCod(lCodUfficioMittente);
