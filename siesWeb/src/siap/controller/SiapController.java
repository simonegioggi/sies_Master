...
public class SiapController extends GenericController {
	 ...
	 *
	 *             //Context envCtx = (Context)initCtx.lookup("java:comp/env"); Context lEnvCtx =
	 *             (Context)lInitialCtx.lookup(F3BProperties.getProperty("ctx.env"));
	 *
	 *             //DataSource ds = (DataSource)envCtx.lookup("jdbc/siap"); DataSource lDataSource =
	 *             (DataSource)lEnvCtx.lookup(F3BProperties.getProperty("datasource.ctx"));
	 *
	 *             Connection lConn = lDataSource.getConnection(); lConn.setAutoCommit( false );
	 *
	 *             return lConn; } catch(SQLException sqlex) { sqlex.printStackTrace(); throw new
	 *             F3BException(sqlex.getMessage()); } catch(NamingException ex) { ex.printStackTrace(); throw
	 *             new F3BException(ex.getMessage()); } }
	 */
