...
public class FascicoloSiepSqlDAO extends SIAPSqlDAO {
	 ...
	 * " FASC.CHIAVE_UFFICIO, DESCR_TIPO_UFF.RV_MEANING DESCR_TIPO_UFFICIO, DESCR_COM_UFF.DESCRIZIONE DESCR_COMUNE_UFFICIO,"
	 ...
	 * " FASC.COD_UFFICIO_UNIONE, DESCR_TIPO_UFFUNIONE.RV_MEANING DESCR_TIPO_UFFICIO_UNIONE, DESCR_COM_UFFUNIONE.DESCRIZIONE DESCR_COMUNE_UFFICIO_UNIONE "
	 * ; lStatement += " FROM FASCICOLO_SIEP FASC, CG_REF_CODES MOTIVO_ARCHIVIAZIONE,"; lStatement +=
	 ...
	 * " WHERE (MOTIVO_ARCHIVIAZIONE.RV_DOMAIN = 'MOTIVO_ARCHIVIAZIONE' AND MOTIVO_ARCHIVIAZIONE.RV_LOW_VALUE = FASC.COD_MOTIVO_ARCHIVIAZIONE)"
	 * ; lStatement +=
	 * " AND (STATO_FASCICOLO.RV_DOMAIN = 'STATO_FASCICOLO' AND STATO_FASCICOLO.RV_LOW_VALUE = FASC.COD_STATO_FASCICOLO)"
	 * ; lStatement +=
	 ...
	 * " AND (UFF.COD_COMUNE = DESCR_COM_UFF.COD_COMUNE)"; lStatement +=
	 * " AND (DESCR_TIPO_UFFUNIONE.RV_DOMAIN = 'TIPO_UFFICIO' AND UFFUNIONE.COD_TIPO_UFFICIO = DESCR_TIPO_UFFUNIONE.RV_LOW_VALUE)"
	 * ; lStatement += " AND (FASC.COD_UFFICIO_UNIONE = UFFUNIONE.COD_UFFICIO)"; lStatement +=
	 * " AND (UFFUNIONE.COD_COMUNE = DESCR_COM_UFFUNIONE.COD_COMUNE)"; lStatement +=
	 * " AND (TIPO_POS_LIBERO.RV_DOMAIN = 'TIPO_POS_LIBERO' AND TIPO_POS_LIBERO.RV_LOW_VALUE = FASC.COD_TIPO_POS_LIBERO)"
	 * ;
	 *
	 * return lStatement; }
	 */
	...
	protected String getFascicoloSqlQuery(String majorOffice) {
		...
		lStatement += " LEFT OUTER JOIN CG_REF_CODES TIPO_POS_LIBERO ON (FASC.COD_TIPO_POS_LIBERO = TIPO_POS_LIBERO.RV_LOW_VALUE AND TIPO_POS_LIBERO.RV_DOMAIN = 'TIPO_POS_LIBERO')";
		// Modifica Accorpamento Uffici
		...
		if (StringUtils.checkValidValue(majorOffice))
			lStatement += " LEFT OUTER JOIN V_SOGGETTO_ETA vse ON (FASC.id_fascicolo_siep = vse.FAS_SIE_ID_FASCICOLO_SIEP)";
	...
	public void ricercaFascicoliReferenziatiDaSIUS(BigDecimal aIdFascicoloSius, BigDecimal aNumFascUnificati)
		...
		lStatement += " FSIEP.CHIAVE_UFFICIO, DESCR_TIPO_UFF.RV_MEANING DESCR_TIPO_UFFICIO, DESCR_COM_UFF.DESCRIZIONE DESCR_COMUNE_UFFICIO,";
		lStatement += " DESCR_TIPO_UFF.RV_LOW_VALUE  COD_TIPO_UFFICIO,";
		lStatement += " FSIEP.COD_MOTIVO_ARCHIVIAZIONE, MOTIVO_ARCHIVIAZIONE.RV_MEANING DESCR_MOTIVO_ARCHIVIAZIONE,";
		...
		lStatement += " FSIEP.COD_UFFICIO_AGGIORNAMENTO, FSIEP.COD_UFFICIO_INSERIMENTO, FSIEP.DATA_AGGIORNAMENTO,";
		...
		lStatement += " FASCICOLO_SIUS FASC,GENERALE_PROCEDIMENTO GP, FASCICOLO_SIUS FASC2,GENERALE_PROCEDIMENTO GP2,";
		...
		lStatement += " AND (MOTIVO_ARCHIVIAZIONE.RV_DOMAIN = 'MOTIVO_ARCHIVIAZIONE' AND MOTIVO_ARCHIVIAZIONE.RV_LOW_VALUE = FSIEP.COD_MOTIVO_ARCHIVIAZIONE)";
		lStatement += " AND (STATO_FASCICOLO.RV_DOMAIN = 'STATO_FASCICOLO' AND STATO_FASCICOLO.RV_LOW_VALUE = FSIEP.COD_STATO_FASCICOLO)";
		...
		lStatement += " AND  TIPO_POS_LIBERO.RV_DOMAIN = 'TIPO_POS_LIBERO' AND TIPO_POS_LIBERO.RV_LOW_VALUE = FSIEP.COD_TIPO_POS_LIBERO";
	...
	protected String getFascicoloByIDSentenzaSqlQuery() {
		...
		lStatement += " 	    F.CHIAVE_PROGR CHIAVE_PROGR, F.SEN_ID_SENTENZA SEN_ID_SENTENZA, S.DATA_PROVVEDIMENTO DATA_PROVVEDIMENTO, ";
	...
	private String setCondizioneFascBySoggetto(SoggettoModel aModel, String strCodUfficioUtenteConnesso,
		...
		if (lCodDistretto.length() > 1) {
			lCondizioni += " AND FASC.CHIAVE_UFFICIO in (SELECT UFF.COD_UFFICIO FROM UFFICIO UFF WHERE UFF.COD_DISTRETTO ='"
					+ lCodDistretto + "')";
	...
	public void ricercaFasCollegatiFascicoloByKey(BigDecimal aIdFascicolo, String tipoRicerca,
		...
		lStatement += " SELECT listagg( s.CHIAVE_ANNO || '/' || s.CHIAVE_PROGR || ' - ') WITHIN GROUP(ORDER BY S.ID_FASCICOLO_SIEP) collegati ";
