 ...
 * @version 1.0
 */
public class ActUploadLSSanSos extends ActionSiap
							                 implements ICostantiEvento
{
	public String processRequest() throws Exception
	{
		...
		if(isRequestChecked( ICostantiEvento.CAMPO_VALIDA) )
		{
			...
			lCtrl.ExUpdateValidaLSSanSos(lModel, lFascMod);
		}
		else
		{
		...
		if (!isRequestParameterNullObj(CAMPO_AZIONE_DETTAGLIO))
		{
			...
			lRedirigi.setAction(getRequestStringParameter(CAMPO_AZIONE_DETTAGLIO) + "&" + CAMPO_ID_EVENTO + "=" + getRequestStringParameter(ICostantiEvento.CAMPO_ID_EVENTO)+ "&fc=" +getRequestStringParameter("fc")+ "&titolo=" +titolo);
