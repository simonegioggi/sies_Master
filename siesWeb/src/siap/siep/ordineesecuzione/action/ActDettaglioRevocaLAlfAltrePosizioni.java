...
public class ActDettaglioRevocaLAlfAltrePosizioni extends ActSIESDettaglioProvvedimento implements
	...
	public String processRequest() throws F3BException {
		...
		lOrdinanza = lMisAltCtrl.ExRicercaMisuraAlternativaByIdEvento(lEveMod.getEvento().getEveIdEvento());
		...
		if (lOrdinanza != null && lOrdinanza.getChiaveUfficioFascicoloSius() != null) {
			...
			if (lUffMod.getCodTipoUfficio().equalsIgnoreCase("UDSM")) {
				...
				lUffMod.setDescrTipoUfficio("MAGISTRATO DI SORVEGLIANZA PER I MINORENNI");
			}
		 ...
		 * lCtrlp.ExRicercaPenaResiduaCorrenteByFascicoloSiep(lEveMod.getEvento().
		 * getFasSieIdFascicoloSiep());
		 */
		 ...
		 * (lVectEvento.size() > 0) { setRequestAttribute("istanza", (EventoModel)
		 * lVectEvento.firstElement()); } }
		 */

		EventoModel lEventoIstanza = this.getEventoIstanza(lEveMod.getEvento().getEveIdEvento(), lEveMod
		...
		DocumentoAllegatoModel mDocAll = mDocAllCtrl.ExRicercaDocumentoAllegatoByIdEventoCodTipo(lEveMod
				.getEvento().getIdEvento(), "06");
		setRequestAttribute("documentoAllegato", mDocAll);

		// valore di ritorno
		return PG_DETTAGLIO_REVOCA_LALF_ALTRE_POSIZIONI;
	}
