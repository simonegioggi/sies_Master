...
/**
 * <p>
 * Title: ActDettaglioOrdineEsecuzioneL78del2013
 * </p>
 * <p>
 * Description: Classe Action per il dettaglio OE Legge 78 del 2013
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class ActDettaglioOrdineEsecuzioneL78del2013 extends ActSIESDettaglioProvvedimento implements
...
//			lDataFinePenaM = llPenMod.getDataFine();
		...
		if (lEveMod.getNotifiche() != null) {
			for (int i = 0; i < lEveMod.getNotifiche().length; i++) {
				if (lEveMod.getNotifiche()[i].getUfficio() != null
						&& lEveMod.getNotifiche()[i].getUfficio().getCodTipoUfficio().equals("UDSM")) {
					lEveMod.getNotifiche()[i].getUfficio().setDescrTipoUfficio(
		...
		if (lEveMod.getNotifiche() != null && lEveMod.getNotifiche().length > 0) {
			...
			if (lPrimaNotifica.getIstDetIdIstitutoDetenzione() != null) {
				...
				lIstDetenzione = ctrld.ExRicercaIstitutoDetenzioneByKey(lPrimaNotifica
						.getIstDetIdIstitutoDetenzione());
				setRequestAttribute("istitutodetenzione", lIstDetenzione);
			}
		}

		return PG_DETTAGLIO_OE_LEGGE_78_2013;
	}
}