...
public class CaricaHTML_Servlet extends RemoteScriptingServlet {
	...
	/**
	 * Calcola la data del rinvio differimento pena partendo da una Data_Inizio e sommandoci i quantum (in
	 * giorni mesi e anni ) di un periodo di rinvio Differimento Pena.
	 * 
	 * Se la data è presente controlla che questa sia valida. I calcoli vengono effettuati considerando il
	 * dies a quo. Il giorno inizio è considerato come espiato. I quantum vengono restituiti normalizzati
	 * (30gg = 1 mese)
	 *
	 * @param ggInizio
	 * @param mmInizio
	 ...
	 * @throws Exception
	 */
	public static String getDataRinvioDifferimento(String ggInizio, String mmInizio, String aaInizio,
			String ggFine, String mmFine, String aaFine, String ggDiff, String mmDiff, String aaDiff)
			throws Exception {
		...
		if (data_fine != null) {
			// Data_fine != null : DEVO CALCOLARE E RITORNARE I QUANTUM del DIFFERIMENTO : Primo carattere di
			// stringa ritorno = 0
			...
			lQuantum = 0 + lSeparatore + lCalDifferimento.getNumAnni() + lSeparatore
					+ lCalDifferimento.getNumMesi() + lSeparatore + lCalDifferimento.getNumGiorni();
			...
			// Data_fine == null : DEVO CALCOLARE LA DATA FINE RINVIO DIFFERIMENTO; e poi metterla nella
			// stringa di RITORNO (Primo carattere = 1)
			...
			// FineDifferimentoModel = lCalcDiffModel.getPenaDaEspiare
			// (data_inizio,lDataSistemaPerCalcoli,"all",null);
			...
			lDataDifferimento = 1 + lSeparatore + SaaF + lSeparatore + SmmF + lSeparatore + SggF;
