...
/**
 * <p>Title: ActLoadInserisciAttesaArchiviazione</p>
 * <p>Description: </p>
...
public class ActLoadInserisciAttesaArchiviazione extends ActionSiap implements ICostantiArchiviazione {
	...
	public String processRequest() throws Exception {
		...
		Collection lColl = DecodificheManager.getInstance().getAttesaArchiviazione();

		Vector<DecodeModel> lNewColl = new Vector<DecodeModel>();

		Iterator itx = lColl.iterator();
		while (itx.hasNext()) {
			DecodeModel decodeModel = (DecodeModel) itx.next();
			if (!decodeModel.getCode().equals("0487")) {
				lNewColl.add(decodeModel);
			}
		}

		// Option lOption = new Option(DecodificheManager.getInstance().getAttesaArchiviazione());
		Option lOption = new Option(lNewColl);
		setRequestAttribute("oggettodefinzione", "" + lOption);

		return PG_LOAD_INSERISCI_ATTESA_ARCHIVIAZIONE;
	}
