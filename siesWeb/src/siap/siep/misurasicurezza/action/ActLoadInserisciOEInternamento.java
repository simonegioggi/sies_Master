 ...
 *
 * <p>
 * Title: ActLoadInserisciOEInternamento
 * </p>
 * <p>
 * Description: Classe la load Emissione O.E. per Internamento
 * </p>
 * <p>
...
public class ActLoadInserisciOEInternamento extends ActionSiap implements ICostantiMisuraSicurezza {
	...
	public String processRequest() throws F3BException {
		...
		// prendo solo l'ultima MISURA (24/10/2014 se esiste)
		...
		DepoOrdinanze = lDepoCtrl
				.ExRicercaDepositoOrdinanzaPcEventoByFascicoloSiep(lFascMod.getIdFascicoloSiep());
		 ...
		 * setRequestAttribute(IWebConstants.GOTO_PAGE, "" + lRedirigi);
		 *
		 * return IWebConstants.PG_MESSAGE; } else {
		 *
		 * if(!lMisSicMod.getCodNatura().equals("01")) { RedirectTo lRedirigi = new RedirectTo();
		 * lRedirigi.setPage(IWebConstants.PG_MAIN); setRequestAttribute(IWebConstants.MESSAGE_TEXT,
		 * "Procedimento N." + lFascMod.getChiaveAnno() + "/" + lFascMod.getChiaveProgr() +
		 * ": La Misura in esecuzione non è di tipo DETENTIVA");
		 * lRedirigi.setAction("siap.siep.fascicolo.action.ActLoadRicercaFascicoloUnivoco&" +
		 * ICostantiFascicoloSiep.CAMPO_AZIONE_CHIAMANTE + "=" + getClass().getName());
		 * setRequestAttribute(IWebConstants.GOTO_PAGE, "" + lRedirigi);
		 *
		...
		// MEV_39: aggiunto recupero info su esistenza verbale per fascicolo ed eventuale istituto di
		// detenzione da impostare come attributo nell request
		// if (!isRequestParameterNullObj("idIstitutoDetenzione")
		// && Utils.isPresent(getRequestStringParameter("idIstitutoDetenzione"))) {
		// IIstitutoDetenzione iid = SIEPLookupRemote.getIstitutoDetenzioneRemote();
		// IstitutoDetenzioneModel idm = iid
		// .ExRicercaIstitutoDetenzioneByKey(getRequestStringParameter("idIstitutoDetenzione"));
		// setRequestAttribute("istitutoDetenzioneModel", idm);
		// }
		...
		return PG_LOAD_INS_OE_INTERNAMENTO; // restituisce la jsp di VIEW
