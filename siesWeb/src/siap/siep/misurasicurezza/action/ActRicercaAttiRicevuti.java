...
/**
 * Action per la ricerca degli atti ricevuti per competenza Misure di Sicurezza non ancora elaborati:
 * FLAG_VISTO = 'N'.
 *
	...
	public String processRequest() throws F3BException, Exception {
		...
		if (isRequestParameterNullObj("vai")) {
			...
			if (!isRequestParameterNullObj(IWebConstants.NUM_PAGE))
				setRequestAttribute(IWebConstants.NUM_PAGE,
			...
			if (lVect != null) {
				for (int i = 0; i < lVect.size(); i++) {
					...
					if (lMessaggioRichiesta.getContaSolleciti() != null
							&& lMessaggioRichiesta.getContaSolleciti().intValue() > 0) {
						// Ticket#20220111018 - FINE
						BigDecimal idMessaggioSollecitato = null;
						if (lMessaggioRichiesta.getJmsCorrelationReplyTo() != null)
							idMessaggioSollecitato = new BigDecimal(
									lMessaggioRichiesta.getJmsCorrelationReplyTo());
						else
							idMessaggioSollecitato = new BigDecimal(
									lMessaggioRichiesta.getJmsCorrelationIdMessage());

						Vector<MessaggioModel> lVectSoll = lCtrl.ExRicercaSollecitiByIdRich(
								ICostantiJMS.DELIVERY_MODE_RICEVUTO, ICostantiJMS.RICHIESTA,
								ICostantiJMS.SOLLECITO_TRASFERIMENTO_COMPETENZA_MS, idMessaggioSollecitato,
								null // "N" // Flag_visto.
								, getCodUfficioUtenteConnesso() // ufficio dest
						);
						if (lVectSoll != null)
