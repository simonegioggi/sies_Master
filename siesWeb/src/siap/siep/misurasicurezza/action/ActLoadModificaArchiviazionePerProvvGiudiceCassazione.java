...
/**
 * <p>
 * Title: ActLoadModificaArchiviazionePerProvvGiudiceCassazione
 * </p>
 * <p>
 * Description: classe per Load Modifica dei Provvedimenti
 * </p>
 * <p>
 * di Archiviazione per provvedimento del Giuduce /Cassazione / Riesame
 * </p>
 * <p>
...
public class ActLoadModificaArchiviazionePerProvvGiudiceCassazione extends ActionSiap implements
	...
	public String processRequest() throws F3BException {
		...
		// EventoNotifica (Provvedimento di Archiviazione Inserito)
		...
		if (lArcMod != null && lArcMod.getIdArchiviazione() != null) {
			if (lArcMod.getCodTipoAutoritaEmittente() != null) {
				lOptionEmi.setSelected(lArcMod.getCodTipoAutoritaEmittente());
			}
		}
		setRequestAttribute("autorita", "" + lOptionEmi);

		// ComboBox Tipo Provvedimento
		...
		if (lArcMod != null && lArcMod.getIdArchiviazione() != null) {
			if (lArcMod.getCodTipoProvvedimentoArc() != null) {
				lOptionProvv.setSelected(lArcMod.getCodTipoProvvedimentoArc());
			}
		}
		setRequestAttribute("tipoprovvedimento", "" + lOptionProvv);
		...
		String lCodUfficioTipoGE = "-";
		String lUfficioDescrSedeGE = "";
		...
		if (lEveMod.getNotifiche() != null && lEveMod.getNotifiche().length > 0) {
			for (int i = 0; i < lEveMod.getNotifiche().length; i++) {
				...
				if (lEveMod.getNotifiche()[i] != null
						&& ("NG").equals(lEveMod.getNotifiche()[i].getCodTipoNotifica())) {
					if (lEveMod.getNotifiche()[i].getUfficio() != null) {
						lCodUfficioTipoGE = lEveMod.getNotifiche()[i].getUfficio().getCodTipoUfficio();
						lUfficioDescrSedeGE = lEveMod.getNotifiche()[i].getUfficio().getDescrComune();
					}
				}
		...
		// ComboBox Notifica Giudice Esecuzione
		Option lOptionDest = new Option(DecodificheManager.getInstance().getTipoUfficioGE());
		if (lCodUfficioTipoGE.compareTo("-") != 0) {
			lOptionDest.setSelected(lCodUfficioTipoGE);
		}
		setRequestAttribute("autoritaDestGE", "" + lOptionDest);
		setRequestAttribute("sedeautoritaDestGE", "" + lUfficioDescrSedeGE);
		...
		return PG_LOAD_MODIFICA_ARCHIVIAZIONE_PROVV_GIUDICE_CASSAZIONE;
