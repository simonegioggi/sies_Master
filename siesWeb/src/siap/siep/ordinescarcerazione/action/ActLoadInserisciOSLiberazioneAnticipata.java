...
/**
 * <p>
 * Title: ActLoadInserisciOSLiberazioneAnticipata
 * </p>
 * <p>
 * Description: Inserisci Ordine si Scarcerazione con Liberazione Anticipata
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class ActLoadInserisciOSLiberazioneAnticipata extends ActOrdineScarcerazione
	...
	public String processRequest() throws F3BException {
		...
		try {
			...
			// LogF3B.getLogger()
			siesLogger.debug("nessun elemento trovato" + e);
		}

		if (llibAntMod == null) {
			throw new SIEPException(SIEPException.USER_MESSAGE,
					"Non ci sono dati sufficienti per emettere Ordine di Scarcerazione.");
		}
		...
		// MEV29 - d.f. - 13/03/2015 x gestire la posizione giuridica (02)
		Option lOptionPolizia = new Option(DecodificheManager.getInstance().getTipoAutorita());
		setRequestAttribute("autoritaEsternaE", "" + lOptionPolizia);
		...
		// NOn è altra Causa
		if (lFascMod != null && lFascMod.getFlagAltraCausa() == null || (lFascMod.getFlagAltraCausa() != null
				&& lFascMod.getFlagAltraCausa().compareTo("N") == 0)) {
			...
			if (lPosMod.isMisAlt() || lPosMod.getCodPosizioneGiuridica().equals("04"))
				throw new SIEPException(SIEPException.USER_MESSAGE,
						"Impossibile effettuare questo Ordine di Scarcerazione per un Condannato in Misura Alternativa.");

			// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
			// LogF3B.getLogger()
			siesLogger.debug(" * * * Posizione = " + lPosMod);
			setRequestAttribute("posizione", lPosMod);
			setRequestAttribute("detenutoAltraCausa", "NO");
			lDataInizioPena = lPosMod.getDataInizio();

			ILuogoDetenzione lLuo = SIEPLookupRemote.getLuogoDetenzioneRemote();
			LuogoDetenzioneModel lDet = lLuo
					.ExRicercaLuogoDetenzioneCorrenteByFascicoloSiep(lPosMod.getFasSieIdFascicoloSiep());
			if (lDet != null && lDet.getIstitutoDetenzione() != null)
				lOption = new Option(DecodificheManager.getInstance().getTipoAutorita(),
						lDet.getIstitutoDetenzione().getCodTipoIstituto());

			setRequestAttribute("luogoDetenzione", lDet);
			// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
			// LogF3B.getLogger()
			siesLogger.debug(" * * * Luogo = " + lDet);
		} else // ALTRA CAUSA
		{
			...
			if (lPos.getPosizioneGiuridica().isMisAlt()
		...
		return PG_INSERISCI_OS_LIBERAZIONE_ANTICIPATA; // restituisce la jsp di VIEW
	}
}