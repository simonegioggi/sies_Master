...
/**
* <p>Title: ActLoadInserisciIstanzaPerSoggetto</p>
...
public class ActLoadInserisciIstanzaPerSoggetto extends ActionSiap implements ICostantiNuovaIstanza
{
  ...
  *****************************************************************************/
  public String processRequest() throws Exception 
  {
	  ...
	  if(!isRequestParameterNullObj(ICostantiFascicoloSiep.CAMPO_ID_FASCICOLO_SIEP))//istanza  
	  {
	    ...
	    setSessionAttribute("fascicolo", lDettaglio.getFascicoloSiep());
	  	...
	  	setRequestAttribute("soggetto",lDettaglio.getFascicoloSiep().getSoggetto() );

	    setSessionAttribute("penaresidua", lDettaglio.getPenaResidua());

	}
	else if(!isRequestParameterNullObj(ICostantiSoggetto.CAMPO_ID_SOGGETTO))//soggetto+istanza
	{
		setRequestAttribute("idsoggetto",""+ getRequestBigDecimalParameter(ICostantiSoggetto.CAMPO_ID_SOGGETTO));
		lTipoInserimento = "soggetto";
		
	  ISoggetto lCtrl = SICOLookupRemote.getSoggettoRemote();
	  SoggettoModel lSog = lCtrl.ExRicercaSoggettoByKey( getRequestBigDecimalParameter(ICostantiSoggetto.CAMPO_ID_SOGGETTO));
	  if (lSog != null){ 
  ...
  // Riempie la combo delle nazioni
    ...
    return PG_LOAD_INSERISCI_SOGGETTO_ISTANZA; 
  }
}