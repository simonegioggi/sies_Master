...
/**
 * Action per il caricamento della form di ricerca del fascciolo su cui trasferire i dati dell'istruttoria
 *
 * @author difiorlett
	...
	public String processRequest() throws F3BException {
		...
		if (!isFascicoloSiepDiCompetenza()) {
			...
			setRequestAttribute(IWebConstants.MESSAGE_TEXT,
					"Fascicolo non di competenza. Impossibile procedere.");
		...
		if (isRequestParameterNullObj(CAMPO_ID_ISTRUTTORIA_CUMULO)
			...
			if (lIstruttoriaModel.getFlagStato().equals(ICostantiIstruttoriaCumulo.FLAG_STATO_APERTA)) {
			...
			} else {// Sto visualizzando una Istruttoria non aperta, non posso chiuderla e trasferirla
				RedirectTo lRedirigi = new RedirectTo();
				lRedirigi.setPage(IWebConstants.PG_MAIN);
				if (lIstruttoriaModel.getFlagStato()
						.equals(ICostantiIstruttoriaCumulo.FLAG_STATO_ANNULLATA)) {
					setRequestAttribute(IWebConstants.MESSAGE_TEXT, "L'istruttoria corrente "
							+ lIstruttoriaModel.getAnnoProtocollo() + "/"
							+ lIstruttoriaModel.getNumProtocollo() + " del "
							+ DateUtils.getDateToString(lIstruttoriaModel.getDataApertura(), "dd-MM-yyyy")
							+ " risulta annullata");
				} else if (lIstruttoriaModel.getFlagStato()
						.equals(ICostantiIstruttoriaCumulo.FLAG_STATO_CHIUSA)) {
					setRequestAttribute(IWebConstants.MESSAGE_TEXT,
							"L'istruttoria corrente " + lIstruttoriaModel.getAnnoProtocollo() + "/"
									+ lIstruttoriaModel.getNumProtocollo() + " del "
									+ DateUtils.getDateToString(lIstruttoriaModel.getDataApertura(),
											"dd-MM-yyyy")
									+ " risulta chiusa a seguito emissione Provvedimento di Cumulo");
				} else {
					setRequestAttribute(IWebConstants.MESSAGE_TEXT, "L'istruttoria corrente "
							+ lIstruttoriaModel.getAnnoProtocollo() + "/"
							+ lIstruttoriaModel.getNumProtocollo() + " del "
				...
				lRedirigi.setAction("siap.siep.istruttoriacumulo.action.ActLoadGrigliaCumulo&"
						+ CAMPO_ID_ISTRUTTORIA_CUMULO + "=" + lIstruttoriaModel.getIdIstruttoriaCumulo());
				setRequestAttribute(IWebConstants.GOTO_PAGE, "" + lRedirigi);
