...
/**
 * <p>Title: PosizioneGiuridicaSqlDAO</p>
 * <p>Description: Classe SqlDAO che rappresenta la tabella PosizioneGiuridica</p>
...
public class PosizioneGiuridicaSqlDAO extends SIAPSqlDAO
{
  ...
  public void ricercaPosizioneGiuridicaByIdFascicolo(BigDecimal aIdFascicolo) throws DAOException
  {
    ...
    //**** lSql += " ORDER BY ID_POSIZIONE_GIURIDICA DESC"; //****** PROBLEMA ORDINE SEQUENCE ID_POSIZIONE!!!!
  ...
  public void ricercaMaxIdPosizioneGiuridicaByIdFascicoloSiep(BigDecimal aIdFascicoloSiep) throws DAOException
  {
    String lStatement = new String();

/* ********************* //****** PROBLEMA ORDINE SEQUENCE ID_POSIZIONE!!!!
    lStatement += " SELECT MAX(ID_POSIZIONE_GIURIDICA) ID_POSIZIONE_GIURIDICA";
    lStatement += " FROM POSIZIONE_GIURIDICA";
    lStatement += " WHERE FAS_SIE_ID_FASCICOLO_SIEP = " + aIdFascicoloSiep;
**********************************/
    lStatement += " SELECT ID_POSIZIONE_GIURIDICA";
    lStatement += " FROM POSIZIONE_GIURIDICA";
    lStatement += " WHERE DATA_INSERIMENTO = ( SELECT MAX(DATA_INSERIMENTO) FROM POSIZIONE_GIURIDICA WHERE FAS_SIE_ID_FASCICOLO_SIEP = " + aIdFascicoloSiep + ")";
  ...
  protected String getSqlQuery()
  {
    ...
    lStatement += "SELECT " +
                  ...
                  " COD_POSIZIONE_GIURIDICA, POS_GIU.RV_MEANING DESCR_POSIZIONE_GIURIDICA,"+
                  " DATA_INIZIO, "+
                  " DATA_FINE, "+
  ...
  public String setCondizione(PosizioneGiuridicaModel aModel)
  {
    ...
    {
      lStatement += " AND DATA_INIZIO=TO_DATE('" + DateUtils.getDateToString (aModel.getDataInizio(), "ddMMyyyy") + "', 'DDMMYYYY') " ;
    }
  ...
  private String setCondizioniPerIdFascicoloCorrente(PosizioneGiuridicaModel aModel)
  {
    ...
    if ( (aModel.getFasSieIdFascicoloSiep() != null) && (aModel.getFasSieIdFascicoloSiep().doubleValue() != 0))
    {
/*******************     //****** PROBLEMA ORDINE SEQUENCE ID_POSIZIONE!!!!!
      lCondizioni += " AND FAS_SIE_ID_FASCICOLO_SIEP = " + aModel.getFasSieIdFascicoloSiep();
      lCondizioni += " AND DATA_FINE IS NULL";
******************************/
      lCondizioni += " AND DATA_INSERIMENTO = ( SELECT MAX(DATA_INSERIMENTO) FROM POSIZIONE_GIURIDICA WHERE FAS_SIE_ID_FASCICOLO_SIEP = " + aModel.getFasSieIdFascicoloSiep() + ")";
      lCondizioni += " AND FAS_SIE_ID_FASCICOLO_SIEP = " + aModel.getFasSieIdFascicoloSiep();
    }
...
//****  //****** PROBLEMA ORDINE SEQUENCE ID_POSIZIONE!!!!
//****lCondizioni += " AND ID_POSIZIONE_GIURIDICA = ( SELECT MAX(ID_POSIZIONE_GIURIDICA) FROM POSIZIONE_GIURIDICA WHERE FAS_SIE_ID_FASCICOLO_SIEP = " + aIdFascicolo + ")";
    lCondizioni += " AND DATA_INSERIMENTO = ( SELECT MAX(DATA_INSERIMENTO) FROM POSIZIONE_GIURIDICA WHERE FAS_SIE_ID_FASCICOLO_SIEP = " + aIdFascicolo + ")";
    lCondizioni += " AND FAS_SIE_ID_FASCICOLO_SIEP = " + aIdFascicolo;
...
/**
 * Aggiorna ed inserisce la posizione giuridica e se aLuogoDetenzione = 'S'
 * aggancia il luogo detenzione precedente alla nuova posizione giuridica
 * @param lConn
 * @param lPosizione
 * @param lPos
 * @param lData
 ...
 * @throws F3BException
 */
  public BigDecimal inserimentoAggiornamentoPosizioneGiuridica(Connection lConn, String lPosizione, PosizioneGiuridicaModel lPos, Date lData,
      EventoModel lEveModel, BigDecimal aKeyFasc, BigDecimal aKeyEve,String aLuogoDetenzione) throws DAOException, F3BException
  {
    ...
    try
    {
      ...
      if (lPos != null && lPos.getDataFine() == null)
      {
        ...
        lPosDao.setDataAggiornamento(lEveModel.getDataAggiornamento()!= null ? lEveModel.getDataAggiornamento() : lEveModel.getDataInserimento());
      ...
      if (lPosizione != null)
      {
        ...
        lPosDao.setDataInserimento(lEveModel.getDataAggiornamento()!= null ? lEveModel.getDataAggiornamento() : lEveModel.getDataInserimento());
