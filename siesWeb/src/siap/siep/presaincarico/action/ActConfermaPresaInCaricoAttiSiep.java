...
/**
 * Action di presa in carico atti ricevuti. Utilizzata sia per atti provenienti stessa BDI che altra BDI. Solo
 * CUMULO.
 *
 * @author d.fiorletta
 * @version 1.0
...
public class ActConfermaPresaInCaricoAttiSiep extends ActionSiap implements ICostantiJMS {
	...
	public String processRequest() throws Exception {
		...
		/*
		 * ParserMessage lPars = new ParserMessage(lMess.getTreeModel()); String lChiaveUffFasc =
		 * lPars.getDettaglioFascicoloSiep().getFascicoloSiep().getChiaveUfficio(); UfficioModel lUfficioAtti
		 * = this.getUfficioByCodUfficio(lChiaveUffFasc);
		 *
		 * boolean lStessaBDI = false; if
		 * (lUfficioAtti.getCodDistretto().equalsIgnoreCase(getCodDistrettoUtenteConnesso())) { lStessaBDI =
		 * true; } else { lStessaBDI = false; }
		 */
		...
		if (lIdIstruttoria != null) {
			 ...
			 *
			 * // n.b. Questa query non servirebbe. L'idFascicolo è già noto, è quello del Messaggio, // le
			 * chiavi infatti non cambiano. Si effettua la query solo x conferma // acquisizione dati.
			 * siesLogger.debug("lFascSiepModel = "+lFascSiepModel); IFascicoloSiep lCtrlFasc =
			 * SIEPLookupRemote.getFascicoloSiepRemote(); lFascSiepModel =
			 * lCtrlFasc.ExRicercaFascicoloSiepByProgrAnnoCodUfficio (lFascSiepModel);
			...
			lManagerSeguito.aggiornaStatoEsecuzioneTitolo(lFascSiepModel.getIdFascicoloSiep(), lIdIstruttoria,
					lDatiOper);
