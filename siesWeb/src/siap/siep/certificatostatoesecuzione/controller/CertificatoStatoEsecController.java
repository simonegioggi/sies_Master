...
public class CertificatoStatoEsecController extends GenericController implements ICertificatoStatoEsec {
	...
	public Vector ExRicercaCertificatoStatoEsec(CertificatoStatoEsecModel aCertificatoStatoEsec)
		...
		try {
			...
			lCerDao.start();
			while (lCerDao.next()) {
				lCertificatoStatoEsei.add((CertificatoStatoEsecModel) lCerDao.getModel());
			}
			lCerDao.stop();
	 ...
	 */
	public Vector ExRicercaCertificatoStatoEsecByIdFascicoloSiep(BigDecimal aIdFascicolo) throws F3BException {
		Connection lConn = null;
	...
	public ByteArrayOutputStream ExStampaDocumento(FascicoloSiepModel aFasc, String lIdTemplate,
		...
		try {
			...
			rollback(lConn);
			throw new F3BException("CertificatoStatoEsecController.ExStampaDocumento: Non posso inserire: "
					+ ex);
		} finally {
			cleanup(lCertSEDao);
			cleanup(lConn);
