 ...
 * MEV_39
 * <p>
 * Title: ActStampaScadenzarioComunicazioneScadenzaMS
 * </p>
 * <p>
...
public class ActStampaScadenzarioComunicazioneScadenzaMS extends ActionSiap implements ICostantiScadenzario {

	public String processRequest() throws F3BException {

		// Lock
		LockModel lm = LockController.lockIfNotLocked(getServletContext(),
				"SCADENZARIO_COMUNICAZIONE_SCADENZA_MS", "1", getCodUtenteConnesso(), getSession().getId());
		...
		// Imposto fisso 20=comunicazione scadenza MS ed N=Tutti i non visti
		...
		Vector scadenzari = is.ExRicercaScadenzario(sm, "CSMS");
		...
		creaFoglioScadenzarioComunicazioneScadenzaMS(sm, scadenzari, wb, getUfficioUtenteConnesso(),
		...
		try {
			...
			throw new F3BException("ActStampaScadenzarioComunicazioneScadenzaMS.processRequest: " + ioe);
		}
	...
	private void creaFoglioScadenzarioComunicazioneScadenzaMS(ScadenzarioModel sm, Vector scadenzari,
			HSSFWorkbook wb, UfficioModel um, String descUffIntestazione, String titolo) {
		...
		ScadenzarioUtils.setCell(row, (short) 0,
				"Consultazione Scadenzario Comunicazione Scadenza Misura di Sicurezza - " + titolo, csNull);
		...
		sheet.setColumnWidth((short) numCol++, (short) (15 * 256));
		sheet.setColumnWidth((short) numCol++, (short) (15 * 256));
		sheet.setColumnWidth((short) numCol++, (short) (25 * 256));
		sheet.setColumnWidth((short) numCol++, (short) (25 * 256));
		sheet.setColumnWidth((short) numCol++, (short) (25 * 256));
		sheet.setColumnWidth((short) numCol++, (short) (25 * 256));
		sheet.setColumnWidth((short) numCol++, (short) (25 * 256));
		...
		while (itx.hasNext()) {
			...
			FascMsToFascSiepModel lFascToFasc = lSca.getFascMsToFascSiep();
			row = sheet.createRow(nRow++);
			ScadenzarioUtils.setCell(row, numCol++, lFas.getChiaveAnno() + "/" + lFas.getChiaveProgr(),
					csCenter);
			ScadenzarioUtils.setCell(row, numCol++, (lFascToFasc.getChiaveAnnoSiepCollegato()!=null ? lFascToFasc.getChiaveAnnoSiepCollegato() :"-") + "/"
					+ (lFascToFasc.getChiaveProgrSiepCollegato()!=null ? lFascToFasc.getChiaveProgrSiepCollegato() :"-"), csCenter);
			...
			Date dataScadenzaComunicazioneMS = null;
			if (lSca.getDataFineScadenza() != null)
				dataScadenzaComunicazioneMS = DateUtils.moveDateTo(lSca.getDataFineScadenza(), Calendar.MONTH,
						-6);
			ScadenzarioUtils.setCell(row, numCol++, StringUtils.toStringJSP(
					DateUtils.getDateToString(dataScadenzaComunicazioneMS, "dd/MM/yyyy")), csCenter);
			ScadenzarioUtils.setCell(row, numCol++, StringUtils.toStringJSP(lSca.getDescrTipoMS()), csCenter);
			// Scaduti
			...
			if (lSca.getDataFineScadenza() != null)
				...
				ScadenzarioUtils.setCell(row, numCol++, "", csCenter);
		}
	}
}