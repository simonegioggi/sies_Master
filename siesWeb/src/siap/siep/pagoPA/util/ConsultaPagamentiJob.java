...
/**
 * Il job preleva dalla tabella BOLLETTINO_PAGOPA i bollettini non ancora pagati (DATA_AVV_PAGAMENTO null o STATO_PAGAMENTO = PN)
 * e per ogni bollettini egffettua lì'interrogazione al servizio "elencopagamenti" di pagoPA.
 * 
...
public class ConsultaPagamentiJob implements Job {
    ...
    public void execute (JobExecutionContext arg0) throws JobExecutionException {
        ...
        try {
            ...
            pagoPaLogger.debug(" Ricerca debitori con posizioni aperte: inScadenzaTraGiorni "+inScadenzaTraGiorni+", controllateDaGiorni "+controllateDaGiorni);
            Vector <BollettinoPagopaModel> lElencoDebitori = lCtrlBollettini.ExRicercaDebitoriConPosizioniAperteInScadenza(inScadenzaTraGiorni, controllateDaGiorni);
            ...
            for (int i=0;i<lElencoDebitori.size();i++) {
                ...
                try {
                    pagoPaLogger.debug(" Inizio richiesta a pagoPa per il Debitore = "+lDebitore.getCodiceFiscale()
                                     +", codice distretto = "+lDebitore.getCodiceDistretto());
                    contaNumPosBebitorieVerificate++;
                    
                    // n.b. la ricerca per codiceCRS vuole comunque il codice fiscale obblgatorio altrimenti non trova nulla
                    ...
                    // - DISPONIBILE - Il valore indica che è presente una Ricevuta Telematica con esito positivo 
                    //                 (pagamento eseguito).
                    ...
                    RisultatoRicerca rr = scpt.elencoPagamenti (codiceCRS, tipologia, codiceFiscale
                                                                , codiceDistretto
                                                                , causale, stato
                                                                , dataRichiestaDa, dataRichiestaA
                    ...
                    if (pagamenti==null || pagamenti.length==0) {
                        ...
                        // recupero in una sola transazione tutti i bollettini a carico del CF per poterli confrontare
                        ...
                        for (int progPagamento = 0; progPagamento<pagamenti.length; progPagamento++) {
                            ...
                            try {
                                ...
                                if (pagamenti[progPagamento] instanceof StatoRichiestaPagamento) {
                                    ...
                                    // Devo verificrae se il pagamento restituito è tra quelli pending e in caso aggiornare
                                    // lo stato
                                    ...
                                    if (bollettinoSIES!=null)
                                    {
                                        if (ICostantiPagoPA.SIES_STATO_NON_PAGATO.equals(bollettinoSIES.getStatoPagamento()) ) {
                                            ...
                                            if (statoRichiesta.getStato().equals(ICostantiPagoPA.PAGOPA_STATO_DISPONIBILE)) {
                                                // Il bollettino è stato pagato. Aggiorno opportunamente i campo 
                                                bollettinoSIES.setStatoPagamento(ICostantiPagoPA.SIES_STATO_PAGATO); // PA = Pagato
                                                ...
                                                //TODO calcolare le scadenze delle successive rate se necessario
                                            }
                                            // else if (statoRichiesta.getStato().equals(ICostantiPagoPA.PAGOPA_STATO_CARRELLO)) {
                                            else if (statoRichiesta.getStato().equals(ICostantiPagoPA.PAGOPA_STATO_GENERATA)) {
                                                // Non aggiorno nulla. Bollettino non ancora pagato ho già salvato stato e data controllo
                                            }
                                            else {
                                        ...
                                        pagoPaLogger.debug("Per il numero di avviso "+iuv+" non esiste alcuna posizione sulla tabella dei bollettini");
                                        esitoEsecuzione+="\nwarn iuv "+iuv+" non presente in banca dati";
                                    }
                                }
                                else {
                                    pagoPaLogger.warn("L'oggetto restituito NON è una istanza di StatoRichiestaPagamento"); 
                                }
                            }
                            catch (Exception e) {
                    ...
                    pagoPaLogger.error(" Errore in fase di invocazine del WS PagoPA.elencoPagamenti per il Debitore: CF = "
