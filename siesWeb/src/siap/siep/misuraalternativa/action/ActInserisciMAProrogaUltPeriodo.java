...
public class ActInserisciMAProrogaUltPeriodo extends ActMisuraAlternativa implements
...
//		String tipoMisura = null;
		...
		if (this.getRequestStringParameter("flagmisura").equals("N")) {
			...
			lTenMod.setCodEsitoTenore("0036");
...
//			IMisuraAlternativa lMisAltCtrl = SICOLookupRemote.getMisuraAlternativaRemote();
			/*
			 * lMisAlModCorr =
			 * lMisAltCtrl.ExRicercaMisuraAlternativaCorrenteByIdFascicolo(lFascicoloModel.getIdFascicoloSiep
			 * ()); //setto la misuraalternativa if(lMisAlModCorr != null) {
			 * 
			 * lMisAlModCorr.setDataFineMisura(getRequestDateParameter(ICostantiEvento.CAMPO_ANNO_DATA_EMISSIONE
			...
			if (lPenaResMod != null && lPenaResMod.getDataFine() != null) {
				lMisMod.setDataFineMisura(lPenaResMod.getDataFine());

			} else {
				lMisMod.setDataFineMisura(getRequestDateParameter(
			...
			if (lPosizione.equals("03")) {
				lEveNot.getEvento().setCodMotivo("0077");
			...
			lPage = IWebConstants.PG_MAIN + "?" + IWebConstants.ACTION_FIELD
					+ "=siap.siep.misuraalternativa.action.ActDettaglioMAProrogaUltPeriodo&"
			...
			lMisAlModConcessa = lMisAltController
					.ExRicercaMisuraAlternativaProrogaUltPeriodoByIdFascicolo(lFascicoloModel
							.getIdFascicoloSiep());

			EventoNotificaModel lEve = new EventoNotificaModel();
			lEve.getEvento().setCodTipoProvvedimento("04");
			if (lPosizione.equals("03")) {
				lEve.getEvento().setCodMotivo("0077");
