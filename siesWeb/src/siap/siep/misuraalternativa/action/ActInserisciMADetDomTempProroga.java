...
/**
 * <p>Title: ActInserisciMADetDomTempProroga</p>
...
public class ActInserisciMADetDomTempProroga extends ActMisuraAlternativa
	...
	public String processRequest() throws F3BException {
		...
		// LogF3B.getLogger()
		siesLogger.debug("ActInserisciMADetDomTempProroga --------->> ");
		...
		// controllo se provengo da una concessione detenzione domiciliare a termine o da una proroga
		MisuraAlternativaModel lMisPre = new MisuraAlternativaModel();
		lMisPre = lMisAltCtrl
				.ExRicercaMisuraAlternativaCorrenteByIdFascicolo(lFascicoloModel.getIdFascicoloSiep());

		if (lMisPre != null)
			// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
			// LogF3B.getLogger()
			siesLogger.debug("lMisPre --------->> " + lMisPre);

		if (lMisPre == null || lMisPre.getIdMisuraAlternativa() == null
				|| lMisPre.getCodTipoDecisione() == null || !lMisPre.getCodTipoDecisione().equals("03")
				|| lMisPre.getCodNaturaDecisione() == null || !lMisPre.getCodNaturaDecisione().equals("DD")
				|| lMisPre.getCodTipoMisura() == null || (!lMisPre.getCodTipoMisura().equals("0011")
						&& !lMisPre.getCodTipoMisura().equals("0197"))) {
			lMisPre = null;
		}
		...
		if (lAnniMisura != null || lGiorniMisura != null || lMesiMisura != null) {
			...
			if (PosizioneGiu.equals("12")) {
				if (lMisPre != null && lMisPre.getDataFineMisura() != null) {
					lDataInizio = DateUtils.getDayAfter(lMisPre.getDataFineMisura());
		...
		if (lDataInizio != null)
			// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
			// LogF3B.getLogger()
			siesLogger.debug("lDataInizio --------->> " + lDataInizio);
		...
		if (lMisAlModConcessa == null) {
			...
			// setto il tenore
			String lEsito = null;
			if (getRequestStringParameter(ICostantiEvento.CAMPO_COD_MOTIVO).equals("0011"))
				lEsito = "0001";
			else if (getRequestStringParameter(ICostantiEvento.CAMPO_COD_MOTIVO).equals("0197"))
				lEsito = "0036";

			TenoreModel lTenMod = setTenore(new BigDecimal(1), lEsito);
			...
			lEveNot.getEvento().setCodMotivo("0197");
			...
			this.setRequestAttribute("tipoMisura", "DETENZIONEATEMPO");

			EventoNotificaModel lEve = new EventoNotificaModel();

			lEve.getEvento().setCodTipoProvvedimento("06");
			lEve.getEvento().setCodMotivo("0197");
