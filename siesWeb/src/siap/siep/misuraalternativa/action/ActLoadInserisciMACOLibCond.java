...
/**
 * <p>
 * Title: ActLoadInserisciMACOLibCond
 * </p>
 * <p>
...
public class ActLoadInserisciMACOLibCond extends ActMisuraAlternativa implements ICostantiMisuraAlternativa {
	...
	public String processRequest() throws F3BException {
		...
		String lcodPos = lPos.getPosizioneGiuridica().getCodPosizioneGiuridica();
		if (lPos.getPosizioneGiuridica() != null
				&& (!lcodPos.equals("03") && !lcodPos.equals("12") && !lcodPos.equals("13") && !lcodPos
						.equals("14")))
			throw new SIEPException(SIEPException.USER_MESSAGE, "La Posizione Giuridica non è corretta.");
			...
			if (lMisAlModConcessa != null && lMisAlModConcessa.getIdMisuraAlternativa() != null) {
				...
				if (lMisAlModConcessa != null) {
					if (lMisAlModConcessa.getCodTipoDecisione() == null
							...
							|| !lMisAlModConcessa.getCodTipoMisura().equals("0025")) {
		...
		Collection lmotivo = DecodificheManager.getInstance().getMotivoProvvedimentoMACOLibCond();
		DecodificheModel lDecMod = null;
//		String lCodMotivo = "";
		if (lmotivo != null && !lmotivo.isEmpty()) {
			Iterator lIter = lmotivo.iterator();
			if (lIter.hasNext()) {
				lDecMod = (DecodificheModel) lIter.next();
//				lCodMotivo = lDecMod.getDescription();
		...
		return PG_LOAD_INSERISCI_MA_CONC_LIB_COND;
	}
