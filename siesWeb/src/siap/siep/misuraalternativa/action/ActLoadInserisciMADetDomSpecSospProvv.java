...
/**
 * <p>
 * Title: ActLoadInserisciMADetDomSpecSospProvv
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class ActLoadInserisciMADetDomSpecSospProvv extends ActMisuraAlternativa implements
	...
	public String processRequest() throws F3BException {
		...
		// Controllo esistenza precedente CONCESSIONE di Detenzione Domiciliare Speciale
		MisuraAlternativaModel lMisAltConcessaMod = lMisAltCtrl
				.ExRicercaMisuraAlternativaConcessaCorrenteByIdFascicolo(lFascMod.getIdFascicoloSiep());
		if (lMisAltConcessaMod != null) {
			String lCodTipoMisura = lMisAltConcessaMod.getCodTipoMisura();

			if (lCodTipoMisura == null || lCodTipoMisura.equals("")
					|| (!lCodTipoMisura.equals("0012") && !lCodTipoMisura.equals("0077"))) {
				throw new SIEPException(SIEPException.USER_MESSAGE,
						"Non è possibile la sospensione provvisoria di una Misura Alternativa non concessa");
			}
		}

		// Ricerca Ultima Decreto di tipo Sospensione legata al Fascicolo
		MisuraAlternativaModel lMisAltMod = null;

		if (this.isRequestParameterNullObj("warning")) {
			lMisAltMod = lMisAltCtrl.ExRicercaMisuraAlternativaSospesaCorrenteByIdFascicolo(lFascMod
					.getIdFascicoloSiep());

			if (lMisAltMod != null && lMisAltMod.getIdMisuraAlternativa() != null) {
				setRequestAttribute("misuraalternativa", lMisAltMod);

				UfficioModel lUffEmiMod = getUfficioByCodUfficio(lMisAltMod.getChiaveUfficioFascicoloSius());

				setRequestAttribute("sedeUfficioEmittente", lUffEmiMod);

				// Controllo Coerenza Richiesta con Decreto TDS
				if (lMisAltMod != null) {
					if (lMisAltMod.getCodTipoDecisione() == null
							|| !lMisAltMod.getCodTipoDecisione().equals("02")
							|| lMisAltMod.getCodNaturaDecisione() == null
							|| !lMisAltMod.getCodNaturaDecisione().equals("SP")
							|| lMisAltMod.getCodTipoMisura() == null
							|| !lMisAltMod.getCodTipoMisura().equals("2152")) {
		...
		if (lMisAltMod != null && lMisAltMod.getIdMisuraAlternativa() != null) {
			// Ricerca evento notifica riferito al Decreto
			lEveMod = lCtrlEvento.ExRicercaEventoNotificaByKey(lMisAltMod.getEveIdEvento());

			// ricerca evento non riferito al Decreto
			EventoModel lEveModelNonOrd = lCtrlEvento.ExRicercaEventoMANonRegistratoByFascicoloSiep(
					lFascMod.getIdFascicoloSiep(), lMisAltMod.getEveIdEvento());
			if (lEveModelNonOrd != null) {
				// ricerca evento notifica non riferito al Decreto
		...
		/*
		 * //Magistrato con TIPO NOTIFICA = C if (lMisAltMod != null) { String lCodMag =
		 * lMisAltMod.getCodMagistrato();
		 * 
		...
		Option lOption = new Option(DecodificheManager.getInstance()
				.getMotivoProvvedimentoDetDomSpecSospProvv());
		...
		return PG_LOAD_INSERISCI_MA_DETDOMSPECSOSPPROVV;
	}
}