...
/**
 * <p>
 * Title: ActUploadMASospProvv
 * </p>
 * <p>
 * Description: Validazione della misura alternativa
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class ActUploadAmmissioneADetDom extends ActionSiap implements ICostantiEvento {
	...
	public String processRequest() throws Exception {
		...
		// EventoModel EveMod=lCtrlEvento.ExRicercaEventoByKey(getRequestBigDecimalParameter(
		// CAMPO_ID_EVENTO));
		...
		if (isRequestChecked(ICostantiEvento.CAMPO_VALIDA)) {
			...
			IMisuraAlternativaBackupSrc lCtrl = SICOLookupRemote.getMisuraAlternativaBackupSrcRemote();

//			String tipoMisura = getRequestStringParameter("tipoMisura");
//			String notificaE = getRequestStringParameter("notificaE");

			lCtrl.ExUpdateValidaAmmissioneADetDom(lModel, lFascMod);
		} else {
		...
		if (!isRequestParameterNullObj(CAMPO_AZIONE_DETTAGLIO)) {
			...
			lRedirigi.setAction(getRequestStringParameter(CAMPO_AZIONE_DETTAGLIO) + "&" + CAMPO_ID_EVENTO
					+ "=" + getRequestStringParameter(ICostantiEvento.CAMPO_ID_EVENTO) + "&tipoMisura="
					+ getRequestStringParameter("tipoMisura") + "&notificaE="
					+ getRequestStringParameter("notificaE") + "&lIdposizione="
					+ getRequestStringParameter("IdPosizioneGiuridica"));
