...
/**
 * Title: TitoloCumulatoSqlDAO Description: Classe SqlDAO che rappresenta la tabella TitoloCumulato
 *
 * @version 1.0
 */
public class TitoloCumulatoSqlDAO extends SqlDAO {
	...
	public void ricercaTitoloCumulatoByIstrIdOrig(BigDecimal aIdIstruttoria, BigDecimal aIdTitoloOrigine)
			throws DAOException {
		// Recupera la select...from
		String lSql = getSqlQuery();
	...
	public void ricercaTitoloCumulatoJoinStatoEsecTitoloCumByIstruttoria_e_Provvedimento(
			BigDecimal aIdIstruttoria, String aOrdinamento, Vector<String> aTipoEve, Vector<String> aTipoProv,
			Vector<String> aCodMot) throws DAOException {
		// Recupera la select...from
		String lSql = getSqlQueryJoinStatoEsecCum();
	...
	public void RicercaTitoloCumulatoPerRevocaBeneficio(BigDecimal aIdTitoloCumulato,
		...
		lSql += " SELECT " + "DISTINCT(tc.ID_TITOLO_CUMULATO), "
				...
				+ "tc.NUM_SEZIONE_AUTORITA_EMITTENTE, " + "tc.ANNO_SENTENZA, tc.NUMERO_SENTENZA, "
				+ "ud.DESCR_TIPO_UFFICIO, ud.DESCR_COMUNE, "
				+ "pc.CHIAVE_ANNO_FAS_CUMULATO, pc.CHIAVE_PROGR_FAS_CUMULATO " + "FROM "
		...
		lSql += " AND ( nvl(tc.COD_TIPO_PROVVEDIMENTO,'-') = CODTIPOPROVVEDIMENTO.RV_LOW_VALUE"
				+ " AND CODTIPOPROVVEDIMENTO.RV_DOMAIN = 'TIPO_PROVVEDIMENTO' ) ";
		lSql += " AND ( nvl(tc.COD_TIPO_AUTORITA_EMITTENTE,'-') = CODTIPOAUTORITAEMITTENTE.RV_LOW_VALUE"
				+ " AND CODTIPOAUTORITAEMITTENTE.RV_DOMAIN = 'TIPO_UFFICIO' ) "; // TIPO_UFFICIO_EMITTENTE
	...
	protected String getSqlQuery() {
		...
		lStatement += " SELECT " + "ID_TITOLO_CUMULATO, "
				...
				+ "COD_TIPO_PROVV_RIF, CODTIPOPROVVRIF.RV_MEANING DESC_TIPO_PROVV_RIF, " + "DATA_PROVV_RIF, "
				+ "ANNO_PROVV_RIF, " + "NUMERO_PROVV_RIF, "
	...
	protected String getSqlQueryDataReatoCum() {
		...
		lStatement += " SELECT DISTINCT " + "ID_TITOLO_CUMULATO, "
				+ "COD_TIPO_PROVVEDIMENTO, CODTIPOPROVVEDIMENTO.RV_MEANING DESC_TIPO_PROVVEDIMENTO, "
				+ "DATA_IRREVOCABILITA, " + "ANNO_REGE_PM, " + "NUMERO_REGE_PM, "
				+ "COD_SEDE_NOTIZIA_REATO, CODSEDENOTIZIAREATO.DESCRIZIONE DESC_SEDE_NOTIZIA_REATO, "
				+ "ANNO_REG_GEN, " + "NUMERO_REG_GEN, " + "TIPO_REG_GEN, " + "DATA_PROVVEDIMENTO, "
				...
				"MESS_ID_MESSAGGIO, TIPO_ISCRIZIONE, " + "DATA_PRESA_IN_CARICO, " +
	...
	public String setCondizioni(TitoloCumulatoModel aModel) {
		...
		if (aModel.getCodTipoAutoritaEmittente() != null
				&& aModel.getCodTipoAutoritaEmittente().length() > 0) {
		...
		if (aModel.getNote1DecisioneCassazione() != null
				&& aModel.getNote1DecisioneCassazione().length() > 0) {
			lCondizioni += " and NOTE1_DECISIONE_CASSAZIONE = '" + aModel.getNote1DecisioneCassazione()
					+ "' ";
		}
		if (aModel.getNote2DecisioneCassazione() != null
				&& aModel.getNote2DecisioneCassazione().length() > 0) {
		...
		if (aModel.getNumeroSentenzaCassazione() != null
				&& aModel.getNumeroSentenzaCassazione().length() > 0) {
		...
		if (aModel.getDataPresaInCarico() != null) {
			lCondizioni += " and to_char(DATA_PRESA_IN_CARICO,'dd/MM/yyyy') = '"
					+ DateUtils.getDateToString(aModel.getDataPresaInCarico(), "dd/MM/yyyy") + "' ";
		}
