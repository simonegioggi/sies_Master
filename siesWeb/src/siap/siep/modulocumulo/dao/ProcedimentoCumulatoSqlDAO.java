...
/**
 * <p>
 * Title: ProcedimentoCumulatoSqlDAO
 * </p>
 * <p>
 * Description: Classe SqlDAO che rappresenta la tabella ProcedimentoCumulato
 * </p>
 * <p>
...
public class ProcedimentoCumulatoSqlDAO extends SqlDAO {
	 ...
	 * @throws DAOException
	 ****************************************************************************/
	public void ricercaProcedimentoCumulatoPaged(ProcedimentoCumulatoModel aModel, int aPage)
			throws DAOException {
	 ...
	 * @throws DAOException
	 ****************************************************************************/
	public void ricercaProcedimentoCumulato(ProcedimentoCumulatoModel aModel, BigDecimal aIdIstruttoria)
			throws DAOException {
		...
		if (aIdIstruttoria != null)
			lSql += " AND (TIT_ID_TITOLO_CUMULATO in (SELECT ID_TITOLO_CUMULATO FROM TITOLO_CUMULATO where ISTR_ID_ISTRUTTORIA_CUMULO = "
					+ aIdIstruttoria + " ))";
	...
	/*****************************************************************************
	 * Effettua la generica ricerca in base ai dati specificati nel model; ( ChiaveAnnoFasCumulato,
	 * ChiaveProgrFasCumulato, CodUfficioFasCumulato ) TIT_ID_TITOLO_CUMULATO deve far parte di una
	 * ISTRUTTORIA data.
	 *
	 * @param aModel,
	 *            IdIstruttoria
	 * @throws DAOException
	 ****************************************************************************/
	public void ricercaProcedimentoCumulatoInIstruttoria(ProcedimentoCumulatoModel aModel,
			BigDecimal aIdIstruttoria) throws DAOException {
		// Recupera la select...from
		String lSql = getSqlQuery();
	 ...
	 * @throws DAOException
	 *********************************************************************************/
	public void ricercaProcedimentoCumulatoByIdFascicoloSiepOrig(BigDecimal aIdFascicoloSiep)
			throws DAOException {
		// Recupera la select...from
		String lSql = getSqlQuery();
	...
	public void ricercaProcedimentoCumulatoByIdFascicoloSiepOrigEIstruttoria(BigDecimal aIdFascicoloSiep,
			BigDecimal aIdIstruttoria) throws DAOException {
		// Recupera la select...from
		String lSql = getSqlQuery();
	...
	protected String getSqlQuery() {
		...
		lStatement += " SELECT " + "ID_PROCEDIMENTO_CUMULATO, " + "ID_FASCICOLO_SIEP_ORIGINE, "
				+ "CHIAVE_ANNO_FAS_CUMULATO, " + "CHIAVE_PROGR_FAS_CUMULATO, "
				...
				+ "COD_UFFICIO_FAS_CUMULATO, " + "DATA_RICHIESTA_FASCICOLO, "
				+ "DATA_PERVENIMENTO_FASCICOLO, " + "NOTE, "
				+ "FLAG_ACCORPATO, CHIAVE_UFFICIO_ORIGINE, CHIAVE_PROGR_ORIGINE, " + "EVE_ID_EVENTO, " +
		...
		// lStatement += " AND (NVL (UFFFASORIG.COD_TIPO_UFFICIO, '-') = TIPO_UFF_ORIG.RV_LOW_VALUE AND
		// TIPO_UFF_ORIG.RV_DOMAIN = 'TIPO_UFFICIO') ";
	...
	public String setCondizioni(ProcedimentoCumulatoModel aModel) {
		...
		if (aModel.getCodTipoUfficioFasCumulato() != null
				&& aModel.getCodTipoUfficioFasCumulato().length() > 0) {
			lCondizioni += " and COD_TIPO_UFFICIO_FAS_CUMULATO = '" + aModel.getCodTipoUfficioFasCumulato()
					+ "' ";
		}
		if (aModel.getCodLuogoUfficioFasCumulato() != null
				&& aModel.getCodLuogoUfficioFasCumulato().length() > 0) {
		...
		if (aModel.getDataRichiestaFascicolo() != null) {
			lCondizioni += " and to_char(DATA_RICHIESTA_FASCICOLO,'dd/MM/yyyy') = '"
					+ DateUtils.getDateToString(aModel.getDataRichiestaFascicolo(), "dd/MM/yyyy") + "' ";
		}
		if (aModel.getDataPervenimentoFascicolo() != null) {
			lCondizioni += " and to_char(DATA_PERVENIMENTO_FASCICOLO,'dd/MM/yyyy') = '"
					+ DateUtils.getDateToString(aModel.getDataPervenimentoFascicolo(), "dd/MM/yyyy") + "' ";
		}
	...
	public void ricercaProcedimentiClasseIVPerRibaltamentoByIdIstru(BigDecimal aIdIstruttoria,
