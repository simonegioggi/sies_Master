...
/**
 * Action per la load inserisci e modifica dei Provvedimenti di Differimento Pena Cumulo
 *
 * @author Intersistemi S.p.A.
 *
 */
public class ActLoadInserisciDifferimentoPenaCumulo extends ActionModuloCumulo
    ...
    siesLogger.debug("--XX-- START ActLoadInserisciDifferimentoPenaCumulo.. ");
		// ==========================================================================
    ...
    TitoloCumulatoModel lTitoloCum = super.getDatiTitoloCumulato();
		...
		if ("I".equals(lModalita)) {
			// Inserimento
      siesLogger.debug(".....--XX-- Sto In Inserimento ");
      PenaResiduaModel lPenRes = new PenaResiduaModel();
      if(lTitoloCum.getProcedimentoCumulato()!=null && lTitoloCum.getProcedimentoCumulato().getIdProcedimentoCumulato()!=null) {
      	if(lTitoloCum.getProcedimentoCumulato().getIdFascicoloSiepOrigine()!=null) {
	      	...
	      	lPenRes = lCtrlPR.ExRicercaPenaResiduaUltimaByDate(lTitoloCum.getProcedimentoCumulato().getIdFascicoloSiepOrigine() );
      	}	
      }
      setRequestAttribute("lPenaResidua", lPenRes);
		...
		// Preparazione codice Motivo Provvedimento per Differimento Pena cumulo.
		DecodificheModel lModel = new DecodificheModel();
		IDecodifiche lDecodifiche = SICOLookupRemote.getDecodificheRemote();
		lModel.setContesto("MOTIVO_PROVVEDIMENTO");
		lModel.setCodiceAlt4("DIFFECUM");

		Vector<DecodificheModel> lDecodMotivoProvv = new Vector<DecodificheModel>();
		...
		return PG_LOAD_INSERISCI_DIFFERIMENTOPENA_CUMULO;
	}
}
