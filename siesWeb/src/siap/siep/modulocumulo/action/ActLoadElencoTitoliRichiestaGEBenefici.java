...
/**
 * Action per la load Elenco dei Titoli possibili oggetto della Richieste al GE di Applicazione Beneficio
 * dell'Indulto/Amnistia
 * 
 * @author Intersistemi Italia S.p.A.
 * 
 */
public class ActLoadElencoTitoliRichiestaGEBenefici extends ActionModuloCumulo
	...
	public String processRequest() throws Exception {
		...
		if (isRequestParameterNullObj(ICostantiRichiestePmInCumulo.CAMPO_CHECK_SCELTA_TITOLI) || "T"
				.equals(getRequestStringParameter(ICostantiRichiestePmInCumulo.CAMPO_CHECK_SCELTA_TITOLI))) {
			// La Query trova Titolo_Cumulato_Model e i suoi aggregati Procedimento_Cumulato e
			// Soggetto_Cumulato
			lListaTitoli = new Vector<>(lIstrCtrl.ExRicercaTitoliByIstruttoriaOrderBy(
			...
			lListaTitoli = new Vector<>(
					lIstrCtrl.ExRicercaTitoliByIstruttoriaDataReatoCumOrderBy(
							lIstruttoriaModel.getIdIstruttoriaCumulo(), lOrdinamento, lDataRea));
		...
		// Combo DPR per selezione della data
		Vector lVect = (Vector) DecodificheManager.getInstance().getDPR();

		Option lOptionDPR = new Option(lVect);
		if (!isRequestParameterNullObj(CAMPO_COD_DPR))
			lOptionDPR.setSelected(getRequestStringParameter(CAMPO_COD_DPR));
		else
			lOptionDPR.setSelected("-");

		setRequestAttribute("VectDPR", lVect);
		...
		return PG_ELENCO_TITOLI_RICG_GE_BENEFICI;
	}
