...
/**
 * Action per l'inserimento, la modifica e la cancellazione dei dati di un provedimento di Sospensione Misura
...
public class ActInserisciSospMisuraAlternativaCumulo extends ActionModuloCumulo
	...
	public String processRequest() throws F3BException {
		...
		if ("I".equals(lModalita)) {
			...
			lPage = IWebConstants.PG_MAIN + "?" + IWebConstants.ACTION_FIELD
					+ "=siap.siep.modulocumulo.action.ActDettaglioSospMisuraAlternativaCumulo" + "&"
			...
			lPage = IWebConstants.PG_MAIN + "?" + IWebConstants.ACTION_FIELD
					+ "=siap.siep.modulocumulo.action.ActRicercaSospMisureAlternativeCumulo" + "&"
					+ ICostantiTitoloCumulato.CAMPO_ID_TITOLO_CUMULATO + "="
					+ this.getRequestBigDecimalParameter(ICostantiTitoloCumulato.CAMPO_ID_TITOLO_CUMULATO)
					+ "&" + ICostantiIstruttoriaCumulo.CAMPO_ID_ISTRUTTORIA_CUMULO + "="
					+ this.getRequestBigDecimalParameter(
	...
	private ComputiCumuloModel getDatiComputo(String aTipoOper) throws F3BException {
		...
		if (!isRequestParameterNullObj(ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_INGRESSO_ISTITUTO)
				&& !getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_INGRESSO_ISTITUTO)
						.equals(""))
			lComputo.setDataIngressoIstituto(DateUtils.getDate(
					getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_ANNO_DATA_INGRESSO_ISTITUTO),
					getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_MESE_DATA_INGRESSO_ISTITUTO),
