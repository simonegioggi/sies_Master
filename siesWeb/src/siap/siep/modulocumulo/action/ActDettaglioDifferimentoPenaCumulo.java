...
/**
 * Action per la visualizzazione del Dettaglio del provvedimento di Differimento Pena (modulo cumulo)
 * @author 
 *
 */
public class ActDettaglioDifferimentoPenaCumulo  extends ActionModuloCumulo implements ICostantiComputiCumulo, ICostantiStatoEsecTitoloCumulato
{
  public String processRequest() throws F3BException 
  {
    ...
    ComputiCumuloModel lComputo = lStato.getListaComputi().firstElement();
    if (lComputo.getCodTDSCompetente() != null  )
	  {
		  UfficioModel lTDSCompetente = getUfficioByCodUfficio(lComputo.getCodTDSCompetente() );
	      setRequestAttribute("descrTDSCompetente", lTDSCompetente.getDescrComune());
	  }
    
    return PG_LOAD_DETTAGLIO_DIFFERIMENTOPENA_CUMULO;
  }
}
