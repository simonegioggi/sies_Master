...
/**
 * Action per la load dell'inserimento del provvedimento di Rimedi Risarcitori DL 2014/92 (Titolo Cumulato)
 * 
 * @author
 *
 */

public class ActLoadInserisciRimediRisarcitoriDL201492Cumulo extends ActionModuloCumulo implements
	...
	public String processRequest() throws F3BException {
		...
		if ("M".equals(lModalita)) {
			...
			if (Stato != null && Stato.getIdStatoEsecTitoloCumulato() != null) {
				if (Stato.getListaLiberazioniAnticipate() != null
					...
					if (LibAntCumMod.getCodTipoLicenza() != null) {
						TipoLic = LibAntCumMod.getCodTipoLicenza();
					}
				}
			} else {
			...
			setRequestAttribute("TipoLicenza", TipoLic);
		...
		try {
			...
			lEsito.setFilter(new String[] { "0028", "0390", "0391", "0392", "2758", "-" });
			if (Stato != null && Stato.getIdStatoEsecTitoloCumulato() != null
		...
		if (lModalita.equals("I")) {
			lpage = PG_LOAD_INSERISCI_RIMEDI_RISARCITORI_CUMULO;
		} else if (lModalita.equals("M")) {
			lpage = PG_LOAD_MODIFICA_RIMEDI_RISARCITORI_CUMULO;
		}
