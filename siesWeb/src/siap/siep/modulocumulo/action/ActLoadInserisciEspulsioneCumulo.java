...
/**
 * Action per la load inserisci e modifica dei Provvedimenti di Espulsione Cumulo
 *
 * @author Intersistemi S.p.A.
 *
 */
@SuppressWarnings({ "rawtypes", "unchecked" })
public class ActLoadInserisciEspulsioneCumulo extends ActionModuloCumulo
	...
	public String processRequest() throws F3BException {
		...
		// Preparazione codice Motivo Provvedimento per Espulsione cumulo.
		DecodificheModel lModel = new DecodificheModel();
		IDecodifiche lDecodifiche = SICOLookupRemote.getDecodificheRemote();
		lModel.setContesto("MOTIVO_PROVVEDIMENTO");
		lModel.setCodiceAlt4("ESPUCUM");
		...
		Option lOptionEsitoUDS = null;
		Option lOptionEsitoTDS = null;

		try {
			// Tipo Esito UDS
			lOptionEsitoUDS = new Option(DecodificheManager.getInstance().getEsitoProvvedimento());
			lOptionEsitoUDS.setFilter(new String[] { "0073", "0074", "-" });

			// Tipo EsitoTDS
			lOptionEsitoTDS = new Option(DecodificheManager.getInstance().getEsitoProvvedimento());
			lOptionEsitoTDS.setFilter(new String[] { "0019", "0025", "-" });
		} catch (Exception e) {
			siesLogger.debug("Esito non trovato");
		}
		...
		if ("M".equals(lModalita)) {
			...
			if (lComputo.getCodTDSCompetente() != null) {
				UfficioModel lTDSCompetente = getUfficioByCodUfficio(lComputo.getCodTDSCompetente());
				lComputo.setDescrTDSCompetente(lTDSCompetente.getDescrComune());
			}

			if (lStato.getCodEsito() != null && lOptionEsitoUDS.toString().contains(lStato.getCodEsito())) {
				lOptionEsitoUDS.setSelected(lStato.getCodEsito());
			}

			if (lStato.getCodEsito() != null && lOptionEsitoTDS.toString().contains(lStato.getCodEsito())) {
				lOptionEsitoUDS.setSelected(lStato.getCodEsito());
			}
		...
		setRequestAttribute("esitoUDS", "" + lOptionEsitoUDS);
		setRequestAttribute("esitoTDS", "" + lOptionEsitoTDS);

		// Imposta Modalità.
		setRequestAttribute("modalita", lModalita);

		// Restituisce la pagina di Inserimento dei Dati
		return PG_LOAD_INSERISCI_ESPULSIONE_CUMULO;
	}
}
