 ...
 * provedimento di Conc. Misura Alternativa disposto su uno dei titoli cumulati
      ...
      implements ICostantiStatoEsecTitoloCumulato,  ICostantiComputiCumulo
{
  ...
  public String processRequest() throws F3BException 
  {
    ...
    String lPage = "";
    //lPage = IWebConstants.PG_MAIN + "?" + IWebConstants.ACTION_FIELD + "=siap.siep.modulocumulo.action.ActRicercaConcMisureAlternativeCumulo" 
    ...
    if ("I".equals(lModalita)) {
      ...
      lPage = IWebConstants.PG_MAIN + "?" + IWebConstants.ACTION_FIELD + "=siap.siep.modulocumulo.action.ActDettaglioConcMisuraAlternativaCumulo" 
  ...
  private StatoEsecTitoloCumulatoModel getDatiProvvedimento(String aTipoOper) throws F3BException
  {
    ...
    lStaMod.setCodUfficioEmittente       ( getCodUfficioByCodTipoUfficioDescrComune (getRequestStringParameter(ICostantiTitoloCumulato.CAMPO_COD_TIPO_AUTORITA_EMITTENTE), getRequestStringParameter(ICostantiEvento.CAMPO_COD_LUOGO_EMITTENTE) ) ) ;
    lStaMod.setCodLuogoEmittente         ( getCodComuneByDescr(getRequestStringParameter (ICostantiEvento.CAMPO_COD_LUOGO_EMITTENTE) ).getCodComune()  );
  ...
  private ComputiCumuloModel getDatiComputo(String aTipoOper) throws F3BException
  {
    ...
    if (!getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_INIZIO_MISURA).equals(""))
    	lComputo.setDataInizioMisura(DateUtils.getDate(getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_ANNO_DATA_INIZIO_MISURA), getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_MESE_DATA_INIZIO_MISURA), getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_INIZIO_MISURA)));
		
	if (!isRequestParameterNullObj(ICostantiMisuraAlternativa.CAMPO_DESCR_LUOGO_PROVA))
		lComputo.setLuogoEsecMisura(getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_DESCR_LUOGO_PROVA));

	if (!isRequestParameterNullObj(ICostantiMisuraAlternativa.CAMPO_NUM_ANNI_MISURA))
		lComputo.setNumAnniMisura(getRequestBigDecimalParameter(ICostantiMisuraAlternativa.CAMPO_NUM_ANNI_MISURA));

	if (!isRequestParameterNullObj(ICostantiMisuraAlternativa.CAMPO_NUM_MESI_MISURA))
		lComputo.setNumMesiMisura(getRequestBigDecimalParameter(ICostantiMisuraAlternativa.CAMPO_NUM_MESI_MISURA));

	if (!isRequestParameterNullObj(ICostantiMisuraAlternativa.CAMPO_NUM_GIORNI_MISURA))
		lComputo.setNumGiorniMisura(getRequestBigDecimalParameter(ICostantiMisuraAlternativa.CAMPO_NUM_GIORNI_MISURA));

    if (!getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_FINE_MISURA).equals(""))
    	lComputo.setDataFineMisura(DateUtils.getDate(getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_ANNO_DATA_FINE_MISURA), getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_MESE_DATA_FINE_MISURA), getRequestStringParameter(ICostantiMisuraAlternativa.CAMPO_GIORNO_DATA_FINE_MISURA)));
