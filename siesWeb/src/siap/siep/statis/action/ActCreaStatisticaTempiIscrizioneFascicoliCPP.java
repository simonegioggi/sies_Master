...
/**
 * <p>
 * Title: ActCreaStatisticaTempiIscrizioneFascicoliCPP
 * </p>
 * <p>
 * Description: Classe Action per la statistica
 * </p>
 * <p>
 * Riepilogo Tempi Iscrizioni Fascicoli per la Classe VII
 * </p>
 * <p>
 * (C.P.P. - Conversione Pene Pecuniarie)
 * </p>
 */
public class ActCreaStatisticaTempiIscrizioneFascicoliCPP extends ActionSiap implements ICostantiStatis {

	@SuppressWarnings("rawtypes")
	public String processRequest() throws F3BException {

		// Lock
		LockModel lck = LockController.lockIfNotLocked(getServletContext(), "STATISTICA_TEMPI_ISCRIZIONE_CPP",
		...
		Vector<IspTempiModel> lRiepilogoIscr = new Vector<>();
		Vector<DettaglioTempiIscrizioneProcedimentiCPPModel> lDettaglioElenchi = new Vector<>();

		if (Tipo.compareTo("datadata") == 0) {
			lRiepilogoIscr = lCtrl.ExRicercaRiepilogoTempiIscrizioniCPP(annoIni, annoFin, ggmmIni, ggmmFin);
			lDettaglioElenchi = lCtrl.ExRicercaDettaglioTempiIscrizioneCPP(annoIni, annoFin, ggmmIni,
					ggmmFin);
			...
			lRiepilogoIscr = lCtrl.ExRicercaRiepilogoTempiIscrizioniCPPPerMese(annosemestre, ggmmIni, ggmmFin,
					Tipo, Quale_Trime_Seme);
			lDettaglioElenchi = lCtrl.ExRicercaDettaglioTempiIscrizioneCPPPerMese(annosemestre, ggmmIni,
					ggmmFin, Tipo, Quale_Trime_Seme);
		} else if (Tipo.compareTo("trimestre") == 0) {
			annotrimestre = Integer.parseInt(AnnoTri);
			lRiepilogoIscr = lCtrl.ExRicercaRiepilogoTempiIscrizioniCPPPerMese(annotrimestre, ggmmIni,
					ggmmFin, Tipo, Quale_Trime_Seme);
			lDettaglioElenchi = lCtrl.ExRicercaDettaglioTempiIscrizioneCPPPerMese(annotrimestre, ggmmIni,
					ggmmFin, Tipo, Quale_Trime_Seme);
		}
		...
		if (Tipo.compareTo("datadata") == 0 || Tipo.compareTo("annoanno") == 0) {
			lCtrl.ECreateXLSRiepilogoTempiIscrizioniCPP(lRiepilogoIscr, wb, getUfficioUtenteConnesso(),
					dataIni, dataFin, DescUffIntesta);
			lCtrl.ExCreateXLSDettaglioTempiIscrizioniCPP(lDettaglioElenchi, wb, getUfficioUtenteConnesso(),
					dataIni, dataFin, DescUffIntesta);
		} else if (Tipo.compareTo("semestre") == 0 || Tipo.compareTo("trimestre") == 0) {
			lCtrl.ECreateXLSRiepilogoTempiIscrizioniCPP_PerMese(lRiepilogoIscr, wb,
					getUfficioUtenteConnesso(), dataIni, dataFin, DescUffIntesta, Tipo, Quale_Trime_Seme);
			lCtrl.ExCreateXLSDettaglioTempiIscrizioniCPP_PerMese(lDettaglioElenchi, wb,
					getUfficioUtenteConnesso(), dataIni, dataFin, DescUffIntesta, Tipo, Quale_Trime_Seme);
		}
		...
		try {
			...
			throw new F3BException("ActCreaStatisticaTempiIscrizioneFascicoliCPP.processRequest: " + ioe);
		}
