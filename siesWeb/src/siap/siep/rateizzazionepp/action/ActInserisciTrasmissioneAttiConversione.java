...
public class ActInserisciTrasmissioneAttiConversione extends ActionSiap implements ICostantiRateizzazionePP {
	...
	public String processRequest() throws Exception {
		...
		enm.getEvento().setCodLuogoEmittente(getUfficioUtenteConnesso().getCodComune());
		enm.getEvento().setCodUfficioEmittente(getUfficioUtenteConnesso().getCodUfficio());
		enm.getEvento().setDataInserimento(DateUtils.getSysDate());
		enm.getEvento().setCodUfficioInserimento(getUfficioUtenteConnesso().getCodUfficio());
		...
		enm.getEvento().setCodTipoEvento("02"); // Tipo_Evento = 'Richiesta'
		enm.getEvento().setCodTipoProvvedimento("31"); // Tipo_Provvedimento = 'Trasmissione'
		// Motivo_Provvedimento = 'Atti all'UDS per la conversione pena pecuniaria'
		enm.getEvento().setCodMotivo("1310");
