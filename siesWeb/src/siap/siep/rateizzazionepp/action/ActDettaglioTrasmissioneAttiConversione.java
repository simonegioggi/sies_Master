...
/**
 * Classe Action per il dettaglio della Trasmissione Atti Conversione
 *
 * @author sgioggi
 * @since MEV_2023-33
 * @version 1.0
 */
public class ActDettaglioTrasmissioneAttiConversione extends ActionSiap implements ICostantiRateizzazionePP {
	...
	public String processRequest() throws F3BException {
		...
		listaRateizzazioni = irpp
				.exRicercaMancatiPagamentiUnicaSoluzione(enm.getEvento().getFasSieIdFascicoloSiep());
		...
		// NOTIFICA AL MAGISTRATO DI SORVEGLIANZA
		NotificaModel[] nmArray = enm.getNotifiche();
		setRequestAttribute("notificaAlMagistrato", nmArray[0]);

		// residenza
		IResidenza ir = SICOLookupRemote.getResidenzaRemote();
		Vector<ResidenzaAssociataModel> residenze = ir
				.ExRicercaResidenzeByIdFascicolo(enm.getEvento().getFasSieIdFascicoloSiep());
		...
		ResidenzaAssociataModel dram = ifs
				.ExRicercaDomicilioFascicoloSiepCorrente(enm.getEvento().getFasSieIdFascicoloSiep());
		...
		// valore di ritorno
		return PG_DETTAGLIO_TRASMISSIONE_ATTI_CONVERSIONE;
	}
