...
public class ActLoadInserisciRateizzazione extends ActionSiap implements ICostantiRateizzazionePP {
	...
	public String processRequest() throws F3BException {
		...
		if (!isRequestParameterNullEmptyObj("inserimento")
				&& getRequestStringParameter("inserimento").equals("true")) {
			...
			// Vector<RateizzazionePPModel> rateizzazioniLibere =
			// irpp.exRicercaRateizzazioniLibereByIdFasc(fsm.getIdFascicoloSiep());
			Vector<RateizzazionePPModel> rateizzazioni = irpp
					.exRicercaRateizzazioniByIdFasc(fsm.getIdFascicoloSiep());
		 ...
		 * Facicolo Vector<RateizzazionePPModel> rateizzazioni = new Vector<>(); IRateizzazionePP irpp =
		 * SIEPLookupRemote.getRateizzazionePPRemote(); rateizzazioni =
		 * irpp.exRicercaRateizzazioniByIdFasc(fsm.getIdFascicoloSiep());
		 * 
		 * boolean isEventoRateizzazioneAnnullato = false; if (!rateizzazioni.isEmpty()) {
		 * Iterator<RateizzazionePPModel> iterRPPM = rateizzazioni.iterator(); while (iterRPPM.hasNext()) {
		 ...
		 * ie.ExRicercaEventoByKey(idEvento); isEventoRateizzazioneAnnullato =
		 ...
		 * siesLogger.debug("Presenti già rate per il fascicolo. Carico il dettaglio"); String lPage =
		 * IWebConstants.PG_MAIN + "?" + IWebConstants.ACTION_FIELD +
		 * "=siap.siep.rateizzazionepp.action.ActLoadDettagloRateizzazione"; return lPage; } else { // Se
		 * fascicolo validato non posso consentire l'inserimento if ("S".equals(fsm.getFlagValidato())) {
		 ...
		 * else { setRequestAttribute("modalita", "I"); return PG_LOAD_INSERISCI_RATEIZZAZIONE_PP; } }
		 * 
		 */
	}
