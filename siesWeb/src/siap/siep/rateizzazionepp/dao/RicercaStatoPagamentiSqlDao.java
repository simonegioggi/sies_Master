...
public class RicercaStatoPagamentiSqlDao extends SIAPSqlDAO {
	...
	// MEV_2023-33
	public void getCountRicercaFascicoliPerStatoPagamento(FascicoloSiepModel aFasMod, String aTipoRicera) throws DAOException {
	...
	private String getSqlRicercaFascicoliPerStatoPagamento (FascicoloSiepModel aFasMod, String aTipoRicera) {
	    ...
	    if (aFasMod.getChiaveAnnoIniziale()!=null) {
	    	// Il vincolo sul progressivo (se presente) vale solo per l'anno indicato, per i successivi vanno presi tutti i progressivi
	    ...
	    if (aFasMod.getChiaveAnnoFinale()!=null) {
	    	// Il vincolo sul progressivo (se presente) vale solo per l'anno indicato, per i precedenti vanno presi tutti i progressivi
	    ...
	    if (aFasMod.getDataIscrizioneIniziale()!=null)
	    	lSql += "    AND FASCICOLO_SIEP.DATA_ISCRIZIONE >= TO_DATE ('"
	    			+DateUtils.getDateToString(aFasMod.getDataIscrizioneIniziale(), "dd/MM/yyyy")+ "','DD/MM/YYYY')";
	    if (aFasMod.getDataIscrizioneFinale()!=null)
	    	lSql += "    AND FASCICOLO_SIEP.DATA_ISCRIZIONE <= TO_DATE ('"
	    			+DateUtils.getDateToString(aFasMod.getDataIscrizioneFinale(), "dd/MM/yyyy")+ "','DD/MM/YYYY')";
