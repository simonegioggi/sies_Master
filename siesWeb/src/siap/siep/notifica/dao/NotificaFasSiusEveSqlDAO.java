...
/**
 * NotificaSqlDAO - Classe SqlDAO estensione della NotificaSqlDAO utilizzata per ricerche sulla tabella
 * Notifica in join con EVENTO; FASCICOLO_SIUS e SOGGETTO.
 *
 * @version 1.0
 */
public class NotificaFasSiusEveSqlDAO extends NotificaSqlDAO {
	...
	/**
	 * La funzione restituisce la select necessaria alla ricerca di notifiche collegate a Fascicoli SIUS.
	 */
	protected String getSqlQuery() {

		String lStatement = getSqlQuerySelect() + getSqlQuerySelectEstesa() + getSqlQueryFrom()
				+ getSqlQueryJoinEventoFascicoloSoggetto();

		return lStatement;
	}

	/*
	 * La funzione aggiunge alla select base le join necessarie ad includere le tabelle: FASCICOLO_SIUS,
	 * EVENTO, SOGGETTO .
	 */
	protected String getSqlQueryJoinEventoFascicoloSoggetto() {
		...
		lStatement += " join EVENTO EVE" + " on (EVE.ID_EVENTO = N.EVE_ID_EVENTO )"
				+ " join FASCICOLO_SIUS FASC" + " on (FASC.ID_FASCICOLO_SIUS = EVE.FAS_SIU_ID_FASCICOLO_SIUS)"
				+ " join SOGGETTO SOGG" + " on (SOGG.ID_SOGGETTO = FASC.SOG_ID_SOGGETTO)";

		return lStatement;
	}

	/*
	 * La funzione aggiunge alla select la join necessaria ad includere la tabella: AUTORITA_ESTERNA e le
	 * condizioni di filtro su questa.
	 */
	protected String getSqlQueryJoinAutoritaEsterna(AutoritaEsternaModel aAutorita, boolean aEsclusoAutorita,
			boolean aEsclusoSedeAutorita) {
		...
		if (aAutorita != null) {
			if (aAutorita.getCodTipoAutorita() != null
					&& aAutorita.getCodTipoAutorita().trim().length() > 0) {
				if (aEsclusoAutorita) {
					// Se si esclude dai destinatari una autorità specifica saranno esclusi anche gli Istituti
					// di detenzione
	...
	/*
	 * La funzione aggiunge alla select la join necessaria ad includere la tabella: UFFICIO e le condizioni di
	 * filtro su questa.
	 */
	protected String getSqlQueryJoinUfficio(UfficioModel aUfficio, boolean aEsclusoUfficio,
			boolean aEsclusoSedeUfficio) {
		...
		lStatement += " join UFFICIO UFF" + " on (UFF.COD_UFFICIO = N.UFF_COD_UFFICIO ";
		if (aUfficio != null) {
	...
	/*
	 * La funzione estende la select base aggiungendo ai campi base già previsti quelli relativi alle tabelle:
	...
	public GenericModel getModel() throws DAOException {
		...
		// si ricava la descrizione del TipoProvvedimento dalle Decodifiche in memoria per risparmiare una
		// JOIN
		lEvento.setDescrTipoProvvedimento(DecodificheUtils.getDescbyCode(
		...
		try {
			...
			// una JOIN
			lEvento.setDescrEsito(DecodificheUtils.getDescbyCode(
					DecodificheManager.getInstance().getEsitoProvvedimento(), lEvento.getCodEsito()));
	...
	private String decodificaCodMotivo(String aCodMotivo) throws DAOException {
		...
		if (aCodMotivo != null) {
			...
			lDecodifica = DecodificheUtils
					.getDescbyCode(DecodificheManager.getInstance().getMotivoProvvedimento(), aCodMotivo);
			// Poi come OGGETTO_PROCEDIMENTO
			if (!(lDecodifica.trim().length() > 0))
				lDecodifica = DecodificheUtils
						.getDescbyCode(DecodificheManager.getInstance().getOggettoProcedimento(), aCodMotivo);
		}
		return lDecodifica;
	}
	...
	public String setCondizione(RicercaNotificheSiusModel aModel, Date aDataInizio, Date aDataFine) {
		...
		if (aModel.getEvento() != null) {
			if (aModel.getEvento().getCodTipoEvento() != null
					&& aModel.getEvento().getCodTipoEvento().trim().length() > 0) {
				lAppoggio = " EVE.COD_TIPO_EVENTO = '" + aModel.getEvento().getCodTipoEvento() + "'";
				lCondizioni += setAND(lAppoggio);
			}
			if (aModel.getEvento().getCodEsito() != null
					&& aModel.getEvento().getCodEsito().trim().length() > 0) {
		...
		if (aDataInizio != null) {
			lAppoggio = " TO_CHAR(N.DATA_INSERIMENTO,'YYYYMMDD') >='"
					+ DateUtils.getDateToString(aDataInizio, "yyyyMMdd") + "'";
			lCondizioni += setAND(lAppoggio);
		}
		if (aDataFine != null) {
			lAppoggio = " TO_CHAR(N.DATA_INSERIMENTO,'YYYYMMDD') <='"
					+ DateUtils.getDateToString(aDataFine, "yyyyMMdd") + "'";
			lCondizioni += setAND(lAppoggio);
		}
	...
	/**
	 * La funzione predispone la ricerca di Notifiche-FascicoloSius-Provvedimento-Soggetto. Il Model parametro
	 * di ingresso contiene il filtro di ricerca da realizzare. Viene richiamata la f.ne getSqlQuery() per
	 * preparare la select, se esistono delle condizioni sulle Autorità Esterne destinatarie delle notifiche
	 * queste vengono preparate tramite la f.ne getSqlQueryJoinAutoritaEsterna, infine tramite la f.ne
	 * setCondizione() vengono preparate le condizioni di filtro su tipo provvedimenti e notifiche.
	 *
	 * @param aModel
	 *            : RicercaNotificheSiusModel
	 * @throws DAOException
	 */
	public void ricercaNotificheFasSiusProvSog(RicercaNotificheSiusModel aModel) throws DAOException {
		...
		if (aModel.getAutoritaEsterna() != null)
			lStatement += getSqlQueryJoinAutoritaEsterna(aModel.getAutoritaEsterna(), aModel.getNoUNEP(),
					aModel.getNoSedeUNEP());
		...
		if (aModel.getUfficio() != null)
			lStatement += getSqlQueryJoinUfficio(aModel.getUfficio(), aModel.getNoUff(),
					aModel.getNoSedeUff());
