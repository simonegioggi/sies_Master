...
/**
 * <p>
 * Title: ActAggiornaAvvenutaNotificaDifensore
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class ActAggiornaAvvenutaNotificaDifensore extends ActionSiap implements ICostantiNotifica {
	...
	public String processRequest() throws F3BException {
		...
		String[] lIdNotificaIns = null;
		if (!isRequestParameterNullObj(ICostantiNotifica.CAMPO_ID_NOTIFICA_INS)) {
			lIdNotificaIns = this.getRequestStringParameters(ICostantiNotifica.CAMPO_ID_NOTIFICA_INS);
		}

		// int lLungArrayAbilita = lArrayAbilita.length;
		int lLungArrayAbilita = lGiornoNot.length;
		...
		for (int i = 0; i < lLungArrayAbilita; i++) {
			if (lGiornoNot[i] != null && !lGiornoNot[i].equals("")) {
				...
				lNotMod.setIdNotifica(new BigDecimal(lIdNotificaIns[i]));
				...
				if (lSedeAutoritaDelegata != null && lSedeAutoritaDelegata[i] != null
						&& !lSedeAutoritaDelegata[i].equals("")) {
		...
		if (lIdNotifica != null && lIdNotifica.length > 0) {
			...
			for (int i = 0; i < lIdNotifica.length; i++) {
				...
				if (!isTrovato && (lGiornoNot[i] == null || lGiornoNot[i].equals(""))) {
					...
					lNotModAgg.setEveIdEvento(this
		...
		lEveVectUno = lCtrlUno.ExAggiornaAvvenutaNotifica(lNotModArray, lFasc.getIdFascicoloSiep(), false);
		setRequestAttribute("notifiche", lEveVectUno);

		String lPage = "";
		lPage = IWebConstants.PG_MAIN + "?" + IWebConstants.ACTION_FIELD
				+ "=siap.siep.notifica.action.ActLoadRicercaOEDifensore&" + ICostantiEvento.CAMPO_ID_EVENTO
