...
/**
 * Title: SanzioneSostResiduaSqlDAO
 * Description: Classe SqlDAO che rappresenta la tabella SanzioneSostResidua
 *
 * @version 1.0
 */
public class SanzioneSostResiduaSqlDAO extends SqlDAO {
	 ...
	 * @throws DAOException
	 ****************************************************************************/
	public void ricercaSanzioneSostResiduaPaged(SanzioneSostResiduaModel aModel, int aPage)
			throws DAOException {
	...
	/**
	 * Imposta la select per recuperare l'elenco delle SS Ordinate per data inserimento desc
	 *
	 * @param aIdFascioloSiep
	 * @param aFlagValidata
	 * 	Se 'S' recupera solo quelle validate, se 'N' solo quelle non validate se null recupera tutte le SS
	 * 
	 * @throws DAOException
	 */
	public void ricercaUltimaSanzioneSostResiduaByIdFasc(BigDecimal aIdFascioloSiep, String aFlagValidata)
			throws DAOException {
	...
	protected String getSqlQuery() {
		...
		lStatement += " SELECT " + "SSR.ID_SANZIONE_SOST_RESIDUA, " + "SSR.FAS_SIE_ID_FASCICOLO_SIEP, "
				+ "SSR.EVE_ID_EVENTO, " + "SSR.PEN_RES_ID_PENA_RESIDUA, "
				+ "SSR.COD_TIPO_SANZIONE, COD_TIPO_SANZIONE.RV_MEANING DESCR_TIPO_SANZIONE, "
				+ "SSR.DATA_INIZIO, " + "SSR.DATA_FINE_PRESUNTA, " + "SSR.DATA_FINE, " + "SSR.NUM_ANNI, "
				+ "SSR.NUM_MESI, " + "SSR.NUM_GIORNI, " + "SSR.SANZIONE_PECUNIARIA_MULTA, "
				+ "SSR.SANZIONE_PECUNIARIA_AMMENDA, " + "SSR.COD_OPERATORE_INSERIMENTO, "
				+ "SSR.DATA_INSERIMENTO, " + "SSR.COD_UFFICIO_INSERIMENTO, "
				+ "SSR.COD_OPERATORE_AGGIORNAMENTO, " + "SSR.DATA_AGGIORNAMENTO, "
				+ "SSR.COD_UFFICIO_AGGIORNAMENTO, " + "PR.FLAG_VALIDATO ";
	...
	public String setCondizioni(SanzioneSostResiduaModel aModel) {
		...
		if (aModel.getDataInizio() != null) {
			lCondizioni += " and to_char(SSR.DATA_INIZIO,'dd/MM/yyyy') = '"
					+ DateUtils.getDateToString(aModel.getDataInizio(), "dd/MM/yyyy") + "' ";
		}
		if (aModel.getDataFinePresunta() != null) {
			lCondizioni += " and to_char(SSR.DATA_FINE_PRESUNTA,'dd/MM/yyyy') = '"
					+ DateUtils.getDateToString(aModel.getDataFinePresunta(), "dd/MM/yyyy") + "' ";
		}
		if (aModel.getDataFine() != null) {
			lCondizioni += " and to_char(SSR.DATA_FINE,'dd/MM/yyyy') = '"
		...
		if (aModel.getDataInserimento() != null) {
			lCondizioni += " and to_char(SSR.DATA_INSERIMENTO,'dd/MM/yyyy') = '"
		...
		if (aModel.getDataAggiornamento() != null) {
			lCondizioni += " and to_char(SSR.DATA_AGGIORNAMENTO,'dd/MM/yyyy') = '"
