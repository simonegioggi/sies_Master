 ...
 *
 * - deve obbligare alla selezione dell'OI - deve verificare che il tipo di rateizzazione sia R - deve
 * verificare che per le rate con origr > 1 siano stati generati i bollettini
 *
 *
 * @author d.fiorletta
 * @since MEV_2023-33
...
public class ActLoadModificaNotaTrasmissione extends ActionSiap implements ICostantiSanzioneSostitutiva {
	...
	public String processRequest() throws Exception {
		...
		siesLogger.debug(
				"ActLoadInserisciNotaTrasmissione: Sono in modifica della nota con ID " + idNotaTrasmissione);

		// Recupero la nota di trasmissione per precaricare in form Data emissione, data trasmissione e
		// notifiche
		...
		EventoNotificaModel lEveNotMod = lCtrlEvento
				.ExRicercaEventoNotificaByKey(lNotaTrasmissione.getEvento().getEveIdEvento());
		setRequestAttribute("ordineIngiunzione", lEveNotMod);

		// Recupero le rate
		IRateizzazionePP irpp = SIEPLookupRemote.getRateizzazionePPRemote();
		Vector<RateizzazionePPModel> listaRate = irpp
				.exRicercaRateizzazioniBollettiniByIdEvento(lEveNotMod.getEvento().getIdEvento());
