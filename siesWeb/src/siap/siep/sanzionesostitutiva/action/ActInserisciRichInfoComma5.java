...
public class ActInserisciRichInfoComma5 extends ActionSiap implements ICostantiNotifica {
	...
	public String processRequest() throws F3BException {
		...
		Vector<RinnovoModel> lRinnovi = new Vector<>();

		// RINNOVO
		if (!this.isRequestParameterNullObj("primoAvvocato")
				&& this.getRequestStringParameter("primoAvvocato").equals("S")) {
			siesLogger.debug("Primo avvocato con idNotifica = "
					+ this.getRequestBigDecimalParameter("idPrimaNotifica"));
		...
		if (!this.isRequestParameterNullObj("secondoAvvocato")
			...
			siesLogger.debug("Secondo avvocato con idNotifica = "
					+ this.getRequestBigDecimalParameter("idSecondaNotifica"));
		...
		if (this.getRequestStringParameter("altri").equals("S")) {
			siesLogger.debug("Richiesta ad altri idNotifica = "
					+ this.getRequestBigDecimalParameter("idAltraNotifica"));
		...
		String lPage = IWebConstants.PG_MAIN + "?" + IWebConstants.ACTION_FIELD
				+ "=siap.siep.sanzionesostitutiva.action.ActLoadDettaglioRichInfoComma5";
