...
/**
 * Title: ActInserisciRinnovoRicercheOIPP
 * Description: Classe Action per l'inserimento del RINNOVO per omesse notifiche per gli ordini di ingiunzione
 * al pagamento
 *
 * @version 1.0
 * @since MEV_2023-33
	...
	public String processRequest() throws F3BException {
		...
		if (this.getRequestStringParameter("Notifica").equals("FP")) // FORZE DI POLIZIA
		{
			if (lVermodel == null || lVermodel.getIdVerbale() == null)
				throw new SIEPException(SIEPException.USER_MESSAGE,
						"Non Esiste Nessun Verbale Vane Ricerche Associato all'ordine di ingiunzione.");
			...
			if (this.getRequestStringParameter("Rinnovo").equals("RA")) // RINNOVO ALTRA AUTORITA'
			{
				...
				lComMod = new ComuneModel(getCodComuneByDescr(
						getRequestStringParameter(ICostantiRinnovo.CAMPO_COD_LUOGO_RINNOVO)));
		...
		} else if (this.getRequestStringParameter("Notifica").equals("UG")) // UFFICIALI GIUDIZIARI
		{
			...
			lVerMod.setDataEmissione(getRequestDateParameter(ICostantiRinnovo.CAMPO_ANNO_DATA_RELATA,
					ICostantiRinnovo.CAMPO_MESE_DATA_RELATA, ICostantiRinnovo.CAMPO_GIORNO_DATA_RELATA));
			lVerMod.setCodTipoUfficioFirmatario("22");
			lComMod = new ComuneModel(getCodComuneByDescr(
					getRequestStringParameter(ICostantiVerbale.CAMPO_COD_LUOGO_UFFICIO_FIRMATARIO_UG)));
			...
			if (this.getRequestStringParameter("Ufficiali").equals("RN")) // RINNOVO NOTIFICA'
			{
				...
				lComMod = new ComuneModel(getCodComuneByDescr(
						getRequestStringParameter(ICostantiRinnovo.CAMPO_COD_LUOGO_RINNOVO_UG)));
				lRinMod.setCodLuogoRinnovo(lComMod.getCodComune());
			...
			} else if (this.getRequestStringParameter("Ufficiali").equals("AR")) // ATTIVAZIONE RICERCA
			{
				...
				lRinMod.setCodTipoAutoritaRinnovo(
						this.getRequestStringParameter(ICostantiRinnovo.CAMPO_COD_TIPO_AUTORITA_RINNOVO_UG));
				...
				lComMod = new ComuneModel(getCodComuneByDescr(
						getRequestStringParameter(ICostantiRinnovo.CAMPO_COD_LUOGO_RINNOVO_UG_AR)));
				lRinMod.setCodLuogoRinnovo(lComMod.getCodComune());
				lRinMod.setNuovoLuogoNotifica(
						this.getRequestStringParameter(ICostantiRinnovo.CAMPO_LUOGO_NUOVA_NOTIFICA_UG_AR));
			}
		}
		...
		lPage = IWebConstants.PG_MAIN + "?" + IWebConstants.ACTION_FIELD
				+ "=siap.siep.sanzionesostitutiva.action.ActLoadDettaglioRinnovoRicercheOIPP&"
