...
public class ActRicercaEstesaSentenza extends ActionSiap implements ICostantiSiepJMS {
	...
	public String processRequest() throws Exception {
		...
		if (!isRequestParameterNullObj(ICostantiSentenza.CAMPO_ANNO_REGISTRO_GENERALE)) {
			...
			if (getRequestStringParameter("valore").equals("capsm")) {
				lSmod.setAnnoRegeCapsm(
						getRequestBigDecimalParameter(ICostantiSentenza.CAMPO_NUMERO_REGISTRO_GENERALE));
			}
		}
		 ...
		 * lMessage.setCodTipoMessaggio(RICHIESTA_RICERCA); lMessage.setCodTipoOperazione(RICERCA_FASCICOLO);
		 ...
		 *
		 *
		 * SIAPSender lSender = new SIAPSender(); lSender.send(lMessage);
		 */
