...
public class PenaResiduaUtil
{
  ...
  /**
   * Calcola il residuo pena a partire da una nuova data inizio. Utilizzata nel 
   * caso di interruzione specificando la data interruzione e fornendo in input
   * il model della pena residua che si va ad interrompere.
   * Il metodo calcola il residuo a partire dalla aNuovaDataInizio+1 alla data fine
   * pena prevista.
   * ????
   * 
   * @param aNuovaDataInizio - Data nuovo inizio pena
   * @param aPenaResidua - model con quantum e 
   * @param aEscludiDiesequo - Indica se conteggiare o meno il giorno di inizio
   *                           nel calcolo dei nuovi quantum
  ...
  public static PenaResiduaModel calcolaPenaNuovaDataInizio(Date aNuovaDataInizio,
                                                            ...
                                                            boolean aEscludiDiesequo) throws Exception

  {
      //Controlli sull'esistenza delle date
      if(aNuovaDataInizio == null)
        throw new SIEPException(SIEPException.USER_MESSAGE, "La nuova data inizio pena non può essere vuota. Impossibile procedere.");

      if(aPenaResidua.getDataInizio() == null)
        throw new SIEPException(SIEPException.USER_MESSAGE, "La pena residua non ha la data inizio pena. Impossibile procedere.");

      if(aPenaResidua.getDataFine() == null)
        throw new SIEPException(SIEPException.USER_MESSAGE, "La pena residua non ha la data fine pena. Impossibile procedere.");
...
//ARRESTO
      ...
      if (!lCalUtil.isZero(lCalArresto))
      {
        ...
        if(lDataInizioArresto == null)
          throw new SIEPException(SIEPException.USER_MESSAGE, "La pena residua non ha la data inizio arresto. Impossibile procedere.");
...
// RECLUSIONE = 0
    ...
    /***************************************************************************
     * Questo metodo ricalcola il periodo di PENA_RESIDUA utilizzando la nuova
     * data fine.
     * Vengono ricalcolati: gg, mm, aa, DataFineReclusione, DataInizioArresto, DataFine 
     * aggiungendo o sottraendo opportunamente i gg da Reclusione e Arresto<br><br>
     *
     * n.b. Vengono aggiornati preferibilmente se possibile i quantum dell'arresto
     *      e solo se non possibile vengono aggiornati i dati della reclusione
     * 
     * n.b. devono essere not null NuovaDataFine e data_inizio e data_fine del 
     *      model penaResidua
     * @param Date - nuova data fine
     * @param PenaResiduaModel - model con i dati di partenza
     * @param boolean - diesaquo
    ...
    public static PenaResiduaModel calcolaPenaNuovaDataFine( Date aNuovaDataFine,
                                                             PenaResiduaModel aPenaResidua,
                                                             boolean aEscludiDiesequo )
      throws Exception
    {
      //Controlli sull'esistenza delle date
      if(aNuovaDataFine == null)
        throw new SIEPException(SIEPException.USER_MESSAGE, "La nuova data fine pena non può essere vuota. Impossibile procedere.");
...
// ARRESTO
      ...
      //      state caricate      
      /**
       * TODO Attenzione al caso di reclusione+arresto e nuovaDataFine=dataInizioArresto
       * in quasto caso viene azzerato il quantum di Arresti, ma non viene
       * cancellata la data inizio arresto che resta valorizzata
       */
      lPenaResidua.setNumAnniReclusione   (new BigDecimal(lCalReclusioneNew.getNumAnni()));
      lPenaResidua.setNumMesiReclusione   (new BigDecimal(lCalReclusioneNew.getNumMesi()));
      lPenaResidua.setNumGiorniReclusione (new BigDecimal(lCalReclusioneNew.getNumGiorni()));
