...
public class ActRicercaMisuraCautelare extends ActionSiap implements ICostantiMisuraCautelare {
	...
	public String processRequest() throws Exception {
		...
		// inizio MEV_10_S3
		// Quando 2 o pi� misure cautelari computabili sono continuative il sistema
		...
		while (lItMisure.hasNext()) {
			...
			if (lMisCautModel.getFlagComputabile() != null && !lMisCautModel.getFlagComputabile().equals("")
				...
				// se la differenza tra la data fine e la data inizio � maggiore
				...
				if (giorni > 1) {
					...
					// MisCautContinuativa uguale ad 'N' solo se quest'ultimo � diverso
					// da 'S'
					MisuraCautelareModel lMisCaut = (MisuraCautelareModel) lMisureApp.get(inc - 1);
					...
					// se la differenza tra la data fine e la data inizio �
					// uguale a 0 oppure uguale a 1 le misure cautelari
					// sono continuative
				} else if (giorni == 0 || giorni == 1) {
					// misure cautelari continuative

					// Ticket#20200715018 - la marco come ultima della serie SU. Verr� eventualmente setta a S
					// se
					// il successivo record � in continuazione con il corrente altrimenti resta SU
				 ...
				 * il flag MisCautContinuativa uguale ad 'N' // perch� il totale dei giorni deve essere
				...
				// perch� il totale dei giorni non deve essere visibile
				lMisCautModel.setMisCautContinuativa("S");
		...
		// Se nell�elenco delle misure cautelari sono presenti 2 o pi� misure cautelari computabili
		// il sistema nella riga dell�ultima misura cautelare computabile continuativa
		// dovr� mostrare il totale espresso in Anni, Mesi e Giorni delle suddette
		...
		for (int j = 0; j < lMisureApp.size(); j++) {
			...
			if ((lMisCautAppModel.getMisCautContinuativa().equalsIgnoreCase("S")
				...
				// Ticket#20200715018 - Informo il record successivo che questo non � in continuazione con lui
		...
		// II) se la posizione giuridica ="07" && COD_MASCHERA="L" non � presente altra causa la
		// DescrPosizioneGiuridica="Libero"
		// else la posizione giuridica ="07" ==> � presente altra causa la DescrPosizioneGiuridica si
		// considera
		...
		if (lPos != null && lPos.getIdPosizioneGiuridica() != null
			...
			// � gi� presente la descrizione giusta
