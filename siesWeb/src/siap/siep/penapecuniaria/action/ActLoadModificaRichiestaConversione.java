...
/**
 * ActLoadModificaRichiestaConversione - Classe Action per la load Modifica di RichiestaConversione
 *
 * @version 1.0
 */
public class ActLoadModificaRichiestaConversione extends ActionSiap implements ICostantiPenaPecuniaria {
	...
	public String processRequest() throws F3BException {
		...
		// 29/09/2015 Nuova gestione Richieste di conversione: si è slegata la richiesta di conversione
		// all'evento corrente di R.C.
		// Per stabilire se le Richieste di Conversione contemplano provvedimenti della Sorveglianza,
		// bisogna cercare Eventi collegati tramite FAS_SIU_ID_FASCICOLO_SIUS, che abbiano
		// COD_TIPO_PROVVEDIMENTO = 02 /03, COD_MOTIVO = 2470 / 2471 e FLAG_DOCUMENTO_REGISTRATO <> "A".
		...
		eveMod = lCtrl.ExRicercaEventoSorvDiRichiestaConversione(lIdRicConv, eveMod, lCodTipoProvv,
				lCodMotivo, lCodEsito);
		// 26/11/2014 Per le Richieste di conversione con provvedimenti della Sorveglianza, la modifica non
		// è consentita.
		// IEvento lCtrlEve = SICOLookupRemote.getEventoRemote();
		// EventoModel eveMod = lCtrlEve.ExRicercaEventoByKey(lRicMod.getEveIdEvento());
		if (eveMod != null && (eveMod.getCodTipoProvvedimento().compareTo("02") == 0
				|| eveMod.getCodTipoProvvedimento().compareTo("03") == 0))
			throw new SIEPException(SIEPException.USER_MESSAGE,
					"Non è consentito modificare le Richieste di conversione in presenza di provvedimenti della Sorveglianza!");

		// autorità per la conversione.
		Option lOption = new Option(DecodificheManager.getInstance().getTipoAutorita(),
				lRicMod.getCodTipoAutoritaEmittente());
