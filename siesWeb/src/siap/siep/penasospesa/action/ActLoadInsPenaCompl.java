...
/**
 * <p>
 * Title: ActLoadInserisciPenaComplessiva
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class ActLoadInsPenaCompl extends ActionSiap implements ICostantiPenaSospesa { // ICostantiPenaComplessiva
	...
	public String processRequest() throws Exception {
		...
		// LogF3B.getLogger()
		siesLogger.debug("INIZIO ActLoadInsPenaCompl ");
		...
		/*
		 * if(lFasMod != null) { if("01".equals(lFasMod.getCodStatoFascicolo())) // COS_STATO_FASCICOLO =
		 * ARCHIVIATO/DEFINITO { RedirectTo lRedirigi = new RedirectTo();
		 * 
		 ...
		 * lFasMod.getIdFascicoloSiep().toString()); setRequestAttribute( IWebConstants.GOTO_PAGE, "" +
		 * lRedirigi );
		 * 
		 * throw new F3BException( F3BException.USER_MESSAGE, "Il fascicolo risulta archiviato" ); }
		 * 
		 * if(lFasMod.getFlagValidato() != null && lFasMod.getFlagValidato().equals("S")) { RedirectTo
		 * lRedirigi = new RedirectTo();
		 * 
		 * lRedirigi.setPage( IWebConstants.PG_MAIN );
		 * lRedirigi.setAction("siap.siep.fascicolo.action.ActLoadDettaglioFascicolo" );
		 * lRedirigi.setParameter(ICostantiFascicoloSiep.CAMPO_ID_FASCICOLO_SIEP,
		 ...
		 * lRedirigi );
		 * 
		 * throw new F3BException( F3BException.USER_MESSAGE, "Il fascicolo risulta validato" ); } } else
	...
	protected String preparazioneForm() {
		...
		return IWebConstants.ROOT_DIR + "files/siap/siep/penasospesa/LoadInserisciPenaComplessiva.jsp";
	}
