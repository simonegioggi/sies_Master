...
public class CompetenzaController extends SiapController implements ICompetenza {
	...
	public Vector ExRicercaCompetenza(CompetenzaModel aCompetenza) throws F3BException {
		...
		try {
			...
			lComDao.start();
			while (lComDao.next()) {
				lCompetenzi.add(lComDao.getModel());
			}
			lComDao.stop();
		} catch (DAOException daoEx) {
			siesLogger.error("DAOException", daoEx);
	...
	public CompetenzaModel ExRicercaCompetenzaById(BigDecimal aIdCompetenza) throws F3BException {
		...
		try {
			...
			siesLogger.error("DAOException", daoEx);
			throw new F3BException(
					"CompetenzaController.ExRicercaCompetenzaById: Non posso leggere : " + daoEx);
		} finally {
	 ...
	 * @throws F3BException
	 ****************************************************************************/
	public Vector<CompetenzaModel> ExRicercaCompetenzaPaged(CompetenzaModel aCompetenza, int aPage)
			throws F3BException {

		Connection lConn = null;
		Vector<CompetenzaModel> lCompetenzi = new Vector<>();
		CompetenzaSqlDAO lCompetenzaSqlDao = null;
		...
		try {
			...
			siesLogger.error("DAOException", daoEx);
			throw new F3BException(
					"CompetenzaController.ExRicercaCompetenzaPaged: Non posso leggere : " + daoEx);
		} finally {
			cleanup(lCompetenzaSqlDao);
			cleanup(lConn);
		}
	...
	public Vector ExRicercaCompetenzaByIdFascicoloSiep(BigDecimal aIdFascicoloSiep) throws F3BException {
		...
		try {
			...
			throw new F3BException(
					"CompetenzaController.ExRicercaCompetenzaByIdFascicoloSiep: Non posso leggere : "
							+ daoEx);
		} catch (Exception ex) {
			// [FT] - 03/08/2016 - MAC_LOG - Utilizzo la variabile di istanza siesLogger al posto di
			// LogF3B.getLogger()
				...
				for (int i = 0; i < aListaCompetenze.size(); i++) {
					...
					try {
						...
						if (daoEx.UNIQUE_CONSTRAINT_VIOLATED) {
							siesLogger.warn(
									"Competenza gia' presente id = " + lCompetenzaModel.getIdCompetenza());
							lCodEsito = "00001";
						} else {
							throw daoEx;
						}
	...
	public void ExModificaEvento_e_Competenza(EventoModel aEvento, CompetenzaModel aCompetenza)
		...
		try {
			...
			siesLogger.error("DAOException", daoEx);
			throw new F3BException(
					"CompetenzaController.ExModificaEvento_e_Competenza: Non posso inserire: " + daoEx);
		} finally {
			cleanup(lEveDao);
			cleanup(lComDao);
			cleanup(lConn);
	...
	public CompetenzaModel ExRicercaCompetenzaByIdMessaggioRichiesta(BigDecimal aIdMessaggio)
		...
		try {
			...
			siesLogger.error("DAOException", daoEx);
			throw new F3BException(
					"CompetenzaController.ExRicercaCompetenzaByIdMessaggioRichiesta: Non posso leggere : "
							+ daoEx);
		} finally {
			cleanup(lCompetenzaSqlDao);
			cleanup(lConn);
	...
	/*****************************************************************************
	 * Effettua l'inserimento di Competenza e EventoNotifica a partire dai dati contenuti nel Model; Usato per
	 * Inserimeto dei provvedimenti di: - atti per competenza (per emissione provvedimento cumulo) (0340) -
	 * atti per competenza (per emissione provvedimento cumulo) - Seguito Atti (0740) - Rigetto Richiesta atti
	 * per competenza (emissione Cumulo) (0599)
	 *
	 * @param aCompetenza
	 *            Model, aEventoNotifica Model con i dati da inserire
	 * @return il model Competenza con i dati inseriti e l'aggiunta dell'id del record inserito
	 * @throws F3BException
	 ****************************************************************************/
	public CompetenzaModel ExInserisciCompetenzaEventoNotifica(CompetenzaModel aCompetenza,
			EventoNotificaModel aEveNotMod) throws F3BException {
		...
		try {
			...
			lConn = getDBTransaction();
			IEvento lCtrl = SICOLookupRemote.getEventoRemote();
			lEveRet = lCtrl.ExInserisciEventoNotifica(aEveNotMod, lConn);
			...
			throw new F3BException(
					"CompetenzaController.ExInserisciCompetenzaEventoNotifica: Non posso inserire: " + ex);
		} finally {
			cleanup(lComDao);
			cleanup(lConn);
		}
	...
	/**
	 * MEV_39 Ricerca tutti i record COMPETENZA associati ad eventi di tipo Trasmessione Atti per competenza
	 * per emissione provvedimento di cumulo legati al fascicolo passato in input
	 */
	public Vector ExRicercaCompetenzePerCumulo(BigDecimal aIdFascicoloSiep, boolean stessoDistr)
			throws F3BException {
		...
		try {
			...
			while (lComSqlDao.next())
				lCompetenze.add(lComSqlDao.getModelCompCumulo());
