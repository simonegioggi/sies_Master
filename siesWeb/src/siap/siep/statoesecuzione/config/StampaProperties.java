package siap.siep.statoesecuzione.config;
 ...
 *
 */
public class StampaProperties extends PropertiesMgr {

	// [FT] - 03/08/2016 - MAC_LOG - Dichiaro un'istanza di Logger per SIESLog
	private static Logger siesLogger = Logger.getLogger(LogF3B.SIES_LOG);

	private static StampaProperties mStampaProperties = null;
//	private Properties mConnectionString;

	protected StampaProperties() {
	}
	 ...
	 */
	public static StampaProperties getInstance() {
		try {
			if (mStampaProperties == null) {
				mStampaProperties = new StampaProperties();
				mStampaProperties.initialize();
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}

		return mStampaProperties;
	}
	...
	private void initialize() throws F3BException {
		...
		String lNameFile = lPathProp + System.getProperty("file.separator") + "stampa.properties";
		mStampaProperties.setFileProps(lNameFile);
		mStampaProperties.init();
	}
	...
	public String getProperty(String aName) throws F3BException {
		...
		if (lProperty == null || (lProperty != null && lProperty.length() == 0))
			...
			// LogF3B.getLogger()
			siesLogger.info("\n Proprietà <" + aName + "> non trovata nel file stampa.properties!");
