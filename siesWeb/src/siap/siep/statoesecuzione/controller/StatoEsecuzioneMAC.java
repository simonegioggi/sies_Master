 ...
 *
 */
public class StatoEsecuzioneMAC extends StatoEsecuzioneElement 
{
	// [FT] - 03/08/2016 - MAC_LOG - Dichiaro un'istanza di Logger per SIESLog
	private static Logger siesLogger = Logger.getLogger(LogF3B.SIES_LOG);

	public StatoEsecuzioneMAC()
	{
	}
	
	public StatoEsecuzioneMAC(StatoEsecuzioneElement aStat){	
		        super(aStat);
			}
		...
		try{
			...
			if(aEvento.getEveIdEvento()!=null && mHashEventiRiferimento.containsKey(aEvento.getEveIdEvento()))
			{
				...
				if(mHashMisure.containsKey(lEveSorv.getIdEvento()))
				{
					...
					if(lMisure.getDataInizioMisura()!=null){
					lMisuraDate = mCostanti.getProperty("MISURA_INIZIO"); 
