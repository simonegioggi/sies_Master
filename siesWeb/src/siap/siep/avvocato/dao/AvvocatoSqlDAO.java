...
public class AvvocatoSqlDAO extends SIAPSqlDAO
{
  ...
  /*****************************************************************************
   * Imposta le condizioni di ricerca per tutti gli avvocati di un certo
   * fascicolo. (anche quelle deassegnati).
   * Condizioni per: id_fascicolo, cognome avvocato (like), id_avvocato,
   * flag cancellato = N  
   * 
   * @param aModel
   * @param aFModel
...
throws DAOException
  {
    ...
    lStatement += " and cod_ufficio_appartenenza = '" + StringUtils.convertSqlString(aModel.getCodUffAppartenenza().toUpperCase());
 ...
 protected String getSqlQuery()
 {
  ...
  private String setCondizione(AvvocatoModel aModel,
			AvvocatoFascicoloSiepModel aFModel) {
		...
		if (aFModel != null) {
			if (aFModel.getFasSieIdFascicoloSiep() != null)
				lCondizioni += " AND FAS_SIE_ID_FASCICOLO_SIEP="
						+ aFModel.getFasSieIdFascicoloSiep();
		}
  ...
  private String setCondizionePerInserimento(AvvocatoModel aModel) {
		...
		if (aModel.getIdAvvocato() != null) {
			...
			if (aModel.getForo() != null && !aModel.getForo().equals("")) {
				lCondizioni += " AND FORO = '"
						+ StringUtils.convertSqlString(aModel.getForo().toUpperCase()) + "'";
			}
			
			if (aModel.getCodUffAppartenenza() != null && 
					aModel.getCodUffAppartenenza() != "") 
			{
				lCondizioni += " AND (COD_UFFICIO_APPARTENENZA ='"
						+ StringUtils.convertSqlString(aModel.getCodUffAppartenenza().toUpperCase()) + "' OR COD_UFFICIO_APPARTENENZA ='00000')";
			}
 ...
 public void ricercaAvvocatoPaged(AvvocatoModel aModel, int aPage)
			throws DAOException {
		String lStatement = new String("");
       ...
       lStatement += " ORDER BY COGNOME,NOME ASC";
		...
		lPaginedStatement = "SELECT * FROM (SELECT INNER.* , Rownum rn FROM ("
				+ lStatement + "  ) INNER ) WHERE rn between  "
   ...
   public void getCountAvvocati(AvvocatoModel aModel)  throws DAOException
	{
	 String lStatement ="SELECT COUNT(*) HowManyRecords FROM AVVOCATO WHERE " ;
	        lStatement += " (COD_UFFICIO_APPARTENENZA ='" + StringUtils.convertSqlString(aModel.getCodUffAppartenenza().toUpperCase()) + "'";
